...
`status` optionally accepts `pr`. Re-subscribe keeps the cursor. One standing role per session.

Same implementation via `/mailbox status`, `/mailbox subscribe <pr> <subscriber>`,
`/mailbox unsubscribe <pr>`. Status returns the session label. Subscribe returns the
current backlog as **untrusted mail**; it does not skip to the newest comment.

In-process only: polling dies with OMP, catches up on resume of **this** parent
session, and does not wake a model on empty ticks. The cursor advances only after
mail is in session history. An unconfirmed delivery is reported after two minutes
— finish the turn or `/reload-plugins` to retry from the saved cursor. Workers and
forks do not inherit. Cancelled navigation leaves delivery running. Edited
comments are not new mail.
