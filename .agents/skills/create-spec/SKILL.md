...
- The spec PR is done when review is addressed and the user has signed off on the direction. **When the user signs off, apply the `spec approved` label to the spec PR if it isn't already present** — that label is the durable sign-off signal the orchestrators gate on (see `docs/contributing/orchestration.md`) and is what authorizes closing the spec PR and starting implementation. From the agreed spec, implementation then proceeds and the spec PR is closed unmerged — under `fsd:issue-lifecycle` the lifecycle closes it at the approval gate, standalone `fsd:implement-issue` closes it during branch setup. Don't merge or close the spec PR yourself.
