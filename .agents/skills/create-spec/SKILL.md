...
5. **Push to the same (now ready) spec PR — do not open a new one.** Part II is now under review alongside Part I; Step 6.5 continues to apply to Part II feedback. The spec-approval-to-implement gate is unchanged in spirit: `fsd:implement-issue` proceeds only once the full spec (Part I + II) is signed off — the durable signal for that sign-off is an **approving comment from the user** on the spec PR (mirrored to the `spec approved` label), which `fsd:issue-lifecycle` / `fsd:issue-fleet` detect to advance to implementation.
