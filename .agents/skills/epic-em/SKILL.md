...
| Handing a call to a worker | An EM call ranks **below** the approved rules — the epic's ER-n and the issue's BR-n in `BUSINESS-RULES.md`. You don't read them, so the dispatch does: name the call as yours and tell the worker to check it against those rules first, and to return it with the rule cited rather than apply it on a conflict. Overriding an approved rule is a direction change (amendment PR, human approval), never an EM call |
