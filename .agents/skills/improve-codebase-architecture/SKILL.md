...
- **block** (handler / generator / evaluator / sequencer / router), **pattern**, **capability**, **flow**, **action**, **scope** (request/session/user/org), **item** (message, reasoning, block_output, component, container, status, context, state_change, error, step_error), **store adapter**.
