...
**Measure the total, not the delta.** Every gate above is per-comment. Check each changed file against `issue-spec` 6.5.2's growth trigger; if it trips, re-draft before closing, then repeat 10.4's test/commit/push and the reconciliation above so no reply points at text that no longer exists.
