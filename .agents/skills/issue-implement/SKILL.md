...
3. Open PR with `gh pr create`:
   ...
   - **Body: follow [`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → "The layout"** — canonical for the block order, what collapses, when a diagram earns its place, and what never collapses. Don't restate it here. What's specific to an **implementation** PR is the mapping:
     ...
     - **Block 1 (the problem)** ← the spec's §1 condensed on the spec route, or the reported failure on a bug. **Block 3 (what's asked of you)** ← the Key Decisions & Ramifications compiled in Step 7, **sorted through the two filters** in [`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → §3 rather than listed top-N. A decision earns a place only if a wrong answer changes **what a user experiences**, **what we've promised**, **when something ships**, or **what it costs to undo** — *and* the human would plausibly answer differently than you did. **Three is the ceiling, hardest first**; a restatement of what the change does belongs to block 2, and an obvious call that costs nothing to reverse is one you make and note. Everything that doesn't pass goes one bullet each in the collapsed **engineering calls** block, where the code reviewer wants it — filtered off the product owner's surface, never dropped from the PR. **Format: a subheading per decision with two bullets under it — never a table** ([`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → §3 "The shape"); the subheading states the decision in plain words, and the *If this is wrong* bullet is priced in consequences — who is affected, what we can no longer change cheaply — not in which code would have to move, because the human reading it holds the roadmap, not the file tree ([`asking-for-decisions.md`](../../../docs/contributing/asking-for-decisions.md)). **A decision you genuinely couldn't settle alone** — a behaviour change users could notice, a deviation you're not sure about — gets the full six-part ask instead of two bullets, under a heading naming both options: **one to three sentences per part**, including its own *what being wrong costs*. Zero to two; three is a signal the change went too long without checking in. Where the spec already settled everything and nothing new was decided, **say exactly that in one line** — its absence reads as an omission.
     - **Block 4 (parts worth reviewing closely)** is where the audience changes to the code reviewer — open it with a line saying so, and keep it to 1–3 items and ~200 words, so a product owner knows their job ended at block 3 instead of skimming mechanism for another ask.
