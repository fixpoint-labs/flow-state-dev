...
**Reconcile the prose with the diff — search for the old answer, don't re-read the new one.** Before closing a round that changed behavior, do both. **Grep** the superseded claim's distinctive noun across source-file headers, inline comments, `--help` and error strings, package READMEs, `docs/`, `apps/docs/`, the PR body and the changeset. Then **sweep the summary surfaces by concept** — the locked-contract reference, package READMEs, and any diagram — because a summary restates a claim in its own words, or in an arrow, and a string sweep cannot see either. Report which of the two you did. Converge every hit in one pass, routing the user-facing ones per the next paragraph rather than editing them here. Re-reading the diff cannot find a stale claim in a file the diff never opened, and that is exactly where these survive. The body was written at Step 9; this loop has been changing the code ever since, and a claim written before the last commit is the one that ships wrong.
