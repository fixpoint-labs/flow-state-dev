...
First, **compile the Key Decisions & Ramifications (top 5)** — the most consequential decisions made *during implementation* (not the spec's): a shape the spec left open, a deviation, a tradeoff under a constraint the spec didn't anticipate. For each: the decision, the alternative rejected, and the ramification — what it locks in, what it rules out, what risk it carries. If implementation was purely mechanical with no real decisions, say so rather than padding to five. **This list is never filtered — only the PR body is.** Step 8 (presentation) and the Linear `## Detail` record carry it **whole and verbatim**, because Linear is the durable engineering record and nobody scans it to decide something. **Step 9 sorts the same list for the PR body** — what the user owns goes in block 3, everything else into the collapsed engineering-calls block. So the full record and the capped ask are two views of one list, and neither drops a decision.
