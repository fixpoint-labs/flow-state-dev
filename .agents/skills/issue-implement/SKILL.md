...
- **An epic's closure issue** → discipline = **QA run**, not `tdd`. Implementation is running the approved QA plan on one `main` commit, per [`orchestration.md`](../../../docs/contributing/orchestration.md#the-closure-issue-every-epic-ends-in-qa). A run that fails files each finding as a child of the epic that blocks this issue, opens **no PR**, and returns the row at `NEEDS_IMPLEMENTATION` with the findings listed. Only a clean run opens the PR: the committed end-to-end checks plus the QA report as the goal verdict.
