...
**A check with no before-state still owes a red state — and a blast radius.** Red-before-green perturbs the code you are about to write, so it needs a *before*. Some checks have none: a **negative** assertion (`not.toMatch`, "no other leg failed", "nothing was hired"), a **cardinality** claim ("exactly three", "one row per piece"), a **control or goal-check leg** that grades a run, an **equivalence** between two artifacts. These are green before the change and green after, so the red gate above is satisfiable in full while the check cannot fail at all. For each one, break **the property the check claims** — not the implementation — and report three things: what you broke, which checks went red, and **that nothing else did**. The third is the part that carries the weight: where a fixture is too clean to contain the failure, perturbing the subject fails *nothing*, and only the blast radius makes that visible. This applies wherever a check stands in for an argument — an equivalence, a count, an identity, an absence, a verdict — and a check you cannot write that report for is not evidence yet (BP-003, tenet 7).
