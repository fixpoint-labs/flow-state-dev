...
**Canonical; `issue-implement` 10.6 points here.** Two triggers: a **direction change** (a different approach, a dropped/added deliverable, a reversed decision), or **growth** past ~1.3× the artifact's length when review opened or was last re-drafted (tag a re-draft commit `re-draft:` so `git log` yields that baseline) — cumulative, never round-over-round. Either way, re-read it whole and rewrite the affected sections so it reads as one coherent document; do **not** bolt a "reconciliation / AUTHORITATIVE" section onto the top that contradicts the body — an incoherent spec produces an incoherent implementation (tenet 1). Small clarifications can be inline. On a spec: Part I and Part II, kept in sync with Linear. **On a direction change, record the pivot as one line in the spec's Spec evolution timeline** (`- **After spec review** — <what changed>, because <why>.`) — the timeline carries the why so the debate isn't lost. Neither a growth-only re-draft nor a §13 note earns one; neither changed a decision.
