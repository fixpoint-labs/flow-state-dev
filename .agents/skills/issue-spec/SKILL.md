...
**When the third move won't come, run [`adhd`](../adhd/SKILL.md).** A stall between *build* and *don't build* is usually a candidate-set problem, not a judgement problem: two options are on the table and neither is good, which means the good one wasn't generated. That is this repo's strongest trigger for divergent ideation, and it is at its cheapest right here — no code exists, so adopting a better shape costs nothing. Dispatch it in its **spec context** with the issue, the Step 2/3 findings, and the two options you're stuck between. Then bring the shortlist back to the six verdicts below and rule on it: **a survivor that adds public surface clears the same bar as the original ask** — normalization, observability, composition, vocabulary — or it dies here like anything else. Ideation feeds this step; it does not outrank it.
...
**First ask which kind of question it is**, because the two have different instruments and reaching for the wrong one burns a day. If you can name the candidates and just can't pick between them, build one — that's [`prototype`](../prototype/SKILL.md). If you suspect the candidates on the table aren't the only ones — the shapes all feel like variations of the same idea, or the "obvious" one is obvious because it's the first thing anyone would say — the set is too narrow, and building any member of it won't fix that. Run [`adhd`](../adhd/SKILL.md) in its spec context first, then prototype the survivor. Divergence is cheap relative to a prototype and much cheaper than a wrong spec; running it *after* you've prototyped the wrong shape is the ordering that wastes the day.

Whichever ran, the alternatives weighed and why the chosen one wins belong in **§3 (Tradeoffs & alternatives)** — one line per rejected branch, not the wide set. A fork the user should genuinely rule on goes to **§6** in the six-part shape from [`asking-for-decisions.md`](../../../docs/contributing/asking-for-decisions.md), with your recommendation; a neutral menu of shapes is not a decision request.
...
2. **Open the spec PR ready for review.** Get onto `spec/<ISSUE-ID>` — and **which command depends on whether Step 4 already created it for a POC**, because the create form resets the branch:
   ...
   **Write the PR description per [`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → "The layout"** — canonical for the block order, what collapses, when a diagram earns its place, and what never collapses. Don't restate it here. What's specific to a **spec** PR is the mapping:
   ...
   - **Block 1 (the problem)** ← Part I §1 condensed. **Block 2 (what this does)** ← §2, plus what it deliberately doesn't do. **Block 3 (what's asked of you)** ← §6, **sorted and shaped per [`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → §3** — it owns the filters, the three-decision ceiling, the subheading-and-bullets shape, and the worked before/after; don't re-derive them here. Two things are spec-specific: every §6 Decision the filters drop goes one bullet each in the collapsed **engineering calls** block, so §6 stays complete and nothing is approved unseen; and **every §12 open question is a live fork** and gets the full six-part ask per [`asking-for-decisions.md`](../../../docs/contributing/asking-for-decisions.md), alongside any §6 Decision that is genuinely still open.
   - **Count §6 and §12 together against the ceiling of three** — an open §12 question is a live fork and occupies a slot, so three §6 Decisions plus one §12 question is four and over. **If the combined total exceeds three, the defect is upstream** — §6 collected implementer's calls or restated §2 as decisions, or the direction isn't settled enough to publish. Fix the spec (and mirror to Linear) rather than shipping a longer list; a six-decision ask reads as six things to think about and gets none of them decided.
