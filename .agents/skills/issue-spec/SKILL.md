...
2. **Open the spec PR ready for review.** Get onto `spec/<ISSUE-ID>` — and **which command depends on whether Step 4 already created it for a POC**, because the create form resets the branch:
   ...
   **Write the PR description per [`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → "The layout"** — canonical for the block order, what collapses, when a diagram earns its place, and what never collapses. Don't restate it here. What's specific to a **spec** PR is the mapping:
   ...
   - **Block 1 (the problem)** ← Part I §1 condensed. **Block 2 (what this does)** ← §2, plus what it deliberately doesn't do. **Block 3 (what's asked of you)** ← an index of §6 with **one row per Decision — every one of them**, each carried **in words** rather than by bare number. §6 is the sign-off surface, so a Decision left out of the index is one the reviewer is asked to approve without seeing. Price the *If it's wrong* column in consequences, not mechanism — it is the only column that tells a product owner what signing this off costs them. **Any §6 row that is a genuine live fork, plus every §12 open question, is pulled out below the table and asked in full** (the fork, plain terms, the trade-off, your recommendation, what would change your mind, what being wrong costs — [`asking-for-decisions.md`](../../../docs/contributing/asking-for-decisions.md)); zero to two per spec. A pulled-out fork carries its own calibration: it left the table, so the *If it's wrong* column no longer covers it. The rest of §6 the human is ratifying, and one line each is what keeps eight of them on a screen.
...
- **Open questions are OK — but never neutral.** It's better to flag uncertainty than to make a wrong assumption. Present the fork **with your recommendation** and with what would change your mind: options and trade-offs alone hand the owner an unpriced menu and cost a round while they ask what you think. You did the research; they didn't. See [`asking-for-decisions.md`](../../../docs/contributing/asking-for-decisions.md). And a question you'd answer the same way whatever they said isn't open — decide it and make it a §6 Decision.
