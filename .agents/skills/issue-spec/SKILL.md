...
2. **Open the spec PR ready for review.** Get onto `spec/<ISSUE-ID>` — and **which command depends on whether Step 4 already created it for a POC**, because the create form resets the branch:
   ...
   **Write the PR description per [`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → "The layout"** — canonical for the block order, what collapses, when a diagram earns its place, and what never collapses. Don't restate it here. What's specific to a **spec** PR is the mapping:
   ...
   - **Block 1 (the problem)** ← Part I §1 condensed. **Block 2 (what this does)** ← §2, plus what it deliberately doesn't do. **Block 3 (what's asked of you)** ← §6 **sorted through the two filters** in [`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → §3, not indexed wholesale. A Decision earns a row only if a wrong answer changes **what a user experiences**, **what we've promised**, **when something ships**, or **what it costs to undo** — *and* the owner would plausibly answer differently than you did. **Three rows is the ceiling, hardest first.** Every §6 Decision that doesn't pass goes one line each in the collapsed **engineering calls** block, so §6 stays complete and nothing is approved unseen — it is sorted by audience, never dropped. Carry each row **in words** rather than by bare number, and price the *If it's wrong* column in consequences, not mechanism — it is the only column that tells a product owner what signing this off costs them. **A §6 row that is a genuine live fork, plus every §12 open question, is *also* asked in full below the table** — all six parts, per [`asking-for-decisions.md`](../../../docs/contributing/asking-for-decisions.md); zero to two per spec, **one to three sentences per part**. Its one-line row stays in the table (same rule as [`spec-template.md`](../../../docs/contributing/spec-template.md) → §6); the full ask carries its own *what being wrong costs*, because the row's cell is sized for ratification and is too compressed to calibrate a call the human is actually making.
   - **If §6 yields more than three product rows, the defect is upstream in §6** — it collected implementer's calls or restated §2 as decisions. Fix §6 (and mirror to Linear) rather than shipping a longer table; a six-row ask reads as six things to think about and gets none of them decided.
   ...
   - **Second collapsed block: the engineering calls** — the §6 Decisions that didn't pass the filters, one line each with what a wrong one costs, in the engineer's register.
   - **Third collapsed block: the rest of Part I** — §3 tradeoffs, §4 focus practices, §5 examples, §6 in full. Blocks 1–3 already carry §1, §2 and the filtered §6 rows, so it picks up where they stop and nothing is said twice.
