...
2. **Open the spec PR ready for review.** Get onto `spec/<ISSUE-ID>` — and **which command depends on whether Step 4 already created it for a POC**, because the create form resets the branch:
   ...
   **Write the PR description per [`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → "The layout"** — canonical for the block order, what collapses, when a diagram earns its place, and what never collapses. Don't restate it here. What's specific to a **spec** PR is the mapping:
   ...
   - **Block 1 (the problem)** ← Part I §1 condensed. **Block 2 (what this does)** ← §2, plus what it deliberately doesn't do. **Block 3 (what's asked of you)** ← §6 **sorted through the two filters** in [`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → §3, not indexed wholesale. A Decision earns a place only if a wrong answer changes **what a user experiences**, **what we've promised**, **when something ships**, or **what it costs to undo** — *and* the owner would plausibly answer differently than you did. **Three is the ceiling, hardest first.** Every §6 Decision that doesn't pass goes one bullet each in the collapsed **engineering calls** block, so §6 stays complete and nothing is approved unseen — it is sorted by audience, never dropped. **Format: a subheading per decision with two bullets under it — never a table** ([`pr-reviewer-guidance.md`](../../../docs/contributing/pr-reviewer-guidance.md) → §3 "The shape"). The subheading states the decision in plain words, never a bare "Decision 3"; the *If this is wrong* bullet is priced in consequences, not mechanism — it is the only line that tells a product owner what signing this off costs them. **A §6 Decision that is a genuine live fork, plus every §12 open question, gets the full six-part ask instead of two bullets** — per [`asking-for-decisions.md`](../../../docs/contributing/asking-for-decisions.md); zero to two per spec, **one to three sentences per part**, under a heading that names both options. It stays in block 3 rather than being dropped (same rule as [`spec-template.md`](../../../docs/contributing/spec-template.md) → §6), and carries its own *what being wrong costs*, because a ratified decision's one-line cost is too compressed to calibrate a call the human is actually making.
   - **If §6 yields more than three product decisions, the defect is upstream in §6** — it collected implementer's calls or restated §2 as decisions. Fix §6 (and mirror to Linear) rather than shipping a longer list; a six-decision ask reads as six things to think about and gets none of them decided.
