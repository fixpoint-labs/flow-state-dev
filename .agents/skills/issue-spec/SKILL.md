...
**Canonical for this step and `issue-implement` 10.6.** Two triggers: a **direction change** (a different approach, a dropped/added deliverable, a reversed decision), or **growth** past ~1.3× the artifact's length when review opened or was last re-drafted — cumulative, never round-over-round. Either way, re-read it whole and rewrite the affected sections so it reads as one coherent document; do **not** bolt a "reconciliation / AUTHORITATIVE" section onto the top that contradicts the body — an incoherent spec produces an incoherent implementation (tenet 1). Small clarifications can be inline. On a spec that means Part I and Part II, kept in sync with the Linear document. **On a direction change, record the pivot as one line in the spec's Spec evolution timeline** (`- **After spec review** — <what changed>, because <why>.`) — the body stays coherent, the timeline carries the why so the debate isn't lost. A growth-only re-draft earns no entry, nor does a §13 note; neither changed a decision.
