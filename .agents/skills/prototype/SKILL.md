...
2. **One command to run.** Whatever the host app's existing task runner supports:
   - Logic prototype: `pnpm --filter @flow-state-dev/fsdev fsdev run <prototype-flow-kind> <action> -i '<json>' --flow-dir apps/kitchen-sink/flows/_prototypes` — wire a one-line helper script in the host app's `package.json` if the invocation is awkward.
...
3. **No persistence by default.** Use the in-memory store. Seed session state via `--seed-session` if the question depends on starting state. Reach for `@flow-state-dev/store-sqlite` only when the question is specifically about persistence semantics — and label the database file `PROTOTYPE-wipe-me.sqlite`.
