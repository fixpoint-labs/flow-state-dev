...
- The inference credential is normally **already in the environment** (e.g. `AI_GATEWAY_API_KEY`, or a provider key the model resolver uses) — check for it and run, rather than assuming it's absent. Only if a real generation attempt actually fails for lack of a working credential (a genuine 401 on *inference*, not a preemptive guess) is the goal blocked — record that explicitly and surface it; never silently skip it on cost grounds.
