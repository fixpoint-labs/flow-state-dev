...
- **Create** (first dispatch): first stand up the **Linear Epic issue** — create it, tag it
  ...
  **Write the epic PR description to the template** —
  ...
  URL doesn't render. After you write, follow
  [`spec-figures.md`](../../docs/contributing/spec-figures.md) → "In the PR body" for the stored-body
  check. When that section says to hand a person the clean line, return those lines in `pins:`.
  Never rewrite a body a person has pasted an image into — return the new pins instead.
