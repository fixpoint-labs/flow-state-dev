...
- **Create** (first dispatch): first stand up the **Linear Epic issue** — create it, tag it
  ...
  issues that have no conflicting parent. Then write the set — `SPEC.md` (the goal and how we'll know it's met, the teams table, why
  ...
  **Write the epic PR description to the template** —
  ...
  the rules. The goal, its *how we'll know* fence and its check line; the teams table; the box figure, pinned to the commit that holds it; why now; the
