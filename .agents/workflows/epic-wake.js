  ...
  plan.foldEpic
    ? agent(
        ...
        (epicAtBudget || !newEpicReviewEvents
          ...
          `Refresh the epic-spec's §4 scoreboard AND running index: the index from the PR handles already recorded, and the Goal check column from each goal's verdict log in goals/<describe>/<it>/goal.md (never from PR narration). Then count the scoreboard's Lead line off that column, excluding n/a rows from the denominator. The scoreboard has no cut tally — that stays in the coordinator's report. Never re-review to satisfy a bot.\n` +
