  ...
  plan.foldEpic
    ? agent(
        ...
        (epicAtBudget || !newEpicReviewEvents
          ...
          `Refresh the epic-spec's §4 scoreboard AND running index from the PR handles already recorded — the scoreboard's Lead line is counted off the per-issue Goal check column, so refresh the column first and the count from it. Never re-review to satisfy a bot.\n` +
