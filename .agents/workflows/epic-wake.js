  ...
  () =>
    agent(
      `Scan epic PR #${epic.prNumber} in this repo for its objective sign-off. Report approved:true ONLY for a human approving comment, or a review whose LATEST state is APPROVED on the CURRENT head by a human who is not the PR author. Exclude bots (Bugbot, Codex, Copilot) and any historical approval invalidated by a later push or CHANGES_REQUESTED.\n` +
        `SEPARATELY, report approvedByLabel:true whenever the PR currently carries the \`epic approved\` LABEL **and a HUMAN applied it**. Two independent checks, and conflating them is the bug this wording exists to prevent:\n` +
        `  · WHO — verify provenance. Read the PR's timeline for \`labeled\` events naming \`epic approved\`, take the MOST RECENT one, and require its actor to be a human (not a bot, not \`github-actions\`, not any \`[bot]\` account). This label is the OWNER'S authorization channel, so accepting it from any actor lets a bot or a passing collaborator release every child issue without the sign-off the gate exists to require. If the timeline is unreadable or no \`labeled\` event can be found for a label that is present, report FALSE — an approval you cannot attribute is not an approval.\n` +
        `  · WHEN — do NOT check. Once the actor is a human, presence is the whole test: do not compare the label against the head commit and do not treat a later push as invalidating it. An epic-spec PR takes commits continuously as feedback is folded, so a staleness rule would revoke the approval on the next edit and hold the entire set. A label is standing state the owner can REMOVE; removal is the revocation.\n` +
    ...
    (row) => () =>
      agent(
        `Derive the current lifecycle phase of Linear issue ${row.id} from durable state only. Spec PR: ${row.specPr || 'none'}. Impl PR: ${row.implPr || 'none'}.\n` +
          ...
          `SEPARATELY report specApprovedByLabel:true whenever the spec PR currently carries the \`spec approved\` LABEL **and a HUMAN applied it**. Same two independent checks as the epic gate. WHO: read the PR timeline for \`labeled\` events naming \`spec approved\`, take the MOST RECENT one, and require its actor to be a human — never a bot or any \`[bot]\` account. This label is the owner's authorization channel, so a label from any other actor would release implementation without the sign-off the gate requires; if the timeline is unreadable or carries no \`labeled\` event for a present label, report FALSE. WHEN: do not check — once the actor is human, presence alone passes: do not compare the label against the head commit and do not treat a later push as invalidating it, since a spec PR takes commits while review is folded and expiring the label would revoke the approval on the next round. Removal is the revocation. Report it independently of specApproved.\n` +
