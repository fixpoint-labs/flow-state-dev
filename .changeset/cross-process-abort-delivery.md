...
Work dispatched by those completion hooks is now waited for too. Because `onCompleted` runs after the background-work barrier, a hook that fans out its own `.work()` tasks was queueing them into a wait that had already finished — the request returned while a notification or state write was still going, and those items could miss the final flush.
