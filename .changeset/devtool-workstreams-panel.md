...
The Tasks tab also shows a task's current state rather than an older one. A task that changed across more than one request was displayed from the earliest of them, so its status, assignee and labels could all be out of date — visible since the tab was added, and now also what workstream links are matched against.
