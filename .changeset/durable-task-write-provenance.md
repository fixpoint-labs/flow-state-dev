...
The `Task` schema gains four optional fields — `revision`, `writeLog`, `writeLogTruncated`, `incarnationId` — maintained by the collection, not by callers. Existing tasks carry none of them and keep working: an absent record reads as "cannot tell", never as "your write did not land". The same holds for a `TaskCollectionRef` you wrote by hand, which needs no migration. If you do maintain the fields in a custom ref, advance the revision on every write that changes a task; a write that skips it makes a later answer wrong rather than merely absent.

`incarnationId` closes a gap in the delete/recreate case: a task removed (explicit delete, or capacity eviction on the resource-backed collection) and recreated under the same id resets `revision` to 1, so a stale token whose baseline happened to match could otherwise read as "nothing committed" for a write that landed on the row that no longer exists. A millisecond timestamp isn't a safe guard against this — a delete-then-recreate under the same id lands in the same millisecond often enough that two different rows share it — so `incarnationId` is a fresh id minted once per row instead, and `didWriteLand` withholds rather than guessing whenever the token and the task disagree on it, in either direction.
