...
Review also surfaced that the wake cannot perform the corrective commit itself — at budget it routes converged epic-PR feedback through a read-only scout to the sub-issues each item names, and an item concerning no sub-issue is dropped with the cursor. Epic-level corrections are the coordinator's to make; the docs now say so rather than implying the machinery handles it.
