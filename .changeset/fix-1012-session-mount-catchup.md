...
The list carries the most recent entries, newest first — 100 by default, raised with `workstreams: { limit }`. It is all-time history rather than only what is running now, so it grows with everything the conversation has ever started. Past the limit the oldest finished work falls off the end and is not reachable from the hook.

The server enforces its own maximum on that value and rejects a larger one, defaulting to 100 and raised with `maxWorkstreamListLimit`. Both halves are deliberate: the list is re-read on every interaction, so a bigger page costs more on every turn, and the deployment paying that cost chooses it.
