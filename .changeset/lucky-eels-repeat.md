...
A write whose parsed result does not parse back to itself is now refused with a `ValidationError` naming the resource and, where there is one, the field that moved (FIX-1260). Rows written before this change are read normally and converge on their next successful write.
...
- **`collection.create()` reports schema failures in the same words as every other write.** The message changed from `Namespace "<pattern>" create("<key>") state validation failed…` to `Resource "<storage-key>" write failed stateSchema validation at "<field>": …`. The `create` / `create(replace)` distinction is no longer in the text; the storage key identifies the instance.
