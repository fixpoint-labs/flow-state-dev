...
Remove the deprecated flat emitters `ctx.emitMessage`, `ctx.emitComponent`, and `ctx.emitStatus` (FIX-506). Use the namespaced `ctx.emit.message` / `ctx.emit.component` / `ctx.emit.status` API instead — the call signatures are identical, so migrating is a mechanical rename (`ctx.emitMessage(...)` becomes `ctx.emit.message(...)`).
