...
Rename `@flow-state-dev/server` to `@flow-state-dev/engine` (FIX-684). The package is the execution runtime (stores, SSE streaming, HTTP routes); the real HTTP hosts are `@flow-state-dev/node`, `@flow-state-dev/next`, and `@flow-state-dev/vercel`, so the name now reads true.
