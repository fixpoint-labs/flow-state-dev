...
The run is confined to its workspace by default (`contain`, on unless you turn it off). Two settings answering different halves: `settingSources: []` stops the run reading its configuration out of a directory whose contents your users write, and the SDK's sandbox settings stop it writing outside the directory. Setting either option explicitly wins over the default.
