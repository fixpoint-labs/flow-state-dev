...
# Doc surfaces that satisfy BP-009 ("document user-facing changes in the same change set").
DOCS=$(printf '%s\n' "$CHANGED" | grep -E '^(apps/docs/|packages/[^/]+/README\.md$|changelog\.md$)' || true)

if [ -n "$SRC" ] && [ -z "$DOCS" ]; then
  REASON="BP-009 docs check: source files changed without matching doc updates.

Changed src files:
$SRC

No edits to apps/docs/**, packages/*/README.md, or changelog.md in this working tree.

Before declaring this task done, decide:
- If the change is user-facing (new/changed public API, capability, block, CLI command, hook, env var, config key, or behavior end users observe) → update the relevant packages/*/README.md and any apps/docs/** pages users reference.
- If purely internal (refactor, bug fix without API change, test-only, internal helper) → confirm that explicitly in your reply, no docs needed.
