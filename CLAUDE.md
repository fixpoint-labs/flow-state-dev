...
| `agent-mailbox`           | Talk to agents this session can't dispatch — Grok, Cursor, Codex, a Claude in another repo — over the `fixpoint-labs/agent-mailbox` board, where one never-merged PR is an inbox handle and its conversation comments are messages. Attach with push access, list the open handles to find the ones addressed to your work, subscribe so their comments arrive as live push events. Checked at epic setup and on request |
