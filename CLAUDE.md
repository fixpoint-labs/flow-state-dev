...
- **BP-003** Verification evidence is mandatory — every deliverable has an evidence path + pass criteria, and so does every claim the change rests on (scope counts, equivalence, runtime behaviour): execute or parse it, don't read it. **A report is a claim too** — "I fixed it", a status line, a summary of someone else's work — so re-derive it from the artifact, not from the list you worked off. The command itself is subject to tenet 7, in three shapes: aimed at a *neighbour* of the claim; **never ran the check** (fallback path, skipped suite, piped exit code, a per-item reporter silent about an item it never got); or its **stated scope wider than its real one** (derive the scope, don't list it, and write down what it cannot see). If you cannot say what would make this check fail, you have not verified anything: produce the red state where one exists, and where none does (parity work) show the check reaches the code it covers.
...
- **Process & docs** (`docs/contributing/best-practices/process.md`): BP-002 spec-driven execution (each change maps to a tracked unit of intent — a spec for a feature, the issue itself for a bug; a surface it decided survives a build constraint — move the mechanism, or raise a conflict to re-gate, never a fork to offer) · BP-004 public boundary first · BP-006 keep planning/tracking labels out of code & tests · BP-008 root README onboarding-first · BP-009 package READMEs current · BP-037 specs live on their spec PR (`spec/<ISSUE-ID>/`, four documents and figures) and in Linear, never on `main` (CI-enforced) · BP-039 lead with the problem in plain language and fold the derivation below it (grok before diving deep) · BP-040 spec review is a direction check — fold only what changes the approach, note the rest for the implementer, converge in two rounds · BP-041 frame every ask as a business decision.
