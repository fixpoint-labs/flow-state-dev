...
| `epic-explainer`          | Build or refresh an epic's **explainer** — four diagrams (today · after · the set · the path) and a hard ≤400-word prose budget, as mermaid markdown on the epic branch so GitHub renders it in one click. The comprehension artifact, not a second epic-spec: it decides nothing and asks nothing. Refreshed before each gate and once at wrap; `epic-em`/`epic-pm` turn it on by default |
