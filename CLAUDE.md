...
| `em-mode`                 | Run an epic with an **engineering-manager posture** — you own the engineering decisions, the user is the product manager who owns the business ones. Composes `epic-lifecycle` unchanged and carries only the delta: an escalated engineering fork is decided and recorded rather than handed up (only a critical product or architectural call goes to the user), the PR-feedback cap is answered in-session, and status is reported as outcomes. **Moves no gate** — epic objective, spec approval, and merge stay the user's |
