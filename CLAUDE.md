...
| `spec-explainer`          | Build or refresh a spec's **explainer** — the diagram-first companion every spec carries, committed beside it so GitHub renders it in one click. Three panels at issue altitude (today · after · where the decision fell), four at epic altitude (today · after · the set · the path), under a hard ≤400-word prose budget. Mermaid by default, with one hand-authored SVG as the escape hatch when the layout itself carries the meaning. The comprehension artifact, not a second spec: **it decides nothing and asks nothing.** Written by `issue-spec` for every issue spec; opt-in per epic and refreshed at each gate and at wrap, with `epic-em`/`epic-pm` defaulting it on |
