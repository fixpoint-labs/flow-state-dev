...
- **BP-030** Tolerate the old shape when you change a persisted/in-flight field — dual-read legacy records (including old keys on rename in the same commit); reject removed keys loudly; `== null`-guard new nullable fields.
- **BP-041** Don't overload vocabulary on adjacent unrelated fields — distinct terms for grouping ids vs body text (`conversationId` + `situation`, not `context` + `contextId`).
