...
- **BP-003** Verification evidence is mandatory — every deliverable has an evidence path + pass criteria that discriminate the load-bearing behavior (not just "it ran").
...
- **BP-028** Fix the bug at the layer that owns it, not where it bit you — ambient-context validation at the owning server seam, not duplicated in isomorphic parsers.
...
- **BP-035** Walk the second-path checklist before declaring a change done — legacy / null-boundary / concurrent-409 / cancel-error / multi-tenant / cost-observability / React-derived-state paths; sibling formats on shared seams; pure-tested rollout gates; test the off/new state of any new flag.
...
- **BP-040** Prefer honest unknowns over silent drops in allocation math — surface unmatched/unknown amounts; never drop proceeds or quantities silently.
- **BP-041** Centralize safety-critical predicates on shared seams — one helper beside the contract, not N inlined copies.
