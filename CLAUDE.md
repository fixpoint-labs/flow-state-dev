...
- **BP-003** Verification evidence is mandatory — every deliverable has an evidence path + pass criteria, and so does every claim the change rests on (scope counts, equivalence, runtime behaviour): execute or parse it, don't read it. The command itself is subject to tenet 7 — watch for a green result from a check aimed at a neighbour of the claim. If you cannot say what would make this check fail, you have not verified anything: produce the red state where one exists, and where none does (parity work) show the check reaches the code it covers.
