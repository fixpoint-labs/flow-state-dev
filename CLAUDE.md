...
- **Blocks & composition** (`best-practices/blocks.md`): BP-011 handlers don't call blocks (compose as a sequencer) · BP-012 `.tap()` for state-mutation-only blocks · BP-013 `connectInput`/`connectOutput` inside the router · BP-014 handlers never return input · BP-024 helpers when the body varies, factories when only identity does · BP-025 declare/validate sequencer output schemas deliberately. Also: prefer conditional step variants (`.workIf`/`.tapIf`/`.stepIf` with an inline connector) over wrapper sequencers.
- **Generators & prompts** (`best-practices/generators.md`): BP-016 outputSchemas OpenAI strict-compatible · BP-017 typed `context` slot for prompts · BP-018 shared prompt formatters in `lib/` · BP-032 secondary model/tool call inherits the primary call's contract (signal, usage, input).
- **Resources & state** (`best-practices/resources.md`): BP-015 `expose`/`exclude` over `data` projections · BP-019 resource refs in leaf modules · BP-020 live mode never falls back to fixtures · BP-021 `cacheable` declared deliberately · BP-023 state schemas `.nullable().default(null)` · BP-027 user-scoped resources default to shared · BP-033 filter before you load on lazy collections.
- **React** (`best-practices/react.md`): BP-010 `useMemo` over `useEffect`; derive flags from the complete input set; signal only on real change.
- **Engine & transport** (`best-practices/engine.md`): BP-026 bundle forwarded options into `RuntimeConfig` · BP-031 key auth/routing off the trusted transport `source`, not caller `metadata`.
- **Process & docs** (`best-practices/process.md`): BP-002 wave-driven execution · BP-004 public boundary first · BP-006 wave labels out of code · BP-008 root README onboarding-first · BP-009 package READMEs current.
