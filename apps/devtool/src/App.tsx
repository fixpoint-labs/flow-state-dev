...
export function App() {
  ...
  const onStartResize = (panel: "nav" | "detail", startClientX: number) => {
    ...
    const onMove = (event: MouseEvent) => {
      if (panel === "nav") {
        const next = Math.min(320, Math.max(NAV_COLLAPSED_WIDTH, startNav + (event.clientX - startClientX)));
        setNavWidth(next);
        return;
      }
        ...
        <main className="flex min-w-0 flex-1 flex-col bg-slate-950">
          <div className="p-3">
            <Tabs defaultValue="stream">
              ...
              <TabsContent value="stream" className="sr-only" />
              <TabsContent value="trace" className="sr-only" />
            </Tabs>
          </div>
