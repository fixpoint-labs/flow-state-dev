...
| Filesystem | `@flow-state-dev/engine` (`filesystemStores({ rootDir })`) | Persists to a nested file tree (resource content as `.md`, resource state as `.json`). Survives restarts, not suitable for multi-instance deployments |
