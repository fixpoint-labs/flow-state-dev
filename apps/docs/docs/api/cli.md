...
The first argument is the flow **instance** to run: its `kind` for an ordinary flow, as above, or the copy's own id for a flow declared `cardinality: "collection"` (`fsdev run review-east run`). A bare collection kind is not found, and the error lists the ids that are. Running a session through a copy that doesn't own it exits non-zero and leaves the session untouched. See [Flows](/docs/fundamentals/flows#how-an-instance-is-addressed).
