...
- `stampOrgId?: boolean` — when `true`, creating an instance whose initial state has no `orgId` writes the organization the run belongs to into it. Creating an instance with an `orgId` other than the run's organization throws. An update whose new state has no `orgId` (`setState`, `updateState`, or `create(..., { replace: true })` over an existing instance) keeps the stored one, and never takes the updating run's organization. The `stateSchema` must declare `orgId`, or `defineResourceCollection` throws. Default `false`
