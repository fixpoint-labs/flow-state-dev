...
- `stampOrgId?: boolean` — when `true`, creating an instance whose initial state has no `orgId` writes the organization the run belongs to into it. Creating an instance with an `orgId` other than the run's organization throws. On an update (`setState`, `updateState`, `patchState`, `upsert`, or `create(..., { replace: true })` over an existing instance), the stored `orgId` stands: leaving it out keeps it, and naming a different one throws without writing. An instance with no stored `orgId` keeps none unless the update names the run's own organization. The `stateSchema` must declare `orgId`, or `defineResourceCollection` throws. Default `false`
