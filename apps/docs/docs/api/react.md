...
const session = useSession(sessionId, {
  ...
  childSessions: { limit: 50 },                  // page size for the child-session list; server default when omitted
});
...
session.childSessionsStale; // boolean — the last re-read failed; rows are kept
