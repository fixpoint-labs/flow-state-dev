...
| Key | Does |
|---|---|
| `j` / `k` or `↓` / `↑` | Move the selected row (click a row). While the slash list is open, `↑`/`↓` choose a verb or board id. While composing, `↑`/`↓` move between compose lines; on the first or last line they walk prior submitted lines |
