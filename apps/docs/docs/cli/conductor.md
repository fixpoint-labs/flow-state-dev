...
On a board with at least one row, when the selected row has no open question, the empty compose line (`❯`) says `talk to the coordinator, or /seed /wake /answer`. The footer includes `s seed`. `s` opens seed compose. `r` refreshes. Typing that is not a slash verb is a talk turn (`steer`). Other letters talk. On a spent row that line is `talk to the coordinator — this row is spent`. On a row with an open question, that empty line says `type to answer`. Typing starts an answer. Letters are the answer, not board keys, including `f`, `h`, `e`, and `H`. `?` opens help. `/` starts a slash command, including `/steer` to talk. `Ctrl-T` expands the todo list. The footer says `type to answer`. `Enter` sends. `Esc` cancels. Slash verbs run the named action either way.
