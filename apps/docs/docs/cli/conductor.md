...
On a board with at least one row, the empty compose line (`❯`) says `talk · enter inspects a row · /seed /wake /answer`. The footer is `type to talk · enter inspect`. The footer includes `s seed` when the selected row is idle. `s` opens seed compose. `r` refreshes. Typing that is not a slash verb is a talk turn (`steer`). Other letters talk, including when the selected row is waiting. On a spent row that line is `talk to the coordinator — this row is spent`. On a waiting row the empty line says `talk · enter inspects the question · /answer`. The footer adds `/answer`. `?` opens help. `/` starts a slash command, including `/steer` to talk. Slash verbs run the named action either way.
