...
After `/quit`, reopening the board recalls prior compose lines for that `--session` and that epic. Talk (`steer`) and answer walk earlier sends, including talk lines. The seed prompt walks sends whose first line is an issue id. Empty lines and slash commands other than talk are not recalled. Without a loaded config, the board does not remember compose lines after `/quit`.
...
- The seed prompt does not recall talk lines. `↑ prior` on seed shows only when there is a seed send to walk.
