...
- [fsdev API](/docs/api/cli) — the full flag reference for `fsdev run`, `fsdev dev`, and `fsdev serve`. `fsdev serve` also reads this config; it supports `--config` but not `--no-config`.
