...
Turns run as whoever your app's resolver says, asked once per turn for that turn's flow. With no resolver, that is `cli-user`, while the DevTool defaults to `devuser`. To see a chat session in the DevTool's session list, or resume one across the two, align them with `--user devuser`. If a flow's resolver refuses the CLI, that turn fails and the chat keeps going. Pass `--org` to get past it. `/status` shows the organization the current flow's last turn ran in. See [Who a run is](./configuration.md#who-a-run-is) for how the resolver is asked and what `--org` and `--user` change.
