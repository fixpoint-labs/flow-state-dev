...
- **Running a background task board** — `fsdev conductor` opens a live board over a flow that queues work. Talk to the coordinator, inspect a row to read a question and type the reply, or script it with headless verbs from CI. See [Conductor](./conductor.md).
