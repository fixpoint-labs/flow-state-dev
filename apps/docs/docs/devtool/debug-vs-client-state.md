...
Omit `writable` and the resource can be written, so a resource that declares nothing carries no mark. A [projected collection](../resources/projected-collections.md#read-only-by-design) is the exception: it has no `writable` field to set, and the panel marks it read-only anyway.
