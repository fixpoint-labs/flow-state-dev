...
A read that fails is never shown as an empty section. A refused read names its status, 403, and a collection the server doesn't know names 404. If a read fails partway, the section says how many pages were read before it stopped and shows none of the rows.
