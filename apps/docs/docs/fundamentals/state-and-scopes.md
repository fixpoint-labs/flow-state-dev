...
`expose` and `derived` share a namespace. A name in both throws at `defineFlow`. `expose` names must be top-level keys on that scope's `stateSchema`. The scope key is `client`; a `clientData` key throws at `defineFlow`.
