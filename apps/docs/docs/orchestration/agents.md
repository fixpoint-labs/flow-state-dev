...
A string key needs a catalog to resolve against. If you declare one and supply no catalog at all, materialization throws a `FlowError` (`code: "agent_capability_unresolved"`) naming the agent and the capability, rather than building an agent that quietly lacks it. A key that a supplied catalog doesn't carry is treated the same way as an unknown tool key: it warns and is skipped. Capability references carry themselves and never need a catalog.
