...
| `boardId` | `string` | omitted | Stable id. Required when any worker uses detached dispatch or any seat holds a `dispatcher({ type: "task" })`. Renaming orphans live work keyed on the old value. |
