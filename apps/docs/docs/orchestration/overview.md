...
A drain normally runs inside the request that mounted it, so every worker's task finishes before that request does. The exception is a worker declared to run in a *dispatch run*, a session of its own, rather than inline. The board claims a task for it as usual, hands the task to that run, and moves on. The drain can finish while the task is still running, and the run records the result when the worker is done. See [Seats that hand off](./task-board#seats-that-hand-off) for what such a board requires, and [Work that outlives the turn](/guides/background-work) for that path beside the other background-work surfaces.
...
- [Channels](../workforce/channels) — several agents on one topic, with one durable transcript and nobody owning a row.
