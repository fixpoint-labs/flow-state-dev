...
When the dispatch arrives, the run re-reads the row and runs the worker only if the claim is still current: same attempt, same row, still `in_progress`, still routed to this seat. Otherwise it throws `StaleTaskClaimError` (`code: "stale-task-claim"`) and writes nothing, so the row is left exactly as it was. If that row is still `in_progress` (a newer attempt holds it, or it is now routed to another seat), it stays that way until its lease runs out and the next drain reclaims it. A row that was deleted (or deleted and recreated under the same id), already settled, or parked has no outstanding claim, so there is nothing to reclaim.
