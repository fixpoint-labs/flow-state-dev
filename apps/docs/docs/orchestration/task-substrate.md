...
All three backings agree on freshness *within* a request: two resolutions of the same collection read the same tasks, so a task added through one is visible through the other right away. The `resource` backing is the only one where the question extends past that, and there the answer is no. A request loads durable state into memory when it starts, so a write from another request does not reach one already running, and resolving the collection again inside it changes nothing. The next request to run reads what was stored, which is the case durable collections exist for.
