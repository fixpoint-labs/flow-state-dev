...
- **Delegation.** A skill that declares an `agents:` field gets a private task board plus `taskTools` and a `runBoard` tool. The generator plans the work as tasks (`addTask` with an `assignee`) and drains the board with `runBoard`; the board runs the agents and tools it declared. See [Delegation](../skills/delegation).
