...
Every session and request records the flow instance that created it, as `flowId`, next to the definition it came from, as `flowKind`. For an ordinary flow the two are the same string. For a flow that runs as several named copies (see [Flows](/docs/fundamentals/flows#flowtype-vs-flowinstance)) they differ: a session started through `review-east` has `flowKind: "review"` and `flowId: "review-east"`. The owner is what the server checks before it runs, resumes, retries or reads anything under the record, so an address naming the wrong copy is refused before any effect (see [Server setup](/docs/server/setup#keeping-a-session-with-its-owner)). The listing filter is exact: `flowId: "review-east"` returns that copy's sessions and never a record with no owner recorded.
