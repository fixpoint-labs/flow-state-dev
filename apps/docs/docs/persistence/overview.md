...
**Records with no organization.** A session or request can carry no `orgId` at all. As with a record that has no owner, a missing field says nothing about which organization it belongs to, so the server does not guess:

- On an addressed route, a resume, a retry or an execution, an **authenticated** caller gets `409 { "error": "migration-required" }`.
- A caller your resolver could not identify gets the ordinary `401`, exactly as they would for a record that does carry an `orgId`.
- Ordinary listings omit the record rather than failing the whole listing for one row, whether or not the caller authenticated.
...
If your whole history was written by one development deployment, map it to `DEFAULT_ORG_ID`. Make that call from your own records of what ran where.
...
1. **Quiesce.** Stop the writers, and this time stop the schedulers and **drain the external queues** too. A job already enqueued carries the identity it was written with, and a job carrying no organization is refused when its worker picks it up. Let the queues empty rather than leaving work that cannot run. A job you cannot attribute stays stopped, or you retire it explicitly. Back up, and work on the copy.
