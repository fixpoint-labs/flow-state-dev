...
This is separate from the scope-level `client` config, which computes derived values from scope state and delivers them to the frontend as a flat `clientData` projection. Resource client access gives the frontend direct, lazy-loaded access to resource content and metadata through dedicated endpoints and React hooks.
