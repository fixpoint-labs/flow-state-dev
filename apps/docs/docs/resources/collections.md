...
The snapshot then includes `prefetched: [{ topic, clientData }, ...]` for the first 20 items. Consumers render immediately without an extra round-trip; the convenience hook surfaces them as the initial paint. (The DevTool lists a collection's items without applying `prefetchWindow`. See [Debug vs client state](../devtool/debug-vs-client-state.md).)
