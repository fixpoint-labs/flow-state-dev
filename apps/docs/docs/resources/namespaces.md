...
Static resources have a fixed name you declare up front: `plan`, `artifacts`, `preferences`. Resource namespaces handle the case where you don't know how many instances you'll need. An AI managing a collection of files, accumulating observations per topic, or creating workspaces on the fly — these are namespace problems.

A namespace defines a shared schema and a key pattern. Instances are created and destroyed at runtime.
...
The pattern string determines which keys a namespace can hold.
...
| `[topic]/observations` | `react/observations`, `rust/observations` | Parameterized segment. The bracketed portion becomes a key parameter. |

`**` must be the last segment. Parameterized segments use `[name]` syntax.

## Runtime API

Namespace entries on scope resource registries (`ctx.session.resources`, `ctx.user.resources`, `ctx.project.resources`) are `ResourceNamespaceRef` instances.
...
When `maxInstances` is set, the namespace enforces a cap on live instances:
...
Set `maxInstances` for any namespace that could grow without limit. An AI creating files in a loop with no cap will cause memory and storage pressure. `"lru"` is the safest default — it keeps the working set and discards stale entries.

## Lifecycle hooks

Namespaces support per-instance hooks for logging, side effects, or cleanup:
...
Sequencers collect namespace declarations from child blocks. `defineFlow` merges them into scope configs. Two blocks declaring different namespace refs for the same name will throw at build time. Same ref instance, no conflict.

## Storage model

Namespace instances are stored in the same flat `resources` map as static resources. A namespace with pattern `files/**` stores instances under keys like `files/readme.md` and `files/src/utils.ts`. No schema changes to scope records are needed.

## When to use namespaces vs static resources

Static resources work when you know the names ahead of time. Namespaces work when you don't.
...
If the collection is bounded and predictable (three artifact slots), a static resource with an array or record in its state is simpler. Namespaces add value when the collection is dynamic and potentially large.
