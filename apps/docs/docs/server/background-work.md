...
| `flow-not-found` | A `flowKind` names a flow this server has not registered, or a [hired seat](../workforce/durable-hire.md) the sending session may not open: one pinned to another organization, or another user's own seat. Both cases read the same, so the refusal does not reveal that the seat exists |
