...
| `agents` | no | map | Board participant declarations that turn on delegation: an inline agent (`prompt`/`prompt-ref`), a registered one (`agent-ref`), or a catalog tool run without a model turn (`tool`). See [Delegation](./delegation) and [Deterministic participants](./delegation#deterministic-participants-tools). |
