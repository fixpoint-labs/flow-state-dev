...
The emitters live under the `ctx.emit` namespace: `ctx.emit.message`, `ctx.emit.component`, `ctx.emit.status`. The old flat forms (`ctx.emitMessage`, `ctx.emitComponent`, `ctx.emitStatus`) have been removed. Update old snippets by moving the suffix into the namespace: `ctx.emitMessage(...)` becomes `ctx.emit.message(...)`.
:::
