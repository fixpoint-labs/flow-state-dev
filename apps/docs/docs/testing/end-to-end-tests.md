...
- The other generators on the run path (`skillClassifierMock`, `autoTitleMock`) use the framework's `mockGenerator()` with `{ when, then }` predicate entries because they need a single fixed response per call.
