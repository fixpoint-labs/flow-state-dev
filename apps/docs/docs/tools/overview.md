...
- [Cursor SDK agent](/docs/tools/cursor) — run Cursor's coding agent the same way, returning the same handle (separate `@flow-state-dev/cursor` package, exact-pinned `@cursor/sdk` peer)
