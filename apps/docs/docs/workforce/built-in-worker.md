...
- **A control on a capability preset**, when the worker selects that preset in its [`capabilities:`](./capabilities-on-disk.md#a-preset-carrying-a-tool) key. Catalog tools in the same preset are still held back; the control is not.
