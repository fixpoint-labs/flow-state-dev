...
Each file default-exports one thing: a flow for the two `flows/` folders, a `BlockDefinition` for a `blocks/` folder, and a capability or a resource for a `resources/` folder. A worker kind needs `cardinality: "collection"`, because a [seat](./workers-on-disk.md#a-workers-identity) — one hired worker — mints its own copy under its own id. A channel kind needs the default, `singleton`, because a channel is a session on one shared copy.
