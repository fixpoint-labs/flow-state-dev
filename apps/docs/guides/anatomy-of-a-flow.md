...
const pipeline = sequencer({ name: "chat-pipeline", inputSchema })
  .step(chatGen)  // output: assistant message
...
An `incState` on a single field, and any `pushState`, do not lose updates on the built-in stores. Each sends the store the operation itself rather than a computed result, and the store applies it to the value it currently holds. The guarantee comes from the store, so a custom adapter without the matching operation doesn't provide it: those calls write the whole record at the version they read, in one attempt, and a concurrent write makes one of them return `false` instead of landing. `atomicState`, the updater form of `patchState`, and an `incState` across two or more fields carry the version they read instead: on a lost race they re-run your function against the value that won, so both updates end up in the record. `setState` carries a version too, but it replaces the record with the object you passed, so it overwrites a concurrent writer rather than merging with one.
