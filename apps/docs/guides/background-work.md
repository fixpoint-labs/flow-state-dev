...
description: "What background work means in flow-state-dev: side chains, queue-backed action runs, and dispatched runs, what each one outlives, and which page documents it."
---
...
| **Dispatch** | a session of its own | the request that started it | yes |
...
## Dispatch: work in its own session

A **dispatch** sends one unit of work to an entry the flow declares, and it runs in a session of its own. Its requests don't show up in the starting session's request list. You reach a run two ways: list the flow's sessions with its dispatch runs included, or ask one conversation which runs were started from it.
...
// Every dispatch run on the flow, beside the conversations people started.
...
A run's `id` is a session id, so every session read works on it.

Having its own session is also what it costs you. The run keeps its own state, its own history, and its own session-scoped resources, and none of those belong to the conversation that started it. What the two share without asking is the user: user- and org-scoped data is the same data on both sides, because the run executes as the same user on the same flow.
...
Run the dispatcher and it sends one request to the `investigate` entry and returns `{ sessionId, requestId, adopted }` as soon as the runtime accepts it, without waiting for the work. With `session: { key }` the work runs in a session derived from that key, created on first use; the same key from the same conversation lands on the same session again, with `adopted: true`. With `session: { id }` it is delivered into a session that already exists, and refused if it doesn't. `defineFlow` throws at definition time when `action` names an entry the flow doesn't declare.
...
A board is a list of tasks plus a set of named workers that claim them. A seat normally runs its tasks inline, in the request that claimed them. Put a `dispatcher({ action, session })` in the seat's position instead and the board hands each claimed row off to a dispatch run. How many runs that is depends on the seat's session key: `per-task` gives every row a run to itself, while `per-worker` and a shared `{ key }` send several rows into one run, one request each. The worker is declared once on the flow, under `task.actions`, and the seat names it with `action`. Do not set `type` — the stamped address is `type: "task"`:
...
const board = taskBoard({
  ...
  workers: {
    investigate: dispatcher({
      ...
      session: "per-task",        // one session per row
    }),
...
The drain claims a row, sends it to the `investigate` entry, and moves on. The run executes `investigateBlock` with the same worker input an inline seat would get, and settles the row itself. Which session a row lands in is the seat's `session` policy: `"per-task"`, `"per-worker"`, or `{ key: (task) => string }`.

Tasks are seeded here to keep the example in one piece. A row added later with `addTask` carries the same `assignee` field.

A board with a seat that hands off needs an explicit `boardId` and a `defineTaskCollection()`. The run settles its row after the request that claimed it is gone, so the ledger it settles against has to outlive that request. Both are checked when the board is built.
...
**The board has to be reachable from the run's session.** The run settles its own row, and resource scope resolves against whichever session is running, so a session-scoped board hydrates empty there and has nothing to settle. Give the collection `sharedToLineage: true` and the whole lineage settles against one ledger. `user` and `org` scope need nothing extra.

**On serverless, the work is bounded by the function unless something else consumes the queue.** A dispatched run executes inside the invocation that started it, so the function's maximum duration is the ceiling. A queue adapter alone does not lift it: in `colocated` mode the same process both enqueues *and* consumes, so the job is picked up by the invocation that is already running out of time. What lifts the ceiling is a consumer with its own lifetime — run the function in `dispatch-only` mode and host the worker separately, as a container or a long-lived process in `worker-only` mode. `colocated` is the right answer on a server you keep running, not on a function.

**A run started from a `worker-only` process isn't durable.** That mode installs no dispatcher, so the work executes in the worker process instead of going to the queue, and nothing re-runs it if that process stops. See [From a worker-only process](/docs/cli/overview#from-a-worker-only-process).

**A row released to a run stops holding up the drain.** A row a run has in hand is not counted by the board that filed it, so the request that filed it finishes without waiting.

That release lasts only while the run is actually holding the row, not for the rest of the row's life. Handing work over says where it belongs; it does not promise the work is still moving. If the run stops without settling the row — its process dies, its host is shut down — the row goes back to being the board's to deal with, and the next drain of that board waits on it like any other outstanding work. A row parked for a person counts again too, unless the board asked not to wait on reviews with [`onReview: "exit"`](/docs/orchestration/task-board#waiting-on-a-person-onreview).
...
Read next: **[Dispatched work](/docs/server/background-work)** for the HTTP surface, its paging, what `status` does and doesn't tell you, and the access rules the listing endpoint applies; **[Client overview](/docs/client/overview#dispatched-runs)** for reading it from an app; and **[Durable execution](/docs/advanced/durable-execution)** for what happens to a run that was interrupted.
