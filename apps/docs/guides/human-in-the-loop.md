...
- **`ItemsRenderer`** supplies the session items to its renderers, including the `suspension_resume` item that records the outcome. The card uses it to show the receipt on resolve and after a reload. A standalone card outside `ItemsRenderer` needs `<SessionItemsProvider value={session.items}>`, imported from `@flow-state-dev/react`.
