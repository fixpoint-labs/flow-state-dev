...
export function BackgroundWorkPanel({ session, flowKind }: BackgroundWorkPanelProps) {
  ...
  const openRow = childSessions.find((childSession) => childSession.id === openId) ?? null;
      ...
      <div className="rounded-md border border-indigo-500/40 bg-indigo-500/5">
        ...
        <ul className="border-t border-indigo-500/20">
          {childSessions.map((childSession) => (
            <li key={childSession.id}>
              <button
                type="button"
                className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs hover:bg-indigo-500/10"
                onClick={() => setOpenId(childSession.id)}
                data-testid="background-work-row"
              >
                <span className="min-w-0 flex-1 truncate">{rowLabel(childSession)}</span>
                {childSession.coordinate !== undefined && (
                  <span className="hidden truncate text-muted-foreground sm:inline">
                    {childSession.coordinate}
                  </span>
                )}
                <span className={statusTone(childSession.status)}>{statusLabel(childSession.status)}</span>
