 ...
 * the turn's own reply arrives first, the dispatch-run list is re-read once when
...
test("background work runs in its own session and renders outside the conversation", async ({
  ...
  // Opening the row reads the dispatched run's OWN session history. The brief is
