 ...
 * the turn's own reply arrives first, the workstream list is re-read once when
...
test("background work runs in its own workstream and renders outside the conversation", async ({
  page,
  userId,
  ...
  // Pick the style that hands the message to a workstream instead of answering.
  ...
  // Opening the row reads the workstream's OWN session history. The brief is
