 ...
 * writes the request onto a durable board and hands it to a **child session** — a
...
export function createBackgroundWorkPipeline(config: PipelineConfig) {
  ...
  const pipeline = sequencer({
    ...
    // the row still open. `.tap` keeps the turn's message as the running value
    // so the report below reads its own input rather than the drain's
    // bookkeeping.
    .tap(board.drain)
