...
### Authentication strategy: per-flow principal resolver (FIX-23)

- **`authentication` config on `defineFlow`** — `{ resolvePrincipal?, defaultUserId?, requireUser?, requireOrg? }`. The framework owns the contract; the host owns credential verification. `resolvePrincipal` returns a `ResolvedPrincipal`, a partial `{ userId?, orgId? }`, or `null`. Per-flow declarations win over a host-level fallback.
- **`createFlowApiRouter({ resolvePrincipal })`** — host-level fallback used when an inbound flow declares no `authentication.resolvePrincipal`. Defaults to `defaultBodyUserIdPrincipalResolver` (reads `body.userId` from the parsed HTTP body) for backward compatibility.
- **Resolution path inside `host.resolvePrincipal`** — pick the per-flow resolver when set, else the fallback; if the result has no `userId` and `defaultUserId` is set, substitute it; if still no `userId` and `requireUser !== false`, throw 401; if `requireUser: false` and no userId resolves, throw 500 naming the flow (configuration error). Adapters are unchanged — they always call `host.resolvePrincipal` and the host applies per-flow routing transparently.
- **`requireUser: false` build-time enforcement** — `defineFlow` rejects flows that declare `user.stateSchema`, `user.clientData`, or any user-scoped resource (block-declared or flow-level) when `requireUser: false`. Errors at registration name the offending field. The Phase 1 hard lock that previously rejected all `requireUser: false` flows is gone.
- **`AuthenticationConfig`, `ResolvePrincipalFn`, `ResolvedPrincipal`, `PrincipalResolutionContext`, `InboundSource`** moved into `@flow-state-dev/core/types`. `@flow-state-dev/server` re-exports them; the historical `PrincipalResolver` name is preserved as a type alias.
- **`createHmacVerifier`** — webhook signature verifier supporting GitHub-style raw `sha256=<hex>` headers, Stripe-style `t=<ts>,v1=<sig>[,vN=...]` with timestamp tolerance, and a `format: "custom"` mode for everything else. Constant-time comparison; the helper is a pure function over `(rawBody, signatureHeader)` so hosts compose it inside their resolver.
- **`createHs256JwtVerifier` + `extractBearerToken`** — HS256 JWT verifier with `iss`/`aud`/`exp`/`nbf` checks and configurable clock skew, plus a case-insensitive `Bearer` parser. RS256/ES256 are out of scope (need JWKS); hosts plug in their own verifier for those.
- **`top-level` `requireUser` shorthand** — `defineFlow({ requireUser })` still works for the common case. When both top-level and `authentication.requireUser` are set, `authentication.requireUser` wins.
- **`defaultBodyUserIdPrincipalResolver` returns `null` instead of throwing** when no `userId` is present on the body. The host applies the fallback chain; existing 401-with-userId-in-message behavior is preserved through the host throw, and the action route's legacy 400-status mapping for missing-userId is unchanged.
- **Tests.** `packages/core/test/flow.test.ts` covers `requireUser: false` happy path, `authentication.requireUser` precedence, and the four user-scope conflict branches (state, clientData, flow-level user resource, block-declared user resource). `packages/server/test/transports/per-flow-resolver.test.ts` covers per-flow override, defaultUserId fallback, 401 vs 500 enforcement, and host-fallback path. `packages/server/test/transports/createHmacVerifier.test.ts` covers GitHub-style, Stripe-style with stale-timestamp rejection, multi-`v1` rotations, custom parsers, and a sha512+base64 path. `packages/server/test/transports/createBearerTokenVerifier.test.ts` covers bearer parsing, signature/algorithm/exp/nbf/iss/aud verification, and malformed-input null returns.
- **Documentation.** New `docs/architecture/authentication.md`. New `apps/docs/docs/server/authentication.md` with three integration patterns (NextAuth-style HTTP session, Stripe webhook with HMAC, MCP/API token over `Authorization`). `docs/architecture/inbound-transports.md` and the user-facing transports doc updated to drop the FIX-23-pending notice and link to the new auth pages. `packages/server/README.md` gains an Authentication section.
- **Out of scope (per spec).** Framework-side credential storage; OAuth provider implementations; org-scope state model (`requireOrg` reserved as a flag with no runtime effect); transport adapters themselves (FIX-438 children); making `userId` optional throughout `runAction`/stores/`ActiveRequestEntry` (host requires a resolved or default-substituted userId either way today).
