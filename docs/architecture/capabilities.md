...
**Diamond with conflicting config throws.** Presets keep first-wins on a diamond. Config carries values, so two paths reaching the same base with different `.config()` values would silently bake one closure and drop the other — `flattenCapabilities` throws instead (`deepEqual` on the raw `.config()` argument; identical config deduplicates).
