...
- **Identity is never a parameter.** Every verb closes over the running request's server-derived identity. A caller supplies the *target* of an operation and never the *authority* for it.

**A session id, however, is a parameter of exactly one verb — and which verbs take one is the contract, not an exception to it.** The seam has two authorization models and every verb uses one of them:

- **By descent** — `startDetached`, `parentTask`, `settleParentTask`, `livenessOf`. A caller supplies a *routing seed*; the seam derives the child session id by hashing it together with the tenant, the principal, **and the parent session**. The parent session is in the key material precisely because these verbs authorise by descent — omit it and one principal's second parent session derives the first's child key, adopts its child, and its work then settles onto the wrong board while its own interrupt and liveness calls refuse.
- **By peerage** — `sendMessage`. It takes a **recipient session id**, because reaching a session you were given the address of is the whole of what it does. The locator is the deliberate BP-031 exception and it is *checked, not trusted*: the seam compares the named session's owner, tenant, org binding and **flow kind** against the running request's own before anything is dispatched, and refuses as `unknown-recipient` when any of them differ — the same answer a session that does not exist gets, so the check leaks no existence. Identity is still never a parameter; what changes is that the check is against a peer rather than against a derivation.

The flow-kind arm of that check is load-bearing and has no downstream backstop: `createExecutionContext` validates user, tenant and org bindings and reads `flowKind` nowhere, and a session id is not a flow boundary — one session can host requests of two flows. Without the check at the verb, a same-owner send naming another flow's session would run *the sender's* handler against that session's history and state.
