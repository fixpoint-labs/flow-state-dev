...
generator({
  name: "chatbot",
  agentType: "agent",      // classifies the identity
...
|-------------|:-----------:|:-------------:|:--------------------:|:-------------------------:|
| `"agent"`     | ✓ | ✓ | ✓ | rendered inline |
| `"sub-agent"` | ✓ | ✓ | — | **hidden** (opt in via `showSubAgents`) |
...
- **`"agent"`** — the user is talking to this thing. Messages go to the UI, and future turns in the session see them.
- **`"sub-agent"`** — this thing is doing work on behalf of an agent. The user can watch it live (observability) but the broader conversation doesn't inherit its chatter.
...
Every generator declares its own identity explicitly. There is no "sub-agent if nested, agent if top-level" inference — nesting is irrelevant. Pattern factories set `agentType` on their internal generators; user-composed generators declare `agentType` at the callsite.
...
Two generators with `agentType: "agent"` and distinct `agentName`s can coexist in the same session. Both see user messages and each other's messages via `history: true`.
...
`session.items.history()` returns the conversation history — user messages + `agent`-typed conversational items. It's the ambient "what did we say so far?" view.
...
| Pattern | Knobs |
|---------|-------|
| Supervisor | `workerAgentType` (default `"sub-agent"`), `workerAgentName`, `synthesizerAgentType` (default `"agent"` if present) |
| Plan & Execute | `plannerAgentType`, `stepExecutorAgentType`, `evaluatorAgentType`, `replannerAgentType` (all default `"sub-agent"`), `synthesizerAgentType` (default `"agent"`) |
| Coordinator | `workerAgentType` (default `"sub-agent"`), `workerAgentName`, `plannerAgentType`, `mergerAgentType` |
| Blackboard | `controllerAgentType` (default `"sub-agent"`), `synthesizerAgentType` (default `"agent"`) |
| Reactive Blackboard | `actorAgentType` (default `"sub-agent"`) |
| Response Auditor | `auditorAgentType` (default `"sub-agent"`) |
| Event Queue | `workerAgentType` (default `"sub-agent"`) |
...
The React `<ItemsRenderer>` filters `agentType === "sub-agent"` items by default. Opt in with `<ItemsRenderer items={session.items} showSubAgents />` to surface them inline. For richer per-agent UIs (collapsed panels, tabs), use `session.getItemsByAgent(name)` to pull the slice you want and render it yourself.
