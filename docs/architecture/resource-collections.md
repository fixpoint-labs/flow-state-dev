...
**Block writes (`writable`).** Same field as a single resource, declared once on the collection. Omit it (or set `true`) and instance `patchState` / `setState` / `updateState` / `upsert` patch / `writeContent` persist as usual. Set `writable: false` and those instance writes throw. Independent of `llmWritable`, which only gates the LLM write tool.
