...
**The address carries the owner, and is not a session key.** `resolveLineageScopeId` (`stores/scope-keys.ts`) hashes framed `(tenantId, userId, rootSessionId)` into `lin_<digest>`. Both halves matter and both close the same hole. Session ids are caller-chosen and a session can be deleted, so a descendant addressing a bare root id can be pointed at a session someone else later created under that id — and it would then read and overwrite that owner's ordinary session-scoped rows, because a bare root id IS an ordinary session key. Conjoining the owner makes the collision unexpressible rather than detectable; putting it in its own namespace stops a lineage address and a session address from ever naming one bucket. The hash rather than a delimiter is because every component is caller-influenced and `resolveSessionStorageKey` joins tenant and session with a bare `:`, so a delimited form is forgeable by choosing a session id — the same reasoning, and the same construction, as `deriveChildSessionId`.

The root computes the same address from its own id, which is what keeps a parent and its descendants on one bucket. Unshared session resources are untouched and still key at the session key, so nothing that predates `sharedToWorkstream` moves.
