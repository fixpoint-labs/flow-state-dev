...
- One dispatch protocol: every arrival is a dispatch of one type — `public` / `chat` / `webhook` / `schedule` / `task` / `internal` — delivered to one entry addressed `(type, name)`. Every entry shares `ActionCore` (handler `block` + execution policy + per-entry `concurrency`). The maps: `flow.actions` (public), `flow.internal` (internal), `flow.tasks` (task; a plain `{ block }` the flow declares, **gated at `defineFlow` by the board whose `dispatcher({ type: "task" })` seat hands off to it** — an entry no board addresses, a task dispatcher no board holds, and two boards on one entry are refused), `flow.chat.on` / `flow.webhooks[p].on[e]` / `flow.schedules.static[id]` (the protocol-owned types, keyed by the adapter's namespaced `metadata` coordinate). Resolution is `resolveEntry(flow, type, name, coordinate?)` (`core/flow/resolve-entry.ts`; the engine wrapper maps the trusted `source` to the type via `dispatchTypeOf`) — **one keyed lookup, no fallback for any type**: an absent entry is a named refusal and never resolves another type's map. Dynamic schedules carry the core on `InboundRequestEnvelope.resolvedActionCore` (transient — not persisted, so durable dynamic schedules don't recover); `runAction` walks a carried core for dispatch addresses the flow does not declare and refuses it.
  → [Action Forms](../architecture/action-forms.md)
- Sending is a block: `dispatcher({ name, type: "internal", target, session: { key } | { id }, payload? })` (`core/blocks/dispatcher.ts`) is a handler carrying its static `(type, target)` as `BlockDefinition.dispatch`; `defineFlow` walks the reachable graph (composition, rescue, static `tools`) and refuses an address that resolves nothing. `{ key }` derives a child of the running session (minted, then adopted); `{ id }` delivers into an existing session of the same flow/principal/tenant, and an unknown id is refused, never created. There is **no `ctx.dispatchMessage`**: the runtime's seam sits under the `DISPATCH_SEAM` symbol (`dispatchThroughSeam` / `markDispatcher` for substrate code), so the set of blocks that dispatch is the set carrying an address. A block may dispatch only `internal` and `task`; `dispatcher({ type: "task", target, session: "per-task" | "per-worker" | { key } })` is a board seat — the board installs its own hand-off (`createHandOff`) there, carrying the seat's address plus the board's binding (`bindTaskDispatcher` / `taskBindingOf`: `{ boardId, gate }`), and its input is the claim envelope `taskDispatchInputSchema`.
- Public re-entry: `isPublicReentryAllowed(source, additionalSources?)` (`engine/routes/public-reentry.ts`) is an **allow-list** — `http` / `mcp` / `chat` / `scheduled` — routed through by retry, continue and resume. Anything else gets the not-found shape. A host extends it for its own out-of-tree transports with the `publicReentrySources` option on `createFlowState` / `createFlowApiRouter` (carried on `RuntimeConfig`); `webhook`, `task` and `internal` are in the **never** set — `assertPublicReentrySources` throws at router construction if one is named.
  → [Action Forms](../architecture/action-forms.md)
...
- A task board hands a seat off by holding a `dispatcher({ type: "task", target, session })` there; the flow declares `tasks: { [target]: { block } }`; `defineFlow` rebuilds the entry as the board's claim gate (re-read the row, verify attempt / createdAt / incarnationId / status / lease / seat, mark the task scope, re-mint the ticket, renew the lease). Uniform and floor workers run inline
