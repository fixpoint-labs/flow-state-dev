...
- Public re-entry: `isPublicReentryAllowed(source)` (`engine/routes/public-reentry.ts`) is an **allow-list** — `http` / `mcp` / `chat` / `scheduled` — routed through by retry, continue and resume. Anything else (including `webhook` and `workstream`) gets the not-found shape. Replaces the three per-route webhook deny-lists, which admitted any source nobody named.
