...
- One message protocol: every arrival is a message of one type — `user` / `chat` / `webhook` / `schedule` / `task` / `internal` — delivered to one entry addressed `(type, name)`. Every entry shares `ActionCore` (handler `block` + execution policy + per-entry `concurrency`). The maps: `flow.actions` (user), `flow.internal` (internal), `flow.tasks` (task; **produced only by a task board** as `tasks: board.tasks`, branded, refused otherwise), `flow.chat.on` / `flow.webhooks[p].on[e]` / `flow.schedules.static[id]` (the protocol-owned types, keyed by the adapter's namespaced `metadata` coordinate). Resolution is `resolveEntry(flow, type, name, coordinate?)` (`core/flow/resolve-entry.ts`; the engine wrapper maps the trusted `source` to the type via `messageTypeOf`) — **one keyed lookup, no fallback for any type**: an absent entry is a named refusal and never resolves another type's map. Dynamic schedules carry the core on `InboundRequestEnvelope.resolvedActionCore` (transient — not persisted, so durable dynamic schedules don't recover); `runAction` walks a carried core for dispatch addresses the flow does not declare and refuses it.
