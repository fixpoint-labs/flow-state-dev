...
- Against the resource handles, those seven split three ways — they are **not** seven ops with no resource counterpart: `patchState` / `setState` are **shared** (declared on both `ResourceContext` and `ResourceRef`); `atomicState` has an **analogue** in `updateState` that is *not* interchangeable with it (`atomicState` returns a partial that is shallow-merged; `updateState` returns the whole next state, which is re-parsed against the resource's `stateSchema`, so a field the callback omits does not survive); `setStateRecord` / `deleteStateRecord` are **deliberately absent**, because a resource *is* the per-key row and the storage key already does that addressing. `incState` / `pushState` are absent on a resource today — the gap this contract's concurrency guidance keeps sending readers away from resources over
...
- Concurrency: bounded retries on the **version-checked** path only. Commutative bag writes (single-field `incState`, `pushState`, `setStateRecord`, `deleteStateRecord`, single-literal-field `patchState`) persist at `expectedVersion: "any"` where the adapter advertises the delta verb, and take no version check at all. Every write **through the resource handles** is version-checked and can be refused
