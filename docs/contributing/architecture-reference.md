...
- Action forms: every action shares `ActionCore` (handler `block` + execution policy). Caller-addressed = `ActionConfig` in `flow.actions`; event-addressed (webhook/chat/scheduled) carries the core inline on its transport map and has no HTTP/MCP caller surface (no `internal` flag — the structural fact is the boundary); detached (`source: "workstream"`) resolves the single pre-assembled `flow.workstream` core; **relay (`source: "relay"`) is a session-to-session delivery and resolves `flow.relay.on[kind]` or, on the gated fallthrough, `flow.actions[kind]`**. Resolution seam `resolveActionCore(flow, actionName, source, metadata)` reads a namespaced coordinate (`metadata.webhook` / `metadata.chat.eventKey` / `metadata.schedule.scheduleId` / `metadata.relay`) gated on `source` (set only by adapters and by the framework's own seams), else falls back to `flow.actions[name]` — **except for `"workstream"` and `"relay"`, where the branch is terminal: an absent core is a named refusal and never reaches `flow.actions` by fallthrough.** Dynamic schedules carry the core on `InboundRequestEnvelope.resolvedActionCore` (transient — not persisted, so durable dynamic schedules don't recover).
- Relay door: **the door is decided at the `sendMessage` verb, not at resolution**, and its answer is stamped on `metadata.relay.door` (`"declared" | "action"`) — resolution honours the stamp and never recomputes, because neither session kind is available where routing happens. Two axes gate the **fallthrough only**: a declared `relay.on[kind]` binding is reachable on every session kind, while `flow.actions[kind]` requires **both** ends to be `SessionRecord.sessionKind` `"top-level"`/`"sibling"`, and an **absent** kind at either end refuses first, ahead of the declared handler. A `durable` fallthrough target refuses `durable-action`; the guarantee behind it is the runtime refusal at `ctx.suspend()` for any `RELAY_SOURCE` request, since `durable` is a correlate and not the capability. `sessionKind` is stamped by `ensureSessionRecord` (default `"top-level"`), the session-create route, the detached child writer (`"workstream"`, the one site), and the published testing helpers; legacy rows are repaired by `backfillSessionKind` / `fsdev migrate session-kind`.
  → [Messaging a session](../../apps/docs/docs/server/session-messages.md)
- Public re-entry: `isPublicReentryAllowed(source, additionalSources?)` (`engine/routes/public-reentry.ts`) is an **allow-list** — `http` / `mcp` / `chat` / `scheduled` — routed through by retry, continue and resume. Anything else gets the not-found shape. Replaces the three per-route webhook deny-lists, which admitted any source nobody named. A host extends it for its own out-of-tree transports with the `publicReentrySources` option on `createFlowState` / `createFlowApiRouter` (carried on `RuntimeConfig`); `webhook`, `workstream` and `relay` are never openable and `assertPublicReentrySources` throws at router construction if one is named.
