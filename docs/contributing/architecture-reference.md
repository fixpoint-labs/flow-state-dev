...
- Against the resource handles those seven split three ways — shared (`patchState` / `setState`), analogue-but-not-equivalent (`atomicState` ↔ `updateState`), and deliberately absent (`setStateRecord` / `deleteStateRecord`); `incState` / `pushState` have no resource form → [State and Scopes](../architecture/state-and-scopes.md#cas-and-concurrency)
