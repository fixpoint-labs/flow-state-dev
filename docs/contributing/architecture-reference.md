...
- Concurrency: bounded retries on the **version-checked** path only. Bag writes split between a version-checked path and an unchecked commutative one, by hint shape, adapter and scope → [Atomicity Guarantees](../architecture/state-and-scopes.md#atomicity-guarantees) is the canonical enumeration. Through the resource handles, the **state** mutators (`patchState` / `setState` / `updateState`, `create`, the delete writers) are version-checked and can be refused; `writeContent` is **not** — `ContentStore.set` takes no version predicate and overwrites
