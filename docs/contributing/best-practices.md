...
- Status: Active
- Date: 2026-06-27
- Scope: Package moves, renames, and extractions.
- Rule:
...
- Rule:
  - For the changed surface, handle (or explicitly scope out) each path the change *touches but didn't add code for*:
    ...
    - React derived state — flags derived from the *complete* input set; change-signals fire only on a *real* change, not no-op patches or filtered items.
