...
- Rule:
  ...
  - The command itself is subject to **tenet 7** (`docs/philosophy.md` — *a check that cannot fire is not a check*). Three shapes, each seen shipping:
...
- Rule:
  ...
  - **Every non-empty fragment names its Linear issue** (e.g. `FIX-123`) in the body. The repo keeps no spec (BP-037), so the changeset is the link from a released change back to the reasoning behind it. Enforced by `scripts/validate-changeset-refs.mjs` on fragments a PR adds or edits.
