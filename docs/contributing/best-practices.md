...
- Rule:
  ...
  - Internal-only changes need no changeset: `pnpm changeset --empty` or state "no changeset needed". An empty fragment releases nothing, so it needs no issue reference.
