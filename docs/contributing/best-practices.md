...
- Rule:
  ...
  - Before opening the PR, scan the full diff for publishable packages (`packages/*` with a `version` field) whose exported or user-observable behavior changed — not only the package named in the title. A drive-by fix in another package still needs that package in the fragment.
...
- Rule:
  - For the changed surface, handle (or explicitly scope out) each path the change *touches but didn't add code for*:
    ...
    - Multi-consumer parity — when engine, history, DevTool, or `client`/`react` each transform the same item log (collapse, visibility, dedup), grep for hand-copied helpers (`Mirrors`, `keep in sync`) in sibling packages; one stale fork shows up as "correct on the server, wrong in the live view."
...
### BP-040: Value-import canonical pure helpers from contracts — never hand-mirror them

- Status: Active
- Date: 2026-07-17
- Scope: Universal — `client`, `react`, and any package that shares item/stream pure logic with `core`/`engine`.
- Rule:
  - Do not copy pure helpers (collapse, visibility, invariant-key lists) into `client` or `react` with "Mirrors … keep in sync" comments. Value-import the canonical implementation from `@flow-state-dev/contracts` (see `docs/architecture/overview.md` boundary rules).
  - Before inlining because "this package may only import types from core," check `package.json` and the architecture doc — browser packages are expected to depend on `contracts` for shared runtime helpers, not to maintain a second implementation.
  - When a rule grows in `contracts` (e.g. a new collapse rule), fix consumers by deleting the fork and importing — not by patching the mirror.
- Why: Hand-mirrored helpers drift silently; divergent live SSE views and server history are the usual symptom.
...
### [Testing & eval harnesses](best-practices/testing.md)

| BP | Rule |
| --- | --- |
| BP-041 | Match comparison-helper failure semantics to the harness — eval scorers must not turn comparator throws into case execution errors |
