...
- Rule:
  ...
  - Include the changeset fragment in the same PR as the functional change — prefer the opening commit so `changeset-bot` and reviewers see a complete unit; don't land the code first and add `--empty` in a follow-up unless the PR is still a draft.
...
| BP-040 | Kitchen-sink gateway model catalog — single-file add, live slug check, no over-wiring |
