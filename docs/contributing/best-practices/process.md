...
- Rule:
  ...
  - **Not every decision is an ask, and most aren't.** A decision reaches the user's surface only if a wrong answer changes **what a user experiences**, **what we've promised**, **when something ships**, or **what it costs to undo** — *and* they'd plausibly answer differently than you did. Everything else is decided and recorded where an engineer reads it. Cap what reaches them at three, hardest first. Decisions they're merely ratifying get a subheading naming the decision and a line on what a wrong one costs — **never a table**, which asks them to read a grid before they can read a decision; the full six-part shape is for the one or two genuinely open forks, plus a spec's §12. Don't ask at all when the call is the implementer's, the answer is derivable, or it's a coin flip with near-zero cost either way — decide it and note it.
  - **Filtering is sorting, not hiding.** Where the artifact defines a sign-off surface, everything on it stays in the artifact — the calls the user doesn't own move to a section addressed to the engineer, so nothing is approved unseen and nothing unactionable is put in front of them ([`../pr-reviewer-guidance.md`](../pr-reviewer-guidance.md) → §3).
