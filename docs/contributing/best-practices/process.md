...
- Rule:
  ...
  - **If the shape genuinely cannot hold, that is a conflict with the unit of intent, not a fork to offer.** Say it where it is read first — above the fold, as *"the spec says X; this does Y because Z"* (for a bug, *the issue says X*) — with the way back to X, and raise it as a blocker on the blind-spot path (`issue-implement`'s challenger: the decision put to a human when one is there, best judgment plus a loud flag when not). It is never framed as one of the asks in *What's asked of you*: an ask reads as a choice between two acceptable shapes, and a reviewer who trusts the artifact has no reason to know one of them contradicts what they already approved.
  - **The second time a reviewer questions a shape, re-read the source before replying.** A repeated "why is it like this" on one surface is a model question, not a naming one; answer it from the spec, the design doc or the issue, not from the cut you have.
...
- Status: Active
- Date: 2026-08-08
- Scope: Process — anything that asks the user to sign off, choose, or unblock: a conversation turn, a spec's `DECISIONS.md`, a PR's *What's asked of you*, a gate, an escalated blocker.
- Rule:
  ...
  - **Not every decision is an ask, and most aren't.** A decision reaches the user only if it passes both filters in [`../asking-for-decisions.md`](../asking-for-decisions.md) → "What reaches them at all"; **three is the ceiling, live forks included**. Everything else is decided and recorded where an engineer reads it.
  - **Filtering is sorting, not hiding.** Where the artifact defines a sign-off surface, everything on it stays in the artifact — the calls the user doesn't own move to a section addressed to the engineer, so nothing is approved unseen and nothing unactionable is put in front of them.
