...
- Why: `expose`/`exclude` are type-checked against the state schema; hand-rolled `data` projections drift silently as fields are added.
...
- Rule:
  ...
  - Capabilities and cross-agent consumers import resource refs from the resource module, not from logic files that re-export them.
- Why: Resources are pure data — keeping refs in leaf modules prevents capability ↔ generator ↔ resource import cycles that break at first use.
...
- Why: A cached mutating or time-sensitive call fails silently on the second hit — caching is a per-tool correctness decision, not a blanket cost optimization.
...
- Why: Schema-level defaults make adding a nullable field a one-place edit instead of touching every create/reset call site.
...
- Why: Load-then-filter reads fine on a small store and silently degrades as it grows.
