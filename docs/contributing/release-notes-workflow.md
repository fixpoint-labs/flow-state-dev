...
`.github/workflows/release.yml` automates this when `CHANGESETS_RELEASE_TOKEN` is configured: it opens a "Version Packages" PR with the proposed bumps and changelog edits. Merging that PR publishes to npm with provenance and creates GitHub Releases from the per-package changelogs.
