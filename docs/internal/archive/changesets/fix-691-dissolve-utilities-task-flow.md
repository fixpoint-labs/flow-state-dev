...
`@flow-state-dev/orchestration` now exports the observation ledger and per-task flow-policy selectors directly — `flowPolicy`, `createObservationLedger`, `createObservationLedgerCapability`, `bindObservationLedger`, `formatPriorWork`, and the `TaskFlowPolicy` / `TaskPriorWork` / `Observation` / `ObservationLedger` / `ObservationLedgerView` types — alongside the task substrate that shapes `TaskWorkerInput.priorWork`.

`@flow-state-dev/patterns` now reaches the tool-cache and flow-policy layers through `@flow-state-dev/core` and `@flow-state-dev/orchestration` directly. Public pattern APIs (`taskBoard`, `planAndExecute`, `supervisor`) are unchanged.
