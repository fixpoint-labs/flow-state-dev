...
- **`reviewOutputSchema` / `ReviewOutput`** (`patterns`) — was one aggregate per review pass: `{ assessments: [{ taskId, verdict: "accepted" | "needs-revision" | "escalate", feedback, score }], needsReplanning, overallAssessment }`. The live `reviewerVerdictSchema` is one verdict per task: `{ decision: "approve" | "reject" | "needs-revision", feedback?: string, criteria?: { name: string; score: number }[], reasoning?: string }`. Different cardinality, different fields, different enum values — and the aggregate's per-assessment `score` (one `0`–`1` number) has no counterpart: rubric scoring is the optional `criteria` array, one `{ name, score }` pair per criterion. Swapping the name compiles and produces a reviewer that does not work.
