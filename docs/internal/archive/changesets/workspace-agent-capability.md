...
**The caller's error hook runs even when reconciliation fails.** They are two independent obligations on one failure path, and the composed hook awaited them in sequence — so a reconcile that rejected exited before the caller's ran. The block runtime swallows hook errors to preserve the agent's own, which made the skip silent.
