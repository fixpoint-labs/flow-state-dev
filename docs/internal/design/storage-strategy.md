...
- **`DeltaStoreOps`** (`types.ts:203`) — `patchField` / `incField` / `pushToArray` / `deleteField` — is an **optional, per-verb capability** (every method is optional on the interface). `createScopePersist` (`stores/scope-persist.ts`) feature-detects each verb (`typeof store.patchField === "function"`) and **falls back to a full-record `set` when the verb isn't advertised** (the FIX-405 decision tree). Coverage is therefore *not* uniform: Postgres implements the verbs via native JSONB operators; other adapters advertise a subset (notably `deleteField` is not wired everywhere — record stores and `RequestStore` differ), and unadvertised verbs route through the `set` fallback. This non-uniformity is exactly what the conformance list (below, §8) needs to pin down — the strategy must not assume uniform delete-delta coverage.
...
- **Postgres-all-in-one:** Records — yes, unconditional. Vectors — pgvector ≥0.8.2 is production-credible to ~1–10M vectors (≤50M with `pgvectorscale`); 2000-dim HNSW cap rarely bites. Durable request-event streams — an **append-only event table read by `sequence_number` cursor** (exactly what `request_events` + `getEvents(fromSequence)` already do), so every attaching or reconnecting client replays the same rows; LISTEN/NOTIFY only nudges. **Do not use `SELECT … FOR UPDATE SKIP LOCKED` for the events/subscribe slot** — that's a single-consumer *claim* primitive for a competing-worker *queue* (a future `queue` slot), and it would hand each event to one consumer, breaking multi-client fan-out and `Last-Event-ID`/`starting_after` resume. LISTEN/NOTIFY is a **wake-up nudge only**: 8000-byte payload cap, no durable replay, single-primary fan-out, a global commit lock that serialized three Recall.ai outages (fix only in PG19, not GA until ~Sept 2026), and it **breaks through transaction-mode poolers** so it's unavailable in exactly the serverless shape. **Apache AGE is not a credible default** — per-PG-version release lag (PG16 stuck on v1.6.0), large issue backlog, and absent from managed PG (Neon/Supabase/RDS-standard). Model edges relationally; treat AGE as a power-user upgrade.
...
- **MongoDB:** `$set`/`$inc`/`$push` map 1:1 to `DeltaStoreOps`; `findOneAndUpdate` + a version field is native atomic CAS — the single strongest structural fit for the records concern. Change streams beat LISTEN/NOTIFY as a *live tail* (resume tokens, 16 MiB events, no payload ceiling) — but they are **not a durable replay log**: resumption only works while the resume point is still in the oplog window, so a client reconnecting after that window cannot satisfy FSD's `Last-Event-ID`/`starting_after` replay from change streams alone. A Mongo `events` adapter must keep durable event rows in a `request_events` collection (cursor by `sequence_number`) and use change streams purely as the live-notify layer. Other caveats: change streams **require a replica set even single-node** (oplog, keyfile, hostname config — more than `docker run`), and **Atlas Vector Search is Atlas-managed only** (not self-host Community). SSPL doesn't reach a shipped adapter (driver is Apache-2.0); the friction is packaging/optics.
...
Tiers describe *who maintains the adapter*, NOT production-suitability, and NOT whether it ships today — those are three separate axes. **First-class** = framework-maintained and in the test matrix · **Community** = intended best-effort support · **Niche** = exploratory, documented caveats.

- **Shipped today vs. candidate:** only **In-memory** and **Filesystem** (both in `engine`), **SQLite** (`@flow-state-dev/store-sqlite`), and **Postgres** (`@flow-state-dev/store-postgres`) exist as adapters right now. Every other row — **Valkey/Redis, MongoDB (FIX-83), Upstash (FIX-85), NATS JetStream, SurrealDB (FIX-86)** — is a **candidate/proposed** adapter (the FIX refs and "(proposed)" mark them). The tier is the *planned* support level, not a claim the adapter exists. Do not read a Community/Niche row as a shipped, supported backend.
- **Production-suitability is the "Honest hosting gradient" column, read independently:** in-memory and filesystem are first-class (maintained, tested) yet **dev-only** — in-memory is ephemeral, and filesystem event persistence is explicitly unsuitable for production load. Do not read "First-class" as "production-supported on every backend."
- **The "Blobs" column is today's string `ContentStore` (under `primary`), not the binary `blobs` object-store slot.** A `✓` there means the backend persists resource *content strings*; the forward-compatible `blobs` capability slot (binary/large objects, e.g. S3, JetStream Object Store) is skipped by `resolveProfileStores` and backs nothing today (§5 step 6). "Object Store"/"GridFS" entries are candidate *targets* for that future slot, not shipped blob support.
...
- **The "Streams/subscribe" cell must be a *replayable* substrate, not bare Pub/Sub.** The landed `RequestStore` contract is not live-only: *replayable boundary events* are durably written (`persistEvents`), replayed by `getEvents(fromSequence)`, then tailed by `subscribeToEvents`, and the locked stream contract guarantees `Last-Event-ID` / `starting_after` resume. A Pub/Sub-only adapter would drop events for disconnected clients and break resume. So a stream adapter needs a durable ordered log (Redis/Upstash **Streams**, NATS JetStream **Streams**, Postgres event table) with Pub/Sub/SSE/NOTIFY used only as the wake-up nudge. **Scope caveat for the events-slot follow-up:** "replayable" means the *boundary* events (`item.added`/`item.done`, `content.added`/`content.done`, request status), **not** per-token `content.delta` / `content.audio.delta` — those are deliberately excluded from replay (`getReplayableEvents`, FIX-479); the durable text path is the item snapshot, and reconnecting clients snap to the next persisted boundary event. An events adapter must not try to persist/replay deltas or promise token-level resume the runtime doesn't provide.
- **A "KV CAS" records cell is not automatically complete `primary` support.** The records contract needs read-your-writes and filtered `session.list` / `request.list`. Pure KV engines (NATS KV; and to a lesser degree Redis-shaped KV) don't guarantee read-your-writes on direct gets and expose key listing/watch rather than secondary indexes — a `primary` adapter on them must pin reads to the leader and build its own indexes. NATS is therefore marked records-caveated and is best kept to the streams/object-store role.
...
- **Cloud / distributed** (K8s / ECS) — *Today:* Postgres for records (LISTEN/NOTIFY tail). *Target:* **Valkey (or NATS JetStream) for streams/pub-sub** behind the events contract once cross-instance fan-out and durable replay matter (LISTEN/NOTIFY's global-commit lock is the documented ceiling). *Alternative:* MongoDB for records (a `request_events` collection for the durable log + change streams as the oplog-bounded live tail) + Atlas Vector. Bring-your-own-DB.
- **Serverless / edge** — *Today:* the shipped path is Vercel/Neon pooled Postgres (`vercelPostgresStores()` — `pg.Pool` + optional Neon WebSocket client) with polling live tail. *Target:* Postgres via HTTP driver for records + **Upstash Redis Streams (REST) for the events slot** (durable/replayable, Pub/Sub-over-SSE only as the wake-up) — the clean fit for the no-long-connection constraint — which needs both the events slot and an Upstash adapter. *Avoid:* LISTEN/NOTIFY (breaks through poolers), self-host NATS/Valkey from functions (connection model fights ephemerality). This is the dimension where the one-binary thesis breaks and composition earns its keep.
...
2. **Do the full docs surface in §9.2 — not just `persistence/overview.md`.** Extend `apps/docs/docs/persistence/overview.md` with §4.2–§4.4 (composition by hosting shape, the LISTEN/NOTIFY ceiling framing, resource-vs-table), **and correct every stale surface §9.2 lists**: `apps/docs/docs/api/server.md`, `packages/engine/README.md` + the store READMEs, the architecture pages `docs/architecture/server-and-client.md` and `state-and-scopes.md`, and the `trading-desk-walkthrough.md` persistence section. *Verify (closure gate):* **all** §9.2 items are done — grep the repo for `createFilesystemStores`-as-production and `basePath`, and confirm no authoritative page still omits Postgres or calls filesystem the production default — before FIX-664 is closed; the `overview.md` page renders and cross-links, and only *shipped-today* compositions are stated as current (target splits labeled future). *Depends on:* step 1, and ideally steps 3–4 for the cross-backend splits.
...
5. **Spawn: stream adapters behind `subscribeToEvents`** — a Valkey/Redis adapter and/or a NATS JetStream adapter, *when* a deployment outgrows LISTEN/NOTIFY. **Depends on step 3 (the `events` slot):** today's `CapabilitySlotMap` has no `events` key, so a stream-only adapter can't be composed alongside a Postgres `primary` until step 3 lands — without it, step 5 would have to masquerade as a full `primary`/`RequestStore`, reintroducing the coupling this strategy rejects. *Verify (there):* cross-instance live tail works behind the unchanged interface, composed via the `events` slot; LISTEN/NOTIFY path unaffected.
...
Steps 4, 6, 7 are independent and can be prioritized separately; **step 5 depends on step 3** (the `events` slot); none blocks step 1 or 2.
...
| Case | Position |
|---|---|
| Adapter declares a slot it can't back | Partially covered today: `resolveProfileStores` fails fast with `FlowStateConfigError` **only** for the "wired into a slot it doesn't list in `adapter.capabilities`" mismatch. It trusts `adapter.capabilities`, skips forward-compatible slots entirely, and `resolveStores()` then fills any uncovered sub-store from in-memory — so an adapter that *declares* a slot but returns **no backing store** is **not** caught and silently degrades to ephemeral. The events/vectors/blobs follow-ups must add **slot-specific conformance/projection checks** (does the resolved registry actually contain the slot's sub-stores?); this row must not imply the current seam already enforces that. |
...
- [x] Reference docs — **extend** `apps/docs/docs/persistence/overview.md` (no new page, no sidebar change); **and correct `apps/docs/docs/api/server.md`** (its `## Stores` section, ~lines 139–146, still calls `createFilesystemStores` "Filesystem-backed persistence (default)" and uses the stale `basePath` option — the current option is `rootDir`, and filesystem is dev-only, not the default/production store). Fix the description and the snippet in the docs follow-up, or the public API reference keeps shipping an invalid, production-implying example.
