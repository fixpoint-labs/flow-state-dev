...
Tiers describe *who maintains the adapter*, NOT production-suitability — those are separate axes. **First-class** = framework-maintained and in the test matrix · **Community** = shipped/accepted, best-effort · **Niche** = exploratory, documented caveats. **Production-suitability is the "Honest hosting gradient" column, read independently:** in-memory and filesystem are first-class (maintained, tested) yet **dev-only** — in-memory is ephemeral, and filesystem event persistence is explicitly unsuitable for production load. Do not read "First-class" as "production-supported on every backend."
