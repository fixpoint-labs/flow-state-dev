...
- **A person pasting a figure into the body works, confirmed.** The form that rendered on PR #1784, pasted by hand, is an HTML `img` tag with a `raw.githubusercontent.com` URL pinned to a **commit SHA**, not a branch:

  ```html
  <img src="https://raw.githubusercontent.com/fixpoint-labs/flow-state-dev/44b6c292b1affcd8ce827fdb736136fa466e3adf/docs/internal/spikes/spec-formats/FIX-1362/e-svg-poc/figures/drawers.svg" />
  ```

  Pin to the SHA: a branch URL changes under the reader and GitHub's image proxy caches by URL, so a figure redrawn after the body was written would show stale or not at all. The `img` form also takes a `width`. Two rules follow for an agent on a PR a person has edited this way: **never rewrite the body through the tool afterwards**, since the write path strips the image again, and put the figure's SHA-pinned line in the spec PR's `PR.md` so the person has it to paste.
