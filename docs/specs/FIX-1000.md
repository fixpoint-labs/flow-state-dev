...
- **`handleCreateSession`'s existence check is a read-then-act** (`session-routes.ts:137-142`): `get` then `set(..., "any")`, so two concurrent creates of the same id can both pass. **This is now [FIX-1007](https://linear.app/fixpoint-labs/issue/FIX-1007) ([#1060](https://github.com/fixpoint-labs/flow-state-dev/pull/1060)), and the obvious fix does not work** — an earlier draft of this bullet said "use `expectedVersion: 0` and 409 on conflict", which would compile, look right, and not close the race. `ResourceStateStore` reserves `0` as *no live row* by starting its versions at 1, but the **scope stores create records at version 0** (`session-routes.ts:187`, `createExecutionContext.ts:580`/`:597`/`:660`) and write them with `"any"`, so `0` is a live version there. `store-cas-contract.test.ts:79-84` pins it: `set(id, rec, 0)` succeeds on insert *and* succeeds again against the live v0 row. SQLite and Postgres have the same ambiguity — both fall back to an `INSERT` when `expectedVersion === 0`, so a live v0 row takes the update path (`sqlite-store.ts:206-224`, `pg-store.ts:255-270`). FIX-1007 adds a distinct `"absent"` sentinel for this. Made worse by this change, not merely more visible: the loser's `set(..., "any")` clobbers the winner's record, and after D1 that orphans the winner's whole generation.
