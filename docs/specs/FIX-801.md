...
4. **A fund is attributed only if it passes two gates: an eligibility predicate
   ...
   *Alternative rejected:* attribute every fund we can fetch, however thin, and let
   ...
   contract). Above it the residual is small enough that the point-estimate
   concentration figures — including the effective-position count — are sound to
   within a few percent and are reported normally rather than as bands.
...
5. **A refusal is remembered, not retried on every read.** *Alternative rejected:*
   ...
   So a refusal is stored as a marker with its reason and its timestamp, and is not
   re-attempted until a stated backoff has passed. Transient failures still heal;
...
7. **The single-name axis comes from the constituent holdings; the sector axis
   ...
   25% on the badly-covered one). It avoids fanning out hundreds of
...
fund inside a fund resolves one level only, and a constituent that is itself a
fund is not counted as a single name); no expense-ratio-weighted cost analysis
...
**Size estimate: Large** — ~15 files and >500 LOC across a provider fetcher, a
table plus migration and repository methods, an injectable source and fill
service, a REST route and client hook, a new pure domain leaf, a widened health
payload, the Health-perspective UI, the analysis-seed prompt block, tests, one new
doc and six doc edits. Split into the three sub-PRs declared in §8 (Decision 10).
No framework-package changes; entirely `labs/trading-desk`.
...
All changes are in `labs/trading-desk`. No framework package is touched. Five
layers, each following a convention the app already has exactly one of.

```mermaid
flowchart TB
  subgraph provider["lib/providers — stateless, throws"]
    AVREQ["alphaVantageRequest\n(landed, FIX-798:\nkey · HTTP-200 bodies · daily budget)"]
    FETCH["ETF_PROFILE fetcher\nnormalize · map sector vocabulary\n· compute coverage · eligibility"]
    FETCH --> AVREQ
  end

  subgraph libcomp["lib — injectable composition"]
    SRC["EtfProfileSource\ngetOrFetch-wrapped, bounded fan-out\n(the PortfolioQuoteSource / SplitFetcher idiom)"]
  end

  subgraph domain["domain/portfolio — pure + services"]
    SVC["lazy-fill service\ntakes the source as a parameter\nstores answers AND refusals"]
  ...
  subgraph consumers["the two consumers"]
    ROUTE["REST route\nderives the eligible fund set SERVER-SIDE"]
    PANE["Health perspective\nhook -> useMemo -> aggregation"]
...
- **Injectable composition.** A source function type plus its `lib`
  implementation, wrapping the fetcher in the process cache and bounding
  concurrency. This is the seam tests stub, and it exists so the domain service
  imports no provider (the established `PortfolioQuoteSource` / `SplitFetcher`
  posture).
...
- **The residual is a named entry, not a position.** It carries its mass and its
  share, and it is **broken out by cause** — provider-thin, ineligible-by-type,
  unfetchable, and non-attributable constituent lines each account separately, so
  "we couldn't see in" is never confused with "this fund isn't the kind of thing
  you look through" (the portfolio epic's never-a-silent-mislabel theme). It is
  **not** rankable as a top position and can never be the largest-name answer.
- **The look-through single-name axis contains only names.** A fund is either
  decomposed into constituents or left opaque; a fund ticker is never itself an
  entry on that axis, and a constituent that is a fund goes to the residual. This
  preserves the wrapper basis's existing conviction that a 30% index fund is not a
  30% single name.
...
|---|---|---|
| a | provider fetcher (normalization, sector-vocabulary mapping, coverage, eligibility) · table + migration + repository pair · injectable source · lazy-fill service with refusal markers and backoff · REST route · client hook | — |
...
3. **Source + lazy-fill service.** Reads cached profiles, fetches only the misses
   and the stale, persists successes **and** refusals, honours the backoff, and
   never fetches a fund outside the eligible set. *Test:* with a stubbed source —
   cache hit avoids the fetch; a miss fetches and persists; a stale row refetches;
   a refusal is written and then *not* re-attempted inside the backoff (the
   budget-protection test); a throwing fetch does not corrupt the stored row.
4. **REST route + client hook.** The route derives the eligible fund set from the
   user's own holdings server-side, returns profiles plus per-fund refusal reasons,
   and short-circuits with no fan-out for a fund-less book. *Test:* route spec
   against the in-memory migrated repository with the source mocked, mirroring the
   classifications route spec.

*Sub-PR b — the arithmetic.*

5. **The pure look-through leaf.** Positions plus a profile map to effective
...
6. **Widen the health payload.** `lookThrough` becomes a union; add the nullable
...
7. **Health perspective UI.** Add the hook and a look-through block beside the
   existing concentration block: effective top names with their per-wrapper
   ...
   *Test:* a
   ...
   check's (BP-003).
8. **Analysis seed + prompt block.** The seed reads profiles read-only alongside
...
| Daily budget spent / rate-limit body | The landed error types propagate; treated as **transient** — a short-backoff marker, not a permanent refusal, so it heals tomorrow. Already-stored profiles keep working. |
...
| A constituent that is itself a fund (fund-of-funds) | Not counted as a single name; its weight goes to the residual. Otherwise an all-ETF asset-allocation fund would report its component funds as single-name concentrations. |
...
| Short / negative-quantity fund position | **Excluded from look-through.** A negative attribution would break both the "residual closes the total" property and the lower-bound reading of every effective weight. It stays in the wrapper basis and is named in the opaque list. |
...
|---|---|---|---|---|
| 1 | `labs/trading-desk/docs/etf-look-through.md` | **CREATE** (~140 lines) | new file | The methodology reference. Named for its subject, matching the four-of-six convention in that directory (`tax-estimate.md`, `financials-recovery.md`); the `portfolio-` prefix on two siblings is part of their noun, not a namespace, and "portfolio look-through" would read as looking through the portfolio rather than the funds in it. Stays in `docs/` (the methodology references README and CLAUDE.md link to), **not** `docs/specs/` (which holds design records nothing links to). Mirror `tax-estimate.md`: `# ETF look-through (FIX-801)`; a framing paragraph with the honesty posture in bold (a provider's *stated* profile, not a verified custody position, and carrying no as-of date of its own); a pointer paragraph naming the user-facing surface and where the code lives; then `## Where the data comes from`, `## The look-through basis` (the states, and coverage/staleness folded in here rather than given their own thin section), `## When we refuse to compute it` (the eligibility predicate and the coverage floor, with the measured international-fund example — the section that earns the doc), `## Exposure and concentration on a look-through basis` with the `### Worked example` nested **under it** (the arithmetic belongs under the axis it exemplifies, as `tax-estimate.md` nests its example under `## The estimate`), `## Cost behavior` (from `financials-recovery.md` — the shared budget, the refusal backoff, the degradation path), `## What it deliberately doesn't do`, and a `See also` line pointing at `specs/bond-etf-asset-class-design.md` (currently orphaned — this gives it its inbound edge). Caveats go in **one** `>` block with two numbered items, matching the precedent's single 13-line blockquote rather than scattered quotes. Keep to ~4–5 H2s so section density matches. |
...
| 8 | `labs/trading-desk/docs/portfolio-mandate.md` | **NO CHANGE** | — | Conditional on look-through affecting mandate compliance. It does **not** — the deterministic gates stay wrapper-basis — so this file is deliberately untouched. Stated here so a reviewer reads the omission as a decision. |
| 9 | `labs/trading-desk/fixtures/README.md` | **NO CHANGE** | — | The fixture corpus addresses *analysis tool* payloads by ticker and date; portfolio data paths are deliberately off those rails, and this change lands on the portfolio side (Decision 1). This file would gain an entry only if the data went through the analysis tool catalog instead. |
| 10 | `labs/trading-desk/README.md` — the app-owned table list and the REST reads list | **NO CHANGE** | — | Both are user-facing enumerations that this change would naturally extend, and both already lag (the table list omits `realized_gains`, `tax_profiles`, and `instrument_classifications`; the reads list omits the quotes and classifications routes — precisely the two precedents this change follows). Extending them here would mean sweeping in unrelated drift, so they are left alone on the same discipline as item 3. Recorded so a reviewer can see they were considered. |
| 11 | `labs/trading-desk/eval/**` and `docs/run-quality-eval.md` | **NO CHANGE** | — | A checked negative worth stating, because a reader would reasonably fear otherwise: the eval invariant surface contains no reference to the health block, sector exposure, or concentration at all, so a new axis breaks no invariant and gates no exit code. |
...
- **Three sequencing notes, none blocking.** (1) FIX-800's spec (PR #802, on hold)
  ...
  degrade-not-throw at the tool boundary). (2) PR #912 rewrites every goal-check
