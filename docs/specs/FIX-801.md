...
4. **A fund is attributed only if it passes two gates: an eligibility predicate
   ...
   *Alternative rejected:* attribute every fund we can fetch, however thin, and let
   ...
   contract).
   Two consequences of passing the gate are *not* symmetrical, and the difference
   is load-bearing. **Single-name figures — largest name, top-N — are honest lower
   ...
   number. Its formula squares the weights, so an unattributed remainder of *r*
   moves it by up to *r²* depending on whether that remainder is one concentrated
   name or spread over hundreds — indistinguishable from the same observed profile.
   At the floor that interval is wide enough to span a factor of two on a
   diversified book, so a point estimate there would be a fabricated precision. The
   interval collapses to a tight range at high coverage and reads like a point
   estimate when the data earns it; a wide range is itself the signal that the data
   doesn't. *Also refused:* a book whose invested denominator is not a long-only net
   — the existing aggregation nets signed market value into invested NAV, so a
   short fund position shrinks the denominator and would inflate every effective
   weight above 100%. The second axis returns null for such a book, with the reason
   named, rather than reporting uninterpretable weights (§9).
...
- **Route.** Owns the fill: derives the eligible fund set from the user's own
  ...
  call will fetch (§8). It also **projects** what it returns: the browser needs a
  ticker-and-weight list, not the whole stored document, and a full-depth profile is
  a few hundred rows per fund. Send the projection, keep the document in the table.
...
7. **Health perspective UI.** Add the hook and a look-through block beside the
   ...
   **Close the price-then-profile ordering hole first — it is a real bug, not a
   nit.** Decision 5 narrows the fetch set to *priced* funds, and the pane fetches
   prices asynchronously after holdings load. So on a cold mount the profile route
   runs before any quote exists, skips every fund as unpriced, and returns nothing —
   and because the query hook only re-runs when its URL changes, the profiles never
   arrive until a remount or reload. The existing price-refresh completion path
   refetches quotes only. Fix it by making profile fetching react to pricing
   becoming available: trigger the profile refetch after the awaited price refresh
   resolves (the same `await REST write → refetch` idiom that path already uses for
   quotes), or key the query so eligibility changes re-run it. *Test:* a spec that
   mounts with no quotes, resolves prices, and asserts the profiles arrive without a
   remount.
...
| Health mounts before prices resolve | The unpriced funds are outside the fetch set, so nothing is fetched. Profiles must arrive once pricing resolves without a remount (§8 step 7) — this is the ordering hole, and it is fixed rather than tolerated. |
