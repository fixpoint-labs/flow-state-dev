...
| Effective-position count on the look-through basis | Reported as an **interval**, never a point estimate (Decision 4): the unattributed remainder *r* can sit anywhere between a very long tail and piling onto the largest already-observed name, so `Σwᵢ² ≤ HHI ≤ Σwᵢ² + 2·wₘₐₓ·r + r²` and the count lies in `[ 1 / (Σwᵢ² + 2·wₘₐₓ·r + r²) , 1 / Σwᵢ² ]`. Note the width is **linear in *r*** (not quadratic) whenever `wₘₐₓ > r/2` — the overlap term dominates, so even high coverage leaves a visible range. Largest-name and top-N stay point estimates — they are genuine lower bounds. |
