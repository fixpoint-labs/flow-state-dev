...
7. **A `tool:` participant must resolve to a deterministic block — enforced at the block's directly-detectable surface.** `ToolCatalog` stores arbitrary `BlockDefinition`s, so a catalog key could point at a `generator`, which would take a model turn and make the feature's "no LLM turn" promise false. So `tool:` is *constrained*: the resolved catalog block is rejected if it is a `generator`, and — for a `router` — if its **directly-inspectable** `config.routes` exposes a generator branch. — *Enforcement boundary (honest about what core exposes):* the check runs where the tool resolves — at `library.ts` build-time for static skills and in `materializeWorker` for runtime activations — by inspecting the resolved block's own `kind` (reject `generator` outright) and a router's `config.routes` (which *is* introspectable). It does **not** promise a deep walk into a `sequencer`'s children: `createSequencer` closes its steps into the block's `execute` and retains only ordinary config fields (unlike a router, whose `config.routes` are exposed), so core offers no public API to enumerate a sequencer's composed leaves. Enforcing "no generator anywhere in the tree" for a sequencer tool would force an implementer to add an unscoped public-introspection API, reach into internals, or leave it unchecked — none acceptable for 925. A deep-traversal guarantee is **explicitly out of scope** and would need its own public-traversal API to be earned. — *Alternative rejected:* allow any catalog block and downgrade the promise to "no *wrapper-agent* turn" wholesale. Weaker than necessary: the directly-detectable check already catches the common case (a bare generator, a router branching to one). — *Ramification:* the "no model turn" goal-check (§10) holds **at the directly-detectable surface** — a `tool:` cannot be a generator or a generator-branching router. A generator buried inside a sequencer that a skill author deliberately registers as a tool is not statically caught by 925 (the author owns that misuse; a follow-up public-traversal API could close it). A node that needs a model turn stays an agent (`prompt`/`prompt-ref`/`agent-ref`).
...
| `tool:` names a catalog key that resolves to a **generator** (or a `router` branching to one) | Throw at both build-time and materialization with a pointed error (Decision 7), detected at the block's directly-inspectable surface (`kind` / router `config.routes`). A generator buried in a `sequencer`'s closed-over steps is *not* statically caught — core exposes no traversal for it, so 925 makes no claim there (out of scope; the author owns that misuse). A node that needs a model turn stays an agent. |
...
- **Goal & goal check (real path).** A delegation skill declaring one `tool:` participant and one agent, run via `fsdev run` (or the orchestration integration harness): assert the tool task completes with the tool's output and **no model turn is emitted for it**. The non-generator constraint (Decision 7) makes this a guarantee at the directly-detectable surface — a `tool:` cannot resolve to a generator or a generator-branching router (a generator buried in a sequencer's closed-over steps is out of scope; core can't detect it); verify via trace/items that the deterministic node produced no model call. This is the outcome a user cares about.
