...
| `tool` spec sets `model`/`context-supply`/`visibility`/inline `tools` | Parse-time throw (agent-only fields on a tool spec); re-checked at materialization, since a persisted `PatternBinding` can bypass the parser (step 3, mirroring the FIX-920 `contextSupply` guard). |
