...
4. **BP-007 (concise API docs) + fail-loud parsing.** The parser already rejects `block-ref` and inline-tuning-on-`agent-ref` with pointed errors. A `tool` spec that carries agent-only fields (`prompt`, `model`, `context-supply`, inline `tools`) must fail the same way, not silently ignore them.
...
5. **A `tool` spec rejects agent-only fields at parse time.** `prompt`/`prompt-ref`/`agent-ref`/`agent-overrides`/`model`/`context-supply`/inline `tools` alongside `tool:` fail loud. — *Alternative rejected:* silently ignoring them. — *Ramification:* mirrors the existing `agent-ref`-tuning rejection; `tool` joins the mutually-exclusive resolution set (now four, still exactly-one).
6. **An unknown tool key fails loud at materialization.** A `tool:` naming a catalog key that doesn't resolve throws for a static skill (build-time) and is warn-skipped for a runtime activation — the same loud/soft split agent materialization already uses. — *Ramification:* a bound-but-broken tool participant can't silently vanish from the board.
...
**Size:** Medium (multi-file, single PR, ~150–300 LOC across core types, the frontmatter parser, the worker materializer, and the roster/guidance prose, plus tests). Single-node PR plan.
...
flowchart TD
  ...
  M -->|"tool branch"| TW["tool adapted:\ntool.connectInput(env =&gt; env.input)"]
...
**Adapter seam** (illustrative — a sketch, not the contract). A declared tool becomes a worker whose input is the substrate's `TaskWorkerInput`, so the board's own `packWorkerInput` wrap composes on top of it:

```ts
// in the materializer's tool branch
const tool = deps.catalog[spec.tool];            // resolve; throw/warn if missing
return tool.connectInput<TaskWorkerInput>((env) => env.input);
```

The board then wraps this with `entry.connectInput<Task>(task => packWorkerInput(...))`, yielding `Task → TaskWorkerInput → tool args`. The tool's native `outputSchema` flows straight to `record-result`.

### 8. Implementation sequence

1. **Core type — add the `tool` resolution kind.** In `@flow-state-dev/core`'s skill types, add `tool?: string` to `AgentSpec`; update the doc comment on `AgentSpec` (currently says "there is no `blockRef`… a tool is a follow-up" — this *is* that follow-up). Frame the `agents:` map as the board's participant registry. *Test:* type-level — a `tool` spec is assignable; no runtime test here.
2. **Frontmatter parser — parse and validate `tool`.** In the skills SKILL.md parser: add `tool` to the known-keys set and to the mutually-exclusive resolution set (now `prompt` / `prompt-ref` / `agent-ref` / `tool`, exactly-one). Reject agent-only fields (`prompt`*, `model`, `context-supply`, inline `tools`, `agent-overrides`) on a `tool` spec with a pointed error, mirroring the existing `agent-ref` tuning rejection. *Test:* a `tool:` entry parses to `{ tool: "..." }`; a `tool:` with `model:` throws; two resolution fields throw.
3. **Worker materializer — the `tool` branch.** In `materializeWorker`, dispatch a `tool` branch (before or alongside the existing three) that resolves `spec.tool` against `deps.catalog`, throws on miss (static) / warns (runtime, matching the surface's existing loud/soft split), and returns the catalog tool adapted via `connectInput<TaskWorkerInput>(env => env.input)`. *Test:* a resolved tool worker, given a `TaskWorkerInput` with `input: X`, invokes the tool with `X` and returns its output; an unknown tool key throws.
4. **Delegation surface — roster + guidance.** In `delegation-surface.ts`: `agentPurpose` gains a tool branch (describe the tool, e.g. from its catalog description, as a deterministic participant); the roster line marks it a tool so the coordinator knows to pass structured `input` and not expect prose reasoning. Nudge the `DELEGATION_PLAYBOOK` prose from "assignee names an agent" to "assignee names an agent or a tool." The `boardWorkers` registration loop already handles any materialized block — confirm a tool entry flows through unchanged, including the per-execution memo snapshot (keyed on participant keys via `Object.keys(agents)`, which already includes tool keys). *Test:* a skill mixing an agent and a tool lists both in the roster with the right kind; the memo snapshot includes the tool key.
...
|---|---|
| `tool:` names a catalog key that doesn't resolve | Static skill: throw at materialization (fail loud). Runtime activation: warn + skip that participant (don't crash the turn) — matches the agent path. |
...
| `tool` spec sets `model`/`context-supply`/inline `tools` | Parse-time throw (agent-only fields on a tool spec). |
