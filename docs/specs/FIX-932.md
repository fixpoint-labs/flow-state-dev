...
**Deliverables.** A rewritten authoring guide; three corrections plus a Related section on the
delegation reference page; three inbound links from the pages readers actually arrive on; one
sidebar reorder; one package-README link; an empty changeset.
...
3. **Three claims on the delegation pages are stale or absent** against merged code. Most
   usefully: a failed dependency strands its dependents and `runBoard` returns `blocked`. The
   substrate page documents that mode; the delegation pages never connect it to `runBoard`.
...
- **Verify before writing — including this spec's own claims.** Three claims in this spec's
  drafts were overstated and caught by validation and review, not by the author. §8 step 1 is
  not a formality.
...
4. **The altitude rule: the guide names behaviors and the fields an author types (`agents:`,
...
6. **Correct three stale-or-absent claims on the delegation pages:** (a) a failed dependency
   strands its dependents and `runBoard` returns `blocked`; (b) `docs/skills/delegation.md`'s
   closing line still calls conversation inheritance a future direction, though
   `context-supply: conversation` has shipped; (c) the roster blurb is derived — an `agent-ref`
   agent renders as `` agent `<ref>` `` and its registry `description` is not used.
   *Alternative rejected:* file these as separate docs bugs.
   *Ramification:* the change fixes the reference page, not just the guide. This is what makes
   the guide true rather than merely present.

7. **FIX-641 (dynamic worker identities) is deliberately excluded. No `identity` concept
   appears anywhere.**
...
**Claims that are wrong or silent today** — what the change must fix. Everything else about the
surface is already documented correctly; step 1 re-verifies rather than re-litigates. Where a
shipped goal check already exercises the behavior, it is named so a reviewer can spot-check fast.

| Claim | The truth on `main` | Evidence |
|---|---|---|
| Failed dependency | Dependents are not skipped — `cascadeSkipDependents` isn't wired into the delegation drain. They stay pending; `runBoard` returns `status: "blocked"`. The substrate page documents the mode; the delegation pages never connect it to `runBoard`. | `delegation-surface.ts` settle step |
| Conversation inheritance | `docs/skills/delegation.md`'s closing line still calls it a future direction. `context-supply: conversation` has shipped and has its own reference page. | `docs/orchestration/context-supply.md` |
| Roster blurb, and agent metadata | **Scope this to the two staffing paths separately.** An *inline* SKILL.md agent has no `name` or `description` field — unknown keys throw — and its roster blurb is its persona's first non-blank line, truncated. A *registered* agent requires both `name` and `description`, yet `agentPurpose` renders it as `` agent `<ref>` `` and does **not** use the registry description. Stating "agents have no description" unqualified is wrong for half the surface. | `core/src/types/agent.ts`; `delegation-surface.ts` `agentPurpose` |
| The two caps behave differently | `enqueued_task_cap_exceeded` is pending-only and refreshes as the board drains — drain, then continue. `total_task_cap_exceeded` is cumulative with no refund — draining recovers nothing, so the coordinator must finish with a smaller plan. Teaching one recovery for both would teach an ineffective retry loop. | `tasks/collection/task-caps.ts`; `DELEGATION_PLAYBOOK` in `delegation-surface.ts` |
| Assignee rejection is roster-conditional | "An unknown assignee is rejected at `addTask`" holds **only when a non-empty declared roster exists.** On a rosterless `delegation: true` board, `buildTaskToolsList` gets no roster, `checkAssignee` accepts any name, and the default worker absorbs the registry miss. Decision 5 puts the rosterless path in the guide, so the guide must not state the rejection rule unconditionally. | `task-tools-capability.ts` `checkAssignee`; `worker-step.ts` fallback |
| **Two "blocked" stories** | Tutorial §7 teaches a code-first board's `terminationReason: "blocked-by-failures"`. The delegation `runBoard` reports `status: "blocked"`. Different surfaces, same word — a reader meeting both will conflate them. **Resolution: the authoring guide's §7 names the delegation one and cross-links the tutorial's §7; the tutorial is not edited** (decision 2). | tutorial §7 |
...
1. **Verify first.** Re-read the delegation surface, the task-tools capability, and the skill
   frontmatter parser on `main`; confirm the four claims above plus every field name the guide
   uses. Anything that disagrees with this spec is evidence the spec missed something — surface
   it. *Test:* the claim table in the PR description, each row naming the module (and goal check,
   where one exists) that proves it.
...
| `apps/docs/docs/skills/delegation.md` | EXTEND + CORRECT — the three §7 corrections, coordinator vocabulary, guide pointer after the lead, new `## Related`. |
