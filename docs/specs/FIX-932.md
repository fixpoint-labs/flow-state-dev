...
4. **The guide teaches against one named skill — `research-company` — and says plainly that the
   ...
   *Ramification:* the guide's **walkthrough** — declaring a team, planning the graph, draining —
   runs on `research-company` throughout, and the runnable-example admonition carries a one-line
   caveat about the shared `chat` action. The per-skill action is a follow-up (§12).
   One carve-out — the walkthrough is `research-company`, but the **staffing section** takes its
   registry example from `competitor-analysis`, because `research-company` has no `agent-ref`. See
   the constraints note below §8.
...
7. When it goes wrong       — three distinct outcomes; see the constraints note below §8
...
synthesizer), with `competitor-analysis` + `src/agents.ts` carrying the registry path.
**Two limits the guide must not paper over:** its test suite covers the code-first and router
actions but **not** the delegation path (§10), and the keyless claim does **not** extend to that
path — the deterministic `research` / `researchCompetitors` actions run with no API key, but the
model-planned `chat` action needs one (`OPENAI_API_KEY=… pnpm fsdev run research-team chat`).
Surface it with the `:::tip Full, runnable code` admonition the tutorial
...
3. **Correct every writer named in §7 — this is two reference pages, not one.**
   `docs/skills/delegation.md`: the cap-recovery contradiction, the stale "Where fork went"
   paragraph, the stranded-dependents addition, the coordinator vocabulary, a pointer to the guide
   after its lead, and a `## Related` section (it has none).
   `docs/orchestration/task-board.md`: the same cap-recovery contradiction in its own words, and
   the rosterless carve-out on its unconditional assignee-rejection sentence.
   `docs/orchestration/agents.md`: half a sentence noting `description` doesn't feed the roster
   blurb.
   Leave `packages/orchestration/README.md` alone on the caps — it is already correct.
   *Test:* re-run the §8.1(b) sweep afterwards and get no hits.
...
### 8b. Constraints carried to implementation

Three facts settled during spec review that the implementer must honour. They are stated as
constraints, not as drafted prose — the wording is the implementer's, and the PR reviews the
actual text.

1. **The failure section splits three outcomes; do not collapse them into one sentence.** With a
   non-empty declared roster, an unknown `assignee` is **rejected at `addTask`** (a soft tool
   error, no task created). An **unset** `assignee` is not an error at all — the task runs on the
   default worker. Only tasks stranded behind a **failed dependency** cause `runBoard` to return
   `status: "blocked"`. Three different mechanisms, three different reader reactions.
2. **Staffing takes its registry example from `competitor-analysis`, not `research-company`.**
   The walkthrough is `research-company` (decision 4), but it declares only inline `prompt-ref`
   agents — it has no `agent-ref` and cannot demonstrate the registry path.
   `competitor-analysis` is staffed both ways deliberately and says so in its own body. One
   sentence and a link in the staffing section; not a second walkthrough.
3. **"Runs without an API key" is true only of the deterministic actions.** `research` and
   `researchCompetitors` use handler workers and need no key; the model-planned `chat` action —
   the path this guide teaches — does. Never advertise the delegation path as keyless. This claim
   also goes through the §8.1(b) writers sweep: the spec asserted it in more than one place, and
   so may the corpus.
...
- **After spec review (codex, round 4) — and the spec closes here.** Three findings, resolved as
  **constraints** (§8b) rather than more spec prose: the three distinct failure outcomes, the
  registry example moving to `competitor-analysis` (round 3's one-named-skill rule collided with
  the staffing section, since `research-company` has no `agent-ref`), and the keyless claim being
  false for the `chat` path the guide teaches. The judgment call behind the format: by round 4 the
  findings had moved from falsifying a decision to tightening wording in a directional outline,
  and one of them was a defect the previous round's fix introduced. Part II is meant to carry shape
  and sequence, not finished prose — so the remaining precision is named for the implementer and
  reviewed on the real text, not litigated here. **Direction approved; the spec is final.**
