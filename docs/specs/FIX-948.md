...
**Where the genuine complexity is.** Not the counting. It is (a) keeping the routing decision atomic — the existing `fail()` reads the task *outside* the CAS and then writes, which for a board-wide sum is a real race, and (b) making sure the model-facing `failTask` tool doesn't advertise a retry the collection is about to refuse. Both are §7.
...
2. **Decide inside the atomic write (BP-035, concurrent path).** A board-wide sum read before the write and acted on after it overshoots under concurrency. The existing per-task path has this shape already; the budget makes it matter.
...
1. **The budget counts cumulative *retries*, not cumulative attempts.**
   *Alternative rejected:* budgeting `Σ attempts`.
   *Ramification:* the number means exactly what its name says, it is independent of `maxTotalTasks` (so no cross-cap ordering rule is needed), and **it can never refuse a first attempt** — by construction it only ever converts a retry into a terminal failure. `Σ attempts` would have entangled the new knob with the task count and needed a `>= maxTotalTasks` validation rule to stay sane.
...
2. **The budget lives on the collection, beside the two creation caps — not on the board.**
   ...
   *Ramification:* one guardrail model instead of two. The count is derived from the ledger (`Σ` over tasks of `max(0, attempts − 1)`) exactly as `maxTotalTasks` is the map size and `maxEnqueuedTasks` the pending count, so it inherits their lifetime semantics and their one documentation section (`collection/task-caps.ts:27-71`) rather than needing a second, differently-scoped story. Decisive technical reason: `fail()` **is** a collection method and the board is not in the retry call path at all — workers and the delegation `failTask` tool call `collection.fail()` directly. A drain-loop check could only observe a storm *after* the spend, which is exactly the failure mode of the nearest existing ceiling (per-action `tokenBudget` is evaluated at `engine/src/execution/runAction.ts:1346-1376`, after the block already ran, so it reports overspend rather than preventing it). It also inherits the two known gaps of the existing caps rather than inventing a third posture: a **supplied** collection gets no budget (`task-board/index.ts:842-852` refuses cap options there), and resource-backed enforces nothing.
...
4. **The routing decision moves inside the atomic write.**
   *Alternative rejected:* keeping today's read-then-branch shape and accepting overshoot.
   *Ramification:* `fail()` currently reads the task outside the CAS and then calls `transitionTo`, which opens its own write (`collection/sequencer-backed.ts:360-362`). For a per-task `maxAttempts` that is nearly benign; for a board-wide sum it is a genuine time-of-check/time-of-use race where N concurrent failures each read the sum under budget and all retry. Bounded by `concurrency` (default 4) so the overshoot is small, but a guardrail that overshoots reports something other than what it did. Fixing it also removes a latent race on the existing per-task path — in scope by necessity, not scope creep.

5. **The bound is reported at two persisted surfaces: the task's terminal state, and the board's completion item — which gains a `retries` count and a distinct `terminationReason`.**
   *Alternative rejected:* letting a budget-exhausted board report the existing generic `"blocked-by-failures"`.
   *Ramification:* widens a public union (`task-board/blocks/board-meta.ts:109`) by one member, `"retry-budget-exhausted"`, and adds one field to `counts`. Justified by this epic's objective and its Decision 4 (a guarantee must be observable at a persisted surface): a board that burned its entire retry budget reporting the same reason as "one task failed" is the dishonest reporting the epic exists to remove. No new `TaskChangeKind` — the task genuinely errored, so the existing `"errored"` kind is correct.

6. **The default is a finite budget, with `null` as the documented opt-out — the same posture FIX-931 took.**
   *Alternative rejected:* defaulting to unbounded (opt-in).
   *Ramification:* a safety default protects boards whose authors never learn the knob exists, which is most of the value; opt-in protects almost nobody. It is a **behaviour change**: a board legitimately consuming more retries than the default starts settling tasks terminally with no call-site change. Accepted for the same reason FIX-931 accepted its own ("a deliberate safety default, not an oversight"), with the same real migration path — raise the number or pass `null`. **Proposed value: 250.** Deliberately generous against the 500-task default (half the board may fail once and retry) and three orders of magnitude below a storm. The exact number is the human's call at sign-off; the *posture* is what this decision fixes.
...
**Deriving the count.** `Σ` over the ledger of `max(0, attempts − 1)`. `attempts` increments at claim time (`collection/internal.ts:209-222`), so a task on its first attempt contributes zero — which is what makes Decision 1's "can never refuse a first attempt" hold by construction. Not a stored counter, so it inherits the existing lifetime story verbatim (`collection/task-caps.ts:27-71`); nothing new to say about persistence.

**Atomicity (Decision 4).** Today `fail()` reads via `readTasks()` and branches, then `transitionTo` opens its own `casWrite` (`collection/sequencer-backed.ts:360-362`). The branch must move inside the mutator so it evaluates against the committed map on every CAS retry — the same discipline `assertWithinCaps` follows (`:146-151`) and the same location as the existing in-mutator advisory-decline hook (`shouldDeclineTransition`, called at `:208` inside the write). Note the existing hook *declines* (no-op); this one *reroutes*, so it is the same seam, not the same hook.

*Illustrative — a sketch, not the contract:*

```ts
// one CAS write decides and applies, instead of read → branch → separate write
await casWrite((tasks) => {
  const task = ownTask(tasks, id);
  const target = retryBudgetLeft(tasks) && shouldRetryOnFail(task) ? "pending" : "errored";
  // …apply target, capture for emit
});
```

## 8. Implementation sequence

1. **Extend the cap options and defaulting.** Add the third axis to `TaskCapOptions`, its validation (positive integer or `null`, mirroring `assertCapValue`), and its default in `resolveTaskCapDefaults`. *Test:* validation rejects `0`/negative/fractional/`NaN`/`Infinity`; `null` survives defaulting; omission takes the default. No cross-cap ordering rule is needed (Decision 1).
2. **Derive the count and extend the predicate.** Add the ledger-derived retry total, and widen the routing predicate so it answers "retry or terminal" from both the task's own budget and the board's. *Test:* the predicate in isolation — budget left / budget spent / budget `null` / no `maxAttempts` at all.
3. **Enforce inside the sequencer backing's CAS.** Restructure `fail()` so the routing decision happens inside one atomic write, then wire the budget in. *Test:* a task with retry budget re-pends while the board budget has room, and settles terminal once it doesn't; concurrent failures at the boundary do not overshoot.
4. **Keep the delegation tool in step.** Thread the budget into the `failTask` target computation. *Test:* with the budget spent, the tool advertises `errored`, not `pending`.
5. **Report it.** Add `counts.retries` and the `"retry-budget-exhausted"` termination reason to the board completion item. *Test:* a budget-exhausted board reports the new reason and a non-zero retry count; a cleanly-drained board still reports `"all-completed"`.
6. **Thread the knob through the configuration path.** The seven sites in §7 — board config + `resolveBoardCaps` (declarative branch only; supplied-collection stays a construction error), the capability's two branches, `getOrCreateTaskCollection`, and `SkillsLibraryOptions` → delegation surface. *Test:* the knob reaches a declarative board's collection; passing it with a supplied collection throws; the delegation board is bounded by default.
7. **Docs and changeset** — §11, plus a `patch` changeset (BP-022).
...
| First attempt of any task | Never affected — a first attempt contributes zero retries (Decision 1). |
| Concurrent failures at the boundary | Serialize through the CAS; the budget's worth lands, no overshoot (Decision 4). |
| Board budget below the retries already spent | The next failure settles terminal. No retroactive effect on tasks already re-pended — the count is derived, not enforced backwards. |
| **Resource-backed collection** | **Not enforced**, same boundary as the existing caps. Not an oversight: the check must be atomic against the whole ledger, and the resource layer has no CAS across instances (the existing caps are absent there for the same reason — `task-caps.ts:52-65`, and grep confirms `resource-backed.ts` has no cap enforcement at all). Follow FIX-931's precedent and keep the option off `ResourceBackingSpec` so asking for it there is a compile error rather than an accepted-and-ignored ceiling. Deferred to FIX-939 / FIX-917. |
...
| Other re-entry paths (`unblock`, `resumeFromReview`, `reclaim`) | Not bounded (non-goal). Note the interaction: `reclaim` returns a task to `pending` *without* touching `attempts` (`internal.ts:100-108`), and the next claim increments — so a reclaim loop does consume this budget, which is correct, since each reclaimed attempt is a real model call. |
...
**Second-path checklist (BP-035).** *Concurrency* — Decision 4 moves the check into the CAS; tested at the boundary. *Legacy/persisted shape* — no new persisted field on the task; the count is derived, so no dual-read is needed (BP-030 does not bite). *Off/new state* — every test runs the budget unset, `null`, and set. *Multi-tenant* — the budget is per collection, i.e. per board, unchanged from the existing caps. *Cost observability* — the whole point; §10 asserts the reported retry count. *Cancel/error path* — a cancelled task keeps its accrued attempts in the sum, which is correct (the spend happened).
...
- A first attempt is never refused, at any budget value.
- Concurrent failures at the boundary land exactly the budget's worth (no overshoot).
...
- Configuration: validation rejects nonsense; `null` is unbounded; omission takes the default; the option with a supplied collection throws; the delegation board is bounded by default.
...
3. **EXTEND `packages/orchestration/README.md`** — public option surface changed (BP-009).
...
**Also worth stating plainly, because the epic-spec names it and the original collision check did not:** FIX-976, FIX-963, and FIX-964 are a constraint-sharing group that all touch `sequencer-backed.ts`, `resource-backed.ts`, and `task-tools-capability.ts` (epic-spec, "A1 is a shared constraint"). **This change touches all three of those files too**, plus `internal.ts`. That is **file** collision, not scope collision — none of them changes retry routing — but it is a real rebase surface, and the epic's A1 rule (only FIX-976's `setAssignee` may guard the patch surface; whichever lands first must not leave a helper-level guard behind) applies to whoever sequences these. This is the kind of thing `cross-spec-review` exists to catch, and it now can run: this spec's existence was the gate (epic-spec `:557`).

**Open question (needs a decision at sign-off, not before implementation).**

- **The default budget value.** §6 Decision 6 proposes **250** and fixes the *posture* (finite default, `null` opt-out, accepted behaviour change). The number itself is a judgement call about how much unsanctioned retry spend is tolerable, and it is the human's. Options: `250` (proposed — generous against the 500-task default, three orders below a storm); a higher number like `1000` (near-zero breakage, weaker guard); or `null` (no behaviour change, but then the guardrail protects only authors who go looking for it, which forfeits most of the value).
