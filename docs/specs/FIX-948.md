...
9. **The delegation-facing option is dropped: `maxTotalRetries` is *not* added to `SkillsLibraryOptions`, because a delegation task cannot retry at all today.**
   *Alternative rejected:* mirroring FIX-931's placement and shipping the knob there anyway; also rejected (for now) adding retry support to the delegation surface inside this issue.
   *Ramification:* **This corrects a reach claim the earlier drafts made, and it is the most consequential change of the implementability pass.** The delegation `addTask` tool's schema does not accept `maxAttempts` and its write does not stamp one (`skills/task-tools-capability.ts:349-359,382-389`), and `shouldRetryOnFail` returns `false` whenever the field is absent (`collection/internal.ts:83-85`). So **every task created through the delegation surface can never retry**, and a budget there would have no reachable subject — an inert option, and the same defect class as FIX-989's "coverage claimed for 16 methods, delivered for 2." FIX-931 put its cap knobs on `SkillsLibraryOptions` because creation-time caps genuinely bind there; retries do not, so copying the placement copies the syntax without the semantics. What this locks in: the bound's real subjects are boards whose tasks are given a `maxAttempts` by app code (`initialTasks`, direct `addTask`) and replanned tasks (`task-board/apply-replan.ts:207`, where `maxAttemptsPerTask` defaults to `1` at `goal-seek-loop.ts:266` — itself no retries — so it binds only when an author raises it or a replanner stamps its own). Adding retry support to the delegation surface is a **separate issue**, deliberately not folded in: it would mean *adding* a retry capability inside the issue whose purpose is *bounding* one, and it is entangled with FIX-993's validation question (§12). **Flagged for the owner** — see §12; if the answer is "the delegation option must ship," this issue grows and re-sequences.

**Non-goals.** Re-entry enforcement inside the existing creation caps (rejected in FIX-931, not reopened). Adding `maxAttempts` support to the delegation `addTask` surface (Decision 9 — a separate issue). Validating `maxAttempts` (a separate defect — §12). A cost/spend ceiling (recommendation only — §12). Enforcement on resource-backed collections (out of scope for the same reason FIX-931 scoped it out — §9). Bounding the other re-entry paths: `unblock`, `resumeFromReview`, `reclaim`.
...
**Deliberately *not* the remaining two.** `SkillsLibraryOptions` (`skills/library.ts:109`) and the delegation surface (`skills/delegation-surface.ts:143`) carry the two creation caps but **must not carry this one** — per Decision 9, a delegation task cannot retry, so the option would be inert there. This asymmetry with FIX-931 is the point, not an oversight, and §11 requires it to be stated where a reader would otherwise assume parity.
...
7. **Thread the knob through the configuration path.** The **five** sites in §7 — board config + `resolveBoardCaps` (declarative branch only; supplied-collection stays a construction error), the capability's two branches, and `getOrCreateTaskCollection`. **Deliberately not** `SkillsLibraryOptions` or the delegation surface (Decision 9). *Test:* the knob reaches a declarative board's collection; passing it with a supplied collection throws; and a type-level assertion that the option is **absent** from `SkillsLibraryOptions`, so a later well-meaning "consistency" change can't quietly reintroduce an inert knob.
...
| **Task created through the delegation `addTask` tool** | Cannot retry at all — the tool neither accepts nor stamps `maxAttempts` (`task-tools-capability.ts:349-359,382-389`) and the predicate returns `false` without it (`internal.ts:83-85`). So the budget has no subject on that path, which is why the delegation option is dropped (Decision 9) rather than shipped inert. |
...
- **The option is absent from `SkillsLibraryOptions`** (type-level), so the inert-knob correction can't silently regress.
...
- **Should this ship at all while a delegation task cannot retry?** Decision 9 drops the inert delegation option and ships the board/collection bound, which is implementable today and protects the paths that *can* retry (app-authored `maxAttempts`, `initialTasks`, replanned tasks). But it means **the delegation board — the path the issue's framing leant on — was never a retry-storm vector**, because its tasks cannot retry. Three ways to go:
  ...
  **The awkward shape, stated plainly because it may drive the answer:** the surface that *can* retry today (the replanner, `apply-replan.ts:207`) is the one with **no validation** on `maxAttempts` — the FIX-993 POC subject — while the surface that *has* schema validation (the delegation tool's zod input) **cannot retry at all**. So validation and reachability are on opposite sides. If (b) is chosen, adding `maxAttempts` to the delegation schema would land it *with* validation, which is why (b) and FIX-993 are naturally one piece of work rather than two.
