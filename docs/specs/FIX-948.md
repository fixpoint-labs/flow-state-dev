...
1. **The budget counts *authorized failure retries*, recorded by a per-task counter incremented in the same CAS write that re-pends the task.**
   *Alternative rejected:* deriving the count from `attempts` (`Σ max(0, attempts − 1)`), which the first draft proposed. **Rejected because it is doubly wrong, verified in code**, not because it was inelegant:
   - **It lags the decision it gates.** `attempts` is written in exactly two places — `collection/internal.ts:59` (init) and `:217` (`applyClaimToTask`) — and the retry patch at `collection/sequencer-backed.ts:399-409` does not touch it. So the retry being authorized contributes **zero** at the moment `fail()` decides. With `maxTotalRetries: 1`, two first-attempt failures both read a total of zero and **both** re-pend. This is not merely a race: it is a systematic lag that defeats the bound even when the two failures are strictly sequential, so no amount of atomicity fixes it.
   ...
   **The retry facts live in ONE new public `Task` field, not two.** The write that authorizes a retry records the grant, and the write that refuses one records the refusal — both belong to the same concern and are written at the same seam, so they are one field holding both (a granted count and a denied-by-budget flag), not two top-level additions. That keeps the public surface to a single addition, gives BP-030 one object to `== null`-guard instead of two fields, and keeps the denial fact structurally branchable rather than buried in `metadata` or parsed out of error text — which matters because the whole reason for a denial fact is that `terminationReason` must not be inferred (Decision 5). §7 specifies the contract. **If review judges even one field too much surface for the termination reason it enables, the recorded fallback stands: drop `terminationReason` rather than infer it** — the bound itself does not depend on it.
   **Counting begins at upgrade, and the guarantee is scoped to say so.** The budget is a lifetime bound over the ledger's *counted* history, and a task persisted before this change carries no counted history — so it starts at zero and the reported count means **authorized retries since the upgrade**, not since the task was created. This is stated rather than papered over, and §7 explains why the exposure is narrow: the two backings that *enforce* can only hold pre-upgrade records across a suspend/resume spanning the deploy, and the backing whose records are genuinely long-lived does not enforce at all.
...
5. **The bound is reported at two persisted surfaces: the task's terminal state, and the board's completion item — which gains a `retries` count, the **limit actually in force**, and a distinct `terminationReason`.**
   ...
   **The reported limit comes from the collection, not the board — otherwise a supplied collection reports a confident wrong `null`.** `resolveBoardCaps` returns `{}` for a supplied collection (`task-board/index.ts:851`), and the completion block receives only the ref and the collection id (`board-meta.ts:87-88`), so a board asked to report *its* configured limit knows nothing about a collection it did not construct. But a caller can build one that genuinely enforces — `getOrCreateTaskCollection({ backing: "request", maxTotalRetries: 5, … })` — and reporting `null` there would state "nothing was enforced" about a limit the caller set themselves. That is the exact false report the limit-beside-count design exists to prevent, in the one case where the caller was most deliberate. So the resolved limit is **exposed on the collection ref** (a readonly value, not a method) and the reporter reads it from the same place enforcement lives, which is also what Decision 2 already argues for. `null` then means one thing everywhere — *no limit is in force* — including on resource-backed refs, which do not enforce (§9), so no third "unknown" state is needed. Acknowledged cost: it adds to `TaskCollectionRef`'s surface, which epic **OQ-D** already flags as over-large; a readonly data property is the lightest form that addition can take, and the alternative is a reporter that guesses.
   **The termination reason is derived from a persisted *denial* fact — carried by Decision 1's single new `Task` field — never from arithmetic.** `retries === limit` does **not** establish that anything was refused: a task can consume the last grant and then succeed, while an unrelated task carrying no `maxAttempts` fails normally — leaving the count at the limit, an errored task on the board, and **no retry ever denied**. The completion block only sees the final collection snapshot (`task-board/blocks/board-meta.ts:97-110`), so it must read an authoritative marker recorded on the task in the same write that denied it (Decision 1's write), not infer one. On this epic a termination reason that can lie is worse than no new reason at all — if the marker proves too costly, **drop the reason rather than infer it.**
...
8. **The option is exposed exactly where tasks can actually retry — and nowhere else.** That means **adding** it to `supervisor`, `planAndExecute`, and `parallelTasks`, and **not** adding it to `SkillsLibraryOptions` or `eventActors`.
   ...
   *Ramification:* **the earlier drafts had the reach precisely backwards, and this is the correction.** One rule decides every surface: *can a task on this board carry a `maxAttempts`?* If yes the knob is required, because the new finite default applies whether or not it is configurable. If no, the knob would be inert and is refused. Swept rather than inferred (`grep` for `taskBoard(` across `packages/*/src`), which found **four** board-building patterns, not the two under review:
   ...
   | delegation (`SkillsLibraryOptions`) | No — `addTask` neither accepts nor stamps `maxAttempts` (`task-tools-capability.ts:448-458,481-488`), and the predicate returns `false` without it (`internal.ts:88-90`) | Refused (inert) |
...
> **Citations are as of FIX-976 landing (`c9fe58a1`, PR #1004), which reshaped this spec's exact seam while it was in review.** It renamed `shouldDeclineTransition` → `transitionDeclineReason` (`collection/internal.ts:143`), made `transitionTo` and `fail()` return a `TaskWriteOutcome` instead of `void`, and shifted line numbers throughout `sequencer-backed.ts`, `resource-backed.ts`, `internal.ts`, and `task-tools-capability.ts` — the four files this change also edits. Every `file:line` below was re-read against that commit. **Still read the current file rather than trusting an offset**: FIX-963 and FIX-964 are open against the same files and will move them again.
...
**The convergence point (tenet 5).** The routing predicate `shouldRetryOnFail` (`collection/internal.ts:88-91`) is already the single place retry-vs-terminal is decided, and it already has **four** consumers. All four must pass the budget, and listing them is the point:

| Consumer | Location | What it does today |
|---|---|---|
| Sequencer-backed `fail()` | `collection/sequencer-backed.ts:398` | routes to `"pending"`/`"retried"` or terminal `errored` |
| Resource-backed `fail()` | `collection/resource-backed.ts:389` | same routing, separately maintained copy |
| Delegation `failTask` tool | `skills/task-tools-capability.ts:210` | computes the target it *advertises* to the model |
| (the predicate itself) | `collection/internal.ts:88` | the rule both backings are kept in lockstep by |

**The configuration path** — **five** of the seven sites the two existing caps thread through, so the new axis is a field addition rather than new plumbing: `TaskCapOptions` and `resolveTaskCapDefaults` (`collection/task-caps.ts:118-218`), the sequencer backing's option read (`collection/sequencer-backed.ts:90`), `getOrCreateTaskCollection` (`collection/get-or-create.ts:168,183`), `TaskBoardConfig` + `resolveBoardCaps` (`task-board/index.ts:327-340,832-863`), and the board capability's two constructing branches (`task-board/capability.ts:229,268`).
...
Gate both the accounting and the denial on the task being in an attempt-owned status, reusing FIX-951's `ATTEMPT_OWNED_STATUSES` (`collection/internal.ts:99`) rather than writing a second list — those two statuses *are* the legal-reroute set, which is why the reuse is exact rather than coincidental. Without this gate the CAS throws `IllegalTaskTransitionError` on a reachable path instead of settling as promised. Note the failure mode differs by caller and both are wrong: the delegation tool boundary catches the illegality by type and returns a recoverable refusal (`task-status.ts:56-66`), so a model would be told it cannot fail a task it legitimately can; **every other caller — a drain's rescue path, user code driving the collection — gets the throw.**
...
Why not `attempts`, restated here because it is the natural instinct and it fails: `attempts` is written at `collection/internal.ts:59` (init) and `:217` (claim) and **nowhere else** — grep-verified. Two consequences, both fatal to the guarantee:
...
Keeping the total *derived* is what preserves the parallel with the two existing caps. Recording it per task is also honest reporting at the task level for free — "how many times was *this* task retried" becomes answerable, which `attempts` never did cleanly.

**The new public `Task` field — one field, two facts, a real contract.** Both facts are written at the same seam inside `fail()`'s CAS: a **granted count** (incremented when a retry is authorized) and a **denied-by-budget flag** (set when one is refused). They ship as a single new field on the public `Task` shape rather than two top-level additions.

- **Schema.** Added to `tasks/schema/task.ts` beside `attempts`/`maxAttempts`, optional at the type level so pre-change records parse. Both members carry the same validation discipline as their neighbours (`maxAttempts` is `z.number().int().positive().optional()` at `:39`) — the granted count is a nonnegative integer.
- **Legacy / default handling (BP-030).** Absent reads as "no counted history": zero granted, not denied. One `== null` guard on the object rather than a guard per member, which is a concrete reason to prefer one field over two. Never `undefined`-arithmetic into `NaN` in the ledger sum. Retirement condition is the §12 follow-up.
- **Docs (BP-007 + the same-change-set rule).** It is a public field, so it is documented in `packages/orchestration/README.md` alongside `attempts` in this same change (§11) — including that the count means *authorized* retries and that the denial flag is what `terminationReason` reads.
- **Why not `metadata` or the error string.** `metadata` is caller-writable, so a denial fact living there is forgeable and removable — the same defect FIX-931 rejected park-and-promote for (its marker would have lived in caller-writable `labels`). Error text is not structurally branchable. The denial fact exists precisely so the termination reason cannot be inferred; hiding it in either place would undo that.

**The rollout epoch, and why the exposure is narrow.** Treating an absent field as zero means a pre-change task that had already retried gets a fresh budget after deployment, and the reported count covers only post-upgrade grants. Rather than backfill from `attempts` — which would reintroduce exactly the non-retry re-claims Decision 1 rejected, and pollute the *reported* number with them — the scope is **stated**: counting begins at upgrade. What makes that acceptable rather than a hole is that the enforcement gap and the legacy-data gap barely overlap:

| Backing | How long can a pre-upgrade record live? | Enforces? | Net exposure |
|---|---|---|---|
| `request` | Dies with the request (`task-caps.ts:34-36`) | Yes | Effectively none |
| `sequencer` | Only across a suspend/resume spanning the deploy (`:37-40`) | Yes | One budget's worth, once, in a narrow window |
| `resource` | Indefinitely | **No** (§9) | Report-only — and the item already declares `limit: null` there |

So where the budget is enforced, legacy records are short-lived; where records are long-lived, nothing is enforced. The one real case — a sequencer board resumed across the deploy — overshoots by at most one budget, once. Say this at the option's docs and in the changeset rather than leaving "lifetime" to be read as "since the task was created."

**Three backings, two implementations — enumerated, not assumed.** This spec has already been bitten once by fixing one path, so the count is traced through all three rather than the two in view. `get-or-create.ts` routes **`backing: "sequencer"` (`:161`) and `backing: "request"` (`:174`) into the *same* `createSequencerBackedTaskCollection`** — request merely passes `requestStateRef(ctx.request)` as the state ref. Only **`resource` (`:188`)** is a separate implementation with its own `fail()` and its own retry patch (`resource-backed.ts:376-415`). So:
...
**Atomicity (Decision 4).** Today `fail()` reads via `readTasks()` and branches, then `transitionTo` opens its own `casWrite` (`collection/sequencer-backed.ts:397-399`). The decision *and* the increment must move inside one mutator so both evaluate against the committed map on every CAS retry — the same discipline `assertWithinCaps` follows (`:150-155`).

**Ordering is a correctness requirement, not a style preference — get this right before anything else.** The board's failure write-back calls `fail()` with **both advisory guards**, `{ ifAllowed: true, expectAttempt }` (`task-board/blocks/record-result.ts:105-108`), which is how a worker's failure arriving *after* its lease was reclaimed or a newer attempt claimed the task is discarded. FIX-976's `transitionTo` evaluates `transitionDeclineReason` **first inside the mutator** (`sequencer-backed.ts:219`), before `assertTransitionAllowed` and before `patch(task)` runs — so a declined write patches nothing and emits nothing.

The restructured `fail()` **must preserve that ordering**: guards evaluated before any retry fact is written, in the same CAS. If a grant or a denial is recorded before the stale write is declined, then a late failure from a displaced worker **silently spends another task's retry allowance, or writes a false denial marker** that makes the board report `retry-budget-exhausted` when nothing was refused. Both are exactly the mis-reporting this epic exists to remove, and neither is visible in a test that only checks the task's status.

This is now **shipped behaviour to conform to rather than a parallel design** — FIX-976 merged (`c9fe58a1`) with its own coverage at `test/collection/advisory-write.test.ts`. Two consequences:

- **A declined `fail()` is a complete no-op.** No status change, no grant, no denial marker, no `task-change` event. §10 asserts all four.
- **A budget denial is a `recorded` outcome, not a new one.** The write happened — the task really did go terminal — so it reports `outcome: "recorded"` through FIX-976's existing `TaskWriteOutcome`. **Do not add an outcome member for it**; the denial fact rides the task field (§7 above), and the reason a caller reads is the board's termination reason.

*Illustrative pseudocode — a sketch of the ordering, not the contract:*

```
casWrite(tasks):
    task <- own(tasks, id)
    reason <- transitionDeclineReason(task, target, guards)   # FIRST — before any write
    if reason: return DECLINED                                # nothing recorded at all
    granted <- retryBudgetLeft(tasks) and retryAllowed(task)  # attempt-owned only
    if granted: status <- pending, increment this task's grant count
    else:       status <- errored,  set this task's denial flag
```
...
2. **Add the per-task retry field — granted count *and* denial flag together.** One new field on the public `Task` schema per §7's contract: added to `tasks/schema/task.ts`, defaulted at build alongside `attempts` (`collection/internal.ts:59`), and read through a single `== null` guard on the object so legacy records read as "no counted history" (BP-030). Add the ledger sum over the granted count. **BP-030 also requires a cleanup pointer** so the transitional dual-read doesn't become permanent (`docs/contributing/best-practices.md:126`) — see step 9. *Test:* a record lacking the field sums as zero rather than `NaN` and reports not-denied; a fresh task starts at zero/false; the granted count rejects a negative.
3. **Extend the predicate and enforce inside the sequencer backing's CAS — guards first, then gated on attempt-owned status.** **Order of business inside the mutator is the deliverable here:** evaluate `transitionDeclineReason` before writing any retry fact, preserving FIX-976's shipped contract (`sequencer-backed.ts:219`), so a declined stale write records nothing. Widen the routing predicate so it answers "retry or terminal" from the task's own `maxAttempts`, the board's remaining budget, **and whether the task is in an attempt-owned status** (Decision 3, reusing `ATTEMPT_OWNED_STATUSES`). Then restructure `fail()` so the decision, the counter increment, and the denial marker happen in one atomic write (Decisions 4, 5). *Test:* the predicate in isolation (budget left / spent / `null` / `0` / no `maxAttempts`); a task re-pends while the budget has room and settles terminal once it doesn't; **concurrent *and sequential* failures at the boundary do not overshoot** — the sequential case is the one the rejected derivation failed, so assert it explicitly; and **`fail()` on a `pending` or `blocked` task at both a zero and an exhausted budget still re-pends and does not throw** — the reachable illegal-transition path, so it needs its own test at both budget states.
...
5. **Nothing to do on the `failTask` tool surface — deliberately.** Recorded as a step so it reads as a decision rather than an omission: `failTask` returns `{ ok: true }` and **never advertises a target status** (`skills/task-tools-capability.ts:526-529`, sharing the `okOrError` shape). `statusChangingTools` computes a target only to filter tool *names* into an illegal-transition recovery message (`:302-304`), and the target is never rendered (`:292`). With the attempt-owned gate the budget binds only from `in_progress`/`awaiting_review`, from which **both `pending` and `errored` are legal** (`task-status.ts:33,35`) — so the filtered list, and therefore the message, is byte-identical either way. **No code change and no test here.** Widening the tool's output to make an assertion possible would be inventing surface to satisfy a test.
6. **Report it — limit from the collection, reason from the denial marker.** Expose the resolved limit as a readonly value on the collection ref, then add the retry count, that limit, and the `"retry-budget-exhausted"` termination reason to the board completion item, choosing the reason by the presence of a denial marker in the snapshot (Decision 5). Reading the limit off the **ref** rather than the board is what makes a supplied collection report its real limit instead of `null`. *Test:* a budget-exhausted board reports the new reason, a non-zero count, and a non-`null` limit; a cleanly-drained board still reports `"all-completed"`; a **resource-backed** board reports a true count with `limit: null`; and **the exact-limit-without-denial case** — last grant consumed by a task that then succeeds, plus an unrelated `maxAttempts`-less task failing — reports `"blocked-by-failures"`, **not** `"retry-budget-exhausted"`. That last one is the test that would have caught the round-3 finding.
...
| **Task created through the delegation `addTask` tool** | Cannot retry at all — the tool neither accepts nor stamps `maxAttempts` (`task-tools-capability.ts:448-458,481-488`) and the predicate returns `false` without it (`internal.ts:88-90`). So the budget has no subject on that path, which is why the delegation option is dropped (Decision 8) rather than shipped inert — **and why the axis must also be filtered out of that surface's cap spread**, since refusing the option alone leaves the cap installed (§7). |
...
| Legacy task record predating the field | Reads as "no counted history" — zero granted, not denied — via one `== null` guard on the object (BP-030). Must not produce `NaN` in the sum. |
| **Pre-upgrade task that had already retried, on a resumed sequencer board** | Gets a fresh budget; the reported count covers **post-upgrade grants only**. Deliberate (§7): backfilling from `attempts` would reintroduce the non-retry re-claims Decision 1 rejected and pollute the reported number. Overshoot is at most one budget, once, and only for a board resumed across the deploy — the sole backing where enforcement and long-lived legacy records could coincide, and they don't (`request` dies with the request, `resource` doesn't enforce). Stated in the docs and changeset, not papered over. |
...
| **Supplied collection** | The *board* applies no budget and passing the option to `taskBoard` alongside one is a construction error, exactly as for the existing caps (`task-board/index.ts:842-852`). But the **collection itself may carry one** if the caller built it that way, and it enforces — so the completion item must report *that* limit, read off the ref, not `null` (Decision 5). |
| **Declined stale failure** (`{ ifAllowed: true, expectAttempt }` from `record-result.ts:105-108`, after a reclaim or a newer claim) | A complete no-op: status unchanged, **no grant recorded, no denial marker written**, no `task-change` event. The guards are evaluated before any retry fact, preserving FIX-976's ordering (`sequencer-backed.ts:219`). Recording anything here would spend another task's allowance or write a false denial — both mis-reports. |
...
- A task record lacking the retry field sums as zero and reports not-denied, not `NaN` (BP-030).
- **A pre-upgrade task with `attempts` already high starts from a zero granted count** — asserts the epoch is deliberate rather than accidental, so a later "helpful" backfill from `attempts` can't quietly reintroduce the rejected quantity.
- **The denial flag, not arithmetic, drives the termination reason** — seed a board whose granted count equals the limit with **no** task carrying the flag, and assert `"blocked-by-failures"`.
- **A declined stale failure records nothing — assert all four invariants in one test.** Drive `fail()` with `{ ifAllowed: true, expectAttempt }` against a task whose attempt was displaced (reclaimed, or re-claimed by a newer attempt) with the budget both available and spent, then assert **status unchanged, grant count unchanged, denial flag unset, and no `task-change` event emitted**. A test asserting only status would pass while the budget was silently consumed, which is the whole failure mode — so the assertion set is the point, not the scenario. Extend `test/collection/advisory-write.test.ts` (FIX-976's suite) rather than starting a parallel one, so the ordering contract has one home.
- **A supplied collection reports its own limit.** Build one directly with a retry budget on an enforcing backing and assert the completion item reports that number, **not** `null` — the case a board-sourced limit gets wrong.
...
4. **EXTEND `packages/orchestration/README.md`** — public option surface changed (BP-009), **and the new `Task` field is public API so BP-007 requires it documented in this same change set.** Cover both members beside `attempts`: the granted count means **authorized** failure retries only (not re-claims after unblock/resume/reclaim, not refunded if the retry never runs, and **counted from the upgrade, not from task creation**), and the denial flag is what the board's termination reason reads — so a consumer branches on it rather than parsing error text.
...
| **FIX-976** (honest task writes) | **MERGED** (`c9fe58a1`, PR #1004) | **No longer a coordination question — it is now the baseline.** It reshaped this spec's exact seam: renamed `shouldDeclineTransition` → `transitionDeclineReason` (`internal.ts:143`), made `transitionTo`/`fail()` return `TaskWriteOutcome`, and shifted line numbers in both files this change edits. This spec's citations are refreshed against it, and its `test/collection/advisory-write.test.ts` is the suite the declined-path test extends. A budget denial reports as `recorded` through its existing outcome type — **no new outcome member.** |
...
- **The default budget value — open, with the owner.** Decision 6 fixes only the *posture* (finite default, `null` opt-out, accepted behaviour change) and deliberately carries no value, so settling this number requires **no re-spec**: nothing in this document derives behaviour from it. **The choice is among finite values**: `250` (working proposal — generous against the 500-task default, three orders below a storm), or a higher figure such as `1000` (near-zero breakage, weaker guard).

  **`null` is *not* on this list, and an earlier draft of this question wrongly offered it.** Decision 6 fixes a **finite** default and explicitly rejects the unbounded posture, so choosing `null` would not be picking a value — it would **reverse Decision 6**, making omission unbounded and stranding the default-binding migration note, the docs, and the tests written against a default that binds. If unbounded-by-default is genuinely wanted, that **reopens Decision 6 and needs re-planning**, not a number swap. Same for `0`: legal as a *configuration* (Decision 8), but as a *default* it would silently disable retries for every existing board.
  **One concrete data point for sizing it:** the finite default binds `supervisor` out of the box, since its `maxAttemptsPerTask` defaults to `3` (`patterns/src/supervisor/index.ts:262`). At a budget of 250 a supervisor board of roughly **125 tasks each failing twice** reaches the ceiling *legitimately* — no storm, just a large plan on a flaky day. Whether that is the right place to stop such a board is the judgement; it is the clearest example of the default binding real work rather than only pathological work.
...
- **Deepening opportunity (`improve-codebase-architecture`).** `fail()`'s retry routing is maintained as two separately-written copies (`sequencer-backed.ts:384-422`, `resource-backed.ts:376-415`) kept in lockstep only by the shared predicate and by comments warning against partial fixes (`sequencer-backed.ts:391-396`). This change adds a third concern to both. The comments are doing work a shared helper should. Not in scope; related to epic **OQ-D** (is `TaskCollectionRef` the wrong shape for an extension point — ~18 methods a custom store must re-derive), explicitly beyond this epic.

---

## 13. Review notes for the implementer

**Nothing is carried here, and that is a statement rather than an omission.** Across six review cycles every finding was either **folded** (it changed the design, or the design didn't work as written) or **corrected inline** (a factual error). None fell below the bar in the way §13 exists to catch, so there are no deferred line-level notes for you to weigh.

**The spec is closed to further automated review rounds.** Anything arriving after this point lands here verbatim for you to judge against real code — it will not reopen the design.

**One documented exception to that closure, recorded so the reasoning is inspectable rather than looking like drift.** The closure was declared after cycle five and then deliberately reopened once, for two findings where the mechanism would have produced a **false report or a corrupted budget on a reachable path** (the declined-write path recording retry facts; a supplied collection reporting `null` for a limit it enforces). The stop condition targets *design churn*; shipping a knowingly mis-reporting mechanism as an "implementer note" would contradict the objective this issue serves. That is the bar for any future exception: **a reachable correctness or honesty defect, not a preference and not a refinement.** Nothing else has met it.

**What is genuinely open is not here — it is in §12, and it is the owner's, not yours:**

1. **The default budget value** (working proposal 250; Decision 6 deliberately fixes only the posture, so a different number needs no re-spec).
2. **Whether to ship at all while a delegation task cannot retry** — options (a) ship as specced *(recommended)*, (b) add delegation retries first, (c) defer.

**One standing caution from how this spec was reviewed**, offered because it is the pattern that produced four of the five cycles and it will apply to the implementation too: every failure was an **enumeration** failure, never a design one — trusting a declaration over the code path (dropping a public option while its cap still installed via a spread), or trusting citations over a sweep (two board-building patterns named, four in the tree). When this spec tells you *where* something must reach, verify the reach by reading the path, not the declaration.
...
- **After spec review (Codex, round 4)** — **removed the `failTask` plumbing rather than widening an API to make it testable.** The tool returns `{ ok: true }` and never advertises a target status; `statusChangingTools` uses the target only to filter tool *names* into an illegal-transition recovery message (`task-tools-capability.ts:266-268`), never rendering it (`:292`). With the attempt-owned gate the budget binds only from `in_progress`/`awaiting_review`, from which both `pending` and `errored` are legal, so that message is byte-identical either way. The old Decision 7 asserted a coherence requirement that did not exist — deleted, along with its sequence step and test. Kept as an explicit no-op step so it reads as a decision rather than an omission.
- **After spec review (Codex, round 5 — final completeness pass)** — **the BP-030 dual-read was compatible but the accounting wasn't.** Treating an absent counter as zero silently reset *lifetime* accounting: a pre-upgrade task that had already retried would get a fresh budget, while the reported count covered only post-upgrade grants — contradicting the lifetime framing in Decisions 1 and 4. Of the three available fixes, **chose to state the scope and reconcile the decisions** rather than backfill from `attempts`, because a backfill would reintroduce exactly the non-retry re-claims Decision 1 rejected *and* pollute the reported number with them. Counting now explicitly begins at upgrade, with the exposure bounded and shown: the two enforcing backings can only hold pre-upgrade records across a suspend/resume spanning the deploy (`request` dies with its request; `sequencer` overshoots at most one budget, once), and the backing whose records live indefinitely (`resource`) does not enforce at all — so enforcement and long-lived legacy data never coincide.
- **After spec review (Codex, round 5)** — **the denial marker had no public contract; it now has one, and it did not cost a second field.** The spec described "a persisted denial marker" while §7 still said one new field and the docs plan covered only the counter — so an implementer would have had to invent an undocumented public field or bury the fact in `metadata`/error text, which would have undone the reason for having a denial fact at all (`metadata` is caller-writable and forgeable — the same defect that sank FIX-931's park-and-promote marker; error text is not branchable). Resolved by carrying **both facts in one new public `Task` field** — a granted count and a denied-by-budget flag, written at the same seam — with schema, legacy defaulting, and README coverage specified (BP-007 and the same-change-set docs rule). `terminationReason` survives rather than being dropped, since one field is a defensible price for it; the recorded fallback (drop the reason rather than infer it) remains available if review disagrees.
- **After spec review (Codex, round 6 — reopened for two reachable mis-reporting defects)** — **the declined-write path could have consumed budget or written a false denial.** The board's failure write-back calls `fail()` with both advisory guards (`task-board/blocks/record-result.ts:105-108`) so a displaced worker's late failure is discarded, and FIX-976's `transitionTo` evaluates the decline reason **first inside the mutator** before any patch (`sequencer-backed.ts:219`). The restructure in Decision 4 replaced that path without *requiring* the same ordering, so a grant or a denial recorded before the decline would have let a stale failure spend another task's retry allowance or mark a board `retry-budget-exhausted` when nothing was refused. Ordering is now an explicit requirement, the declined path is specified as a complete no-op, and the test asserts **all four** invariants (status, grant count, denial flag, emitted event) because one checking only status would pass while the budget drained. Also folded: **FIX-976 merged (`c9fe58a1`) while this spec was in review**, reshaping this exact seam — citations refreshed, and a budget denial reports as `recorded` through its existing `TaskWriteOutcome` rather than adding a member.
- **After spec review (Codex, round 6)** — **a supplied collection would have reported `null` for a limit it actually enforces.** `resolveBoardCaps` returns `{}` for supplied collections (`task-board/index.ts:851`) and the completion block sees only the ref and collection id (`board-meta.ts:87-88`), so a board-sourced limit is unknowable for a collection it did not construct — yet a caller can build one that enforces. Reporting `null` there would say "nothing was enforced" about a limit the caller set deliberately: the precise false report the limit-beside-count design exists to prevent. Fixed by exposing the resolved limit as a readonly value **on the collection ref** and having the reporter read it from where enforcement lives (which Decision 2 already argues for), so `null` means exactly one thing everywhere and no "unknown" third state is needed. Cost acknowledged: it adds to `TaskCollectionRef`, already flagged as over-large by epic OQ-D.
- **After spec review (round 6, coordinator correction)** — **the owner's default-value question wrongly listed `null` as an option needing no re-spec.** Decision 6 fixes a *finite* default and rejects the unbounded posture, so `null` would reverse that decision rather than pick a value — stranding the default-binding migration note, docs, and tests. The choice is now stated as finite-only, with unbounded-by-default called out as reopening Decision 6 and requiring re-planning.
