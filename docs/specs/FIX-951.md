...
**The one genuine loss.** A worker that calls `completeTask` on its own task and *then* throws is recorded as a success with no trace of the throw. Do not solve this by throwing. Emit a status item from `recordError` when the write-back no-ops, naming the task and the status that made it moot, so the signal exists in the item stream without endangering the drain. The implementer owns the item's exact wording.
...
| File | Change | Why |
|---|---|---|
| `apps/docs/docs/orchestration/task-board.md` | **Extend** the "Concurrency and error handling" section (the `onError` bullet). | The line "`skip` records the error on the offending task; siblings continue" is the claim this bug violates. Add a short paragraph: a task settled while its worker was still running — cancelled by a coordinator, or completed by the worker itself — makes the board's write-back moot, and the board treats it as a no-op rather than an error. No issue numbers (published docs). |
| `packages/orchestration/README.md` | **Extend** the `TaskCollectionRef` lifecycle entry. | BP-009 / BP-007: `complete` and `fail` gain a public optional argument. Document the enumerated failure set — only a disallowed transition no-ops. |
| `.changeset/*.md` | **New**, `patch`. | BP-022: observable behaviour change in a published package. |

**Considered and excluded:** no new page (this is one paragraph under an existing concept, and new pages fragment the sidebar); no guide (no new end-to-end workflow); no `docs/architecture/*` change (`ALLOWED_TRANSITIONS` is untouched and no locked contract moves); no blog post.

**Voice risks to watch** in the task-board paragraph: no em-dash as default connector, no "robust"/"seamless", and introduce "settled" in plain terms on first use rather than assuming it.
