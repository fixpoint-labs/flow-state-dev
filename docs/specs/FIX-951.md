...
**The trade we are accepting.** A drain that today dies loudly on this race will, after the fix, keep going — and in one narrow shape (a task repeatedly returned to `pending` under a worker that keeps failing) that means a quiet re-dispatch loop bounded only by `maxIterations`, up to ten thousand turns of a possibly model-backed worker. Trading a loud crash for a bounded-but-expensive loop is the right call for a containment fix, but it is a real trade, not a free win. §9 requires a test that the bound terminates.
...
8. **Advisory writes are scoped to the attempt that produced them (`expectAttempt`), not just to transition legality.** *Alternative rejected:* shipping `ifAllowed` alone and filing the stale-worker clobber separately — re-weighed openly in §3 after spec review. *Ramification:* this is the one genuinely **new concept** the change introduces, so decision 3's earlier "net new concepts: zero" claim is withdrawn. The reason it is in scope rather than deferred: containment alone would *increase* the clobber's reach (a surviving drain lands stale writes that a dying drain never got to), and a fix that makes an adjacent bug more reachable owns it. It binds to `Task.attempts`, which already exists and is already incremented on claim, so no new bookkeeping — the worker body stamps the claimed attempt beside the `currentTaskId` it already stamps.
...
- **`expectAttempt`** — decline when `task.attempts` no longer matches the attempt the caller ran under (decision 8).
...
3. **Mirror it in `resource-backed.ts`.** Same rename, same two-arm condition, same `cancel()` update — plus the decline-path change in step 4b, which the sequencer backing does not need.
4. **Thread the option through `complete` and `fail`** in both backings, and onto `TaskCollectionRef` in `tasks/collection/types.ts` with a doc comment naming the enumerated failure set (BP-007).
   ...
   **4b. On the resource backing, the declined path must not persist or notify.** Returning `current` from `transitionRef`'s updater is **not** a true no-op: `ResourceRef.updateState` (`packages/engine/src/context/resource-registry.ts:706–720`) persists unconditionally and then calls `notifyInstanceChange`, which fires `onInstanceUpdated` and `onResourceChanged` with no equality guard. A declined advisory write would therefore emit a false `resource_change` and can dispatch a `reactTo.stateUpdated` block. Today that misfires only for `cancel()` on an already-terminal task; converting six write-backs widens it, so this change owns it (the same rule §3 applies to attempt scoping).

   Implement it by evaluating the decline condition against the mirrored task **before** calling `updateState`, and returning early when it fires — no persist, no notify. Keep the in-updater check exactly as specified; it stays authoritative.

   This does **not** reintroduce the pre-check §3 rejects, and the reason is checkable rather than asserted. The pre-read can only cause the write to be *skipped*, never to be applied on stale information, so the failure it could introduce is a write wrongly declined. Enumerate those: a pre-read of a **terminal** status is always right to skip, because terminal is absorbing. A pre-read of `pending`/`blocked` that becomes `in_progress` before the write is the reclaim race — where a new worker has claimed and `attempts` has advanced, so `expectAttempt` declines the write anyway. And `cancel()`, the one caller with no `expectAttempt`, is legal from `pending`/`blocked`, so its pre-read does not skip there at all. No case remains where the skip is wrong.

   The sequencer backing needs no equivalent: `casWrite`'s mutator returning `undefined` writes an empty state patch and emits nothing (`sequencer-backed.ts:128–130`).
...
| `pending` → `pending` *(same-status, retry budget left)* | `pending` | neither | yes | **Yes** — this is `fail()`'s soft-retry branch doing its job: fresh `feedback`, lease cleared, `kind: "retried"`. Not a no-op, and should not be. |
| `blocked` (**no** retry budget) | `errored` | disallowed | no | **Yes.** The coordinator parked it; erroring would fight that decision. |
| `blocked` (retry budget left) | `pending` | neither | yes | Yes — `blocked → pending` is legal, so the soft-retry branch re-queues it. |
...
With attempt scoping in place the `in_progress` rows above read: the write happens **only** when it is still this worker's attempt.
...
The board's `maxIterations` cap (default `10000`) bounds it. Be honest about what that bound buys: today the failure mode is a loud crash, and after the fix it is a quiet re-dispatch loop that can run up to ten thousand iterations of a possibly model-backed worker before stopping. That is a cost-observability regression in exchange for a correctness fix (BP-035), and it is worth an explicit test that the cap actually terminates the loop, not an assumption that it does.

**A declined write must be silent on the resource backing too.** The two backings decline differently, and only one of them is quiet about it. On the sequencer backing a declined write returns `undefined` from `casWrite`'s mutator, which patches nothing and emits nothing. On the resource backing the updater returns `current`, and `updateState` persists and notifies regardless — it has no equality guard — so a declined write would announce a `resource_change` that did not happen and can wake a `reactTo.stateUpdated` block. §8 step 4b specifies the early return that avoids it. This is pre-existing behaviour that today misfires only for `cancel()` on a terminal task; the fix's job here is not to let six write-backs inherit it.
...
**The re-queue spin's bound must be asserted, not assumed (§9).** Seed the spin the way §9 describes — something returns the task to `pending` under a worker that keeps failing — and assert the loop **terminates at the cap**, by counting worker dispatches and asserting the count is bounded by `maxIterations` (set it low in the fixture). Asserting only that the drain doesn't throw would pass against an unbounded loop that happens to exit for another reason, which is the whole thing this test exists to rule out.
