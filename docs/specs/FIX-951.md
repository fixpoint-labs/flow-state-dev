...
**The trade we are accepting.** A drain that today dies loudly on this race will, after the fix, keep going — and in one narrow shape (a task repeatedly returned to `pending` under a worker that keeps failing) that means a quiet re-dispatch loop bounded only by `maxIterations`. Note what that bounds: `maxIterations` is a **per-worker** cap and the drain runs `concurrency` workers, so the aggregate ceiling is `concurrency × maxIterations` — forty thousand turns of a possibly model-backed worker at the defaults, not ten thousand (§9 shows the code). Trading a loud crash for a bounded-but-expensive loop is the right call for a containment fix, but it is a real trade, not a free win. §9 requires a test that the bound terminates.
...
The board's `maxIterations` cap (default `10_000`) bounds it — **per worker, not per board**, which is the number that matters and which an earlier draft of this spec got wrong. `maxIterations` is applied to each worker sequencer's own `.loopBack` inside `makeWorker` (`task-board/index.ts:768–770`), and the drain fans out `concurrency` of those workers (`:789–791`, `maxConcurrency: concurrency`). The option's own doc comment says so: "Per-worker `loopBack` safety cap. Each worker exits after this many iterations" (`:383–385`). So the aggregate ceiling on worker-body dispatches is `concurrency × maxIterations`, which at the defaults (`concurrency = 4`, `:537`) is **40,000**, not 10,000.
...
The assertion has to name a concurrency, because `maxIterations` caps each worker sequencer separately and the drain runs `concurrency` of them (§9). Write it one of two ways, and say in the test which one it is:

- **Pin `concurrency: 1` in the fixture** and assert `dispatches <= maxIterations`. Simplest, and it isolates the loop bound from the fan-out — but then it proves nothing about the multi-worker shape, so it should say so rather than imply generality.
- **Leave `concurrency > 1`** and assert either per-worker counts (`<= maxIterations` each, keyed by worker id) or the aggregate `dispatches <= concurrency * maxIterations`. Asserting the aggregate against `maxIterations` alone would fail against a correct implementation at any concurrency above one.

Prefer the second with per-worker counts: it is the shape that would actually catch a regression in the per-worker cap, and it does not bake the fan-out factor into a magic number. Asserting only that the drain doesn't throw would pass against an unbounded loop that happens to exit for another reason, which is the whole thing this test exists to rule out.
