...
**Size: Medium.** Six source files plus the worker-body state schema, tests, four documentation pages, one goal check, and a changeset. One PR. (Attempt scoping, added after spec review, is what pushes this to the upper half of Medium.)
...
Both are evaluated **inside** the atomic write, alongside the existing `assertTransitionAllowed`. Both return `void` as today — nothing reports which guard fired (decision 9). The implementer owns exact naming and placement; `WriteBackOptions { ifAllowed?: boolean; expectAttempt?: number }` is the shape §5's examples assume.

The worker body needs to carry the claimed attempt so it can pass `expectAttempt`. It already stamps `currentTaskId` onto its own sequencer state on claim; stamp the attempt in the same place, and widen that state schema accordingly. `dispatch-and-execute` and `routedSpecialists` both already hold the claimed `Task` and can read `attempts` directly.
...
3. **Mirror it in `resource-backed.ts`.** Same rename, same two-arm condition, same `cancel()` update. Note the two files' no-op branches differ in an existing, unrelated way (the resource path returns `current` and still runs a redundant persist); leave that as found.
...
6. **Convert the six write-backs** to pass `{ ifAllowed: true, expectAttempt }`.
...
5. **`fail()` chooses its retry branch from a read taken outside the CAS.** `shouldRetryOnFail` is evaluated against a snapshot read before the atomic write, so the branch target can be chosen from a stale status. Pre-existing, and the new flag makes it strictly *less* harmful (a stale target that turns out illegal now declines instead of throwing) — but it does not close it, and §3 rejects the pre-check alternative on precisely this TOCTOU ground, so it should not go unmentioned.
