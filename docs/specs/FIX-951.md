...
- **Spec approved; two findings carried as implementation concerns.** The two open review findings were triaged as implementation-stage concerns rather than direction changes, and folded in as notes without reopening the design. (1) §10 now requires a `packages/integration-tests/src/scenarios/` scenario for the drain regression, per `AGENTS.md:146–151`, alongside the package-level tests. (2) **Decision 10's premise was false and is corrected in place, keeping the decision.** The task layer *can* abort a resource write atomically — the updater runs inside `serializeResourceWrite` and throwing out of it skips both the persist and the notify — so closing the phantom `resource_change` here is now an implementation-time scope call rather than something the spec forecloses. Neither finding changes the approved direction; both are settled with code in front of the implementer.
...
10. **The resource backing's phantom `resource_change` on a declined write is left as found unless the implementer finds a clean way to close it here.** *Alternative rejected:* pre-reading the task's status before `updateState` and returning early — specified by an earlier draft of §8 step 4b and **withdrawn**, because it fails in both race directions (it does not suppress the notify when the status moves after the pre-read, and it wrongly skips a legal write when a claim lands after the pre-read, leaving the two backings disagreeing on whether a write landed). *Ramification:* this change widens the misfire from one caller (`cancel()`) to seven, which §3's "a fix that makes an adjacent defect more reachable owns it" rule would normally put in scope.

   **Implementation concern — the premise this decision was argued on is false, and is corrected here.** An earlier draft said the task layer *cannot* abort a resource write from inside the registry's serialized section, so the guard could only live in `@flow-state-dev/engine`. It can. Verified in `packages/engine/src/context/resource-registry.ts`: `updateState` runs its updater **inside** `serializeResourceWrite` (`:711–719`); `persistNamespaceInstanceState` (`:716`) and `notifyInstanceChange` (`:719`) are both skipped when the updater throws; and `serializeResourceWrite` stores `result.then(undefined, () => undefined)` as the chain tail (`:544`), so a rejection reaches the caller without wedging the chain for the next writer. So `transitionRef` *can* throw a private decline sentinel from inside the updater and catch it at the `updateState` call — an abort that is atomic, needs no pre-read, and touches no other package.

   What stays decided: this spec does not require that. What is left to implementation: **if the sentinel proves clean in code, close the phantom `resource_change` here; if it turns out to carry a cost this spec did not foresee, leave it to §12.6.** The implementer makes that call with the code in front of them, and records which way it went in the PR. Note the sentinel is a *narrower* fix than §12.6 — it removes the misfire for this caller only, and `patchRef`'s identical shape (§9) still wants the registry-level change either way.
...
4. **Thread the option through `complete` and `fail`** in both backings, and onto `TaskCollectionRef` in `tasks/collection/types.ts` with a doc comment naming the enumerated failure set (BP-007). `expectAttempt`'s check is **two conditions, not one** — the attempt number matches *and* the task's status is `in_progress` or `awaiting_review` (§6 decision 8). Implementing only the number comparison passes the obvious reclaim→re-claim test and still ships the reclaim-window defect §9 describes; the §10 case that catches it is the reclaimed-and-parked one.
   ...
   **What to do instead, by default: nothing, on the resource backing's decline path.** `transitionRef` returns `current` and `updateState` does what it does today. §9 records the widened exposure and §12.6 owns the general fix.

   **The one thing left open here (§6 decision 10).** What is ruled out is the *pre-read*, in any position. What is not ruled out — and what the withdrawn draft never considered — is aborting from **inside** the updater by throwing a module-private sentinel and catching it around the `updateState` call. That is atomic, so neither race direction above applies. Decision 10 carries the code verification; try it, keep it if it is clean, and fall back to leaving the decline path as found if it is not.
...
**Why it may not be fixed here, given §3's own rule that a fix making an adjacent defect more reachable owns it.** That rule is why `expectAttempt` is in scope, so declining to apply it here needs an argument rather than an exemption. The argument is tenet 5: the owning layer for "a no-op `updateState` persists and notifies" *in general* is `ResourceRef.updateState` in `@flow-state-dev/engine`, not the task collection that calls it, and §12.6 is the mechanism that closes it for every caller including `patchRef`.

What that argument does **not** establish — and an earlier draft of this section asserted anyway — is that the task layer cannot abort its own write. It can: the updater runs inside the serialized section, and throwing out of it skips both the persist and the notify. **Decision 10 carries the verification and leaves the scope call to implementation.**
...
**Implementation concern — the three-task drain regression also needs a flow-integration scenario, not only package-level tests.** `AGENTS.md` (`:146–151`) requires a `packages/integration-tests/src/scenarios/*.test.ts` case using `testFlow` when a bug is fixed in a pattern factory and only manifests end-to-end; `taskBoard` is named there explicitly. The drain escape is exactly that shape — it emerges from full `runAction` composition (worker sequencer → `rescue` → `forEach`), which is the layer the package-level tests above do not exercise. Add the cancelled-mid-flight three-task board as a scenario there **in addition to** the package tests, with a synthetic fixture flow under `src/scenarios/fixtures/` and `unmockedGeneratorPolicy: "error"`. The package tests stay the discriminating ones (§10's case list); the scenario proves the composition.
