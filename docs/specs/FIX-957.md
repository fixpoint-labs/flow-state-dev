...
3. **The creation caps move to one backing-agnostic check that both backings call; the durable backing accepts cap options and re-reads the collection immediately before each check. The guarantee is explicitly bounded: enforced within an execution and across sequential turns, not across concurrently in-flight requests.**
   ...
   *If the owner would rather not ship a bounded guarantee under the same option names,* the alternative is to leave the durable backing uncapped in this spec and say so loudly in the docs, deferring all durable-board bounding to FIX-939. That is a smaller, equally honest scope; it is not the recommendation, because "enforced except under concurrent requests" is materially better than "unbounded" for the `session`-scoped default.
...
| Fan-out workers adding concurrently near a cap, same execution | The re-read sees every committed insert in the execution, so the cap binds. Must be exercised with fan-out workers sharing the board capability, not two direct `addTask` calls on one reference. |
| Two concurrently in-flight **requests** adding near a cap on the same durable board | Each sees only its own execution's view; the cap can be overshot by up to the number of simultaneous requests. **Known and accepted** (Decision 3) — unfixable without a store-backed count or multi-key atomic op. Test that it is bounded and documented, not that it cannot happen. |
