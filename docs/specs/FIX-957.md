...
|---|---|
| No `board` option supplied (the default) | Identical to today in every respect. The settle filter does not run at all on this branch, so the returned set is today's by construction, not by argument. Includes the sequence `addTask` → `cancelTask` → `runBoard`, where the cancelled task is still reported. |
