...
3. **A durable delegation board is UNBOUNDED *and* its task claims are NOT SERIALIZED across concurrent executions. The spec stops claiming otherwise on both counts. The existing type-level ban on cap options for the resource backing was correct and stays, and this issue changes nothing there.**
   **Evidence (BP-003).** Both consequences are reproduced by a spike rather than argued from code reading — `labs/SPIKE-durable-board-claims/` on `spike/durable-board-claims` @ `40bcd0cc0`, real `store-sqlite` on a real file, two concurrent `testFlow` executions sharing one store, each with its own execution context and resource cache, nothing load-bearing stubbed. Re-run with `pnpm --filter @flow-state-dev/spike-durable-board-claims test`. This matters because the reading below changed four times before it settled.
