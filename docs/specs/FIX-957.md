...
- **Tenet 5 (fix at the owning layer — find where paths converge).** Applied twice, both times as a negative result worth having: the creation caps have several writers (the coordinator's tools, a fan-out worker's tools, the drain, internal batch callers), and on durable storage there is *no* point where they all converge under one critical section — which is why the honest answer is "unbounded, and claims unserialized" rather than a guard at one entry point. The typed capacity error, by contrast, does have an owning layer, and it is the one that raises the limit.
...
**Size estimate: Large** (multi-PR, >500 LOC across `core`, `orchestration`, `engine`, and docs). **PR plan shape** (the detail is in §8): four sub-PRs. Three are independent and build in parallel — the typed capacity error, the run's working set, and the `unblockTask` tool. The configurable board depends only on the working set, because shipping durability before it means shipping a board whose report replays finished work. Docs ship inside each PR.
...
| `packages/orchestration/README.md` | EXTEND | The per-binding `board` option, `unblockTask`, the new soft error (BP-009). | a, c, d |
