...
2. **Be honest that a durable board has no ceiling.** The two guards that bound runaway delegation cannot be enforced on durable storage — nor can the storage layer's own retention limit. Turning durability on removes the ceiling at the moment it starts mattering most, and that has to be a stated trade rather than a surprise.
...
flowchart TB
  subgraph lib["skills library"]
    ...
    RES["library declares one collection<br/>per durable binding"]
  end
  ...
  subgraph coll["task collection"]
    ...
    RESB["resource backing<br/>UNCAPPED (Decision 3)"]
  end
...
**Second-path checklist (BP-035).** *Legacy:* the persisted task envelope is a migration surface now that rows outlive deploys; this spec adds no field to it, which is the cheapest way to stay safe. *Null boundary:* the board option and the capacity error are optional/absent by default; guard each. *Concurrency:* the unbounded durable board (Decision 3) and the memo/staleness interaction in §12 — neither is closed here, both are stated. *Multi-tenant:* a durable board at `user` or `org` scope is shared across sessions by construction — the caller's explicit choice, with `session` documented as the conservative default. *Cost observability:* more surviving tasks means more context replayed, which is exactly what Decision 2's working set bounds. *Flag off-state:* covered above and the hardest test target.
