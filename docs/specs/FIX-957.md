...
The configuration itself is deliberately small. The skills library gains **one** option naming how long the board should live — `block` (today's behavior, the default) through `request`, `session`, `user`, `org`. The storage backing is derived from that answer rather than chosen, and for the durable values the library builds and registers the task collection itself. No new durability machinery is invented anywhere: the durable values resolve to the same `defineTaskCollection` primitive the code-first path already uses.
...
const skills = createSkillsLibrary({
  collection: "skills",
  catalog,
  boardLifetime: "session",      // ← the only new surface
...
1. **The delegation board is configured by one option naming its lifetime — `block` (default, today's behavior) | `request` | `session` | `user` | `org`. The storage backing is derived, not chosen, and for the durable values the library constructs and registers the task collection itself.**
...
3. **The creation caps move to one backing-agnostic check that both backings call; the durable backing accepts cap options and re-reads the collection immediately before each check. The guarantee is explicitly bounded: enforced within an execution and across sequential turns, not across concurrently in-flight requests.**
   *Alternative rejected:* a second copy of the check; keeping the type-level ban; checking against the held mirror alone. Also rejected: claiming the re-read is "authoritative" — it enumerates a per-execution cache, so that claim would be false.
   *Ramification:* the paragraph justifying why caps can't apply to the durable backing is deleted rather than amended. **This is a weaker guarantee than the state backing's**, and it is weaker than this decision claimed in its first two drafts. It cannot be strengthened without a store-backed count or a multi-key atomic operation, neither of which the resource registry offers — so closing the gap is a substrate change (FIX-939 territory), not something to attempt here. Costs one extra collection read per batch add on the durable backing only. Cap breaches are evaluated for a whole batch before anything is written; **the storage-retention limit is pre-checked the same way**, so neither ceiling lands a partial batch within an execution. A store failure mid-batch, or a cross-request race, still can — documented rather than papered over with rollback machinery.
   **⚠️ STATED CONSEQUENCE AT SIGN-OFF — as this spec currently stands, a durable delegation board has no working ceiling of any kind.** This is the interaction of Decisions 1 and 3, and it is visible only when they are read together, so it is written here rather than left to be derived:

   | ceiling | durable board |
   |---|---|
   | `maxTotalTasks` (task layer) | **not enforced** — no collection-wide critical section; `serializeResourceWrite` chains per storage key |
   | `maxEnqueuedTasks` (task layer) | **not enforced** — same reason |
   | `maxInstances` (registry) | **not enforced**, and not exposed by Decision 1's enum |

   All three fail for one structural reason. Every check reads the per-execution resource cache, and that cache is mutated only *after* the store write resolves (`persistResourceKey` awaits `stores.resourceState.set(...)` and assigns to the cache on the following line). So the window between a creator's count and its visible effect spans a full store round-trip — the longest yield on the path. With the delegation board's concurrency of 4 and fan-out workers holding board tools, concurrent creators will essentially always read the same stale count and all pass. This is the over-spawning hazard FIX-931 exists to close, reappearing at the durable tier.

   *Recommendation for the owner, not enacted here (Decision 3 is yours to settle):* the **within-execution** half looks recoverable without any substrate change — serialize check-and-create at the *task* layer, over the memoized collection reference that Decision 1's sub-PR `d` already makes mandatory and shared. That is an ordinary in-process promise chain above the registry, so it does not depend on `serializeResourceWrite` and does not need a collection-level atomic. Its guarantee would rest entirely on the memo being universal — every writer (flat tools, `boardTaskTools`, the drain, settle, internal batch callers) resolving through the one ref — which is exactly the enumerate-every-writer discipline tenet 5 demands, and is the thing to verify before believing it. **The cross-execution half is not recoverable here** under any option: it needs a store-backed count or a multi-key atomic the registry does not have, which is FIX-939's substrate work.

   *So the owner's choice is between three honest positions:* (i) ship durability unbounded and say so in the docs, deferring all bounding to FIX-939; (ii) add task-layer serialization so a durable board is bounded within an execution and unbounded across concurrent ones; or (iii) hold durability until FIX-939 lands a real ceiling. This spec does not pick one — an unbounded durable board may well be an acceptable trade for the `session`-scoped default, but it is a trade someone should make deliberately.
...
|---|---|---|
| a | Shared cap check lifted into the caps module; resource backing enforces both caps with a pre-check re-read (execution-scoped, per Decision 3); **retention headroom pre-checked from `count()` + collection config so a batch never lands a prefix**; resource backing spec accepts cap options; obsolete justification comment removed; ceiling precedence. Capacity error type in `core`, raised by the engine registry, re-raised as a typed cap breach, mapped to `board_at_capacity`. **Plus the docs for all of it.** | — (see Decision 8) |
| b | `runBoard` pre-drain snapshot and working-set report **on the durable branch only**; per-turn branch left unfiltered; verdict unchanged. **Plus docs.** | — |
...
| d | The `boardLifetime` option on the skills library; derive the board id and **construct + declare the collection library-side** for the durable values; backing derivation at the construction site; own-state guard made conditional; mandatory shared memo for `boardCollection()`. **Plus docs.** | a, b |
...
*b — the working set.* Add the pre-drain snapshot and the settle partition together, behind the durable-branch condition — they're one behavior. Test both branches separately: on the per-turn branch the filter must not run at all (including `addTask` → `cancelTask` → `runBoard`); on the durable branch, a carried-over task completed by this run comes back with its output, a task finished before this run does not, a task added mid-drain does, and the verdict reflects only the working set.
...
*d — the option.* The lifetime option, the derived board id, library-side construction and declaration of the durable collection, the backing derivation, the conditional guard, the memo. Test the off state hardest (BP-035): at the default `block` lifetime every behavior above must be inert and today's path bit-for-bit unchanged. Test each rung resolves to the backing it should, and that two libraries in one flow get distinct board ids.
...
| Fan-out workers adding concurrently near a cap, same execution | **As specified today: no cap binds.** Each reads the per-execution cache before any of their writes reach it, so all pass. Recoverable via task-layer serialization over the shared memo — Decision 3's open recommendation, not yet chosen. |
| Two concurrently in-flight **requests** adding near a cap on the same durable board | No cap binds, and none can without a store-backed count or a multi-key atomic (FIX-939). Not recoverable in this spec under any option. |
| `maxInstances` set on a durable board | Does not bind either, and the enum does not expose it. Same stale-cache cause. Do not present it as a fallback ceiling. |
...
| Batch larger than the remaining retention headroom | Pre-checked before the create loop, so nothing is inserted and the refusal is whole-batch — not a landed prefix followed by a refusal. |
| Two skills libraries in one flow | Board ids derive from each library's collection key, so they are distinct and neither can equal a skills collection key. No collision to detect. |
| `boardLifetime: "request"` | Resolves to request-state backing; board survives block boundaries within the turn, dies with it. Durable-branch settle filter does **not** apply (it isn't a durable board). |
...
**Required: a bounded-overshoot test, asserting the documented limit rather than its absence.** Two executions against one durable board, each admitting up to its own view's headroom, must overshoot by *at most* the number of in-flight requests — and a same-execution fan-out must not overshoot at all. A test that asserts overshoot is impossible would be asserting something false.

**Other CI specs, in observable terms.** Default path returns the same task set with no `board` supplied, including for `addTask` → `cancelTask` → `runBoard`. A batch exceeding retention headroom inserts nothing. Caps refuse past both bounds on the resource backing. A cap-crossing batch inserts nothing on both backings. Capacity returns `board_at_capacity` rather than throwing, and precedence holds when retention sits below the task caps. Settle: carried-over-and-completed returned with output; finished-before-this-run omitted; added-mid-drain included; verdict from the working set only. `unblockTask` returns a blocked task to `pending` with a stable id and dependents still runnable. The own-state guard does not fire under a durable board. Board-id collision throws at construction. Per Decision 4(B) if chosen: the unblock path refuses `in_progress` / `awaiting_review`.
