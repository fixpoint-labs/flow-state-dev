...
The question is deliberately narrow, and the narrowness is the point. A throw the seam *cannot* attribute to a pre-commit decline includes the case where the write **committed and something after it threw** — and that case belongs to FIX-963, whose whole purpose is to make it loud. So the seam contains only what is its own and rethrows the rest. §7 states the attribution rule and the composition contract — **and flags where that contract is not yet satisfied.**
...
> ### ⚠️ The contract above is NOT yet satisfied at `dispatchAndExecute`. Open, unresolved.
>
> Verified: `onError` defaults to **`"skip"`** (`dispatch-and-execute.ts:154`) and the catch rethrows only when it is `"fail"` (`:195`). So on a post-commit throw the sequence is: `complete()` commits → `onChange` throws → the seam correctly **rethrows** → **`dispatchAndExecute`'s own `catch` swallows it** and calls `fail()` → the task is now terminal, so the seam contains *that* write under clause (a) → the helper returns normally. **FIX-963 never receives the S4 it was promised**, and no amount of correctness in the seam changes that, because the loss happens at the call site's try/catch shape rather than inside the seam.
>
> This is a **genuinely open composition conflict**, recorded here as unresolved rather than described as handled. Resolving it means changing how `dispatchAndExecute` treats a rethrown post-commit failure — which is a decision about FIX-963's error channel and this helper's `onError` semantics, not something this spec should settle unilaterally. Its sequencing is the repo owner's call.
>
> The board writer (`record-result.ts`) does not have this shape: `recordError` *is* the rescue handler, so a rethrow from it propagates rather than being re-caught.
...
**No new pages** — every affected concept already has a home, and a new page would fragment the orchestration sidebar for a behavioral fix. **No blog post.**
...
**Structural limit — containment classification cannot be made exact from a post-write re-read.** This is the round-3 conclusion and it supersedes any implication that the snapshot rule is exact. Inference from observed state, however carefully staged, cannot separate these histories:

- *Snapshot already displaced, then the store fails for an unrelated reason* (an outage). Clause (a) fires on the snapshot and **suppresses a real failure**.
- *Another worker advances the attempt; a legacy `complete()` commits and then `onChange` throws.* Clause (b) sees a state this call "could not have produced" and **suppresses a committed-then-threw error** — the exact case FIX-963 exists to expose.

Both are the same root cause: **the write never reports what it did**, so every classifier downstream of it is guessing. FIX-963 hit this wall from the opposite direction in the same review round, which is the tell that it is structural rather than either spec's mistake.

**What would make it exact:** an outcome returned by the atomic mutation itself — mutation-originated provenance rather than inferred state. That is precisely FIX-976's widening of `complete`/`fail`, **currently labelled droppable**. If that widening lands, this spec should **consume** it and replace the snapshot-plus-clauses inference with the write's own verdict; the seam's structure survives, only its evidence source changes. If it does not land, the seam remains a best-effort containment with the misclassifications above, and this section is the honest statement of that. **The sequencing call — whether FIX-976 keeps the widening so FIX-964 and FIX-963 can both stop guessing — belongs to the repo owner and has been put to them.**
...
- "still cannot prove which operation caused the throw" — Codex, spec PR #994 (line 204). *Variant 1:* "if the snapshot is already displaced and the store then fails, clause (a) suppresses an unrelated outage." *Variant 2:* "if another worker advances the attempt and a legacy `complete()` commits then `onChange` throws, clause (b) suppresses a committed-then-threw error." Asks for **mutation-originated provenance rather than inferred state**. *(Correct, and not fixable by inference — see the structural limit in §12. Recorded rather than folded because the remedy is a cross-issue sequencing decision, not a change this spec can make alone.)*
