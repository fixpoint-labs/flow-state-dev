...
1. **The write verdict is a three-variant discriminated outcome** — *recorded* /
   ...
   *Precedence, and why it is fixed:* terminal → disallowed → lost-claim, which is the order
   ...
   remove.
   *Ramification:* one outcome type across both the transition and patch paths.
   **`recorded` and `declined` are required for FIX-976; `unchanged` is epic-coherence
   work and is explicitly droppable** — it exists because the patch helpers genuinely
   return without writing when a field already holds the requested value, and reporting
   that as *recorded* would reintroduce this epic's own defect shape in miniature. But the
   tool boundary maps it to `{ ok: true }` either way, so dropping it changes no
   FIX-976 behaviour: ship two variants, and add the third only if change-item fidelity
   turns out to matter to a caller. Dropping it also removes the `unchanged` rows in §9/§10
   and shrinks step 1. See the required/droppable column in §7.
...
a **compile-time** error at implementation-declaration time (epic-spec §5, CONFIRMED by POC),
...
| `unchanged` variant (Decision 1) | — | 1 | *Droppable* — epic coherence | tool maps it to `ok: true` either way |
...
- *Substrate, parameterized over both backings* (extend the existing advisory-write suite,
  ...
  - `setAssignee` where the value already matches → *unchanged*, **no** change item *(drops
    with the `unchanged` variant if Decision 1 is trimmed)*
