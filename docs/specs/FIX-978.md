...
> **Four P1s below are recorded and deliberately NOT FOLDED, pending the sequencing decision now
> with the repo owner.** A second-opinion research pass concluded the task board's shape is right
...
> **The fourth (suspension/resume) is a gap against a shipped first-party feature**, so if the
> owner decides to build this spec as specified, **that one requires an answer** rather than
> possibly being deleted. Filing all four under one "pending" header without that distinction
