...
**Blocking dependencies: FIX-990 and FIX-989, plus two undecided design questions — see §13
before starting anything.** (The POC block is discharged; its verdicts are folded into §7 and
recorded below.)

> **⚠ Do not treat this spec as ready to implement.** §13 records seven findings NOT FOLDED, four
> of which block building it as written:
> - **§7's freshness mechanism needs FIX-989** (durable write provenance). No seam outside the
>   atomic write can satisfy its version requirement — verified twice over, from the
>   emit-after-write ordering in both backings and from the delegation surface's three-refs-one-
>   storage shape. This is a hard prerequisite, not a scoping preference.
> - **The suspension/resume ledger loss** is a gap against `ctx.suspend()`, a shipped first-party
>   feature, and needs an answer.
> - **The wake gate contradicts §9's own retry promise**, and the count-based threshold it rests
>   on is recorded as unsound, so there is no correct value to patch it to.
> - **The supplied-ref path is live on a shipped first-party consumer** — the delegation surface.
>   An earlier deferral called it an unexercised surface on the strength of a mis-scoped search;
>   that is withdrawn. A version bump on any single ref is bypassed by the other two writers.
>
> A sequencing decision is with the repo owner on whether to build the missing primitives first
> and re-derive how much of §7 survives. Until it lands, this spec is an input to that decision
...
**What is implementable now, if the sequencing decision says to proceed piecemeal:** the
`awaiting_review` keep-alive (Decision 7, restricted to the automatic-idle modes), the meta-rule
precedence and its `onIdle` gate, the docs sweep, and the lease-semantics corrections. These are
reachable and fixable on today's `main`.

**Explicitly NOT in that set**, though an earlier draft of this paragraph listed both — the
correction is recorded rather than quietly dropped:
- **§7's board-level quiescence mechanism** — blocked on FIX-989 per the callout above.
- **The invocation-keyed ledger placement (Decision 2)** — its own §13 finding (suspension/resume
  loses the ledger while the task row stays persisted `in_progress`) is unanswered, and the
  placement is what that finding is about.
...
One further fact belongs with that, because it bears on what the machinery is worth: the
refusal-count problem exists to serve **`classifierDispatcher`**, which has **no production
consumer** — verified as definition, barrel export, tests and docs only.

**The supplied-ref extension point is NOT in that category — an earlier draft of this paragraph
said it was, and that was wrong.** It has a shipped first-party consumer: the delegation surface
(`skills/delegation-surface.ts:315`, `:620`, `:693` — three writers over one storage, with
mid-drain fan-out as designed behaviour). The revision problem is therefore live on a real path,
not carried for an unexercised surface. See §13's supplied-ref finding for the withdrawal and the
mis-scoped search that produced the original claim.
...
> **Seven findings below are recorded and deliberately NOT FOLDED, pending the sequencing decision
> now with the repo owner.** **Four** of them now block building this spec as specified: the
> suspension/resume ledger loss; the mutation seam (round 14 escalated it — no seam outside the
> atomic write can satisfy it, so it needs FIX-989 first); the supplied-ref path (round 15 upheld
> it as live on a shipped first-party consumer, withdrawing the deferral); and the wake gate's
> contradiction of §9. A second-opinion research pass concluded the task board's shape is right
...
>
> **They are not equivalent, and should not be read as one bucket.** Two of them — the two-`null`
> cutoff and the pre-dispatch version stamp — are design questions in machinery serving
> **`classifierDispatcher`, which has no production consumer**; if the mechanism goes, they go
> with it. One (`onTaskChangeFor`'s request-scoped filter) is routed elsewhere, to FIX-939 m2.
>
> **Four are different, and every one of them needs an answer if this spec is built as
> specified:**
> - **suspension/resume** — a gap against a shipped first-party feature (`ctx.suspend()`);
> - **the mutation seam** — round 14 established that *no* seam outside the atomic write can
>   satisfy §7's version requirement, so this needs **FIX-989**, not a scope widening;
> - **supplied refs** — live on a shipped path (the delegation surface), **not** an unexercised
>   surface. An earlier version of this paragraph listed it in the no-consumer bucket on the
>   strength of a mis-scoped search; that is withdrawn, and the withdrawal is recorded at the
>   finding itself;
> - **the wake gate** — it closes on the very refusal it exists to retry, contradicting §9's own
>   promise, and the threshold it rests on has no principled value to patch it to.
>
> Filing all seven under one "pending" header without that distinction would misrepresent the risk.
...
- "Caller-supplied refs lose **correctness**, not just promptness. A mutation through another
  ...
  **⚠ Round 15 — that deferral is WITHDRAWN. It was verified with the wrong search scope, and
  the finding is live on a shipped first-party path.** The search looked for call sites *outside*
  `packages/orchestration/src`, which excluded the skills runtime — where the consumer is. The
  delegation surface supplies a ref and has **three distinct writers against one storage**:
  `taskBoard({ collection: () => boardCollection() })` (`skills/delegation-surface.ts:315`), the
  executive's flat task tools (`:693`), and a worker's board-bound fan-out capability
  (`createTaskToolsCapability(() => boardCollection(), roster)`, `:620`) — whose own comment
  states the intent plainly: *"an inline agent that declares `tools: [taskTools]` fans out
  mid-drain onto the same ledger the drain is watching."* Mid-drain adds are the designed
  behaviour, not a hypothetical. Credit: Codex
  ([PR #990](https://github.com/fixpoint-labs/flow-state-dev/pull/990)).

  *One correction to the reviewer's mechanism, which does not rescue the finding.* It attributes
  the failure to `getOrCreateTaskCollection` returning a new ref per call leaving mutations
  invisible. For this board that half is wrong: the backing is `"sequencer"`
  (`delegation-surface.ts:589-596`), and `createSequencerBackedTaskCollection.readTasks()` reads
  `options.sequencer.state` **at call time** (`sequencer-backed.ts:93-99`) through the one shared
  `StateRef`, so every ref sees the added task. The per-call-mirror blindness is resource-backed
  only — that is FIX-990, a different finding. **But the task data being visible is not the
  thing that breaks.** A version bump attached to the *drain's ref wrapper* is still bypassed by
  a write through either of the other two refs, so `versionAdvancedSinceLastRefusal()` stays
  false while runnable work exists, and quiescence can still reach its threshold and exit
  `blocked`/`stranded`. The reviewer's conclusion holds; only its stated cause needed narrowing.

  *And it converges with round 14.* Three refs over one storage means **any** per-ref version is
  wrong by construction, independent of backing. That is the same conclusion round 14 reached
  from the emit-after-write ordering: the revision has to live in the store and advance inside
  the atomic mutation — FIX-989. Two independent findings, one remedy.
...
- **After spec review (round 11 — recording, not folding)** — one correction and three deliberate
  ...
  **↳ Corrected at round 15:** the supplied-ref half of that claim was wrong — the delegation
  surface is a shipped consumer. Only `classifierDispatcher` lacks one. Left in place as the
  record of what round 11 concluded; see §13's supplied-ref finding for the withdrawal.
- **After spec review (round 12 — one recording)** — a fourth P1 recorded NOT FOLDED: **the ledger
  ...
  three**, which were then believed to serve surfaces with no production consumer (**round 15
  corrected that for supplied refs** — see above): `ctx.suspend()` is a shipped
