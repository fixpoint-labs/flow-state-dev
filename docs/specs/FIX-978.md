...
>
> Where `versionAdvancedSinceLastRefusal()` is true unless this worker's most recent claim
> attempt returned `null` **at the collection's current version**. The governing invariant:
...
**Blocking dependencies: one, scoped to §7's freshness mechanism.**

> **The in-flight POC is non-blocking for spec approval and blocking for implementation of
> §7's wake / claim / version design.** The spec can be reviewed and approved exactly as it
> stands. But that mechanism **must not be built** until the verdict lands and is folded,
> because a `REFUTED` verdict changes its *shape*, not its details — anything built against
> the current formulation would be rewritten rather than adjusted.

**Everything else in this issue is implementable now** — the invocation-keyed ledger's
placement (Decision 2), the `awaiting_review` keep-alive (Decision 7), the meta-rule
precedence and its `onIdle` gate, the docs sweep, and the lease-semantics corrections do not
depend on the verdict. Scope the block to §7's mechanism, not to the issue.

No other blocking dependencies: this is otherwise reachable and fixable on today's `main`.
...
**⚠️ The wake / claim / version interaction is under empirical settlement (POC in flight).**
A throwaway POC is running that interaction against the real board; its verdict lands on
[PR #990](https://github.com/fixpoint-labs/flow-state-dev/pull/990) and may revise §7's
freshness mechanism.

**Non-blocking for spec approval; blocking for implementation of §7's mechanism.** The two are
distinct and an earlier draft of this paragraph collapsed them into a bare "non-blocking,"
which contradicted §13's own instruction to wait. The spec can be reviewed and approved as it
stands — the POC gates no part of the *review*. §7's wake / claim / version design must not be
**built** until the verdict is folded, because a `REFUTED` verdict changes its shape rather
than its details. The dependency is stated in the blocking-dependencies list above, where a
reader scanning for blockers will find it, not only here.
...
- "The wake filter is subscribed *after* the claim attempt, so a `task-change` emitted between
  a worker's `null` and its subscription is never observed — the version does not advance for
  that worker and its refusal stays stamped as current." — Codex
  ([PR #990](https://github.com/fixpoint-labs/flow-state-dev/pull/990))
  **⚠️ UNDER EMPIRICAL SETTLEMENT — not fixed here, and deliberately not re-argued.** A
  throwaway POC is running the wake / claim / version interaction against the real board; its
  verdict lands on this PR. Do **not** implement a seventh prose formulation of the freshness
  mechanism from this note — wait for the POC. **This is a hard block on building §7's
  mechanism** (see §12's blocking-dependencies list), though not on approving the spec and not
  on the rest of the issue, which is implementable now. Five review rounds each fixed a real defect in
  this same interaction, which is the evidence that it is past what a spec can settle on paper
  (§12). The mechanism as written in §7 stands until the POC says otherwise.
