...
- **One board-level last-observed outcome, stamped with the collection version it was taken
  ...
  **The version is bumped at the collection's mutation seam — not off a subscription.** Each
  ...
  `getOrCreateTaskCollection` already wires to `ctx.emit.component`. The board composes that
  callback so every `TaskChangeEvent` increments the version **synchronously with the write**.
...
**A second recurring pattern, and the mitigation has to be widened.** Four times now a
...
severity from behaviour after §11 had settled it on the exported type; and the round-9 mechanism
rewrite reached §7 and Decision 8 but left **§8's build plan** telling the implementer to store
per-worker outcomes — the most dangerous of the four, because §8 is what gets built.

The round-6 mitigation ("after any prose pivot, diff the decision list against the sections it
summarises") was too narrow. **The check must cover the build plan as well as the decision
list**: after any mechanism change, re-read §8 and §10 against §7 and confirm each names the same
mechanism. §7 is the design, §8 is the instruction — an implementer follows §8, so a stale §8 is
not a documentation defect but a shipped one.
