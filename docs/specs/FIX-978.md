...
- **One board-level last-observed outcome, stamped with the collection version it was taken
  ...
  off)", so a bare `null` cannot carry an exit decision. The version is a monotonic counter
  bumped off the board's existing `onTaskChangeFor` subscription — no new subscription.
...
>   if (!ledger.lastObservationIsIdleAtCurrentVersion()) return "continue";
...
whose lifetime actually mattered. FIX-987 is a fourth sighting. The corrective that generalises:
**name the entity whose lifetime the state must match, then key on it** — and prefer a signal
with no participant bookkeeping at all when one is available, which is what the board-level slot
is.
