...
ledger. The third has to be passed in — see the constraint below. Deterministic precedence:
...
2. **`onIdle === "complete-or-blocked"`** *and* any `in_progress` row **not held by this
   invocation's ledger** → **`stranded-claim`**.
...
freshness mechanism. Recorded here rather than in the blocking open-questions list, because a
settlement in flight is non-blocking by design.
...
- "The wake filter is subscribed *after* the claim attempt, so a `task-change` emitted between
  ...
  **⚠️ UNDER EMPIRICAL SETTLEMENT — not fixed here, and deliberately not re-argued.** A
  ...
  mechanism from this note — wait for the POC. Five review rounds each fixed a real defect in
