...
8. **"Nothing is claimable" is read from observed dispatcher outcomes, not re-derived by
   ...
   *Ramification:* `hasClaimableTask` leaves the **exit path** entirely, and stays only as the
   wake predicate's promptness disjunct — safe there, because an over-permissive hint costs one
   wasted claim attempt while an over-permissive exit ends a live board (§7).
   ...
   **8a. A recorded outcome is valid only at the collection version it was taken at. Any
   task-change retires every recorded outcome, and no exit may rest on a retired one.**
   ...
   *Ramification:* the ledger carries a monotonic version bumped from the task-change signal
   the board **already subscribes to** (`onTaskChangeFor`, used today as the worker wake
   filter), so this needs no new plumbing. Retired outcomes read as "not idle," which yields
   `continue`, which sends workers back to the dispatcher — the invalidation and the recovery
   are the same motion. *Declared residual:* a dispatcher whose eligibility depends on state
...
- **Per-worker last-claim outcome, stamped with the collection version it was taken at** —
  did this worker's most recent `dispatcher.claim` return a task or `null`, and as of when?
  This replaces `hasClaimableTask` in the exit path (Decision 8). It is ground truth for
  every dispatcher: `eventDispatcher` filters on topic and returns `null` when no topic
  resolves, `classifierDispatcher` can return `null`, and a custom dispatcher can do
  anything — none of which `hasClaimableTask` models.

  The version stamp is what makes it *safe* rather than merely accurate (Decision 8a).
  `TaskDispatcher.claim`'s own contract is "returns `null` when nothing is ready to dispatch
  **right now** (loop should exit or back off)", so a bare `null` cannot carry an exit
  decision. A board is claim-quiescent only when every worker's last attempt came back
  `null` **at the current version**. The version is a monotonic counter bumped off the
  board's existing `onTaskChangeFor` subscription — the same signal already used as the
  worker wake filter, so no new subscription is introduced.
...
> `hasClaimableTask(collection) || boardQuiescence(collection, ledger) !== "continue"`

Round 1 wrote the second disjunct as `outstanding === 0`, and **that is broken** in the case
Decision 7 protects: on an `awaiting_review`-only board `hasClaimableTask` is false but
`outstanding === 0` is true, so the predicate fires, `.waitForCondition` returns immediately,
`checkBoard` says `continue`, and the loop comes straight back — a **hot spin through
`maxIterations`** that ends the drain before review resumes. Any hand-written wake formula is
a second implementation of the exit condition and will drift from it again, so the second
disjunct is the helper itself.

**`hasClaimableTask` stays as the first disjunct, deliberately, and this is a considered
divergence from "wake only on a terminal verdict."** Dropping it would regress claim latency
on *every* board: newly-claimable work (a dep completing, a task added) makes the helper
return `continue`, so a verdict-only predicate would not wake and the worker would sit out the
full `timeoutMs` — 5s per task at defaults — where today it wakes immediately.

The asymmetry is the point, and it is why the same call is safe here and unsafe in
`checkBoard`: **a promptness hint that is too permissive costs at most one wasted claim
attempt; an exit test that is too permissive ends a live board.** And the hint can no longer
cause a spin, because Decision 8 made `checkBoard` dispatcher-correct — on the
topic-mismatched `eventDispatcher` case where `hasClaimableTask` lies, the worker now wakes,
attempts one claim, gets `null`, and `checkBoard` returns a terminal verdict. One wasted
attempt, then an exit, rather than a loop.
...
>   // Every worker saw `null`, AND saw it at the collection's current
>   // version. A resume, a dep completing, or an add bumps the version and
>   // retires those observations — so this is false again until the workers
>   // have actually re-asked the dispatcher.
>   if (!ledger.allWorkersIdleAtCurrentVersion()) return "continue";
>   return c.count({ status: ["in_progress"] }) > 0 ? "stranded" : "blocked";
> }
> ```
>
> The idle check reads recorded per-worker claim outcomes and their version stamps — not
> `hasClaimableTask` (Decision 8), and not an unversioned `null` (Decision 8a).
...
Instead it re-derives once, at meta time, from **fresh collection state plus the still-live
ledger.** That is available: the drain is
...
ledger. Deterministic precedence:

1. `inFlightCount === 0` → **`all-completed`**. Nothing unfinished, whatever any worker
   concluded earlier.
2. Any `in_progress` row **not held by this invocation's ledger** → **`stranded-claim`**.
3. Otherwise → **`blocked-by-failures`** (today's value, unchanged).

This is both deterministic and *fresher* than any worker verdict — it resolves the
A-stranded-then-B-drained case as `all-completed`, which is correct, where `any(stranded)`
would report a condition that had already resolved. And it keeps the cross-mode safety above,
because step 2's "held by this invocation" comes from the ledger rather than from counts: a
`wait`-mode board exiting with its own `in_progress` rows fails step 2 and lands on
`blocked-by-failures`, exactly as today.
...
2. **Mint the invocation token and add the ledger keyed by it** (Decision 2's rule), in the
   request resolver bag rather than `BoardRunFlowState` (§7) — an in-flight attempt counter
   around `dispatcher.claim()`, held entries keyed by `(taskId, attempt)`, per-worker
   last-claim outcomes **stamped with a collection version bumped off the existing
   `onTaskChangeFor` signal**, released by both recorders.
...
3. **Extract `boardQuiescence` as the one shared exit test** (Decisions 7 and 8) and have
   ...
   exits stranded** rather than riding `maxIterations`; and **the wait→resume→claim
   sequence runs the resumed task** rather than exiting `blocked` on stale outcomes
   (Decision 8a — this is the round-3 regression bar and it manifests at flow-integration
   tier, not unit tier).
...
remains only as the wake predicate's promptness disjunct, and stays untouched in `complete`
and `wait`.
...
| **Transient dispatcher decline** (a classifier or custom dispatcher backing off, per `TaskDispatcher.claim`'s documented "exit or back off") | Not treated as a verdict. If nothing in the collection changes, nothing will make the task claimable and exiting is correct; if something changes, the version bump forces a re-ask. |
...
**Flow integration (the tier where the round-3 defect lives).** One scenario, and it does not
reduce to a unit test: a board parks a task `awaiting_review`, every worker goes idle, an
external actor calls `resumeFromReview()`, and **the resumed task must run to completion**.
Assert the task reaches `completed` with its worker's output, and that the drain's verdict is
`drained` rather than `blocked`. Unit-tier coverage of `boardQuiescence` cannot catch this,
because the bug is in the *sequence* — the stale observation is only stale relative to a
mutation that happened between two calls.
