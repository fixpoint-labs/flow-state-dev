...
- **The guard, one place.** `transitionDeclineReason` (`tasks/collection/internal.ts:298`)
  already runs inside both backings' atomic writes, so it evaluates against the freshest
  state by construction. It gains the target-bound question and a new decline reason.
...
- **The refusal is a named union member, not a string.** `TaskWriteDeclineReason`
  ...
  guard change, with its tool-surface mapping in PR e (§8 step 5).
...
1. **The epic's filesystem "done" criterion cannot be written here** (Decision 6). It asks for a
   *detached* durable board to be refused at construction, and detachment does not exist until
   FIX-982. Confirm it is routed there rather than dropped.
2. **The ABA case in §9** — close it by putting `createdAt` in the ticket (one more field, makes
   the ticket total), or accept it as unreachable from the task surface? Recommended: accept,
   and record it.
