...
**The honest limit on durability, corrected in round 2 and now narrower than this spec first
claimed.** On the sequencer and request backings the answer is sound across concurrent
callers, because those writes go through real compare-and-swap. **On the resource backing —
the durable one — it is sound only within a single execution context.** Two concurrent
requests in the *same Node process* each build their own resource registry, their own write
serialization and their own state cache, so one can silently overwrite the other's committed
write, receipt included. That is a lost update in the storage layer, not something this
primitive introduces, but it does bound what the primitive may claim. **Decision 6 carries the
correction and puts the choice between degrading the answer and fixing the store in front of
the repo owner** — it is the one open question in this spec.
...
6. **This primitive is sound within one execution context. It is a detector, not a guarantee, across
   ...
   **This is the spec's one open question, and it is the owner's — see §12 for the (a)/(b)
   tradeoff written out.** It is not resolved here because (a) costs the durable backing its
   definitive answers and (b) widens this issue into every store adapter; picking either
   silently would be the wrong call to make on someone else's behalf.
...
**Size: Medium — *conditional on OQ-1* (§12).** If the owner takes option (a) (degrade the
resource backing's answer), this stays Medium as scoped below. If they take option (b) (give
the resource store real CAS), it becomes **Large and multi-PR** — a store-interface change
across every adapter — and this spec should be split rather than stretched. The estimate below
describes option (a).
...
> **Blocked on OQ-1 (§12) for the resource backing's read behaviour only.** Steps 1–5 are
> buildable now; what the resource backing may *answer* under concurrency is the owner's call,
> so do not harden that arm until it is answered.
...
| **Two concurrent execution contexts write to a durable board (same process or not)** | One silently overwrites the other, receipt included — a lost update in the storage layer, present today. Behaviour here is **OQ-1's to settle** (§12): degrade to cannot-tell on the resource backing, or give the store real CAS. Do not implement a definitive "did not land" on the resource backing until OQ-1 is answered. |
...
siblings is now the only scope the storage layer can actually support, so the boundary is a
correctness consequence rather than a scoping preference, and FIX-990 may want to say so in
those terms. **No change to FIX-990's design follows from this**, and this spec proposes none;
raised on their PR as a non-blocking note.

**Open question — OQ-1, and it is blocking, for the repo owner.** *Round 2 established that
concurrent writes are unsafe between execution contexts in a single process, not only across
processes (Decision 6). What should the resource backing answer while that is true?*

The mechanism, verified: `resourceWriteChains` is per-registry (`resource-registry.ts:540`),
registries are per execution context (`createExecutionContext.ts:1649` onward), state caches are
per-context locals (`:1070-1071`), and `persistResourceKey` blind-`set`s then updates only its
own cache (`:1399-1405`). Two concurrent requests both read revision 3; the second overwrites
the first's committed row, receipt included.

| | **(a) Degrade the resource backing** | **(b) Give the resource store real CAS** |
|---|---|---|
| **What ships** | On the resource backing, never return a definitive "did not land" where an overwrite could have removed a receipt — fall back to cannot-tell. Sequencer and request backings keep exact answers (they have CAS). | `ResourceStateStore.set` grows an expected-version parameter and a conflict result, mirroring the contract `runWithCAS` already relies on for scope state (`stores/cas.ts:119-174`). The resource write path becomes a real read-modify-CAS. |
| **Cost** | The **durable** backing — the one the docs advertise for cross-session boards — mostly answers cannot-tell under concurrency. FIX-963's guarantee then holds strongly on request/sequencer boards and weakly on durable ones, which is close to inverted from where it is most wanted. | Touches `stores/types.ts:546-570`, `persistResourceKey` (`createExecutionContext.ts:1399-1405`), the three instance write ops (`resource-registry.ts:682-720`), **and every adapter** including SQLite, filesystem, memory and the Postgres reference. Turns a Medium issue into a Large one, probably multi-PR. |
| **Also fixes** | Nothing else. | The lost update itself — a live correctness bug today, independent of provenance — plus the stale-cache write-skip recorded below. |
| **Risk** | Low. Honest but disappointing. | Real: a store-interface change with a migration surface, done to serve consumers that do not exist yet. |

**My recommendation, offered as input rather than a decision: (a) now, (b) filed as its own
issue with (a)'s degradation as the stated reason it exists.** Two arguments. First, tenet 3 —
(b) has no consumer today, and the three issues waiting on this primitive are all
single-request. Second, sequencing: (a) is a correct, shippable subset that unblocks FIX-963
and FIX-964 immediately, and it does not foreclose (b) — when (b) lands, the degradation
simply stops firing, with no consumer change. The argument *against* my recommendation is real
and worth the owner hearing plainly: **the epic's objective is an honest substrate, and (a)
leaves the durable backing least able to answer the question the epic is about.** If the owner
would rather fix the foundation now, (b) is the coherent choice and this spec should be split.
**Not decided here.**

*(Everything else in Decision 6 stands: cross-process remains detector-only under either
option, since (b) would make it a guarantee only once every adapter implements the CAS.)*
