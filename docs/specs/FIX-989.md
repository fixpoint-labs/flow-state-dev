...
history 1 — my write committed, then the notify threw, then another worker claimed it
  task.writeLog → [ …, { id: "<mine>", revision: 4 }, { id: "<claim>", revision: 5 } ]
  didWriteLand(task, mine) → true      ← reported. Today: silently called "displaced".

history 2 — my write never landed; lease expired, reclaim, another worker claimed
  task.writeLog → [ …, { id: "<reclaim>", revision: 4 }, { id: "<claim>", revision: 5 } ]
  didWriteLand(task, mine) → false     ← correctly routine.
```
