...
> **Blocked by:** FIX-976 ([#1004](https://github.com/fixpoint-labs/flow-state-dev/pull/1004)), [FIX-992](https://linear.app/fixpoint-labs/issue/FIX-992) ·
...
6. **The resource backing's soundness comes from fixing the store (FIX-992), not from weakening
   ...
   store-visible version (`stores/cas.ts:119-174`, `state-container.ts:390-405`); the resource
   ...
   **⚠ Round-2 correction — "within one process" was wrong, and the correct scope is one
   ...
   `createScopeResourceRegistry` (`resource-registry.ts:540`), the registries are constructed
   per execution context (`createExecutionContext.ts:1649` onward), the state caches are
   per-context locals seeded at context creation (`:1070-1071`), and `persistResourceKey` is a
   ...
   **Resolved by the repo owner: fix the store.** The alternative — having the resource backing
   ...
   - **It was not implementable as scoped.** The read takes a task and a token, and the task
     ...
     (`schema/task.ts`), `TaskHandle` is `Task & { items() }`, and `collectionId` lives on the
     *ref* (`types.ts:144`). The persisted fields are identical across backings, so the reader
   ...
   - **The store fix is far cheaper than round 2 estimated**, because the contract already
     ...
     bare `Promise<void>` documented *"Creates or overwrites"*, and its own header says
     *"plain per-key last-write-wins (no CAS), matching `ContentStore`"* (`:537`, attributed
     there to FIX-689). **That is the mismatch:** it was modelled on the *content* store — a
...
**Blocking dependency 1: FIX-976 ([PR #1004](https://github.com/fixpoint-labs/flow-state-dev/pull/1004),
