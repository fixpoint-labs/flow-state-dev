...
> **§12 carries eight settled empirical claims** — four POC-run before drafting, four settled
> during review. **Five of the eight are refutations, four of them of claims this spec itself
> made**, and every correction is already folded into the framing below. One of the five has
> now been argued twice and settled the same way both times. Please don't reopen them from
> prose.
...
4. **Claims stay scoped to what was measured.** Several claims here have been narrowed or
   withdrawn under scrutiny, including four the spec itself made (§12) — three of them
   *overclaimed scope*, which is this spec's recurring failure mode. When you find yourself
   writing that something is "separate", "unaffected", or "safe", check the shared state before
   asserting it. Prefer a narrower true statement to a broader plausible one; the epic is named
   for that.
...
| Two concurrent drains of one board in one request | **Their task-set view agrees — and that is the only guarantee this change makes.** It does *not* make overlapping drains safe: they share one definition-lifetime `runState` (`task-board/index.ts:613`), and each invocation's install replaces the ledger, cache and policy on it (`flow-policy-wiring.ts:188,197-200`) while each teardown clears them (`:249-255`), so one drain can still disrupt an overlapping one. Known limitation, **out of scope here, owned by FIX-987.** The existing freshness comment (`index.ts:610-612`) only claims a board *re-entered* within a request starts fresh — sequential re-entry, not overlap. |
...
6. **Both consumers read the same classifier verdict**, table-driven over board states ×
   `onIdle` modes. Assert on the *verdict* each consumer acts on — not on wake and exit
   agreeing, which is **not** what Decision 3 guarantees and would be false by design.
   **Include the claimable-pending state as a named row whose expectation is divergence:**
   verdict `continue`, exit continues, **and the wake test returns true** via
   `hasClaimableTask`. That row is what pins the fast path; a test asserting literal
   wake/exit agreement would fail on it, or pressure someone into deleting the fast path and
   silently reintroducing the promptness defect — making the test the cause. Fails if a second
   spelling of the exit rule reappears. Should pass before and after step 3 — a DRY guard, not
   a bug guard.
...
| `packages/orchestration/README.md` | **EXTEND** | The **`### TaskCollection`** section (`:44-51`) documents all three backings, including "resource-collection (outlives the request…)". This change alters that package's observable freshness contract, so the same-request boundary belongs in that paragraph — otherwise the package-local contract stays silent while the site docs and changeset describe it. Required by the repo rule that public API changes update the relevant `packages/*/README.md` in the same change set (BP-009), so this is a deliverable, not a nicety. |
...
- **Settled (review round 2):** that `makeWorker` allocates `cell` at board-*construction*
  time, so a `TaskCollectionRef` from one request or tenant survives into the next — framed as
  a multi-tenant leak — **REFUTED.** The premise is false: `makeWorker` is never called at
  construction. It is passed as the **factory** to `.forEach` —
  `task-board/index.ts:797` is `(workerId: number) => makeWorker(workerId)`, and the only other
  references in the file are the header comment (`:14`) and the declaration itself (`:692`).
  `.forEach`'s `run` invokes `blockOrFactory(item, index, ctx)` **inside `run`**, per item,
  under `mapLimit` (`core/src/blocks/sequencer.ts:1280` `run:`, `:1290` `mapLimit`, `:1293` the
  invocation). So `const cell = {}` (`:701`) is allocated fresh per drain invocation per
  worker; there is no definition-lifetime retention and nothing crosses a request or tenant
  boundary. **Decision 1's scoping is unchanged as a result.**
  *This is the second time the claim has been argued and it settles the same way both times* —
  FIX-978's review reached the identical conclusion against the same two file:line pairs.
  **The likely conflation, recorded so it isn't rediscovered:** `runState`
  (`task-board/index.ts:613`) *is* allocated once in the board factory body and *is* real
  definition-lifetime state — that is a genuine defect, and it is already filed as **FIX-987**.
  The `cell` closure is not it.
  (Codex, [PR #1006](https://github.com/fixpoint-labs/flow-state-dev/pull/1006))
- **Settled (review round 3, accuracy pass):** that two concurrent drains of one board in one
  request have "board state separate and untouched" once their task-set view agrees — **REFUTED,
  and the claim was narrowed.** Verified: `runState` is a single definition-lifetime object
  (`task-board/index.ts:613`, in the factory body), and every invocation's install *replaces*
  the ledger, cache and policy on it (`flow-policy-wiring.ts:188` `runState.cacheStore = store`,
  `:197-200` `runState.ledger = ledger` / `policy` / `collectionId`) while every teardown clears
  them (`:249-255`). So an overlapping drain can still disrupt its sibling **even though the
  task-set view now agrees.** The existing freshness comment (`index.ts:610-612`) claims only
  that a board *re-entered* within a request starts fresh — sequential re-entry, which says
  nothing about overlap. §9's row now guarantees task-set membership **only** and names
  **FIX-987** as the owner of the residue. Distinct from the round-2 refutation: that one was
  the per-worker `cell`, this one is the shared `runState`.
  (Codex, [PR #1006](https://github.com/fixpoint-labs/flow-state-dev/pull/1006))
...
- **After spec review (round 2)** — **reworded the agreement test to what Decision 3 actually
  guarantees**: both consumers read the same *verdict*, with claimable-pending as a named row
  whose expectation is divergence (verdict `continue`, wake `true`). As previously worded the
  test would have failed on that state or pressured someone into deleting the fast path,
  making the test the cause of the defect. Recorded a **refutation of the claim that `cell` is
  allocated at construction time** — it is the `.forEach` factory argument, invoked per drain
  invocation — so Decision 1's scoping is unchanged; the genuine definition-lifetime state is
  `runState`, already filed as FIX-987. **Two-round budget now spent.**
- **Accuracy pass (not a design round; the spec stayed converged)** — **narrowed the
  concurrent-drain guarantee to task-set membership only.** Verified that overlapping drains
  share one definition-lifetime `runState` whose ledger and cache each invocation replaces and
  clears, so the previous "board state separate and untouched" was an overclaim; the residue is
  FIX-987's. **Added `packages/orchestration/README.md`** to the docs plan — the change alters
  that package's observable freshness contract and the package-local docs can't stay silent
  about it. No decision reopened and no approach change: this is the **third** overclaimed-scope
  correction on this spec, so §4's practice now names that failure mode directly.
