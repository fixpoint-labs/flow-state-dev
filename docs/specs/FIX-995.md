...
- **Settled:** four of the unsafe sites report work that never committed — **CONFIRMED**
  ...
  (`packages/memory/test/_fix995-replay-poc.test.ts`, committed to the spec branch and not
  to ship) ran the **real** helpers under a double that replays the updater once. All four
  failed as predicted on `05d1eed4`: `evict()` returned `true` for an entry it did not
  remove; `pin()` returned `true` when the pin slots filled up underneath it;
  `cullByTTL()` returned `["ep_a","ep_b","ep_b"]` for an expected `["ep_b"]`; `markStale()`
  the same. The remaining 11 unsafe sites are classified by reading against the identical
  shape and each gets its own such spec in step 3–5.
