...
- **After spec review (round 1)** — The **enforcement rule was wrong and is rewritten**, because a probe implementing it literally missed the three accumulator sites this spec's own POC prints the failure of: `culled.push(id)` assigns to nothing. Decision 4 now bars all three write forms (assign, mutate-through, property-assign) and resolves a registry of replay entry points rather than direct arguments only, since `casWrite` hides six callbacks behind a wrapper. **Decision 5 was re-costed** from "three lines" to three helper migrations, because moving the read inside cannot change what those functions return without violating the invariant this spec introduces. Net: the migration grew 17 → **24** sites, and §13 now exists.
- **After spec review (round 2)** — Two corrections, both to what round 1 introduced, both **verified by running the rule rather than reasoning about it**. (1) The round-1 helper shape *could not build*: `casWrite` forwards a mutator into `atomicState` and is not a ref, so the four callbacks round 1 added to scope had no shape to migrate to — the helper is now parameterised by the **runner**, which covers both entry points, keeps the exemption at one file, and costs ~20 lines rather than the advertised ~10. (2) Round 1's call rule was **too broad**: it red-lights `assertWithinCaps(next)`, a pure validator, which would have failed `pnpm typecheck` on safe code. Narrowed to receiver-of-a-known-mutator: 21 sites flagged, zero false positives. Site count unchanged at 24.
...
1. **The outcome travels out through the write's return value, via one shared helper —
   not through a closure variable reset per site. The helper is parameterised by the
   *runner*, not by a resource ref, so it covers both mutation runners.**

   Its first argument is a function that applies a mutator (`(mutator) => Promise<unknown>`),
   which both entry points already satisfy: `ref.updateState` directly, and
   `sequencer-backed.ts`'s `casWrite` (`(mutate) => Promise<void>`) **with no change to
   `casWrite` at all**. A thin `updateStateWith(ref, updater)` convenience covers the 20
   resource-side sites so they never see the runner argument.

   *Rejected:* (a) resetting the captured variable at the top of each unsafe callback (§3);
   (b) a helper that takes a resource ref — round 2 found that shape cannot express the four
   `casWrite` callbacks, which would have forced either a second outward capture or a second
   exemption, and either one reopens the judgement call Decision 4 exists to close.
   *Locks in:* the reset exists in exactly one place for *both* replay entry points, so
   Decision 4's exemption stays at one file and `atomicState` is not left as an uncovered
   hole. **Cost of the generalisation: the helper is ~20 lines across both exports, not the
   ~10 an earlier draft advertised.** That is the honest number.
...
4. **A repo-wide static check enforces that an updater treats every binding declared
   outside it as read-only. It ships in `scripts/`, `ts-morph`-based, wired into
   `pnpm typecheck` beside `validate-package-boundaries.mjs`.**

   **"Read-only" means three write forms, not one.** An earlier draft of this decision
   said "assigns to no binding declared outside it". That rule is *wrong* — verified, not
   conceded. Implemented literally and run over the real updaters, it flags 14 of the 17
   capturing sites and **misses exactly the three accumulators** (`cullByTTL`,
   `markStale`, `cullByEffectiveConfidence`) that produced `["ep_a","ep_b","ep_b"]` in
   this spec's own POC — because `culled.push(id)` assigns to nothing. The rule must
   reject all three write forms, and the repo has a live example of each:

   | write form | example |
   |---|---|
   | assignment to the binding | `found = true` — `working-memory-helpers.ts:167` |
   | mutating call through it | `culled.push(ep.id)` — `episodic-memory-helpers.ts:137` |
   | property assignment through it | `reclaimed.length = 0` — `sequencer-backed.ts:478` |

   Mechanically, reject exactly two things: **(a)** assignment / compound-assignment /
   `++` / `--` / `delete` targeting an outer binding **or any member path rooted at one**;
   **(b)** a call in which an outer binding is the **receiver** of a known-mutating method
   (`push`, `pop`, `shift`, `unshift`, `splice`, `sort`, `reverse`, `fill`, `copyWithin`,
   `set`, `add`, `delete`, `clear`). Only bindings declared inside the *enclosing function*
   count, so module imports and globals are out of scope.

   **(b) is deliberately narrow, because the obvious wider rule fails.** Round 2 proposed
   "any expression-statement call whose callee is rooted at an outer binding". Run over the
   real files, that flags **four** call sites, and all four are safe:
   `assertTransitionAllowed(…)` and `Object.defineProperty(…)` (excused anyway — a module
   import and a global), but also **`assertWithinCaps(next)` at `sequencer-backed.ts:294`
   and `:309`, which is not excused**: it is declared at `:160`, *inside* the factory, and
   it is a pure validator over callback-local state. A rule that red-lights safe code in
   `pnpm typecheck` gets widened until it passes, which is worse than no rule. The
   receiver-plus-mutator form flags **26 findings across 21 sites and zero false
   positives** on the same run.

   **The residual hole, stated plainly:** a *custom* mutating method on an outer binding
   (`myBuffer.append(x)`) is not caught. Three things bound it — after migration an updater
   has no reason to hold an accumulator at all, rule (a) still catches the assignment and
   property-assignment shapes, and the check is a backstop rather than the mechanism (the
   helper is). Widening (b) to close it costs the false positives above, which is the worse
   trade.

   **The check sees 21 of the 24 migrations.** Decision 5's three sites are stale
   *reads* — they write nothing outward, so no write-oriented rule can see them. They are
   migrated for correctness, not because the check demands it.

   **The check resolves an explicit registry of replay entry points**, not just direct
   arguments: `updateState`, `atomicState`, the new helper, and `sequencer-backed.ts`'s
   `casWrite`. A direct-argument check is blind to any wrapper that forwards a
   caller-supplied callback — `casWrite` (`:131-148`) takes `mutate` and invokes it at
   `:143` inside the closure it hands `atomicState` at `:136`, so a direct-argument check
   inspects only that internal closure and never sees the six callbacks at `:214`,
   `:260`, `:294`, `:309`, `:342`, `:472`. Any future wrapper must join the registry or
   return its callback's outcome.

   **Exactly one file may write outward inside a replay callback: the helper's own
   module.** The check asserts its exemption list holds that single entry, so it cannot be
   quietly broadened into a no-op. `casWrite`'s internal closure needs no exemption — it
   writes nothing outward.

   *Rejected:* (a) no enforcement, relying on review; (b) introducing ESLint, which this
   repo does not use; (c) the assignment-only rule above, refuted by its own evidence
   section; (d) a rule that permits outward writes when every return path is dominated by
   one — the judgement call this issue exists because people get wrong.
   *Locks in:* the rule is mechanical and, after the migration, false-positive-free. It
   also means the four `casWrite` callers that hand-reset today (`:214`, `:260`, `:342`,
   `:472`) migrate rather than get exempted — permitting `reclaimed.length = 0` as a
   sanctioned "reset idiom" would reintroduce exactly the judgement call the bright line
   removes.

5. **Three sites in an adjacent class — a value derived from state read *outside* the
   callback and then committed inside it — migrate to the outcome helper here.** The
   sites are `working-memory-helpers.ts:128` (reads `state.currentTurn` at `:113`),
   ...
   **These are helper migrations, not the three-line edits an earlier draft claimed.**
   "Move the read inside the callback" does not work: each function constructs its record
   outside `updateState` and `return`s that same object afterwards, so building a fresh
   one inside the callback leaves the function returning the stale one — and assigning the
   fresh one outward violates the very invariant Decision 4 introduces. The only shape
   that works is the one Decision 1 already establishes: return the record as the
   callback's outcome. Cost is three more helper migrations, not three lines.

   *Rejected:* filing them separately.
   *Locks in:* they sit in three functions the migration already rewrites, and leaving a
   known-stale read inside a function just hardened for replay is the incoherence tenet 1
   warns about.

   > **Scope note for the implementer.** Review grew the migration from 17 sites to **24** —
   > the original 17, plus these 3, plus the 4 `casWrite` callers Decision 4 reaches. Both
   > additions are separable from the core fix. If scope needs to move once the work is
   > underway, this is where to look; sizing that trade belongs to implementation, not here.
...
> this defect · application code outside this repo · `atomicState` call sites *other than*
> the four `casWrite` callers Decision 4 reaches — the rest are already correct and the
> check simply keeps them so · `getOrPatchState`, whose `compute` callback runs at most
> once by contract (`resource.ts:310-326`).
>
> **Size:** Large — **24** mechanical site migrations (17 + Decision 5's 3 + Decision 4's
> 4), a **~20-line** runner-generic helper across its two exports, and a check script of
> roughly ~150 lines (two write forms, a mutator set, and a wrapper registry — more than
> the ~80 first estimated). One PR (Decision 7); **no PR plan.** Review grew this from 17
> sites and the helper from ~10 lines. Decision 5's scope note flags where scope can move
> if it needs to; that trade is sized at implementation, not here.
...
- **`@flow-state-dev/memory`**, **`@thought-fabric/core`**,
  **`@flow-state-dev/orchestration`** — migrate their capturing sites (§8). All three
  ...
  In `orchestration` this now covers **both** task backings: `resource-backed.ts` (the
  defective one) and the four `casWrite` callers in `sequencer-backed.ts` (correct today,
  migrated so the check can see them — Decision 4).
...
**The reference discipline already in-repo — and why it is being replaced, not copied.**
`packages/orchestration/src/tasks/collection/sequencer-backed.ts` is the sibling backing of
the same `TaskCollectionRef` interface, and it already runs under a CAS retry loop
(`:121-124`). It solves this by hand: the rule is stated at `:198-200` ("Both `captured`
and `declined` are reset on every mutator entry … so they describe the winning attempt, not
an earlier one"), applied at `:221`, `:225`, `:266`, `:270`, `:273`, `:351`, and — for the
accumulator variant — again at `:474-478`, where `reclaimed.length = 0` carries its own
five-line comment explaining that a replay must not push duplicates from a previous
attempt.

**That file is three proofs at once.** It proves the defect is solvable. It proves the
solution doesn't stick: its own twin, `resource-backed.ts`, implements the same interface
and gets it wrong in three of four places. And it proves the rule is *not one rule* — the
same author had to reason out the assignment form and the accumulator form separately, in
different idioms, with a comment each. That is precisely the judgement call Decision 4's
bright line removes, which is why these callers migrate (step 7) rather than stand as the
pattern to copy. Reason from the file's *intent*, not its mechanism — the helper is that
intent, hoisted to where it cannot be forgotten.

**Solution sketch — pseudocode. Illustrative, not the contract. React to the shape.**

```
the shared helper, in core — parameterised by the RUNNER, not by a resource:

    withOutcome(runMutator, the caller's updater):
        outcome ← nothing                  ← the reset, in the ONE place it can live
        call runMutator with:
    ...
    updateStateWith(resource, updater) = withOutcome(resource's updateState, updater)

a migrated resource call site:
...
a migrated casWrite call site — same helper, casWrite passed as the runner unchanged:

    reclaim():
        return withOutcome(casWrite, given tasks:
            gather the expired ones
            if none:  yield (no change, answer = [])
            else:     yield (tasks with those reset, answer = those tasks))

the check, over the callback at every REGISTERED replay entry point
(updateState · atomicState · the helper · casWrite — not just direct arguments):
    treat every binding declared outside the callback as read-only, i.e. fail on
        writing the binding            (found = true)
        writing through the binding    (culled.push(…), reclaimed.length = 0)
    (the helper's own module is the single audited exception)
```
...
6. **Migrate the three stale-read sites** per Decision 5 — `working-memory-helpers.ts:128`,
   `perspective-helpers.ts:227` and `:349`. Each returns its constructed record as the
   callback's outcome, so the record is built from the state that actually committed.
   *Test:* a replay returns a record stamped from the **winning** attempt's turn counter /
   position index, not the losing one. *Depends on:* 3, 4 (same functions).
7. **Migrate `sequencer-backed.ts`'s `casWrite` callers** per Decision 4 — the four that
   write outward today: `:214`, `:260`, `:342` (`captured` / `declined`) and `:472`
   (`reclaimed.push` + `reclaimed.length = 0`). They pass `casWrite` to the helper as the
   runner; **`casWrite` itself does not change.** **Remove** each manual reset, including
   the explanatory comment at `:474-478`, which the shape makes unnecessary. Behaviour is
   identical; these are already correct, and the point is that the check can now see them.
   `:294` and `:309` write nothing outward and need no change — note their
   `assertWithinCaps(next)` calls are exactly what Decision 4's narrowed rule must *not*
   flag. *Test:* the existing sequencer-backed suites pass unchanged. *Depends on:* 1.
8. **Add the check** and wire it into the root `typecheck` script. *Test:* §10.
   *Depends on:* 2–7 — it must land last or it fails on the unmigrated sites.
9. **Docs and changeset** per §11. BP-022: user-facing (a new exported helper), so a real
   `.changeset/*.md`, `patch`.
...
closure per write. *Flag off/new state* — no flag; the "off" state is the 16 safe sites
that stay on plain `updateState`, which the check must pass unchanged.
...
- **The check itself, one case per write form Decision 4 names** — it must fail on an
  updater that assigns an outer binding, on one that calls `.push(...)` through an outer
  binding, and on one that assigns `.length` through an outer binding. **The middle case
  is the regression test for this spec's own round-1 defect**: the assignment-only rule
  passed it, which is how it reached review. It must also fail on a callback reached only
  *through a wrapper* (`casWrite`), or the registry is decorative.
- **The check's negative space — the half that keeps it usable.** It must **not** flag: a
  binding the updater declares itself; module imports or globals; the 16 safe sites that
  stay on plain `updateState`; and specifically **`assertWithinCaps(next)` at
  `sequencer-backed.ts:294` and `:309`** — a pure validator declared at `:160` inside the
  factory, over callback-local state. That case is the regression test for round 2's
  finding, the same way the `.push(...)` case is round 1's. Its exemption list holds
  exactly one entry.
...
- **Settled:** Decision 4's original rule ("an updater assigns to no binding declared
  outside it") does not catch the defect it was written for — **REFUTED**, by running it
  rather than arguing it. Round 1 raised this; a throwaway `ts-morph` probe implementing
  the rule *literally* and run over the real updaters flagged **14** of the 17 capturing
  sites and **missed exactly the three accumulators** — `episodic-memory-helpers.ts:114`,
  `:160` and `semantic-memory-helpers.ts:166` — because `culled.push(id)` assigns to
  nothing. Those three are the sites whose stale output this spec's own POC prints. A
  second form, `reclaimed.length = 0` (`sequencer-backed.ts:478`), is likewise neither an
  assignment to the binding nor a method call. Decision 4 now names all three write forms.
  The probe was a scratch script, kept only long enough to produce this verdict.

- **Settled:** round 1's replacement rule ("any expression-statement call whose callee is
  rooted at an outer binding") is too broad and would fail CI on safe code — **CONFIRMED**,
  again by running it rather than reasoning about it. Over the same files it flags four
  calls, all safe: `assertTransitionAllowed(…)` and `Object.defineProperty(…)` (excused by
  the imports/globals carve-out) and **`assertWithinCaps(next)` at
  `sequencer-backed.ts:294` and `:309`, which is not excused** — declared at `:160`, inside
  the factory. Narrowing (b) to "an outer binding is the receiver of a known-mutating
  method" yields **26 findings across 21 sites and zero false positives** on the same run,
  and still catches all three accumulators plus `reclaimed.push` / `reclaimed.length = 0`.

- **Settled:** the round-1 helper shape could not express the four `casWrite` callbacks —
  **CONFIRMED** from the signatures. `casWrite` is `(mutate) => Promise<void>` forwarding
  into `atomicState`, which is `(mutator) => Promise<boolean>`
  (`packages/core/src/types/state.ts:47`); neither is an object carrying `updateState`, so
  a ref-shaped helper cannot take them. Decision 1's helper is now parameterised by the
  runner, which both satisfy — and `casWrite` needs no change to qualify.
...
- **Already noted by FIX-992's review:** *treat an audit boundary as a claim to be
  ...
  **classification** moved a lot, and that is what sizing depended on. Round 1 then found
  the same failure one level up: the *enforcement rule* was asserted rather than run
  against the enumeration it was written to protect. A rule is a claim too.

### 13. Review notes for the implementer

Below-the-bar round-1 feedback, recorded verbatim and **not** folded into the design.

- "Hoist a shared `createReplayingUpdateStateRef` into `@flow-state-dev/testing` rather than copying the replaying double into ~15 test files; the orchestration cases fit `advisory-write.test.ts`'s existing parameterization." — cursor ([PR #1022](https://github.com/fixpoint-labs/flow-state-dev/pull/1022))
- "Add replay coverage for the three Decision 5 sites — double-invocation assertions that the returned timestamp/index comes from the winning attempt." — codex ([PR #1022](https://github.com/fixpoint-labs/flow-state-dev/pull/1022)) *(Note: Decision 5 now migrates these to the helper, and step 6 carries exactly this assertion, so this may already be satisfied — the implementer should confirm rather than assume.)*
- "Implementation should fold the four proven behaviours into the normal package suites and delete the throwaway POC." — cursor ([PR #1022](https://github.com/fixpoint-labs/flow-state-dev/pull/1022))
- "Use typed refs instead of `as any` when the replay doubles ship." — cursor ([PR #1022](https://github.com/fixpoint-labs/flow-state-dev/pull/1022))
- "Trim the meta-prose." — cursor ([PR #1022](https://github.com/fixpoint-labs/flow-state-dev/pull/1022))
