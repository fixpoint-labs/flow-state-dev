...
4. **A repo-wide static check enforces that an updater treats every binding declared
   ...
   **"Read-only" means three write forms, not one.** An earlier draft of this decision
   ...
   Mechanically: reject assignment / compound-assignment / `++` / `--` / `delete`
   targeting an outer binding **or any member path rooted at one**, plus any
   expression-statement call whose callee is rooted at one. Only bindings declared inside
   the *enclosing function* count, so module imports and globals (`console`, `Math`) are
   out of scope and cannot false-positive.
...
5. **Three sites in an adjacent class — a value derived from state read *outside* the
   ...
   *Locks in:* they sit in three functions the migration already rewrites, and leaving a
   ...
   > **Scope note — the one place the owner may reasonably cut.** Review grew the
   > migration from 17 sites to **24**: the original 17, plus these 3, plus the 4
   > `casWrite` callers Decision 4 now reaches. Both additions are separable. Cutting them
   > yields the smallest change that unblocks FIX-992, at the cost of a documented blind
   > spot in `sequencer-backed.ts` and a known-stale read in three helpers. I recommend
   > keeping them — but this is the scope call worth making deliberately rather than
   > inheriting.
...
> 4), one ~10-line helper, one check script (bigger than the ~80 lines first estimated:
> three write forms and a wrapper registry, call it ~150). One PR (Decision 7); **no PR
> plan.** Round-1 review grew this from 17 sites; see Decision 5's scope note.
...
7. **Migrate `sequencer-backed.ts`'s `casWrite` callers** per Decision 4 — the four that
   ...
   (`reclaimed.length = 0`). **Remove** each manual reset, including the explanatory
   comment at `:474-478`, which the shape makes unnecessary. Behaviour is identical; these
   are already correct, and the point is that the check can now see them. `:294` and
   `:309` write nothing outward and need no change. *Test:* the existing sequencer-backed
   suites pass unchanged. *Depends on:* 1.
...
- **The check's negative space:** it does not flag a binding the updater declares itself,
  does not flag module imports or globals (`console`, `Math`), does not flag the 16
  unmigrated safe sites, and its exemption list holds exactly one entry.
