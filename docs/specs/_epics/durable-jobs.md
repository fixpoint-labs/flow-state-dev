...
3. **(Conditional — C3.) Redelivery, not merely reclamation.** A stranded claim returns to the
   ...
   carried: FIX-978 ships the verb, FIX-1005 ships the recovery. **A pending task with no live initiating request has no
...
- **(2a) — SATISFIED 2026-08-07, and it gated FIX-982 starting**, not FIX-981: its blockers having
  Linear ids. `create-if-absent` is **FIX-1007**, S1a is **FIX-1009**, and FIX-991a is **withdrawn**
  (N64). Nothing in FIX-982's blocker set is unfiled. This condition is spent — FIX-982 is gated by
  those issues *landing*, which is (3)'s kind of constraint, not by their existing.
- **(2b) — RESTATED, and it gates the epic *wrapping*, not any lifecycle starting.** It read "all six
  consumer-surface rows having ids"; all six have ids as of 2026-08-07 (**FIX-1009 · FIX-1010 ·
  FIX-1011 · FIX-1012 · FIX-1013 · FIX-1014**), so that phrasing is permanently true and gates nothing.
  **The condition that actually gates finishing is: S4/FIX-1013 has RUN and produced §5's five pass
  criteria.** "Every issue merged" is not "the epic is done" until then — an id was only ever a proxy
  for the evidence being tracked, and the proxy has outlived its use. **Note this is not satisfiable
  today: S4's prerequisites S1b/S2/S3 are deferred — see OQ-H, which is open and the owner's to
  settle.** Conflating this condition with the one that gates *starting* is what froze FIX-981 once.
- **(3) — SATISFIED, and it gated FIX-981 specifically:** `FIX-995 → FIX-992` landing. **Both are
  DONE** — FIX-995 on 2026-08-03, FIX-992 on 2026-08-06 (#1035/#1036/#1039), verified against Linear.
  **So FIX-981 has no outstanding blocker at all**; it is *Spec Approved* and free to run, and every
  condition in this list is now spent. **FIX-925 is gated only by (1) plus its own spec PR merging** (#900 is unmerged, §7) —
...
> **FIX-981 WAS blocked by [FIX-992](https://linear.app/fixpoint-labs/issue/FIX-992); it is not any more.** FIX-992 is **DONE (2026-08-06)** and FIX-995 before it (2026-08-03), so the `blocks` relation this note was written to record is satisfied and **FIX-981 is unblocked**. What survives is the division of labour, unchanged: FIX-992 delivered the conditional write M1 fences with, and explicitly *does not* close claim safety — its own non-goals name **FIX-981 as "enables both, closes neither"** for the claim protocol and the `maxInstances` race. **FIX-981 consumes a primitive that now exists.**
...
|---|---|---|---|---|---|
| **FIX-982** | M3 | No out-of-request executor — a leased task can't run outside its request | ~~N68 (the detached-board scope decision)~~ **DECIDED 2026-08-07 — option A, no longer blocking; the settlement seam is now FIX-982 scope** **+** FIX-981 **+** FIX-978 **+** [FIX-1007](https://linear.app/fixpoint-labs/issue/FIX-1007) (create-if-absent) **+** [FIX-1009](https://linear.app/fixpoint-labs/issue/FIX-1009) (S1a, parent-session store filter — **filed, spec approved [#1058](https://github.com/fixpoint-labs/flow-state-dev/pull/1058)**). **N68 is answered** (option A): session scope is preserved and FIX-982 builds the narrow trusted parent-board settlement seam. The four technical dependencies are now the whole blocker set. **FIX-991a is withdrawn** — the result-read surface was filed as FIX-1008 and cancelled (N64); settlement holds the value in a live context, so there is no cycle to break | — | — |
| **FIX-983** | M4 | Tasks have no blocking/background disposition | **NOT SCHEDULED — deferred at the gate (N52), not merely blocked.** Cross-request waiting has no consumer; it becomes eligible only when a concrete one appears, **not** when FIX-982 lands. §5's sequence already excludes it (N69); this row said otherwise (N71). | — | — |
...
|---|---|---|
| **FIX-984** (M5) | sub-issue of FIX-939 | **DISSOLVED — to be closed, not blocked, and this row moved here from "Filed, held as blocked" on 2026-08-07.** The owner accepted N35's in-flight-delta gap as a documented limit (OQ-C), so M5 has no remaining scope: completed-item progress is the background request's own item stream, and the lifetime residue is FIX-991's. **It is not waiting on FIX-982** — the old row said so, and a coordinator reading it would hold FIX-984 for a landing that changes nothing about it. **Closing it in Linear is an owner/coordinator action, not taken here** (this pass touched no Linear issue); until it is, expect Linear to show it *Backlog*. |
...
| Set | Size | Members |
|---|---|---|
| **Sub-issues** — parented under FIX-939 | **16** | FIX-981, FIX-982, FIX-983, FIX-984, FIX-991, FIX-925, FIX-957, FIX-825, **FIX-1005** (M2, *Backlog*), **FIX-1007** (create-if-absent, *In Spec Review*), **and the six consumer-surface issues** — **FIX-1009** (S1a, *In Spec Review*), **FIX-1010** (S1b), **FIX-1011** (S2), **FIX-1012** (S3), **FIX-1013** (S4), **FIX-1014** (S5), all *Backlog* — *FIX-1008 was a seventeenth and is cancelled (N64)*. **Was 10: the six S-rows are filed and parented as of 2026-08-07, so they are sub-issues now rather than a separate unfiled bucket** (verified against Linear) |
| **Active set** — once the gate passes | 2 | FIX-981 (M1), FIX-925 (independent) |
| **Filed, held as blocked** — eligible when the blocker lands | 3 | FIX-982, FIX-991, **FIX-1005** (M2 — blocked on FIX-981's primitive). **Was 5.** FIX-984 left (closed as dissolved, OQ-C) and FIX-983 was never blocked in the first place — its own row has said *"NOT SCHEDULED — deferred, not merely blocked"* since N71, and this count had not caught up |
| **Filed, NOT scheduled** — eligible when the *owner* says so, not when anything lands | 5 | **FIX-983** (M4 — deferred at the gate, N52/N71) · **FIX-1010** (S1b), **FIX-1011** (S2), **FIX-1012** (S3), **FIX-1014** (S5) — deferred by the owner 2026-08-07 (OQ-E). **Rendering any of these as "blocked" is the error this table now guards against:** a blocked issue starts when its dependency lands, a deferred one does not start at all |
| **Filed, closed as dissolved** — no successor state | 1 | **FIX-984** (M5) — owner, 2026-08-07 (OQ-C). Neither blocked nor deferred: it will not become eligible. Linear close is an owner/coordinator action, not taken in this pass |
| **Carried consumer-surface work** | 1 | **FIX-1013** (S4) — the epic's BP-003 evidence path; **gates the wrap** (condition 2b). See **OQ-H**: its prerequisites S1b/S2/S3 are in the deferred set above |
...
| **External dependency — LIVE** (owned by FIX-980) | **1** | **FIX-978** (blocks FIX-982) — *Backlog*. **The only external dependency still outstanding.** |
| **External dependency — SATISFIED**, retained for the audit log | 2 | **FIX-992** — **DONE 2026-08-06** (#1035/#1036/#1039), the conditional write FIX-981 fences with · **FIX-995** — **DONE 2026-08-03**, FIX-992's replay-hardening prerequisite. **Neither blocks anything now.** This row read *"FIX-992 (blocks FIX-981; In Spec Review) · FIX-995 (Ready to Spec)"* until 2026-08-07 while §6's OQ-A row already recorded FIX-992 as merged — and **FIX-981 is the released, startable issue** (*Spec Approved*), so a coordinator reading this authoritative row would have re-frozen the one issue that is free to run, on a blocker that no longer exists. Verified against Linear, not inferred |
| **The consumer surface, in scope per OQ-E** | 6 | **All filed and parented as of 2026-08-07** — *S1a* **FIX-1009** store filter + adapters (spec approved, [#1058](https://github.com/fixpoint-labs/flow-state-dev/pull/1058)) · *S1b* **FIX-1010** route & request metadata · *S2* **FIX-1011** `client` · *S3* **FIX-1012** `react` · *S4* **FIX-1013** kitchen-sink (**the epic's evidence path — gates the wrap**) · *S5* **FIX-1014** docs. **This row was headed "Unfiled" and is not any more**; the six are counted inside sub-issues above, and repeated here because the S-numbering is how §5 and §7 refer to them |
| **Unfiled blockers** — must be filed before FIX-982 starts | **0** (+1 conditional) | **NONE REMAIN.** ~~*create-if-absent* (N5a)~~ — **filed as [FIX-1007](https://linear.app/fixpoint-labs/issue/FIX-1007)** (In Spec Review) · ~~*FIX-991a — the result-read surface*~~ — **withdrawn (N64)**: it was filed as FIX-1008 and cancelled; FIX-991 needs no split · ~~*S1a*~~ — **filed as [FIX-1009](https://linear.app/fixpoint-labs/issue/FIX-1009)**, spec approved; it must still *land* before FIX-982 (N16; **not** get-or-create's lookup, which is a keyed `get` on the derived id), but landing is a dependency, not a filing gap · **conditional:** a batched cursor read (`RequestListOptions.ids`) becomes a blocker **only if** FIX-982 takes REFERENCE forking without accepting the N+1 (N36) |
| **Indexed rows** = sub-issues (16) + external dependencies (1 live + 2 satisfied) | **19** | *Unchanged at 19, and the arithmetic moved twice rather than growing: the six consumer rows were counted as a separate unfiled bucket and are now sub-issues (10+3+6 → 16+3), and the three external rows now split 1 live / 2 satisfied. **The index is an audit log, so a satisfied dependency keeps its row; only the LIVE count gates anything.** FIX-991a's row is withdrawn (N64); FIX-1008 is cancelled and excluded.* |
...
> **The create-if-absent blocker is indexed here deliberately, and it is now FILED.** §5 names it
> and puts it first in the execution sequence, but for several rounds it appeared in neither the
...
> FIX-1007 (create-if-absent) + FIX-1009 (S1a)`** — FIX-991a is withdrawn (N64) — and **all four now
> have Linear ids** (2026-08-07). The remaining constraint is that they *land*, which Linear's
> relations carry; the filing gap this note was written about is closed.
>
