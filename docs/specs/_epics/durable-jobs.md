...
| **Independent — startable at the gate alongside FIX-981** | 1 | FIX-925 (enters post-spec, §5) — **but its stated gate is unreachable and needs an owner decision (N63).** PR [#900](https://github.com/fixpoint-labs/flow-state-dev/pull/900) is **closed, unmerged, labelled `wontfix`**; it will not merge. `docs/specs/FIX-925.md` is **not on `main`**, while every other landed spec is (FIX-895/911/918/951/954/990/995), so "spec PRs are never merged" is not this repo's convention despite #900's body saying so. Until the owner says whether to revive #900, re-spec, or drop FIX-925, **the active set is FIX-981 alone.** |
