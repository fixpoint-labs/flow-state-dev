...
| **Indexed rows** = sub-issues (16) + external dependencies (3) | **19** | *Unchanged at 19, and the arithmetic moved rather than grew: the six consumer rows were counted as a separate unfiled bucket and are now sub-issues, so 10+3+6 became 16+3. FIX-991a's row is withdrawn (N64); FIX-1008 is cancelled and excluded.* |
