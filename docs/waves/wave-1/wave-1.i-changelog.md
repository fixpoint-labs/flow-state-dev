...
- Added session client support for list/get/create/delete, session request listing, and state snapshots with optional `include_items` and projection filters.
...
- Added React-facing wrappers for flow/session/projection/action/request-stream usage (`useProjections` introduced, `useTypedFlowClient` removed).
- Added item render helpers (`ItemRenderer`, `ItemsRenderer`, `MessagesRenderer`, `BlockRenderer`) with FlowProvider context renderer resolution and `useBlockContext`.
- Aligned core/server contracts with the React direction: `renderKey` naming, grouped projections in snapshots, and projection computation for all scopes.
...
| React hook wrappers | `packages/react/src/hooks/useFlow.ts`, `packages/react/src/hooks/useSession.ts`, `packages/react/src/hooks/useProjections.ts`, `packages/react/src/hooks/useAction.ts`, `packages/react/src/hooks/useRequestStream.ts` |
