...
metrics keys: guidanceVsConsensus, revisionTrend, callTone, ratingsPosture, filingFlags.
...
body sections (exact h values, in this order):
  ...
  2. "Earnings call"          — guidance vs consensus, management tone, Q&A signal from transcript content. Degrades to "transcript unavailable — no FMP key or no transcript for this quarter" when transcript.available === false.
  3. "Consensus & revisions"  — beat/miss history, revision direction, ratings distribution, price-target dispersion from estimates. FMP-only fields (consensusEstimates, priceTargets, recentRatingActions) degrade honestly when null.
  4. "Read for {ticker}"      — the cross-cutting synthesis: how the quarter + guidance + Street posture net out for this name. This section earns the analyst's slot — connect the filing read, call read, and consensus picture into a coherent disclosure-grounded view.
  5. "Data coverage"          — honest accounting of what resolved vs. was unavailable (no FMP key, non-US filer, no transcript, stale filing). Name the source tags.
