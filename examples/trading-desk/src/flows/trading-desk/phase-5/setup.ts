 ...
 * `setupPhase4Memos`: explicit `null` for every nullable field on
 * `memoStateSchema` (Zod defaults are not applied at runtime writes),
 * spreads existing `memoStatus` so prior phases' entries survive, and
 * flips `activePhase` to `"phase-5"`.
 *
  ...
  execute: async (_input, ctx) => {
    ...
    for (const [shortName, mapping] of Object.entries(PHASE_5_MEMO_KEYS)) {
      ...
      const initial = {
        ...
        completedAt: null,
        errorMessage: null,
        // Phase 2 extension fields (null on P5 memos).
        stance: null,
        conviction: null,
        keyRisks: null,
        keyOpportunities: null,
        unresolvedDisagreements: null,
        // Phase 3 extension fields (null on P5 memos).
        direction: null,
        sizePct: null,
        stopPrice: null,
        targetPrice: null,
        ...
        dependsOn: null,
        // Phase 4 extension fields (null on P5 memos).
        posture: null,
        raisedRisks: null,
        ...
        // Phase 5 extension fields (populated at commit time).
