...
import { validateCitations } from "../src/flows/trading-desk/phase-2/validate-citations";
    ...
    const integrity = await run([
      {
        round: 1,
        agentName: "bullResearcher",
        text: `[memo:macro "rates will fall"] and [memo:fundamentals "${FUNDAMENTALS_QUOTE}"]`,
      },
    ]);
    // Only the fundamentals tag is a valid analyst; `macro` never matches.
    expect(integrity.tagsChecked).toBe(1);
    expect(integrity.tagsValid).toBe(1);
  });
