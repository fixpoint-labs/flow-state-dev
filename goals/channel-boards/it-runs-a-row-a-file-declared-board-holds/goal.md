...
**Input:** `fixtures/workforce/` — one team `eng`, two workers (`em`, `coder`) and one channel `feature` whose `CHANNEL.md` declares `members: [eng.em, eng.coder]` and `boards: [triage]`. Held out: the channel's id, the board's local name, the members and which seat runs which kind all come off the tree at runtime — `run.mts` reads `channel.declared.boards[0]` and `channel.id` rather than naming either. Rename the team folder, the channel folder or the board and a correct implementation still passes; every assertion is phrased against what the tree said.
