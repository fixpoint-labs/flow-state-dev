...
import { HELLO_CHAT, intentFreeEnv, loadFixture, runGoal } from "../../lib/index.mts";

const fixture = loadFixture<{ name: string; statement: string; question: string }>(
  import.meta.url,
  "conversation.json",
);
...
await runGoal(() => {
  ...
  // override. (This previously deleted only FSDEV_INTENT_PLAN/_REASON by name,
  // so any other pinned intent still broke the run — intentFreeEnv strips the
  // whole prefix.)
  ...
  return {
    failures,
    evidence:
      `the answer to "${fixture.question}" contained "${fixture.name}", recovered from turn-1 history ` +
