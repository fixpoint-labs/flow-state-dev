...
|------|--------|-------|---------|-------|
| 2026-09-22 | `432f08554` (on `fix/FIX-1497` at `51064a891`, which carries `main` at `d3700555b`) | n/a | **FAIL: row 4 PASS, row 6 not graded** | The first graded run after #2051 lifted ER-26's fence. **Row 4 (multi-seat-collab hire):** `eng.builder`, whose own file answers for `build`, parked the row on its own. Its run, opened from the navigator, showed `parked`, and the Reason column read *"may the backup take the Sunday maintenance slot instead, or does finance need that too?"*, which is exactly the ledger's reason, with no expander open. The text is clamped, and the whole reason is on the cell's title. At the graded 1600px window, 180px of the cell is visible. At 1440px 20px is visible, and at 1280px none is. The pane scrolls horizontally (see Findings). **Row 6: not graded.** No live hire declares a sealed document. `devforce-lab`'s handbook was tried under `org/references/` and restored, because the move reddens that lab's model-free gate (details in the PR). The subject is open with the owner. |
