# harness-child-session › it reconstructs a run from state alone
...
**Input:** `fixtures/input.json` — the child session topic, two files the run is asked to create,
...
Pairing is scoped **within a run**. A child session is reused, so two requests touching the same
...
- Assertion 2 paired across the whole child session rather than within a run, which would have failed
  a faithful record the moment a reused child session held two requests touching one path.
...
- **Two runs in one child session are never produced by a real dispatch.** This goal dispatches one
...
- **Two runs in one child session are never GRADED.** The expectation belongs to the run this goal
  dispatched, and a child session holding more than one request has runs no expectation can be
...
That the child session is a child session carrying the held-out topic; that `include_items=true` is
