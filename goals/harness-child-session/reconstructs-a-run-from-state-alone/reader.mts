 ...
 *
 * A child session is reused, so an account spanning runs is an account with two
...
/** The state's answer to "what happened in this child session", partitioned by run. */
...
async function readCollection(
  ...
  for (;;) {
    ...
    const page = (await read(
      `/sessions/${childSessionId}/resources/${collection}?${query.toString()}`,
    )) as CollectionPage;
    pages += 1;
...
/**
 * Derive an account of what a child session's runs did, from state alone.
 *
 * `read` is the only input besides the child session's own id. Every collection
 * read is scoped to one run's namespace and paged to exhaustion, and every
 * derivation lands on that run's view.
 */
export async function readAccount(read: Read, childSessionId: string): Promise<Account> {
  const listing = (await read(`/sessions/${childSessionId}/requests?include_items=true`)) as {
  ...
  for (const request of requests) {
    ...
    const files = await readCollection(read, childSessionId, OBSERVED_FILE_OPS, runId);
    ...
    const gapPage = await readCollection(read, childSessionId, OBSERVED_GAPS, runId);
    ...
    const planPage = await readCollection(read, childSessionId, OBSERVED_PLAN, runId);
