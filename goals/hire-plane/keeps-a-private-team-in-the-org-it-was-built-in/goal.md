...
**Signal:** against `createFlowState`'s HTTP router and worker pool, in-memory stores, debug endpoints on, no model. Each person hires a private `research` seat through the app's branded private roster writer; the host reloads the roster with `reloadHiredSeats` and registers each seat under the pin copied from its row. Callers: Alice in Acme (owner), Alice in Globex, Bob in Acme.
