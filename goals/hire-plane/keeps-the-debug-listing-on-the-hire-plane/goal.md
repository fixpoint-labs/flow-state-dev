...
- Drop `ownerKeyAdmits` from `collectionKeyVisible` in `packages/engine/src/routes/debug-snapshot.ts`. Must FAIL leg (b) only.
...
| 2026-09-24 | `f8c299b8e` + FIX-1549 PR-A, with `ownerKeyAdmits` dropped from `collectionKeyVisible`, then restored | n/a | FAIL (expected) | **Leg (b) only:** `teammate debug listing contained the owner's marker`, `teammate private topics ["~alice/research"]`, `teammate private itemCount was 1`. |
| 2026-09-24 | `f8c299b8e` + FIX-1549 PR-A | n/a | PASS | Legs (a)–(c) green on the generic key predicate. |
