/**
 * The orchestrator's production `claude` exec: runs the CLI under a
 * pseudo-terminal via script(1) so `claude --remote` sees a TTY.
 *
 * Why this exists: `claude --remote` refuses to dispatch a cloud session unless
 * stdout is a TTY — a bare subprocess (stdout is a pipe) auto-engages local
 * `--print` mode and exits 1 with "--remote requires an interactive terminal".
 * The package's `defaultClaudeCliExec` is a bare spawn, so it can never
 * dispatch; the `resolveClaudeCli` seam exists precisely so the host injects a
 * working strategy. We allocate a PTY with script(1) — the same workaround the
 * `fsd:dispatch-remote` skill uses (.claude/skills/dispatch-remote/dispatch.mjs,
 * the canonical reference) — and scrub the inherited environment so the
 * dispatched child starts clean: CLAUDE_* / CLAUDECODE (parent-session state) and
 * the local ANTHROPIC_API_KEY/ANTHROPIC_AUTH_TOKEN, which would otherwise make
 * `claude --remote` block on a "Detected a custom API key" confirmation dialog.
 *
 * Output is read from script(1)'s forwarded stdout stream (the typescript file
 * is only flushed on exit, so it's just a close-time fallback) with ANSI/VT
 * sequences stripped, so the package's `parseRemoteDispatchOutput` reads the
 * banner and resolution can fire the instant the session URL streams in.
 */
import { spawn } from "node:child_process";
import { mkdtempSync, readFileSync, rmSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import type { ClaudeCliExec, ResolveClaudeCli } from "@flow-state-dev/claude-code/cli";
 ...
 * custom `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` (which `babysit` loads from
 * `.env.local` for the framework's own generator calls) it blocks on a
 * "Detected a custom API key" confirmation dialog that hangs the dispatch.
 */
...
/**
 * Strip ANSI/VT control sequences and script(1) EOF markers from PTY output so
 * the dispatch banner regexes match cleanly. PTYs emit CRLF and terminal setup
 * sequences around the real content.
 */
 ...
 * banner appears the session exists in the cloud, so we resolve `code: 0` and
 * detach (kill the local attach) — a cold start that leaves `claude` lingering
 * does not stall the dispatch. Otherwise it resolves on natural exit (a failed
 * dispatch is a non-zero `code`). It rejects only when the process can't be
 * launched (ENOENT) or the timeout fires with no banner — and that rejection
 * carries the captured output/stderr so the failure is diagnosable.
 */
export const scriptPtyClaudeCliExec: ClaudeCliExec = (bin, args, opts) =>
  new Promise((resolve, reject) => {
    const dir = mkdtempSync(join(tmpdir(), "fsd-orchestrator-dispatch-"));
...
/**
 * Production resolver for the orchestrator: run PATH `claude` under a PTY so
 * `claude --remote` dispatches instead of being rejected. Inject this in the
 * babysit CLI; tests stub the resolver instead.
 */
