/**
 * Tests for the orchestrator's production `claude` exec.
 *
 * These encode the FIX-832 root cause: `claude --remote` refuses to dispatch
 * unless stdout is a TTY. A bare spawn (the package default) pipes stdout, so
 * the child sees no terminal and `--remote` exits 1. The PTY exec runs the CLI
 * under script(1) so the child sees a real terminal. The probe runs `node`
 * (a stand-in for `claude`) and reports whether its stdout is a TTY — no cloud
 * dispatch, so the test is hermetic.
 */
import { describe, it, expect } from "vitest";
import { defaultClaudeCliExec } from "@flow-state-dev/claude-code/cli";
import { scriptPtyClaudeCliExec } from "../../src/driver/claude-cli-pty";
...
describe("scriptPtyClaudeCliExec", () => {
  ...
  it("resolves once the session URL has streamed in, without waiting for exit", async () => {
    ...
    // process exiting (cold starts hung babysit for the full timeout) — but we
    // also must wait for the `View: <url>` line so the handle carries the URL,
    // not resolve on the earlier "Created remote session" line and lose it.
    // Fake a claude that prints the header first, the URL ~800ms later, then
    // hangs.
  ...
  it("does not leak a local ANTHROPIC_API_KEY into the dispatched child", async () => {
    ...
    // the logged-in user, so the orchestrator's local key must be scrubbed.
