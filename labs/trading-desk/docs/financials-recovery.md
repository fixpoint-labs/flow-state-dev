...
| 3 | **Registration / prospectus filings** (S-1, 424B*, F-1) → **bounded LLM transcription** → hard validate | one model call |
| — | Honest `unavailable` + `recoveryAudit` | — |

Recovery (tier 3) is a two-step path: discover S-1 / 424B* / F-1 primaries and
fetch their HTML, then one bounded model call transcribes the audited statements,
which the hard validator gates before anything reaches the spine. There is no
deterministic HTML-table parser — a brittle regex tier that used to precede the
model was removed (FIX-913); the model is the only reader, the validator is the
only gate.
...
A prospectus-derived candidate (the bounded model transcription) may only update
the spine if it clears every hard gate (`validate-financial-candidate.ts`):
...
- **Reconciliation** — when operating cash flow, capex, and a stated FCF are all
  ...
  absolute and 1% relative). An unreconciled triple is rejected. The recovery
  model no longer transcribes FCF (it is always derived from operating − |capex|),
  so this gate is dormant on the model path — it stays as a tier-agnostic
  invariant guarding any hand-built candidate.
...
Tiers 1–2 are keyless and free. The prospectus recovery tier (the bounded LLM
extract) is a **correctness path, not analyst color**, so — unlike `investigate`
...
- `validate-financial-candidate.spec.ts` pins the hard-reject gate matrix (the
  validator is unchanged and tier-agnostic — this coverage outlived the deleted
  parser).
- `critical-financials-recovery.spec.ts` pins the runtime with the model tier
  mocked: single-flight, promote, no-candidates, stale-reject, extract-failed,
  and the supersession-before-model-resolution cost cap.
- `financials-recovery-spine.spec.ts` drives the three statement tools live
  (providers + the model extractor mocked to the SPCX shape) and asserts the
  prospectus statements land on the spine, tagged `edgar-prospectus`, with a
  `promoted` audit — the end-to-end regression that a newly listed issuer is not
  mislabeled as having no primary financial statements.
- `goals/trading-desk-financials/ipo-prospectus-recovery/` is the machine-checked
  goal — it drives the REAL model tier (a 5-run promote-stability loop plus a
  zero-model anti-game reject), the only place model-tier flakiness against the
  validator is measured.
