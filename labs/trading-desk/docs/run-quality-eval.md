...
| `evidence/*` | valuation spine, reward-to-risk resource, PM memo, snapshot | the evidence-sufficiency verdict recomputes from the spine + reward-to-risk bases + the persisted `criticalDataThin` and mirrors agree (recompute ↔ memo ↔ snapshot); on `insufficient-evidence` the committed decision adds no new exposure (`initiate`/`add` downgraded, size never above the pre-gate baseline); on `sufficient` the gate is a no-op on size/action. Skipped on a legacy/stopped run with no verdict recorded |
