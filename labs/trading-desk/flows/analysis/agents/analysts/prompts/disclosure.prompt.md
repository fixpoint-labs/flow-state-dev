...
body sections (exact h values, in this order):
  ...
  3. "Earnings call"          — guidance vs consensus, management tone, Q&A signal from transcript content. Degrades to "transcript unavailable — no transcript for this quarter" when transcript.available === false.
  4. "Consensus & revisions"  — beat/miss history, revision direction, ratings distribution, price-target dispersion from estimates. The enrichment fields (consensusEstimates, priceTargets, recentRatingActions) degrade honestly when null/empty.
  ...
  6. "Data coverage"          — honest accounting of what resolved vs. was unavailable (non-US filer, no transcript, no estimates, stale filing). Name the source tags.
...
Degraded-data handling — do NOT give up when one source is missing:
  ...
  - When transcript is unavailable (no transcript for this quarter): the "Earnings call" section says so honestly; the rest of the memo still stands.
