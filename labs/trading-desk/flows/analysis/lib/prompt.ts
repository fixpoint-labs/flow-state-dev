 ...
 * (`flows/analysis`), which is anchored at the package directory resolved
...
const FLOW_ROOT = path.join(APP_ROOT, "flows/analysis");
const PARTIALS_DIR = path.join(FLOW_ROOT, "prompts", "_partials");

/**
 * Load a trading-desk `.md` prompt. The argument is relative to the
 * `flows/analysis` directory (e.g. `"phase-5/prompts/portfolio-manager.prompt.md"`);
