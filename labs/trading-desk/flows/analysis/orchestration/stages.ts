 ...
 *   .parallel({ lensId: lensStep })  // independent lenses, fanned out
 *   .tap(computeAndStoreConvergence)  // DETERMINISTIC convergence → resource
 *
 * INDEPENDENCE is the honesty guarantee (FIX-655): each lens reads ONLY the
 * shared post-Phase-2 bundle (investmentThesis + phase1Memos + valuationSpine)
 * via `defineLensGenerator`, NEVER another lens's memo, so they are blind to
 * each other. Running them in `.parallel` is therefore safe — the fan-out is a
 * pure performance choice, not a semantic one. This is NOT a staged debate; it
 * is N independent reads of the same evidence.
 *
 * The lenses previously ran as a sequential chain to dodge a runtime bug where
 * parallel branches' distinct-key collection writes clobbered each other in the
 * continuation cache, so a convergence tap after the fan-out read a stale view
 * (3 of 4 lens memos still `pending`). FIX-744 made distinct-key collection
 * writes cache-consistent, so the convergence tap now sees all N committed memos
 ...
 */
/** One independent lens sub-sequencer per pack entry, fanned out via `.parallel`
 *  (each lens is blind to the others — see the file header). Each lens is placed
 *  via the shared `defineMemoStep` apparatus: its generator is the body, the
 *  per-lens commit is `commitLensVerdict`, and the keyed memo lifecycle
 *  (`markWriting → … → rescue(markError)`) is resolved from the registry. */
const lensSteps = Object.fromEntries(
  LENS_PACK.map((lens) => {
    const lensId = lens.id as LensId;
    return [
      lensId,
      defineMemoStep(defineLensGenerator(lens), {
        key: lensId,
...
export const lensStage = sequencer({
