...
/**
 * Query options for listing the child sessions under one conversation.
 *
...
export function createSessionClient(options: CreateSessionClientOptions = {}): SessionClient {
  ...
  const listChildSessions = async (
    ...
    return payload.children.filter((child) => {
      if (child.parentSessionId === parentId) return true;
      warnParentMismatchOnce(
        `${parentId}:${child.id}`,
        `listChildSessions("${parentId}") dropped row "${child.id}" whose ` +
          `parentSessionId is "${child.parentSessionId}", not the requested parent.`
      );
      return false;
    });
  };
