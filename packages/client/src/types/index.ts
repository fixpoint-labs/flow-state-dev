...
/**
 * How a child session's runs ended, or that they have not ended. `"active"`
 * asserts only *not finished* — never gloss it as "running" or "live".
 *
 * See `docs/architecture/server-and-client.md` § Background work (child sessions)
 ...
 *
 * A child session outlives the turn that started it, so its history is read
 * with {@link SessionClient.listSessionRequests} — the same call a
 * conversation's own turns are read with.
 *
...
export type ChildSessionSummary = {
  /** Bare child session id — the address for reading this child session's requests. */
  id: string;
...
export type SessionRequestSummary = {
  ...
  state?: Record<string, unknown>;
  /**
   * Item log. Omitted by the `GET /sessions/:id/requests` list endpoint by
   * default — it returns summaries only. Pass `includeItems: true`
   * (`include_items=true`) to populate it, or read a single request's items
