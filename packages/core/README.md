...
- `sendMessage` refuses `no-send-operation` where the host wired no send operation, the sibling of `no-start-operation` and reachable in the same residual case. Its other refusals are about the send itself: `unknown-recipient` for a session that is absent or outside the caller's owner, tenant or flow, `org-mismatch` where sender and recipient are bound to different organizations (including one bound and the other not), `no-relay-door` for a kind the recipient's flow declares nothing for, `recipient-not-addressable` where one end is a background job or one end's record does not say what kind of session it is, `durable-action` where the fallback lands on an action declared `durable`, `recipient-busy` for the recipient's own `reject` policy, `external-dispatcher` on a queue-backed deployment, and `mode-not-available` for `waitForResponse`, which has not shipped.
...
`sendMessage` is the only operation that takes a session id. It is checked against the running request's owner, tenant, org binding and flow; a recipient outside any of those is refused `unknown-recipient`.
...
`relaySendTool()` is the model-facing surface. An author adds it to **one** generator's `tools`, so the capability is scoped by declaration. It makes the same `sendMessage` call and returns the same result, so a model reads a named refusal instead of a thrown tool error. The factory sets its own `timeoutMs: 0` and `retry: { maxAttempts: 1 }`. A flow-wide `tools.defaults.timeoutMs` does not apply to it; your other tools keep the flow defaults.

`BlockConfig.retry` merges over `tools.defaults.retry` (block wins field by field); `BlockConfig.timeoutMs` overrides `tools.defaults.timeoutMs` for that tool alone. Both apply only on the tool path, and `0` for `timeoutMs` means "this tool's own clock governs".
