...
- `startDetached` refuses `no-workstream-core` for a flow that declares no workstream core, and `no-start-operation` where the host wired no start operation. The shipped HTTP router wires one, so a flow that declares detached work can start it there. A `worker-only` runtime builds no router and so wires none — that deployment must supply its own start operation. Input naming a task-board worker the flow does not route refuses `board-not-routable` without starting anything, so the caller still holds its claim and can settle the row itself.
