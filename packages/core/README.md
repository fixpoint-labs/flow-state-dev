...
- `defineResource(config)` — Portable resource definition (also usable for block-level resource declarations via `sessionResources`, `userResources`, `orgResources`)
  ...
  - `prefetchMode?: 'eager' | 'lazy'` (default `'eager'`) — `'lazy'` defers the load until the declaring block dispatches. Reads stay synchronous (`ref.state`). Declaring `'lazy'` on a flow-level single resource throws at build time (no per-block load trigger).
- `defineResourceNamespace(config)` — Dynamic resource collection with pattern-based keys (`files/*`, `files/**`, `[topic]/observations`), optional `maxInstances`/`eviction`, and lifecycle hooks
  - `prefetchMode?: 'eager' | 'lazy'` (default `'eager'`) — eager loads the whole prefix up front with synchronous `get`/`list`/`count`; `'lazy'` reads per access so `get`/`getOptional`/`list`/`count` return Promises. `'lazy'` requires `eviction: 'none'` (a partial cache can't drive eviction) and throws at build time otherwise.
