...
- `sendMessage` refuses `no-send-operation` where the host wired no send operation — the sibling of `no-start-operation`, and reachable in the same residual case. Its other refusals are about the send itself: `unknown-recipient` for a session that is absent or outside the caller's owner / tenant / org / flow, `no-relay-door` for a kind the recipient's flow declares nothing for, `recipient-not-addressable` for a pair that may not use the door that exists, `recipient-busy` for the recipient's own `reject` policy, `external-dispatcher` on a queue-backed deployment, and `mode-not-available` for `waitForResponse`, which has not shipped.

Nothing on this interface names a store, a flow, a session record or a task row, and no operation takes an identity. Values read from another session cross as `unknown`, to be parsed with your own schema.

**A session id is a parameter of exactly one verb, and that is the contract rather than an exception to it.** Most verbs authorize **by descent**: a caller supplies a routing seed and the runtime derives the child session key from it plus the running request's principal and parent session, so another parent's child is not a value a capability can produce. `sendMessage` authorizes **by peerage** instead — it takes a recipient session id because reaching a session you were *given* the address of is the whole of what it does, and the locator's authority comes from an owner / tenant / org / flow-kind check at the verb. Identity is server-derived either way.
...
### Messaging: `relay` and `relaySendTool()`

A flow declares which messages it accepts on a `relay` group, a sibling of `webhooks` and the same shape — a binding carries its `ActionCore` inline plus an `input` mapper, so its handler never enters `flow.actions`:
...
`defineRelayBinding<TPayload>()` narrows the mapper's `message.payload` without annotating it at the call site. A binding declaring `durable: true` is refused at construction: a message has no caller-facing entry, so a suspended one could never be resumed.

`relaySendTool()` is the model-facing surface — an author adds it to **one** generator's `tools`, which is what scopes the capability by declaration rather than by hope. It is a thin surface over the same verb and returns the same result, so a model reads a named refusal instead of a thrown tool error. It also declares its own `timeoutMs: 0` and `retry: { maxAttempts: 1 }`, because the generic tool timeout rejects on its timer without cancelling the underlying promise — a short `tools.defaults.timeoutMs` would otherwise let the retry start a second live send and deliver the message twice.

That per-tool composition is now general: `BlockConfig.retry` merges over `tools.defaults.retry` (block wins field by field, matching `executeBlock`) and `BlockConfig.timeoutMs` overrides `tools.defaults.timeoutMs` for that tool alone. Both are tool-only knobs on a general block config, the convention `cacheable` already set; `0` for `timeoutMs` means "this tool's own clock governs".
