...
**The harness contract.** A *harness* is a coding agent driven as a block — you hand it a prompt, it runs its own agentic loop, and it hands back a handle describing the run. `HarnessRunInput` is what it is handed: the prompt, and nothing else. `HarnessRunHandle` is what it returns: which harness and door produced it (`source`, by convention `<package>/<door>`), the run's status, session id, URL and dispatch time, plus how it ended (`outcome` — finished, stopped at a limit, or failed), the final assistant message, token usage, and what it cost with whether that number was reported or estimated. Any harness package declares these shapes as its block's input and output schemas — `harnessRunInputSchema` and `harnessRunHandleSchema` from the package root — and adds its own fields beside them; `HarnessBlock` names a block that conforms. Everything that decides *where* a run writes or *which* session it continues is host configuration, not input, so a model calling the block as a tool cannot set it. `HarnessResolver` and `HarnessSessionHook` name the two signatures a host supplies that configuration through — `cwd` and `resume` are resolvers handed the block's context and never the prompt; `onSession` is called by the harness the moment it names its session, early enough that a run which is later cancelled is still resumable. Two harnesses ship on these shapes: [`@flow-state-dev/claude-code`](../claude-code) (`claude-code/sdk`) and [`@flow-state-dev/codex`](../codex) (`codex/sdk`). [`@flow-state-dev/harness-manager`](../harness-manager) drives either through the same three feeds, which is what the shared spelling is for.
