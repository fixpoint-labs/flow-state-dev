...
- `defineResource(config)` — Portable resource definition. Requires `scope: "session" | "user" | "org"`. Register on a flow's or block's `resources` map.
  ...
  - `stateSchema` may normalize (fill a `.default()`, strip an undeclared key, map a retired enum value) but has to **settle**: parsing its own output must yield that same output. A `.transform()` that returns something new on each pass would move the stored value on every write, so a write that isn't a fixed point of the schema is refused with `ValidationError` rather than stored. See [the schema has to settle](https://flow-state.dev/docs/state/mutation-model#the-schema-has-to-settle).
  ...
  - `sharedToLineage?: boolean` (default `false`, `scope: "session"` only) — give the resource ONE identity across a session lineage, so a session, every session dispatched under it, and their own children resolve the same resource through the ordinary resource API. Session **state** is never shared, and sharing does not serialize writes — two children writing one shared resource is ordinary same-resource contention fenced by `expectedVersion`. `true` at `user`/`org` scope throws at build time (those scopes already span every session the principal touches). Also accepted by `defineResourceCollection`, applying to every instance. Named `sharedToWorkstream` before 0.5.
...
- `defineResourceCollection(config)` — Dynamic resource collection with pattern-based keys (`files/*`, `files/**`, `[topic]/observations`), required `scope: "session" | "user" | "org"`, optional `maxInstances`/`eviction`, lifecycle hooks, and `reactTo` (same `{ created?, stateUpdated?, deleted?, contentUpdated? }` shape as `defineResource`; supersedes the `onInstance*` callbacks for the block case)
  ...
  - **`create(key, initial, { replace: true })`** — overwrites an existing instance instead of throwing. `setState` semantics; Zod `.default(null)` fills nullables on both the create and replace branches. `maxInstances` only checked when adding a new instance. Use for setup/reset paths. A `writable: false` collection refuses that overwrite the same way `setState` does; a missing key creates.
  ...
  - "If-exists / if-missing" summary: `create` throws / `create({ replace })` replaces / `getOrCreate` returns as-is / `upsert` patches — all four create on missing; replace-on-existing and `delete` refuse when `writable: false`.
- `defineFacetedCollection(config)` — Defines a resource collection whose bodies are classified once on write by the evaluator you pass (`evaluator`, with a fixed questions object), storing its answers as `facets` beside your `stateSchema` fields. `name` is the accessor it registers under. Returns `{ collection, search, reindex, resources }`: `search` takes one optional value per choice question plus `minConfidence` and returns `{ keys }` with no model call; `reindex` takes `{ force? }` and classifies rows without facets (every row with `force`), returning `{ reindexed }`; `resources` is the collection under `name` plus the evaluator's declared resources, for `defineFlow({ resources })`. Refuses, when built, client body creates/updates, a `reactTo.contentUpdated` binding, parameterized key patterns, a `stateSchema` that declares `facets` or `indexedAs`, and an evaluator with computed questions, a `flowConfigSchema` or a lazy single resource. See [Find by facets](https://flow-state.dev/docs/resources/searching#find-by-facets).
...
`defineResourceCollection` accepts `ownerPrivate?: { param: string }`, which serves each row only to the user its owner segment names. Build that segment with `ownerSegment(userId)` (`~` plus the escaped user id) and pass it as `param`'s value in the key. Key segments beginning `~` are reserved for owner-private collections in every app: no other collection lists, reads or writes them. The pattern must name `param` exactly once and must not use `**`, and the collection may not enable a browser read; each is refused at definition. Once a flow declaring one is registered, Engine refuses any flow whose collection in the same scope could reach its keys. Engine also refuses, in every app, a flow declaring a single resource whose key (its `ref`, else its accessor name) has a segment beginning `~`. See [Owner-private collections](https://flow-state.dev/docs/resources/collections#owner-private-collections).
...
**LLM navigation tools.** `resourceSearchTools()` returns the Glob/Grep/Search trio over resources — `globResources` (path glob), `grepResourceContent` (regex/substring over content), and `searchResources` (lexical, term-frequency ranked). All three span static resources and collection instances and return each match's scope-qualified uri — the handle `readResourceContentTool` accepts. All three gate on `llmReadable` (including collection instances whose collection opts in); a collection that did not opt in is skipped before it is listed. Lexical only, not semantic. See [Searching resources](https://flow-state.dev/docs/resources/searching).

**Resource CRUD tools.** `resourceTools()` returns `createResource` / `readResource` / `updateResource` / `deleteResource`, each acting on a path the caller already names. It no longer returns `listResources`: that tool enumerated every collection's stored state without applying the `llmReadable` gate. To let an agent find out what it can work with, register `resourcesManifestSource()` and give it the discovery tool below.

**Agent discovery.** `discoveryTools(createManifestRegistry([...sources]))` returns `discover({ domain?, detail? })` — one on-demand tool that answers what is in scope for this agent right now. A source projects one domain's existing reader into manifest entries (`{ id, kind, purpose, contract? }`); nothing is stored and nothing is copied. The registry is the fence: the tool can only answer for domains it was built with, so a model-supplied `domain` has no wider list to reach past. `resourcesManifestSource()` ships here; seats, channels and skills are registered by their own packages.
