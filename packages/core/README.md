...
- `defineFacetedCollection(config)` — Defines a resource collection whose bodies are classified once on write by the evaluator you pass (`evaluator`, with a fixed questions object; it receives the body string), storing its answers as `facets` beside your `stateSchema` fields. `name` is the accessor it registers under. Returns `{ collection, search, reindex, resources }`: `search` takes one optional value per choice question plus `minConfidence` and returns `{ keys }` with no model call; `reindex` takes `{ force? }`, classifies rows without facets (every row with `force`) and returns `{ reindexed, failed }`, retrying nothing; `resources` is the collection under `name` plus the evaluator's declared resources, for `defineFlow({ resources })`. See [Find by facets](https://flow-state.dev/docs/resources/searching#find-by-facets).
