 ...
 * read. Only the object carrying {@link HIRED_ROSTER_PRIVATE_BRAND}, which
 * `defineHiredRosterPrivateCollection` sets, reads or writes a user-owned row.
 * Engine's hire-plane module enforces that, and refuses other declarations of
 * this pattern once a registry holds the branded one.
 */
 ...
 *
 * Non-enumerable, so copying the config with a spread drops it. Engine
 * recognizes the private roster writer by this brand alone.
 */
