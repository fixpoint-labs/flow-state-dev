/**
 * The request host — the declared seam through which a capability reaches
 ...
 * and `core` must not depend on `engine`. Some helpers need to settle a durable
 * row owned by another session, or ask whether work they dispatched is still
 * running — both of which live on the engine's context type. Before this seam
 * the only route was a cast, and a cast holds nothing: nothing warns when the
 * shape it asserts stops being true.
 *
 ...
 *
 * **Identity is never a parameter, and neither is a session id.** Every verb
 * closes over the running request's server-derived identity (tenant, user, org,
 * session, flow) and the claim it was dispatched under. A caller supplies the
 * *target* of an operation and never the *authority* for it.
 *
 * **Starting other work is not a verb here.** A request that wants a message
 * delivered — into a child session it derives, or into a session that already
 * exists — declares a `dispatcher()` block (`blocks/dispatcher.ts`), so the set
 * of blocks that dispatch is visible at definition time. The runtime's dispatch
 * operation reaches the block context under a symbol key, not as a member of
 * this bundle; see `types/dispatch.ts`.
 *
 * The seam is **closed at three verbs**. Adding a fourth is a decision someone
...
export interface RequestHost {
   ...
   *
   * The board coordinate is server-stamped into the request at hand-off and
   * closed over. This is not a cross-session resource browser: one coordinate,
   * one row, both server-derived. The value is untyped — parse it with your own
   * schema.
   */
   ...
   * child request at hand-off and closed over, exactly as the parent session and
   * the board coordinate are. Every field of a ticket is readable off the row
