/**
 * The capability: host opt-in for running Cursor, and the shape a generator
 ...
 * keeps conversation state; this one keeps none — the agent to continue is the
...
import { createCursorAgentCapability } from "../src/capability";
import { INTERNAL_SDK_VERSION_READER, type CursorAgentOptions } from "../src/agent";
...
describe("createCursorAgentCapability", () => {
  it("is a named capability whose default-on tools preset carries the block", () => {
    const cap = createCursorAgentCapability(GATE_OFF) as unknown as {
      name: string;
      __presetDefs: { tools: { tools: Array<{ name: string }> }; default: string[] };
    };

    expect(cap.name).toBe("cursor-agent");
    expect(cap.__presetDefs.default).toContain("tools");
    expect(cap.__presetDefs.tools.tools).toHaveLength(1);
    expect(cap.__presetDefs.tools.tools[0].name).toBe("cursor-agent");
  });

  it("declares NO session state — this package keeps none", () => {
    const cap = createCursorAgentCapability(GATE_OFF) as unknown as {
  ...
  it("forwards its options to the block, so the version gate still refuses here", () => {
    expect(() =>
      createCursorAgentCapability({
        [INTERNAL_SDK_VERSION_READER]: () => ({ kind: "version", version: "1.0.30" }),
      } as CursorAgentOptions),
    ).toThrow();
  });
  ...
  it("promotes a resource-declaring `uses` capability's resources onto itself", () => {
    ...
    const cap = createCursorAgentCapability({ ...GATE_OFF, uses: [withResources] }) as unknown as {
