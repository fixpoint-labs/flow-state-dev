/**
 * Lists the child sessions hanging off one session (FIX-1071).
 *
 * Wraps `sessionClient.listChildSessions`, the same client-method idiom the
 * other listing hooks use — transport lives in the client, never here.
 *
 ...
 * `useSession`'s `children` does in `@flow-state-dev/react`, so the DevTool
...
import type { ChildSessionSummary, SessionClient } from "@flow-state-dev/client";
 ...
 * correctly: the Children tab did, the Tasks tab held one and reported a
...
/**
 * Read the child-session axis for one session.
 *
...
export function useChildSessions(sessionId: string | null): UseChildSessionsResult {
  ...
  return {
    children: holdsCurrent ? children : EMPTY_ROWS,
