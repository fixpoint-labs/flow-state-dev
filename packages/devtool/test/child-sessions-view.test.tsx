/**
 * The Children panel (FIX-1071).
 *
 * What is pinned here is that a row renders from the *session* record alone. A
 * child session can arrive with no topic, no coordinate, no status and no task
...
describe("ChildSessionsView", () => {
  ...
  it("renders a row for a child session with no task, no labels and no status", () => {
    // The case the whole panel exists for: a child session reachable from no
    // board. Every derived column is absent, and the row still has to be
    // clickable.
    const { onOpen } = renderView({
      children: [childSession({ id: "dsx_bare" })],
    });
  ...
  it("decodes the type and target out of the coordinate label", () => {
    // The raw label is `task:implement`. Splitting it into two cells beside a
    // hashed id tells a developer more than the string does on its own.
    renderView({
      children: [
        childSession({
          id: "dsx_1",
  ...
  it("shows an unrecognised coordinate raw instead of dropping it", () => {
    // Some other writer's label — one with no `:` in it — is still the most
    // specific thing known about that dispatch.
    renderView({
      children: [childSession({ id: "dsx_1", topic: "t", coordinate: "opaque" })],
    });
    expect(screen.getByText("opaque")).toBeInTheDocument();
  });

  it("names the board tasks a child session is running", () => {
    renderView({
      children: [
        childSession({ id: "dsx_1", topic: "task-a", coordinate: "task:implement" }),
      ],
      items: [
        {
          id: "item-1",
  ...
  it("surfaces a failed re-read without blanking the rows it already has", () => {
    ...
    renderView({
      children: [childSession({ id: "dsx_1", topic: "FIX-1" })],
      error: "network down",
    });
  ...
  it("says the list is unknown after a failed read, not that it is empty", () => {
    ...
    renderView({ children: [], error: "network down", isLoading: false });
  ...
  it("opens a child session from the keyboard, not only by pointer", async () => {
