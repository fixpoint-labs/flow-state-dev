...
A dispatched request's record carries the dispatch type as its `source` — `"internal"` for a `dispatcher()`, `"task"` for a task-board hand-off — and a `metadata.dispatch` bag: `type`, `target`, `from: { block, sessionId }` (the dispatching block and session), `key` when a child was derived, and `taskId` on a task hand-off. Read the bag only when `source` is one of those two values: `metadata` is caller-writable on an ordinary request, `source` is not. `key` and `taskId` are optional, so guard with `== null`.

`task` and `internal` requests have no caller-facing entry, so the public `retry`, `continue`, and `resume` routes refuse them with the same not-found shape a missing record gets. A child session itself is reachable from outside in the ordinary way — by calling one of its flow's actions, which is a `public` dispatch.
