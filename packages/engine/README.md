...
results to the caller; reached without one, they serve the flows that
configure no resolver of their own to anyone. A row owned by a flow instance
with its own resolver is listed only when that resolver accepts the caller as
the row's owner (and a pinned instance's pin admits them).
