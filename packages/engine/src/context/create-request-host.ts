...
export function createRequestHost(inputs: RequestHostInputs): RequestHostBuild {
  ...
  /**
   * `{ from: true }` is delivery into the stamped sender — the same path as
   * `id`, with the id taken from `readDispatchStamp`, not from the spec.
   * The author cannot name a reply-to; a missing or untrusted stamp is
   * `no-sender`. Wrong principal / other flow / incarnation drop then share
   * the `id` guards.
   */
  const resolveSessionTarget = async (spec: DispatchSpec): Promise<ResolvedSession> => {
  ...
  const seam: DispatchSeam = async (spec: DispatchSpec): Promise<DispatchOutcome> => {
    ...
    const session = await resolveSessionTarget(spec);
