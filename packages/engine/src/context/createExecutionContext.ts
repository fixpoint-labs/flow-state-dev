...
> {
  ...
  //    `dispatch` created children outside the parent's org, contrary to
  ...
  const dispatchedActionBlock = resolveEntry(
    flow,
    options.actionName,
    options.source,
    options.metadata
  ...
  const createContext = (
    ...
    const context: ExecutionContext<TRequestState, TSessionState, TUserState, TOrgState> = {
      ...
      // The bundle and the dispatch seam, the latter under its symbol key so it is
      // reachable from `dispatcher()` blocks and from nothing a handler body
      // names (see `types/dispatch.ts`).
