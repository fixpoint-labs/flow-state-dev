...
> = {
   ...
   * `_requestSideChainSignal` and substituted for `ctx.signal` inside
   * `.sideChain()` task trees. Fires only on explicit user-requested abort, not
   * on transport-level teardown. Absent for non-server callers.
   */
  sideChainSignal?: AbortSignal;
...
export type RequestHostConstructionInputs = {
  ...
  /**
   * Resolves another registered flow instance by its exact address (its id),
   * for a cross-instance dispatch address. Wired from the host's flow
   * registry. Absent → every cross-flow address refuses `flow-not-found`, the
   * same fail-closed shape `dispatchOperation` has.
   */
