/**
 * Resolve the typed entry a dispatched request runs.
 *
 ...
 * none (or the source has no type). Callers decide whether that is a hard
 * error (initial dispatch) or a tolerable absence (optional prefetch /
 * token-budget reads).
 */
export function resolveEntry<TEntry>(
  ...
  const type = dispatchTypeOf(source);
  if (type === undefined) return undefined;
  return resolveTypedEntry(flow, type, actionName, metadata as EntryCoordinate | undefined);
}
