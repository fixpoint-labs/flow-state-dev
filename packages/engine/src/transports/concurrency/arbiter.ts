...
export interface ConcurrencyFlowView {
  ...
  internal?: Record<string, EntryPolicyView>;
  tasks?: Record<string, EntryPolicyView>;
...
export function createConcurrencyArbiter(): ConcurrencyArbiter {
  ...
  return {
    resolve(flow, actionName, view): ResolvedDecision {
      ...
      // whose name collides with a public action never inherits that action's
      // `queue` / `reject` — each reads only its own map.
      const entryConfig = resolveEntry(flow, actionName, view.source, view.metadata)?.concurrency;
