/**
 * Child-session derivation and adoption for the dispatch seam (FIX-999).
 *
 * The seam's central safety rule is that a caller supplies the *target* of an
 * operation and never the *authority* for it. A `key`-targeted dispatch hands
 * over a plain string that names the unit of work; the seam derives the child
 * session id from that key together with the running request's server-derived
 * identity. Two collisions must therefore be **inexpressible** rather than
 * rejected:
 *
...
} from "../../src/context/child-session";
...
describe("child session derivation", () => {
  it("is deterministic for the same identity and key — that is what makes adoption possible", () => {
    const a = deriveChildSessionId(base, "review");
    const b = deriveChildSessionId(base, "review");
    expect(a).toBe(b);
  });

  it("gives two principals different children for an identical key", () => {
    const alice = deriveChildSessionId(base, "review");
    const bob = deriveChildSessionId({ ...base, userId: "u_bob" }, "review");
  ...
  it("gives ONE principal's two parent sessions different children for an identical key", () => {
    ...
    const fromA = deriveChildSessionId(base, "review");
    const fromB = deriveChildSessionId({ ...base, parentSessionId: "s_parent_b" }, "review");
    expect(fromA).not.toBe(fromB);
  });

  it("separates tenants", () => {
    const acme = deriveChildSessionId(base, "review");
    const other = deriveChildSessionId({ ...base, tenantId: "t_other" }, "review");
    expect(acme).not.toBe(other);
  });

  it("distinguishes keys", () => {
    const t1 = deriveChildSessionId(base, "review");
    const t2 = deriveChildSessionId(base, "triage");
    const t3 = deriveChildSessionId(base, "review|second");
...
describe("adoption identity validation", () => {
  const key = "review";
  const childId = deriveChildSessionId(base, key);
