/**
 * Which milestone a dispatch operation waits for (FIX-982).
 *
 * `createDispatchOperation` is the seam a block reaches when it dispatches a
 * message, and its caller does something irreversible on the result: a task
 * board releases the claim on the row it just handed over. Nobody holds the
 ...
 * startup — the thing dispatch exists to remove.
 *
...
describe("a dispatch operation waits for the dispatch to be accepted", () => {
  it("waits for acceptance before reporting the child started", async () => {
    let settleAccepted: () => void = () => {};
    const accepted = new Promise<void>((resolve) => {
      settleAccepted = resolve;
    });
    const start = createDispatchOperation({ host: hostReturning({ accepted }) });
  ...
  it("reports a failed registration as not-started instead of reporting success", async () => {
    ...
    const start = createDispatchOperation({
  ...
  it("does not wait on a dispatcher that reports no acceptance", async () => {
    ...
    const start = createDispatchOperation({ host: hostReturning({}) });
  ...
  it("puts the spec's source and target onto the envelope, unforged", async () => {
    let seen: { source?: string; action?: string } = {};
    const start = createDispatchOperation({
      host: {
        dispatch: (envelope: { source?: string; action?: string }) => {
          seen = envelope;
          return {
            requestId: "req_child",
            finished: new Promise<never>(() => {})
    ...
    await start(SPEC);
    expect(seen).toMatchObject({ source: "internal", action: "core" });
  });
});

describe("not-started is reserved for a throw that happened before any child existed", () => {
  /**
   * A flow whose `internal` entry runs a runner that records the fact it ran,
   * so "a child existed" is an observation rather than an argument.
   */
  function flowWithRunner(kind: string, ran: string[]): FlowInstance {
    ...
    return defineFlow({
      kind,
      actions: {},
      internal: { core: { block: runner } }
    } as never)({ id: kind }) as unknown as FlowInstance;
  }

  const dispatchInput = { note: "detached-payload" };
