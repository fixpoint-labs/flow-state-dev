/**
 * Every topology a `FlowState` owns can dispatch detached work (FIX-1077).
 *
 ...
 *    real adapter does.
 *
 * `worker-only` dispatches detached work too, rather than refusing it — see the
...
import { z } from "zod";
import { DispatchRefusedError, defineFlow, dispatcher, handler, sequencer } from "@flow-state-dev/core";
import type { DispatchHandle, ModelResolver } from "@flow-state-dev/core/types";
...
type Observed = {
  start?: DispatchHandle;
  /** The message of a caught `DispatchRefusedError`, when the dispatch was refused. */
  error?: string;
  /** The refusal code off a caught `DispatchRefusedError`. */
  refused?: string;
  /** One entry per child request that executed the internal entry. */
...
/**
 * A flow whose action dispatches, plus the internal entry the child enters.
 *
 * The launcher is a sequencer: a `dispatcher()` step puts the message through
 * the seam, a step immediately records its output handle into `observed.start`
 * — the same instant a plain `await` would have set it — and then an optional
 * step holds the request open, so it keeps holding its concurrency key the way
 * the old handler-shaped launcher did by staying inside its own `await` after
 * that assignment. A refusal surfaces as a thrown `DispatchRefusedError` — the
 * `.rescue()` branch catches it and records the refusal instead of letting it
 * fail the request, mirroring the try/catch the old launcher used.
 */
function detachingFlow(
  ...
  const spawn = dispatcher({
    ...
    payload: () => ({ note: "detached-payload" })
  });

  // Captures the dispatch's own output handle THE INSTANT the dispatch
  // resolves — before any hold delay — so `observed.start` becomes
  // observable at the same moment it did when this was a plain `await` in a
  // handler body, whatever `launcherDelayMs` is set to below.
  const recordStart = handler({
    name: "record-start",
    execute: async (input: unknown) => {
      observed.start = input as DispatchHandle;
      return input;
    }
  });

  // Keeps the launching request running (and therefore still holding its
  // concurrency key) after `observed.start` is already recorded — mirroring
  // the old handler body, which set `observed.start` and only THEN awaited
  // the delay inside the same `try`.
  const hold = handler({
    name: "hold-launcher",
    execute: async (input: unknown) => {
  ...
  const captureRefusal = handler({
    name: "capture-dispatch-refusal",
    execute: async (error: unknown) => {
      const refusedError = error instanceof DispatchRefusedError ? error : undefined;
      observed.error = error instanceof Error ? error.message : String(error);
      observed.refused = refusedError?.refused;
      return {};
    }
  });

  const launcher = sequencer({ name: "launch" })
    .step(spawn)
    .step(recordStart)
    .step(hold)
    .rescue([{ block: captureRefusal }]);
...
describe("detached start on a runtime-only init (FIX-1077)", () => {
  it("starts a real child request in this process — no router involved", async () => {
    ...
    // Before this wiring the dispatch was refused `no-dispatch-operation`.
    expect(observed.start).toMatchObject({ adopted: false });
    ...
    // dispatch is discoverable rather than merely executed.
  ...
  it("dispatches a detached child from inside a COLOCATED queue worker", async () => {
    const observed: Observed = { children: [] };
    ...
    const worker = {
      ...
      // could never show a detached child launching, which is the point here.
    ...
    // Before this, the worker met `no-dispatch-operation`: the only installer
    // wrote to the router's fork of `requestHost`, which this object is not.
    expect(observed.error).toBeUndefined();
    expect(observed.start).toBeDefined();

    // And the observable that matters — a detached child actually launched and
    // ran, not merely that an operation was installed on a runtime.
    await until(() => observed.children.length === 1, "the detached child to run");
    expect(observed.children[0]!.sessionId).not.toBe("s_worker");

    await state.dispose();
  });

  it("dispatches a detached child from a worker-only process too", async () => {
    const observed: Observed = { children: [] };
    ...
    // queue-backed dispatch operation owned by the queue's own adapter is the
  ...
  it(
    ...
    async () => {
      ...
      const flow = detachingFlow("runtime-drain-wedged", observed, {
        // In-process, so it IS tracked — and never finishes. A detached child
  ...
  it("refuses to admit detached work once disposal has begun", async () => {
    ...
    // jobs is the shipped shape — dispatching afterwards. Before the gate this
    // registered a child the drain had already walked past: never waited for,
    // never cancelled, never reported, and writing while the stores closed
    // underneath it.
    await runLikeCli(runtime, flow, "s_late");

    // The seam refuses `dispatch-rejected` as a value, so the caller still owns
    // the work and could settle its own row — the dispatcher block turns that
    // into a thrown `DispatchRefusedError`, which the launcher's `.rescue()`
    // catches instead of letting it fail the request.
    expect(observed.refused).toBe("dispatch-rejected");
    expect(observed.error).toContain(
      "the runtime is shutting down and is no longer starting detached work"
    );
    expect(observed.children).toEqual([]);
    expect(observed.start).toBeUndefined();
  });
  ...
  it("enforces a reject policy ACROSS the router and detached hosts, not once each", async () => {
    ...
    // dispatch.
    expect(observed.children).toEqual([]);
    // The seam refuses synchronously before dispatching, as a returned value —
    // the caller still owns the work and could settle its own row rather than
    // leaving it to lease recovery. The dispatcher block turns that value into
    // a thrown `DispatchRefusedError`, which the launcher's `.rescue()` catches.
    expect(observed.refused).toBe("dispatch-rejected");
    expect(observed.start).toBeUndefined();
  });

  it("drains a detached child started over HTTP on the router-first path", async () => {
    const observed: Observed = { children: [] };
