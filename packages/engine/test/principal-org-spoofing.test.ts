...
/** Flow with no `authentication` — the host resolves no principal either. */
function buildUnauthenticatedFlow() {
  return defineFlow({
    kind: "org-unauth-flow",
    actions: {
      run: {
        inputSchema: z.object({}),
        block: handler({
...
describe("session creation with no principal resolver", () => {
  it("binds a new session to DEFAULT_ORG_ID, not the body's orgId", async () => {
    ...
    const res = await router.POST(
      new Request("http://localhost/api/flows/org-unauth-flow/sessions", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ userId: "u1", sessionId: "sess-unauth", orgId: "victim-org" })
      }),
      { params: { path: ["org-unauth-flow", "sessions"] } }
    );
    expect(res.status).toBe(201);

    const stored = await stores.session.get("sess-unauth");
