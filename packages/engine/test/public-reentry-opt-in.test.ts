 ...
 * - The framework's three deliberately-excluded sources are NOT openable. A
 *   deployment reading "add it to the allow-list" would reasonably try
 *   `webhook`, and `task`/`internal` are the dispatch-seam sources whose entire
 *   security argument is that they have no caller-facing entry at all. Re-opening
...
describe("isPublicReentryAllowed — host-declared sources", () => {
  ...
  it("refuses to re-open the framework's excluded sources, however they are declared", () => {
    ...
    // of them is a deployment's own transport, so none is a deployment's to
    // re-open.
