...
describe("upsert", () => {
  ...
  it("throws on schema-invalid update on the patch branch (symmetric with create)", async () => {
    // An invalid update on the patch branch must throw rather than silently
    // overwrite the resource with `{}`. Persist is the write parse, so the
    // message matches every other resource write.
    ...
    ).rejects.toThrow(/Resource "strict\/k" write failed stateSchema validation at "count"/);
