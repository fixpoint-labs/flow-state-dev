...
const SPAWN_FLOW = dispatchableFlow("fix1258-spawn");

/**
 * Dispatch a child from one fixed key, and return its session key.
 *
 * The key is constant on purpose: the child id is derived from it, so every
 * call targets the same child — which is what makes the delete/re-dispatch case
 * below reachable at all.
 */
function spawnChild(stores: StoreRegistry): Promise<string> {
  return dispatchChild(stores, SPAWN_FLOW, "review", {
    identity: {
      userId: "user_1",
      tenantId: undefined,
      orgId: undefined,
...
describe("FIX-1258: a write after a delete does not revive the resource", () => {
  ...
  it("still writes resources after a dispatched child is deleted and re-dispatched", async () => {
    ...
    // from its dispatch key, so the same key always lands on the same id — which is
    ...
    // here is only whether the re-dispatch cleared what would refuse it.
    const stores = createInMemoryStores();
    ...
    expect(second).toBe(first); // same key, same derived id — the whole point
  ...
  it("leaves a live child's resources alone when the key is merely adopted", async () => {
    // The control for the case above. A dispatch that ADOPTS an existing child
    ...
    // Re-dispatch with the record still in place, so this adopts rather than creates.
