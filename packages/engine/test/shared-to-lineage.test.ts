 ...
 * identity across a session lineage, so a session and the child sessions it
 * dispatches reach the same resource through the ordinary resource API.
 *
...
describe("FIX-1068: sharedToLineage resources across a session lineage", () => {
  ...
  it("resolves a nested child session to the lineage root, not its immediate parent", async () => {
    const stores = createInMemoryStores();
    await seedSession(stores, "s_root", undefined, lineageAddr("s_root"));
    await seedSession(stores, "s_child", "s_root", lineageAddr("s_root"));
