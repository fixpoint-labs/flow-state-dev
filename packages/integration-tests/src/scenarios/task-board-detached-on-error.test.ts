/**
 * A detached board honours its configured `onError` (FIX-982).
 *
 * `onError: "fail"` is a board-level contract: a worker failure fails the run
 * that was executing it. Inline, that is the drain's own request. Detached, the
 * run executing the worker is the Workstream's — so `"fail"` has to fail that.
 *
 * It did not. The runner hard-coded `"skip"` for its error recorder, so a
 * detached worker that threw recorded the row `errored` and its Workstream
 * request completed **successfully**. Everything that reads background work by
 * request status — `listWorkstreams`, the DevTool panel, any consumer polling a
 * run — saw a success for work that failed, and the board's configured policy
 * decided nothing.
 *
...
/** A board whose detached worker always throws, under the given failure policy. */
...
/** Drain the parent, then run the Workstream the hand-off produced. */
...
describe("a detached board's configured onError decides the Workstream's outcome", () => {
  it("fails the Workstream request when the board declares onError: fail", async () => {
  ...
  it("completes the Workstream request under the default onError: skip", async () => {
