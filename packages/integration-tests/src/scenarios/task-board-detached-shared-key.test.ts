/**
 * A detached child under a concurrency key its parent already holds (FIX-982).
 *
 * A detached dispatch takes the **flow-level** policy, and the default key is
 ...
 * fast-follow lands, this file fails and someone has to decide what a detached
...
function buildFlow(kind: string, concurrency: "allow" | "queue" | "reject") {
  ...
  return defineFlow({
    kind,
    // Keyed on `user`, so the parent and its detached child resolve the SAME
...
async function drain(kind: string, concurrency: "allow" | "queue" | "reject") {
  ...
  const parent = await runAction({
    ...
    runtimeConfig: {
      ...baseRuntimeConfig(),
      requestHost: {
        startOperation: async (spec: { input: unknown }) => {
...
describe("a detached child under a key its parent already holds", () => {
  it("hands the row over under the default allow policy", async () => {
    // The control. Without it, a guard that stranded every detached dispatch
  ...
  it("hands the row over under reject when the start operation is stubbed", async () => {
    // This harness supplies its own `startOperation`, so the arbiter that would
    // refuse a shared `reject` key is never reached — the policy is applied by
    // `host.dispatch`, which only the real start operation calls. What the
    // refusal does to the row is therefore pinned in `engine`, against
    // `createDetachedStartOperation` itself
    // (`context/detached-start-milestone.test.ts`). Kept here so the gap is
  ...
  it("does not block the launching request under queue", async () => {
    ...
    // waiting is the deadlock, and returning is the whole point of detachment.
  ...
  it("has no behaviour to pin for the reserved policies", () => {
    ...
    // and someone has to decide what a detached child does under them rather
