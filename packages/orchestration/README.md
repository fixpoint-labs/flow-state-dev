...
`status` the task was in when the write was refused. `immutable-assignee` means a
board that hands tasks off to child sessions draws from this ledger, where the
assignee is fixed at admission; `not-my-task`
...
terminal task, and on a durable collection that a hand-off board draws from it
declines every reassignment whatever the task's status (see "Handing tasks off to
child sessions" below).
...
`{ drain, collectionId, capability, backing, boardId, tasks, handedOff, hasIdlessInitialTasks, caps }`.
...
child's first step. A child that starts after the lease has lapsed refuses its claim
as stale, and the next drain picks the row up again. A deep queue backlog in front
of the child is where this shows up; the board's lease is not configurable today.

**Serverless without a queue adapter** is the last bound. A child dispatched
in-process runs inside the invocation that started it and is bounded by that
function's maximum duration. With a queue adapter it moves to a worker process and
is not.
