 ...
 *
 * The board is a **ledger**, not an execution engine — nothing drains it; the
 * executive runs delegated work by calling a worker tool. `addTask` records a
...
function buildTaskTools(resolve: TaskCollectionResolver) {
  ...
  const addTask = handler({
    name: "addTask",
    description:
      "Add a new task to your delegation board (a ledger). Returns the new task id. " +
      "Nothing runs the task automatically — call the assignee's worker tool to execute it.",
