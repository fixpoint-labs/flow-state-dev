...
export interface TaskBoardConfig<TInput = unknown, TOutput = unknown> {
   ...
   *
   * Renaming it is a **breaking re-key**: live child sessions keyed on the old
   * value are orphaned.
   */
  boardId?: string;
...
> {
   ...
   * Always present on a board that hands off — that is what
...
>(
  ...
  // A board that hands off is refused here, loudly and by name, never degraded
  // with a warning. Runs after the binding so the once-chosen backing is known.
  assertHandOffBoardSupported({
    name,
    boardId,
    backing,
  ...
  // The hand-off. A seat that hands off routes to a block that DISPATCHES the
  // row to the seat's task entry in a child session and returns, instead of to
  // the worker itself. The drain's shape is untouched (claim, route, record);
  // only what the route does changes, so an inline board composes exactly as
  // it did.
  //
  ...
  // The floor never hands off — it has no seat name to address.
  ...
  const drain = sequencer({
    ...
    .forEach(
      ...
      (workerId: number) => makeWorker(workerId),
      {
        ...
        // The per-worker wrapper is built at run time, so the walk `defineFlow`
  ...
  if (resolvedWorkers.handedOff.length > 0 && isDefinedTaskCollection(collectionConfig)) {
