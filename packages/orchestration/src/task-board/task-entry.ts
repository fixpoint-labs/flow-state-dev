...
import type { BlockContext, TaskEntry } from "@flow-state-dev/core/types";
import { markTaskEntry } from "@flow-state-dev/core/types";
import { z } from "zod";
 ...
 * request's identity. `attempt`, `createdAt` and `incarnationId` say *which*
 * claim this dispatch believes it is running; the gate decides whether that is
 * still true. They are verified, never trusted.
 */
...
export interface BuildTaskEntryOptions {
  /** Board name, used to name the assembled blocks. */
  name: string;
  /** The board's stable id — what a dispatch for this board carries, and the entry's brand. */
  boardId: string;
  /** The seat this entry serves — the task entry's name on the flow. */
  seat: string;
  /** The worker the seat declared. */
  worker: TaskWorker;
  /**
...
/**
 * Build the task entry for one handed-off seat.
 *
 * The entry's `inputSchema` is the dispatch envelope narrowed to THIS board —
 * `runAction` parses the input before the block is entered, so a dispatch
 * addressed to a board that has since been removed or renamed is refused
 * before the gate reads a row. Without that the gate could pass every arm on a
 * row that happens to match under a different board's ledger.
 */
export function buildTaskEntry(options: BuildTaskEntryOptions): TaskEntry {
  const { name, boardId, seat, worker, collection: collectionFactory, uses, onError } = options;
  ...
  const recordSuccess = createRecordSuccess({
    name: `${name}-${seat}-record-success`,
    collection: collectionFactory,
  });
  const recordError = createRecordError({
    name: `${name}-${seat}-record-error`,
    collection: collectionFactory,
    onError,
  });

  const block = sequencer({
    name: `${name}-${seat}-entry`,
    ...
    .tap(async (dispatch: TaskDispatchInput, ctx) =>
      ...
      withLeaseRenewalScope(async () => {
        ...
        // The row must still route to THIS seat. A hand-off board freezes a
        // task's assignee at admission precisely because it is the address the
        // task entry is reached by; a mismatch means that guard was bypassed,
        // and running would mix two workers' histories under one child.
        if (row.assignee !== seat) {
          throw new StaleTaskClaimError(
            dispatch.taskId,
            `this dispatch was addressed to seat "${seat}" but the row now routes to ` +
    ...
    .step(
      (worker as TaskWorker).connectInput<TaskDispatchInput>(
  ...
  return markTaskEntry(
    {
      block,
      inputSchema: boardScopedSchema(boardId),
    },
    { boardId }
  );
}
