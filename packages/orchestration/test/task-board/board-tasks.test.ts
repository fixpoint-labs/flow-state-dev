...
describe("the gated task entry — what the flow carries for a handed-off seat", () => {
  it("rebuilds the declared entry as the board's claim gate around its block", () => {
    ...
    const flow = defineFlow({
      ...
      tasks: { implement: { block: implement, concurrency: "queue" } },
    });

    const entry = flow.tasks?.implement;
  ...
  it("lets a seat hand off to an entry named differently from the seat", () => {
    ...
    const flow = defineFlow({
      kind: "board",
      actions: { run: { block: board.drain } },
      tasks: { implement: { block: worker("implement") } },
    });

    expect(board.handedOff[0]?.dispatch?.target).toBe("implement");
    expect(flow.tasks?.implement.block.name).toBe("issue-work-implement-gate");
  });
...
describe("defineFlow({ tasks }) — declaring the entries a board hands off to", () => {
  it("defines when the drain IS the action root", () => {
    ...
    expect(flow.tasks?.implement).toBeDefined();
  });
  ...
  it("defines when two boards hand off to two entries", () => {
    ...
    const flow = defineFlow({
      ...
      tasks: { implement: { block: worker("implement") }, review: { block: worker("review") } },
    });

    expect(Object.keys(flow.tasks ?? {}).sort()).toEqual(["implement", "review"]);
    expect(flow.tasks?.implement.block.name).toBe("issue-work-implement-gate");
    expect(flow.tasks?.review.block.name).toBe("review-work-review-gate");
  });
