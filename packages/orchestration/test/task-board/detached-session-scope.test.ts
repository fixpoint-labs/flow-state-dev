/**
 * FIX-1068: a detached board may be session-scoped once, and only once, its
 * ledger resolves to the lineage root.
 *
 * The refusal this narrows exists because a Workstream runs in its own session:
 * an ordinary session-scoped ledger resolves against the running session, so the
 * child addresses an empty collection, the start gate reads the missing row as a
 * stale claim, and the board reclaims and redispatches until the abandonment cap
 * errors it. `sharedToWorkstream` removes exactly that: parent and child resolve
 * one ledger, at the lineage address.
 *
...
const BOARD_ID = "session-scoped-detached";
...
function boardFor(collection: ReturnType<typeof defineTaskCollection>) {
  return taskBoard({
    ...
    collection,
    workers: {
      brief: { worker: worker as unknown as TaskWorker, dispatch: { mode: "detached" } }
    }
  });
}

describe("FIX-1068: session-scoped detached boards", () => {
  ...
  it("still refuses a session-scoped ledger that does not reach the Workstream", () => {
  ...
  it("files a row a Workstream can actually resolve", async () => {
    ...
    // reads the ledger from a second session — one stamped the way the
    // detached-start writer stamps a Workstream — and expects the parent's row.
    const shared = defineTaskCollection({
      id: "briefs",
      scope: "session",
      sharedToWorkstream: true,
    ...
    // THE ASSERTION THE REFUSAL IS ABOUT: the Workstream resolves the same
