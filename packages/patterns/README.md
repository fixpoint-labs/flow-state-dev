...
`awaiting_review` is fully supported: standard dispatchers skip it, the loop counts it as in-flight, and a resume from `awaiting_review` wakes the loop on the next idle poll. A board that sets `onReview: "exit"` counts it the other way. Parked tasks are excused, the drain returns, and the resumed task waits for the next drain instead. The mode requires a durable collection and the default `onIdle`; see [Waiting on a person](https://flow-state.dev/docs/orchestration/task-board#waiting-on-a-person-onreview).
