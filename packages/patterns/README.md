...
`awaiting_review` is fully supported: standard dispatchers skip it, and the loop counts it as in-flight, so a resume from `awaiting_review` wakes the loop on the next idle poll. A board that sets `onReview: "exit"` counts it the other way — parked tasks are excused, the drain returns, and the resumed task waits for the next drain instead. `reviewPolicy`, review UI, and the `tasks.review.requested` topic ship in Wave 2.
