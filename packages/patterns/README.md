...
The final `task-board-meta` item carries a `terminationReason: "all-completed" | "blocked-by-failures" | "retry-budget-exhausted" | "handed-off" | "parked-for-review"` field so callers can tell a clean drain from a dep-blocked exit, from one the board's retry budget stopped, from one whose remaining work was handed off to child sessions, and from one whose remaining work is waiting on a person — without inspecting `counts`.
