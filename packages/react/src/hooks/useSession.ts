...
export function useSession(
  ...
  const refreshChildSessions = useCallback(async () => {
    ...
    try {
      const rows = await sessionClient.listChildSessions(
        ...
        childrenLimit === undefined ? undefined : { limit: childrenLimit }
      );
  ...
  // Retire every child-session read in flight and clear the axis whenever what
  ...
  const sendAction = useCallback(
    async (
      ...
      // child session — gating it that way is how a just-launched job would
