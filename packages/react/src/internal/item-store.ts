...
// Canonical collapse from the zero-dependency contracts layer (value-import).
// Previously hand-mirrored here — with a lossy inline `logicalIdOfInstance`
// instead of the full `parseBlockInstanceId` — because react may only
// type-import core. Now we run the same implementation the server runs.
import { collapseToCanonicalLog } from "@flow-state-dev/contracts";
