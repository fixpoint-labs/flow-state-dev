...
import { createHandleCache } from "../createHandleCache";
import type { HandleProvider } from "@flow-state-dev/core";

type FakeHandle = { id: string; disposed?: boolean };

function createFakeProvider(): HandleProvider<FakeHandle> & {
  created: Map<string, FakeHandle>;
  released: string[];
} {
  const created = new Map<string, FakeHandle>();
  const released: string[] = [];
  return {
    created,
    released,
    async resolve(key: string) {
      const handle: FakeHandle = { id: key };
      created.set(key, handle);
      return handle;
    },
...
describe("createHandleCache", () => {
  ...
  it("resolves a handle on cache miss", async () => {
    const provider = createFakeProvider();
    const cache = createHandleCache({ provider, maxSize: 10, ttlMs: 60_000 });

    const handle = await cache.resolve("a");
  ...
  it("deduplicates concurrent resolve calls for the same key", async () => {
    let resolveCount = 0;
    const provider: HandleProvider<FakeHandle> = {
      async resolve(key: string) {
        resolveCount++;
        // Simulate async work
  ...
  it("uses default maxSize and ttlMs when not specified", async () => {
    const provider = createFakeProvider();
    const cache = createHandleCache({ provider });
