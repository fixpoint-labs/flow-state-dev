...
import { resolveProvider, ENV_VAR_MAP } from "../../src/fetch/resolver";

describe("fetch resolveProvider", () => {
  ...
  describe("explicit provider selection", () => {
    ...
    it("throws when firecrawl requested without key", () => {
      expect(() => resolveProvider({ provider: "firecrawl" })).toThrow(
        'Fetch provider "firecrawl" requested but no API key found'
      );
    });

    it("selects jina even without API key", () => {
      const result = resolveProvider({ provider: "jina" });

      expect(result.adapter.name).toBe("jina");
      expect(result.apiKey).toBeUndefined();
    });

    it("selects jina with API key", () => {
      const result = resolveProvider({
        provider: "jina",
        keys: { jina: "jina-key" },
      });
      ...
      expect(result.adapter.name).toBe("jina");
      expect(result.apiKey).toBe("jina-key");
    });
  ...
  describe("auto-selection priority", () => {
    it("selects firecrawl first when available", () => {
    ...
    it("selects jina when firecrawl not available", () => {
      process.env.JINA_API_KEY = "jina-key";
