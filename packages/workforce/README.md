...
| `hiredSeatOwnerPin(orgId, row)` / `hiredSeatOwnerPinFromRosterOwner({ orgId, userId? })` | The pin a hired seat registers under, built from a stored row or from the row's roster owner. An org-visible row gets `{ orgId }`, and a user-owned row gets `{ orgId, userId }`. `userId` is kept only when it is a non-empty string. A missing or empty organization id throws the same refusal `registerHiredSeat` throws. |
