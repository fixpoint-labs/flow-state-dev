...
resources stored at org scope resolve inside it. File-declared documents are all org-scoped, so a
channel opened without an org cannot read one. Adding `orgId` later does not move a channel that is
already open.
