...
| `defineHiredRosterPrivateCollection()` | The branded server-side writer for a user-owned row, at `workforce/roster/~<userId>/<seatId>`. No browser read. Registration admits that pattern only from this function. A block that holds it lists and writes the session user's rows only. Any other two-segment roster pattern, including `workforce/roster/**` and `workforce/roster/[owner]/notes`, is refused. |
...
| `hiredRosterStorageKey(row)` | `seatId` for an org-visible row, `~<userId>/<seatId>` for a user-owned one. |
