...
| `createSeatHireBlocks({ kinds, register, unregister, kindAt?, allowKinds?, channelBoards? })` | The seat-hire sequence itself, with no model in front of it — the same `{ hire, fire }` handlers `createSeatHireCapability` mounts as catalog tools, returned so an action can dispatch `hire` (or `fire`) directly. Same options, same behaviour; use this when nothing should call a model to run the sequence. |
