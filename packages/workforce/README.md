...
it is kind replacement, not an option on `hireWorkforce`. The factory takes five options in all:
`catalog`, `skills`, `model`, and the matcher tuning pair `classifierModel` / `confidenceThreshold`.
