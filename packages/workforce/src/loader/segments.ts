/**
 * What a team or worker folder name is allowed to be.
 *
 * One rule, shared by every reader that names a level of the tree, because the
 * two readers must agree on it for the same reason a worker id and a path must:
 * the segment is a folder on disk and half of an identity at the same time. A
 * reader that accepted a name the other refused would load a seat that cannot
 * be addressed, or address one that cannot be loaded.
 *
...
/** Which half of a seat's address a segment names, for the error to say. */
export type SegmentLabel = "Team" | "Worker";
