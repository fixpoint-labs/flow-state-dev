...
question: `poc/<ISSUE-ID>-<slug>/`, or `poc/epic-<name>/` at epic altitude.
...
codebase — and the approved spec *document* being landed onto the implementation branch is not
a licence for the POC beside it. If you want to keep something a POC produced, **graduate it**
(a real CI spec, or a `goals/` entry written under `tdd`) rather than copying it.

Why here and not inside a package: `poc/` is not a pnpm workspace package, so the
`turbo`-driven `pnpm typecheck` and `pnpm test` never see it, and `poc/**` is in `knip.json`'s
...
orchestrators read that signal. So: no `package.json` here, and never add `poc/` to
`pnpm-workspace.yaml`.
