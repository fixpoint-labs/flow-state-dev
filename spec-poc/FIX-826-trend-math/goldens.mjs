 ...
 * literal fixture expressions §10 states, and ALSO cross-checks the SMA series
 * bar-by-bar against an independent naive mean. Two independent methods that
 * agree is the check; one method plus plausibility is not.
 *
...
for (const { name, closes } of fixtures) {
  ...
  console.log(`  => extensionSma50Atr = ${round3(extension)}`);
