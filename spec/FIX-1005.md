...
4. **Recovery is bounded by an abandonment allowance, consulted inside the claim write, and
   ...
   **The counter is specified here because round 3 proved it cannot be left to the
   ...
   atomic write that recovers an abandoned row.
   **It is a top-level field and it must NOT live in `metadata` — BP-031, and a review round
   caught this pointing the other way.** `patchMetadata` merges arbitrary caller keys into
   `task.metadata` with a plain spread and no reserved-key protection
   (`resource-backed.ts:764-769`), and that verb is **model-facing**: `updateTask` hands an
   agent `metadata: z.record(z.string(), z.unknown())` and applies it unguarded
   (`task-tools-capability.ts:651-693`). A counter there could be **reset** by any ordinary
   metadata patch — unbounded recovery — or **inflated** to terminalize a live task early.
   That is an execution decision driven by caller-controllable input, which BP-031 forbids
   outright. §7 enumerates the seams and shows a top-level field is reachable by none of
   them. **This retires the placement §13's rolling-deploy note floated**: `metadata` survives
   an old writer's normalize precisely *because* it accepts unknown keys, and that same
   property is what makes it caller-writable. Strip-resistance and tamper-resistance pointed
   in opposite directions here, and tamper-resistance wins — the strip is bounded in policy
   (below), while a resettable counter has no bound at all.
   **Absent reads as zero** (BP-030), the
...
9. **The task row records `claimedBy` — where attempt N ran — stamped by the substrate
   inside the claim write, cleared wherever the claim ends, and **redacted at the client
   emission boundary**. It ships as its own PR, first and alone, and no logic in this design
   consumes it.**
   ...
   **"Read by nobody" was false as written, and a review round caught it — the schema is
   itself a reader.** Adding a field to `taskSchema` puts it inside `TaskChangeEvent.task`,
   and the collection factory emits that whole task as a **client-visible** component item on
   every successful mutation (`get-or-create.ts:143-160` spreads `task: event.task` into
   `ctx.emit.component`). The delegation board sets that stream `{ client: true, history:
   false }` explicitly (`task-tools-capability.ts:90`). So without a repair, every client
   subscribed to a board would receive the worker's `sessionId`, `requestId` and tenant id
   the moment a task is claimed — no consumer required, the streaming path delivers it. That
   pair addresses stream and recovery routes and `requestId` is treated as a capability
   elsewhere, so this is an exposure, not an untidiness (same class as FIX-1046).
   **The repair is redaction at the emission boundary, not moving the field off the row.**
   *Rejected: storing the coordinate outside the streamed task shape.* It would have to be
   written in the same atomic write as the claim to be correct, and cleared at every site
   that ends a claim — so it would either lose atomicity with the claim or rebuild the second
   "where a claim ends" list that §7 deliberately refuses to invent. That is the drift class
   decision 3 spent a round removing, traded for a leak that one projection closes.
   *Also rejected: narrowing the emitted envelope to an allow-list.* The whole-task envelope
   is a property **FIX-1011 depends on** to let clients read task lifecycle, so "stop emitting
   the envelope" is not available — the useful property and the leak are the same property.
   The repair is therefore a **named server-only field set**, omitted from the payload at the
   one boundary that spreads the row, so the envelope stays whole for every field a client
   should see and the exception is one list a future author edits rather than a rule they
   must remember. §7 has the shape and §10 the test.
   *Locks in:* a substrate-stamped identity coordinate is server-side truth and never crosses
   to a client. `claimedBy` is the first such field on this row, so this decision is where the
   boundary gets stated; the model-facing read path already works this way and is the
   precedent — `listTasks` returns an explicit projection (`id`, `goal`, `status`,
   `assignee?`, `attempts`), not the row (`task-tools-capability.ts:706-748`).
...
own contract; any *consumer* of `claimedBy`, including the cancellation routing FIX-982
builds on it (decision 9 ships the field, the redaction that keeps it server-side, and
nothing that acts on it); any read of the engine's request registry, and therefore any
dependency on FIX-999.

**Size:** Medium (multi-file, two PRs — see §8; ~300 LOC plus tests for the mechanism,
~80 for decision 9's field and the emission-boundary omission that ships with it).
...
**The abandonment counter's placement, and the seams it has to be unreachable from
(decision 4).** BP-031 makes this a correctness question rather than a taste one: the
allowance drives a terminal settlement, so the value it reads must not be caller-writable.
The enumeration below is the argument — asserting "there is one seam" is how this gets
missed, so here is the whole write surface (`collection/types.ts:264-372`) and what each can
reach.

| Seam | Can it write a top-level field? |
|---|---|
| `addTask` / `addTasks` (`types.ts:264-265`) | **No.** `buildInitialTask` constructs the row field by field from `TaskInit` (`internal.ts:50-72`), and `TaskInit` (`task-init.ts:14-35`) is an explicit allow-list with no `attempts` — a counter is not in it either, so a caller cannot seed one. |
| `patchMetadata` (`types.ts:372`) | **No — it writes only under `metadata`.** `{ ...(task.metadata ?? {}), ...patch }` (`resource-backed.ts:764-769`, `sequencer-backed.ts:649`). This is the only free-form verb on the surface, and it is namespaced. **A counter inside `metadata` would be fully caller-writable through it** — which is why decision 4 forbids that placement. |
| `setAssignee` / `setPriority` / `addLabel` / `removeLabel` (`types.ts:368-371`) | **No.** Typed single-field writes. Unfenced against terminal tasks by design (`types.ts:349-372`), but they cannot name another field. |
| The lifecycle transitions (`types.ts:268-347`) | **Only as specified.** Typed; the counter moves in exactly one of them, the claim write that recovers a lapsed row. |
| The model-facing `updateTask` (`task-tools-capability.ts:651-693`) | **No.** It exposes exactly `priority`, `metadata`, `assignee`, `addLabel`, `removeLabel` — a subset of the two rows above. No raw task patch reaches a model. |
| A rolling-deploy normalize (`patchRef`, `resource-backed.ts:445-482`) | **It can drop it** — not a caller mutation but the one way the value is lost. In policy per decision 4: absent reads as zero, so a strip costs recovery and can never strand a row. |

**So a top-level field needs no per-seam guard** — the typed surface already protects it, and
the guard would be the metadata placement's cost, not this one's. That is the whole reason
the placement is stated as a decision rather than left to the implementer.
...
- **The emission boundary, and it is the part PR a cannot skip (decision 9).** Schema
  membership is itself a publication: `onChange` spreads the whole post-mutation row into a
  component item — `ctx.emit.component(TASK_CHANGE_COMPONENT_TYPE, { …, task: event.task, … })`
  (`get-or-create.ts:143-160`) — and `TaskChangeEvent.task` is the full `Task`
  (`change-event.ts:36-43`). The delegation board marks that stream `{ client: true,
  history: false }` (`task-tools-capability.ts:90`), so the payload reaches a browser.
  **So the claim write is not the last place this field appears, and PR a must close the
  boundary in the same change that opens it.**
  - **Shape:** one exported projection — call it `toEmittedTask` — applied where the item
    payload is built, omitting a named `SERVER_ONLY_TASK_FIELDS` set of which `claimedBy` is
    the first and, today, only member. **A set, not an inline `delete`**, so the next
    server-only field is one entry rather than a rule someone has to remember, and so a test
    can assert the set is honoured rather than asserting one field name.
  - **Why omission rather than an allow-list.** The whole-task envelope is load-bearing for
    **FIX-1011**, which relies on it to let clients read task lifecycle. An allow-list would
    close this leak by breaking that, and would silently drop every field added afterwards.
    The useful property and the leak are the same property, so the exception is stated as an
    exception. Say this out loud in the code comment too — a later reader who finds a
    deny-list will otherwise "fix" it into an allow-list.
  - **Not the visibility flags.** `changeVisibility` is per-board and optional
    (`get-or-create.ts:44`), and the board that matters already sets `client: true`.
    Redaction cannot be conditional on a caller's flag.
  - **The read path needs nothing.** `listTasks` already returns an explicit projection —
    `id`, `goal`, `status`, `assignee?`, `attempts` — rather than the row
    (`task-tools-capability.ts:706-748`), so no model-facing surface exposes the coordinate.
    That asymmetry is worth naming: the model-facing surface projects, the streaming surface
    spreads, and only the second one needed a repair.
- **What is deliberately not built:** any *consumer*. No predicate consults it, no decline
  reason mentions it, and `reclaim()` is untouched. The first consumer is FIX-982's. What
  changed is the wording — "read by nobody" was false while the emission path read it by
  construction.
...
0. **PR a — record the coordinate, and close the boundary that publishes it** (decision 9).
   Add `claimedBy` to `taskSchema`; add the identity option on the backing options and read
   it at the claim write; clear it at every site that clears `leaseUntil` today; **and add
   the `SERVER_ONLY_TASK_FIELDS` omission at the emission boundary in the same change**
   (`get-or-create.ts:143-160`). Nothing else — no consumer, no predicate, no decline reason.
   **The redaction is not a follow-up.** Landing the schema field without it publishes the
   worker's `sessionId` and `requestId` to every subscribed client from the first claim, so
   a PR that ships only the field is the leak, shipped alone and early — the opposite of
   what "small and independent" is supposed to buy.
   *Test:* set on claim, with the tenant carried when the identity has one and omitted when
   ...
   absent and nothing infers from it; **and the emitted `task-change` item for a claim does
   not carry `claimedBy` while still carrying the rest of the row** — both halves asserted,
   because a test that only checks the field is gone also passes against a projection that
   dropped everything and would break FIX-1011. *Depends on:* nothing — not step 1, not
   FIX-981, not FIX-999. **Ship it without waiting for the rest of this list.**
...
6. **Disposition — the allowance check, its persisted counter, and the terminal settlement**,
   ...
   one decision and splitting them leaves the budgets separate in name only. **Put the
   counter at the top level of the row, never in `metadata`** (decision 4, BP-031): §7's
   seam table is the check, and the one seam that would defeat it is `patchMetadata`.
   *Test:* a default task with no `maxAttempts` **is recovered** (the headline case, and the
   ...
   side, which is the whole point of separating them; **and a `patchMetadata` carrying a key
   of the counter's own name leaves the counter unchanged** (BP-031), which is the test that
   fails if a later author moves it into `metadata` for strip-resistance. *Depends on:* 5.
...
| **A client subscribed to a board while a task is claimed** | Receives the `task-change` envelope with the row **minus** `claimedBy` (decision 9). Every other field is unchanged, because FIX-1011 depends on the envelope being whole. Without the omission the worker's `sessionId` and `requestId` reach the browser on every claim. |
| **An agent calls `updateTask` with a metadata key named like the abandonment counter** | The counter is untouched — it is a top-level field and `patchMetadata` writes only under `metadata` (decision 4, BP-031). Had it lived in `metadata`, that call would reset or inflate it, driving a terminal settlement from caller-controlled input. |
| **An old-version writer normalizes a row and drops the abandonment counter** | Absent reads as zero, so the row is recovered with the full allowance rather than none (BP-030). Costs recovery, never strandedness — and unbounded rather than one extra when such patches recur (§13). This is the cost accepted in exchange for the counter being tamper-proof; the `metadata` placement that would survive the strip is exactly the one a caller can rewrite. |
...
- **Decision 9's suite stands alone and ships with PR a**, before any of the below exists.
  ...
  separates.
- **The coordinate does not reach a client, asserted at the emitted item and not at the
  row** (decision 9). Drive a real claim through the factory's `onChange` and inspect the
  component-item payload: `claimedBy` is absent **and** the other fields are present. Both
  halves matter — asserting only the absence passes against a projection that narrowed the
  envelope and broke FIX-1011, and asserting on the stored row tests nothing, because the row
  is *supposed* to carry it. Assert it on a board with `client: true` explicitly, since that
  is the configuration the delegation surface ships (`task-tools-capability.ts:90`) and a
  redaction conditional on a visibility flag would pass a test run without one.
- **The counter cannot be moved by a caller** (decision 4, BP-031): `patchMetadata` — and the
  model-facing `updateTask` above it — with a key of the counter's own name leaves the
  counter's value unchanged and does not terminalize the task early. This is the test that
  fails if a later author relocates it into `metadata` to survive a mixed-version strip; the
  §13 note that suggests that placement is answered in decision 4, and this test is what
  keeps the answer.
...
- **PR a documents nothing user-facing, deliberately.** `claimedBy` is substrate
  bookkeeping with no consumer until FIX-982 ships one, and it is redacted from the client
  stream, so there is nothing a user can observe or act on. It gets a JSDoc block on the
  schema field carrying the BP-030 posture, the rolling-deploy strip (decision 9's cost),
  **and the server-only marking** — that the field is omitted at the emission boundary and
  why, so the next author does not "simplify" the projection away. The omission set itself
  gets a comment naming FIX-1011 as the reason it is a deny-list rather than an allow-list.
  Its changeset is `patch` and says one sentence: the substrate records where a claimed task
  is running. **No release-note line about the redaction** — the field never shipped
  unredacted, so there is no behaviour change to announce.
...
| **FIX-1046** | Not previously listed | **Related, not a dependency, and no edge asserted.** *"Session-requests listing can leak a protected flow's request history and items to a caller authorized only for a sibling flow"* (Backlog). Same **class** as decision 9's emission leak — a server-side identity/coordinate reaching a caller that should not address it — and it is why that leak is repaired in PR a rather than deferred. Nothing here consumes or blocks it: this issue closes its own boundary at `get-or-create.ts:143-160` and touches no engine listing path. If FIX-1046 becomes a release prerequisite, PR a is already on the right side of it. |
...
>   existed for.
>
>   **The obvious "storage old writers preserve" candidate was `metadata`, and a later round
>   refuted it — so this part of the note is now ANSWERED rather than open.** The old schema
>   carries `metadata: z.record(z.unknown())` and therefore passes unknown keys through
>   instead of dropping them, which is exactly the strip-resistance asked for. **It is also
>   what makes it caller-writable**: `patchMetadata` merges arbitrary keys with a plain spread
>   (`resource-backed.ts:764-769`) and the model-facing `updateTask` exposes that verb to an
>   agent (`task-tools-capability.ts:651-693`), so a counter there could be reset (unbounded
>   recovery) or inflated (early terminal settlement). BP-031 forbids driving an execution
>   decision from caller-controllable input, and it outranks the convenience here. **Decision 4
>   now fixes the depth: top level, never `metadata`**, and §7 enumerates the seams showing a
>   top-level field is reachable by none of them. What is left to the implementer is the
>   narrower question this note also raised — whether the counter is worth carrying at all
>   versus accepting the unbounded strip case as inside decision 8. The two properties genuinely
>   conflict, and the resolution is that a bounded-in-policy strip beats an unbounded tamper.
...
- **After the review round on the applied answers — two findings, and the first falsified a
  sentence this spec had carried since decision 9 was written.** *"`claimedBy` ships first,
  alone, read by nobody here"* was **false, because the schema is the reader.** Putting a
  field on `taskSchema` puts it inside `TaskChangeEvent.task`, which the factory spreads into
  a **client-visible** component item on every mutation (`get-or-create.ts:143-160`,
  `{ client: true }` on the delegation board). The worker's `sessionId` and `requestId` would
  have reached every subscribed browser from the first claim — the same exposure class as
  FIX-1046. Repaired by **redacting at the emission boundary** rather than moving the field
  off the row: moving it would cost atomicity with the claim write and rebuild the second
  "where a claim ends" list §7 refuses to invent, and narrowing the envelope is unavailable
  because **FIX-1011 depends on that envelope being whole**. So the leak and the useful
  property are the same property, and the exception is a named server-only field set with a
  test asserting both that the field is gone and that the rest of the row is not. Decision 9's
  sentence now says *no consumer* rather than *no reader*.
  **The second finding inverted a §13 note.** That note had floated `metadata` as
  strip-resistant storage for the abandonment counter — true, and disqualifying: `metadata`
  survives an old writer's normalize *because* it accepts unknown keys, and that is the same
  property that lets `patchMetadata`, exposed to an agent through `updateTask`, reset or
  inflate it. BP-031 forbids driving a terminal settlement from caller-controllable input, so
  decision 4 now fixes the counter at the **top level** and §7 enumerates the whole write
  surface to show a top-level field is reachable from none of it. Strip-resistance and
  tamper-resistance genuinely conflicted here; a bounded-in-policy strip beats an unbounded
  tamper. **Neither finding moved the mechanism, and neither reopens the owner's budget
  answer** — the counter is still one counted field with no per-seam guard, because the
  typed write surface already protects it.
