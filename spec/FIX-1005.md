...
4. **Earn the addition (tenet 3) — and the honest count, which has now moved three times.**
   ...
   **no new knob and no new configuration surface.** Decision 9's `claimedBy` is a second
...
9. **The task row records `claimedBy` — where attempt N ran — stamped by the substrate
   ...
   *What it is:* `{ sessionId, requestId, tenantId? }`, sourced from the `BlockContext` the
   ...
   `ctx.session.identity.id`, `identity.tenantId` — `packages/core/src/types/scope.ts`
   defines `ScopeIdentity` with `id` and the optional `tenantId`). A **fact**
...
**PR a is deliberately alone, and it is not waiting on b.** The two share no file the
other changes and no decision: a stamps a field nothing reads, b never reads it. Ordering
them would only import b's review latency into FIX-982's unblock, which is the coupling
decision 9 exists to remove. **Decision 7 is not weakened by this** — it forbids splitting
*the mechanism*, and b is the mechanism, whole.
...
**Owner decisions received 2026-08-09 — one applied, one that does not fit this spec.**
...
>
> **Not applied, and escalated instead — the answer labelled OQ-1.** The recorded answer
> reads: *"Option A… FIX-999 owns the narrow, identity-bound liveness capability (mirroring
...
> processes."* **Every clause of it is about the design in #1083**, which closed unmerged
> the same day, and it answers that spec's OQ-1 — not the OQ-1 below, which is a retry-budget
> policy question and shares only the label. Applying it literally means deleting this spec:
> it directs the mechanism this document replaced, an enablement gate this design has none
> of, and a "Decision 5" that is now a lease minimum. **This spec does not silently discard
> an owner decision and does not silently overrule one**, so it is on the PR as the first
> thing asked of the reviewer. Two facts the owner may not have in view when answering:
> `docs/specs/_epics/durable-jobs.md` at `4111bf0f7` already records the registry-oracle
> approach as superseded and adapter sharedness as *"not a dependency of this issue at
> all"*; and rounds 1–5 of review on this branch, including the owner's own, were run
> against the renewal design.
