...
3. **BP-030, and round 3 inverted what it demands here.** Every write normalizes the whole
   ...
   (`internal.ts:116-123`), which costs an extra recovery and can never strand a row. A
...
4. **Recovery is bounded by an abandonment allowance, consulted inside the claim write, and
   ...
   **The counter is specified here because round 3 proved it cannot be left to the
   ...
   than none. That direction is deliberate: over-recovery is bounded and in-policy
   (decision 8); withholding recovery is the bug this issue exists to fix.
...
5. **One knob: `leaseDurationMs`, validated with a minimum. Everything else is derived from
   ...
   *Locks in:* a caller states one thing — how long they may be gone before the work is
   ...
   construction rather than by agreement: it reads the span the claim actually committed, so
   no delay between the claim and the helper's first tick can shrink it.
...
6. **A caller's `eligibility` predicate *narrows* the substrate's candidate set; it does not
   ...
   **And narrowing may not assert `status` — that dimension is the substrate's, so a
   predicate asserting it is a caller error rather than a supported narrowing.** Not a style
   rule: the wake probe reads the shared predicate alone, so a status-asserting predicate
   leaves it permanently true on a lapsed row `claim` will never take and nothing will ever
   settle, and `waitForCondition` returns without waiting when its predicate is already true
   (`sequencer.ts:2102`) — so the worker spins to `maxIterations` and the drain then
   completes with the task still stranded, which is the defect §1 opens with arriving through
   the mechanism meant to fix it. *Cost, stated:* this keeps recovery a substrate invariant
   at the price of a contract nothing enforces. The alternative — composing each dispatcher's
   effective predicate into wake and quiescence — means surfacing that predicate on the
   public `TaskDispatcher`, which today exposes only `claim()` (`dispatchers/types.ts:28-39`)
   and hides its `eligibility` inside it; that is a larger public change than this issue
   carries. The documented recipe changes (§11).
...
8. **What we promise is bounded, and the bound is the lease: at-most-once *settlement*,
   ...
   *Locks in:* a crashed job is stranded for up to one lease, and a *paused* process can
   ...
   caller's property and not a timer we can tune. **Decision 3 is what makes this sentence
   true rather than merely likely** — before round 3 the bound was enforced by a timer the
   worker ran, so a slow store could settle a task whose lease had gone. Now the write
   refuses it.
...
- **The fence is one arm on the shipped guard, not a new guard (decision 3).**
  ...
  **one arm**: when a ticket is presented and the row it names is `in_progress` with
  ...
  - Untouched: writes that present **no** ticket. `transitionDeclineReason` returns early on
    `options === undefined` (`:355`), so `reclaim()` and every unfenced write behave exactly
    as they do today.
...
- **Two numbers, named, and only two.** `MIN_LEASE_DURATION_MS = 1_000`, enforced at the
  claim seam (decision 5), and `RENEWAL_DIVISOR = 3`, so the cadence is `span /
  ...
  **There is deliberately no separate cadence floor:** it existed to stop a
...
4. **One `withLeaseRenewal` helper**, exported from `orchestration`, imported at all three
   ...
   `leaseUntil - updatedAt` (decision 5), and carries `RENEWAL_DIVISOR`, the single-flight
   skip, and **one stop condition: a decline.**
   *Test:* a healthy long worker is never disturbed at any lease age; a worker whose row
   moved is aborted; a store outage does not abort a worker inside its lease; **a helper
   started late in a valid lease still derives the committed span, not the remainder** (the
   1,000 ms claim / 990 ms delay case); a slow store does not stack renewals. *Depends on:*
   2, 3.
5. **Admission — one `isClaimable(task, lookup, now)`** in `internal.ts`, consumed by
   ...
   *Test:* an abandoned row is admitted by all three consumers; `deps` still gate it; an
   ...
   on something other than status still recovers. *Depends on:* 4 — never merged ahead of it.
...
| **Custom dispatcher narrowing on `status === "pending"`** | **A caller error, not a supported narrowing** (decision 6). It does *not* degrade to today's behaviour: the wake probe admits the lapsed row, `claim` refuses it, nothing settles it, and the already-true predicate skips the wait — so the worker spins to `maxIterations` and the drain completes over a stranded task. Narrowing on anything *other* than status still over-reports the probe, and that stays pre-existing and transient, because the row it names is claimable by some drain. The documented recipe changes (§11). |
...
- **The committed span, not the remainder:** start the helper deep into a valid lease and
  assert the cadence it derives. The concrete case is a 1,000 ms claim reached 990 ms late —
  333 ms, not 3 ms. A suite that only ever starts the helper immediately after the claim
  cannot distinguish the two rules at all.
...
- **EXTEND** `apps/docs/docs/orchestration/task-substrate.md` a third time, at the
  ...
  **replaces** the default and recommends `t.status === "pending"` in the worked example.
  ...
  not assert `status`, because claimability is the substrate's to decide. This is a
  **documented contract changing**, not a wording fix.
...
- **EXTEND** `apps/docs/docs/orchestration/task-substrate.md` once more, on the write
  ...
  half of decision 3 and the reason to size a lease to the job.
...
three arrived after this spec converged; all three are implementation-level obligations that
live inside decisions already made (2 and 5), not direction changes. The last two are one
