...
4. **Each row reports the job's last recorded status — running, done, or failed — resolved when the list is read.**
   *Rejected:* keeping a copy of the status on the conversation record so the listing is a single query. That would add a write on **every** request completion across the whole product, foreground included, to a record that is already the contention point for conversation state — a system-wide cost to save reads on one screen. Also rejected: returning existence only and letting the UI ask about each job separately, which turns one page load into one request per job.
   *Locks in:* the UI can show a customer that their work is running, finished, or failed from a single call, which is the whole reason they open the view. The cost is real and bounded: the server does a little more work per row, so the page size has a ceiling (decision below) and the view is not free to poll aggressively. **What we are *not* promising: that a job shown as running is definitely alive.** A worker that crashed still reads as running until the substrate notices and corrects it. That gap is honest and it is the difference between this and true liveness.
...
>
> **Size:** Medium — one package, one PR, roughly 250 lines plus tests and docs.
...
**The cost of decision 4, priced honestly.** Status is resolved per row, so a page of N jobs costs N extra reads on top of the one that finds them. Three things keep that acceptable rather than merely tolerable. The reads are **indexed** — every adapter that has indexes carries `requests(session_id)`, `(session_id, status)` and `(session_id, tenant_id)`, so each is a keyed lookup rather than a scan. They are **in-process**, so the browser pays one round trip where the alternative made it pay N. And they are **bounded by the page size** decision 4 already requires, which is the neat part: the listing cap and the per-row cost solve each other, and a page of jobs has a fixed worst case rather than an open-ended one.
...
→ 200 { "workstreams": [
    { "id": "ws_9f2…", "parentSessionId": "sess_abc", "status": "in_progress", … },
    { "id": "ws_1c7…", "parentSessionId": "sess_abc", "status": "failed",      … },
    { "id": "ws_4b0…", "parentSessionId": "sess_abc", "status": null,          … }
  ] }
      ↑ enough to render running / failed / not-yet-started without a second call.
        `null` means no run yet — deliberately not defaulted to "running" (§9).
...
- **Untouched:** the session store and all four adapters (S1a shipped the predicate and its tests), the existing list-conversations handler and its default, the request-completion path, `client`, `react`, the DevTool, and every execution path.
...
3b. **Resolve last recorded status per row** (decision 4), over the request store's session indexes, inside the page the cap bounds. *Test:* a child whose last run completed, one that failed, one still in progress, and one with no runs at all; plus that the number of store reads scales with the page size and not with the total number of children. *Depends on:* 3.
...
| A child with several runs (a follow-up into the same job) | The most recent run's status, by the same ordering the request listing already uses. Not an aggregate across runs, which would have to invent a merge rule |
...
- **The status read scales with the page, not the parent** — a parent with many children, read one page at a time, must not read the request store once per child in the whole set. Asserting the shape of the cost, not a timing.
