...
1. **You reach a conversation's background jobs *through that conversation*, and you get them in two hops — the jobs, then each job's history.**
   ...
   *(An earlier draft justified this by saying the query-param route leaks whether a parent exists. That argument does not hold — the addressed route answers 403 for a parent the caller does not own, which leaks existence too, deliberately. The reason above is the one that survives.)*
...
> **Non-goals** — the client library and React hook that will wrap this (S2 / FIX-1011, S3 / FIX-1012); *creating* background jobs, and stamping what runs in them (FIX-982); **live worker liveness** — whether a job's worker is running *right now*, which needs the liveness substrate and is a genuinely separate problem; the end-to-end demo (S4 / FIX-1013); filtering jobs by topic or worker, which nothing asks for yet.
>
> **Not a non-goal, and not yet decided: coarse *recorded* status per row** — whether a job finished, failed, or is still going, read from what was persisted rather than from a live probe. An earlier wording of this section ruled it out along with liveness; that was too broad, because the two are different questions with different costs. It is now an open cross-spec fork the owner is deciding — see §12. **Nothing is built for it here either way.**
>
...
**Open questions:** one, and it is a **cross-spec conflict the coordinator raised and the owner is deciding** — not mine to settle, and recorded here so nobody approves this spec without seeing it.

> **Does a background-job row carry coarse recorded status, or only that the job exists?**
>
> This spec's read returns existence. S2 types its Workstream summary as a session summary plus coordinates, and the client's `SessionSummary` carries no status field at all. S3 then promises a user watches background work appear, fill in step by step, and stay visible as failed when it fails — none of which is renderable from a row that says only "this job exists." S3 therefore either issues one extra read per row on a screen every conversation renders, or drops two of its three sign-off decisions.
>
> **Branch (a) — this route returns coarse recorded status per row.** S3 renders from one read. The cost is stated below and it is not route-local.
> **Branch (b) — the route stays existence-only** and the cost lands in S3 as an N+1, or as reduced scope there.
> **Branch (c) — status is read from the task board, not from the session list.** Neither this route nor S2's summary carries it. See "the third answer" below; I think this is the one, and it is not mine to choose.

**The fact the decision turns on — settled here, because this issue owns the route.**

**Recorded status is *not* available to this route without an extra per-row read.** Measured against the tree:

- `SessionRecord` carries `latestRequestId` and **no status field of any kind** (`packages/engine/src/stores/types.ts:28-58`). It records which request was last started, never what that request did.
- Even the id is a *start-time* pointer: it is written when a request begins, for auto-resume discovery (`packages/engine/src/execution/runAction.ts:717-731`), and nothing writes back to the session record on completion or failure.
- There is no bulk path from ids to statuses. `RequestListOptions.sessionId` is **singular** (`stores/types.ts:218-255`) — no array — so N children means N queries, and its `status` field is an input filter, not an output projection. Sessions and requests are separate store contracts with separate adapters and no shared join layer.

So branch (a) costs one of two things, and the choice between them is itself the owner's:

1. **N+1 at the route** — one request read per child row. Better than N HTTP round-trips from the browser, and still N store reads per page (N separate queries on Postgres) on a screen every conversation renders.
2. **Denormalize a status onto the session record** — written wherever `latestRequestId` is written *and* at every terminal transition. That is a store-contract change across all four adapters with a legacy-read path (BP-030) — the same shape and roughly the same size as S1a itself, and not a route change.

**Waiting for FIX-982 does not produce it either.** FIX-982 declares `topic` and `coordinate` on the session record, not a status; and its liveness work is an internal drain reconciler that reads the *board's own rows* to find stalled wakes, deliberately not a queryable read surface. So "the chain is blocked on 982 for this" is not the answer — 982 as specced never supplies a per-session status.

**The third answer, and the reason I think the fork may be a false one.** The status this chain wants already exists, on a different axis. A task row carries a durable seven-value status with an enforced state machine — `pending`, `in_progress`, `blocked`, `awaiting_review`, `completed`, `errored`, `cancelled` — and `task-change` items are keyed, non-transient and client-visible, replaying on reload. That is exactly S3's three promises: work appears, fills in step by step, and failed work stays visible as failed. It is a **board read, not a session read**, and the epic's own position is that the board is the truth. On this reading, S3 is asking the session axis for something the task axis already answers better — a session list would give one coarse status per job where the board gives the real lifecycle — and the right fix is in S3's data source rather than in this route or S2's summary.

I have not changed anything on this basis. It is a scope call across three specs and it is the owner's.

**What none of this turns on:** liveness. Recorded status is a persisted field; liveness is whether a worker is alive right now, needs the substrate's gated verb, and can be absent by construction. This spec's non-goal was wrongly worded to exclude both — §6 is narrowed accordingly. **No field is added here under any branch.**
...
- **After spec review (round 1), cross-spec** — §6's liveness non-goal was too broad: it excluded coarse *recorded* status alongside live worker liveness, which are different questions with different costs, and S2/S3 need one of them. Narrowed to liveness only, with the recorded-status question recorded in §12 as an open three-branch fork for the owner. Settled the fact it turns on: recorded status is not reachable from this route without a per-row read, and FIX-982 does not supply it either — but the task row already carries a full lifecycle status, which may make the fork a false one.
