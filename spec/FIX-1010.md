...
Each row also says enough to render: which piece of work the job was for, and whether it is running, finished, or failed. Both come from the server — the first stamped when the job starts, the second read from what the job's last run actually recorded. Nothing a caller wrote is treated as fact.
...
>
> **Size:** Medium — one PR, roughly 250 lines plus tests and docs. Mostly one package; review established that it also needs **one index on each of the two persistent store adapters**, without which decision 4's cost claim does not hold (§3).
...
So the honest statement is conditional: **the cost claim holds only once a matching index exists, and creating it is part of this change.** With `(session_id, tenant_id, created_at)` on the two persistent adapters the planner walks to the newest matching row and stops — a genuine bounded seek, and the page cap then does bound the page. Without it, a polled endpoint pays N sorts over histories that grow, which is the failure this decision was supposed to avoid.

What that costs, stated rather than waved through: **it is a schema change in two adapter packages**, so §7's "untouched" list shrinks and this issue is no longer confined to the engine. It is additive and idempotent — index creation already runs on every startup and skips what exists — so no migration step and no downtime, and the change is two lines. And it is **not universal**: the in-memory and filesystem adapters carry no indexes at all and scan by construction, so the bound is a property of the production adapters, not of the contract. That is acceptable because those two are development and test stores, but a spec that claimed otherwise would be making the same unbacked promise again.
...
- **`@flow-state-dev/store-sqlite` and `@flow-state-dev/store-postgres`** — one index each, `(session_id, tenant_id, created_at)` on the requests table, so rule 5's lookup is a bounded seek rather than a sort over a child's history (§3). Additive and idempotent: index creation already runs at startup and skips what exists, so there is no migration and no downtime.
- **Untouched:** the session store contract and its four implementations (S1a shipped the predicate and its tests), the existing list-conversations handler and its default, the request-completion path, `client`, `react`, the DevTool, and every execution path. *An earlier draft listed the adapters as untouched outright; the index above is the exception, and it is a schema addition only — no store logic changes.*

**Where the status comes from, and where it must not come from.** The conversation record carries no status — its only run-adjacent field records which request last *started*, written before the run and never updated when one ends, so it cannot answer "did this finish." The status therefore comes from the request store, per child, over the indexes named in §3. **Nothing on the request-completion path changes**, and no status is copied onto the conversation record; decision 4 rejected that and §3 prices why.

**Four rules bind the status lookup. They are not the implementer's to weigh — each one is a defect if it is dropped, and two of them are silent.**
...
4. **Pass the status through unchanged.** Seven states ship; do not map, collapse, or default them (decision 4). A child with no runs reports absence, and absence is not a status.
5. **The lookup needs an index that does not exist, and creating it is part of this change.** Rules 1–3 describe a query — filter on session and tenant, order by start time, take one — that **neither persistent adapter can serve without sorting**: their composite index stops at the tenant and nothing indexes the start-time column. Add `(session_id, tenant_id, created_at)` to both, matching the filter-then-order shape so the planner seeks and stops. **Do not treat rules 1–3 as satisfied by a query that returns the right answer slowly** — the whole point of the bound is that this endpoint is polled, and the wrong plan is invisible in a test with three rows per child. §3 prices it; §10 verifies the plan, not just the result.
...
Both fields are **optional by design** (BP-030): an ordinary child session has neither, and a record carrying neither is by definition not a Workstream — which is the same signal a client needs to decide whether a row is labelable. **No response projection may drop `topic` or `coordinate`.** If the implementer narrows the returned shape (see §13), those two survive the narrowing; without them a UI can only label background work by an opaque derived id, which is most of what makes the list unreadable.
...
**Hop 2 inherits the parent's authentication policy, and it does so by construction — but the invariant is undocumented and worth pinning.** A reviewer raised this as a hole: in a mixed app where the parent's flow authenticates, could the child be served anonymously, since route authorization picks the resolver from the *addressed* record's flow kind? **It cannot, today.** A Workstream is not a separate flow — the detached core hangs off the parent's own flow definition, the spawn stamps the child with `flowKind: flow.kind`, and the adoption check refuses any record whose flow kind differs. So the child resolves the same resolver the parent does, and both hops are governed identically.

That makes this a **guard, not a fix**: the property holds, nothing states it, and one change would silently break it. If a Workstream were ever routed under a distinct flow kind, hop 2 in a mixed app would quietly become anonymous while hop 1 stayed protected. **Constraint on FIX-982: a Workstream's flow kind is its parent's.** §10 asserts it from the outside so the substrate cannot drop it unnoticed.
...
3b. **Add the `(session_id, tenant_id, created_at)` index** to the SQLite and Postgres request tables (§7 rule 5). One line each, alongside the existing request indexes, which are already created idempotently at startup. Do this *before* 3c so the query is written against an index that exists rather than retro-fitted to one. *Test:* the index is created on a fresh database and on one that already has the older set. *Depends on:* nothing.
3c. **Resolve last recorded status per row** (decision 4) — tenant key present, ordered by start time, one record per child, inside the page the cap bounds. *Test:* a child whose last run completed, one that failed, one still in progress, one cancelled, and one with no runs at all; the two-tenant case; the overlapping-run case; and the cost shape from §10. *Depends on:* 3, 3b.
...
| A child with no runs yet — admitted but not started | Status is absent, not invented. Do not default it to running; a job that has not started and one that is running are different things to a person reading the screen |
...
- **The lookup is served by an index rather than a sort** (§7 rule 5). This is the one behaviour a result-only assertion cannot reach: the query returns the correct row either way, and the difference shows up as a sort over a child's whole history on a polled endpoint. Check the query plan on the two persistent adapters, on a child with enough runs that a sort would be visible — a fixture with three rows per child proves nothing here. **Deliberately not asserted for the in-memory and filesystem stores**, which have no indexes and scan by construction; the bound is a property of the production adapters and §3 says so.
...
**Open questions:** none. One cross-spec conflict was raised during review and **has been decided** — recorded here with the evidence so it is not reopened.

> **Settled: a background-job row carries its last recorded status.** The draft returned existence only. S2 types its Workstream summary as a session summary plus coordinates, and that summary carries no status field; S3 then promises a user watches background work appear, progress, and stay visible when it fails — none of which renders from a row that says only "this job exists." Decision 4 resolves it: the route resolves status per row, in-process.
>
...
- "`handleListSessions` spreads full `SessionRecord` including `journal`; a workstreams listing may want summary fields only — otherwise N children × full blobs is expensive." — cursor ([review](https://github.com/fixpoint-labs/flow-state-dev/pull/1121#pullrequestreview-4891709234)) *(worth weighing against the page cap §9 now requires, which bounds the same cost from the other end)*
...
- **After spec review, closing folds** — two, both cost-free to the direction. §3 claimed the per-row status lookup was a keyed lookup on existing indexes; checked against both adapter schemas and it is not — the composite stops at the tenant and **nothing indexes the start-time column at all**, so the database sorts the child's whole matching history before the limit applies. `limit: 1` bounds the result, not the work. The fix is a `(session_id, tenant_id, created_at)` index on the two persistent adapters, which makes the claim true and makes this issue touch two packages rather than one; §3 prices it, §7 rule 5 requires it, §10 verifies the *plan* rather than only the result, and the size note says so. The claim was the third unbacked cost claim this epic has produced, which is why it is corrected rather than softened. Second: documenting `taskboard` as a request source is assigned to FIX-982 in §12 — the enumeration in the architecture doc and the store contract lacks it, and an unassigned doc deliverable between two issues is how it goes stale.
