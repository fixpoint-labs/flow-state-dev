  ...
  - **What replaces it is already in the stream, and carries more than the correlation would
    ...
    - **Cross-request resolution — a Workstream has many requests, so the same task can arrive
      ...
      **`createdAt` is the incarnation key, and it already exists — nothing new is invented here.**
      ...
      consumer holds. *One honest limit, of the same class as the tie-break below:* `createdAt` is
      wall-clock from the same injectable provider, so two incarnations created in the same
      millisecond are indistinguishable — that needs a delete and a recreate inside one
      millisecond, and the fallback is the ordinary rule over the merged pair.
