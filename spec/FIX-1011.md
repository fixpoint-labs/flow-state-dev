...
- **The last recorded status, which nothing on a session record supplies today** (decision 4).
  ...
  - **The union, stated plainly, because step 1 of §8 builds this type before the route exists.**
    ...
    - **Which runs count as live is POSITIONAL, not categorical — there is no single non-terminal
      set, and writing one is the mistake to guard against.** Two statuses are live wherever they
      sit; a third is live only when it is the job's **most recent** run:

      | | `in_progress` | `suspended` | `interrupted` | the other four |
      |---|---|---|---|---|
      | **any run** | live | live | **dead** | dead |
      | **most recent run** | live | live | **live** | dead |

      **Why `interrupted` splits.** A `/retry` supersedes it — and supersession is undetectable
      directly, because `retryOf` is written only on the **successor**
      (`packages/engine/src/routes/recovery-routes.ts:157`,
      `packages/engine/src/execution/request-recovery.ts:175-178`), never back onto the original,
      and it rides in `metadata` inside the record blob rather than a filterable column. But the
      successor is created **after** the original, so **a superseded record is never the most
      recent run.** Position is the evidence. So an `interrupted` record that is not most-recent has
      already been retried and is dead history; one that *is* most-recent has not been, and is
      genuinely awaiting continuation.
      *(Without the split, a job that failed, was retried and **succeeded** keeps a dead
      `interrupted` record that an existence check finds forever — the row reads running for work
      that is done. With `interrupted` treated as dead everywhere, a crashed run awaiting
      continuation reports finished. Both are false claims; the position is what separates them.)*

      **Why `suspended` does not split — and this is a reason, not a tolerance.** `/retry` accepts
      only `interrupted` and `failed` (`recovery-routes.ts:87-96`), so **nothing can supersede a
      suspended run.** An interrupted run is a claim on **the system**, discharged by a re-dispatch
      whose evidence is the newer record; a suspended run is a claim on **the user**, which no other
      run discharges. `in_progress` is live by definition.
    - **Why the tree's existing predicate is the wrong one to ask, in both positions.** The exported
      `isTerminalRequestStatus` is a *stream-termination* predicate ("past the in-flight phase") and
      counts `suspended` **and** `interrupted` as terminal
      (`packages/engine/src/stores/subscribe-helpers.ts:15-24`). It is wrong for the any-run test
      (it drops `suspended`, so a Workstream waiting on a person reports finished) and wrong for the
      most-recent test (it drops `interrupted` too). It is the obvious thing to reach for; see §12
      constraint 5. For the terminal side: `"completed"` and `"incomplete"` are the two values of
      `runAction`'s `terminalStatus` (`packages/engine/src/execution/runAction.ts:1467`, `:1488`),
      and `"failed"`/`"aborted"` join them in the retention prune set
      (`packages/engine/src/durability/durability-sweeper.ts:104`).
...
| A Workstream whose parent was deleted | **Empty list — never the orphans.** Deletion does not cascade, so the children outlive the parent, and the session guard returns `ALLOWED` when the record cannot be loaded — so without this rule an unauthenticated caller who knows a deleted parent's id enumerates them. Fail closed (§12 constraint 6) |
...
| **The DevTool reading the same route — a deliberate, accepted regression** | The DevTool sets `include_items: true` on this route to render historical requests' full item tree on cold open. Once the route filters unconditionally, **it stops receiving execution traces for requests it did not watch live** and its Trace tab is empty for them. This is **decided, not overlooked** (§12 constraint 7): the boundary is correct and the loss is ours, not customers'. **What restores it:** an operator/debug authorization tier that trace retrieval is gated on — a privileged-caller concept the framework does not have today. Do not restore it with a caller-supplied flag; that was tried and withdrawn |
...
| An unknown but well-formed parent id | **Empty list, not a 404** — indistinguishable from a parent with no background work, and deliberately so. Do not document a 404 here. Note the parent *is* loaded, but for **authorization** (§12 constraint 6), not as an existence feature: a load that fails denies rather than falls through |
...
- **The assertion that must not overreach:** decision 4's status is what was last *recorded*, so
  ...
  unmapped, or `"active"`. **Six cases carry it, and the last three are where a flat non-terminal
  set fails** — liveness is positional, not categorical (§7, §12 constraint 5):

  | Workstream's runs | row reports | why it is the case that catches a flat set |
  |---|---|---|
  | one `completed` | `"completed"` | baseline |
  | `completed` + `in_progress` | `"active"` | existence check, not a selection |
  | one `suspended` | `"active"` | `isTerminalRequestStatus` calls this terminal |
  | no requests at all | **absent** | neither `"active"` nor terminal (§9) |
  | one `interrupted`, most recent | `"active"` | a crashed run awaiting continuation has not finished |
  | `interrupted` **superseded by** a later `completed` retry | `"completed"` | the dead record must not keep the row live |

  The last two are the same status reported differently **because of position**, which is exactly
  what a single shared set cannot express. The absence case is the cheapest to write and the
  easiest to forget, because a row with no runs looks like a row that has not loaded. Do **not** write a test asserting a row goes stale-to-correct on
...
5. **What S1b needs from the store, and the one rule that is S1b's to write.** Split these, because
   ...
   **The active determination is an existence check over a set**, not a choice among its members,
   ...
   - **The determination must be atomic in effect, and three sequential probes are not. This is a
     ...
     **The shape S1b settled on, and why it needs no store change.** An unfiltered most-recent read
     that **reclassifies** the row it returns, plus an **unordered existence probe** for a live run
     that is not the most recent. The probe finds a run by *membership*, not position, so `orderBy`
     being descending-only (`packages/engine/src/stores/types.ts:239-247`) cannot hide a run that has
     been suspended for an hour behind one that started and completed after it — which was the
     interleaving this spec raised. The two reads test **different sets**, and that asymmetry is the
     rule rather than an inconsistency; see the next bullet.
   ...
   - **Which runs count as live is POSITIONAL, and the asymmetry between the two reads IS the rule.**
     Resolved with S1b after this spec pushed back on a flat reclassification; neither flat set was
     right, because each shipped a false claim in the opposite direction.

     > **Read 1 — does any run other than the most recent count as live?** Test
     > `{ in_progress, suspended }`.
     > **Read 2 — reclassify the most recent run.** Test
     > `{ in_progress, suspended, interrupted }`.

     **Do not collapse those into one constant.** The difference is the whole mechanism, and a
     single shared set reintroduces one of the two false claims whichever value it takes.

     **Why `interrupted` is live only in the most-recent position.** A `/retry` supersedes it, and
     supersession cannot be detected directly — `retryOf` is written only on the **successor**
     (`packages/engine/src/routes/recovery-routes.ts:157`,
     `packages/engine/src/execution/request-recovery.ts:175-178`), never back onto the original, and
     it rides in `metadata` inside the record blob rather than a filterable column. But the successor
     is created **after** the original, so **a superseded record is never the most recent run** —
     which read 2 already computes. Position is the evidence, at zero extra reads.

     **What each flat answer got wrong**, kept because it is why the rule looks asymmetric:
     - *`interrupted` always live* (this spec's earlier position) — a job that failed, was retried
       and **succeeded** keeps a dead `interrupted` record that the existence check finds forever,
       so the row reads running for work that is done.
     - *`interrupted` always dead* (S1b's earlier position) — a crashed run awaiting continuation
       reports that the work finished.

     **Why `suspended` does not split, with a reason rather than by tolerance.** `/retry` accepts
     only `interrupted` and `failed` (`recovery-routes.ts:87-96`), so **nothing can supersede a
     suspended run.** An interrupted run is a claim on **the system**, discharged by a re-dispatch
     whose evidence is the newer record; a suspended run is a claim on **the user**, which no other
     run discharges. That is why one is supersedable and the other is not.

     **Do not implement this with the exported `isTerminalRequestStatus`**
     ...
     implementer will find. **It is wrong for both reads, differently:** in read 1 it drops
     `suspended`, so a Workstream **stopped waiting for a person** reports a finished outcome — the
     precise sub-state the owner deferred to a later additive field, which folding into `terminal`
     now would make a **breaking change** instead of an additive one; in read 2 it drops
     `interrupted` as well, so a crashed run awaiting continuation reports finished too. No single
     call to it produces either read.
...
6. **The listing must authorize through the stored parent session's flow, not as a host-scoped
   ...
   **And the missing-parent branch must be specified, because it is the hole in this very
   constraint.** Authorizing on the *stored* parent session presumes a stored parent. When there is
   none, the guard **fails open**: `if (session === undefined) return ALLOWED`
   (`packages/engine/src/routes/route-auth.ts:224`). That state is reachable and not rare —
   **deletion does not cascade.** `handleDeleteSession` removes the session's content, resource
   state and the record itself and never touches child sessions
   (`packages/engine/src/routes/session-routes.ts:203-229`), so a deleted parent leaves its
   Workstreams behind. Combined, an unauthenticated caller who knows a deleted parent's id
   enumerates its orphaned children, because there is no record left to authorize against.

   **Required: a parent that cannot be loaded yields an empty list — never its children.** Fail
   closed. Empty rather than 404 keeps it consistent with §9's existing rule that an unknown parent
   and a parent with no background work are indistinguishable, and it leaks no existence
   information.

   *This does not contradict §9's "do not add parent-existence validation for this alone."* That
   line rejected existence-checking as **its own feature**. This is not that: constraint 6 already
   requires loading the parent **to authorize**, so the load is happening regardless — the
   requirement is only that a failed load denies rather than falls through. No extra read, and the
   justification is authorization rather than validation.
...
7. **Hop 2 must not hand a browser the raw item log. This spec makes that route the canonical
   ...
   **The requirement, unconditional: hop 2's browser response carries client-visible items only,
   and no caller-supplied parameter may widen it.** `listSessionRequests(ws, { includeItems: true })`
   returns the conversational and structural items — including the `task-change` items §7's fold
   depends on, which are `component` items and client-visible — and never the trace log. The client
   does no filtering of its own; this is the route's guarantee (BP-031).

   **An earlier revision of this constraint specified a caller-supplied `?include=trace` opt-in,
   mirroring the stream route. That is withdrawn — it was not a repair.** A query parameter is not
   an authorization boundary: any caller who can reach the route appends it and gets the prompts,
   history, block I/O and tool arguments back. BP-031 is *never make an authorization decision from
   caller-controllable input*, and a query parameter is its canonical example. The previous revision
   **named that violation and shipped it anyway with a caveat attached**, which is not how a
   best-practice violation is discharged. Recorded rather than quietly deleted, because the failure
   was in the reasoning, not the citation.

   **The precedent that revision leaned on is itself an instance of the pattern below.** The stream
   route's `?include=trace` (`packages/engine/src/routes/stream-routes.ts:88`, `:142`, `:208`,
   `:254`) has the same property: caller-supplied input decides whether trace items are returned.
   Adopting it as precedent without examining it was reproducing an exposure rather than reasoning
   about one. **Not this spec's to fix** — it is a different route with a different owner — but it
   belongs in the epic-level record below rather than being silently copied.

   **What blocks a complete answer, reported rather than papered over: the framework has no
   privileged-caller concept to gate trace access on.** `ResolvedPrincipal` is
   `{ userId: string; orgId?: string }` — **no roles, no scopes, no operator tier**
   (`packages/core/src/types/auth.ts:30-33`) — and the `debug_*` routes are authorized exactly like
   any other session-addressed route, by the flow's principal resolver and the stored owner
   (`packages/engine/src/routes/route-auth.ts:112-117`, `:218-229`). So "gate trace retrieval on a
   trusted operator path" has nothing to gate on today.

   **And the DevTool genuinely depends on the unfiltered log** — `include_items: true` in
   `packages/devtool/src/react/hooks/use-session-requests.ts:34`, consumed as `req.items` in
   `DevToolPanel.tsx:309`, added for it per `.changeset/fix-733-list-requests-include-items.md`.
   So the unconditional requirement above and the DevTool's needs cannot both be met by this route
   as the framework stands.

   **Decided: filter unconditionally and accept the DevTool regression.** The trade is lopsided —
   being wrong here costs **us** cold-open trace history in an internal tool, recoverable in a later
   change; being wrong the other way ships an open door on the route this spec makes the canonical
   browser drill-in, on customers' data. A correct boundary today beats a boundary we intend to
   build. The regression is recorded as a named consequence in §9 rather than a footnote, so
   whoever restores it knows what they are restoring and why it was taken away.

   **The shape that would restore it** is an operator/debug authorization tier — a privileged-caller
   concept the framework does not have today — with trace retrieval gated on it. **Not designed
   here**, and deliberately not sketched: it is a framework-level addition with its own security
   surface, and guessing at it in this spec is how the withdrawn opt-in happened.
...
**Open questions: none belonging to this spec. One gap remains open; two more are now closed.**
...
None of the items below is this spec's to close, and none gates its build. The non-terminal-set
disagreement that stood here is also resolved — liveness is **positional**, and the rule is in
constraint 5.
...
> **1b · ~~How the DevTool keeps its trace access once hop 2 is client-filtered.~~ CLOSED —
> decided: filter unconditionally and accept the regression.** The trade was lopsided: the cost of
> being wrong lands on us (cold-open trace history in an internal tool, restorable later), the cost
> of the alternative lands on customers (an open door on the canonical browser drill-in). Recorded
> as a named consequence in §9 and in constraint 7. The operator/debug authorization tier is the
> shape that would restore it — `ResolvedPrincipal` is `{ userId, orgId? }` with no roles or scopes
> (`packages/core/src/types/auth.ts:30-33`), so it does not exist today — and it is scoped
> deliberately elsewhere, not designed here.
...
**The epic has now produced six.** Five are citable from this spec; all were found in review rather
than in design:
...
3. **Constraint 7** — the unfiltered item log: a route whose `include_items` flag was added *for the
   DevTool* becomes, in this spec, the canonical drill-in for an app's browser.
4. **The trace opt-in (constraint 7, withdrawn)** — and this one is the sharpest, because it was
   produced *by the repair for #3*. Reaching for the stream route's `?include=trace` as precedent
   copied a caller-supplied flag into an authorization position. **The precedent was itself an
   unexamined instance of the pattern**, which is how the pattern propagates: a fix that borrows a
   neighbour's shape inherits its unexamined assumptions. Worth recording precisely because the
   author had already written down the BP-031 objection and shipped anyway — *naming a violation
   does not discharge it.*

   > **The precedent instance is LIVE, in shipped code, and on a different owner's route.** The
   > stream route's `?include=trace` (`packages/engine/src/routes/stream-routes.ts:88`, `:142`,
   > `:208`, `:254`) decides trace exposure from a caller-supplied query parameter today. **Nothing
   > in this spec fixes it**, and it is named here so a browser-exposure pass starts from it rather
   > than rediscovering it. It is also the evidence that this class is **not confined to what this
   > epic adds** — it is already in the codebase, which is what makes the pass worth running as a
   > pass rather than as six individual fixes.
5. **The deleted parent (constraint 6)** — deletion does not cascade, and the session guard fails
   **open** when the record is gone, so orphaned children stay enumerable by anyone holding the dead
   parent's id. Same shape from the other side: an authorization model that was correct while its
   subject existed, promoted to a surface that outlives it.
...
- **After the classification disagreement resolved — liveness is positional, not categorical.**
  Neither flat set was right, and each shipped a false claim in the opposite direction: with
  `interrupted` always live, a job that failed, was retried and **succeeded** keeps a dead record
  the existence check finds forever; with it always dead, a crashed run awaiting continuation
  reports finished. The resolution is a **recency rule** rather than a reclassification —
  `interrupted` is live **only when it is the job's most recent run** — and it needs no new data,
  because supersession is undetectable directly (`retryOf` is written only on the successor and
  lives in an unfilterable blob) but the successor is created *after* the original, so a superseded
  record is never most-recent. So read 1 tests `{in_progress, suspended}` and read 2's
  reclassification tests `{in_progress, suspended, interrupted}`, and **the asymmetry between them
  is the rule** — collapsing them into one constant is the mistake to guard against. Swept every
  place this spec named a set or a count: §7's table, §12 constraint 5, and §10's cases, which now
  carry the superseded-`interrupted` case explicitly. The `suspended` half survives with a reason
  rather than by tolerance: `/retry` cannot supersede it at all, because an interrupted run is a
  claim on the **system** and a suspended one is a claim on the **user**.
- **After the DevTool fork was decided — filter unconditionally, accept the regression.** The trade
  was lopsided: being wrong costs us cold-open trace history in an internal tool and is restorable;
  being wrong the other way ships an open door on the canonical browser drill-in. Constraint 7
  stands as written, and the regression is a **named consequence in §9** rather than a footnote, so
  whoever restores it knows what it was and why it went. The operator/debug authorization tier is
  named as the restoring shape and deliberately not designed here — guessing at it is how the
  withdrawn opt-in happened. Also marked the propagation instance **live**: the stream route's
  `?include=trace` is shipped code on a different owner's route, which is the evidence that this
  exposure class is not confined to what this epic adds.
- **After the trace opt-in was challenged — withdrawn, and the DevTool conflict escalated instead.**
  The previous round specified a caller-supplied `?include=trace` opt-in, **named it as a BP-031
  violation, and shipped it anyway with a caveat.** That was wrong: a query parameter is the
  canonical example of caller-controllable input deciding what a caller may see, so the "repair"
  was a default in front of an open door. Constraint 7 now requires client-visible items
  unconditionally, with no caller-supplied widening. Checked whether the trusted path the fix needs
  exists, and **it does not** — `ResolvedPrincipal` is `{ userId, orgId? }` with no roles or scopes,
  and the `debug_*` routes carry no extra tier. So the DevTool's dependence on the raw log is
  **reported as an open item for the owner** rather than resolved with a second caveat. Also
  recorded that the precedent the withdrawn fix leaned on — the stream route's `?include=trace` —
  is itself an unexamined instance of the same pattern, which is how it propagated.
- **After the deleted-parent case was checked — constraint 6 gains its missing-parent branch.**
  Authorizing on the *stored* parent presumes one exists; when it does not, the guard **fails
  open** (`route-auth.ts:224` returns `ALLOWED`), and deletion **does not cascade**
  (`session-routes.ts:203-229` never touches children), so orphaned Workstreams stayed enumerable
  by anyone holding a dead parent's id — a case this spec had left to "whatever the server returns"
  while also forbidding parent-existence validation, so nothing owned it. Now required to fail
  closed and return empty, with the §9 rows corrected and the apparent contradiction reconciled:
  the parent is loaded to **authorize**, not as an existence feature, so a failed load denies
  rather than falls through.
- **After S1b answered those facts — the ordering question is settled and the membership one is
  now an open disagreement.** S1b confirmed read 1 is an **unordered existence check**, so a
  long-suspended run is found by *membership* rather than position and the ordering concern is
  answered; the set-predicate withdrawal stands. S1b also reclassified **`interrupted` as
  terminal**, on the ground that `/retry` mints a new record and leaves the original stranded.
  **This spec does not adopt that**, and the case is recorded in constraint 5 rather than folded:
  `/continue` exists only for `interrupted` and re-enters **in place under the same id**, making it
  the structural twin of `suspended`/`/resume`; `/retry` accepts `failed` on identical terms, so
  "mints a new record" separates nothing; stranding applies equally to an unanswered `suspended`
  gate; and since `interrupted` is what crash reclamation *writes*, calling it terminal makes a
  crashed worker awaiting continuation report that its work **finished** — a false completion where
  the current rule gives a documented staleness. Flagged as needing one answer, because two specs
  shipping different non-terminal sets is the failure `cross-spec-review` exists to prevent.
