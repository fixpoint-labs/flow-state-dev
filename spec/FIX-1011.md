...
- **One summary type, extended — not a parallel hierarchy.** A Workstream row *is* a session row,
  ...
  - The coordinates take **orchestration's existing vocabulary rather than invented names**:
    `boardId`, `topic`, and the `coordinate` tagged union (`assignee` / `uniform` / `floor`) that
    a Workstream's own id is derived from. Reusing those names is what lets a reader match a row
    back to the board that dispatched it; inventing parallel ones would break that by hand.
  - Everything added is optional and `== null`-guarded (BP-030). The direction is extend, then
    narrow: a session summary becomes a Workstream when the parentage and coordinates are there.
- **Be exact about what that row carries, because "a session summary plus extras" reads as more
  than it is.** A session summary today is **identity and description only** — id, flow kind,
  user, title, description, tags, metadata, timestamps. **There is no run state on it.** The one
  run-adjacent field anywhere on a session record is `latestRequestId`, which is *an id, not a
  status*, is written when a request **starts**, and is never updated when one finishes — so a row
  carrying it can point at a request that failed an hour ago and say nothing about it.
- **So a Workstream row tells you what exists, not how it is going.** A UI can list background
  ...
  changes is an open cross-spec dependency, not an implementer's call — see §12. **The extended
  type above is also where a coarse run-state field would land** if that fork resolves that way,
  which is why the type story is settled once here with the fork in view rather than twice.
...
3. **The rows the listing returns carry the coordinates that identify a Workstream** — at
   minimum `topic`, which body of work it is for. **This is the one assumption whose failure
   changes what this issue is worth**, so here is the answer plainly rather than as a caveat:
   **S2 does not block on it, and ships with a labeling fallback.** The coordinate fields are
   optional under BP-030 regardless, hop 1 works without them, and blocking would stall the very
   chain S4 gates on. What degrades is the product: a UI can then only label background work by an
   opaque derived id, and decision 3 buys much less than it should. **So S1b shipping `topic` is
   not a prerequisite but is the difference between a usable surface and a technically-correct
   one** — worth raising with S1b directly rather than discovering at S3.
...
**The 1+N is the same fact in different clothes, and it is the strongest argument in the fork.**
Say it outright rather than leaving S3 to find it: under branch (b), an app that shows every
Workstream with its status makes **one call for hop 1 plus one per child** — from a browser, on a
screen every conversation renders. That is not a polling detail an implementer tunes; it is the
structural cost of a stateless row, and it scales with how much background work a user has.
**Under branch (a) it disappears entirely** — the status arrives in the rows hop 1 already
returned, and hop 2 becomes drill-down rather than a render-time dependency. Any consumer
documentation written before this resolves must steer list-and-poll paths to summary-only reads
and reserve full item logs for drill-down, or the canonical example becomes the naive loop.
