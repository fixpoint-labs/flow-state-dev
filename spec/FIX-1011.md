...
```ts
// A Workstream request that has not finished yet. Note the flow kind comes
// from the request, not from the conversation — a detached request runs under
// the substrate's own flow, and the stream route 404s on a mismatch.
createSSEClient({
  url: `/api/flows/${requests[0].flowKind}/requests/${requests[0].id}/stream`,
...
- **The assertion that must not overreach:** decision 4's status is what was last *recorded*, so
  ...
  turns on the states a collapsing implementation would quietly lose. **And cover the overlapping
  case** (§12 constraint 5): two requests on one Workstream where the earlier one finishes after
  the later one starts, asserting the row does not read *completed* while work is still running. A
  test with one request per Workstream passes whether the resolution is deterministic or not, so
  it proves nothing about the guarantee decision 4 makes. Do **not**
...
5. **That resolution has to be deterministic under overlapping requests, and the obvious
   ...
   failure class as the ambiguous correlation above. **S1b must resolve the row's status from a
   deterministic anchor** — dereferencing the session's own latest-request pointer, or ordering
   explicitly by start time — and say which. §10 requires the overlapping case be covered, because
   a test with one request per Workstream passes either way.
