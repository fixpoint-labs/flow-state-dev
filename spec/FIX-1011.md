...
4. **A Workstream row says whether its work is running, finished, or failed.**
   The listing carries each Workstream's last *recorded* status alongside its identity, resolved
   server-side as part of the same read.
   *Rejected:* a stateless row, which would leave every consumer fetching one extra read per
   Workstream from the browser just to render a label — on a screen every conversation shows.
   *Locks in:* an app can render a list of background work **with its state in a single call**,
   which is what makes a "what's running" view cheap enough to put on a main screen rather than
   behind a click. The honest caveat ships with it and must be documented, not buried: this is the
   **last recorded** status, not a health check. If a worker's process dies, its row keeps reading
   *in progress* until the substrate's reclamation notices and corrects it. An app can trust it to
...
- **The last recorded status, which nothing on a session record supplies today** (decision 4).
  ...
  - **Where the correlation actually lives, because the rule above is undeliverable without it.**
    ...
    **a hop-1 row on its own has no task correlation** — `topic` and `coordinate` identify the
    ...
    behind the item log**, so reading it costs no over-fetch. A UI that wants task-lifecycle
    authority already had to reach hop 2 to have anything to be authoritative about.
    This depends on a key a consumer may rely on, which does not exist as a contract today —
    constraint 2 in §12 names it.
...
- **The assertion that must not overreach:** decision 4's status is what was last *recorded*, so
  the test for it seeds a request in a known status and asserts the row reports that. Do **not**
...
2. **A detached request carries its task id at a named, documented key in request metadata —
   `metadata.taskId`.** FIX-982 already writes exactly that key, in its own usage example and in
   its §10 behaviour 2 ("`source: "taskboard"` and its `taskId` in metadata"). **What is missing
   is not the value but the promise:** request metadata is an opaque bag, so absent a stated
   contract no consumer may depend on the key, and §7's display rule has nothing to correlate on.
   The ask is therefore to *pin what FIX-982 already does*, not to add a field.

   *Why a named metadata key rather than a typed field on the request summary.* A typed field
   would be cleaner and is what I would choose on a blank sheet — it would survive a rename and
   be discoverable from the type. But it is a new persisted surface on a spec that has already
   converged, with its own back-compat story for rows written before it, and it buys nothing this
   rule needs: one documented key is enough to make the correlation reliable. **If a later
   consumer wants the stronger guarantee, promoting it to a typed summary field is additive** —
   flagged in the follow-ups, not built here.
...
- **Promoting the task id to a typed field on the request summary** — §12 constraint 2 settles for
  a documented metadata key, which is enough for the display rule. A typed field would survive a
  rename and be discoverable from the type; it is additive whenever a consumer wants it.
