...
- **The last recorded status, which nothing on a session record supplies today** (decision 4).
  ...
  - **And the correlation is a pair, not an id.** A task id does not identify a task on its own:
    ...
    The identity a consumer needs is therefore **`(collectionId, taskId)` together, and a client
    must treat an incomplete pair as no correlation at all** — falling back to the recorded status
    rather than guessing.
  - **`collectionId` specifically, not "whichever identifier is handy."** It is the collection's
    *address*: the same value forms the resource pattern and the `ctx.resources` lookup key, so a
    consumer holding it can actually reach the collection whose lifecycle the display rule calls
    authoritative. `boardId` is a **separate, optional** field — a routing and re-key identity —
    and there is no public mapping from it to a collection. A correlation carrying only `boardId`
    would name the right task and still leave a consumer unable to look it up, which is the
    display rule failing in a new way rather than a smaller version of it. §12 constraint 2 states
    the requirement on FIX-982, including which half is a genuine addition.
...
- **The assertion that must not overreach:** decision 4's status is what was last *recorded*, so
  ...
  turns on the states a collapsing implementation would quietly lose. **And cover tenant
  isolation on the per-row lookup** (§12 constraint 5): two tenants whose Workstreams share a bare
  session id, asserting one tenant's row never reports the other's request. Omitting the tenant
  key is an unrestricted read, so a single-tenant test cannot fail. **And cover both overlap
  races** (§12 constraint 5), because they fail differently: (a) *completion order* — two requests
  where the earlier finishes after the later starts, asserting the row does not read *completed*
  while work is still running; and (b) *concurrent starts* — two requests starting together,
  asserting the row names the later one. A test with one request per Workstream passes whether the
  resolution is deterministic or not, and a test for (a) alone passes an implementation that still
  loses (b). Do **not**
...
not advice.** For each call in an example, answer two questions from the source: **what does this
call actually return, and in what order?** Then check that the surrounding code is safe if the
answer is "nothing" or "not the order you assumed."
...
that. Four of the five are exactly what those two questions surface. The rule is cheap and the
...
5. **That resolution has to be deterministic under overlapping requests, and the obvious
   implementation is not.** A Workstream can legitimately have two requests in flight — request
   concurrency defaults to `allow` — and the request listing's **default ordering is by last
   update, not by start**. So "the most recent request" resolved the obvious way reports the wrong
   one whenever an earlier request finishes after a later one started: the first completes, sorts
   to the top on `updatedAt`, and the row reads *completed* while work is still running. That is
   decision 4's single-call guarantee returning a confidently wrong answer, which is the same
   failure class as the ambiguous correlation above. **S1b must resolve the row's status by
   ordering explicitly on start time, with a deterministic tie-breaker.**
...
2. **A detached request carries the *complete identity* of its task at named, documented keys in
   ...
   - **The `collectionId` — a genuine addition, and priced as one.** FIX-982's spec states only
     `taskId` and the routing `coordinate` in request metadata; **nothing there persists the
     collection the task belongs to.** So this is a new key at the spawn write site, not a promise
     about existing behaviour. The value is resolvable at that moment — the board carries its
     `collectionId` — so it is cheap where it is written, but it is still a change to a converged
     spec and must be raised with FIX-982 as one, not slipped in.

   **The value is `collectionId`, and this spec names it rather than leaving the choice open.** An
   earlier revision offered `boardId` or `collectionId` as equally acceptable; they are not. The
   collection's id *is* its address — the same value forms the resource pattern and the
   `ctx.resources` lookup key — so it is what lets a consumer reach the collection at all.
   `boardId` is a separate and optional field carrying routing and re-key identity, with no public
   mapping to a collection, so a correlation built on it names a task nobody can look up.
   Permitting either would have satisfied the letter of the constraint and still left the display
   rule undeliverable.

   *If FIX-982 would rather carry `boardId`,* then the mapping from it to a collection has to be
   defined and public — and that is another unwritten mechanism on a constraint that has already
   been wrong twice, so the recommendation is not to.

   **Why this constraint has moved three times, stated so the next reader does not have to
   reconstruct it.** The correlation has been specified three ways, each more precise than the
   last and each still wrong about what the runtime exposes: **`taskId` alone** (unreachable — no
   typed task id on a request or session at all), **the pair with either identity** (ambiguous
   across boards, then unaddressable), and now **the pair with `collectionId`**. The failure was
   the same every time — describing an identifier from how it reads in prose rather than from what
   the code returns and what it can be used for. That is the same class as this spec's five
   example defects and its four selection rules, and it is the argument for applying §11's check —
   *what does this call actually return* — to **constraints placed on other issues**, not only to
   published examples. A constraint is a promise someone else has to keep; it deserves the same
...
- **Resolved at the route — taken.** One read per row, in-process: the client pays one round trip
  ...
  **What this costs S1b is larger than this section first claimed, and the correction matters
  ...
  true of the correct one. Getting the answer right under overlapping requests (constraint 5)
  needs start-time ordering with a tie-breaker, which S1b reports pulls in index work across both
  SQL adapter packages, a query-plan requirement, and possibly ordering changes in all four store
  implementations. **The product outcome in decision 4 is unchanged** — a row still carries its
  state, still in one call. What changed is that the mechanism behind it is real work in S1b
  rather than something that falls out of indexes we happen to have. If decision 4 is worth
  ratifying it is worth ratifying at that price, not at the cheaper one this section quoted.

  **And the price is not yet fully known, because the rule for choosing *which run a row reports*
  is still unwritten after four attempts.** Recorded as evidence rather than solved here — a fifth
  attempt is exactly what should not happen while the owner is deciding whether we need such a
  rule at all:
