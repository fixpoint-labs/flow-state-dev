...
- A **Workstream summary type**: everything a session summary already carries, plus the
  coordinates that make a Workstream meaningful to a UI — which body of work it is for, and
  which board dispatched it. Every one of those optional and `== null`-guarded, because a server
  that predates them returns none of them (BP-030).
...
**Open questions:** none that block implementation. Decision 1 is a genuine live fork and is
asked in full on the spec PR, but it *narrows* scope rather than gating the build — reversing it
would be a separate, larger issue with its own authorization design, not a variation of this one.
