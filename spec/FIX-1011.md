...
- **The last recorded status, which nothing on a session record supplies today** (decision 4).
  ...
  - **What replaces it is already in the stream, and carries more than the correlation would
    ...
    - **Cross-request resolution — a Workstream has many requests, so the same task can arrive
      ...
      > `(data.collectionId, data.taskId)`. Within a group, **prefer a snapshot whose
      > `data.task.status` is terminal over one that is not**; then, among the snapshots that
      > remain, take the greatest `data.task.updatedAt`, breaking an exact tie on
      > `(requestId, itemIndex)` descending.

      **Two clauses, because either alone is wrong.** The terminal preference comes first because
...
5. **What S1b needs from the store, and the one rule that is S1b's to write.** Split these, because
   ...
   **The active determination is an existence check over a set**, not a choice among its members,
   ...
   - **But it must be ONE read, and today's contract cannot express it. This is a new constraint on
     FIX-1010.** An earlier revision of this constraint offered "three `limit: 1` probes, one per
     non-terminal status, or one scan — S1b's call." **The three-probe option is wrong and must not
     ...
     What the determination needs is **one read whose predicate is `status IN (<non-terminal
     set>)`**, with `limit: 1` — a single snapshot, tenant-scoped, evaluated once.
     **That is not expressible against the store contract today:**
     - `RequestListOptions.status` is a **single** `RequestStatus`, not a set
       (`packages/engine/src/stores/types.ts:236`), and the adapters implement it as one equality
       test (`packages/engine/src/stores/memory/request-store.ts:268-270`).
     - `RequestStore` exposes no `count` or `exists` — `list` is the only read
       (`packages/engine/src/stores/types.ts:438-459`).
     - The one shape that *is* atomic today — an unfiltered `list({ sessionId, tenantId })`, then
       test the statuses in memory — is **unbounded**: `limit` has no default, so it returns every
       request the Workstream has ever had (`packages/engine/src/stores/shared.ts:7-15`), once per
       row, on a screen every conversation renders. A `limit` cannot be added to bound it either,
       because the window is ordered by `updatedAt` and the non-terminal request may sit outside it.

     **So FIX-1010 needs the contract to express a status set** — widen `RequestListOptions.status`
     to accept `RequestStatus | RequestStatus[]`, or add an existence predicate — and this spec
     names it rather than assuming it. **Routed exactly like the terminal reduction below: a named
     constraint on S1b, not a decision taken here.** Nothing in this spec's build waits on it; the
     client carries whatever value the route returns.
