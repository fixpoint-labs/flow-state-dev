...
5. **S1b's listing resolves each row's last recorded status** — the mechanism decision 4 rests on.
   Unlike the four above this one is **settled, not assumed** (see below), but it is S1b's to
   build, so it is named here as a dependency rather than left implicit.
