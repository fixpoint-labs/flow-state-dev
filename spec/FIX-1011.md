...
5. **What S1b needs from the store, and the one rule that is S1b's to write.** Split these, because
   ...
   **The active determination is an existence check over a set**, not a choice among its members,
   ...
   - **The right non-terminal set, which is the one real trap.** Non-terminal is exactly
     ...
     > **⚠ Open cross-spec disagreement — S1b currently classifies `interrupted` as terminal, and
     > this spec does not. One answer is needed; the two specs must not ship different sets.**
     >
     > **S1b's argument:** `/retry` mints a new request record and never writes the original, so an
     > `interrupted` record strands and its row would read `"active"` forever.
     >
     > **Why this spec holds at three.** The premise is true about `/retry` and does not support the
     > conclusion:
     > - **`interrupted` is leavable in place, exactly as `suspended` is.** `/continue` exists
     >   *only* for it — *"only `interrupted` requests continue; use `/resume` for `suspended`,
     >   `/retry` for a fresh run"* (`packages/engine/src/routes/recovery-routes.ts:235-243`) — and
     >   `continueRequest` re-enters **under its OWN id, no new request**, with the record
     >   transitioning *"`suspended → in_progress → terminal` in place"*
     >   (`packages/engine/src/execution/request-continuation.ts:4-13`). `/resume` and `/continue`
     >   are structural twins; `suspended` and `interrupted` are the two statuses they recover.
     > - **The discriminator does not discriminate.** `/retry` accepts `interrupted` **and
     >   `failed`** (`recovery-routes.ts:87-96`), minting a new record and leaving the original
     >   untouched in both cases. `failed` is uncontroversially terminal, so "retry mints a new
     >   record" separates nothing.
     > - **The stranding worry applies identically to `suspended`.** A gate nobody answers sits
     >   `suspended` forever and reads `"active"` forever. S1b keeps `suspended` non-terminal, so
     >   stranding cannot be the test without taking `suspended` too.
     > - **The consequence is a false completion, not a stale one.** `interrupted` is the *output of
     >   crash reclamation* — the sweeper writes it when it finds an orphaned in-flight request
     >   (`packages/engine/src/execution/request-recovery.ts:60-70`). Classifying it terminal makes a
     >   Workstream **whose worker crashed and is awaiting continuation report that the work
     >   finished.** Reading `"active"` for a crashed worker is the staleness this spec already
     >   documents and accepts; reporting a terminal outcome asserts something untrue.
     > - **It is the `isTerminalRequestStatus` trap by another route.** That predicate counts
     >   `interrupted` as terminal for stream-termination purposes, which is correct there and wrong
     >   here — the warning immediately below applies unchanged.
     >
     > **Recommendation: non-terminal is `in_progress`, `suspended`, `interrupted`.** If the owner
     > wants a crashed-and-unattended Workstream to be visually distinct from a running one, that is
     > the deferred sub-status axis — the owner explicitly anticipated *"a state meaning a user
     > decision is needed"*, and an interrupted request awaiting continuation is exactly that. It is
     > additive later and costs nothing now; reclassifying to terminal is neither.
...
7. **Hop 2 must not hand a browser the raw item log. This spec makes that route the canonical
   ...
   **And the DevTool genuinely depends on the unfiltered log** — `include_items: true` in
   ...
   as the framework stands. **That conflict is escalated, not resolved here** — it needs either an
   operator-authorization tier, a separate non-public transport for the DevTool, or an accepted
   DevTool regression, and none of those is this spec's call. This spec states only what its own
   contract requires; the resolution is named as an open item below.
...
> **1b · How the DevTool keeps its trace access once hop 2 is client-filtered — escalated, owner's
> call.** Constraint 7 requires the browser drill-in to return client-visible items only, with no
> caller-supplied widening. The DevTool depends on the unfiltered log through that same route, and
> the framework has **no privileged-caller concept** to gate an exception on — `ResolvedPrincipal`
> is `{ userId, orgId? }`, with no roles or scopes
> (`packages/core/src/types/auth.ts:30-33`). So the two cannot both be satisfied as things stand.
> Three ways out, none of them this spec's: **(a)** add an operator/debug authorization tier and
> gate trace retrieval on it; **(b)** give the DevTool a separate non-public transport for the raw
> log; **(c)** accept that the DevTool loses cold-open trace history. This spec requires only its
> own contract and does not pick.
