...
> updating on its own has cost nine defects in one section of the build plan, all of them in the
> machinery that exists only to do that. The coordinator has escalated it as a direction question
> with three priced options — ship it, block on new engine work, or drop decision 4 so the panel is
...
**What was found.** `sendAction` resolves when the response *headers* arrive, not when the turn
finishes: on the default streaming path it returns `status: "in_progress"` immediately
(`useSession.ts`). So a discovery read fired there can run before the turn has created its child,
return nothing, and — with `items: false`, where the hook cancels the SSE body and never attaches —
leave no accelerator and no poll to find it later. Verified in the tree.

**Why this is escalated rather than fixed.** It is a **ninth** defect in §7, and every one of the
nine has been in the machinery that exists solely to make the panel update *on its own*. Nothing
else in this document has produced a defect. The flavours have kept changing — a signal with no
producer, a cost boundary, a delivery hole, and now a local fact that means something other than
what the design needs — which is what a design being asked to do something the platform does not
support looks like from the inside. **Decision 4 is the cost driver**, and how much it is worth is
a product question.
...
- *Honest read on the risk:* most of these are the kind real code makes obvious — `sendAction`'s
  timing and the `items: false` path surface in the first hour of writing the `renderHook` tests
  §10 now requires. The residual risk is a *design* hole shipping quietly, which is what the
  remove-the-accelerators tests in §10 exist to catch.
...
patching it, and after nine defects that is the evidence-based move. It does not block the demo,
...
- **Escalated to the owner (round 4, fifth pass)** — stopped fixing §7 and put decision 4 itself to
  the owner. The ninth defect showed `sendAction` resolving means *the request was accepted*, not
  *the turn ran*, so the new floor could read before the child existed — a local fact that is
  unsuppressible and still means the wrong thing. All nine defects have been in the machinery that
  exists only to make the panel self-update, so §12 now carries three priced options (ship it,
  block on an engine-side session channel, or drop decision 4) rather than a tenth fix. The fix for
  the finding is deliberately unapplied, because it differs by option.
