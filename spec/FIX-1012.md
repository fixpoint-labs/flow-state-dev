...
| Uses inline boards, never launches background work | One read on mount, plus **at most one per turn that ran a board**. The doorbell stays silent, so board traffic costs nothing |
| **Ran background work earlier; nothing outstanding now** | **The same as the row above** — the doorbell is gated on an *unfinished* row, not on a non-empty list, so finished work sitting on screen costs nothing forever after |
...
The middle two rows are the ones that matter, and they are the same app at different points in its
life: one that does *not* currently use this feature. BP-035 says that is the case to check, and
the third row is where this spec's first attempt at the gate failed — it billed such an app forever
after its first completed job. Neither can reach literally zero without a signal that distinguishes
a detached board from an inline one, and inventing that marker in this layer is what the package
boundary forbids. So the floor is one read per board-running turn, named here rather than hidden.
