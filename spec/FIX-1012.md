...
> **Decision 4 is half-settled, and the open half is a smaller question than it was.** Decision 4
> bundled two things: *what a row says*, which you have now answered, and *when the panel re-reads*,
> which is still open. Only the second is in front of you. It is the one that has carried every
> design defect this spec has taken (§12), and §12 puts it to you on its own with a recommendation.
> **Read decision 4's timing half as provisional; its status half and the other three decisions are
> settled.**
...
4. **A row says either how the work ended, or just `active` — and nothing finer.** *(Settled by you;
   ...
   **4b. Still open — how often the panel re-reads.** Whether it keeps updating on its own while the
   ...
   not answered; **§12 puts it to you with a recommendation.** Everything above holds either way.
...
**One predicate governs everything that costs anything: *is any row `active`?*** Since the owner
settled the status shape (§12), this is no longer a classification the hook performs over a
lifecycle — it is a literal field value on a row, and "active" is the only non-terminal value there
is. The accelerator
...
- **the per-turn read count is pinned, not assumed** (§3): an ordinary turn on a session with no
  background work performs **exactly two** reads with items enabled and **exactly one** with
  `items: false`. Pin the number rather than a bound — this figure was stated wrongly once, and a
  test asserting "at most a few" would not have caught it. It is also the test that will fail loudly
  if 4b is later answered the other way, which is the point
...
- **the status values are not enumerated in this package**: feed the hook a status value it has never
  seen and assert it renders and is treated as non-terminal without throwing. This is what keeps the
  deferred sub-status axis (§12) additive — a hook that pattern-matches the current values breaks on
  the day `waiting` is introduced, and no other test here would catch it
...
**What this dissolves.** *Which* run the row reports stops being a question — "is any run
non-terminal?" is an existence check over the set, not a selection out of it, so there is no
most-recent-attempt rule to define and no tie to break. S1b/FIX-1010 owns the route-side resolution
and is applying it now.

Four things this binds, and all four bind S3:
...
- **`active` carries no sub-state, deliberately.** Not *running*, not *waiting*, not *scheduled*, not
  *needs a decision from you*. The owner has explicitly anticipated that axis and explicitly deferred
  ...
  no `isActive` derived by listing the terminal ones, no placeholder field held open for the future
  axis. A new value must be renderable and treated as non-terminal without a change here (BP-030,
  and §10 pins it with a test).
...
**One consequence to settle on S1b, not here — where the terminal line falls.** `RequestStatus` has
...
**What was found — four defects in the discovery floor, all verified in the tree, none fixed here.**
...
4. **The generation guard does not order reads within one session.** A generation that advances on
   the read *source* (§7, as corrected this round) still leaves every concurrent read inside one
   session sharing it — a poll tick overlapping a doorbell or terminal refresh can land out of
   order, regress a terminal row to `active`, and restart polling, all while passing the guard.
   **Its size depends on the answer below**, which is why it is priced rather than fixed: keeping
   the panel self-updating needs a per-read ordering token across five triggers; dropping it leaves
   two reads that cannot overlap in normal use, which is ordinary single-flight.
...
- *Deleted:* the poll, the doorbell, `task-change` entirely, the accelerators, the active-row
  predicate, the coalescing, the per-turn discovery reads. §7 becomes two reads, and **the machinery
  the 18 findings lived in is gone.** It also deletes the two-reads-per-turn tax on apps that never
  use the feature (§3), and the abandoned-job-polls-forever exposure that the terminal line creates
  (§7) — neither of which exists when nothing polls.
...
- *`refresh()` is required work here, not an existing escape hatch.* Today `session.refresh()`
  ...
  it is a cost of C that the earlier text spent as if already paid.
...
per-turn reads under self-updating whether or not it uses background work. Under interaction-only,
the people who feel it are customers watching a long job, and whoever gives the demo.
