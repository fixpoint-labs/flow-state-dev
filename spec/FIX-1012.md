...
wherever it runs — it just never types.

**One caveat that is a real defect rather than a limit, found in review:** a job that finishes in the
narrow window *while the drill-in is mounting* currently ends up incomplete and stays that way. The
hook fetches the snapshot and then separately looks for an in-progress request to attach to
(`useSession.ts:924-928`, then `:946-949`); a completion between the two means it attaches nothing
and keeps a snapshot taken before the last items were written. §8 step 8 requires the catch-up, and
§9 carries the case — the example above is only honest once that is fixed. Worth stating correctly, because understating it here
...
**So discovery rests on a local fact instead: the hook dispatched the interaction itself.** That is
this hook's own method call, not something delivered over a connection — no dropped stream, board
setting or visibility option can remove it. **The hook already works this way** when it has no
stream: with items disabled it calls `refreshSnapshot()` directly after the action. The workstream
read follows the same precedent.

| | Producer | Removable by |
|---|---|---|
| **Floor** | a local interaction (below) · mount | nothing — both are local |
...
**Which interactions — and this is specified as a *test*, not as a list, because the list has now
been wrong twice.** It was `sendAction`; `performInlineReentry` was added to close a gap; it was
still incomplete, because `resumeLatestRequest` calls `recoveryClient.retry()` directly
(`packages/react/src/hooks/useSession.ts:1288-1321`) and bypasses both — so retrying a failed parent
after a child finished leaves the panel stale. Enumerating a fourth time and calling it complete is
the move that has already failed twice.

**No funnel exists, and I checked before concluding that.** The closest candidate is real but
disqualified: all three work-starting paths do converge on `attachToStream` — and on setting
`activeRequestIdRef.current` to a fresh id (`:1137`, `:1310`, `:1339`, the only three non-null
assignments in the file). But `attachToStream` is gated on `itemConfig.enabled` at every call site
(`:1184`, `:1205`, `:1346`, `:1359`), so a floor resting on it dies under `items: false` — the exact
configuration that broke this section three times. And the ref assignment is a write, not a hook
point, so nothing can observe it; requiring the read "wherever that assignment happens" is an
enumeration of three sites wearing a different hat.

**So the requirement is an exhaustiveness test that fails when a new path is added.** The hook
returns a fixed object (`useSession.ts:1424-1447`), so the test enumerates its **callable** members
at runtime and requires every one to be classified:

- **work-starting** — asserted to perform the discovery read (today: `sendAction`,
  `resumeLatestRequest`, `resumeSuspension`, `continueRequest`);
- **declared non-work-starting** — on an explicit allowlist, each with a stated reason (today:
  `refresh`, which *is* the read; `abortRequest`, `dismissRequest`, `subscribeAudioDelta`, and the
  pure selectors).

A callable in neither bucket fails the suite. **Adding a method to the view therefore breaks the
build until someone decides which bucket it is in** — which is the property the list never had.

**The residual, stated rather than implied.** This catches a *new public method*. It does **not**
catch work-starting behaviour added to a method already on the allowlist, and it does not catch
starts outside this hook instance (already named above). Those remain hand-maintained, and this
...
3. **The floor first, so the feature is correct before it is prompt: a read on every local
   interaction**, independent of the stream and of `items`. **Where in the interaction depends on
   4b, and an earlier version of this step encoded only the self-updating answer** — it required a
   read when `sendAction` *resolves*, which under C1 is an unpriced second read and under C2 is a
   read that must not exist at all.
   - *Self-updating:* on **resolution** — the accelerators cover the wait.
   - *C1:* at **action start**, which is what its promise pays for (§3). One read per interaction.
   - *C2:* **no step here.** Mount and `refresh()` (step 2b) are the only reads.
   *Test (all answers):* the **exhaustiveness test** from §7 — every callable on the returned view is
   classified work-starting or explicitly not, so a newly added interaction path fails the suite.
   Plus: with `items: false` and with the stream never connecting, an interaction that starts
   background work still discovers it. *Depends on:* 2.
...
8. **Close the drill-in's own catch-up race — a fix to existing behaviour, not new surface.** On
   mount the hook fetches the snapshot and *then*, separately, lists in-progress requests to decide
   whether to attach (`packages/react/src/hooks/useSession.ts:924-928`, then `:946-949`). If the
   job finishes **between** those two calls, the list returns nothing, `autoResume` attaches no
   stream, and the already-fetched snapshot predates the final items — so the drill-in sits
   incomplete until a manual refresh or a remount, contradicting §5's promise that it fills in as
   steps complete. **Required:** after a no-active-request result, re-read the snapshot (the simple
   fix), or attach regardless of terminal timing if the transport replays a finished request — the
   implementer's call, but one of them must happen. *Test:* resolve the completion between the two
   calls and assert the final items are present without any further interaction. *Depends on:* 2.
   Independent of 4b and of decision 2.
9. **Docs and README in the same change set** (§11), including the plain statement that background
   work shows finished steps rather than live text (decision 1).

**Removals: none, but not purely additive either — and an earlier version of this line overstated
that.** There is no superseded path to delete: the `latestRequest`-is-singular gap the epic once
recorded here dissolved with the Workstream model. But **step 8 changes existing `useSession`
behaviour** rather than adding beside it — it fixes a pre-existing mount race that S3 does not
introduce and cannot deliver §5's promise without. Worth naming, because "purely additive" is what
lets a reviewer skip the regression question, and here that question is live: the change is on the
path every existing consumer of `autoResume` already takes.
...
| **The user retries a failed parent after a child finished** (`resumeLatestRequest`) | **Discovered.** It calls `recoveryClient.retry()` directly (`useSession.ts:1288-1321`) and bypasses both `sendAction` and `performInlineReentry`, so a floor named after either misses it. This is the third public path found by enumeration, which is why §7 specifies a test over the returned view instead of a list |
| **The drill-in's job completes between the mount snapshot and the in-progress lookup** | **The final items still arrive.** Today they do not: the snapshot is fetched first (`useSession.ts:924-928`), the lookup runs after (`:946-949`), and a completion in between means no stream is attached *and* the snapshot is already stale — the drill-in stays incomplete until a manual refresh or remount, breaking §5's promise. §8 step 8 requires the catch-up |
...
- **every interaction path is classified, and a new one fails the suite** (§7): enumerate the
  callable members of the returned view at runtime and assert each is either work-starting (performs
  the discovery read) or on the declared non-work-starting allowlist. **This is the assertion that
  replaces a list which has been wrong twice** — writing four named per-path tests instead would pass
  today and stay silent on the fifth path. Under C2, where no interaction reads, the same
  enumeration asserts the inverse; the classification is required under every answer
- **the drill-in catches up when its job finishes mid-mount** (§8 step 8): resolve the completion
  between the snapshot fetch and the in-progress lookup, then assert the final items are present with
  no further interaction. Written as a race, not as a snapshot assertion — a test that completes the
  job before mount, or after both calls, passes against the broken code
...
- **After spec review (round 4, tenth pass)** — **stopped enumerating the interaction paths and
  specified a test instead.** The list was `sendAction`; `performInlineReentry` was added to close a
  gap; it was still incomplete, because `resumeLatestRequest` calls `recoveryClient.retry()` directly
  (`useSession.ts:1288-1321`) and bypasses both. A funnel was looked for first and does not exist:
  all three paths do converge on `attachToStream` and on the file's only three non-null
  `activeRequestIdRef` assignments (`:1137`, `:1310`, `:1339`), but every `attachToStream` call site
  is gated on `itemConfig.enabled` (`:1184`, `:1205`, `:1346`, `:1359`) — so a floor resting on it
  dies under `items: false`, the configuration that broke this section three times — and a ref write
  is not observable. §7 now requires an **exhaustiveness test** over the callable members of the
  returned view (`:1424-1447`): each is classified work-starting or explicitly not, so adding a
  method breaks the suite. The residual is stated rather than implied — it catches a new *public*
  path, not new behaviour inside an allowlisted one.

  **§8 still encoded the self-updating design**, requiring a read when `sendAction` *resolves* —
  an unpriced second read under C1 and a forbidden one under C2. Step 3 is now per-answer. That is
  the reverse of the conditional fault the standing check already named, so the check gained its
  second half: *unconditional text that should have gained a condition*, whose tell is a sentence
  naming one mechanism where the design now has several.

  **And a defect independent of both open questions:** if the drill-in's job completes between the
  mount snapshot fetch (`:924-928`) and the in-progress lookup (`:946-949`), nothing attaches and the
  snapshot already predates the final items, so the drill-in stays incomplete until a manual refresh
  — contradicting §5's promise. §8 step 8 requires a catch-up. It also made the "purely additive"
  claim false, since it changes existing `autoResume` behaviour on a path every current consumer
  takes; that line is corrected rather than left to imply no regression risk.
...
**And it runs in the other direction too, which the check missed on its first writing.** Text
written for one option can survive into the others without ever being marked conditional — §8's
"read when `sendAction` resolves" was a step for the self-updating design that, left alone, would
have added an unpriced read under C1 and a forbidden one under C2. So the check has two halves:
**a conditional that lost its condition**, and **unconditional text that should have gained one.**
The tell for the second is a sentence naming a specific mechanism where the design now has several.

**A corollary the enumerations earned:** when a claim of completeness has been wrong twice, stop
writing the third list. Either name the single point everything passes through, or require a test
that fails when something is added — and if neither is possible, say the list is hand-maintained and
will drift. §7's interaction paths went that way after `sendAction`, then `performInlineReentry`,
then `resumeLatestRequest`.)*
