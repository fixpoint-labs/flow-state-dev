...
> row shows — running, finished, failed. Review found that the two issues feeding this one do not
> currently supply that state: S1b excludes liveness by design and S2's row carries coordinates.
> So decisions 1, 3 and 4 are the right calls but are **not deliverable as the three specs stand**,
> and the fix belongs across S1b/S2 rather than here. §12 has both quotes and what S3 looks like
> either way. Sign off on the direction; the coordinator is putting the cross-spec fork to you
> separately.
...
**The stop condition depends on the §12 blocker.** "At least one row is unfinished" is computable
only if a row carries state. If the cross-spec fork resolves the other way, this half needs a
different bound — see §12; it is not S3's to settle.
...
**BLOCKER — a cross-spec conflict, confirmed in review, and NOT S3's to settle.** This spec assumed
a Workstream row carries a coarse run state. Checked against the sibling specs, **it does not**:

- **S1b (`spec/FIX-1010.md`)**, non-goals, verbatim: *"telling you which jobs are **running** rather
  than which exist (a separate liveness question, deliberately not this read)."* The read returns
  existence, by design.
- **S2 (`spec/FIX-1011.md`)** §7 types the row as *"everything a session summary already carries,
  plus the coordinates that make a Workstream meaningful to a UI — which body of work it is for,
  and which board dispatched it."* Coordinates, not state.
- **`SessionSummary` today** (`packages/client/src/types/index.ts`) carries id, flow, user, title,
  description, tags, metadata and timestamps. **No status.** `latestRequestId` is on the detail
  shape, not the summary.

As the three specs stand, **decisions 1, 3 and 4 are not renderable from one read.** Under each
resolution, S3 is:

- **(a) the row carries coarse state** — S3 is exactly as specced. Decisions 1, 3 and 4 hold, the
  poll's stop condition is computable, one read per refresh.
- **(b) the row stays existence-only** — S3 either issues **an extra read per row** on every refresh
  (an N+1 on the screen every conversation renders, and it is a *repeating* N+1 because of the
  poll, not a one-off on mount), **or drops decisions 1, 3 and 4** and becomes a list of names with
  no state, no failure visibility and no live updating — at which point the panel shows *that*
  background work exists and nothing about how it is going.

**Not decided here.** Which way this resolves is a cross-spec call and belongs to the owner with
S1b's and S2's scope in view, not to whichever of the three specs happens to be written last.
