...
|---|---|
| Never uses a task board | One read on mount. Nothing per turn, nothing per notice, no polling |
| Uses inline boards, never launches background work | One read on mount, plus **at most one per turn that ran a board**. The doorbell stays silent because the list is empty, so board traffic costs nothing |
| Actually launches background work | The above, plus coalesced refreshes while a turn is open, plus the poll — which stops when nothing is outstanding |

The middle row is the one that matters: it is the app that does *not* use this feature, and BP-035
says it is the one to check. It cannot reach zero without a signal that distinguishes a detached
board from an inline one, and inventing that marker in this layer is precisely what the package
boundary forbids — so the cost is bounded at one read per board-running turn and named here rather
than hidden.
...
- **The doorbell is ignored while the list is empty.** It may only refresh work already known to
  exist. Discovery is trigger 1 and trigger 3, both bounded.
...
And because discovery must stay bounded too, **trigger 3 fires at most once per turn, and only for
a turn that actually ran a board** — no `task-change` seen on that turn means no board ran, so no
detached work can have started in it. An app with no boards at all pays nothing per turn.
...
3. **The launching turn's terminal status — the guaranteed floor, once per turn.**
   ...
   conditional, this one is not). The workstream read joins it there, **gated on that turn having
   run a board at all** (per the rule above). Detached work is started *during* a turn, so this is
   a bootstrap a suppressed board cannot defeat, and it is what makes trigger 2 an accelerator
...
DISCOVERY — may find work not yet known. Bounded:
    mount                                      ← once
    this turn's terminal status                ← at most once per turn,
                                                 and only if the turn ran a board

ACCELERATION — refreshes work already known. Never discovers:
    a task-change item on this turn's stream
        ignored entirely while the list is empty      ← or every inline board
                                                        taxes an app with no
                                                        background work at all
        coalesced to one in-flight read, trailing edge

THE BACKSTOP:
    the poll, while any row is unfinished      ← decision 4; stops when none is
```
...
| **An app runs a busy inline board and has no background work at all** | **No reads from the doorbell.** The list is empty so notices are ignored, and the per-turn discovery read is the only cost. This is the second path (BP-035) and the row worth testing hardest |
...
| A board sets `changeVisibility: { client: false }` | The in-turn trigger goes silent. The terminal-status trigger and the poll still fire, so the panel is late rather than wrong — this is why the floor is not the doorbell |
...
- **the floor holds without the doorbell**: with `task-change` never firing, the panel still
  populates and still updates. This is the test that stops the optional trigger becoming load-bearing
...
- **After spec review (round 4, triaged not re-opened)** — the batch was **split by the bar**
  ...
  ordinary inline boards, which contradicted §3's own cost claim. The doorbell is now barred from
  discovering work (ignored while the list is empty) and coalesced, discovery is capped at one read
  per board-running turn, and §3 states the real per-app cost. Corrected one factual claim: the
  drill-in *does* receive completed items live cross-process — the live tail is store-driven and
  the in-process registry is gone (FIX-569), so only `content.delta` has the gap, which is exactly
  OQ-C and no wider. Three remaining items were recorded in §13 for the implementer.
