...
3. **The launching turn's terminal status — the guaranteed floor.** `onRequestStatus` already runs
   an unconditional `refreshLatestRequest()` on every terminal status
   (`packages/react/src/hooks/useSession.ts` — note the snapshot refresh beside it *is* conditional,
   this one is not). The workstream read joins it there. Detached work is started *during* a turn,
   so that turn's terminal event is a sound bootstrap that cannot be missed by a suppressed board,
   and it is what makes trigger 2 optional rather than load-bearing.
...
fired by, in increasing order of certainty:
    a task-change item on this turn's stream   ← in-turn, best-effort, doorbell only
    mount                                      ← covers arriving after the fact
    this turn's terminal status                ← the floor; cannot be suppressed
...
| A `task-change` item arrives for a board this session does not show | Harmless — it is a doorbell, so the worst case is one redundant re-read. It is never used to identify or classify a row |
