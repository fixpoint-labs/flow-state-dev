...
flowchart LR
  ...
  TC["task-change item<br/>accelerator, only while a row is unfinished"] --> RE
...
- **EXTEND** `docs/architecture/server-and-client.md` — **the one this plan was missing, and it is
  ...
  `refresh` (`:237-245`). This change adds an axis to `SessionView` and — under option 3 — changes
  what `refresh` covers, so both halves of that snippet go stale on the day this ships. An
...
> **Corrected in round 3, and the correction is the reason the round was opened. Option 3 does not
> deliver "current as of your last interaction." It delivers *current as of the **start** of your
> most recent interaction*** — which is one interaction behind. Reading at the start of an action
> means the work that action creates is not in that read, so **the panel structurally cannot show
> the job the user just launched.** It appears on their *next* action, on remount, or on a manual
> refresh. Two earlier bullets asserted otherwise and are rewritten below rather than softened.

- *Survives:* decisions 2 and 3 untouched; the list, the status, the drill-in, the staleness marker.
...
- *Deleted:* the poll, the doorbell, `task-change` entirely, the accelerators, the unfinished-row
  predicate, the coalescing, the per-turn discovery read. §7 becomes two reads, and **the machinery
  most of the twelve defects lived in is gone.** (The exact sub-count is deliberately not
  re-derived here — the previous "six of nine" was carried from an older total and asserting a new
  precise figure would repeat that mistake.)
...
- *Decision 1 must be reworded* — "fills in step by step" becomes **"is current as of your previous
  interaction"**, not "your last interaction". The distinction is the whole finding: the first
  background job a conversation launches is invisible until the user does something else.
...
- *`refresh()` is required work here, not an existing escape hatch.* Today `session.refresh()`
  ...
  so it does not refresh this axis and wiring it to a button would do nothing. **Under option 3 it
  stops being optional**: it is the only way a user sees the job they just launched without sending
  another message, so extending and testing it moves into §8 and §10 as scope. Priced here because
  it is a cost of option 3 that the earlier text spent as if already paid.
- *Reversibility:* adding self-updating later is additive and breaks nothing — and if option 2 ever
  lands, it drops in cleanly.

**My recommendation — still option 3, but at a higher price than it was quoted at, and the gap has
narrowed.** Recorded as *mine* rather than as the coordinator's, who is putting a separate view
alongside it. Three reasons stand. It deletes the defect class rather than patching it, and after
twelve defects that is the evidence-based move. It does not block the epic's wrap gate, which
option 2 cannot say. And it is the reversible direction: we can add updating once the substrate
makes it cheap, whereas the owner cannot easily un-ship a mechanism that is quietly wrong.

**What round 3 changed about this recommendation, stated rather than absorbed.** Option 3 was sold
as "a panel with a lag." It is really "a panel that is one interaction behind, and never shows the
job you just started." That is a different product, and it is closer to option 1's pitch than the
earlier text implied — option 1 buys the moment where you ask for a long job and watch it appear.
The offset is recoverable by the app (call the extended `refresh()` after dispatch), but that is
the app's work and the framework declining to do it.

**What would change my mind** — and it is now a lower bar than it was. If the thing being sold is
"ask for something long, and see it appear," option 3 fails that beat without app-side wiring, and
option 1's risk becomes the trade worth taking. Previously I judged that gap to be about *watching
work progress*; the correction shows it is also about *seeing work start*, which is a more common
expectation and a harder one to explain away.
...
*(Standing check on the examples, added after the sixth example defect on this spec: **every code
example is verified by asking what each call actually returns and in what order it evaluates**, not
by reading it for plausibility. Five of the six were argument-shape or evaluation-order faults —
a value read before it exists, validation running before the guard that would have skipped it — and
none of them is visible to a reader checking that the example "looks right". The pseudocode sketch
in §7 is exempt by the spec-PR contract; it is not code and is not reviewed at the line level.)*
