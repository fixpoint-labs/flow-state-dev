...
2. **This renderer holds no level-label vocabulary.** It reads FIX-780's shared rule —
   ...
   **Round-2 correction — it must not *pass through* a stored level label either.** Holding no
   ...
   and guessing is what FIX-780's legacy reason exists to disclose instead. §7 has the mechanism
   and where the predicate lives.

   **Round-3 widening — the filter is a DENYLIST of what the structured line already renders,
   never an allowlist of what may pass.** Levels are not the only duplicated key: `direction`
   and `size` are in the metrics bag too, and the trade one-liner renders those same values
   from the *typed* fields. Nothing enforces that the free-form string and the typed field
...
4. **Absent stays absent, and empty renders nothing — by IMPORTING the existing behaviour,
   ...
   **Round-3 addition — the `—` placeholder also arrives through the DATA, and it is the same
   ...
   epic's subject in miniature. So the risk cards drop sentinel-valued entries and keep the
...
**The rule, therefore — and it is BOTH cards, not just the trader's:** a card owns its metrics
row and **never hands a raw stored bag to `ThesisMetrics`**. It filters through a pure helper
(decision 5 — these are rules with honesty consequences, so they must be reachable from the node
harness). Two filters, one on each card:

| Card | Drops | Keeps | Why |
|---|---|---|---|
| Trader | The level keys, plus `direction` / `size` / `holding period` | `conviction`, and **any key it does not recognize** | The structured trade line already renders all of them from the typed fields, and nothing enforces that the free-form string agrees with the typed value — two copies on one card can contradict each other |
| Risk (all four) | Entries whose value is the `—` sentinel | Every populated entry | The persona schema requires all six keys, so a persona spells the ones it has no read on as `—` (decision 4). Rendering them asserts a slot for a measurement nobody took |

**The trader filter is a denylist, and that is load-bearing** (decision 2, round 3). It removes
what the structured line already draws and lets everything else through, so a metric a later
schema adds still reaches the screen. Inverting it into an allowlist of today's keys would make
this fix reproduce the defect it fixes. The risk filter is value-based rather than key-based for
the same reason: it drops non-measurements, not unrecognized names.
...
**The non-label legs come from the same place as the labels — and that is why the whole trade
line waits for FIX-780 (round 3).** Direction, size, and holding period are formatted by
`tradeLineParts`. FIX-780 makes it an **exported** function taking `(trade, levels)`; **on the
reviewed parent it is neither.** It is `function tradeLineParts(trade)` — module-private, single
-argument, and still pushing the old `stop` / `target` label strings itself (verified; claims
table). So an implementer building the legs before FIX-780 merges has only two moves, and both
are wrong: re-implement the formatter, which forks the rule §7 exists to converge, or import a
symbol that is not exported yet, which does not compile.

**The legs therefore belong to step 4, not step 3.** They are not a separable "non-level" part
of the card — they are the same one-liner as the labels, produced by the same function, and that
function only becomes reusable when FIX-780 lands. Step 3 keeps everything genuinely independent
of FIX-780 (the invalidation criteria, depends-on, the header treatment, the jump control) and
stays unblocked. Per the EM ruling `tradeLineParts` is moving into `trade-levels.ts` alongside
the label rule — follow it there when it lands. If the labels converge and the legs are forked,
...
| Trade one-liner legs | `tradeLineParts` — **module-private on the parent; exported only by FIX-780** | Import **in step 4**, never re-implement; follow it into `trade-levels.ts` |
...
3. **Add `TRADER_AGENTS` + the trader arm + `TraderProposalCard`** — **without the trade line
   and without the metrics row.** What lands here is only what is genuinely independent of
   FIX-780: the invalidation criteria, depends-on, the header treatment, and the jump control.
   *Test:* §10's trader cases that do not involve levels. *Depends on:* nothing.
4. **Once FIX-780 is on `main`: add the whole trade line and the metrics row together** —
   levels through `buildTradeLevelModel`, **the legs through `tradeLineParts`** (round 3: it is
   module-private and single-argument until FIX-780 exports it, so the legs cannot be built in
   step 3 without forking the rule), and the stored `metrics` bag through the trader denylist
   (§7) before it reaches `ThesisMetrics`; add the provenance-notice call site, with the
   suppression fallback if the shared marker did not survive. *Test:* a flat, a directional, and
   a **legacy** record each render the labels the rule returns and no others — asserted on the
   **complete card**, so a stale metrics grid beside them fails; plus the marker-absent case
   rendering no level rows, and the duplicate-legs case. *Depends on:* 3 **and on FIX-780 having
   merged** (§12).
5. **Docs** — §11. *Depends on:* 4.

Steps 2 and 3 are independent. **Step 4 is the only one gated on another issue, and it carries
every trade-line surface deliberately** — the labels, the legs, the metrics bag, and the
disclosure. Splitting any of them out is what would let a half-migrated card ship the
contradiction: the filter without the merge still shows the schema's own `stop`, the merge
without the filter still shows a legacy record's, and the legs without the merge fork the
formatter. **Step 3 got smaller in round 3 and that is the right trade** — a genuinely
unblocked step that ships nothing needing FIX-780 beats a larger one that quietly needs it.
...
| A trader card rendered before FIX-780 merges | No trade line at all — labels **and** legs — and no metrics row. Never one without the other (§8 steps 3–4) |
...
| `tradeLineParts` is exported and takes the level model — **only on FIX-780's branch** | `origin/fix/FIX-780` · `components/summary/decision-header.tsx` | `export function tradeLineParts(trade, levels)`; still in `decision-header.tsx`, not yet moved to `trade-levels.ts` |
| **On the reviewed parent `tradeLineParts` is module-private and single-argument — this is why the legs cannot ship in step 3** | `origin/main` · `components/summary/decision-header.tsx:226` | `function tradeLineParts(trade: NonNullable<TradeLevels>): ReadonlyArray<string>` — no `export`, no `levels` argument, and it pushes the `stop` / `target` label strings itself. **Round-3 P2, ruled directional:** step 3 as originally written could only be built by forking the formatter or importing a symbol that does not exist |
