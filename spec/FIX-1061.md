...
- **Forward — the unmerged schema.** On `origin/main` the trader's `metrics` object still
  ...
  the commit derives level entries through `tradeLevelMetricEntries`. **Do not render the
  trader card's metrics row until that schema change is on `main`** (§8 step 4).
...
2. **The metrics row** — needs the schema change **and a filter, because the schema change is
   forward-looking**. On `origin/main`, `tradeProposalOutputSchema.metrics` is
   `{ direction, size, stop, target, conviction }`; on `origin/fix/FIX-780` it is
   `{ direction, size, conviction }` with level entries derived at commit (both verified).
   `ThesisMetrics` renders whatever keys it receives, so a trader card shipped against `main`
   shows a stale `stop`/`target` grid next to correctly-labeled monitoring rows — the FIX-780
   defect, reintroduced by the renderer built to fix the surrounding one. **Round 2 found the
   other half:** waiting for the merge fixes only records written *after* it.
   `memoStateSchema.metrics` is an unrestricted `z.record(z.string(), z.string())`
   (`resources.ts:176`) and every memo persisted before FIX-780 keeps its `stop` / `target`
   entries permanently, so a reopened legacy report shows the obsolete grid no matter when this
   card ships. The card must filter (decision 2, §7). The dependency stands; it just was never
   sufficient.
