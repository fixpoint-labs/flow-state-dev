...
**The metrics row is blocked on FIX-780 reaching `main`, and that is a separate hold from the
level line.** On `origin/main` the trader's `metrics` object still carries model-written `stop`
and `target` keys, and `ThesisMetrics` renders whatever keys it is given, verbatim. So a trader
card built before FIX-780 merges would show a stale `stop / target` grid *beside* the new
reassess/invalidate rows — two contradictory readings of the same flat position on one card,
which is the exact defect FIX-780 exists to remove, reintroduced by this issue. Holding only
the level line does not prevent this, because the grid is a different render path. On
`origin/fix/FIX-780` the schema drops both keys and the commit derives level entries through
`tradeLevelMetricEntries`. **Do not render the trader card's metrics row until that schema
change is on `main`.**
...
4. **Once FIX-780 is on `main`: add the level line and the metrics row together**, routing
   levels through `buildTradeLevelModel` and the legs through `tradeLineParts`, and add the
   provenance-notice call site. *Test:* a flat, a directional, and a legacy record each render
   the labels the rule returns and no others — asserted on the **complete card**, so a stale
   metrics grid beside them fails. *Depends on:* 3 **and on FIX-780 having merged** (§12).
5. **Docs** — §11. *Depends on:* 4.

Steps 2 and 3 are independent. **Step 4 is the only one gated on another issue, and it carries
both level-bearing surfaces deliberately** — splitting the metrics row out is what would let a
half-migrated card ship the contradiction.
...
- **Field coverage as a property, not a list — walked to the leaves.** A test walks
  ...
  - The **exclusion set stays small, and every entry carries a stated reason naming the field
    ...
    `metrics` / `citations` are excluded because the shared header and body draw them. An
...
3. **on the trader card, that no `stop` / `target` metrics grid appears beside the level
   rows** — the step-4 gate, checked on the rendered card rather than on the view model.
...
**FIX-780 blocks step 4, and it blocks TWO surfaces on that step, not one.** The epic's §4
...
ships `flows/analysis/lib/trade-levels.ts` with `buildTradeLevelModel`, `levelsForStance`,
`tradeLevelMetricEntries`, `LEGACY_LEVELS_CAPTION`, and
`PRE_FLAT_STANCE_LABELING_FIX_REASON`. The contract is real code now, so §7 names the symbols
instead of describing them. It is still **unmerged**, which is what step 4 waits on.

The two blocked surfaces:

1. **The level line** — needs `buildTradeLevelModel`.
2. **The metrics row** — needs the schema change. On `origin/main`,
   `tradeProposalOutputSchema.metrics` is `{ direction, size, stop, target, conviction }`; on
   `origin/fix/FIX-780` it is `{ direction, size, conviction }` with level entries derived at
   commit (both verified). `ThesisMetrics` renders whatever keys it receives, so a trader card
   shipped against `main` shows a stale `stop`/`target` grid next to correctly-labeled
   monitoring rows — the FIX-780 defect, reintroduced by the renderer built to fix the
   surrounding one.
...
| `ThesisMetrics` renders whatever keys it is handed | `components/theses/thesis-metrics.tsx` | `Object.entries(metrics).map(...)` — no key filter, so a stale `stop` key renders |
