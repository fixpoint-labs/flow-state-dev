...
is derived rather than filed on most paths; the other seven metrics gain the typed default:
...
flowchart TB
  ...
  subgraph compute[3 · compute + label]
    DV[lib/valuation.ts · computeValuation<br/>effective-rate ROIC · derived EBIT · growth-adjusted P/E<br/>REQUIRED basis on the 7 variable metrics<br/>typed reported-components default on the other 7<br/>− both band tables + both verdict helpers]
  end
...
10. **Carry the figures onto the valuation spine and render them in `formatValuationSpine`**
    ...
    *Test:* the §10 spine behaviours including the legacy read, and a formatter unit test
    ...
    cohort, produces byte-identical output**, and that the **four overlapping metric lines**
    — the ones the spine persists — match between the analyst's block and the spine's block.
    **Do not assert the two whole blocks are identical:** the analyst renders all fourteen
    metrics and the spine renders the four it persists (decision 5's narrowing), so
    whole-block identity is a criterion no correct implementation can satisfy (§10 states the
    same rule).
    *Depends on:* 6–9.
...
- **what must not change — assert *values*, and do not assert byte-identity on any metric
  ...
  working rather than a regression: the seven variable metrics gain a computed basis, and the
  other seven gain the typed `reported-components` default, which today's formatter renders
  for none of them. A byte-identical assertion therefore fails on a correct implementation
  **for all fourteen**, not merely for the FCF trio. So: **values over all fourteen**, and if a
