...
7. **A derived figure uses components from the statement's own current fiscal year, and
   never reaches back for a missing one.** The anchor is the period the statement already
   claims to describe. Every newly-read component — tax, pre-tax income, interest, and the
   operating income the fallbacks report — is read at that year; anything absent there is
   unobserved, and the figure takes its labelled fallback or abstains. *(Added in review
   round 1, widened to operating income and pinned to the current year across rounds 2–3; it
   ...
   *Rejected:* pairing each field's latest reported value, which is what the statement
   ...
   fallback this spec promises. *Also rejected:* anchoring on "the latest year that has a
   pre-tax income", which was this spec's own round-1 answer. It reaches back to an older
   year to find a rate and then applies it to that year's operating income against a
   **current** balance sheet — the invested capital in return on capital is a present-day
   instant, and no amount of income-statement alignment fixes that. It is the defect wearing
   the fix's clothes. *Also rejected:* anchoring the balance-sheet components too, which
   would pull two more mappers into an income-statement change for a case the rule above
   already answers honestly.
   *Locks in:* fewer derived figures than a naive reading would produce, on real large-cap
   names. A company that has not yet reported this year's tax provision keeps the labelled
   21% assumption rather than borrowing last year's rate — the claim being made is the
   provision rate *for the period being measured*, and we do not have one. The published
   operating income is untouched; the anchored value travels beside it (§7).
...
> 2 deliberately left alone and documented as such · ~22 test behaviours · 3 documentation
> surfaces · 2 PRs.
...
- **The two provider mappers read them period-aligned, and this is the load-bearing part.**
  ...
  new fields must be read at one anchor period, and **the anchor is the statement's own
  current fiscal year** — the period the payload already claims to describe — not "the latest
  year that happens to have a pre-tax income". Every anchored component is read at that
  period end and is `null` where absent there; nothing reaches back a year to find a value.

  **Why the anchor is the current year and not the latest available pre-tax fact** (round 3):
  return on capital divides by invested capital, which `computeValuation` builds from
  `totalDebt + totalEquity − cash` — balance-sheet **instants**, selected independently and
  effectively current. An anchor that reached back to an older pre-tax fact would apply that
  year's provision rate to that year's operating income against a present-day balance sheet,
  which is the same cross-period defect one field over. Pinning the anchor to the current
  statement year keeps the income components and the balance-sheet instants describing the
  same period, and where the current year has no reported pre-tax income the honest answer is
  the labelled 21% assumption, not last year's rate.
...
1. **Widen the income-statement schema** with the six anchored fields, each
   ...
   *Depends on:* nothing.
2. **Read them period-aligned in the filings mapper.** Anchor on the statement's current
   fiscal year; read tax, pre-tax income, interest **and operating income** at that year
...
test or goal check that reads them — saying that these two intentionally omit the tax,
pre-tax, interest and anchor keys, that this is what exercises the missing-key path, and that
filling them in removes the coverage silently. *Depends on:* 1.
...
| **The current statement year reports no pre-tax income, but an earlier year does** | The labelled 21% assumption is used. We do **not** reach back for the older rate: applying it to this year's operating income and a current balance sheet would be a cross-period figure claiming to be one period's (decision 7) |
...
- The legacy fixtures produce a spine in which **return on capital and EV/EBIT carry fallback
  bases, and the two growth-adjusted metrics carry abstention reasons** — not bases, because
  with no earnings-growth input they have no value, and decision 4 gives a valueless metric a
  reason instead. *Fails when:* the new fields' missing keys throw on parse rather than
  defaulting to null.
  *(Round 3 corrected this condition: it previously demanded fallback bases on all four, which
  a correct implementation of decisions 2 and 4 could never satisfy. That is the mirror image
  of the defect fixed three times above — those criteria could not **fail** for their stated
  reason; this one could not **pass** for its. Both are the same fault, a criterion whose
  stated meaning and actual behaviour differ, so check §10 in both directions.)*
...
missing. §10's fourth pass condition is what reads it. **The intent has to be written into the
repository, not left here** — §8's PR-a deliverable covers it, because this spec is never
merged and a fixture that looks incomplete for no stated reason gets "fixed". A one-time `dataSource: "record"` run
