...
- **Where this spec's defects actually came from, and what to do differently on the next one.**
  ...
  **Size is not the cause, but it is the price.** A shorter spec restates a decision twice
  ...
  not. That is an argument for fewer restatements, not for a smaller spec or a split — and it
  was tested: the split was proposed on this evidence and rejected, because the seam that made
  the defects go away also shipped a fail-open interval (decision 5 records it).
