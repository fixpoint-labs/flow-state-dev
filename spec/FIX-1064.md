...
5. **These four figures are put on the shared evidence surface every downstream reader sees,
   ...
   *Locks in:* two call sites remain, kept honest by one shared classifier and one shared
   ...
   add the same block. **Also locks in a visible change to old reports:** the cohort guard
   (§7) withholds enterprise-value multiples from banks and insurers, and it **fails closed**,
   so a report stored before this change — which carries no cohort discriminator — reads as
   `unknown` and **stops showing enterprise value, the three EV multiples, net debt and net
   leverage**. That is intended and is the price of failing closed; it is the same
   forward-looking position decision 1 already takes, where re-running an old report can
   disagree with the stored one. A reader opening an archived report will see fewer figures
...
> 2 deliberately left alone and documented as such · ~24 test behaviours · 3 documentation
...
- **The shared surface is the valuation spine, and there are two computation points that
  ...
  **BP-030, stated rather than left to fall out:** a report stored before this change carries
  no `sectorCohort`. It dual-reads as **`unknown`**, which fails closed, so **legacy reports
  stop showing the capital-structure family**. That is a visible change to what old reports
  display, and it is consistent with decision 1's already-approved position that the guarantee
  is forward-looking and older reports will disagree with newer ones. Decision 5 states the
  consequence.
...
| **A report stored before this change** | Its spine carries no `sectorCohort`, dual-reads as `unknown`, and therefore **stops showing the capital-structure family**. Intended, and the visible cost of failing closed (§6 decision 5, BP-030) |
...
**The failure class this epic keeps producing — worth naming here, because it is the third
instance tonight.** In each case the answer was *carry the three-state instead of collapsing
to two*, and in each case the collapse happened early, cheaply, and looked like a
simplification:
...
| **This issue** | `isFinancialSector(sector)` at the spine | "unknown sector" and "known non-financial" become one value, and a later guard that needs the distinction cannot be written where it was placed |
