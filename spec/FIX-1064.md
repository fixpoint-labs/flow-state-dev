...
(§7). The run does get marginally more expensive, in two places rather than one, stated
rather than glossed. Five new fields join the fundamentals analyst's data block, because in
this app widening a tool payload schema is a prompt change (§8). And decision 5 adds four
metric lines plus their basis text to the shared valuation formatter, which the desk injects
into five agents — the research manager, the scenario forecaster, the risk consolidator, the
trader and the portfolio manager — and into every seated lens, four of them today. So a
full-preset run carries those lines in nine prompts, not one. Still a handful of numbers per
prompt and no new call — but it is not nothing, an earlier draft claimed "no extra cost per
run" flatly, and the correction that replaced it fixed the size and left the reach wrong.
...
4. **A figure whose computation can vary must declare which basis it ran on, and cannot be
   ...
   `priceToFcf`), because **free cash flow is not a filed figure on any path** — no `us-gaap`
   concept defines it, so every producer either computes it or passes through a number whose
   basis it cannot attest (§7). Each carries a required basis descriptor
   ...
   *(Round 11 moved the three FCF ratios from the defaulted set to the required set. Round 16
   went one step further, on the same rule: the basis label describes what **the desk** did,
   not what the filer did, so the FCF provenance vocabulary lost its `reported` state
   altogether — no producer can truthfully emit it. See §7.)*
...
5. **These four figures are put on the shared evidence surface every downstream reader sees,
   ...
   *Locks in:* two call sites remain, kept honest by one shared classifier and one shared
   ...
   resolved — **or resolved to a label the desk does not recognise** — loses them too. **Published reports are not affected** — Past Reports re-renders
...
is never a filed figure — the desk either computed it or was handed it (§7); the remaining
metrics gain the typed default:

```
before  FCF yield: 3.10%
after   FCF yield: 3.10% (basis: derived — operating cash flow − capital expenditure)
        …or, where a provider handed us the figure, or the record predates this change
...
That second free-cash-flow line is the state worth looking at: the figure is real and usable,
and what we do not know is how it was made. Saying so is the point — the alternatives are
claiming a basis we cannot attest (a false label) or suppressing a good number.
...
flowchart TB
  ...
  subgraph carry[2 · carry]
    ...
    CF[cashflow payload<br/>+ FCF provenance: derived / null = basis not recorded<br/>.nullable.default null]
  end
...
- **The metric type gains a required basis descriptor on the *seven* variable metrics**,
  ...
  **Seven, not four, because free cash flow is never a *filed* figure — and a blanket
  `reported-components` default would make three metrics lie.**

  **Free cash flow is not a GAAP line item.** No `us-gaap` XBRL concept defines it, so no
  structured provider can establish that the *filer* reported it. A provider that ships an FCF
  series is shipping its own arithmetic over the operating-cash-flow and capex lines, under a
  definition it does not publish. That is the load-bearing fact here: **series presence is not
  provenance.** This spec already conceded it for one path — `mapEdgarCompanyFacts` always
  derives — without noticing that it generalises to every path.

  Walked in all four producers:

  | producer | what it does with FCF | what the desk can truthfully say |
  | --- | --- | --- |
  | `mapEdgarCompanyFacts` (`edgar-companyfacts.ts:160`) | **always** computes `operating − capex`, on the *primary* provider for US filers | **derived** |
  | `mapYahooTimeseries` (`yahoo-timeseries.ts:144`) | assigns the `annualFreeCashFlow` series through unchanged; computes `op + capex` only when the series is absent | series branch: **basis not recorded**; fallback branch: **derived** |
  | `candidateFreeCashFlowRaw` (`lib/financial-candidate.ts:69`) | its stated-FCF branch is **unreachable**: the only constructor of a `FinancialCandidate` hardcodes `freeCashFlow: null` (`recover-financials-extract.ts:216`), and the extraction prompt instructs the model *"Do NOT report free cash flow — it is derived downstream"* | **derived**, always |
  | the empty payload | no value at all | unobserved — an abstention reason, not a basis |

  **So the vocabulary is two states, not three: `derived`, or `null` meaning the basis is not
  recorded. There is no `reported`.** Round 11 introduced that state on the assumption that a
  provider series meant the filer had said so. It does not, and no producer left in the tree
  can truthfully emit it. A state no producer can reach is not harmless: it invites the reader
  to believe the desk distinguishes something it cannot. The extractor's own header already
  knew the reason — *"FCF is a non-GAAP figure rarely printed as a line"* — which is why that
  path stopped transcribing it and derives instead.

  *The Yahoo series branch specifically, because it is the branch that changed.* The mapper
  does not recompute; it assigns the series value directly. Labelling that `derived` would put
  "operating cash flow minus capital expenditure" on a number the desk never computed — a
  false *definition* claim traded for a false *provenance* claim, which is decision 4's own
  defect wearing the other hat. It happens to reconcile to the dollar on the checked-in AAPL
  fixture (2024-09-30: 118.254 − 9.447 = 108.807; 2025-09-30: 111.482 − 12.715 = 98.767), but
  one ticker agreeing is not a definition, and the label is a claim about every ticker.
  *Rejected: make the mapper recompute `op + capex` and ignore the series, so `derived` becomes
  true.* That changes the published **number** wherever Yahoo's definition and ours disagree,
  not merely the label — a behaviour change outside decision 4's remit, arriving at round 16.
  *Reopen if:* the desk ever wants one FCF definition across all providers, which is its own
  issue with its own before/after.

  All four collapse into one `cashflow.freeCashFlow` field, read once in `computeValuation`
  ...
  cashflow payload carries an FCF provenance discriminator** — `derived`, or `null` where the
  producer could not record a basis — set by each producer, which the three metrics read for
  their basis.
  ...
  *Why a discriminator rather than a softer default:* by the time `computeValuation` sees a
  ...
  And a default asserting a basis over a figure whose basis is unknown is **worse than no
  label**: it turns an unknown into a false claim, which is decision 4's own defect appearing
  inside decision 4.
  ...
  **A stored `null` is two consumer states, not one, and they need opposite behaviour.**
  ...
  | --- | --- | --- |
  | value present, discriminator `derived` | as recorded | render, with the derived basis |
  ...
  The third row has two populations, and they get the same treatment because the desk owes the
  reader the same sentence for both: every checked-in cashflow fixture and every
  `financialsData` record written before this change, **and every live Yahoo payload that used
  the `annualFreeCashFlow` series**. In both, a real and usable FCF figure arrives whose
  derivation the desk cannot state. Collapsing it into *unobserved* would suppress figures that
  are fine — and would do so on fixture replay, the default development and CI path, not on
  some rare edge. Labelling it `derived` invents a definition. So the state is named and
  rendered, and decision 4's invariant holds as written: **every metric with a value carries a
  basis**, and for these records the basis is that we do not know.

  **The Yahoo series branch reuses the stored `null` rather than earning a value of its own.**
  Not only because a third stored value costs a vocabulary change: `null` already means exactly
  *"no basis is recorded for this figure"*, which is true of that branch on its own terms. A
  distinct value would encode "a provider handed us a number without saying how" against "this
  record predates the field" — a distinction no reader acts on, and inventing vocabulary for an
  unactionable distinction is the instinct decision 4 exists to check. Nothing mechanical
  argues the other way either: **the two `null` consumer states are told apart by whether
  `freeCashFlow` itself is present**, never by discriminator-absent-versus-null, which
  `.nullable().default(null)` makes indistinguishable after parse anyway. A legacy record
  dual-reads correctly with the field absent (BP-030).

  **The consequence, stated because it is a real cost:** *basis unrecorded* is no longer a
  transitional state that self-clears as payloads are re-recorded. It is permanent on the
  Yahoo series path, and it is the honest permanent answer there. *Backfilling was considered
  and rejected:* the basis is not recoverable from a stored payload —
  `freeCashFlow === operating − |capex|` holds both when a producer derived it and when a
  provider's own definition happened to agree — so a migration would guess exactly the label
  this discriminator exists to stop us guessing.
...
- **The shared surface is the valuation spine, and there are two computation points that
  ...
  *recognised* and is not a financial.** Absent, unresolved, **or present but outside the
  vocabulary the desk classifies against** withholds it, with a reason saying the cohort could
  not be established. "Known" here means matched, not non-null — that distinction is the whole
  guard, and getting it wrong is what round 18 found (below). The two errors are not symmetric —
  ...
  **So the spine persists the three-state, and the formatter reads it.** `compute-spine.ts`
  ...
  - **Add a three-valued `sectorCohort: "financial" | "non-financial" | "unknown"` to the
    ...
    receives, through **one new exported classifier** — *not* through `isFinancialSector`
    plus a null check. **That composition is the row-3 defect one level in, and round 18
    caught it here after it had been written into this spec for eight rounds.**
    `isFinancialSector` matches five exact lower-cased strings and returns `false` for
    everything else, so `financial ? … : "non-financial"` labels **any non-null string the
    desk does not recognise** as a known non-financial — and the null check cannot see it,
    because the string is not null. The fail-closed guard is then bypassed by exactly the
    input it exists for: a company whose sector arrived, but arrived in a vocabulary the
    predicate was not built against.

    **The rule: an unrecognised non-null string is `unknown`, not `non-financial`.** Three
    states, and the fail-closed guard hangs off `unknown` exactly as it hangs off null.

    **Widening the alias list is rejected as the fix.** Any list of financial-sector strings
    is a list a real value can fall outside of, and the next unrecognised string fails the
    same way and just as silently. That is chasing instances of a shape. The list is
    *already* a guess: three of its five entries (`financials`, `banks`, `financial`) match
    nothing any current provider emits — evidence that it was written by predicting strings
    rather than by reading a vocabulary.

    **So classify against vocabularies, and let "matched neither" be the answer.** The desk
    already declares its sector vocabulary — `GICS_TO_ETF` in `lib/sector-resolution.ts`, the
    eleven standard GICS sectors in Yahoo's naming, which is the naming `sector` actually
    arrives in (verified table below). `financial` is a match in the financial set;
    `non-financial` is a match in the recognised **non-financial** vocabulary; anything else,
    null or not, is `unknown`. **This is a precedent in this codebase, not a new idea:**
    `mapSectorLabel` (`lib/providers/etf-profile.ts`) maps Alpha Vantage's sector labels onto
    that same vocabulary and, per FIX-801's decision 7, returns an unmapped label as its own
    explicit `Other: …` bucket rather than folding it into a neighbour. Same question, same
    answer, one module over — an unrecognised string stays unrecognised.
  - **`isFinancialSector` is kept, as a derived predicate** (`classify(sector) === "financial"`),
    and its two existing consumers are **not** touched. `computeFairValue` and `dcf.ts` abstain
    for a *known* financial and proceed on `unknown`, exactly as they do today. Widening them to
    abstain on `unknown` too would withdraw fair value and the DCF from every company whose
    sector did not resolve, which moves `setupScore`, the triangulation and the rating envelope
    — a decision-level consequence, and the owner approved decision 1 without it. **That is the
    same call, and the same reason, as ROIC's** (FIX-1118 above): the boolean shape of those two
    guards is a constraint on this change, so the classifier goes *beneath* the predicate rather
    than replacing it.
  - **`valuationMethod` is left alone, and `unknown` takes its `ev-multiples` branch —
    deliberately, and it stops being readable as a sector verdict.** Making *it* three-valued
    was the alternative and is rejected: it is a persisted enum with a rendered line and seven
    test/helper sites bound to its two values, so widening it changes stored data and existing
    assertions to carry a distinction none of those consumers asked for. A new field is
    additive and dual-reads cleanly. **What the field actually records is which method the run
    ran** — and on an unknown sector the run genuinely ran the non-financial path, because
    `computeFairValue` and `dcf.ts` are unchanged (above). Mapping `unknown` to
    `equity-multiples` instead would put the spine in contradiction with itself: `fairValue.method`
    would read `justified-pe` while `valuationMethod` claimed the equity-multiples cohort — the
    two-surfaces-disagree defect this decision exists to close, manufactured inside the fix for it.
    **The correction is that no reader may convert that label into a cohort claim.** The
    capital-structure guard reads `sectorCohort` and never `valuationMethod`, and the formatter's
    method line renders the cohort alongside it, so an `unknown` run reads
    `Valuation method: ev-multiples (sector cohort unresolved — capital-structure metrics
    withheld)` rather than an unqualified assertion that this company is not a financial. One
    field answers "what did we run", the other answers "what is this company", and after this
    change only the second one claims to.
  ...
  - **Both call sites classify through one exported helper** and hand the *cohort* to the
    ...
    **It lives in `lib/fair-value.ts`**, beside the vocabulary and the predicate it subsumes,
    and takes `string | null` — the same input `isFinancialSector` takes today, so neither call
    site needs a null check of its own. Where the recognised non-financial vocabulary is
    *sourced* from is the implementer's call and the only latitude here: import `GICS_TO_ETF`'s
    keys from `lib/sector-resolution.ts` (one vocabulary, no copy — but it couples valuation to
    a module whose subject is sector ETFs), or declare the set locally next to
    `FINANCIAL_SECTORS`. **Either is fine; what is not fine is a shape where an unmatched string
    lands on `non-financial`.** If the set is declared locally, its docblock must say which
    vocabulary it mirrors and that a label outside it is `unknown` by design, not by oversight —
    the next reader's instinct will be to "complete" the list.
...
| **`isFinancialSector` returns `false` for every non-null string it does not match, so the collapse above is not only about `null`** | `lib/fair-value.ts:58-61` · `if (!sector) return false; return FINANCIAL_SECTORS.has(sector.toLowerCase())` over **five** exact strings. A sector that arrived and was simply not recognised is reported as a *known* non-financial. **This is the round-18 finding, and it is why the classifier is a new function rather than this predicate plus a null check** (§7) |
| **`sector` arrives in Yahoo's vocabulary, and only Yahoo's** | `lib/providers/finnhub.ts:169-205` · the header states *"Finnhub does not provide a sector field"* and the mapper sets `sector: null` unconditionally; Finnhub's `finnhubIndustry` maps to **`industry`**, a different field in a different vocabulary. `get_company_profile.ts` · `pick("sector")` prefers Finnhub only when it `isPresent`, which for `sector` it never is — so the merged `sector` is `fetchYahooCompanyProfile`'s `assetProfile.sector` or null. *(Round 18's dispatch stated Finnhub wins the `sector` field with values like `Banking`; it does not, and the finding stands on the vocabulary facts in this table instead.)* |
| **The desk already declares the recognised sector vocabulary, in Yahoo's naming** | `lib/sector-resolution.ts` · `GICS_TO_ETF`'s eleven keys — Technology, Communication Services, Consumer Cyclical, Consumer Defensive, Energy, **Financial Services**, Healthcare, Industrials, Basic Materials, Real Estate, Utilities — documented as *"the 11 standard GICS sectors"*. Exactly one of them is financial, and it is the one string in `FINANCIAL_SECTORS` any current provider actually emits |
| **An unrecognised sector label already has a settled treatment in this codebase, and it is not "assume the default"** | `lib/providers/etf-profile.ts` · `AV_SECTOR_TO_APP_SECTOR` + `mapSectorLabel`, whose docblock reads *"A label not in this table is NOT silently folded into a neighbour (Decision 7's trap) — `mapSectorLabel` returns it as its own explicit bucket instead"* (`Other: …`). FIX-801 answered this exact question one module over |
| **The two boolean consumers constrain the fix** | `lib/fair-value.ts:71` · `isFinancialSector(sector) ? "equity-multiples" : …` and `lib/dcf.ts:160` · `if (isFinancialSector(sector)) return unavailable("financial-sector")`. Both abstain only for a *matched* financial and proceed on everything else, including an unresolved sector. Changing that changes `fairValue`/`dcf` → `triangulation` → `setupScore` → the rating envelope, so §7 keeps the predicate and adds the classifier beneath it |
...
The discriminator is honest data and belongs there. It tells a fundamentals analyst whether a
free-cash-flow figure was computed by the desk from operating cash flow minus capital
expenditure, or arrived from a provider with no basis the desk can attest — provenance the
desk's real-money discipline wants in front of the model, not hidden from it. But it is a
**fifth field and a fifth source of prompt tokens**, and any
...
1. **Widen the income-statement schema** with the four new fields, each
   ...
   as *basis unrecorded* rather than claiming a basis. The discriminator's type is
   **`"derived" | null`** — two states, because no producer can truthfully emit a third (§7).
   *Test:* **a payload that omits the
   ...
   reads as basis-unrecorded — not as `derived`, and not as unobserved.** That last pair is
...
distinction still exists (§7). `mapEdgarCompanyFacts` always sets `derived`;
`mapYahooTimeseries` sets `null` — *basis not recorded* — when it passed the
`annualFreeCashFlow` series through, and `derived` only when it fell back to `op + capex`;
`candidateFreeCashFlowRaw`'s promote always sets `derived`, since its stated-FCF branch is
unreachable (§7); the empty payload leaves the value absent, which is the unobserved state.
**No producer sets a `reported` state, because there is none to set.**
*Test:* each producer's flag matches the branch it actually took — **including the Yahoo pair,
where the same fixture must produce the unrecorded basis with the `annualFreeCashFlow` series
present and `derived` with the series removed and only the components left.* The two branches
still emit different values, so the pair still fails a producer that hardcodes one flag —
which was always the point of the pair. *Depends on:* 1.
...
| **Free cash flow the desk computed itself** — the EDGAR path always, the prospectus promote always, and the Yahoo path when the series is absent | The three FCF ratios declare a **derived** basis (operating cash flow minus capital expenditure), never `reported-components`. The provenance is set by the producer that made the choice; by `computeValuation` it is one number and the distinction is gone (§7) |
| **Free cash flow a provider handed us** — the Yahoo `annualFreeCashFlow` series branch | Its provenance is **basis unrecorded**. Series presence is not provenance: FCF is not a GAAP concept, so the series is the provider's own arithmetic under a definition it does not publish, and the desk can attest neither that the filer reported it nor that it equals operating cash flow minus capex (§7). This state is permanent on that path, not transitional |
| A cashflow payload stored before this change — **including every checked-in fixture**, so this is the fixture-replay path, not a rare one | Its provenance dual-reads as **basis unrecorded**, never as `derived` (a false label) and never as *unobserved* (which would suppress a real figure). The three FCF ratios render the value with the basis stated as not recorded (§7). Fail-closed on the *label*, not on the number |
...
| **A company whose sector is unknown** — profile absent, or the provider missed | Classified `unknown`, and **the capital-structure family is withheld** with a reason saying the cohort could not be established. The guard fails closed (§7). Note this is *not* `isFinancialSector(null)`, which returns `false` and so answers "no" to a question that was never answerable — the classifier must produce three states, and a bare call to that predicate produces two |
| **A company whose sector is a non-null string the desk does not recognise** — a provider vocabulary change, a hand-authored fixture, an `industry`-shaped value (`Banking`, `Capital Markets`, `Asset Management`) reaching the field, or a sector genuinely outside the eleven | **Classified `unknown`, exactly as the row above, and the capital-structure family is withheld.** This row is the one a two-state predicate gets wrong: the string is not null, so a null check cannot catch it, and `isFinancialSector` answers `false`, so it is labelled a *known* non-financial and the fail-closed guard never fires. A bank arriving under an unrecognised label would then be published with EV/EBIT, EV/Sales and EV/FCF — the exact figure this guard exists to withhold, since enterprise value is not a meaningful concept for a deposit-funded balance sheet. **The remedy is the classifier's shape, not a longer alias list** (§7): matched-neither is `unknown`, so the next unrecognised string fails closed too |
...
- **no metric claims a basis it does not have — the FCF trio specifically:** on a payload
  whose free cash flow the desk computed, `evToFcf`, `fcfYield` and `priceToFcf` declare the
  derived basis; on one carrying a figure with no recorded basis, they say so and claim
  nothing. **No input to any of the three produces a "reported" basis**, because no producer
  emits one (§7) — assert that as a property over the producer set, not as a missing case.
  *Fails when:* the implementation gives the ten non-variable metrics a blanket
  `reported-components` default and sweeps these three in with them — which is what the spec
  said until round 11, and which reads as correct right up until you check what the EDGAR
...
- **FCF provenance, all three consumer states, at the formatter — not in the goal check.**
  Given a cashflow payload with the discriminator `derived`, one with a **non-null value and no
  discriminator**, and one with **no value at all**, the three FCF ratios render respectively
  with the derived basis, a stated *provenance-not-recorded* basis, and an abstention reason
  (§7). Three payloads, three states — the fourth payload this condition used to ask for does
  not exist, because no producer emits a reported basis. *Fails when:* the unrecorded
  case is collapsed into either neighbour — labelled `derived` (a false claim about how the
  number was made) or suppressed as unobserved (discarding a figure the whole fixture corpus
  carries).
...
- **the classifier, over all three states and *four* inputs — the fourth is the round-18
  case and it is the one that fails against the obvious implementation:** a financial sector
  maps to `financial`, a recognised non-financial sector to `non-financial`, `null` to
  `unknown`, **and a non-null string outside both vocabularies to `unknown`.** Asserted on the
  exported helper, once, since both call sites use it. Use JPM's `sector: "Financial Services"`
  as the financial input and NVDA's `"Technology"` as the non-financial one, so both run against
  the corpus rather than an invented string; the fourth input has no corpus source by
  construction, so use a real out-of-vocabulary label (`"Banking"` — Finnhub's `industry`
  vocabulary, verified in §7's table) rather than a nonsense string, because the failure this
  guards is a plausible label, not a garbled one.
  **Two implementations pass the first three assertions and fail the fourth**:
  `isFinancialSector` plus a null check, and any widened alias list. That is the point of the
  test — write the fourth assertion first.
- **the guard, on both call sites:** given cohort `financial` **or** `unknown`, the formatter
  ...
  spine block cannot diverge. *Fails when:* the formatter renders EV multiples for a bank, for
  a company whose sector never resolved, **or for one whose sector arrived unrecognised**.
  **A test that only covers the financial case
  passes against a formatter that treats `unknown` as `non-financial`** — the exact defect
  rounds 6 and 8 shipped — so the `unknown` branch is not optional coverage. **And a test that
  covers `financial` + `null` + `non-financial` still passes against a classifier that answers
  `non-financial` for an unrecognised string**, which is why the coverage that matters is at the
  classifier bullet above, not here: this bullet takes a cohort as its input and cannot see how
  it was decided.
- **the method line never asserts a cohort it does not have:** on an `unknown` run,
  `formatValuationSpine`'s valuation-method line carries the unresolved cohort alongside
  `ev-multiples` (§7), so the rendered spine cannot be read as "this company is not a financial"
  while the capital-structure family is being withheld two lines away. *Fails when:* the line
  renders bare `Valuation method: ev-multiples` next to six withheld metrics — internally
  contradictory, and the sector claim the persisted enum is explicitly not making.
...
- **EXTEND** `labs/trading-desk/CLAUDE.md` — the live-mode paragraph describes what the three
  ...
  requirement before they write one. **Say in one sentence why the discriminator has no
  `reported` state** — free cash flow is not a GAAP concept, so a provider's FCF series is that
  provider's arithmetic and not the filer's disclosure (§7). Without it, the next contributor
  who sees `annualFreeCashFlow` in the Yahoo response will helpfully add the state back, and
  nothing will fail. **Also record the §8 rule there** — widening a tool
...
sixth instance.** In each case the answer was *carry the extra state instead of collapsing
it away*, and in each case the collapse happened early, cheaply, and looked like a
simplification. **Two rows were produced by the fix for the row above them** — the third-time
row by the round-11 fix, and the last row by the round-9 fix. Adding a discriminator does not
end the class; a discriminator has its own absent state, that state is not always one thing,
and **the code that computes the discriminator can reproduce the collapse it was added to
undo.**
...
| **This issue, a fourth time** (round 18) | the *fix* for row 3: the three-state cohort computed as `isFinancialSector(sector)` plus a null check | "unrecognised sector" and "known non-financial" become one value, so the fail-closed guard built to catch the collapse is bypassed by any sector string outside five hard-coded aliases. **The correction restated the same collapse one level in** — the null check makes the *absent* case three-state and leaves the *unmatched* case two-state, which reads as a complete fix and is not one |
...
**Round 18 sharpened the tell, and the sharpening is the transferable part.** *Absent* is not
the only way a value is unknown — **unrecognised is the other, and it does not look like
absence.** A null announces itself; an unmatched string arrives populated, passes a null check,
and gets a confident answer from a predicate whose vocabulary never covered it. So the check
"does this handle the missing case?" is not sufficient, and it is exactly what round 9's fix
answered: it added a null check, which is the visible half. The question that finds the other
half is **"what does this return for an input it doesn't recognise, and is that answer a
claim?"** — because a lookup's default branch is where a two-state answer hides inside a
three-state type.

**And the mirror image, which round 16 found in the fix for the class above.** Carrying a
state is the right instinct, and it has its own failure: **a distinction asserted that the
data cannot support.** Round 11 gave the FCF discriminator a `reported` state on the reasoning
that a provider's FCF series meant the filer had reported it. No producer could ever have set
it truthfully — free cash flow is not a GAAP concept, so a series is the provider's arithmetic,
not the filer's disclosure. A state with no truthful producer is not inert: it tells the reader
the desk distinguishes something it cannot observe, which is the same lie decision 4 exists to
prevent, arriving through the mechanism built to prevent it.

| the check | the question |
|---|---|
| before **collapsing** a state | what distinction does this destroy, and where would a later guard have needed it? |
| before **adding** one | which producer sets this, on what evidence, and can it be wrong? |
| before **trusting a fix** for either | does it cover every way the input is unknown — absent *and* unrecognised — or only the visible one? |

The second question is the cheaper one and it was never asked. The third is the one round 18
had to ask twice, because the answer to it was written into this spec eight rounds ago and read
as settled. All three are one habit: **a state's worth is decided at its producer, not at its
consumer** — and so is its completeness.
...
- **Trust the verified-in-code table; distrust anything in this spec that reads like wiring.**
  ...
  **Round 18 found the second instance, and it widens the rule.** §7 said the cohort was
  computed *"through the shared `isFinancialSector` predicate plus an explicit null check"* —
  a sentence that names two real symbols and is still wrong, because the defect is in what the
  named symbol *returns for an input it does not match*, not in whether the symbol exists. So
  the check is not "does this symbol exist and get called": it is **"run the composition in your
  head over every input the type allows, including the ones nobody wrote a fixture for."** For
  a `string | null` that means null, a matched string, and an unmatched string — three inputs,
  and the third is the one eight rounds of review never evaluated.
...
- **Review round 16 — the discriminator's `reported` state had no truthful producer, and is
  deleted.** Codex found that §8 told `mapYahooTimeseries` to set `reported` when it used the
  `annualFreeCashFlow` series, and that series presence does not establish the **filer**
  reported anything — attaching a false provenance claim to EV/FCF, FCF yield and Price/FCF,
  the exact defect decision 4 exists to prevent. **Confirmed on the structural ground rather
  than a fixture:** free cash flow is not a GAAP line item, there is no `us-gaap` concept for
  it, so a provider's FCF series is that provider's own arithmetic under an unpublished
  definition. No structured provider can establish filer provenance for FCF on any ticker,
  ever. This spec had already conceded the point for `mapEdgarCompanyFacts` ("always derives")
  without noticing it generalises. The checked-in AAPL fixture happens to reconcile to the
  dollar in both periods, which is corroboration and not the argument — one ticker agreeing is
  not a definition.
  **The principle adopted: the basis label describes what the *desk* did, not what the *filer*
  did.** A label the desk cannot truthfully produce is a promise it should stop making.
  **Three consequences.** The Yahoo series branch now sets **basis not recorded**, not
  `derived`: the mapper assigns the series value through and never computes it, so `derived`
  would trade a false provenance claim for a false *definition* claim. It reuses the stored
  `null` rather than earning a third value — `null` already means "no basis is recorded", the
  two `null` consumer states are told apart by whether `freeCashFlow` is present, and a
  distinct value would encode a provider-versus-legacy distinction no reader acts on. The cost
  is stated in §7: *basis unrecorded* is permanent on that path, not transitional.
  **And `reported` is deleted from the vocabulary**, which is `derived | null`. Its last
  candidate producer was the prospectus promote, and that branch is unreachable: the only
  constructor of a `FinancialCandidate` hardcodes `freeCashFlow: null` and the extraction
  prompt says *"Do NOT report free cash flow — it is derived downstream"* (FIX-913's fix, whose
  header already knew that FCF "is a non-GAAP figure rarely printed as a line"). A state no
  producer can reach tells the reader the desk distinguishes something it cannot observe.
  *Also rejected, with a reopen condition:* recomputing `op + capex` in the Yahoo mapper to make
  `derived` true — it changes the published number, not just the label.
  Regenerated from the decision rather than reviewed in place: decision 4, §5's worked example,
  §7's diagram and its FCF passage, §8's schema step and producer step, §9's edge rows, §10's
  two pass conditions, and §11's docs plan. **§8's test pair survives with a new pass
  condition** — the same fixture must produce the unrecorded basis with the series present and
  `derived` with only the components left; the branches still differ, so it still fails a
  producer that hardcodes the flag, which was always its point.
  **§12 names the mirror of its own failure class:** carrying state is the right instinct, and
  its failure mode is *a distinction asserted that the data cannot support*. The check that was
  never run is one question — which producer sets this state, on what evidence, and can it be
  wrong?
- **Review round 17 — the cost paragraph understated its own footprint.** §2 said the run gets
  more expensive **"in one place"**. It is two, and the second one is wide: decision 5's four
  metric lines go into the shared valuation formatter, which the `valuationSpine` preset
  injects at **six** call sites (`agents/research/generators.ts`, `scenario-forecaster`,
  `risk/consolidator`, `trader`, `portfolio-manager`, and `lenses/lens-generator.ts`) — and the
  last of those is a factory, so it yields one prompt surface per seated lens. A full-preset
  run today is **nine prompt surfaces**, not one, on top of the fundamentals analyst's block.
  No decision moves: per-run model cost is not a constraint the owner is trading against. What
  moves is the accuracy of a sentence the owner signs off from, which describes what the change
  *touches*. **The entry is worth keeping for its shape, not its size:** that paragraph exists
  because an earlier draft claimed "no extra cost per run" flatly. It was corrected once for
  exactly this defect, and the correction fixed the *magnitude* and left the *reach* wrong —
  the same shape as the honesty stamp being narrowed and the narrowed list then turning out
  unaudited. A correction to a false claim inherits the claim's blind spot unless it re-derives
  the whole thing.
- **Review round 18 — the fix for the collapse class reproduced the collapse.** §7 prescribed
  the three-state `sectorCohort` as *"the shared `isFinancialSector` predicate plus an explicit
  null check."* That composition has two live states, not three: `isFinancialSector` matches
  five exact strings and returns `false` for everything else, so **any non-null sector the desk
  does not recognise is labelled a known non-financial**, the null check cannot see it, and
  decision 5's fail-closed guard is bypassed by exactly the input it exists for. A bank arriving
  under an unrecognised label gets EV/EBIT, EV/Sales and EV/FCF published against it —
  enterprise value applied to a deposit-funded balance sheet, which is the wrong-instrument
  figure this epic exists to remove. The finding met the directional bar on both stated
  criteria: a third state collapsed to two, and a change in what the report claims to a reader.
  **It survived eight rounds because §9 and §10 already described three states correctly** —
  only the construction in §7 was two-state, and prose describing the right behaviour reads as
  though the mechanism produces it.
  **Widening the alias list was rejected as the fix**, and that rejection is the entry's point:
  any list of financial-sector strings is a list a real value falls outside of, and the next
  unrecognised string fails identically and just as silently. The list was *already* a guess —
  three of its five entries match nothing any current provider emits. So the classifier now
  classifies against vocabularies and lets **matched-neither be `unknown`**, which is the
  treatment `mapSectorLabel` already gives an unmapped label one module over (FIX-801's
  decision 7 — *"NOT silently folded into a neighbour"*).
  **Two constraints came out of reading the code rather than the spec.** `isFinancialSector` is
  kept as a derived predicate because `computeFairValue` and `dcf.ts` gate on it as a boolean,
  and widening them to abstain on `unknown` would withdraw fair value and the DCF from every
  unresolved-sector company — moving `setupScore` and the rating band, the same decision-level
  consequence that sent ROIC to FIX-1118. And `valuationMethod` keeps its two values with
  `unknown` on the `ev-multiples` branch, because that field records *which method ran* and the
  run genuinely ran that path; mapping it to `equity-multiples` would contradict
  `fairValue.method` on the same record. What changes is that it stops reading as a cohort
  verdict — the guard reads `sectorCohort`, and the rendered method line states the unresolved
  cohort beside it.
  **One dispatch fact did not survive verification and is corrected in §7's table**: Finnhub
  does not win the `sector` field with values like `Banking`. `lib/providers/finnhub.ts` sets
  `sector: null` unconditionally and maps `finnhubIndustry` to `industry`; `sector` is
  Yahoo-only. The finding stands on the predicate's own shape and on the desk's two sector
  vocabularies, not on that merge.
  Regenerated from the decision: §6's decision-5 reach line, §7's fail-closed statement, the
  classifier bullets and the `valuationMethod` bullet, six rows of §7's verified-in-code table,
  §9's new unrecognised-string row, §10's classifier and guard conditions plus a new method-line
  condition, and §12's failure-class table (sixth instance) with a third question added to the
  check table: **does the fix cover every way the input is unknown — absent *and* unrecognised
