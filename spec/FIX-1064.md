...
> 2 deliberately left alone and documented as such · ~21 test behaviours · 3 documentation
...
- **The shared surface is the valuation spine, and there are two computation points that
  ...
  **The guard fails closed, and this is a property of the formatter rather than a case in a
  list: the capital-structure family renders only when the sector is known *and* is not a
  financial.** Unknown, unresolved or absent sector withholds it, with a reason saying the
  cohort could not be established. *(Round 6 had this the other way — null sector treated as
  non-financial, on the argument that withholding from every company whose profile failed to
  load is its own defect. Round 8 reversed it, and the reversal is the point: a guard whose
  default branch is "expose" is one provider miss away from the defect it exists to prevent.
  The two errors are not symmetric — withholding six metrics from a non-financial costs a
  reader some context and says why; exposing EV multiples for a bank is the misleading figure
  this epic exists to remove. And with the profile now a direct tool input rather than a
  raced resource read, a null sector means genuinely unresolved rather than merely not-yet-written,
  so the fail-closed branch fires on real absence instead of on scheduling.)*
  ...
  **This is a behaviour change, not plumbing, and it must be reviewed as one.** The
  generator's `context.data` is `asDataBlock(input)` — a `JSON.stringify` over the whole input
  — so adding the profile to the input schema puts the **entire company-profile payload** in
  front of the fundamentals model, not just its sector. That is coherent here: a fundamentals
  analyst that knows what business it is looking at is better, not worse. But it changes what
  the model sees, so it is stated rather than smuggled in as wiring, and it belongs in the
  same review breath as PR a's schema widening (§8).
...
| **A company whose sector is unknown** — profile absent, unmapped, or the provider missed | **The capital-structure family is withheld**, with a reason saying the cohort could not be established. The guard fails closed (§7): it renders those metrics only when the sector is known and non-financial. Note this is *not* `isFinancialSector(null)`, which is `false` — the formatter must distinguish "known non-financial" from "unknown", and a bare call to that predicate does not |
...
- **the cohort guard, on both call sites — and this is the *only* place it is asserted:** for
  ...
  **shared** formatter so the analyst block and the spine block cannot diverge on it — that is
  ...
  above cannot see it. Use JPM's `sector: "Financial Services"` as the financial input so the
  assertion runs against the corpus rather than an invented sector string.
- **the guard fails closed — the case that would otherwise ship broken:** a company whose
  **profile payload is absent, or present with a null sector**, withholds the
  capital-structure family. Assert it at **both call sites, with the same assertion**: the
  spine formatter (sector from `profileData`) and the fundamentals block (sector from the
  profile now in the generator's input, §7). *Fails when:* the formatter renders EV multiples
  for an unresolved sector — which is the state the analyst call site would sit in permanently
  if `get_company_profile` were not added to its tool map, so this condition is what catches a
  half-applied §7. **A formatter-only test on a known-financial input passes against that
  broken code**, which is exactly why this condition exists as well.
  *Observable at:* the formatter, both sites — not the goal check, for the same reason the
  cohort assertion is not there (the guard is presentation, and formatter output is never
  persisted). The absent-profile *run* behaviour is therefore not asserted end to end; the
  formatter pair is the layer where it is visible, and this spec prefers a check that can fail
