...
> 2 deliberately left alone and documented as such · ~16 test behaviours · 3 documentation
...
- **The two provider mappers read the three tags they already fetch.** The filings mapper
  ...
  `YAHOO_TIMESERIES_TYPES` — still one request — and derives earnings growth the way it
  already derives revenue growth. **Both reads sit on whatever selection FIX-1113 settles
  on**, and this spec deliberately does not name the mechanism: it named one in each of
  rounds 1–3 and was wrong each time, at a finer grain each round.
...
1. **Widen the income-statement schema** with the four new fields, each
   ...
   *Depends on:* nothing.
2. **Read the three tags in the filings mapper**, through whatever selection FIX-1113 leaves
   in place. *Test:* a response carrying the tags maps them; a response missing one maps it
   null, never zero. *Depends on:* 1, FIX-1113.
...
coverage silently. *Depends on:* 1.
...
- The legacy fixtures produce a spine in which **return on capital and EV/EBIT carry fallback
  ...
  *(Round 3 corrected this condition: it previously demanded fallback bases on all four, which
  ...
  stated meaning and actual behaviour differ, so **§10 is checked in both directions**, and
  was re-checked end to end in round 4 when the alignment work left the spec.)*
...
coverage gap, as the fallback case. **Leave `JPM` and `SPCX` untouched** —
...
- **what must not change, narrowed to what can actually hold:** a company reporting none of
  the four new fields produces byte-identical numbers to today for **every metric except the
  two growth-adjusted ones** — return on capital falls back to the 21% assumption and EV/EBIT
  to operating income, so both keep today's values, and the other ten never moved.
