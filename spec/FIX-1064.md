...
pays for and throws most of away, and the one lookup this adds is already made elsewhere in
the same run and is cached (§7).
...
- **The metric type gains a required basis descriptor on the *seven* variable metrics**,
  ...
  **Seven, not four, because free cash flow is derived more often than it is reported — and a
  ...
  cashflow payload carries an FCF provenance discriminator** — reported / derived / unobserved
  — set by each producer, which the three metrics read for their basis.

  *Why a discriminator rather than a softer default:* by the time `computeValuation` sees a
  number, the provenance is gone — the same collapse this spec has now hit three times (§12).
...
1. **Widen the income-statement schema** with the four new fields, each
   ...
   discriminator** (§7), defaulting to the unobserved/unknown state so a legacy payload
   dual-reads honestly rather than claiming "reported". *Test:* **a payload that omits the
   ...
   re-parses (BP-030), and **a legacy cashflow payload does not read as `reported`**.
   *Depends on:* nothing.
...
| A cashflow payload stored before this change | Its provenance dual-reads as unknown, not as `reported` — the fail-closed direction for a label, matching the cohort discriminator's treatment |
...
- **what must not change, narrowed twice to what can actually hold:** on a **non-financial**
  ...
  EV/EBIT to operating income, so both keep today's values, and the untouched metrics never moved.
...
- **Field count for the growth inputs.** Whether the payload carries a derived
  ...
  rather than as zero — **and provided the non-positive-base rule (§7) is applied at the
  derivation either way.** If you store only the derived rate, that is the *only* place it can
  be applied, because the sign of the base is gone by the time any consumer sees the number.
