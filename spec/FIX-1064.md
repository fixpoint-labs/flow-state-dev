...
2. **One convergence point for the basis descriptor (tenet 5)** — the basis is part of the
   metric type for the seven variable metrics, so the producers of a derived figure cannot
...
5. **Add the basis descriptor to the metric type** — required on the seven variable metrics
   (ROIC, EV/EBIT, the two growth-adjusted multiples, and the three FCF ratios), typed
   `reported-components` default on the other seven. Lands first and alone because it is
...
| A metric with a value but no determinable basis | Cannot occur — the basis is required by the type on the seven variable metrics and defaulted on the rest (decision 4). A producer that cannot name one is a compile error, not a runtime fallback |
...
- **what must not change, narrowed three times to what can actually hold — assert values, and
  assert byte-identity only where it is true.** On a **non-financial** payload reporting none
  of the four new fields: **every metric keeps today's value**, the two growth-adjusted ones
  excepted (see below) — return on capital falls back to the 21% assumption and EV/EBIT to
  operating income, and the untouched metrics never moved. **Byte-identity holds only for the
  seven metrics outside the required-basis set.** The three FCF ratios keep their numbers but
  their rendered lines now carry a basis they did not carry before (decision 4 covers seven
  metrics, not four — round 11), so a blanket byte-identical assertion over the whole block
  fails on a correct implementation. Split the condition: **values** over all fourteen,
  **bytes** over the seven unlabelled ones.
  **Assert this on a non-financial fixture, not on JPM**: the cohort guard deliberately
  ...
  same trap this condition has now sprung three times, twice from a change made elsewhere in
  the spec rather than from this condition being written carelessly.
...
- **one formatter, two call points — asserted on the shared subset, not on the two blocks.**
  The two call sites render **different metric sets** by design: the analyst renders all
  fourteen metrics, the spine renders the four it persists (decision 5's narrowing). So "the
  ...
  each call site's overlapping lines — the four persisted metrics — match between the two
...
- **FCF provenance, all three consumer states, at the formatter — not in the goal check.**
  ...
  **This belongs here and not in the goal check**: `evToFcf`, `fcfYield` and `priceToFcf` are
  three of the ten metrics decision 5 deliberately keeps **off** the spine, so `runArtifacts`
  cannot see them at all — the same layer constraint as the cohort guard.
...
- **EXTEND** `labs/trading-desk/CLAUDE.md` — the live-mode paragraph describes what the three
  ...
  this spec PR is never merged (BP-037). *Voice
