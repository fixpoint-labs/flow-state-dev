...
- **The shared surface is the valuation spine, and there are two computation points that
  ...
  **Deliberately *not* covered: `roic`.** Invested capital is arguably as meaningless for a
  ...
  the owner approved decision 1 without it. §12 carries it as a follow-up.
...
- **The legacy fixture — JPM, and only JPM** — produces a spine in which return on capital
  carries the **assumed-21% fallback basis**, the two growth-adjusted metrics carry
  **abstention reasons** (no earnings-growth input, so no value, so a reason rather than a
  basis per decision 4), and the **capital-structure family is withheld for the
  equity-multiples cohort** — JPM's profile fixture reads `sector: "Financial Services"`, so
  EV/EBIT is not expected to carry a fallback basis here; it is not expected to appear at all.
  *Fails when:* the missing keys throw on parse rather than defaulting to null, **or** the
  sector guard did not fire and EV multiples reached the spine for a bank.
  ...
  *(This condition has now been corrected three times, and the pattern is worth more than the
  fixes. Round 3: it demanded fallback bases on all four metrics, which decisions 2 and 4
  make impossible. Round 6: it named **SPCX** as a second fixture, which cannot reach a spine
  at all — `fixtures/README.md` marks it statements-only, its directory has no fundamentals or
  profile file, and its `totalEquity` is null, so ROIC is null even in isolation. Round 6 also
  found the guard interaction above. Three criteria elsewhere in §10 had the opposite fault —
  they could not **fail** for their stated reason. Same underlying error either way: **the
  criterion was written against the intent and never against the fixture it names.** So the
  standing rule is now stronger than "check both directions" — for every condition, open the
  fixture it names and confirm that fixture actually reaches the state asserted about it.)*
...
- **the cohort guard, on both call sites:** for a financial sector, the formatter withholds
  `enterpriseValue`, `evToSales`, `evToEbit`, `evToFcf`, `netDebt` and `netLeverage` with a
  stated reason, and renders the equity-side metrics normally; for a non-financial it renders
  all of them. Assert it through the **shared** formatter so the analyst block and the spine
  block cannot diverge on it — that is the whole reason the guard sits there rather than on
  the spine. Include a null-sector case: absent profile data must not be treated as financial
  (`isFinancialSector(null)` is false), because silently withholding EV multiples for every
  company whose profile failed to load would be its own quiet defect.
...
- **[FIX-1113](https://linear.app/fixpoint-labs/issue/FIX-1113) must land first — it now
  ...
  **Why it moved rather than being fixed here:** each round fixed alignment for one more
  ...
  this spec now assumes aligned statements and builds on them.
...
- **Return on capital for financials.** The cohort guard (§7) withholds the
  capital-structure family for banks and insurers but deliberately leaves ROIC rendering,
  because invested capital feeds the quality sub-score and therefore the rating band —
  withholding it would move ratings for every financial, which is a decision-level change and
  the owner approved decision 1 without it. Whether `totalDebt + totalEquity − cash` means
  anything for a deposit-funded balance sheet is a real question and deserves its own issue,
  with the rating impact priced in.
...
- **Review round 6 — after the owner's approval, and disclosed rather than folded quietly.**
  ...
  deliberately excludes ROIC, which would move the rating band (§12 follow-up). **The
