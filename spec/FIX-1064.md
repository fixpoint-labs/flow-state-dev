...
5. **These four figures are put on the shared evidence surface every downstream reader sees,
   ...
   renderer). Doing it here means the figures reach it already honest, rather than being
   exposed blunt and sharpened afterwards.
...
7. **A derived figure combines statement components from one fiscal year only.** Where the
   components for a year are not all present, the figure takes its labelled fallback or
   abstains — it never pairs a stale field with a current one. *(Added in review round 1; it
   ...
   *Locks in:* fewer derived figures than a naive reading would produce, on real large-cap
   ...
   operating-income basis instead, and says so.
...
>
> **Size:** Large — 11 production files (enumerated in §8) · ~18 test behaviours · 2
> documentation surfaces · 2 PRs.
...
3. **BP-030 (tolerate the old shape)** — the valuation spine is persisted per report, and the
   new figures land on it. A report stored before this change must still open, and must not
   render an absent figure as a computed one.
...
flowchart TB
  ...
  subgraph carry[2 · carry]
    IS[income-statement payload<br/>+ tax · pre-tax · interest · earnings growth]
  end
  ...
  subgraph consume[4 · consume]
    ...
    AN[analysts.ts · fundamentals evidence block<br/>reads the spine instead of recomputing]
...
- **The income-statement schema** (`flows/analysis/tools/schemas.ts`,
  ...
  interest expense, and a year-over-year *earnings* growth figure beside the `yoyRevenueGrowth`
  that already exists. All nullable, matching the statement convention this epic's
  [FIX-1063](https://linear.app/fixpoint-labs/issue/FIX-1063) is hardening.
...
- **The two provider mappers read them period-aligned, and this is the load-bearing part.**
  ...
  *Scope guard:* align **the new fields only**. Re-anchoring revenue / operating income /
  net income is a larger change that would move existing numbers and break §10's
...
- **The derived-valuation function** (`flows/analysis/lib/valuation.ts`, `computeValuation`)
  is where the work is. Return on capital taxes at the observed effective rate when pre-tax
  income is positive and the computed rate is inside the §9 band, and at the assumption
  otherwise. EBIT is derived from pre-tax income plus interest expense when both are observed
  **for the anchor year**, and falls back to operating income. The growth-adjusted multiples
...
- **The shared surface is the valuation spine, and there are currently two render paths.**
  `buildValuationSpine` (`lib/valuation-spine.ts`) already takes `valuation: DerivedValuation`
  and consumes it into the fair-value and setup-score inputs, but
  `valuationSpineStateSchema` (`valuation-spine-resource.ts`) stores none of the metrics and
  `formatValuationSpine` renders none of them — so every reader of `<valuationSpine>`,
  including the lens pack, sees no multiples at all. Meanwhile the fundamentals analyst calls
  `formatValuation(computeValuation(input))` inline
  (`agents/analysts/analysts.ts`, the generator's `valuation` context slot), which is the
  only place the multiples are rendered today. **PR b puts the figures on the spine state and
  renders them in `formatValuationSpine`, and re-points the analyst at that same rendered
  block rather than keeping a second computation.** Keeping both would leave two
  `computeValuation` call sites that can drift apart on inputs and on labels — precisely the
  divergence decision 4 exists to prevent — and it would leave FIX-1066's lenses reading a
  surface that disagrees with the analyst's. The spine state's new fields default per BP-030:
  a stored report predating this must open and render the block as absent, never as computed.
...
2. **Read them period-aligned in the filings mapper.** Anchor on the latest fiscal year with
   a reported pre-tax income; read tax and interest at that same year via the existing
   `annualByFy` index; prior-year earnings from the same index. *Test:* a response carrying
   all tags at the anchor year maps them; a response whose interest expense stops two years
   earlier maps interest `null` (**not** the stale value) while pre-tax income still maps;
   a missing tag maps null, never zero. *Depends on:* 1.
3. **Read them period-aligned in the market-data mapper.** Add the annual series to
   `YAHOO_TIMESERIES_TYPES` (still one request), and read each new field at the anchor
...
10. **Carry the figures onto the valuation spine and render them in `formatValuationSpine`**
    (decision 5), defaults-first, then **re-point the fundamentals analyst's `valuation`
    context slot at the spine** so `computeValuation` has one caller for rendering.
    *Test:* the §10 spine behaviours including the legacy read, and that the analyst's block
    and the spine's block are the same text. *Depends on:* 6–9.
...
Pass conditions, on the captured `runArtifacts` output:

- `summary.status === "completed"` with a decision present — the rating path survived a
  return on capital that moved.
- `valuationSpine !== null`, and the spine's return-on-capital figure carries a **basis of
  ...
  income. **Assert the basis, not `setupScore.quality != null`** — the quality sub-score
  averages up to three parts (Piotroski, ROIC, Altman zone) in `lib/setup-score.ts`, so a
  non-null quality proves only that *one of them* ran. Asserting quality alone would rebuild
  the same hole one level down.
- The EV/EBIT figure carries the derived-components basis on the well-reported fixture and
  the operating-income basis on the stale-interest fixture.
- No rendered metric anywhere in the bundle contains a cheap / fair / expensive word or the
  letters `PEG`.

**The goal check needs fixture data that does not exist yet.** The corpus stores the *mapped
payload*, and `fixtures/{NVDA,AAPL}/2026-05-06/income-statement.json` carry six fields with
no tax, pre-tax or interest — so a fixture run today would exercise only the fallback and
prove nothing about the effective-rate path. Hand-author the four new fields onto the two
fixtures (`fixtures/README.md` sanctions hand-authoring for edge cases): **NVDA** as the
well-reported case, **AAPL** as the stale-interest case, matching §3's worked example. A
one-time `dataSource: "record"` run is the alternative and is more expensive and less
deterministic. Use the headless two-step in the trading-desk guide, driven by the
`verify-trading-desk` skill; no new harness.
...
- **the deletion and the rename:** no rendered multiple carries a cheap, fair, or expensive
  word, and no rendered output contains "PEG".
- **one render path:** the fundamentals analyst's block and `formatValuationSpine`'s block
  agree, because they are the same text (step 10).
