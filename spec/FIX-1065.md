...
18 test behaviours · 2 doc surfaces · 1 recompute script · 1 goal · 1 PR. Decision 6b's *bound*
...
3. **Stop the no-downside distribution clearing the reward-to-risk bar** (decision 2).
   ...
   **3b — teach the capacity clamp the current position, then align the action.** Give
   `clampTargetWeight` a `currentWeightPct` (the `computeEvidenceGate` signature precedent) and
   floor the capacity cap at it, so the veto never forces a sale of a held name; skip the numeric
   clamp when the weight is unknown. Then downgrade `initiate`/`add` → `hold`, feeding the
   existing FIX-761 → FIX-781 chain. A reducing action and a nonzero capped buy stay untouched.
...
5. **Rewrite both sizing instructions together** (decision 6, if the owner takes it):
   ...
   *Bound:* add one odds-independent per-preset cap and let it succeed `kellyFraction` in the
   schema and the presets. Enforce it in `clampTargetWeight` on **every mandate-aware target** —
   not on a cleared verdict, which would skip the override path (§9), and not behind the
   `rr != null` guard, since the bound does not read the figure. Floor it at `currentWeightPct`
   like the capacity cap (step 3b), and extend the preset invariant to
   `capacityVetoCapPct ≤ unclearedCapPct ≤ clearedCapPct` in `risk-mandate.ts`'s invariant
   comment. This is new gate logic and three new constants — size the step accordingly, and note
   it is the first mandate effect that binds a *passing* name, so the downward-only language in
   the module docs and `CLAUDE.md` needs restating rather than reusing.
