...
still produce a bigger number, and no amount of prompt wording can prove otherwise. The only
thing that closes it is a size limit the desk enforces itself, in code, that does not come from
the odds — which is what decision 6b asks you about.
...
4. **Stop letting a guess ask for a bigger position.** Today a cleared read tells the PM to
   ...
   **This is decision 6, and it is the owner's** (§6, §12). But removing the instruction does
   ...
   writes the size in one request (§1, §9). Closing that needs a size limit the desk enforces
   in code and that does not come from the odds. Removing the pull also retires the
   size-appetite dial, since the two sentences are its only readers. Both land in **decision
   6b** (§12), and the bound is the load-bearing half of this move, not a tidy-up.
...
adds a number, and it earns it by being the only thing that closes the path (§9).
...
6. **An estimate may permit a size; it may not authorize a larger one.** *(Owner's call — see
   ...
   **Round 3 promoted 6b.** The prose change cannot establish the invariant this decision was
   ...
   longer a tidy-up: the odds-independent bound is the **only** remedy that closes the path,
...
**Size:** Medium — 14 production files (12 + the PM prompt + the PM writer's action alignment) ·
15 test behaviours · 2 doc surfaces · 1 recompute script · 1 goal · 1 PR. Decision 6b's *bound*
branch adds three constants to `lib/risk-mandate.ts` plus a cleared-read clamp in
`lib/mandate-gates.ts` and pushes this to the top of Medium; the *no bound* branch instead
retires `kellyFraction` and removes a test.
...
**The same case on the conservative book, where the capacity cap is zero** *(the round-3
correction — the number was already right, the card was not)*:
...
The size was already being enforced; only the *action* the card colours off was left alone, so
the report showed a green INITIATE on a position of zero. Nothing about the money changes here —
what changes is that the report stops contradicting it.
...
**The fix:** in the FIX-752 block, when the capacity veto fires **and the resulting target is
zero**, downgrade `initiate`/`add` → `hold` and feed that into the existing chain, so each
subsequent gate can only downgrade further. Tie it to a zero result, not to the veto alone —
balanced and aggressive cap at 0.5% and 1.0%, and a capped buy is still a legitimate smaller
buy, which is precisely the distinction the writer already draws for `maxPositionWeight`. A
reducing action (`trim`/`exit`/`hold`) is left intact, as in rows 3 and 4. The `mandateDecision`
mirror carries the downgrade flag alongside `capacityVetoed`, so the stored record and the eval
recompute can both see it (row 7).
...
at the PM commit, after the capacity clamp:
    capacity vetoed AND clamped target is 0  →  an "initiate"/"add" becomes "hold"
                                                (a reducing action is left alone;
                                                 a nonzero capped buy stays a buy)
...
3. **Stop the no-downside distribution clearing the reward-to-risk bar** (decision 2).
   ...
   **3b — align the action with the enforced size.** At the PM commit, a capacity veto that
   clamps the target to **zero** downgrades an `initiate`/`add` to `hold`, feeding the existing
   FIX-761 → FIX-781 downgrade chain; a reducing action and a nonzero capped buy are untouched.
   Carry the flag on the `mandateDecision` mirror and teach `eval/invariants.ts` the new rule.
   *Test:* the all-presets case above asserts **action alignment as well as weight** — on the
   conservative preset a zero target must not publish as `initiate`/`add`. A test that checks
   only the weight is what let this through. *Depends on:* 3. **Not conditional on decision 6
   or 6b** — this is decision 2's own consequence and lands either way.
...
5. **Rewrite both sizing instructions together** (decision 6, if the owner takes it):
   ...
   nothing.
   **5b — land decision 6b in the same step**, whichever way the owner calls it.
   *Bound:* add one odds-independent per-preset cap, enforce it in `clampTargetWeight` on a
   **cleared** read, and let it succeed `kellyFraction` in the schema and the presets. This is
   new gate logic and three new constants — size the step accordingly, and note it is the first
   ...
   *No bound:* drop `kellyFraction` from `riskMandateSchema` and the three presets, drop the
   ...
   and leaving it in place is how the next reader re-derives a Kelly sentence. Then make the
   disclosure explicit on the report surface, because that branch ships a known unbounded path.
   Either way, do **not** ship step 5 with the dial left dangling.
...
| Capacity veto clamps the target to **zero** (the conservative preset) | An `initiate`/`add` is downgraded to `hold`, so the report cannot show a green add on a position of zero. A `trim`/`exit`/`hold` is untouched |
| Capacity veto clamps to a **nonzero** cap (balanced 0.5%, aggressive 1.0%) | Action untouched — a capped buy is still a smaller buy, matching the `maxPositionWeight` precedent |
...
| Cleared read, mandate-aware run | Decision 6: the PM is no longer *told* to scale the size toward the odds — but it still reads them in the call that writes the size (§9's invariant). Both mandate caps are keyed to a *failing* gate, so unless decision 6b adds a cleared-name bound, nothing limits the result (the IPS cap binds only where a household mandate exists) |
...
- **Only enforceable with a bound:** *the desk does not let an unmeasured figure produce a larger
  position.* This one requires an odds-independent ceiling the desk applies itself (decision 6b's
  *bound* branch). Prose alone cannot establish it, because the size is written by the same
  call that reads the odds.

**Consequence for decision 6b:** the deterministic bound is not one option among equals. It is
the **only** remedy that closes the path from invented odds to a larger position; retiring the
...
- decision 6b: on *bound*, a cleared read is capped at the new per-preset ceiling on all three
  presets — the first mandate behaviour that binds a **passing** name, so it needs its own case
  rather than riding the existing clamp specs; on *no bound*, the dial is gone from the schema
  and the presets (a typecheck, plus the deleted ordering assertion, is the whole evidence)
...
- the **enforceable** invariant (per §9): no deterministic code path scales `targetWeightPct` off
  ...
  takes it, this line gains a second assertion: a cleared read is capped at the bound on all
  three presets
...
- **EXTEND** `labs/trading-desk/CLAUDE.md` — the FIX-752 and FIX-781 sections currently
  ...
  round-3 corrections there: the capacity veto now aligns the published action when it enforces
  zero exposure, and — this is the one a future reader will otherwise re-derive wrongly — the
  desk's downward-only guarantee covers the *deterministic* clamps, not the size the PM writes,
  which is produced by the same call that reads the odds.
...
> in the product), but it is **not sufficient**. The thing that actually closes the path is a
...
> closes the hole.** My recommendation on *this* fork is unchanged — the correction is to what
> taking it buys you.
>
...
>
> *Raised in round 2, rewritten in round 3. **Two things about it changed**, and both matter to
> how you answer. It is no longer conditional on decision 6 — the size limit is needed whether
> you delete the upward pull or keep it; keeping it only makes the gap wider. And my
> recommendation has flipped, because round 3 showed that decision 6 alone does not close the
> path (see the second correction above).*
>
...
> settings** — a passing name held to, say, 1% / 3% / 6% of the book however good the odds look,
> enforced by the desk in code — or **don't**, and accept that on a passing name the size is
> whatever the AI wrote, with the exposure disclosed. There is a related tidy-up either way: the
> "aggressiveness" number each setting carries today has no effect once decision 6 lands, so it
> is either replaced by this limit or retired.
>
> **The trade-off.** The limit is the only thing that makes "invented odds cannot buy a bigger
> position" a true sentence — everything else in this spec either labels the figure or fails a
> check closed, and none of it constrains the number the AI writes on a clean read. But it is
> the first rule in the desk that would bind a name it *approved of*, it needs three numbers
> chosen without evidence to choose them from, and set too low it quietly caps the
> high-conviction positions an aggressive book exists to take. Not adding it keeps the desk's
> current behaviour exactly, which is also its current exposure.
>
...
> was wrong in a way that matters to you: this spec's headline promise is that an unmeasured
> number can no longer enlarge a position, and without the limit that promise is not kept — it
> is only *uninstructed*. Shipping the labels, the two fail-closed defaults, and the deleted
> Kelly sentence, and describing the result as "invented odds can no longer size the book up",
> would be the same kind of claim this whole issue exists to stop. The desk already has this
> exact shape of limit in the household-policy layer; it is simply optional there, so a book
> with no policy recorded has nothing. Adding it to the three risk settings makes it apply
> everywhere the standard applies.
>
