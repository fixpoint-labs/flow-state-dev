...
the decision-snapshot basis marker) · 26 test behaviours · 2 doc surfaces · 1 recompute script
(plus the per-run pre-gate capture it reads, §10) · 1 goal · 1 PR. Decision 6b's *bound*
...
1. **BP-030 (tolerate the old shape) — two persisted shapes change here, not one.** Every stored
   ...
   record was. **And the mandate shape itself changes on either axis** — 6b's *bound* adds a
   dial, decision 6's *delete* removes one — while `riskMandateSchema` is frozen onto session
   state and into run artifacts. A new required dial breaks resume and replay; a removed one must
   be tolerated rather than rejected (§8 5b). Round 5 caught this because the first pass read
   "persisted shape" as meaning only the figure; round 6 corrected it from "6b's branch" to
   "either axis".
...
| Reader | What it does with the dial | After decision 6 **deletes the pull** (the only branch that retires it — on *keep pull* every row below is unchanged) |
|---|---|---|
...
5. **Rewrite both sizing instructions together** (decision 6, if the owner takes it):
   ...
   **5b — land decision 6b.** Round 5 caught this step conflating two independent things: it
   ...
   - **`clearedCapPct` is added iff decision 6b says bound** — applied to **every mandate-aware
     target**: not on a cleared verdict, which would skip the override path (§9).
     **This requires moving the call site, not just adding a branch.** The writer today runs the
     whole FIX-752 block inside `if (mandate != null && rr != null && rr.evidenceBasis != null)`,
     so a ceiling written inside `clampTargetWeight` as it is invoked now would still be skipped
     on a forecastless run. The ceiling is odds-independent, so its application must be reachable
     whenever `mandate != null` — whether that is a separate clamp call outside the `rr` guard or
     a restructured guard is the implementer's choice, but §9's "outside the `rr` guard" and this
     step must end up describing the same code. The same restructure is what lets the forecastless
     mandate mirror be written (§9, round 6).
     ...
     `capacityVetoCapPct ≤ unclearedCapPct ≤ clearedCapPct` in `risk-mandate.ts`, with the values
     themselves set by the owner (§12), not by the implementer. Nothing to do with the dial.
   ...
   *Remedy — and it is two different remedies, which round 5 collapsed into one.* Re-deriving the
   missing dial from `MANDATE_PACK` is right for one case and rewrites history in the other:

   - **In-flight resume** (a session that has not committed its decision yet): re-derive the
     missing dial from the stored `id` against `MANDATE_PACK`. The mandate is config-as-data
     keyed by a stable id, the run is still going to gate, and it must gate under a complete
     mandate. Prefer this over a bare `.default()`, which would hand an old session a zero-ish
     cap.
   - **Historical replay** (a completed run's artifacts, read by the report, the eval layer, or
     the recompute): **do not derive it.** Attaching today's ceiling to a decision committed
     before the ceiling existed invents a constraint that was never applied — an old 8% target
     would then fail a 3% invariant it was never gated against, and every later retune of the
     pack would rewrite the same artifact again. Leave the dial absent, mark the record as
     predating the ceiling, and have the invariant layer **skip** the ceiling check for those
     decisions rather than fail them.

   On removal (`kellyFraction`, if decision 6 deletes the pull), tolerate and ignore the extra key
   rather than rejecting it — the value is no longer read by anything.
   *Test:* resume a session frozen with a pre-change mandate **and** replay a completed
   pre-change artifact — the first parses and gates with the dial re-derived; the second parses,
   keeps its original committed target, and is not failed by an invariant that did not exist when
   it ran. The existing legacy coverage only exercises the reward-to-risk shape, which is why
   this was missed.

   **BP-030 in the direction that gets forgotten.** The BP is usually read as *tolerate the old
   shape*; the other half is *do not invent history for it*. A default that looks harmless on a
   live record is a fabrication on a historical one.
...
| Mandate-aware run whose **forecast is missing** | No reward-to-risk figure, so no soft or capacity gating (as today) — but 6b's ceiling still binds, because it does not read the figure. The mandate mirror is still written, with every figure-derived field null, so the panel and the run summary show the ceiling that fired rather than showing nothing |
...
illustrative ceilings of 1 / 3 / 6% (the real values are the owner's — §12) against uncleared
caps of 0.5 / 1.5 / 3.0% and capacity caps
...
3. **No usable forecast** (`rr == null`, or no `evidenceBasis`) — the whole FIX-752 block is
   ...
   **And it must publish its attribution, which the current shape cannot** (round 6). The writer
   builds `mandateDecision` *inside* that same `rr` guard, and the memo declares it
   `mandateDecisionSchema.nullable().default(null)` with the panel gated on non-null — so moving
   the ceiling outside the guard without changing the mirror lets the desk enforce a limit that
   the mandate panel, the decision snapshot, and the run summary all show nothing about. A clamp
   nobody can see is the same defect class as a figure nobody labels, which is this issue's
   subject. **Requirement:** on a mandate-aware, forecastless run the mirror is still written,
   with the figure-derived fields (`lossAdjustedGlr`, `expectedValuePct`, `worstCaseReturnPct`,
   `noDownside`, `evidenceBasis`, both verdicts) **null** and the ceiling's own fields populated —
   which is what `.nullable()` on those fields is for. Verified on the **report and the run
   summary**, not only on the committed number.
...
**The goal that does hold, narrowed in round 6 to what it can actually prove** —
`goals/trading-desk-reward-to-risk/forecaster-self-report-reaches-the-gate/`: on an ordinary
fixture run, the forecaster's self-report is **carried from the memo onto the stored record and
read by the evidence gate**, with the published verdict agreeing with a recompute from the stored
value. That is a **provenance** claim, and its precondition holds on every run because the
forecaster always reports something — the `standing-thesis-injected` shape, on the same
`analyze` → `runSummary` ladder.

**Say plainly what it does not prove.** It does not show that a *thin* self-report gates a
would-be add on the real path, because producing a `thin` output on demand depends on model
behaviour — the same wall decision 2 hit. So **decision 3 has no real-path behavioural proof
either**; it has the provenance goal above plus CI coverage of the gating logic over both
self-report values. Both decisions are pinned at the pure seam and plumbing-verified end to end,
and neither has an end-to-end behavioural demonstration. That is the honest scope, not a gap to
be closed by inventing a mechanism — the general fix is the same generator record/replay seam
already flagged in §12.
...
- decision 6b, *bound* branch: **every** mandate-aware target is capped at the new per-preset
  ceiling on all three presets — cleared, override-cleared, and a run with no forecast at all —
  the first mandate behaviour that binds a **passing** name, so it needs its own case rather than
  riding the existing clamp specs
- **`kellyFraction`'s removal is asserted against decision 6, never against 6b** (round 6 — the
  round-5 fix tabulated the matrix and then left this instruction contradicting it). Assert the
  dial is gone **iff decision 6 deletes the upward pull**; on *keep pull* it must still be in the
  schema and still read by `formatRiskMandate`, whatever 6b decides. A test keyed to the bound
  branch would fail the supported *keep pull + no bound* and *keep pull + bound* combinations
...
model variance with the gate's real effect, in both directions. Instead: capture each corpus run's
pre-gate decision context once, then apply the **old and new pure gate rules to the same captured
context** offline (`computeRewardToRisk` → `computeMandateGates` / `computeEvidenceGate` /
`clampTargetWeight` are pure, so this is a script, not a sweep). That answers "how often" exactly,
costs no model spend, and is attributable by cause. It is **ship-blocking for decision 2** — if
the no-downside rule fires on most runs rather than the rare ones, decision 2 goes back to the
owner rather than shipping quietly.

**What "capture" has to mean, or the count cannot run** (round 6 — the plan said *forecast
artifacts*, which is not enough to call a single one of these functions). The pure gate chain
needs, per run:

| For | Inputs |
|---|---|
| `computeRewardToRisk` | the scenario buckets (probability + return) and the mandate's `lossAversion` |
| `computeMandateGates` | the **frozen mandate** (not its id — the dials as they were), the figure, `decisionConfidence`, and whether an override reason was stated |
| `clampTargetWeight` | the PM's **pre-gate** `targetWeightPct`, the mandate, the gates, the override, and the **current weight** (needed by the round-4/5 floors) |
| `computeEvidenceGate` | `spineEvidenceBasis`, `spineLowConfidence`, `rewardToRiskEvidenceBasis`, `criticalDataThin`, the incoming `action`, the target, and the current weight |

Two of those are not persisted today and must be added to the capture: the PM's **raw proposed
target and action before the FIX-752 clamp** — the writer keeps `preGatePolicyTargetPct` and
`preGateEvidenceTargetPct` but nothing upstream of the mandate clamp — and the **frozen mandate
dials**, which matter because re-deriving them from the id would measure today's pack against
yesterday's decision (the same mistake §8 5b guards against). Capture the whole pre-gate context
as one record per run, versioned, rather than assembling it from separate artifacts.

**This is the epic's recurring defect, and it is worth naming as such:** a verification step that
cannot execute from what its own plan preserves. §10 has now hit it twice — the seeded-memo goal
in round 2 and this count in round 6.
...
>
> **One limitation to weigh, stated as a fact rather than a caveat.** The limit binds newly-sized
> exposure **where the desk knows what the book already holds**. When a held position cannot be
> priced, the desk does not know its current weight, and every size gate in the product — this
> one included — skips its numeric clamp rather than guess a number, falling back to refusing the
> increase. So the limit cannot bind what the desk cannot measure. This is not a gap being fixed
> later; it is what the limit is.
>
> **And the numbers themselves are yours, not mine.** I have used 1% / 3% / 6% throughout as an
> illustration, and they should not survive into the build as a default I picked. Three things
> to settle if you take the limit:
>
> - **What is the most of the book any single name may be worth, per risk setting?** One number
>   each for the conservative, balanced and aggressive books. They only have to satisfy
>   `conservative ≤ balanced ≤ aggressive`, and each must be at least its book's existing
>   uncleared cap (0.5 / 1.5 / 3.0%), or the new limit would bind names that fail the standard
>   more tightly than ones that pass it.
> - **Or defer the numbers — which is not the same as deferring the limit.** ("What would change
>   my mind" above defers the whole mechanism; this defers only the three constants.) Take the
>   limit in principle and set its values after §10's recompute reports what sizes the desk has
>   actually been committing. A real option, not a
>   delaying tactic: the recompute is already ship-blocking work for decision 2, so the data
>   arrives either way, and shipping the mechanism with provisional numbers is harder to correct
>   than shipping it once with measured ones. The cost of deferring is that the unbounded path
>   stays open until then, which the report would have to disclose in the meantime.
> - **Who owns retuning them later** — the same person who owns the existing mandate dials, or a
>   different call, given this is the first one that binds a name the desk approved of.
>
> I am deliberately not choosing these. Every other number in the mandate pack was set as a
> product judgment about how each book should behave, and this one is the same kind of number.
...
- **Review round 5 — the remedy was still being oversold, and the same defect class surfaced a
  ...
  - **Method note:** round 5 executed nothing. Four findings are static reads; the first needed
    no code at all.
- **Review round 6 — four mechanical folds, and two of them were round-5 corrections that were
  internally inconsistent.** The owner-facing promise is deliberately **not** rewritten again;
  the coordinator is taking that to the product owner directly, since three successive
  formulations have each been falsified.
  - **Confirmed:** §10's CI instruction asserted the Kelly dial is gone on 6b's *no bound*
    branch — contradicting the four-combination matrix the same round-5 edit had just added.
    Dial removal is keyed to **decision 6**, never to 6b. Swept the rest of round 5's new text
    against the matrix and found two more: §4's BP-030 line said the mandate shape changes "on
    6b's bound branch" (it changes on either axis), and §7's dial table was captioned "after
    decisions 6 + 6b" (it is decision 6 alone). All three aligned.
  - **Confirmed:** the offline count could not run from what it captured. `computeMandateGates`
    needs the frozen mandate, `decisionConfidence` and the override; the clamp and evidence paths
    need the PM's pre-gate proposal and action, the current weight, the spine basis and the
    critical-data signal. §10 now tabulates every input and names the two that are not persisted
    at all — the PM's raw target/action **upstream of the FIX-752 clamp** (only the post-mandate
    `preGatePolicyTargetPct` / `preGateEvidenceTargetPct` exist) and the **frozen dials**, which
    must be stored rather than re-derived or the count measures today's pack against yesterday's
    decision. Captured as one versioned pre-gate record per run.
  - **Confirmed:** a forecastless ceiling clamp published no attribution. `mandateDecision` is
    built inside the `rr` guard and declared `.nullable().default(null)`, with the panel gated on
    non-null — so moving the ceiling outside the guard without changing the mirror would enforce
    a limit that the panel, the snapshot and the run summary all show nothing about. Now requires
    a forecastless mirror with the figure-derived fields null, verified on the report and the run
    summary rather than only on the number. §8 5b also had to say that the **call site** moves,
    not just that a branch is added — §8 and §9 were describing different code.
  - **Confirmed:** re-deriving a missing `clearedCapPct` from the current `MANDATE_PACK` rewrites
    history for completed artifacts — attaching a ceiling that did not exist when the target was
    committed, failing an old 8% target against a 3% invariant, and re-rewriting the same artifact
    on every later retune. Split into two remedies: derive on **in-flight resume** (the run still
    has to gate), never on **historical replay** (leave the dial absent, mark the record as
    predating the ceiling, skip the invariant). BP-030 in the direction people forget — not just
    tolerating the old shape, but not inventing history for it.
  - **Also folded, at the coordinator's direction and without restating any promise:** the
    unpriced-held-position limitation is stated in §12 as a fact for the owner to weigh (the limit
    binds newly-sized exposure where the current weight is known, and cannot bind what the desk
    cannot measure); the per-preset ceiling **values** are now an explicit ask rather than an
    illustration, with defer-until-measured offered as a real branch and distinguished from
    deferring the mechanism; and §10's surviving goal is narrowed to a **provenance** claim, with
    the plain statement that decision 3 — like decision 2 — has no real-path behavioural proof,
    because forcing a `thin` self-report depends on model behaviour.
  - **Method note:** round 6 executed nothing. Two findings are checks of this spec against
    itself; two are static reads (`writer.ts`'s guard placement and mirror construction; the pure
    gates' input signatures).
