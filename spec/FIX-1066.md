...
11. **`GAP_WEIGHT` is `0.5` — a lens that flagged a gap counts as half a seat.**
    *Rejected:* leaving it as "some value below 1". The dial decides whether the headline and the
    permitted size actually change, so an open range makes the implementer invent product policy.
    On the motivating case — four lenses agree, two of them flagged gaps — `0.9` moves agreement
    to 0.95 and changes nothing a reader would notice, while `0.5` moves it to 0.75. Half a seat
    is also the honest reading of the claim: a lens that says it lacked its own core metrics has
    given you something, but not a whole opinion.
    *Locks in:* the number is now product policy and yours to confirm. Re-tuning it later moves
    every future report, and the spec pins comparisons rather than literals in the tests so the
    dial stays a dial (§10).
...
>
> **Size:** Large — 14 production files · 19 test behaviours · 2 documentation surfaces · 3 PRs.
...
| Gap case: 2 of 4 agreeing lenses flagged | 0.500 mixed | 0.667 mixed | **0.500** mixed |
...
| Three bullish at 0.8, two bearish at 0.7 + bearish skeptic | +0.025 | **+0.200** — more bullish |
...
domain.** Three consumers must be updated together, or the suppression is cosmetic:

| Consumer | Today | Below quorum |
|---|---|---|
| `formatLensConvergence` (PM context) | always emits `Read: … agreement N% …` | emits the quorum statement instead — no `Read:` line, no percentage, no sizing rule keyed to a number that does not exist |
| `LensConvergenceStrip` (hero) | renders the classification pill | renders a "panel did not reach quorum" state; the pill is suppressed, not shown as `divergent` |
| `LensConvergenceBlock` (summary) | same pill + per-lens cells | same suppression; the individual reads that arrived still render, labelled as individual reads rather than as a panel |
...
9. **Teach the three consumers the null branch** — `formatLensConvergence`, the hero strip, and the
   summary block — per §7's table. The PM context must state the quorum shortfall instead of a
   `Read:` line, and must not carry a sizing rule keyed to a figure that does not exist.
   *Test:* one assertion per consumer on a below-quorum record, plus one that "did not run" (cheap
   preset) and "did not reach quorum" render differently. *Depends on:* 8.
...
| c | sizing floor + one-directional clamp, quorum guard + its nullable representation, three consumers taught the null branch | a |
...
| **One methodology verdict + the skeptic** | Insufficient quorum → `divergent`. Today this lands on 0.000/divergent via the tie-break — the right answer for the wrong reason, so it must be pinned deliberately |
| **Two methodology verdicts + the skeptic, 4 seats (3 seated methodology)** | Quorum met (2 ≥ 2); normal path |
| **Two methodology verdicts + the skeptic, 6 seats (5 seated methodology)** | **Quorum NOT met** (2 < 3) → `divergent`, no full sizing. Naively this is `2/2 = 1.000 convergent`; the same verdict count is sufficient at four seats and insufficient at six, which is the point of a fraction |
...
- **suppression is an absence, not a value**: a below-quorum record carries `null`, and a genuinely
  tied 3–3 pack carries `0`. Assert both, and assert they render differently in **all three**
  consumers — the PM context, the hero strip and the summary block. A test that only checks the
  stored record passes while the formatter still prints `agreement 0%`, which is the failure mode
  decision 9a exists to prevent
...
- **EXTEND** `labs/trading-desk/CLAUDE.md` — the lens-pack paragraph under "Portfolio-aware
  ...
  **State the membership contract exactly as it ships**, because it is two rules and they are easy
  ...
  finding about a split panel — and that all three consumers branch on it.
...
**Open questions:** one, and it is a genuine fork rather than a detail.

> ### After this, the desk will say "the philosophies agree" far less often. Confirm how much less.
>
> **Plain terms.** "Convergent" is the strongest thing the panel says, and it is what lets the desk
> size a position fully. Two numbers decide how hard it is to earn, and both are currently set by us
> rather than by you. A lens that admits it lacked the numbers it needed counts as **half a seat**
> (`GAP_WEIGHT = 0.5`, decision 11). And "convergent" now requires the counted lenses to be
> **unanimous**, where today it tolerates one dissenter as soon as there are five or more
> (decision 10). Together they mean a report reading "convergent" today can read "mixed" on
> identical evidence — a weaker conviction signal handed to the sizing step. Worth being precise,
> because the spec was loose about this for four rounds: **that lowers the signal, it does not by
> itself cap the position.** The desk's own portfolio manager still writes the number, and the
> enforced ceiling is the one you already approved on FIX-1065 (decision 6b).
>
> **The trade-off.** Set them loose and the honesty fix is cosmetic — at a 0.9 gap weight the
> motivating case still prints "convergent" and nothing a manager acts on changes. Set them tight
> and the desk rarely reaches full conviction, which is safe but can read as a product that never
> commits. No measurement settles this; it is a risk-appetite call, which is why it is yours.
>
> **My recommendation: take both as proposed.** Half a seat is the honest reading of a lens that
> lacked its own core metrics — it gave you something, not a whole opinion. Unanimity is the only
> definition of "convergent" that means the same thing as the pack grows from four to six, and the
> alternative is a headline that silently loosens the moment the two new lenses are seated.
>
> **What would change my mind:** if "convergent" is load-bearing downstream in a way I have not
> traced — if a large share of past reports sized up on it and you would consider that
> correct in hindsight. That is a claim about past decisions being right, which I cannot check and
> you can. If so, `GAP_WEIGHT = 0.75` with unanimity is the middle setting.
>
> **What being wrong costs:** both are single constants, cheap to reverse. What is not reversible is
> that published reports keep the sizing rationale they were written with — after this, the same
> evidence can produce a different recommendation, and seating the two new lenses re-baselines it
> again (decision 9b). Too tight costs opportunity you can see and complain about; too loose costs
> capital on names the panel never actually agreed on.
...
- **Revision 7 — two earlier rulings applied where they had been missed. Spec review closes here.**
