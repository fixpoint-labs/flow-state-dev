...
9a. **A panel that mostly failed cannot report agreement. Below a quorum of methodology verdicts
    ...
    **Zero is not available as the sentinel**, and the reason is *reachability*, not merit:
    `agreementScore: 0` is a value a real panel already produces — a 3–3 bull/bear split lands
    there today — so reusing it for "we could not assemble a panel" makes a panel that split and a
    panel that failed render identically, and quietly violates this decision's own "quote no
    figure" rule, since the formatter would print `agreement 0%`.
    *This argument deliberately does not rest on `0` being a good number.* An earlier revision
    justified the choice by calling `0` "a genuine finding about a genuinely split panel" — which
    contradicts §3, where the same value is flagged as an artefact of the tie-break, and this
    decision's own sentence two paragraphs up calling it "an accident of the tie-break rather than
    by design". The spec cannot hold both. The one it needs is weaker and true either way:
    **an occupied point in the domain cannot be an absence sentinel, whatever it means.** Whether
    `0` is the right number for a tied panel is a separate, still-open question (§12) that this
    decision neither answers nor depends on. Same three-state problem
...
9c. **Every agreement figure is computed over the SEATED pack, not over the lenses that reported.
    ...
    *Deliberately not extended:* the directional lean has the same absence sensitivity and this
    decision does **not** touch it — see §12, where it is an open item rather than a silent fix,
    because shortening a lean is a different question from diluting an agreement number and
    decision 3 says the lean is untouched.
...
**0%** on a bull/bear split, with every lens listed as a dissenter. A 3–3 split is not total
disarray, and six lenses tie more easily than four. **This spec does not redesign the tie-break,
and the reason is that it fails closed — provably, not by inspection.** On a tie `majorityStance`
is `neutral`, and `counts[neutral] <= maxCount` by construction: neutral is either one of the tied
leaders, in which case the two are equal, or it is not a leader, in which case it is strictly
smaller. So the tie-break's agreement can never exceed the modal fraction — it understates or
matches, and it never overstates. Characterized against the real function in
`spec-poc/FIX-1066-attendance/`: the 3–3 bull/bear tie reads `0.000` against a modal `0.500`, the
3–3 bull/neutral tie reads `0.500` on the nose, and a 2–2–2 three-way reads `0.333` on the nose.
That is why it can wait and why decision 9c's attendance hole could not: **one errs toward the
disclaimer, the other errs toward the headline**, and this epic's rule is that only the second kind
costs money. It remains an open question — §12 carries it, and §9 pins today's values as
*characterized*, not as endorsed.)*
...
aggregate(seated, reported):           ← TWO sets, not one.  ← decision 9c
    ...
    majority stance      = modal stance over `reported`, ties → neutral
    raw agreement        = (count on the majority stance)          / |seated|
    discounted agreement = (sum of weights on the majority stance) / |seated|
    ...
    lean                 = sum(sign × conviction) / |reported|     ← unchanged from today.
                            Never gap-weighted (dec. 3), and deliberately still
                            over `reported` — see §12's open item. Do not
                            "make it consistent" with the line above.
...
sentinel because it is **occupied** — today's tie-break produces exactly `0` on a 3–3 bull/bear
pack, so a real run already lands there. Reusing it for "the panel did not convene" makes a panel
that split and a panel that failed render identically, and the formatter would print `agreement 0%`
in direct violation of decision 9a. Note the argument does *not* claim `0` is the right figure for
a tied panel — §3 says it probably is not, and §12 keeps that open; occupancy alone is enough, and
is the only form of the claim this spec can support (decision 9a). Same class of problem as
FIX-1063's zero-filled payloads, same resolution: **absence is `null`, never a value inside the
domain.**
...
- **No mechanism in this spec weights the lean or changes which members it runs over**, so nothing
  here can lengthen it. This is the half the original design got wrong, and it is why the skeptic
  stays in `N` — dropping a member is a membership change, and a membership change moves the lean.
- **Lens errors still change it, as they do today.** Named, not fixed (§12).
...
| **Below quorum vs a genuinely tied panel** | A 3–3 bull/bear tie yields `agreementScore: 0`; a below-quorum run yields `null`. The two must be distinguishable on the record **and** in all three renderings — this is the test that proves `0` was not reused as the sentinel. **It pins that the two differ, not that `0` is the right figure for a tie** — that is open (§3, §12) and this row must not be read as settling it |
...
- **the denominator is not a function of the verdicts** (decision 9c): hold the seated count fixed
  and remove verdicts one at a time; agreement must fall monotonically. A test that lets the
  denominator track the verdict list sees it stay flat, which is the bug. This is the assertion
  that would have caught the round-11 design
...
- **suppression is an absence, not a value**: a below-quorum record carries `null`, and a genuinely
  tied 3–3 bull/bear pack carries a number. **Assert that the two are distinguishable — do not
  assert that the number is `0` as though `0` were correct.** Today's tie-break produces `0` there
  and §3 keeps whether that is the right figure open (§12), so pin it as *characterized current
  behaviour* with that label on the assertion; a later tie-break change should move this test, not
  break the suppression guarantee it exists to prove. Assert they render differently in **every consumer
...
- **EXTEND** `labs/trading-desk/CLAUDE.md` — the lens-pack paragraph under "Portfolio-aware
  ...
  **State the membership contract exactly as it ships**, because it is two rules and they are easy
  ...
  Note the deliberate asymmetry so it does not read as an oversight: **`netLean` still divides by
  the reported count** and is not part of this rule (§12).
...
- **What `agreementScore` should be on a genuinely tied panel is still open, and this spec
  deliberately does not settle it.** A 3–3 bull/bear split reads `0.000` today because the
  tie-break names `neutral` as the majority and no lens holds it; an honest modal reading would be
  `0.500`. Two things make it worth naming now rather than later: PR b makes ties **more frequent**
  (six seats tie more easily than four), and decision 9a's null-vs-zero argument used to lean on
  `0` being a meaningful finding — that dependency is now removed, so the two no longer argue with
  each other. **Why it is not in scope:** the tie-break **fails closed**, provably. On a tie
  `majorityStance` is `neutral` and `counts[neutral] <= maxCount` by construction, so the tie
  agreement can never exceed the modal fraction — it understates or matches, never overstates
  (characterized in `spec-poc/FIX-1066-attendance/`). Under this epic's own rule only the
  overstating direction costs money. Fixing it also means a fifth representation change (a tie
  needs a state `majorityStance` does not have) landing in an already-large PR a, which is the
  scope defect §13 names. **The probe, if you want it settled:** how often the captured runs
  actually tie, and whether any tied report was acted on. Worth its own issue, not a line in this
  one.
...
| 2 | a panel that **failed** vs. a panel that genuinely **tied 3–3** | `agreementScore: 0` — a point in the domain a real run already reaches |
