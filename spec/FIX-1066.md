...
11. **`GAP_WEIGHT` is `0.5` — a lens that flagged a gap counts as half a seat. It is on the owner's
    ...
    *Locks in:* the number is product policy. Half a seat is the honest reading of the claim — a
    ...
    word attached to it. It moves **the conviction signal and its label. It does not size the
    position** (§3, §12) — that is FIX-1065. §10 pins comparisons rather than literals everywhere
...
12. **Ships as three pull requests: the agreement-honesty fix *including the quorum guard*, then
    ...
    *Locks in:* the quorum guard and its nullable representation move to **PR a**, and PR c carries
    ...
    the pack config, so seating two lenses in PR b re-tunes it from 2-of-3 to 3-of-5 with no further
    edit; and the guard closes a fail-open that exists at **four** seats today — if every
    methodology lens errors and only the skeptic publishes, the aggregate is a one-member set
    scoring `1/1 = 1.000` **convergent**, which should not wait on the valuation work to be fixed.
...
> interval in which PR b has seated six lenses and nothing checks quorum, and in that interval
> three methodology lenses plus the skeptic erroring leaves two survivors scoring `2/2 = 1.000`
> **CONVERGENT** — decision 9a's own worked case, fail-open and upward. The guard is also worth
> shipping on its own: at today's four seats, a lone surviving skeptic already scores `1/1 = 1.000`
> convergent. PR a is correspondingly the large one; that is deliberate, not drift.
...
3. **Add the quorum guard** (decision 9a) ahead of the aggregate, **and the representation it
   ...
   the aggregate when quorum fails. **Do not write `0`** — §7 says why. The threshold is a strict
   majority of the **seated** methodology lenses read from the pack config, never a literal — that
   is what makes PR b re-tune it from 2-of-3 to 3-of-5 with no edit here.
   *Test:* skeptic-only, one-methodology-plus-skeptic, and **two-of-five-seated** all produce a
   ...
   distinguishable from a suppressed one. *Depends on:* 2.
...
6. **Say it in the report, and teach both renderers the null branch** — the convergence strip and
   the summary card show the discount; per decision 6, a cheap-preset report renders a one-line
   "not run at this preset" note where the block would be; below quorum both render a "panel did
   not reach quorum" state with the pill suppressed rather than shown as `divergent`, and the
   individual reads that arrived still render, labelled as individual reads.
   *Test:* discounted and undiscounted reports, the cheap-preset note, and a below-quorum record in
   each renderer — with "did not run" and "did not reach quorum" asserted to read differently.
   *Depends on:* 4.
...
7. **Seat the two lenses.** Two pack entries, two id/agent registry entries, two badge identities,
   ...
   the bundle that FIX-1064 shipped; **and the quorum threshold PR a shipped has re-tuned itself
   to 3-of-5 with no edit in this PR** — the two-of-five-seated case suppresses. That last one is
   the regression that proves the guard was written against the seated pack rather than a literal.
   *Depends on:* FIX-1064 landing, and PR a.
...
|---|---|---|
| a | gap-aware agreement math at `GAP_WEIGHT = 0.5` (agreement + classification only, lean untouched), **unanimity threshold (decision 10)**, stored record, **quorum guard + its nullable representation**, **the two projection guards opened**, every consumer taught both the discount and the null branch (manager block, report disclosure, cheap-preset note, three renderers **+ the PM prompt**) | — |
| b | pack to six, one-figure-path confirmation, quorum re-tunes to 3-of-5 with no edit | a, FIX-1064 |
...
(`orchestration/run-artifacts-action.ts:57-88`). Note the coupling to §8 step 4 (PR a): until that step
...
- **FIX-1064 blocks PR b, and now owns a step this spec used to.** Its decision 5 takes over
  ...
  honest. §8's PR b is re-pointed accordingly and the valuation-exposure step is deleted from this
  spec.
...
**Need your sign-off:** two decisions, hardest first. They were one two-part ask through revision 7
and that ask was false; revision 8 then over-corrected and demoted the second to a setting, which
was false the other way. Both are now stated at the width the arithmetic actually supports, and the
line both of them hold — **this moves the conviction signal and the word attached to it, it does not
size the position** (FIX-1065 does that) — is the one this file has had to correct five times.
