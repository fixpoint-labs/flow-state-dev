...
6. **A cheap-preset report states that the panel did not run, instead of omitting the block.**
   ...
   *Locks in:* a reader can tell "nobody checked the philosophical spread" from "the philosophies
   agreed" — the two look identical today. Costs one line of chrome on every cheap report, and a
   little more than it first appeared: a cheap run currently stores **nothing** about the panel, so
   saying "not run" requires the cost gate to write a one-field marker where it writes nothing now
   (§7). No model spend, no new lens; it is the smallest thing that makes this decision buildable
...
- **Revision 10 — the procedural sections were regenerated from §6 rather than swept for stale
  ...
  - **A fourth state was collapsed into a two-valued channel, and decision 6 was unbuildable.**
    ...
    decision deliverable. This is the epic's most repeated *design* defect and this spec's fourth
    encounter with it; it and the P1 above share one mechanism, so they are fixed once.
