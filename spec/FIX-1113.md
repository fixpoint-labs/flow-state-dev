...
6. **A compatible period is a property of the three statements as a set, checked once before
   anything is computed from them — and when they don't share one, every output that reads
   more than one statement is withheld, up to and including the rating.** "Agree" means the
   same fiscal year-end, not the same string. Each statement is fetched through its own
   provider ladder, so income can come from the filings source while the balance sheet falls
   through to market data, and nothing compares their dates before anything is built from
   both.
   ...
   **Three of the four surfaces are gated deterministically; the fourth is advisory and that
   is a residual, not a guarantee.** The two mappers and every computation over the statement
   ...
   *Locks in:* **on a company whose sources disagree about the period, the desk declines to
   rate it.** Not just the multiples — the rating envelope is computed from an expected
   return that reads all three statements, so withholding cross-statement outputs withholds
   the headline the report exists to deliver. That is the largest single behaviour change in
   this spec: a report that today shows a rating will show none, and the spine reading thin
   means the exposure gate caps new positions on that name. The alternative was to publish a
   rating built on a year of earnings over a different year's equity, which is the thing this
   issue exists to stop. **And the guarantee stops at the model's doorstep:** the computed
   ...
   being told when a memo carries a figure the spine withheld. A bound set too tight silently drops those figures on
...
> set, never what it calculates when it does.
>
> **Size:** Large — 14 production files · 28 test behaviours · 2 documentation surfaces ·
> 1 PR. *(Up from 8 / 14 across three review rounds: the cross-provider layer in decision 6,
...
> rating envelope inside the guard.)*
>
...
**Four surfaces, and they do not carry the same strength of guarantee. Say which is which.**
...
| 3. Every spine computation reading the statement set — **including the rating envelope** | **Deterministic** — cross-statement outputs withheld on mismatched periods, and the spine reads thin |
...
ONE LAYER UP — where the three statements meet:   (decision 6)
    ...
    decide ONCE, over the SET, before anything is computed from it:
        compatible ← all three statements share a period
    ...
    then every computation that reads more than one statement consults it:
        ...
        the expected return                        ← withheld
          …and everything downstream of it: fair value, the cash-flow model,
            triangulation, the setup score, THE RATING          ← withheld
...
7. **The period-compatibility contract on the statement set** (decision 6). Three parts, in
   order. **(a)** The equivalence rule itself, pure and standalone — a bounded distance, not
   a calendar-year comparison. **(b)** One compatibility determination over the three
   statements, made where they are assembled, not inside any computation. **(c)** Consulted
   by everything that reads more than one statement: the spanning multiples, **the expected
   return and everything downstream of it including the rating envelope**, and the spine's
   sufficient/thin signal, which must read thin when outputs were withheld. A figure inside
   one statement is untouched.
   ...
   *Test:* the equivalence rule in both directions (far apart within one calendar year; near
   across a year boundary); the withheld set **asserted at the rating, not only at a
   multiple**; a single-statement figure surviving; the thin signal; and the cross-provider
   cases at the **analyst** layer, not only the spine. *Depends on:* 5.
8. **The analyst's raw-statement path — advisory, and the only step here that is.** The half
...
| **Assembled-spine check** (zero model, CI) | **Yes** — outcome level | That the **assembled** spine withholds its cross-statement outputs — *the rating included* — on mismatched periods and still produces them on matched ones. Neither mapper in isolation shows that |
...
- **mismatched periods → the cross-statement outputs are withheld, the rating included, and
  the spine reads thin**, and
...
- **Decision 6 asserted at the rating, not only at a multiple — and this is the criterion
  most likely to be written wrongly.** On mismatched periods the rating envelope is absent,
  the expected return is absent, and the spine reads thin; on matched periods all three are
  produced. **A suite that asserts only "the spanning multiple is withheld" passes against
  exactly the defect this exists to catch**, because the rating is computed from an expected
  return that reads all three statements and never passes through the valuation function. Add
  the inverse too: a figure that reads a single statement is produced in *both* cases, so
