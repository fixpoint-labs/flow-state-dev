...
6. **When the three statements come from different providers, no figure is built across
   ...
   same string. That holds at every place a cross-statement figure is produced, which is two
   places, not one.** Each statement is fetched through its own provider ladder, so income
   can come from the filings source while the balance sheet falls through to market data, and
   nothing compares their dates before a ratio is built from both. **The check goes in the
   shared computation both places already call, not on the stored result** — the fundamentals
   analyst computes and publishes its own valuation from its own tool payloads, *before* the
   stored spine exists, so a rule enforced on the spine never reaches it. And because that
   analyst is also handed the three raw statements and asked in its own instructions to
   divide one by another, the periods have to be visible to it and a mismatch stated to it,
   not merely gated in code it does not run.
   ...
   *Locks in:* a cross-statement figure — return on equity, return on capital, leverage,
   ...
   getting a plausible wrong one. A bound set too tight silently drops those figures on
   healthy companies; set too loose it lets back in the defect this issue exists to fix. **This
...
> sites share one check rather than making one of them go away.
>
> **Size:** Large — 13 production files · 24 test behaviours · 2 documentation surfaces ·
> 1 PR. *(Up from 8 / 14 across two review rounds: the cross-provider layer in decision 6,
> the fixture replay boundary, the unavailable-path representation in decision 5, and — round
> 2 — the analyst's own valuation site and its raw-statement path, which no spine-side gate
> reaches.)*
...
flowchart TB
  ...
  L --> X["shared valuation computation<br/>+ cross-statement period check<br/>(decision 6)"]
  L --> D["raw statements, as a data block<br/>each carrying its own period"]

  X --> AN[fundamentals analyst memo<br/>runs FIRST]
  D --> AN
  X --> SP[valuation spine → rating]
  SP --> C3[stored report]
...
**The two consumers are the reason the check sits where it does.** The analyst runs *before*
the spine is written and computes its own valuation from its own tool payloads, so a rule
enforced on the spine is invisible to it. Both consumers already call the same shared
valuation computation, so that function is the one place both pass through — putting the
check there covers both without inventing a seam, which is this epic's rule about fixing
drift by sharing the code the sites disagree through rather than patching each site.
...
**The shared computation does not cover the whole exposure, and the remainder is not code.**
...
ONE LAYER UP — where the three statements meet:   (decision 6)
    ...
    same period?  ←  |end A − end B| within a few weeks
                     NOT "same calendar year": that merges a January year-end
                     with a December one, and splits 2025-12-28 from 2026-01-03

    in the shared valuation computation, which BOTH consumers already call:
        if the periods are not the same:  the spanning figures report nothing
...
7. **The cross-statement period check** (decision 6), in **two** places. First the period
   equivalence rule itself, pure and standalone — a bounded distance, not a calendar-year
   comparison. Then wire it into the **shared valuation computation** that both the spine tap
   and the fundamentals analyst already call, so a single change covers both consumers.
   Read `loadStatementWithRecovery`, the spine-compute tap, and the fundamentals generator's
   context slot first, and **check FIX-1064's spec for the shared surface it is specifying at
   the same site** so the two converge rather than each adding a seam. Do **not** change the
   ladder's order or its fallback conditions. *Test:* the equivalence rule in both directions
   (far apart within one calendar year; near across a year boundary), and the cross-provider
   cases at the **analyst** layer, not only the spine. *Depends on:* 5.
8. **The analyst's raw-statement path** — the half no computation seam reaches. The three
   statements reach the model as a data block and its instructions ask it to divide one by
   another. The periods are already in the payloads after step 5; surface a mismatch in the
   analyst's context and say in the instructions not to combine across it. *Test:* an analyst
   run on mismatched periods carries the mismatch in its context, and one on matched periods
   does not. *Depends on:* 7.
9. **Count the movement — narrative, not a gate.** Run the fixture corpus before and after
   ...
   for it — the vitest behaviours and the goal check are the gates. *Depends on:* 6, 8.
...
| **Goal check** (zero model) | **Yes** — outcome level, through the real run | That a real analysis on a mismatched filing *withholds* the cross-period figures and one on a matched filing *still publishes* them. Neither the mappers nor the analyst in isolation shows that the assembled run does the right thing |
| **Movement count** (§8 step 9) | **No — narrative only** | Nothing. It is a description of the blast radius for a human reader |
...
deliberately. The outcome is deterministic (given a filing, does the run publish a figure
spanning two periods?), so a model in the loop would add noise without adding evidence; the
existing goal check in this directory is model-backed because its subject is transcription,
which this is not.

**It drives the real run, not the mappers.** `fsdev run` over the analysis flow against a
fixture snapshot, then the artifacts read-back action, asserting on what the assembled run
actually published. Re-running the mappers here would duplicate the vitest layer while
proving nothing about the integrated path — and the integrated path is exactly where the two
consumers, the ladder, and the analyst's own computation live.

**Its anti-game arm is a pairing, not a comparison against the old code**, and that is a
correction: an arm that runs "the same check against the pre-change mappers" is not runnable
on a clean post-change checkout, because steps 3–4 delete those selectors and this section
forbids keeping a copy. A pass condition that is executable today and not tomorrow is the
same defect this section is about, one level up. The durable form is two arms over the same
run path:

- **mismatched periods → the cross-period figures are withheld**, and
- **matched periods → those same figures are still published.**

A gate that withholds everything fails the second arm; a gate that never fires fails the
first. Both remain runnable forever, because both are statements about outcomes on data
rather than about code that no longer exists. The red-before-green evidence — that today's
mappers *do* mix periods on F1 and F3 — belongs in the vitest layer, where the TDD sequence
produces it naturally and where it does not have to survive a deletion.
...
- **The analyst's raw data block carries each statement's period, and states a mismatch.**
  The model is instructed to divide across statements, so this is a context assertion, not a
  computation one: the periods are present in the block, and on a mismatch the context says
  so. Cheap to assert and the only coverage of the half no code gate reaches.
...
- **FIX-1064** is blocked *by* this and should be re-based on the aligned figures once this
  ...
  **It is also specifying a shared computation and formatter at the same analyst call site
  this spec's decision 6 targets** (its decision 5, where its sector-cohort guard lives).
  Read that spec before building step 7 and put the period check on the surface it names —
  one shared seam carrying both guards, not two.
