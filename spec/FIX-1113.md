...
| 6 | Compatible periods are checked **once over the three statements as a set**, at the newest period observed. On mismatch, every cross-statement computation is withheld | **The rating still publishes — unclamped and marked unanchored.** Withholding the envelope removes the *bound*, not the rating. See the question below |
...
**Two limits worth knowing before you sign off**, both deliberate and both named in full below:
the desk's period guarantee covers the providers it actually consulted, not every provider
that exists (decision 6, priced in §12); and the analyst's own prose arithmetic is
...
6. **A compatible period is a property of the three statements as a set, checked once before
   ...
   **Agreement alone is not the property — uniform staleness satisfies it.** The ladder
   prefers a *complete* statement over a *partial* one regardless of period, so a filings
   statement that is missing one critical field loses outright to a market-data statement
   from an older year. When all three fall back that way they agree perfectly, at the wrong
   year, and a check written as "do the periods match" passes while the report silently walks
   backward — contradicting decisions 1 and 3, which require the newest anchor with missing
   fields left absent. So the contract is *the newest annual period **observed among the
   providers the ladder actually consulted***, and a whole-statement replacement that moves a
   statement to an older period is refused; a partial statement at the anchor is preferred
   over a complete one behind it.
   **"Consulted" is doing real work in that sentence, and the weaker claim is the honest
   one.** The ladder returns the first provider payload that is complete on the critical
   fields *without fetching the next provider at all* — verified, not assumed
   (`statement-recovery.ts`: the filings payload short-circuits the return before the
   market-data fetch is issued). So when the filings source answers completely at an older
   year, the market-data source's newer annual period is **never observed**, all three
   statements short-circuit the same way, they agree at that older year, and the set
   satisfies the anchor check. The desk cannot claim *the subject's* newest period, because
   it does not look at every provider before choosing. It can claim *the newest period it
   saw*. Claiming the first while implementing the second is this issue's own defect one
   level up, so the guarantee is stated at the level the code supports.
   **The narrowing must not become a hiding place, and this is the part to get right.** It
   weakens only the case where a later provider was *never fetched*. It changes nothing about
   the case where the ladder **did** observe a newer period and settled on an older one — an
   older complete statement beating a newer partial one is still a violation, still refused,
   and still required to fail. One predicate covers both: *every statement sits at the newest
   annual period observed while resolving the set.* Uniform staleness reached by
   short-circuit passes it (a limit we disclose); uniform staleness reached by preferring
   completeness over recency fails it (the defect we fix).
   *Rejected:* inspecting every provider candidate before choosing the anchor. It closes the
   short-circuit case completely, and it spends an extra provider request on **every
   statement of every run** to do it — against a case whose frequency nobody has measured,
   for a source (the authoritative US filing source) that lagging the market-data source is
   the exception rather than the rule. The epic's fence already excludes new requests, and
   lifting it on an unmeasured frequency buys a certain cost for an uncertain benefit. It is
   also cleanly additive later: nothing in the narrowed wording blocks grounding the anchor
   across providers if the case turns out to be common. §12 records what would justify it.
...
> change is a preference, not a reordering** — an older whole-statement replacement is refused
> in favour of the partial statement at the anchor; the providers tried and their order are
> unchanged.
>
> **Size:** Large — 16 production files · 34 test behaviours · 2 documentation surfaces ·
> 1 PR. *(Up from 8 / 14 across four review rounds: the cross-provider layer in decision 6,
...
> rating envelope inside the guard; and — round 4 — anchor preservation through the provider
> ladder, and disclosing the unanchored rating once it turned out the rating is not withheld
> at all.)*
>
...
So the compatibility determination is against **the newest annual period observed while
resolving the set**, and a whole-statement replacement that moves a statement backward is
refused in favour of the partial statement at the anchor — which is what decisions 1 and 3
already say the payload should look like.

**And the ladder has a second, quieter way to land on a stale set: it stops early.** A
provider payload that is complete on the critical fields returns *before the next provider is
fetched*, so a newer annual period at a later provider is never observed at all. All three
statements can short-circuit that way, agree at the older year, and satisfy the check. That
case is **out of the guarantee by decision**, not by oversight: closing it means fetching
every provider on every statement of every run, which the non-goals exclude and which §12
prices. The two staleness routes are worth holding apart while building — *observed a newer
period and chose an older one* is a defect the check must catch, *never fetched the newer
provider* is a limit the wording admits. Both are the same predicate; only one is reachable.
...
ONE LAYER UP — where the three statements meet:   (decision 6)
    ...
    anchor ← the newest annual period OBSERVED while resolving the set
             (not whatever the statements settled on)
             ← NOT "the newest period the subject reports". the ladder
               returns a complete payload WITHOUT fetching the next
               provider, so a newer period at a later provider is never
               seen. that case is outside the guarantee, on purpose
               (decision 6). the check still fails when a newer period
               WAS observed and an older statement was chosen anyway

    refuse, in the ladder, any whole-statement swap that lands BEHIND the
    anchor — prefer the partial statement AT the anchor
        ← without this, a complete older statement beats a partial newer one
          and all three drift back together

    decide ONCE, over the SET, before anything is computed from it:
        compatible ← every statement sits at the ANCHOR (as observed above)
                     NOT "all three agree with each other" — three statements
                     that agree at the WRONG year pass that and the report
                     has silently walked back a year
...
7. **The period-compatibility contract on the statement set** (decision 6). Four parts, in
   ...
   a calendar-year comparison. **(b)** **Anchor preservation through the ladder:** refuse a
   whole-statement replacement that moves a statement to an older period than the newest
   annual one **observed while resolving the set**; prefer the partial statement at the
   anchor. Without this the rest is satisfied by uniform staleness. Note what (b) does *not*
   reach: the ladder short-circuits on a complete payload without fetching the next provider,
   so a newer period never observed cannot become the anchor — that case is outside the
   guarantee by decision 6, and §12 carries what would justify closing it. Do not quietly
   close it here by adding a fetch. **(c)** One compatibility determination over the three
   ...
   *Test:* the equivalence rule in both directions (far apart within one calendar year; near
   across a year boundary); **uniform staleness rejected**; a partial statement at the anchor
   beating a complete one behind it; the withheld set **asserted on the published rating, not
   only on the envelope**; a single-statement figure surviving; the thin signal; and the
   cross-provider cases at the **analyst** layer, not only the spine. *Depends on:* 5.
...
| Filings statement at the anchor missing one critical field, market-data statement complete at the prior year | The anchor-year statement wins, with the field absent (decisions 1 and 3). The older complete statement is not substituted |
...
- **A partial statement at the anchor beats a complete one behind it.** Assert the choice
  directly: filings statement at the newest year missing one critical field, market-data
  statement complete at the prior year → the anchor-year statement wins with the field
  absent (decisions 1 and 3), and the older complete one is not substituted. This is the
  *observed-a-newer-period* route, and it must fail closed.
- **A later provider holding the newest period — the narrowed guarantee, pinned in both
  directions.** The ladder returns a complete payload without fetching the next provider, so
  the two routes to a stale set need separate cases and a suite that conflates them will
  either miss the defect or chase the limit:
  - **Never fetched → accepted, and asserted as such.** A complete filings statement at the
    older year with a market-data source that *would* have reported a newer annual period:
    the set passes, the outputs are produced, and the test says in its name that this is the
    documented limit of decision 6. Asserting the passing behaviour is what stops a later
    reader from "fixing" it into an extra fetch without pricing it (§12).
  - **Fetched and observed → rejected.** The same newer market-data period, but reached on a
    statement whose filings payload was incomplete so the ladder *did* fetch it: choosing the
    older complete payload is now a violation and the cross-statement outputs are withheld.
  The pair is the whole point — same data, different ladder path, opposite verdicts. It is
  also the regression fence on the narrowed wording: if someone widens "observed" into
  "reported by any provider", the first case flips and this test says so.
...
- **The anchor is grounded only in the providers the ladder consulted — and here is what
  would justify changing that.** The ladder returns the first payload complete on the
  critical fields without fetching the next provider, so a newer annual period at a later
  provider is never observed and the set can sit, truthfully by its own lights, a year
  behind. Decision 6 narrows the guarantee to match rather than buying the fix, and the
  price is why: closing it means fetching **every provider for every statement on every
  run** — three extra provider calls per subject where today's ladder usually makes zero, on
  a path that already carries a request fence in the non-goals. **The number that would flip
  it** is how often the authoritative filing source is complete-but-a-year-behind a
  market-data source that has the newer year. Nobody has measured it, and the direction of
  the guess is that it is rare — the filing source is the authoritative one for US issuers
  and is normally *ahead*, not behind. **How to get the number without paying for it:** the
  run marker decision 6 already adds records when the guard fires; extending it to record
  the anchor's provenance — which provider supplied the anchor, and whether the ladder
  short-circuited before consulting another — makes the frequency observable from ordinary
  runs at no request cost. If it turns out common, grounding the anchor across providers is
  clean and additive: nothing in the narrowed wording blocks it. Recorded here as a **priced
  decision, not an oversight** — the alternative was a certain per-run cost against an
  unmeasured benefit.
