...
6. **A compatible period is a property of the three statements as a set, checked once before
   ...
   **Agreement alone is not the property — uniform staleness satisfies it.** The ladder
   ...
   fields left absent. So the contract is *the newest annual period the subject reports*, and
   a whole-statement replacement that moves a statement to an older period is refused; a
   partial statement at the anchor is preferred over a complete one behind it.
...
So the compatibility determination is against **the subject's newest annual period**, and a
whole-statement replacement that moves a statement backward is refused in favour of the
partial statement at the anchor — which is what decisions 1 and 3 already say the payload
should look like.
...
ONE LAYER UP — where the three statements meet:   (decision 6)
    ...
    anchor ← the newest annual period the SUBJECT reports
             (decision 1's anchor, not whatever the statements settled on)
    ...
    decide ONCE, over the SET, before anything is computed from it:
        compatible ← every statement sits at the ANCHOR
...
7. **The period-compatibility contract on the statement set** (decision 6). Four parts, in
   ...
   whole-statement replacement that moves a statement to an older period than the subject's
   newest annual one; prefer the partial statement at the anchor. Without this the rest is
   satisfied by uniform staleness. **(c)** One compatibility determination over the three
...
| All three statements fall back to the *same older* period | **Rejected** — the check is against the subject's newest anchor, not mutual agreement. Uniform staleness is the case a naive compatibility test passes |
...
- **A partial statement at the anchor beats a complete one behind it.** Assert the choice
  ...
  absent (decisions 1 and 3), and the older complete one is not substituted.
