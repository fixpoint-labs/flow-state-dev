...
- **Output re-parses as input → a runtime refusal, not an availability condition.** The verbs
  **are** offered on a type-changing transform, and **every call refuses non-retryably** via
  the schema-invalid-result rule below. It cannot be a compile-time condition: both handles
  carry only the schema's **output**, where a transforming schema is structurally
  indistinguishable from an ordinary one (§10 17b; §12 records the rejected alternative and
  what would reopen it).

**Say the weaker promise out loud rather than letting a reader infer the stronger one.** For
an open schema the guarantee is *absence*; for a transforming schema it is *a loud, typed,
terminal refusal on first call* — never silent drift, never row 11's destructive replacement.
Both keep `updateState`, which has the same constraints and never promised otherwise. **§11
must publish the second as a refusal, not as an exclusion.**
...
| `updateStateWith(ref, …)` | transparent forwarder to `ref.updateState` (`core/src/helpers/update-state-with.ts:146`) |
...
4. **Add a Changeset** (BP-022). Four new public methods on a publishable package plus a
   ...
   **`getOrPatchState`'s miss path** and **`ref.edges.*`** now fail fast instead of retrying.
...
| Either verb on a resource whose schema has a **type-changing** transform (`z.string().default("0").transform(Number)`) | **Offered, and refuses non-retryably on every call** — `ValidationError`, state unchanged. *Not* an availability exclusion: the handles carry only the schema's output, where such a schema is indistinguishable from an ordinary one, so this is enforced at the write and not at the call site (§7; §10 17b; §12 for the rejected alternative). The resource keeps `updateState` |
...
17b. **on a type-changing transform the verbs ARE offered, and every call refuses
    non-retryably** — a **runtime** behaviour test, not a type test, and the difference is the
    promise, not the plumbing. `z.object({ n: z.string().default("0").transform(Number) })`
...
24. **the new read-only semantics hold on every pre-existing entry point that reaches them,
    ...
    **non-retryably**, asserted by **attempt count**. Testing this only on `incState` /
    ...
    assertion that it stays a plain read), and **`ref.edges.add` / `.supersede` / `.remove`**
    on a resource declaring an `edges` slot. Two negative controls belong here, because they
...
is 5–10, 16 and **17b**, plus the `writable: false` refusal (§7's table) — and re-derive it
whenever a behaviour is added, rather than trusting this sentence's parenthetical to have kept
up.
...
**Every behaviour in that same set also asserts the resource's other fields survive** —
derived by the same predicate, for the same reason. The
...
implementation that threw *after* the reset landed. Behaviour 9's string-delta case is the
live proof that this is not hypothetical: on `main`'s path it resets the row and reports
success today.
...
- **EXTEND** `docs/architecture/state-and-scopes.md` — the internal contract reference, and
  ...
  **Also carry the read-only refusal's reach**, since this is the page an implementer checks
  ...
  misses and computes a value**, and for **`ref.edges.*`** — but not for content writes, and
...
- **And the schema-transform case is a *different promise* — publish it as one.** This is the
  ...
  present, and every call fails** with a terminal validation error that leaves the stored value
  untouched. Say the second as a **refusal**, never as "not supported" or "excluded" — a reader
  who takes it as absence will conclude their code could not have compiled, and it does. One
  sentence on why, because it is the honest version and it is short: the handle is typed from
  what the schema *produces*, which cannot show what it *accepts*. Do not promise a future
  compile-time check; §12 holds the condition under which that would be revisited.
...
offered and every call refuses non-retryably** (§7; §10 17b). It costs no implementation — the
refusal is §7's already-planned schema-invalid-result rule — and the safety is unchanged: the
call is terminal and the stored state is untouched.
...
execution. The guarantee is that they learn **loudly** — a typed, non-retryable
...
- **Behaviour 17 asked for a test that cannot be written; settled as a runtime refusal.** Both
  ...
  excluded at compile time. **Settled: the verbs stay offered and every call refuses
  non-retryably**, via a rule §7 already carried. The rejected alternative — carrying the
