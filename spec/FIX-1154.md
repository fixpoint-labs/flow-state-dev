...
**The two halves are not equally enforceable, and round 14 established which is which.** The
first is a type test that works. The second **cannot be** — both handles carry only the
schema's output, where a type-changing transform is indistinguishable from an ordinary schema
— so "unavailable" is currently a promise the type system cannot keep. **§12 carries this as
the spec's one open question**; §10 17b is blocked on it. The paragraph above is unchanged
pending that answer.
...
17b. **BLOCKED — the type-changing-transform half of §7's availability rule cannot be tested
    the way this list asked.** An earlier draft required it as a **compile-time** test. It
    cannot pass: both handles are built from the schema's **output**
    (`StateOf<T> = S["_output"]`, `resource.ts:381-385`; `ContextOf` feeds that into
    `ResourceContext`, `:407-413`), and the output of
    `z.object({ n: z.string().default("0").transform(Number) })` is **structurally identical**
    to `z.object({ n: z.number() })` — verified by type-level `Equal<>` assertion in round 14,
    along with the same identity for the coercing, defaulting and same-type-transform cases,
    which is also *why* 17 and 17a can require those to stay offered without a special case.
    **Do not build this until §12's open question is answered** — the two candidate
    resolutions differ in what the spec promises, not just in how it is checked
...
is 5–10 and 16, plus the `writable: false` refusal (§7's table) — and re-derive it whenever a
behaviour is added, rather than trusting this sentence's parenthetical to have kept up.

That instruction is written this way because the numeric form has now gone stale **twice**,
the same way both times. The range `5–9` was written in round 9; behaviour 10 arrived in
round 11 carrying the JSON-unsafe `pushState` class, and nothing re-checked the round-9
...
one round after the lesson was written down.
...
**Open questions: one, raised in round 14 and not decided here.**

**Can a caller be stopped at compile time from calling these verbs on a type-changing
transform, or only told at runtime?** §7 states availability as *"output type that is valid
input"*, and §10 behaviour 17 asked for that as a **type test**. It cannot be written: both
handles carry only the schema's **output**, and a type-changing transform's output is
structurally identical to an ordinary schema's (verified, round 14 — see 17b). The two
resolutions are not the same promise:

- **(a) Carry the schema's input type through `ContextOf` and `ResourceRef`.** Expressible in
  principle — zod 3.25 exposes `_input`. **But three verified problems.** A naive input-vs-output
  rule **wrongly excludes a defaulting schema** (`z.number().default(0)` has an optional input),
  which is the case behaviour 17 explicitly requires to stay offered — so the comparison must be
  per-key and default-aware, not a type swap. It still would **not** catch the **same-type**
  transform (input equals output there), so it buys only part of the class and 17a stays pinned
  regardless. And it is **vacuous where the verbs are consumed**: all 22 in-tree
  `ResourceContext<…>` annotations are hand-written against a plain state interface (15 of them
  behind an `as` cast off `ctx.resources` — `memory/src/capabilities.ts:68`,
  `memory/src/internal/recall.ts:48`, `thought-fabric-core/src/identity/perspective-helpers.ts:23`
  among them), so a defaulted second parameter reads as "available" at every one of the consumer
  files §8 step 1 names. The guarantee would hold only where the type flows from
  `ContextOf<typeof resource>` — which is none of them today.
- **(b) Drop the compile-time promise for this half and state the limit plainly.** The verbs
  stay offered on such a schema, and every call **refuses non-retryably** — the candidate is
  re-parsed, fails, and hits the *already-planned* "schema-invalid result" refusal in §7's
  table. So the residual behaviour is loud, typed and terminal, not the silent
  default-replacement §7 warns about, and it costs **no new implementation** — behaviour 17's
  second half converts from a type test to a runtime one. What the spec loses is §7's line that
  such an API "is worse than one that does not exist"; that claim would have to be re-scoped to
  the passthrough half, which does hold.
...
**This is a promise, not a mechanism, so it is the owner's call and not the implementer's.**
Until it is answered, §7's availability rule stands as written and 17b is blocked.
...
- **`writable: false` is not enforced on collection instances at all.** Instance writes persist
  through `persistNamespaceInstanceState`, which never consults the flag
  (`resource-registry.ts:670-684`), so a read-only *collection* accepts every write today.
  Pre-existing and out of this issue's scope — the sweep above surfaced it while closing the
  enumeration. **Wants its own issue**, and it is the more serious of the two.
...
- **Closing the enumeration found two defects the sweep was not looking for.** Asking what
  ...
  stop being, and that `writable: false` is **not enforced on collection instances at all**.
  Both filed in §12, neither fixed here. A sweep that only extends a list cannot find either.
- **Behaviour 17 asked for a test that cannot be written, and the fork is the owner's.** Both
  ...
  excluded at compile time. Verified by type-level assertion rather than argued: the same
  probe also showed that carrying the schema **input** would *wrongly exclude a defaulting
  schema* — the case behaviour 17 requires to stay offered — and would still miss the
  same-type transform. **The decisive finding was not cost but reach:** all 22 in-tree
  `ResourceContext<…>` annotations are hand-written against a plain state interface, so an
  input-carrying parameter would default to "available" at every consumer file, and the
  compile-time guarantee would hold nowhere it is consumed. Recorded as §12's open question,
  not decided — the two branches promise different things to a caller.
