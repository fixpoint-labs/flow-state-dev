...
One row is the headline, and review has now corrected it three times. The verbs that look
...
- **Cost.** A resource write carries the **whole value**: appending one item reads the
  resource's entire state, rebuilds the list, and writes all of it back. The bag's native
  append sends only the appended item and leaves the rest of the record alone. **The copy
  itself is not the difference** — every shipped native append rebuilds the array too, so
  appending is proportional to the existing list on both sides. What differs is how much
  travels per write and how much of the record is rewritten. The practical consequence is
  the same either way and it is about *repetition*: appending in a loop means N whole-state
  writes, so the docs steer bulk appends to one compound write.
...
this spec **drops** a tier of work the epic had deferred, **splits out** two unrelated
...
2. **We are not building store-native increment and append for resources, and it comes off
   ...
   described as one. It does not remove the per-append copy either — every shipped native
   append rebuilds the array. What is left is a smaller, real benefit: less data per write,
   and unrelated fields not rewritten.
   *Rejected:* leaving it deferred, which reads as "a contention fix is coming" to anyone
   planning against it.
   *Locks in:* resource writes keep sending the whole value. **Reversing this is a genuine
   piece of work, not an additive tweak** — see the note below, because this spec has now
   twice said otherwise. The bill for changing our minds lands on whoever needs it, and it
   is a storage-contract change plus a per-adapter implementation, so this is a decision
   worth making deliberately now rather than drifting into.

   > **Which interface bears that cost — the substitution that keeps happening.** The
   > optional, feature-detected delta verbs (`patchField?` / `incField?` / `pushToArray?`
   > on `DeltaStoreOps`) belong to the **scope-record** stores: `SessionStore`,
   > `RequestStore`, `UserStore` and `OrgStore` each `extends DeltaStoreOps`
   > (`stores/types.ts:516, 551, 772, 784`). **Resources do not go through any of them.**
   > Resource state persists via `ResourceStateStore` (`stores/types.ts:1036`), which
   > extends nothing and whose entire mutation surface is `set`, `delete` and `deleteAll` —
   > all whole-state. So "the contract already makes these optional" is true of the scope
   > side and **does not transfer**: native resource verbs mean extending
   > `ResourceStateStore` and implementing them in every adapter that wants them, plus the
   > conformance suite. Reason about this from `ResourceStateStore`, never from the
   > scope-side precedent — that substitution has produced two wrong answers already.
...
**Size:** Medium — 2 production files · 14 test behaviours · 4 doc surfaces · 1 PR.
...
exactly as often as a whole-value write does. It does not buy back the per-append copy
either: every shipped native append rebuilds the array. What it buys is bandwidth and a
smaller blast radius per write — not conflicts, and not asymptotic cost. And it is bought
at the price of extending `ResourceStateStore` and implementing it per adapter, which is
the opposite of the additive change an earlier draft of this spec twice claimed it was.
...
- **A non-finite delta, or a non-finite computed result** — `Infinity`, `-Infinity`, or a
  sum that overflows there → **throw**, naming the call site. This one needs its own rule
  because the schema check above **does not catch it**.
...
**Why non-finite needs a rule of its own, and why it is the worst of the three.** A plain
`z.number()` **accepts** `Infinity` and `-Infinity` — only `NaN` is rejected — so
`incState({ n: Infinity })`, and any increment that overflows to it, passes the schema check
above and is written. Then the adapters disagree: the in-memory store clones with
`structuredClone` and **keeps** `Infinity`, while the SQLite and filesystem resource stores
`JSON.stringify` it (`filesystem/resource-state-store.ts:83`), which turns it into **`null`**.
On the next durable read that `null` fails `z.number()` — and *that* failure lands in the
destructive fallback above, resetting the whole resource.

So this defect does not surface at the write that causes it. It surfaces later, in a
different request, as unrelated fields silently disappearing, and only on some adapters.
That is why finiteness is checked at the call site rather than left to the schema: it is the
one input that converts a caller's arithmetic mistake into deferred, adapter-dependent data
loss. A resource whose schema declares `.finite()` would be caught by the schema check, but
nothing makes that the default and no existing resource does it.
...
on the resource handle, beside patchState:
    ...
    incState(increments):
        reject non-finite deltas up front → throw
        hand the write path a mutator that, for each field:
            absent        → start from zero, add the delta
            not a number  → throw, naming the field
            otherwise     → add the delta, then
            non-finite    → throw   (overflow — the schema will NOT
                                     catch this; z.number() accepts Infinity)
...
| 10 | **Guarantee and cost of the verbs they share** | **Guarantee:** a write skips the version check — and so cannot be refused — only when it emits a commutative hint **and** the adapter implements the matching delta verb. Both conditions are narrow: `incState({a:1,b:1})` emits no commutative hint and takes checked CAS, and the delta verbs are **optional on the store contract**. Every adapter we ship implements them. **Cost:** a native delta verb sends only the delta and rewrites only that field — but still **rebuilds the array** (memory and SQLite spread it, Postgres concatenates into a new JSONB value), so appending is proportional to the existing list here too | **Guarantee:** every write is version-checked and can raise a conflict or a deleted-resource error. No exceptions and no adapter can change it. **Cost:** every write sends and rewrites the whole state. **Not** an asymptotic difference from the bag — both rebuild the list — but more data per write and a wider blast radius, and appending in a loop means N whole-state writes | **Deliberate and permanent on the resource side** — resources have a lifecycle and bag records do not, so a check-free write to a resource could resurrect a deleted one; decision 2 locks in the whole-value cost. **The bag side is doubly conditional and the conditions must travel with the claim.** No caller-facing doc states either half today. The sharpest row, and the easiest to publish wrongly |
...
| `incState` whose sum **overflows** to `Infinity` (e.g. adding to `Number.MAX_VALUE`) | **Throws.** Same path as above, reached by arithmetic rather than by the argument |
| `incState({ n: NaN })` | **Throws** — caught by the schema check, since `z.number()` does reject `NaN`. Listed because it is the one non-finite value that behaves differently from the others |
...
6. **a schema-refinement violation that keeps the right JavaScript type throws** — e.g.
   ...
   the reset landed
7. **non-finite deltas and non-finite results throw** — `Infinity`, `-Infinity`, and an
   **overflow** case that reaches `Infinity` by arithmetic rather than by argument. Each
   asserts the resource's other fields survive, as in behaviour 6. Cover `NaN` too, and note
   in the test why it is the odd one out (the schema rejects it; the others it accepts).
   **These must not be written as schema-check tests** — the whole point is that the schema
   passes them
8. multi-field increment on a resource behaves as any other resource write
9. concurrent increments from two contexts converge
10. a verb racing a delete is terminal
11. a no-change verb is a verified no-op that does not bump the version
12. **a no-change verb emits no `resource_change`** — on a **collection instance** and on a
    ...
    *because* the notification is gated on it, and the two ref builders each carry their own
    ...
    the other
13. a `ResourceContext`-typed value exposes `incState`, `pushState` **and**
...
14. `updateState`, `patchState` and `setState` behave byte for byte as today

**Constraint on graduating rows 9–11 from the POC:** the POC models the verbs as wrappers
...
- **Non-finite numbers survive a resource write on some adapters and become `null` on
  others.** `z.number()` accepts `Infinity`; the in-memory store clones with
  `structuredClone` and keeps it, while SQLite and filesystem `JSON.stringify` it to `null`
  (`filesystem/resource-state-store.ts:83`). The `null` then fails the schema on the next
  read and triggers the reset below — so one bad write corrupts a *different* request, on
  *some* deployments. The new verbs refuse non-finite values (§7), but every existing write
  op still accepts them. **Wants its own issue**, and it is the sharpest of the three filed
  here because the damage is deferred and adapter-dependent.
...
- **After spec review, round 2** — split two unrelated cleanups out of §8 (the driver's
  ...
  named trigger. *(Both halves of this — the downward correction and the cost trigger — were
  themselves falsified in the next pass. See the entry below; do not act on this line.)* Row 5's "same op under two names" was **replaced, not refined** — the bag
...
- **Factual correction after convergence** — row 11 described per-write schema handling as
  ...
  **throw** on any schema-invalid result rather than entering that path; the three shipped
  ...
  §6 did.
- **Second factual pass after convergence** — three corrections, two of them to earlier
  corrections. **(a)** The schema check added above does not catch **non-finite** numbers:
  `z.number()` accepts `Infinity` and `-Infinity` (only `NaN` is rejected), and the adapters
  then disagree — memory keeps the value, SQLite and filesystem serialize it to `null`, which
  fails the schema on a *later* read and triggers the reset. The verbs now refuse non-finite
  deltas and results explicitly. **(b)** The per-append **cost** claim added in round 2 was
  wrong: every shipped native append rebuilds the array too, so native routing reduces
  payload and blast radius but not asymptotic cost. Restated, and decision 2 lost a benefit
  it never had. **(c)** The round-2 "reversal is cheap and additive" correction was itself
  wrong, and the original claim was closer to right: the optional feature-detected delta
  verbs live on `DeltaStoreOps`, which the four **scope-record** stores extend and which
  **resources never touch** — resource state goes through `ResourceStateStore`, whose whole
  mutation surface is `set` / `delete` / `deleteAll`. Reversing means extending *that*
  contract per adapter. Decision 2's answer is unchanged; its supporting argument now rests
  on the one premise that has survived every round — native verbs remove no conflicts on a
  version-checked write — and the reversal cost reads honestly as expensive.
