...
**Size:** Medium — 2 production files · 23 test behaviours · 7 doc surfaces · 1 PR + changeset.
...
holds. What `z.custom` does *not* give is a meaningful `safeParse`, so the schema-invalid
refusal degrades to whatever that custom validator checks — **the refusal is only ever as
strong as the schema behind it.** That is already true of `updateState` and every other write
method, so these verbs inherit it rather than introducing it.
