...
**Size:** Medium — 2 production files · 12 test behaviours · 4 doc surfaces · 1 PR.
...
The reason to throw rather than coerce is specific to resources: a resource declares its
own `stateSchema` and **every write is normalized against it** (map row 11), so a
wrong-typed field is a programming error, not a data condition. The store contract already
...
| 11 | Per-write schema validation | none at the mutator layer | every write is normalized against the resource's own state schema before it persists | **Deliberate** — a resource carries its own schema; the bag's is composed from block partials and validated elsewhere |
...
| Either verb producing a value the resource's schema rejects | Normalized by the existing per-write schema step (row 11) — no new behaviour |

**Taxonomy:** the three errors resource writes already raise, for the same reasons, plus
one new *fatal* case — the wrong-type throw, which is a programming error at the call site
...
6. multi-field increment on a resource behaves as any other resource write
7. concurrent increments from two contexts converge
8. a verb racing a delete is terminal
9. a no-change verb is a verified no-op that does not bump the version
10. **a no-change verb emits no `resource_change`** — on a **collection instance** and on a
    ...
    *because* the notification is gated on it, and the two ref builders each carry their own
    ...
    the other
11. a `ResourceContext`-typed value exposes `incState`, `pushState` **and**
...
12. `updateState`, `patchState` and `setState` behave byte for byte as today

**Constraint on graduating rows 7–9 from the POC:** the POC models the verbs as wrappers
