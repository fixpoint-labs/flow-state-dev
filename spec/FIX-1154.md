...
15. a `ResourceContext`-typed value exposes `incState` and `pushState` — easy to get
    half-right, because adding to `ResourceRef` alone type-checks everywhere
16. `updateState`, `patchState`, `setState` **and `getOrPatchState`** behave byte for byte
