...
**Size:** Medium — 2 production files · 21 test behaviours · 7 doc surfaces · 1 PR.
...
**Closed object schemas only, and say so.** The `keyof TState` typing (above) needs a schema
it can reflect. For an open schema — `z.object({ count: z.number() }).passthrough()` — the
inferred state carries a string index signature, so no key can be recovered as *the*
number-valued one: a conditional-key API either admits every string (reinstating the typo
hole the typing exists to close) or admits none. Runtime is no better: under an open schema
an absent dynamic key and a typo are the same observation, so the unknown-key rejection has
nothing to reject on.

So **the verbs are declared only for resources whose `stateSchema` is a closed, reflectable
object**, and a resource with an open schema keeps `updateState`, which never claimed key
safety. This is a restriction, not a fallback: defining open-schema semantics nobody has
asked for would mean specifying which of two bad behaviours to ship. **No resource in
`packages/*/src` uses a passthrough state schema today**, so the restriction costs nothing
now — and the POC's own collection fixture was closed for exactly this reason, so it does
not demonstrate on a shape the verbs will not support.
...
3. **Make the `writable: false` refusal non-retryable** (§7's table) — it is on the path
   ...
   `patchState` / `setState` / `updateState` too. *Test:* behaviour 10. *Depends on:* nothing.
...
4. **Ship the map** per §11, and **correct the concurrency-guidance line** that tells
...
**No model or provider required.** The flow is a handler block that calls `incState` and
`pushState` on a declared resource and returns; the check is that the NDJSON shows the
resource's state advanced as expected and the run completed. Run it before the unit suite,
per the ordering `AGENTS.md` gives when a change spans more than one row.
...
10. **a verb on a resource declared `writable: false` refuses non-retryably** — the
    pre-existing refusal the new verbs route through (§7's table). Assert attempt count, not
    just rejection
11. multi-field increment on a resource behaves as any other resource write
12. concurrent increments from two contexts converge
13. a verb racing a delete, **from a context that observed the resource live**, is
    terminal — the qualifier matters, because behaviour 16 pins the never-observed case as
    a defect and an implementer who writes only the held-version case meets it as a surprise
14. **a resource whose schema has no valid complete default throws on first touch** rather
    than creating a half-populated row — the case that looks like ordinary creation and is
    not
15. **the verbs are unavailable on an open (`.passthrough()`) schema** — a type test; the
    resource keeps `updateState`
16. **CURRENT BEHAVIOUR, pinned as a defect:** a write from a context that never observed
    ...
    fix it here**
17. a no-change verb is a verified no-op that does not bump the version — against a
    **live** row and against a **never-stored** resource
18. **a no-change verb against a row we held and lost is terminal** — a stale positive
...
19. **a no-change verb emits no `resource_change`** — on a **collection instance** and on a
...
20. a `ResourceContext`-typed value exposes `incState` and `pushState` — easy to get
    half-right, because adding to `ResourceRef` alone type-checks everywhere
21. `updateState`, `patchState`, `setState` **and `getOrPatchState`** behave byte for byte
