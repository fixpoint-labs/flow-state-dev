...
**What this condition does NOT cover, said out loud rather than left to inference: a
*same-type* transform.** `z.object({ n: z.number().transform((v) => v + 1) })` has an output
type identical to its input type, so it **satisfies** the rule above, the verbs are offered —
and the stored value drifts anyway:
