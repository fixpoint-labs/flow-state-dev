...
2. **Store-native increment and append for resources comes off the roadmap** — it removes
   no conflicts and does not remove the per-append copy.
   *Locks in:* resource writes keep sending the whole value, and reversing is real work
   rather than an additive tweak (§7b).
...
**Size:** Medium — 2 production files · 16 test behaviours · 4 doc surfaces · 1 PR.
...
**Why the last two need rules of their own — the write path does not reject, it replaces.**
...
| 6 | `getOrPatchState` | absent | present on `ResourceRef`, **absent on `ResourceContext`** | **Bag side: deliberate** (no demand; no per-key single-flight seam there). **Handle split: closed here** — `ResourceContext` is the handle the eight consumers hold, so a verb missing from it is the same failure this issue closes. The runtime already exists; this is the type surface catching up |
...
Rows 1, 2 and half of 6 are the code change. The rest is documentation or a follow-up.
...
Tier 2). Rejected in decision 2, for three reasons that each survived a review round:

1. **It removes no conflicts.** On the bag, a native delta verb avoids contention *because*
   the version check is skipped alongside it. Resources may not skip it, so a native
   resource increment would still carry a held version and conflict exactly as often as a
   whole-value write.
...
2. **Close the `getOrPatchState` handle gap** (row 6) — add it to `ResourceContext`, which
   the runtime already satisfies. Same BP-004 gap, same consumers, one line of type surface.
   *Depends on:* nothing.
3. **Implement both ops in the resource registry**, in **both** ref builders, with every
...
*(No removals. Subtraction here is decision 2 and the §12 split.)*
...
programming errors at the call site, must never be retried, and must throw **before**
reaching the write path, which would reset state and report success.
...
13. a no-change verb is a verified no-op that does not bump the version
14. **a no-change verb emits no `resource_change`** — on a **collection instance** and on a
...
15. a `ResourceContext`-typed value exposes `incState`, `pushState` **and**
...
16. `updateState`, `patchState` and `setState` behave byte for byte as today
...
- **Settled:** the retry driver's conflict policy survives under increment- and
  ...
  **terminal** against a deleted resource rather than resurrecting it · a zero increment is
  ...
  than raising already-exists. (`spec-poc/FIX-1154-resource-mutation-verbs/`; run command in
  §7.)
...
- **After spec review, round 2** — split two unrelated cleanups out of §8; kept
  ...
  existing consumers hold. Those eight files replaced the unfalsifiable autocomplete
  argument as decision 1's evidence. Row 5's "same op under two names" was **replaced, not
  refined** — the bag merges a partial, the resource replaces wholesale, so porting a
  callback drops fields.
