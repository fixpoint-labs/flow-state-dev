...
*Tracked as [FIX-1154](https://linear.app/fixpoint-labs/issue/FIX-1154). An implementation
spec — it lives on its spec PR and never lands on `main` (BP-037). The ref and the label sit
here rather than above the problem, per `AGENTS.md`: nothing precedes the problem.*
...
One difference is the headline, and it holds for as long as resource writes go through the
shared mutators: **every resource write through those mutators — `incState`, `pushState`,
`patchState`, `setState`, `updateState` — is version-checked and can be refused**, where the
bag's write can skip that check. (The
...
1. **Resources get `incState` and `pushState` on both public handles, typed from the
   ...
   *Locks in:* two methods on a shipped interface we cannot remove without a breaking
   change, and a difference in what "atomic" means across the two primitives that stands
   **for as long as resource writes go through the shared mutators**. Not permanent, and
   saying so would be stale: decision 2 keeps native resource deltas **deferred**, and §12
   records that the liveness-gated variant would write without a held version by design. A
   later cycle taking that up changes this difference rather than inheriting it.
...
**Size:** Medium — 2 production files · 26 test behaviours (behaviour 6 is four assertions) · 7 doc surfaces, written by `docs-writer` then `docs-editor`, not inline · 1 PR + changeset.
...
**A runtime unknown-key rejection backs the types up — where the schema's declared keys are
reachable, and only there.** It exists for the JavaScript caller or the `as any` that walks
straight past the types. Implementing it means reading those keys, so it is available exactly
when an object schema is reachable. **Reachable is not the same as `.shape` on the schema
handed in, and the difference is the whole finding:** a closed object wrapped in
`.default()`, `.refine()`, `.catch()`, `.readonly()` or `.brand()` exposes **no `.shape` of
its own** — the wrapper does — while still declaring exactly the keys the check needs.
Verified: for all five, `s.shape` is `undefined`, and the object is one hop down
(`_def.innerType.shape` for `.default()`, `_def.schema.shape` for `.refine()`). **So the rule
is: unwrap to the underlying object schema, and the backstop is absent only when no object
schema is reachable at all** — `z.custom<{ count: number }>()`, which has none. Stating it as
"`z.object` has `.shape`" was the rule drawn at the width of its motivating instance again,
and it would have classed five ordinary schemas as uninspectable.

**Stripping is a second, independent axis — do not derive one from the other.** Which silent
failure an untyped typo produces depends on whether the schema *strips* unknown keys, not on
whether its keys are readable, and the two do not line up:

- **A closed object, wrapped or bare, strips** (verified for all five wrappers:
  `parse({count: 1, typoKey: 2})` → `{count: 1}`). The result deep-equals current state, so
  the typo is the **silent verified no-op** this backstop exists to prevent — and for the
  wrapped ones the backstop was, until this round, written out of existence.
- **`z.custom` does not strip.** `safeParse({count: 1, typoKey: 2})` succeeds **and keeps
  `typoKey`**, so the result differs from current state, the write lands, and the junk key
  accumulates in stored state.

Two different silent outcomes from one typo, and the axis that selects between them is
stripping, not inspectability. The type-level guarantee is what closes both — which is why it
is the guarantee that matters and the runtime one is a backstop.

**What the verbs must refuse.** One rule per case, and the last three are the ones that look
like they need no rule.

**This list is the spec's refusal set, and it is named by reference everywhere else** — §9's
error taxonomy and §7a's new-refusals row point at it rather than restating it, so a rule
added here reaches them without an edit. That is deliberate: both of those places have
already been the stale copy, listing the rules as they stood when they were written.
...
- **A delta that is not a finite number** → **throw, before any arithmetic runs.** The
  ordering *is* the rule. The three existing rules above all check the stored *field*;
  this one checks the caller's *argument*, and nothing checked it until an untyped or
  `as any` caller reached it. Evaluating `current + delta` first produces two different
  silent failures, both verified in the POC: a `bigint` or symbol delta throws a raw
  `TypeError` — **not** a `FlowError`, so `isRetryableError` classifies it **retryable**
  (`execution/retry.ts:57-88`) and the block re-runs every side effect it already performed;
  and a **string** delta does not throw at all — `0 + "5"` concatenates, the schema rejects
  the result, and the write path **replaces the whole state with the schema default and
  reports success**. A live row holding real data came back as its default with the version
  bumped and no error raised. That is the destructive-replacement path (row 11) reached
  through an argument, and it is the worst outcome on this list.
- **Any value that would not survive a JSON round-trip unchanged** — anywhere inside a
  delta, a computed result, or an appended value → **throw**. Applies to `incState` *and*
  `pushState`. **Stated as round-trip stability rather than as a list of forbidden kinds, on
  purpose:** every previous phrasing enumerated the kinds known when it was written, and
  every later round found a neighbour outside it. Round-trip stability is the property the
  storage layer actually needs, so it covers the kinds already found — non-finite numbers,
  `undefined`, functions, symbols, `bigint`, cycles — **and the branded objects that no
  kind-based check sees**, which is the case that closed this rule. A `Date`, `Map`, `Set`
  or `RegExp` passes `z.array(z.any())`, is preserved intact by the memory store's
  `structuredClone` (`core/src/helpers/clone.ts:16`), and is flattened by every JSON-backed
  adapter — `Date` to a string, `Map` / `Set` / `RegExp` to `{}`. All four verified in the
  POC. Same adapter-dependent corruption this rule exists to stop, arriving as a value that
  looks perfectly ordinary to a guard walking enumerable properties.
...
- **Unknown keys** lose their backstop entirely: no object schema is reachable — not on the
  handle and not under a wrapper — so the check cannot be implemented at all for that
  resource, and an untyped caller's typo lands in stored state rather than being refused.
  **This applies to `z.custom` and not to a wrapped `z.object`**, which is inspectable one
  hop down and keeps its backstop.
...
**A second condition, and it is not about keys at all: the output must re-parse as input.**
The write path validates a *candidate state* by running the resource's schema over it, so a
schema whose output type differs from its input type produces an API that compiles and always
fails. `z.object({ n: z.string().default("0").transform(Number) })` exposes `n: number`, so
`incState({ n: 1 })` type-checks — and then the candidate `{ n: 1 }` is re-parsed against a
schema whose input side wants a string, which **rejects it every time**. Verified: `parse({})`
yields `{"n":0}`, and `safeParse({n:1})` is `false`.

**The two conditions are enforced at different layers, and round 14 settled which is which.**
That is a decision about what this API *promises*, so it is stated here rather than left to
the implementer:

- **No string index signature → an availability condition, enforced at compile time.** An open
  (`.passthrough()`) schema is not offered the verbs at all. This one works: `string extends
  keyof TState` separates an open schema from closed, coercing and defaulting ones alike
  (verified, round 14). **Here the "state it as a condition of the contract, not an edge case"
  framing holds in full — an API that type-checks and throws on every call is worse than one
  that does not exist**, and for open schemas we can and do make it not exist.
- **Output re-parses as input → a runtime refusal, not an availability condition.** The verbs
  **are** offered on a type-changing transform, and a call **refuses non-retryably when the
  candidate does not re-parse as input**, via the schema-invalid-result rule below. It cannot
  be a compile-time condition: both handles carry only the schema's **output**, where a
  transforming schema is structurally indistinguishable from an ordinary one (§10 17b; §12
  records the rejected alternative and what would reopen it).

  **The refusal is conditional, not universal — say it as the condition it is.** An earlier
  round wrote "every call refuses", which is false and would have been published as a
  contract. A type-changing transform whose **input side also accepts its own output** re-parses
  fine and the call **succeeds**: `z.union([z.number(), z.string()]).default("0").transform(Number)`
  exposes `n: number`, and `safeParse({ n: 1 })` is **`true`** (verified) — where the
  non-overlapping `z.string().default("0").transform(Number)` is `false`. "Type-changing" was
  never the property doing the work; **re-parse acceptance is**, and most type-changing
  transforms fail it, which is exactly how a universal got written from a sample of one.

**Say the weaker promise out loud rather than letting a reader infer the stronger one.** For
an open schema the guarantee is *absence*; for a transforming schema whose output the input
side rejects it is *a loud, typed, terminal refusal on first call* — never silent drift, never
row 11's destructive replacement. Both keep `updateState`, which has the same constraints and
never promised otherwise. **§11 must publish the second as a refusal, not as an exclusion.**

**Two orthogonal properties decide what a transforming schema does, and neither of them is
"same-type versus type-changing".** That axis reads as if it were doing the work in the two
paragraphs above and it is not — it merely correlates with both. The properties are:

| | **Output re-parses as input?** | **Transform idempotent on its own output?** | Outcome |
|---|---|---|---|
| `z.string().default("0").transform(Number)` | no | — | **refuses** every call (§10 17b) |
| `z.union([z.number(), z.string()]).default("0").transform(Number)` | **yes** | yes | **succeeds** normally (§10 17c) |
| `z.number().transform((v) => v + 1)` | yes | **no** | **drifts**, call reports success (§10 17a, FIX-1260) |

Re-parse acceptance decides **refusal**. Idempotence decides **drift**. A schema can be
type-changing and land in any of the three rows — row 2 is type-changing and does neither.
Keying either promise off same-type-ness is how "every call refuses" got written, and it is
the phrasing to avoid when §11 publishes this.

**What the re-parse condition does not cover, said out loud rather than left to inference: a
...
- **Transforming or coercing: zero.** So **nobody hits the runtime refusal today**, and
  FIX-1260's drift is latent rather than live — both become reachable the first time someone
  writes a perfectly reasonable normalizing schema. That count is also why the refusal is an
  acceptable place to enforce this: the case it catches does not exist yet, so nobody is
  currently relying on a compile-time answer to it.
...
|---|---|---|---|
| **Every refusal in §7's "what the verbs must refuse" list** — today: unknown key on a key-declaring schema · wrong-typed field · non-finite-number delta · a value that would not survive a JSON round-trip unchanged · schema-invalid result | **New** (this spec, above) | — | `ValidationError`, `retryable: false` |
...
**Blast radius of the `writable` fix — a derivation, not a list.** That throw sits in
`persistResourceState`, which only the **single**-resource handle's `patchState` / `setState` /
`updateState` reach (`resource-registry.ts:1542`, `:1558`, `:1582`). **The reach is therefore
every public entry point that funnels into those three**, not the three themselves — so
**derive it from the callers at implementation time** rather than trusting the list below to
have kept up. Today that derivation yields:

| Reaches the changed refusal | How |
|---|---|
| `patchState` / `setState` / `updateState` on a single resource | direct |
| **`getOrPatchState`** — the **miss** path, when `compute` returns non-`undefined` | `applyGetOrPatchState` calls `ref.patchState` (`core/src/types/resource.ts:531`) |
| **`ref.edges.add` / `.supersede` / `.remove`** | `createResourceEdgeApi` calls `ref.updateState` (`core/src/graph/resource-edges.ts:162`, `:176`, `:185`), wired onto single handles at `resource-registry.ts:1653` |
| **`ref.edges.pruneDangling`** | the **fourth** edges method, and the only one that reaches `ref.updateState` *indirectly* — via `updateStateWith` (`resource-edges.ts:210`). Live in-tree caller: the memory janitor (`memory/src/janitor-blocks.ts:123`) |
| `updateStateWith(ref, …)` and `withOutcome(ref.updateState, …)` | transparent forwarders to `ref.updateState` (`core/src/helpers/update-state-with.ts:146`), both public via the `@flow-state-dev/core/helpers` subpath export |

And, stated so the enumeration is **closed rather than merely extended**, what does *not*
reach it: **every write on a collection instance**, which persists through
`persistNamespaceInstanceState` — a path with **no `writable` check at all**
(`resource-registry.ts:670-684`), so `upsertResource`, the `patch` resource-tool, the bash
mounts and `.edges` on an instance ref are all outside this change. Scope state ops
(`createScopeStateOps`), sequencer and session state are different subsystems with no
`writable` flag. **`writeContent` is the near miss worth naming:** it carries its **own**
read-only throw, outside `persistResourceState` (`resource-registry.ts:1636`) — so after this
change one `writable: false` resource refuses state writes non-retryably and content writes
retryably. Left as-is deliberately; §12 carries it.

**The helper row is scoped by its *runner*, not by its name — do not widen it to the helper.**
`withOutcome` is generic over any mutation runner: hand it a scope's `atomicState` or the task
collection's `casWrite` and it never touches `persistResourceState` at all. Only the calls
whose runner is a **single-resource** `ref.updateState` are in this radius, which is the shape
`updateStateWith` fixes by construction and `withOutcome` does not. Reading the row as "the
helpers are affected" would sweep two unrelated subsystems into a resource-side change — the
mirror image of the omission that put the row here.

The change is one-directional — an error that was pointlessly retried now fails fast — and
there is no case where retrying a read-only refusal was the desired outcome. Worth doing on
the path rather than shadowing it with a second check inside the new verbs, which would leave
every entry point above still wrong.

**This is the round-13 lesson landing on the paragraph that taught it.** The text above once
read "the blast radius was drawn around the new methods instead of around the path a caller
reaches" — and then drew the new radius around the three *direct* callers of the shared
function and stopped one hop short of the wrappers. A radius stated as a list goes stale the
same way a numeric range does; stated as a derivation, it does not.
...
**Values the storage layer cannot round-trip** reach that same destruction by a longer route,
which is what makes them the worst case here — and the hazard class is **round-trip
stability, not JSON-safety and not finiteness.** The rule has been drawn too narrowly twice,
each time at the instance that motivated it: first at non-finite numbers (because `Infinity`
prompted it), then at JSON-safety (because `undefined`, functions, symbols and `bigint` broke
it). Both are still too narrow — a `Date` is perfectly JSON-safe and still stores differently
on two adapters. The boundary that holds is *anything a JSON round-trip does not return
unchanged*. Zod accepts all of these under an ordinary schema, and the adapters then
disagree:
...
**The last two rows are the ones that force the rule's final phrasing.** A branded object is
not "unsafe" in any sense a type check can see: it serializes without complaint, and it
serializes to something *else*. Memory keeps a `Date`; SQLite stores a string. Memory keeps a
`Map`'s entries; SQLite stores `{}`. Nothing throws on either side, so there is no failure to
notice — the two deployments simply hold different data from the same call, and the one that
lost its contents finds out on a later read. That is why the rule is **"survives a JSON
round-trip unchanged"** and not a list of kinds: `Date` and `Map` are JSON-*safe* and still
corrupt. Both verified in the POC.

So the defect never surfaces at the write that causes it. It surfaces later, in a different
request, as unrelated fields vanishing, and only on some deployments. That is why
**round-trip stability is checked at the call site** rather than left to the schema. A
resource declaring `.finite()` would catch one row of the table above at the schema check,
but nothing makes that the default, no existing resource does it, and no Zod modifier covers
the branded-object rows at all.
...
on the resource handle, beside patchState:
    ...
    incState(increments):            keys: number-valued keys of TState
        reject unknown keys → throw
        reject any delta that is not a finite number → throw
            (BEFORE any arithmetic. `1n` and symbols make `current + delta`
             throw a raw retryable TypeError; "5" does not throw at all —
             it concatenates, fails the schema, and RESETS THE ROW.)
        mutator, per field:
            absent       → start from zero, add the delta
            not a number → throw
            otherwise    → add, then check the sum round-trips → throw
                           (the schema will NOT catch this)

    pushState(field, value):         field: array-valued keys of TState
        reject unknown field → throw
        reject any value anywhere inside `value` that would not
        survive a JSON round-trip unchanged → throw
            (kinds JSON drops or chokes on — undefined, functions,
             symbols, bigint, non-finite numbers, cycles — AND branded
             objects that serialize to something ELSE: Date, Map, Set,
             RegExp. The second group passes every typeof check.)
...
2. **Implement both ops in the resource registry**, in **both** ref builders, with every
   ...
   shared normalizer, FIX-1260, not to these two verbs). **One asymmetry to inherit rather
   than repair:** the collection-instance builder has **no `writable: false` check at all**
   today (§7), so the verbs land there unguarded like the three methods beside them. **Do not
   add one** — that would be new enforcement on a shipped path, not this issue's change; §12
   files it. *Test:* the behaviours in §10, including behaviour 24's negative control.
   *Depends on:* 1.
3. **Make the `writable: false` refusal non-retryable** (§7's table) — it is on the path
   these verbs route through, and it lives in `persistResourceState`, so the fix covers every
   public entry point that funnels into the single handle's `patchState` / `setState` /
   `updateState`. **Re-derive that set from the callers before you build** — §7's blast-radius
   table records today's derivation (`getOrPatchState`'s miss path and `ref.edges.*` are on it;
   collection instances are not), but the derivation is the contract, not the table.
   *Test:* behaviours 11 and 24. *Depends on:* nothing.
...
4. **Add a Changeset** (BP-022). Four new public methods on a publishable package plus a
   ...
   **The engine note must name the reach**, because a caller who never touches the new verbs
   still sees this: on a `writable: false` resource, `patchState` / `setState` / `updateState`,
   **`getOrPatchState`'s miss path**, **all four `ref.edges` writers (`pruneDangling`
   included)** and **`updateStateWith(ref, …)`** now fail fast instead of retrying.
   Content writes are unaffected (§7).
...
| Unknown key — `incState({ factExtractd: 1 })` — on a schema that **declares its keys** (`z.object`) | **Compile error**, and a **runtime throw** for callers who reach it untyped. Never the bag's silent strip-and-no-op |
| Unknown key on an **opaque** schema (`z.custom`) | **Compile error only.** No `.shape` to read means the runtime backstop is **absent, not weaker** (§7), and `z.custom` does not strip — so an untyped caller's typo lands in stored state. Pinned deliberately (§10 behaviour 6). An implementer who "fixes" this by inventing a key check for a schema that declares no keys has invented a guarantee this spec does not make |
...
| An appended value that **would not survive a JSON round-trip unchanged for any other reason** — `undefined`, a function, a symbol, a `bigint`, or any of these nested inside it | **Throws.** Same hazard class, non-numeric: Zod accepts them under `z.array(z.any())`, SQL and filesystem adapters serialize the first three to `null` (corrupting a later read), and `bigint` **throws mid-write** with a raw `TypeError` naming no field |
| An appended value that is a **branded object** — `Date`, `Map`, `Set`, `RegExp`, or any of these nested inside it | **Throws.** The round-trip rule's hardest case, because nothing about the value looks wrong: `z.array(z.any())` accepts it and a guard walking enumerable properties finds nothing to reject. It still splits by adapter — the memory store's `structuredClone` preserves the instance, every JSON-backed adapter flattens it (`Date` → a string, `Map` / `Set` / `RegExp` → `{}`). **Verified in the POC**, all four kinds. This is why the rule is stated as round-trip stability and not as a list of forbidden kinds (§7) |
| A **delta that is not a finite number** — `incState({ n: 1n })`, a symbol, or a string — reached untyped or through `as any` | **Throws, before the arithmetic.** Two distinct silent failures otherwise, both verified in the POC: `bigint` and symbol produce a raw `TypeError` that is **retryable by classification**, re-running the block's earlier side effects; a **string** produces no error at all — `0 + "5"` concatenates, the schema rejects it, and the whole state is **replaced by the schema default while the call reports success**. The type surface already excludes all three; this is the backstop for callers who walk past it |
...
| Either verb on a resource whose schema **transforms, and whose input side rejects its own output** (`z.string().default("0").transform(Number)`) | **Offered, and refuses non-retryably** — `ValidationError`, state unchanged. *Not* an availability exclusion: the handles carry only the schema's output, where such a schema is indistinguishable from an ordinary one, so this is enforced at the write and not at the call site (§7; §10 17b; §12 for the rejected alternative). The resource keeps `updateState`. **Scoped to re-parse rejection, not to "type-changing"**: a transform whose input also accepts its output (`z.union([z.number(), z.string()]).…transform(Number)`) re-parses and the call **succeeds** (§7's table; §10 17c) |
...
**every call this spec's rules say the verb refuses** (§7's "what the verbs must refuse"
list: unknown key on a key-declaring schema, wrong-typed field, non-finite-number delta, a
value that would not survive a JSON round-trip unchanged, schema-invalid result). **Named by
reference to that list rather than re-enumerated here on purpose** — this taxonomy has twice
been the stale copy, listing the rules as they stood when it was written while §7 grew a new
one. A rule added to §7 joins this class without an edit here. All are programming errors at
the call site and must throw **before** reaching the write path, which
...
6. **unknown key — test the boundary, not just the good case.** Four assertions, because
   the runtime half is scoped to schemas whose keys are **reachable** (§7) and an
   implementation could otherwise collapse the cases silently:
   ...
   - **opaque schema, untyped caller**: pin that the runtime backstop is **absent** and the
     ...
     pins the tombstone-resurrection defect — an implementer who "fixes" this by inventing a
     key check for a schema with no declared keys has invented a guarantee the spec does not
     make
   - **wrapped closed object, untyped caller**: `z.object({…}).default(…)` — the backstop is
     **present** and the typo throws. This is the assertion that stops the check being
     implemented as `schema.shape` and silently losing five ordinary schemas: `.default()`,
     `.refine()`, `.catch()`, `.readonly()` and `.brand()` all expose **no `.shape` of their
     own** while declaring the keys one hop down (verified). Use at least `.default()` and
     `.refine()`, which unwrap through different `_def` fields, so a single-wrapper unwrap
     does not pass. **Without this row the failure is invisible**: those schemas still *strip*
     the unknown key, so the untyped typo degrades to the silent verified no-op the backstop
     exists to prevent, and every other assertion here stays green
...
9. **`incState` refuses every delta and result the rules forbid.** *Result values:*
   `Infinity`, `-Infinity`, and an **overflow** case reaching `Infinity` by arithmetic; cover
   `NaN` too, noting why it is the odd one out. *Argument values, and do not skip these
   because the types exclude them* — the point is the untyped caller: a `bigint` delta, a
   symbol delta, and a **string** delta. Assert each throws **non-retryably and before the
   arithmetic**. The string case needs its own assertion that the resource's other fields
   **survive**: today it does not throw at all, and the whole state comes back as the schema
   default with the call reporting success (verified in the POC), so a test asserting only
   "it rejected" would pass against the version that destroys the row
10. **`pushState` refuses every appended value that would not survive a JSON round-trip
   unchanged** — and test the class, not the instance, along **three** axes. *Type:* a
   non-finite number, `undefined`, a function, a symbol, and a `bigint`, each **also nested
   inside an appended object** so a shallow check fails. `bigint` asserts a clean refusal
   rather than the raw serialization `TypeError` it produces today. *Identity:* a **branded
   object** — `Date`, `Map`, `Set`, `RegExp`, again also nested. This axis is the one a
   kind-based guard misses entirely: all four pass `z.array(z.any())`, all four are preserved
   by the memory store and flattened by JSON-backed adapters, and none of them looks wrong to
   a walk over enumerable properties (all verified in the POC). A guard written against the
   *type* axis alone passes every test above and still ships the corruption. *Shape:* a
   **self-referential** appended value — `z.array(z.any())` accepts
   ...
   same symptom.
   **Two more on the shape axis, and they are the ones the assertions themselves have to be
   written carefully for** — both pass every row above, and both are *values the rule already
   forbids* that the prescribed tests cannot see:
   - **negative zero**, direct and nested. `z.number()` accepts `-0`, normalization preserves
     the sign, the memory store keeps `-0`, and JSON gives `0` (all verified). It clears
     `Number.isFinite`, so a finiteness-shaped guard admits it. **Compare with `Object.is`,
     never `===`** — `-0 === 0` is `true`, so a `===` assertion on this row *cannot fail* and
     the test is decoration.
   - **a sparse array**, direct and nested — `const v: unknown[] = []; v.length = 1`. **The
     hazard here is guard evasion, not the value:** `forEach`, `map` and `filter` **skip holes
     entirely** (verified: `forEach` visits 0 elements), so a guard that recursively validates
     "present elements" walks the array and finds nothing to reject, while `for…of` and an
     index loop both surface the `undefined` this rule already forbids. **Require the guard to
     walk by index over `length`.** Assert **occupancy** (`0 in value`), not just element
     values: `[undefined]` and `[<hole>]` read identically through `v[0]`.

   *Recorded because it changes what the sparse row proves:* under an array-typed schema
   (`z.array(z.any())`) zod's own parse **densifies** the hole to `undefined`, so both adapters
   converge on `[null]` and there is no adapter divergence left to catch — the whole defect is
   that the pre-write guard never fired. Under a schema that does not re-walk the array
   (`z.any()`) the hole **survives** normalization and the adapters genuinely disagree: memory
   keeps a hole (`0 in value === false`), JSON stores `[null]` (`true`). Both verified. Write
   the row against the second, and the guard requirement against the first
...
16. **a resource whose schema has no valid complete default throws on first touch** rather
    ...
    not. **It throws, so it is in the retry sweep below**, and the refusal must be typed
    non-retryably like every other: no retry can make a missing required field appear, so a
    plain `Error` here buys nothing and re-runs the block's earlier side effects
17. **the verbs are unavailable on an open (`.passthrough()`) schema** — a type test, and it
    works: an open schema's output carries a string index signature, so `string extends keyof
    TState` separates it from closed, coercing and defaulting schemas alike (verified, round
    14). The resource keeps `updateState`. Assert a **coercing** (`z.coerce.number()`) and a
    **defaulting** schema are *still offered* the verbs: they satisfy the condition, and
    excluding them is the mistake an implementation that tried to detect "is it a transform"
    would make
...
17b. **on a transform whose input side rejects its own output the verbs ARE offered, and the
    call refuses non-retryably** — a **runtime** behaviour test, not a type test, and the
    difference is the promise, not the plumbing.
    `z.object({ n: z.string().default("0").transform(Number) })`
    exposes `n: number`, so `incState({ n: 1 })` compiles; the candidate `{ n: 1 }` is then
    re-parsed against a schema whose input wants a string, fails, and hits §7's
    **schema-invalid result** refusal. Assert the `ValidationError` **and the attempt count** —
    it is on the retry sweep like every other throw here — **and assert the state is
    unchanged**, which is the whole point: this is the one place row 11's destructive
    replacement is *prevented* rather than inherited.
    **Do not write this as "every call on a transforming schema refuses"** — 17c is the
    counterexample, and that phrasing is what this round removed.
    **Why this is not a type test, recorded so nobody re-derives it:** both handles carry only
    the schema's **output** (`StateOf<T> = S["_output"]`, `resource.ts:381-385`; `ContextOf`
    feeds that output into `ResourceContext`, `:407-413`), and a transforming schema's output
    is **structurally identical** to `z.object({ n: z.number() })` — verified by type-level
    `Equal<>` assertion (round 14), which also showed the same identity for the coercing,
    defaulting and same-type-transform cases. That identity is *why* 17 and 17a can require
    those to stay offered with no special case. §12 records the rejected alternative
17c. **a type-changing transform whose input ALSO accepts its output SUCCEEDS** —
    `z.object({ n: z.union([z.number(), z.string()]).default("0").transform(Number) })`. The
    verbs are offered, `incState({ n: 1 })` compiles, the candidate re-parses (`safeParse({n:1})`
    is `true`, verified), and the write **lands normally**. This row exists to pin the
    **condition against the universal**: an implementation that refuses whenever it detects a
    transform passes 17b and fails here, and the false universal it would be built from is now
    published in §11. Assert the value **and** that no error is raised — the negative control
    for 17b, and the reason "type-changing" is not the property the contract is stated over
18. **CURRENT BEHAVIOUR, pinned as a defect:** a write from a context that never observed
    the resource **revives** it after a delete. Assert today's behaviour. **Name the test for
    the tombstone-resurrection behaviour and keep the
    [FIX-1258](https://linear.app/fixpoint-labs/issue/FIX-1258) reference in a provenance
    comment, not in the test name** — BP-006 bans planning and tracking labels in code and
    tests, the identifier outlives the tracking artifact, and this behaviour graduates into
    the CI suite where the title would ship. The comment still lets the failure be read as the
    fix arriving rather than a regression, which was the point of naming it. **Do not
    fix it here**
...
23. `updateState`, `patchState`, `setState` **and `getOrPatchState`** are unchanged in
    **success behaviour and in every error path except the one step 3 deliberately
    ...
    `persistResourceState`, so **all four** change on that one path. That is the stated blast
    radius, not a regression
24. **the new read-only semantics hold on every pre-existing entry point that reaches them,
    not just the new verbs** — against a `writable: false` resource, each refuses
    **non-retryably**, asserted by **attempt count**. Testing this only on `incState` /
    `pushState` would leave the change's actual reach unverified. **The set is §7's
    blast-radius derivation, re-derived at implementation time, not this line's list** —
    today: `patchState` / `setState` / `updateState`, **`getOrPatchState` on a miss whose
    `compute` returns non-`undefined`** (the hit path never writes, so it is a separate
    assertion that it stays a plain read), **all four `ref.edges` writers — `.add` /
    `.supersede` / `.remove` / `.pruneDangling`** on a resource declaring an `edges` slot, and
    the public forwarder **`updateStateWith(ref, …)`** called directly.
    **`pruneDangling` and the forwarder are this round's correction, and they were missed
    together for one reason:** the other three edges methods call `ref.updateState` inline, so
    a derivation that grepped for that call found them and stopped — `pruneDangling` reaches it
    through `updateStateWith` (`resource-edges.ts:210`), one hop further out, and the forwarder
    itself was on §7's table but never copied here. It has a live in-tree caller
    (`memory/src/janitor-blocks.ts:123`), so this is an observable retry-class change with a
    consumer, not a theoretical entry point. **The derivation was right and its published
    output was incomplete** — which is the failure a derivation was supposed to prevent, so
    assert the set by walking callers, not by reading this line. Two negative controls belong here, because they
    are what makes the enumeration a boundary rather than a list: the **same three methods on
    a collection instance** refuse *nothing* (no `writable` check on that path at all), and
    **`writeContent` still refuses retryably** — the asymmetry §7 names and §12 carries

**Every refusal on the path — not just the new ones — is asserted through the execution retry
path, not just at the call.**

**The rule is a predicate over the list, not a range: *every behaviour above whose expected
outcome is a throw* is in scope.** Read the list and take each one that throws — today that
is 5–10, **15**, 16, **17b** and **20**, plus the `writable: false` refusal (§7's table) — and
re-derive it whenever a behaviour is added, rather than trusting this sentence's parenthetical
to have kept up. (**15 and 20 are this round's correction**: both raise `ResourceDeletedError`,
both are throws, and both sat outside an enumeration that claimed to be the predicate's output
— the fourth instance of exactly the staleness the next paragraph is about.)

That instruction is written this way because the numeric form has now gone stale **three
times**, the same way each time. The range `5–9` was written in round 9; behaviour 10 arrived
in round 11 carrying the JSON-unsafe `pushState` class, and nothing re-checked the round-9
enumeration — so an append validator could have thrown a plain `Error`, satisfied the plan,
and left the retry layer re-running the block. Round 12 widened the range to `5–10` and
*still* left behaviour 16 out: a first touch with no schema-valid complete default is
a refusal that cannot succeed on retry, and it sat outside the sweep for the same reason,
one round after the lesson was written down. **And round 14 added 17b — a behaviour that
throws — which this sentence did not pick up on its own.** Three rounds, one lesson, and the
paragraph explaining the lesson still needed a manual edit: **that is the argument for the
predicate, not against it.** The predicate is what makes the omission findable in one pass;
the parenthetical is a convenience that will keep going stale, and a reader who trusts it over
the predicate has read this paragraph backwards.

**A widening has a blast radius that runs backwards**, over every rule that enumerated the
old, narrower set — and a range rewritten to a *wider range* has not learned the lesson, it
has only postponed it. The fix is a rule that reads the list rather than restating it.
Scoping the assertion to the refusals this spec introduces is the same mistake at a different
scale — it is what left the pre-existing one retrying.

Each in-scope behaviour runs the verb inside a block under a
...
the type rather than to one call site's behaviour. **The default is against you here**: a
raw `TypeError` or a plain `Error` is not a `FlowError`, and `isRetryableError` returns
`true` for it under any policy (`execution/retry.ts:57-88`, verified) — so a refusal that
throws the wrong *type* is retried even though the spec calls it terminal.

**Field survival takes its own predicate, and it is narrower than the retry one — do not
reuse the set.** *Every behaviour whose refusal **leaves the row live*** also asserts the
resource's other fields survive. The failure being guarded is not a bad value being stored —
it is the whole state being reset while the call reports success, so a test that only asserts
"it threw" would pass against an implementation that threw *after* the reset landed. Behaviour
9's string-delta case is the live proof that this is not hypothetical: on `main`'s path it
resets the row and reports success today. Today the set is 5–10, 16, 17b and the
`writable: false` refusal.

**Why it is not simply "the throws" — and this is the round's more useful half.** The throw
predicate correctly widened to take in 15 and 20, and **the same widening carried them into a
requirement that is impossible for them**: both throw `ResourceDeletedError` *because the row
is gone*, so there are no other fields to inspect. A test demanding survivors there cannot be
written, and an implementer meeting it would either fake a row or quietly drop the assertion —
the second of which looks exactly like the enumeration going stale again.

**The distinction is real, not a carve-out.** "Leaves the row live" is the property that made
this assertion worth making in the first place: it is a **pre-write refusal**, one that
rejects a candidate while the stored state stands. A deleted-row refusal is a *post-mortem* —
the state is already gone and nothing about it is the verb's to preserve. Two different
guarantees that happened to share an outcome shape.

**And the general form, because rounds 13–15 have now produced it twice.** Every sweep this
spec has run asked *has a neighbour outgrown this rule?* — the backwards question, which found
real defects three rounds running. Nobody ran it **forwards**: *now that this rule is wider,
what does it wrongly include?* A widening fixes an under-inclusion and can create an
over-inclusion in the same edit, and the new members do not look wrong from inside the
widened rule — they satisfy it. **After widening a predicate, walk its new members and ask
what each one now owes that it cannot pay.** Cheaper than the round that finds it, and it is
what this entry is.

**Any behaviour whose hazard is a value the schema *accepts* must not be written as a
schema-check test** — today behaviours 9 and 10, and the same predicate catches the next one.
The whole point is that the schema passes these values; a test that reaches the refusal
through the schema is testing the wrong component and passes against an implementation with
no guard at all. (Behaviour 8's instruction is a different one and stays in its own entry:
its hazard is *placement*, not the value.)

**Constraint on graduating *any* row from the POC** (stated over the file, not over a row
range — the POC gained four rows in round 13 and a range would already be stale): the POC
models the verbs as wrappers
...
**And the POC's four refusal rows inverted, not copied.** They assert what `main` does today
— that a branded object is stored, that a `bigint` delta is retryable, that a string delta
resets the row — because no guard exists yet. The CI specs assert the opposite for each. A
graduated row that kept the POC's polarity would pin the defect as the contract.

**And the tracking labels stripped from every graduated title (BP-006).** Two POC rows carry
one — `"CURRENT BEHAVIOUR (defect, FIX-1258): …"` and the FIX-1260 row — which is fine in a
file that never merges and is banned the moment it reaches CI, for the reason BP-006 gives:
the identifier outlives the tracking artifact. Graduated titles name the **behaviour**
(tombstone resurrection, same-type-transform drift); the ref moves to a provenance comment.
This is the same correction behaviour 18 carries, applied at the other end of the pipe —
recorded here because the graduation constraint is where a title crosses from throwaway into
shipped, and nothing else in §10 would catch it.
...
- **EXTEND** `docs/architecture/state-and-scopes.md` — the internal contract reference, and
  ...
  **Also carry the read-only refusal's reach**, since this is the page an implementer checks
  before assuming a write retries: on a `writable: false` **single** resource the refusal is
  now terminal for `patchState` / `setState` / `updateState`, for **`getOrPatchState` when it
  misses and computes a value**, for **all four `ref.edges` writers including
  `pruneDangling`**, and for **`updateStateWith(ref, …)`** — but not for content writes, and
  not on collection instances. Take that set from §7's blast-radius table at writing time.
...
- **The availability restriction gets a documented line, because it excludes a shipped
  ...
  describes what the thing does, not which of our resources happened to hit it.
- **And the schema-transform case is a *different promise* — publish it as one.** This is the
  one place the page is most likely to overreach, because the two cases feel like a pair and
  are not. **Open schema → the verbs are absent.** **Transforming schema → the verbs are
  present, and a call fails when the value the verb produces is not something the schema will
  accept back**, with a terminal validation error that leaves the stored value untouched. Say
  that as a **refusal**, never as "not supported" or "excluded" — a reader who takes it as
  absence will conclude their code could not have compiled, and it does. One sentence on why,
  because it is the honest version and it is short: the handle is typed from what the schema
  *produces*, which cannot show what it *accepts*. Do not promise a future compile-time check;
  §12 holds the condition under which that would be revisited.

  **Write the condition, not the universal — this brief said "every call fails" and that is
  wrong** (§7's table; §10 17c). A transform whose input side also accepts its output re-parses
  and the call **succeeds**, so "every call fails" would publish a false restriction and
  contradict the test suite. **Do not describe availability in terms of idempotence either**:
  idempotence decides whether a value *drifts*, not whether the verbs are offered, and a brief
  that conflates them makes the docs and 17a disagree. The page states one availability rule
  (open schemas do not get the verbs) and one runtime rule (a value the schema will not take
  back is refused).

  **The drift case is the honest gap and the page owes it a sentence** (§7's table row 3;
  FIX-1260): a schema that rewrites the value it stores keeps advancing it on every write, and
  the call reports success. That is current behaviour on `updateState` too, so it is not a new
  limitation — but a page that describes refusals and omits this one implies the verbs are
  safe on any schema that does not throw.
...
**Settled in round 14 — transform exclusion is a runtime refusal, not a compile-time
absence.** *(An engineering call, settled here. Deliberately **not** numbered alongside §6's
decisions 1 and 2, which are the owner's sign-off surface — this one changes how a promise is
enforced, not what the product commits to.)* §7 once stated *"output type that is valid input"* as an **availability**
condition and §10 asked for it as a **type test**. That test cannot be written: both handles
are built from the schema's **output** (`StateOf<T> = S["_output"]`), where a type-changing
transform is structurally identical to an ordinary schema. **Resolved as: the verbs stay
offered, and a call refuses non-retryably when the candidate does not re-parse as input**
(§7; §10 17b). It costs no implementation — the refusal is §7's already-planned
schema-invalid-result rule — and the safety is unchanged: the call is terminal and the stored
state is untouched.

**Corrected in round 15: the refusal is conditional, not universal.** This entry read "every
call refuses non-retryably", which is false for a transform whose input side also accepts its
own output — that candidate re-parses and the call succeeds (verified; §10 17c). The
*settlement* is unaffected: it decided **where** the exclusion is enforced (runtime, not
availability), and that still holds. Only the description of its consequence was overstated,
and it had propagated to §7, §7a, §10 17b and §11 before anyone read the two halves of the
same bullet against each other — the heading already said "output re-parses as input" while
the body said "every call".

**What is lost, stated plainly because §11 has to publish it:** for a transforming schema the
promise is no longer *absence*. A developer can write the call, ship it, and learn on first
execution — where it refuses at all. The guarantee is that they learn **loudly** — a typed, non-retryable
`ValidationError`, never silent drift and never row 11's replacement — not that they are
stopped. **Do not let a docs writer or an implementer read §7's open-schema exclusion and
generalize it to this case; they are different promises.**

**The rejected alternative, and why — it was found *expressible* and rejected on efficacy.**
Carrying the schema's **input** type through `ContextOf` and `ResourceRef` is possible: zod
3.25 exposes `_input`. A 13-assertion type probe (round 14) is the artifact, and it showed
three things, in ascending weight:

1. **It is wrong on a case this spec wants working.** A naive input-vs-output rule **excludes
   a defaulting schema** — `z.number().default(0)` has an optional input — which §10 behaviour
   17 explicitly requires to stay *offered*. A guard that refuses a case the spec admits is not
   an expensive correct answer; it is an incorrect one. **This alone settles it.**
2. **It would still miss the same-type transform.** `InputOf` equals the output for
   `z.number().transform(v => v + 1)`, so 17a's drift stays pinned either way and the change
   buys only part of the class.
3. **It would be vacuous where it has to bite.** All **22** in-tree `ResourceContext<…>`
   annotations are hand-written against a plain state interface, **15 of them behind an `as`
   cast** off `ctx.resources` (`memory/src/capabilities.ts:68`,
   `memory/src/internal/recall.ts:48`,
   `thought-fabric-core/src/identity/perspective-helpers.ts:23` among them). A defaulted second
   type parameter reads as "available" at every one of those — which are precisely the consumer
   files §8 step 1 cites as the reason `ResourceContext` is in scope at all. A guarantee that
   holds at no real call site is a claim, not a guarantee.

**Cost was not the reason.** The type-surface breadth (69 `ResourceRef<…>` occurrences across
11 files, plus `AnyResourceRef` / `ResourceRegistry` / `ProjectedClient`) is real but was not
weighed — a correct and effective version would have been worth paying for.

**What reopens this — the reversal condition.** If the consumer files migrate from hand-written
`ResourceContext<SomeState>` annotations to `ContextOf<typeof resource>` inference, objection 3
disappears: the schema would then flow into the handle type at real call sites and the
compile-time exclusion would bite. Objection 1 still has to be answered — the comparison must be
per-key and default-aware, not a type swap — and objection 2 stands regardless, but at that
point (a) becomes worth paying for. **That migration is the trigger; nothing else is.**

**Available but not proposed:** an **authoring-time** round-trip probe inside `defineResource`
(`safeParse(parse(default))`). Expressible today, but it is new behaviour running over all 137
in-tree `stateSchema` declarations at import time, with its own blast radius.
...
- **Settled:** the retry driver's conflict policy survives under increment- and
  ...
  **terminal** against a deleted resource, failing fast instead of retrying into a generic
  ...
  §7.)
- **The refusal hazards are now run, not read — four rows, added in round 13.** They pin the
  *gap*, not the guard: the guard does not exist yet, so each row asserts what `main` does
  today and therefore what the shipped refusal has to stop.
  - **Branded objects** — `Date`, `Map`, `Set` and `RegExp` all pass `z.array(z.any())`,
    are preserved as live instances by the memory store, and flatten under JSON to a string
    and three `{}`. The adapter divergence is reproduced, not inferred.
  - **A `bigint` or symbol delta** throws a raw `TypeError` that is **not** a `FlowError`,
    and `isRetryableError` returns `true` for it — so the block re-runs.
  - **A string delta** throws nothing at all: it concatenates, fails the schema, and the
    write path **replaces the whole state with the schema default and reports success**. A
    row holding `keep: "DO-NOT-LOSE"` came back as `keep: "schema-default"` with the version
    bumped. This is the destructive-replacement path (row 11) reached through an argument.
  - **A control on the classifier** — a plain `Error` and a bare `TypeError` are both
    retryable under any policy; only a `FlowError` with `retryable: false` aborts the loop.
    This is why "throws" is never a sufficient instruction in §9 or §10.
- **The transform fork is now run too — one row, added in round 15**, because the spec had
  been asserting a universal over it. Two type-changing schemas that differ only in whether
  the input side accepts the schema's own output: `z.string().default("0").transform(Number)`
  **rejects** the candidate a verb builds (and `main` responds by resetting the row —
  `keep: "DO-NOT-LOSE"` came back as the default), while
  `z.union([z.number(), z.string()]).default("0").transform(Number)` **takes it back and the
  call succeeds**, row intact. The pair is what makes "output re-parses as input" a testable
  condition rather than a phrase sitting above a contradicting universal, and it is the
  negative control for §10 17b.
- **Still not reached, and stated rather than implied:** the *schema-side* refusals — a
  refinement violation and an opaque-schema unknown key. Those still rest on **reading**
  `normalize-resource-state.ts:44-51`, `resource-registry.ts:679-680`,
  `filesystem/resource-state-store.ts:83` and `core/src/helpers/clone.ts:16`, plus a direct
  check of Zod's behaviour — not on running the shipped verbs, which do not exist yet. That
  is what the throwing behaviours in §10 are for.
...
- **A schema-invalid resource write silently resets state and reports success.** Not a
  ...
  **Now reproduced on the real path rather than read off the code** (round 13, POC): a row
  holding `{ n: 5, keep: "DO-NOT-LOSE" }` came back as `{ n: 0, keep: "schema-default" }` with
  the version bumped and no error raised. Worth carrying into the issue — it is a one-line
  repro for a defect that has so far been argued from source. The route used was an untyped
  string delta, which **this** spec's verbs refuse before the arithmetic (§7); the underlying
  replacement stays reachable through `updateState` / `patchState` / `setState` regardless,
  which is why it is still filed rather than closed here.
...
- **`writeContent`'s read-only refusal stays retryable while state writes stop being** —
  it throws its own plain `Error` outside `persistResourceState`
  (`resource-registry.ts:1636`), so step 3 leaves one `writable: false` resource with two
  refusals of different retry class. **An asymmetry this change creates**, found by the round-14
  blast-radius sweep. Not fixed here: the refusal is real either way, the cost is wasted retries
  on a content write, and folding it in would widen a change whose §7 table is scoped to
  *state*-write refusals. **Wants its own issue** — and it is one line, so it is a cheap one.
- **`writable: false` on a collection is silently accepted and ignored — the author is never
  ...
  type parameter** (`resource-collection.ts:266-274`), so excess-property checking never runs
  and `writable: false` **type-checks clean** on a collection. Confirmed with
  `tsc --noEmit --strict`: no error on the collection, no error on the single-resource control.
  The flag is then ignored at the write — instance state persists through
  `persistNamespaceInstanceState`, which never consults it (`resource-registry.ts:670-684`) —
  so a developer can declare a read-only collection, get no feedback of any kind, and have
  every write land. **A silent-acceptance defect, not a missing feature**, which is why it
  outranks the `writeContent` asymmetry above. Pre-existing and out of this issue's scope; the
  round-14 blast-radius sweep surfaced it while closing the enumeration. **Being filed
...
- **The trap that followed from it, checked and found in three places.** Having inherited the
  ...
  **Resurrection is real at exactly one version, `0`, on the branch this driver does *not*
  ...
  merges.
- **After spec review, round 13 — the backwards blast radius, applied deliberately instead of
  discovered.** Five findings; **all five confirmed**, two of them again edges of this spec's
  own recent fixes. *(a)* The JSON-safety rule, widened in round 11 across value **types** and
  in round 12 across value **shape**, still had no case for a **branded object**: a `Date`,
  `Map`, `Set` or `RegExp` passes `z.array(z.any())`, is preserved intact by the memory store,
  and is flattened by every JSON-backed adapter — `Date` to a string, the rest to `{}`. The
  rule is now stated as **"survives a JSON round-trip unchanged"**, which is the property the
  storage layer needs and which no future kind can fall outside. *(b)* The retry-path sweep,
  widened in round 12 from `5–9` to `5–10`, still missed **behaviour 16**. *(c)* An edge-case
  row still promised a universal runtime unknown-key throw, contradicting round 10's own
  opaque-schema correction. *(d)* Decision 1 still called the atomicity difference
  **permanent** after D-6 changed decision 2 from *dropped* to *deferred*; §12 records that
  the liveness-gated variant writes without a held version by design, so the difference is
  scoped to the shared mutators rather than permanent. *(e)* Behaviour 9 tested only numeric
  deltas.
- **Round 12's fix for the range was itself the lesson repeating.** It wrote *"every throwing
  behaviour in the list"* and then immediately narrowed it with `5–10` in the same sentence —
  so the constructive form was decorative and the numeric one still governed, which is how
  behaviour 16 sat outside a sweep one round after the lesson was written down. **A range
  rewritten to a wider range has not learned the lesson; it has postponed it.** Both sweeps
  are now predicates that read the list (*every behaviour whose expected outcome is a throw*),
  with the current members named as a derived aid rather than as the rule. The same treatment
  was applied to four more enumerations the sweep turned up on its own: §9's refusal taxonomy,
  §7a's new-refusals table row, the "must not be written as schema-check tests" pair, and the
  POC-graduation row range.
- **The delta argument had never been checked, and that is a data-loss path, not a type nit.**
  Chasing (e) into the POC rather than reasoning about it turned a P2 into the worst refusal
  gap in the spec. Every one of §7's refusal rules checked the stored **field**; none checked
  the caller's **argument**. Verified on the real path: a `bigint` or symbol delta throws a raw
  `TypeError` — not a `FlowError`, so `isRetryableError` returns **`true`** and the block
  re-runs its side effects — and a **string** delta throws nothing at all. `0 + "5"`
  concatenates, the schema rejects the result, and the write path **replaces the whole state
  with the schema default and reports success**: a row holding `keep: "DO-NOT-LOSE"` came back
  as `keep: "schema-default"`, version bumped, no error. That is row 11's destructive
  replacement reached through an argument. The rule is now *reject any delta that is not a
  finite number, **before the arithmetic***, and the ordering is stated as the contract.
- **Four POC rows instead of four more paragraphs.** The refusal hazards had been argued in
  prose for three consecutive rounds, each round enumerating the kinds known at the time and
  each round missing a neighbour. They are now **run**: branded objects, the retryable raw
  `TypeError`, the silent row reset, and a control pinning that a plain `Error` is retryable
  under any policy. They characterize the **gap**, not the guard — the guard does not exist
  yet — so §10 notes that graduating them requires inverting their polarity. **The transferable
  part: when a rule has been corrected in prose twice, the third correction should be a test.**
  Prose can be wrong in a way that reads fine; a row that asserts `keep: "schema-default"`
  cannot.

**After spec review, round 14** — two findings of a different class from the twelve rounds
before them, which had converged on wording rather than on calls.

- **The blast radius was drawn one hop short, and it was the paragraph that taught the
  lesson.** §7 already said the earlier draft "drew the radius around the new methods instead
  of around the path a caller reaches" — and then enumerated the three *direct* callers of
  `persistResourceState` and stopped. Two more public entry points funnel into those three:
  **`getOrPatchState`'s miss path** (`applyGetOrPatchState` calls `ref.patchState`,
  `resource.ts:531`) and **`ref.edges.add` / `.supersede` / `.remove`**
  (`createResourceEdgeApi` calls `ref.updateState`; wired onto single handles at
  `resource-registry.ts:1653`). Both were silently taking the changed refusal. **The fix is
  the round-13 form applied one level up:** the radius is now stated as a *derivation over the
  callers*, with today's set as its output — the same move that took eight numeric
  enumerations onto predicates, applied to a list of names rather than a range of numbers.
  **The transferable part: a blast radius written as a list of names goes stale exactly the
  way a numeric range does, and the fix is identical.**
- **Closing the enumeration found two defects the sweep was not looking for.** Asking what
  does *not* reach the refusal — rather than only extending what does — surfaced that
  `writeContent` carries its own read-only throw and so **stays retryable** while state writes
  stop being, and that **`writable: false` on a collection type-checks clean and is then
  ignored**. Both filed in §12, neither fixed here. A sweep that only extends a list cannot
  find either.
- **And the second of those was initially doubted on a reasonable-sounding ground that turned
  out to be backwards.** The objection was that `ResourceCollectionConfig` declares no
  `writable` field, so nothing could be "ignored" — the declaration would simply not compile.
  It compiles: `defineResourceCollection` takes `config: TConfig` with `TConfig` a **naked type
  parameter**, so TypeScript's excess-property check never runs. Confirmed with
  `tsc --noEmit --strict`, against a single-resource control. **The absence of a field is not
  the same as the rejection of one** — and the gap between those is exactly where a
  silent-acceptance defect lives. §12 states it as *accepted and ignored*, which is both the
  accurate claim and the more serious one.
- **Behaviour 17 asked for a test that cannot be written; settled as a runtime refusal.** Both
  handles are built from the schema's **output** (`StateOf<T> = S["_output"]`), so a
  type-changing transform is structurally identical to an ordinary schema and cannot be
  excluded at compile time. **Settled: the verbs stay offered and the refusal happens at
  runtime**, via a rule §7 already carried. *(This entry originally read "every call refuses
  non-retryably" — corrected in round 15: the refusal is conditional on the candidate not
  re-parsing as input. The settlement about **where** it is enforced is unaffected.)* The rejected alternative — carrying the
  schema's input type through both handles — was found **expressible** and rejected on
  **efficacy**, which is the part worth keeping: a 13-assertion type probe showed it would
  *wrongly exclude a defaulting schema* (a case the spec requires offered), would still miss
  the same-type transform, and would be **vacuous at all 22 in-tree consumption sites**, which
  hand-write their annotations. **Cost never entered it.** §12 records the evidence and the one
  condition that reopens it, so the next person to have the idea meets the probe instead of
  re-running it.
- **The transferable part: "we can't check this at compile time" is a decision about what to
  promise, not a limitation to note.** The tempting move was to keep §7's wording and soften
  the test. That would have left the spec promising *absence* while shipping *refusal* — two
  different things to a caller, and the docs plan would have published the wrong one. Splitting
  the rule by **enforcement layer** cost a paragraph and made §11 publishable.

### Round 15 — the sweep run forwards

- **Every sweep so far ran backwards, and that direction has a blind spot.** Rounds 13 and 14
  asked *has a neighbour outgrown this rule?* and found real defects three rounds running.
  Nobody asked the forward question: *now that this rule is wider, what does it wrongly
  include?* **A widening fixes an under-inclusion and can create an over-inclusion in the same
  edit** — and the new members do not look wrong from inside the widened rule, because they
  satisfy it. Three of this round's findings are that shape.
- **The over-inclusion the sweep was aimed at: field survival.** Rounds 13–14 correctly
  rewrote the retry sweep as a predicate over *every behaviour whose outcome is a throw*, and
  the field-survival rule was written to inherit that same set. The predicate now also selects
  behaviours 15 and 20, which raise `ResourceDeletedError` **after the row is gone** — so
  "assert all other fields survive" is not merely untested there, it is **impossible**. Field
  survival now carries its own predicate: *refusals that leave the row live*. That is a real
  distinction — a pre-write refusal rejects a candidate while the state stands; a deleted-row
  refusal is a post-mortem — not a carve-out to keep the old list working. The same pass found
  the throw enumeration itself stale for the fourth time (15 and 20 were missing from it).
- **The same shape, authored one round earlier: "every call refuses non-retryably."** A single
  bullet in §7 stated a **condition** in its heading ("output re-parses as input") and a
  **universal** in its body, and the universal had already propagated to §7a, §10 17b, §11's
  docs brief and §12. It is false:
  `z.union([z.number(), z.string()]).default("0").transform(Number)` is type-changing, its
  input side accepts its own output, and the call **succeeds**. "Type-changing" was never the
  property doing the work — **re-parse acceptance is**, and most type-changing transforms fail
  it, which is exactly how a universal gets written from a sample of one. Now stated as a
  three-row table over two orthogonal properties (re-parse acceptance decides refusal;
  idempotence decides drift), with POC row 18 and behaviour 17c as the counterexample.
- **And a third, on the round-15 fold that scoped the unknown-key backstop.** That fold defined
  "inspectable" as `.shape` on the schema handed in. Five ordinary wrappers — `.default()`,
  `.refine()`, `.catch()`, `.readonly()`, `.brand()` — expose no `.shape` of their own while
  declaring their keys one hop down, so the rule as written classed them uninspectable and
  wrote their backstop out of existence. Worse, they **strip** unknown keys, so the untyped
  typo degrades to the silent no-op the backstop exists to prevent, and every prescribed
  assertion stays green. The fix separates two axes the fold had collapsed: **reachability**
  decides whether the check can be implemented, **stripping** decides which silent failure a
  typo produces, and they do not line up.
- **The transferable rule: after widening a predicate, walk its new members and ask what each
  now owes that it cannot pay.** Cheaper than the round that finds it. And the blast radius
  does not respect authorship — two of the three over-inclusions above were introduced by the
  same pass that ordered the sweep.
- **A derivation's published output goes stale even when the derivation is right.** §7 states
  the read-only blast radius as a derivation over callers precisely so a list cannot rot — and
  its worked output still missed `ref.edges.pruneDangling`, which reaches `ref.updateState`
  through `updateStateWith` rather than calling it inline, one hop further than a grep for the
  call site reaches. It has a live in-tree consumer (the memory janitor), so this was an
  observable retry-class change with a caller, not a theoretical entry point. Recorded because
  the failure mode is specific: **a derivation stated correctly and then enumerated by hand
  once inherits every weakness of the list it replaced.**
- **The scoping trap on the way out, noted because it is the mirror image.** Adding the public
  forwarders to that radius invites writing "the helpers are affected" — but `withOutcome` is
  generic over any mutation runner, including a scope's `atomicState` and the task
  collection's `casWrite`, neither of which touches `persistResourceState`. The row is scoped
  by its **runner**, not by the helper's name.
- **Two test-only findings, both of which every prescribed assertion already passed.**
  Negative zero clears `Number.isFinite`, survives normalization with its sign, is preserved
  by the memory store and becomes `0` under JSON — and `-0 === 0`, so a `===` assertion on
  that row *cannot fail*. Sparse arrays are subtler than they first read: `forEach`, `map` and
  `filter` **skip holes entirely**, so a guard that "recursively validates present elements"
  finds nothing to reject. Under an array-typed schema zod's own parse densifies the hole, so
  both adapters converge and the whole defect is that the guard never fired; only under a
  schema that does not re-walk the array do the adapters genuinely disagree. Both rows now
  specify the comparison (`Object.is`, and occupancy rather than element values), because the
  natural assertion is the one that cannot detect the bug.
