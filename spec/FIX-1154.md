...
promise the same thing. On the state bag an increment can be *contention-free* — it skips
the version check entirely, so a concurrent writer cannot refuse it. On a resource it can
never be, because a resource can be deleted and a check-free write would bring a deleted
one back. So a resource increment is version-checked and can, under sustained contention,
be refused.

The bag's half of that is **conditional, not an API-wide promise**: skipping the check
depends on the storage adapter implementing a native increment, which the storage contract
makes optional. Every adapter we ship implements it; one that doesn't falls back to a
version-checked write that can be refused like any other. So the honest contrast is between
a resource, which can *never* skip the check, and the bag, which skips it on the path our
adapters take. Shipping the verb without shipping that distinction would replace one trap
with a worse one.
...
counter-intuitive: on the state bag a native delta verb avoids contention *because* the
version check is skipped alongside it. Resources may not skip it — a check-free write to a
resource can resurrect a deleted one — so a native resource increment would still carry a
held version, and conflicts exactly as often as a whole-value write does. It saves
bandwidth, not conflicts.
...
scope bag    await ctx.session.incState({ n: 1 })
             → on an adapter with a native increment (all of ours): the version
               check is skipped, so a concurrent writer cannot refuse it
             → on an adapter without one: falls back to a version-checked write,
               and can be refused like any other bag write
...
| 10 | **Concurrency semantics of the verbs they share** | a commutative write (single-field literal patch, increment, append) is contention-free **on the native-delta path only** — the version check is skipped when the adapter implements the matching delta verb. Those verbs are **optional on the store contract**: an adapter without one falls back to a version-checked whole-record write, which can be refused. Every adapter we ship implements them | every resource write is version-checked and can raise a conflict or a deleted-resource error | **Deliberate and permanent on the resource side** — resources have a lifecycle and bag records do not, so a check-free write to a resource could resurrect a deleted one. The bag side is **conditional**, and the condition has to travel with the claim: the durable contrast is *never skips the check* versus *skips it on the path our adapters take*, not *always* versus *never*. **No caller-facing doc states either half today.** The sharpest row in the table, and the easiest to publish wrongly |
...
- **Map row 10, the bag side** — on an adapter without the native delta verb, a losing
  commutative bag write is not retried: it is reported to the caller as a no-op. That is a
  silently dropped write, and it is what makes row 10's bag half conditional rather than
  absolute. Out of scope here (this issue does not change bag behaviour) and already
  tracked by [FIX-710](https://linear.app/fixpoint-labs/issue/FIX-710), which exists to make
  commutative bag writes contention-free. Recorded so the row ships with its reason.
...
- **After spec review, round 1** — the bag half of map row 10 was wrong. It claimed a
  commutative bag write is unconditionally contention-free; the persist bridge only reaches
  the check-free path when the adapter implements the matching delta verb, and those verbs
  are optional on the store contract. Row 10, §2 and the §5 example now state the condition
  instead of an API-wide promise. The resource half is unchanged, and so is decision 2 — a
  native resource delta verb still could not skip the check, so it was never a contention
  fix. The narrower claim is the one that ships to the docs.
