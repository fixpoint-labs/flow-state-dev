...
module · ~13 test behaviours · 3 doc surfaces · 3 PRs.
...
you   add FSD to this project
      ...
      ▸ running FSD detection …
        host: next 15.4 (app router, ./app)  ·  package manager: pnpm (pnpm-lock.yaml)
        module: commonjs
...
|---|---|
| `next` | `fsdev.config.mts` · `flows/hello/flow.mts` · `app/api/flows/route.ts` · `app/api/flows/[...path]/route.ts` · `.gitignore` · `.env.local` · `AGENTS.md` — **seven**, because the mount is two files |
...
3. **The install skill's content and the docs.** `SKILL.md` authored against the report's JSON
   ...
   instruction set names no file outside its host's allowlist. *Depends on:* 1, 2,
