...
**Size:** Medium — detection scripts · 1 authored Claude skill · 2 next-steps source blocks ·
1 wiring contract · ~29 test behaviours · 3 doc surfaces · 3 PRs.
...
1. **The safety properties are stated once and carried as instructions, not as branches.** Each
   ...
   - **Write the ignore entry first, then ask git whether the file is actually ignored, and only
     then write the key.** Untracked is not the same as ignored, and our own entry is not the
...
The config declares all three as `intents.chat` candidates with the chosen one as `default`, so
the resolver picks the first candidate that has both a key and an installed package. **The key
may already be present** — a project with `OPENAI_API_KEY` in an untracked `.env.local` is asked
to confirm rather than retyping it, which is what the goal check exercises when it supplies the
key at the prompt rather than pre-exporting it. A refusal to answer is a refusal to proceed:
there is no default provider, because guessing one installs the wrong package and writes the
wrong variable name.
...
**Declared substitutions — everything that varies is one of these or it is not in the block:**

| placeholder | filled with |
|---|---|
| `{{run}}` | the package manager's run form — `npm run`, `pnpm`, `yarn` |
| `{{exec}}` | its exec form — `npx`, `pnpm exec`, `yarn dlx` |
...
**The host shape selects which block, and is not a substitution.** A Next host and a plain-Node
host do not differ in a value — they differ in *which servers exist and what the caveats are*.
The Next block names the host's own dev command alongside `fsdev dev`; the Node block names
`fsdev dev` plus the loopback-only `fsdev serve --host 127.0.0.1` and the authentication caveat
that goes with it, and it has no host dev command at all. Forcing that into one text would need
undeclared variation, which is precisely what disqualifies something from being a source block.
**So there are two blocks, each canonical, each digested, sharing the substitution set above** —
and `{{devScript}}` / `{{devUrl}}` are required only by the Next block, because only the mounted
path has a host server to point at.

*This was raised at epic altitude too, against theme 5's package-manager-only substitutions.*
**It is fixed here, once**, because theme 9's table names FIX-1159 as this artifact's author and
rule 2 makes "what varies, and how" the author's declaration. The epic needs no second fix; what
it should carry forward is that a shape difference is a block boundary, not a placeholder.

**The check is a digest, per theme 9 rule 1**, and both sides are recomputed from content with
neither hash covering itself: take the bytes between the delimiters, delete every line matching
the marker pattern, normalize line endings and trim trailing whitespace and blank lines, then
`sha256`. The canonical side carries no marker, so step two is a no-op there and is exactly what
breaks the self-reference on a shipper's side. **The normalization is implemented once as a
shared helper** rather than twice — FIX-1160 runs the same steps for the agent-instructions
block, and "exclude the marker" is the instruction two implementations satisfy differently
(§12).
...
ASK provider + key, before any write        ← no default; the answer decides the
     package, the env var name and the config's model intent
     (a key already in an untracked .env.local → confirm, don't retype)
...
CREDENTIALS, in this order and no other:
     ...
     3. only if it is ignored, write the key. Otherwise stop and name the
        line that re-included it
...
| artifact | author | kind | this issue's role | check |
|---|---|---|---|---|
| wiring contract | **this issue** | specification | author; the skill is also a shipper | behavioural — theme 8 parity across both paths, plus the dependency rule, run under pnpm |
| next-steps block | **this issue** | source block (substitutions above) | author; the skill embeds it | digest |
| agent-instructions block | FIX-1160 | source block | **shipper only, and not even the embedder** — the skill carries a named placeholder that FIX-1160 substitutes with the canonical block and its marker when it packages the plugin | digest, owned by FIX-1160 |
...
prose: that is the drift the digest exists to catch, and it would also make this issue a second
author of something it does not own.

**A specification has no digest, and that is deliberate rather than an omission.** The wiring
contract's shippers produce genuinely different output — the template writes a chat page and
`flows/chat.ts`, a brownfield Next run writes `flows/hello.ts`, a plain-Node run writes no mount
at all — so there is no common text to hash. Conformance is the only honest check, and it is
behavioural: both paths' proofs have to produce the same observable shape, under pnpm, with the
dependency rule holding. A digest here would either force the outputs to be identical, which
they are not, or hash something so abstract it could not fail.
...
2. **The next-steps blocks.** The two canonical texts, their declared substitutions, and the
   shared digest-normalization helper. *Test:* substituting the same block for different managers
   changes only the commands; the two host shapes are separate blocks with separate digests; the
   helper strips marker lines identically on both sides; and the executed-command check from §10.
...
| b | the next-steps blocks: canonical texts, declared substitutions, shared digest helper (step 2) — **this is what FIX-548 waits on** | — |
...
| Node below the floor | The floor is the higher of the repo-wide `>=22` every FSD package declares and the 22.18 boundary native type stripping needs, and detection reports the host's version so the run refuses before writing. **A version check is not optional here:** Next 16 accepts Node 20.9+ and npm's engines check only warns, so without this gate a run completes and hands the developer an app whose FSD dependencies cannot run. This gates the project being wired up; it narrows no package's own `engines`. |
...
| The developer declines to choose a provider, or supplies no key | Refuse before writing. There is no default provider: guessing installs the wrong `@ai-sdk/*` package and writes the wrong environment variable name, and the generated config's model intent would name a model with no credential behind it. |
...
- **a config carrying our marker does not trigger the unregistered-flow warning**, and one
  ...
  rule covered both
- **the skill's copy of each next-steps block is current** — recompute the digest from the
  canonical text **and** from the skill's embedded block, using the shared normalization helper,
  and compare. Both sides recomputed, neither hash covering itself. Asserted per host shape, so a
  stale Node block is not hidden by a current Next one, and it fails by naming which copy is
  stale
- **the normalization helper is the same one FIX-1160 uses** — asserted by both artifacts' digest
  tests importing it, since "delete the marker line" is exactly the step two implementations
  satisfy differently and a divergence there is invisible until a copy wrongly passes
...
- **The currency mechanism landed at the epic and this issue now conforms — nothing is deferred.**
  It was the one outstanding external item last revision. Theme 9 settles it as four explicit
  normalization steps with the marker line stripped, both sides recomputed, neither hash covering
  itself, and §7 adopts them verbatim. Two consequences this issue owns: **the normalization is
  implemented once as a shared helper**, since FIX-1160 runs the identical steps for the
  agent-instructions block and "exclude the marker" is where two implementations would silently
  diverge; and the earlier worry that a digest cannot see whether the prose still *expresses* the
  content is answered by rule 2 rather than by hashing harder — the block is embedded **verbatim
  with placeholders intact**, so there is no separately-worded copy for the hash to miss. What
  varies is the rendered output, not the block.
...
- **After spec review — a credential leak, a deadlock, an undefined interaction, and a shape the
  ...
  second-process Node host — those are two blocks now, not one block with a value swapped.
- **Folded from the epic at `ca88539`: theme 9's finished mechanism.** The next-steps block is a
  **source block** with declared substitutions, embedded verbatim with placeholders intact and
  ...
  normalization steps with the marker line stripped, both sides recomputed, **implemented once as
  a shared helper** with FIX-1160. The agent-instructions block reverses direction: FIX-1160 both
  authors and embeds it, and this skill carries a named placeholder. The wiring contract is a
  **specification**, checked behaviourally rather than by digest, because its shippers' outputs
  legitimately differ. And the runtime floor became a stated rule — Next 16 accepts Node 20.9+ and
  npm's engines check only warns, so a run without the gate hands over an app that cannot start.
