...
conditional branches) + its exported comparison · 1 wiring contract · **30 named CI behaviours**
(≈43 assertions once the per-decision and per-fixture sub-cases are expanded) + the 2 goal checks ·
...
**Three roots, defined separately because they genuinely differ:**
...
| **the workspace root** | the nearest ancestor of the write root carrying a lockfile, a `packageManager` field, `pnpm-workspace.yaml`, or a `workspaces` array — the write root itself when there is no workspace. Reported as a path, or as "same as the write root" | **the package-manager answer.** `packageManager` and lockfiles are searched from the write root **upward** to it, so a workspace app resolves on the root's declaration instead of refusing |
...
| package manager | searched from the **write root upward to the workspace root**, nearest declaration first — a workspace app resolves on the root's field or lockfile rather than on the absence of its own. The `packageManager` field wins outright when present. Otherwise exactly one lockfile → that manager. **More than one lockfile at the same level and no `packageManager` field anywhere in the chain → `ambiguous`, which refuses.** **Neither a field nor any lockfile in the whole chain → `undeclared`, which also refuses** — there is no third source (below). **Whatever the source, the answer is then validated against the supported set — npm, pnpm, Yarn — and anything else refuses**, because being certain which manager a project uses is no help when no command forms exist for it. The report names the directory the answer came from |
...
| **credential file** | every `.env.local` **from the write root upward**, the chain `loadEnvFiles` itself walks, with the nearest occurrence of the provider variable and its emptiness reported by path — because the nearest file wins at load and an empty value in a nearer file masks a real one above it. For the write root's own file: **tracked by git**, from `git ls-files --error-unmatch`; **and whether git would actually ignore it**, from `git check-ignore -q`. Tracked and ignored are different questions and both have to be asked |
...
| **package manager outside the supported set** | `packageManager` field / lockfiles | no |
| package manager ambiguous (two lockfiles, nothing declared) | lockfiles + `packageManager` field | no |
| **package manager undeclared and unevidenced** (no field, no lockfile) | lockfiles + `packageManager` field | no |
...
**So the credential rule is keyed on the value, and its scope is the whole chain the loader walks
— not our delimiters, and not one file:**

| state of the provider variable, **anywhere from the project directory up** | what the run does |
|---|---|
| absent from every `.env.local` in the chain | append our delimited section containing `KEY=` with an empty value to the **project's own** `.env.local`, then ask the developer to fill it |
| present with a non-empty value in **any** file in the chain | **write nothing at all.** Do not add our section, do not duplicate the key, do not touch the existing line. Report that the credential is already configured **and name the file that holds it**, since it may not be the one the developer is looking at |
| present with an empty value | leave that line exactly where it is and ask the developer to fill **it**, naming its path when it is not the project's own file. Never add a second one |
...
READ THE ROOTS FIRST — every path below is relative to one:
    write root      = the dir whose package.json declared the host.
                      THE ONLY DIR YOU EVER WRITE IN
    workspace root  = nearest ancestor with a lockfile / packageManager /
                      pnpm-workspace.yaml / workspaces. The package
                      manager may be declared THERE, not beside the app
    the repository  = what git answers, for tracked/ignored
if the report names a refusal condition:              ← BEFORE any write, always
    ...
    refusals: tracked .env.local · next-unsupported · package manager
              neither declared nor evidenced anywhere in the chain (two
              lockfiles, or none at all) · UNSUPPORTED package manager
              (only npm/pnpm/yarn)
...
DECIDE what .env.local needs, by reading THE WHOLE CHAIN:
    every .env.local from the write root UP — the same walk loadEnvFiles
    does. NEAREST file wins at load; LAST line wins within a file
    the provider variable non-empty in ANY of them → touch nothing, and
      NAME THE FILE that holds it — it may not be the one they can see
    present but empty → leave that line where it is; ask them to fill IT
    absent from the whole chain → append our section with KEY= to the
      write root's own .env.local
    ← never a second assignment for a key that already appears anywhere
      the loader reads. An empty KEY= in a NEARER file masks a working
      key in a parent, because "" is a set value to the loader
...
| The provider variable already has a value | Write nothing to `.env.local` — no section, no duplicate assignment. Report that the credential is already configured. **Replacing our delimited section would overwrite their key with the canonical empty value, and appending would mask it**, since the parser's later occurrence wins. |
| The provider variable exists but is empty | Leave that line where it is and ask the developer to fill it. Never add a second assignment for the same key, wherever the first one sits. |
| **The key lives in an ancestor `.env.local` — a workspace root above the app** | Write nothing. Report that the credential is already configured **and name the file**, since the developer looking at `apps/web/.env.local` sees an empty directory's worth of nothing. Appending `KEY=` here would be the quietest failure in this document: the nearer file is loaded first, `""` counts as set, and the working parent key is never applied — leaving a project whose own `.env.local` contains the right variable name and which fails with `No provider available`. |
