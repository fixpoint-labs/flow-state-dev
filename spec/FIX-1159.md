...
conditional branches) + its exported comparison · 1 wiring contract · ~36 test behaviours ·
...
enabled extension for every slot the run would occupy**, and any occupant refuses — the same
generalization already applied to the app root and the extension, carried one step further to the
thing those two facts are used to build.
...
| **any file in the route slot already exists** | the filesystem, across every enabled extension | no |
...
if the report names a refusal condition:              ← BEFORE any write, always
    ...
    refusals: tracked .env.local · next-unsupported · ambiguous package
              ...
              · an occupied route slot · no identifiable dev script
...
PREFLIGHT every path on your host's allowlist:
    ...
    their fsdev.config → not a collision; you will skip writing a config
...
| A file of any enabled extension already occupies a route slot | Refuse before writing, naming the occupant. `route.js` occupies the slot `route.ts` would take: whichever file Next then prefers, the result is either silently replacing the developer's endpoint or silently failing to serve ours. |
...
- **a populated credential survives a rerun** — seed `.env.local` with a real-looking value, run
  ...
  value is non-empty
- **a route slot is checked across every enabled extension** — a fixture owning
  `<appRoot>/api/flows/route.js` under default extensions refuses, with nothing written. Asserted
  per extension in the enabled set rather than for `.js` alone, so the check does not encode the
  one example that was reported
