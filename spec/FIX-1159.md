...
conditional branches) + its exported comparison · 1 wiring contract · **25 named CI behaviours**
(≈34 assertions once the per-decision and per-fixture sub-cases are expanded) + the 2 goal checks ·
...
1. **The safety properties are stated once and carried as instructions, not as branches.** Each
   ...
   - **Never overwrite or duplicate a credential the developer supplied.** A populated provider
     variable means write nothing to that file at all — replacing our own section would put the
     canonical empty value over their key, and appending a second assignment masks it at load.
...
| package manager | the `packageManager` field wins outright when present. Otherwise exactly one lockfile → that manager. **More than one lockfile and no `packageManager` field → `ambiguous`, which refuses.** **Neither a field nor any lockfile → `undeclared`, which also refuses** — there is no third source (below). **Whatever the source, the answer is then validated against the supported set — npm, pnpm, Yarn — and anything else refuses**, because being certain which manager a project uses is no help when no command forms exist for it |
...
| **`.env.local` would still be publishable after our ignore entry** | existing `.gitignore` bytes, evaluated as *proposed* content in a scratch location | no |
| provider not chosen | the developer's answer, taken before any write | no |

**The two rows in bold are new this pass, and the third bold row was previously a
write-then-check** — the same violation, arrived at from a different direction. §10 asserts the
invariant directly rather than only its instances: for each row, a fixture that trips it must
...
appended when it is present, replaced between our own delimiters on a second run. Nothing outside
the delimiters is read or rewritten. `package.json` is not on that list — the project's own
...
**So the credential file gets its own rule, keyed on the value rather than on our delimiters:**

| state of the provider variable | what the run does |
|---|---|
| absent from the file entirely | append our delimited section containing `KEY=` with an empty value, then ask the developer to fill it |
| present **anywhere** with a non-empty value | **write nothing at all.** Do not add our section, do not duplicate the key, do not touch the existing line. Report that the credential is already configured |
| present with an empty value | leave it exactly where it is and ask the developer to fill that line. Never add a second one |

**The rule never writes a second assignment for a key that already appears**, which is what makes
the masking case unreachable rather than merely unlikely. A key the developer later moves outside
our delimiters keeps working, because the rule looks at the whole file rather than at our section.
...
tidy, but because the model the generated flow names, the dependency that gets installed, the
...
declared contract, not the implementer's to invent**, because four artifacts have to agree:
...
if the report names a refusal condition:              ← BEFORE any write, always
    ...
    refusals: tracked .env.local · next-unsupported · package manager
              neither declared nor evidenced (two lockfiles, or none at
              all) · UNSUPPORTED package manager (only npm/pnpm/yarn)
...
DECIDE what .env.local needs, by reading it:
    the provider variable present and non-empty ANYWHERE → touch nothing
    present but empty → leave it; you will ask them to fill THAT line
    absent → you will append our section with KEY=
    ← never a second assignment for a key that already appears: the later
      one wins at load and masks their working key
