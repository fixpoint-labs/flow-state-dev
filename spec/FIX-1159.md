...
Three deliverables, and none of them is something a person types.

**Detection.** Which package manager, App Router or Pages, whether a config already exists,
whether `.env.local` is tracked by git. Cheap, checkable facts about a repository, and an agent
guessing at them is exactly the failure this issue exists to prevent. They ship as deterministic
scripts, and their output — a report — is the contract everything else reads.
...
1. **This issue ships no `fsdev init` — not the command, and not a library entry point behind
   ...
   *Locks in:* **FIX-548's spec now has a hole in it** — its decision 3, its hand-off table, its
   fourth build step, four test behaviours and its dependency note all rest on a seam that will
   not exist, and it converged over three review rounds in that shape. Nothing in this document
   can close that; it is the first ask on the PR. What this issue gains is that FIX-548 waits on
   one small module rather than on this issue merging.
...
**Size:** Small–Medium — detection scripts · 1 authored Claude skill · 1 shared next-steps
module · ~11 test behaviours · 3 doc surfaces · 3 PRs.
...
**The next-steps block.** The steps as structured content — which server, what it is for, which
port, which caveat — plus a renderer taking the detected package manager. Two consumers: this
skill, and FIX-548's `create-flow-state`. It ships where `create-flow-state` can depend on it,
and it is the only thing that crosses that seam (decision 1). The reviewer note that once called
a reusable next-steps module premature is answered by the epic requiring exactly one authored
source for it.
...
**What the skill is required to point at, and the seam that does not exist yet.** The epic makes
FIX-548's checked-in template the reference shape and forbids the skill from restating its
contents; an instruction that restates the template instead of pointing at it is the named drift
signal. The same epic forbids any instruction that tells an agent to read our monorepo. A
brownfield developer installs the plugin and nothing else, and the template is neither shipped in
the plugin nor assigned to it. **So the skill has no specified way to reach the shape it is
required to point at.** What it needs to reach is the template's *shape* — which files exist,
what each contains, how they wire together — at the moment it is authoring the equivalent files
in someone else's repo. The mechanism is a cross-issue question the epic owns and is deciding
separately; it is recorded as a blocking dependency in §12 and is deliberately not guessed at
here.
...
3. **The install skill's content and the docs.** `SKILL.md` authored against the report's JSON
   and the block's renderer. *Test:* every fact the skill acts on appears in the report; its
   instruction set names no file outside the six it is permitted to touch. *Depends on:* 1, 2,
   **and the epic's decision on how the skill reaches the template** (§7, §12).
...
| c | the `install-fsd` skill content, docs (step 3) | a, b |
...
**Taxonomy:** detection writes nothing, so it cannot fail destructively. Every refusal —
tracked credential file, Node below the floor, unidentifiable host — is stated before anything
is written. A failed install is non-fatal and retryable.

### 10. Testing strategy

**Goal checks** (the epic's proof for this issue; real projects, real model, both brownfield):

- *Mounted-route path* — a fresh `create-next-app`, with distinctive content seeded into
  `package.json`, `.gitignore`, `.env.local` and `AGENTS.md` and a route that existed before the
  run. An agent is given the install skill and nothing else. Afterwards every seeded string is
  intact, the pre-existing route still responds, the bare `/api/flows` and a nested path both
  respond, and `fsdev run` returns a streamed model response. **The run's own report is compared
...
- **the skill's rules are checkable** — its first instruction runs detection; its instruction set
  names no file outside the six it is permitted to touch; it states which shape the host got
- **the skill does not restate the template** — the drift tripwire, as a check on the
  instructions' contents. *This check cannot be written until the epic decides how the skill
  reaches the template* (§7, §12); the two goal checks above are unaffected by that decision,
  because they assert what the skill produced rather than where it read the shape from
...
- **The epic owes a decision on how the install skill reaches FIX-548's template.** The skill is
  required to point at the template rather than restate it, must not read our monorepo, and
  reaches a stranger only through the plugin — which carries neither the template nor a pointer
  to it. §7 states what the skill needs to reach and why. The mechanism is a cross-issue shape
  and the epic is deciding it; this document deliberately does not guess. **What it blocks:** the
  drift check in §10 and the final wording of the skill's authored instructions — step 3 of §8,
  sub-PR c. **What it does not block:** detection, the next-steps block, and both goal checks.
...
- **FIX-548 depends on the next-steps block, and on nothing else here.** That is sub-PR b, which
  has no dependencies of its own and can land first. **What FIX-548's spec currently says instead
  is broader and no longer holds** — see the open question below.
...
- **FIX-548's spec depends on a seam decision 1 removes, and only its owner can route that.**
  Its decision 3 calls `fsdev init` in-process for the API key, the ignore entry, the
  agent-instructions section, the dependency install, and the printed block; its hand-off table
  describes that seam as five inputs and one return; its fourth build step depends on "FIX-1159's
