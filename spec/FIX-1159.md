...
conditional branches) + its exported comparison · 1 wiring contract · ~40 test behaviours ·
...
| package manager | the `packageManager` field wins outright when present. Otherwise exactly one lockfile → that manager. **More than one lockfile and no `packageManager` field → `ambiguous`, which refuses.** Neither → `npm_config_user_agent`, reported as a guess: the documented invocation is `npx`, so the user agent says npm regardless of what the project uses. **Whatever the source, the answer is then validated against the supported set — npm, pnpm, Yarn — and anything else refuses**, because being certain which manager a project uses is no help when no command forms exist for it |
| **route slot occupancy** | for each slot the run would write — `<appRoot>/api/flows` and `<appRoot>/api/flows/[...path]`, leaf `route` — whether **any** enabled extension is already present. The unit is the slot, not the filename we picked |
...
mount silently never answers. Both are undetected collisions. **So the preflight tests every
enabled extension for every slot the run would occupy** — the same generalization already applied
to the app root and the extension, carried one step further to the thing those two facts are used
to build.
...
| the slot's occupants | what the run does |
|---|---|
| none | write our file at the selected extension |
| exactly one, carrying our marker at the selected extension | **ours.** Rewrite or leave, per the rerun rule below |
...
plus a leaf name, across extensions) differs from the *unit being owned* (a file with a marker in
it), and collapsing them is what produced the contradiction.
...
| package manager ambiguous | lockfiles + `packageManager` field | no |
| no identifiable dev script | `package.json` `scripts` | no |
| **a route slot holds an occupant that is not ours** — no marker, a stale marker, or ours at an extension no longer enabled | the filesystem, across every enabled extension, plus our markers | no |
...
|---|---|---|
| absent | nothing there — for a mount, **no occupant in the slot at any enabled extension** | write it, with a marker |
| ours | marker present and matching — for a mount, the slot's **only** occupant is ours, at the currently enabled extension | rewrite or leave, and report which |
...
if the report names a refusal condition:              ← BEFORE any write, always
    ...
    refusals: tracked .env.local · next-unsupported · ambiguous package
              manager · UNSUPPORTED package manager (only npm/pnpm/yarn)
              · host unknown · Node below 22.18 · unreadable pageExtensions
              · a route slot holding an occupant that is NOT ours
              · no identifiable dev script
...
PREFLIGHT every path on your host's allowlist:
    for a MOUNT path, test the whole SLOT — every enabled extension, not
    just the one you picked. route.js occupies the slot route.ts wants
...
| Our `route.<ext>` from a previous run sits at an extension `pageExtensions` no longer enables | Refuse before writing, naming the file. It is not a route today and becomes a second live handler in the same slot if the setting widens again. The run never deletes; the developer removes one file. |
...
- **a populated credential survives a rerun** — seed `.env.local` with a real-looking value, run
  ...
  value is non-empty
- **a route slot is checked across every enabled extension, and classified by marker** — three
  fixtures, because the refusing half alone is what produced the contradiction:
