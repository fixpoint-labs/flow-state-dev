...
1 wiring contract · ~23 test behaviours · 3 doc surfaces · 3 PRs.
...
**Nothing may print a command the project cannot run, and the renderer had no way to keep that
promise.** A brownfield host is not `create-next-app` output: a project may have renamed
...
port therefore become detected inputs to the renderer, not constants.** The script is chosen by
...
So the rule is a **preflight**: resolve the host's whole allowlist, test every path in it, and
only then write anything. If any required path exists and is not ours, the run stops with nothing
touched, names each colliding path, and says what would resolve it — remove it, rename it, or
accept that the project already has its own. **Partial application is never a legitimate
outcome**, which is what turns a per-file decision into a whole-run one.
...
|---|---|---|
| `create-flow-state` (FIX-548) | a published npm package, i.e. code | **imports the module** and calls the renderer with the inputs it already holds |
...
**The renderer's inputs are the package manager, the dev command and its port — not the manager
alone.** That was the gap that let a fixed `pnpm dev` and a fixed `localhost:3000` reach a project
where neither was true. Greenfield passes what it just wrote; brownfield passes what detection
found. The content does not vary with any of them; only the rendered commands and the URL do.

**Currency between the module and the skill's copy is deferred to the epic, deliberately.** The
mechanism belongs at a higher altitude — the epic is reworking exactly this, because the digest it
first landed for the agent-instructions file turned out to verify the wrong thing: a hash can
confirm the copy was produced from canonical version X, and still pass when the copy's prose no
longer *expresses* X. Editing a command in the module, updating the embedded hash, and leaving the
skill's wording stale would satisfy the check while shipping brownfield users different
instructions. Two artifacts in this epic have that same shape, so **one general mechanism is worth
more than a local fix here that would then disagree with it** — §12 carries this as the one thing
still owed from outside. What is settled and does not depend on it: one canonical authored source,
two carriers, and a CI check that fails when they diverge.
...
PREFLIGHT every path on your host's allowlist, before writing anything:
    any of them present and not ours → stop, name them all, write nothing
...
run every command before printing it, then print the next-steps block
     ← rendered with the REPORT's dev command and port, never a fixed
       `pnpm dev` or a fixed localhost:3000
...
- **The epic owes the mechanism that keeps a copied content block current, and this issue
  conforms rather than inventing its own.** The next-steps block has one canonical source and two
  carriers (§7), so something has to fail when they diverge. A content digest alone does not: it
  can confirm the copy was produced from canonical version X and still pass when the copy's prose
  no longer expresses X. **The epic is reworking this now** — the agent-instructions file has the
  identical shape — and one general mechanism is worth more than a local variant here that would
  then carry different semantics for the same problem. **What it blocks:** the exact form of the
  skill's embedded marker and its CI check, inside sub-PR c. **What it does not block:** the
  canonical module, the renderer and its inputs, the skill's content, or any goal check. *If the
  mechanism the epic lands does not cover a block that is deliberately rendered differently by
  each carrier, that is this issue's case going unserved and it goes back to the epic rather than
  being patched here.*
