...
3b. **Write the fixture matrix before any scan** — two positives (one authoring skill, one
   ...
   violation injected into a copy (§10 tabulates what makes each check go red). **A negative goes
   wherever its check is claimed to reach** — for the packaging-token and tracking-ID scans, which
   are asserted tree-wide, that means a non-skill artifact, so an implementation that walks only
   `skills/**` cannot go green.
...
> the check it violates and green on all the rest. A negative lives wherever its check is claimed
> to reach: for the two tree-wide scans that means a non-skill artifact, not a `SKILL.md`.**
...
|---|---|---|
| framework facts | `"a generator streams model output"` **and** `"generators emit model-generated responses"` — the same shape in different words, in a **non-`SKILL.md`** skill file | `"if the block you wrote is a generator, verify with fsdev run"` — the term in a conditional, not a subject |
| project-shape facts | `"this project uses pnpm"` — the project as subject of a declarative clause | `"handles npm, pnpm and Yarn"`, and `pnpm-lock.yaml` as a thing to look for — the skill as subject, or imperative |
...
| `{{fsd:…}}` packaging token | a literal `{{fsd:agent-instructions}}` **in a non-skill artifact** — `plugin.json`, a detection script, a tree README | a fixture carrying `{{run}}` and `{{mountPath}}` — FIX-1159's invocation templating, which **must** survive packaging |
| outsider rule (issue/PR number) | an issue ID **in a non-skill artifact**, placed per the row above | *(the positive covers it)* |
...
**A negative also has to sit where the check claims to reach, which is a second axis and was
missed the first time.** Two of these scans are asserted over the **whole published plugin tree**,
not over skill files: the packaging-token scan (§9's most-likely failure) and the tracking-ID half
of the outsider rule. If every negative lives inside a `SKILL.md`, an implementation that walks
only `skills/**` goes green on the entire matrix while a literal `{{fsd:agent-instructions}}` in
`plugin.json` or an issue ID in a detection script ships. That is the same vacuity as a no-op
scan, arrived at through **scope** rather than through pattern — so the negatives for those two
rows are placed outside `SKILL.md`, and narrowing either scan's file walk turns the matrix red.

**The mood rows inherit this, and they are more exposed to it than the tree-wide pair.** A
structural check invites a narrow implementation — reading one file's prose is the obvious way to
build it — so the framework-fact and project-shape negatives also go in a **non-`SKILL.md` file
inside a skill directory**. The scan's unit is *a packaged skill's procedure, wherever that
skill's own files carry it*, not a filename: `skills/install-fsd/` already ships detection scripts
beside its `SKILL.md` (§7), a script's comment header is prose an assistant reads, and §13 floats a
shared `_spine.md` stitched in at build time. A second writer that sets up shop in any of those is
the same defect as one in `SKILL.md`, and a scan hardcoded to `SKILL.md` would go green on it.
