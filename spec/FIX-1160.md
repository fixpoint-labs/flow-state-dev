...
4. **Write the four authoring skills** on the §7 spine. Each grounds in `AGENTS.md`, reads
   ...
   *Test (re-drafted — the old form would fail the install skill; §6 decision 1):* every
   skill's first instruction reads or runs its declared grounding source, **and that source
   resolves to something the published plugin tree actually contains** — `AGENTS.md` for the
   authoring four, a detection script shipped in `skills/install-fsd/` for the install skill.
   The second half is new and it is the half with teeth: while the install skill's declared
   source was a CLI command fetched at runtime, this check read the skill's text and passed
   for a skill whose first instruction invoked something that would never exist — it could not
   fail on the scenario it was written for (§10).
   And no skill's
...
- **no unresolved substitution token appears anywhere in the published plugin tree** — a
  literal `{{…}}` in a committed skill is release-blocking (§9). This replaces the packed-tarball
  and print-versus-import checks, which proved surfaces §7 has removed
...
**What "body" means, for every scan below — fixed once, because three of them had the same
...
> minus any delimited canonical payload.**

The payload is exempt **by ownership, not by exception** — the same reasoning as the file-layout
row below. It is not a fact the skill states; it is the single canonical copy, authored here,
and it already has exactly one check of its own: equality against canonical. Scanning it would
be a second, weaker test of a fact that already has a strong one, and the two would disagree the
moment the canonical text mentioned a block kind — which it must, since that is its whole job.
The delimiters that bound it are the ones theme 6 already requires, so nothing new marks it.
...
**And the half that gives that check teeth: the declared source must resolve to something the
published plugin tree contains.** For the authoring four the source is the project's
`AGENTS.md`, which cannot be asserted from here — their check stays textual, and §9's row
covers the absent-file behaviour at runtime. For the install skill the source is a **detection
script shipped in `skills/install-fsd/`**, which is a file in the artifact this issue publishes,
so it is asserted directly: the path the skill's first instruction names exists in the tree
that a `github` install serves.
