...
2. **The plugin is distributed from a marketplace manifest in the `flow-state-dev`
   ...
   *Rejected:* a separate repository for the plugin (a second thing to keep in step with
   every framework release), and a public directory listing (epic Q2, re-priced by Q4 once
   the plugin became the brownfield channel; the owner held URL-only and attached the docs
   placement as the condition).
   ...
   *Locks in:* the marketplace name and the plugin name become public strings printed in our
   ...
   docs placement is now part of the deliverable rather than a preference: with brownfield
   shipping through this channel, a stranger who does not find the install source has no
   brownfield path at all, and burying it one click deep is the failure Q4 priced.
...
**Size:** Medium — 9 production files · 10 test behaviours · 5 doc surfaces · 1 PR.
...
we do not have and will not before launch — §12 carries it as a follow-up rather than a guess.
...
declarations for exact signatures, verify with the CLI.**

**Solution sketch — the spine every packaged skill shares. Pseudocode, illustrative.**
The spine is the same for both kinds; only the first line differs, and that difference is
decision 1 stated as code.

```
read the skill's grounding source                        ← the only line that varies
...
3. **Stand up the plugin skeleton** — `.claude-plugin/marketplace.json` at the repository
   ...
   *Depends on:* nothing.
4. **Write the four authoring skills** on the §7 spine. Each grounds in `AGENTS.md` and ends
   in a CLI run.
...
5. **Wire the manifest checks into CI** — validate the marketplace and the plugin, and run
   the install round trip. *Depends on:* 3.
6. **Docs** per §11, including the install source on the getting-started page and in the root
   README (decision 2). *Depends on:* 3.

The install skill's `SKILL.md` is **not a step here** — FIX-1159 authors it and drops it into
`skills/install-fsd/`. The POC established that costs no manifest edit, so neither issue
blocks the other on it; §12 says who wires what.
...
- **file layout — the four authoring skills only.** No procedure body names a path into the
  ...
  its file list is pinned from the other side by FIX-1159's own check that it "names no file
  outside the six it is permitted to touch". The fact still has exactly one owner and exactly
  one test; the test just lives in the other issue
...
source**, and **every packaged skill's procedure ends in a `fsdev` command**.
...
**The install source is a deliverable, not a preference** (decision 2). With no public
listing and brownfield shipping through this plugin, a stranger who does not meet the two
install commands has no brownfield path — so they go where a stranger actually lands, not
only in this issue's own guide.
...
- **FIX-1159** — now the heavier of the two directions, and it is no longer symmetric.
  ...
  - *FIX-1159 → this issue:* the install skill's `SKILL.md`, which lands in
    `skills/install-fsd/`, and the detection report that skill grounds in. **Neither blocks
    this issue's build** — the POC established that a skill directory needs no manifest
    entry, so the pack ships its structure and the fifth skill is a file drop whenever it
    arrives. What *is* blocked is the epic's brownfield outcome: a pack without
    `skills/install-fsd/` is a valid, installable, four-skill plugin with no brownfield path
    in it, and nothing warns (§9). §10's inventory assertion names the expected skills for
    exactly that reason.
...
- **Epic decisions that bind and must not be reopened:** the filename is `AGENTS.md`,
  ...
  source we publish, with no public directory listing — the epic labels this "install-by-URL"
  (Q2), re-priced and held by the owner at Q4, with the docs placement attached as its
  condition (§11); FSD's runtime `skills` concept keeps its bare name and FIX-417 / FIX-424
  are not touched (Q3).

**Open questions:** none that block. Two things are noted rather than asked. The fifth
authoring skill (storage adapters) is decided in §6 rather than handed over as a menu, and is
reversible in a word at sign-off. And the one-vendor gap in §3 — a brownfield developer not on
Claude Code has documentation and no skill — is a consequence of decisions already made at
epic level (theme 3, Q2/Q4), not a fork this spec is holding open; it is recorded as a
follow-up so it is visible if launch traffic contradicts it.
...
- **Settled:** adding a packaged skill requires **no manifest edit** — **CONFIRMED**: dropping
  ...
  `plugin.json` and `marketplace.json` untouched. Neither file lists its skills. This is what
  makes the FIX-1159 hand-off a file drop rather than a coordinated edit.
...
- **Q4's condition folded into the deliverable.** The owner held install-by-URL and required
  the install source on the getting-started page and in the root README, so §11 gains both
  surfaces and §6 decision 2 states them as part of the decision rather than as documentation
  taste.
