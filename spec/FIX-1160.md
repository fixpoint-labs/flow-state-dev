...
4. **Write the four Claude skills** on the §7 spine. Each defers to `AGENTS.md` for facts and
   ends in a CLI run. *Test:* every skill's first instruction reads `AGENTS.md`; no skill
   states a block kind or a config key. *Depends on:* 1, 3.
...
- no packaged skill states a block kind, a config key, or a directory convention —
  decision 1's convergence point, enforced mechanically rather than by discipline
