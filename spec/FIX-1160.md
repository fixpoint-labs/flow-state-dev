...
3. **Stand up the plugin skeleton** — `.claude-plugin/marketplace.json` at the repository
   ...
   *Depends on:* nothing.
3b. **Write the satisfiability fixtures before any scan** — one skill that satisfies every §10
   check at once, and one install-shaped equivalent. *Test:* every scan in §10 passes on the
   fixtures. **If a fixture cannot be written, stop and fix the check set** — four review rounds
   have each found a pair of checks that could not both hold, and this is the step that catches
   the next one. *Depends on:* nothing.
...
- **Approved is not the same property as implementable, and this spec is currently the first
  ...
  closing condition rather than a promise to be more careful: **build step 3b's fixtures.**
  Until a fixture exists that satisfies every check at once, treat §10 as unvalidated. That is
  the honest status line for anyone scheduling this work.
