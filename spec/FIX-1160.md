...
3b. **Write the fixture matrix before any scan** — two positives (one authoring skill, one
   install-shaped) that satisfy every §10 check at once, and one negative per check, each a single
   violation injected into a copy (§10 tabulates what makes each check go red). **Where a check's
   scope spans more than one class of file, it gets a negative in each class** — `SKILL.md` and a
   companion file for the two mood rows and file layout, a skill file and a non-skill artifact for
   the tree-wide packaging-token and tracking-ID scans. Coverage is a property of the set: one
   negative proves only that the checker looked where that negative sits, because an uninspected
   file and a clean one report the same green.
   *Test:* the positives are green on every scan; each negative is red on exactly its own check
   and green on every other. **Step 3b's failure output names which of two things went wrong**,
   because they have opposite fixes: a **convention problem** (a positive fixture cannot be
   written in pure imperative, so the mood rule forbids something a mandated procedure requires —
   fix §10) or a **tooling problem** (the mood distinction needs a part-of-speech tagger to be
   worth anything, so the rule is right and the mechanism is missing — fix the checker, or fall
   back to the narrower assertion §10 licenses). A bare red here would send an implementer to
   rewrite the rule when the checker was at fault, or the reverse.
   **If a positive cannot be written, stop and fix the check set** —
...
> *independently stating* a fact — asserting what a generator is, what a capability does, what
> a config key means, or what a project's layout is. What is *not* banned is using the
> vocabulary to name a target, route a step, or reference the declared grounding source.**
>
> **Mechanically: a procedure body is written in the imperative, and a framework or project-shape
> term may not be the subject of a declarative clause.** That is the check — a property of the
> clause, not of its meaning.
...
**The distinction is grammatical mood, and this is a correction — an earlier draft of this
section glossed §6's rule as *explains it* versus *points at it*, and that gloss is what made the
check undecidable.** §6 decision 1 says a skill **"states no framework fact of its own."** *States*
is a property of a clause: an assertion, in the indicative, with the term as its subject.
*Explains* is a property of meaning, and no scan settles meaning. The two read as the same rule,
which is why the substitution survived several rounds, and every attempt to mechanise the gloss
failed for the same reason — it was the wrong rule.

So the check reads mood, not sense:

| text | shape | verdict |
|---|---|---|
| `"a generator streams model output"` | framework term as subject, finite verb, indicative | **states** — fails |
| `"generators emit model-generated responses"` | same shape, different words | **states** — fails |
| `"if the block you wrote is a generator, verify with fsdev run"` | imperative main clause; the term sits in a conditional about *this* artifact | routes — passes |
| `"read AGENTS.md to choose the kind"` | imperative | points — passes |

**This is what terminates the exemplar regress.** The second row is a paraphrase of the first, and
it fails by the *same rule* rather than by a second entry in a list — so paraphrases stop being an
infinite family a narrow scan can be tuned around and become instances of one pattern. A check
specified by example can always be specified to the examples; a check specified by structure
cannot.

**And the convention is not an imposition, which is the tell that it is the right rule.** A
procedure is a list of instructions, so it is imperative already — this names the form the
artifact has rather than adding one. The four authoring skills need no rewriting to satisfy it.

**It also restates decision 1 as a shape.** The canonical agent-instructions block is precisely
where framework terms *do* head indicative sentences — and it is the delimited payload this
section already exempts. Declarative claims about the framework are the single writer's monopoly;
everything else writes instructions. The payload exemption and the scan stop being two rules that
need reconciling and become one rule seen from two sides.

**Where this check stops, stated here rather than discovered later.** §6 `:98` frames the
rationale more broadly than `:89` frames the rule: *"do not **carry** framework knowledge inside a
skill."* Mood enforces `:89`'s **states**, not `:98`'s **carry**. A skill can hold framework
knowledge in imperative form — *"always add `uses: [cap]` to a generator when you need tools"* —
and pass. No mechanical check closes that gap, and asserting otherwise would be the over-claim
this section has been correcting all along. The operative decision text is what CI enforces; the
rationale's remainder is a review concern. **If an implementer cannot make even the mood
distinction mechanical, the check asserts the narrower thing rather than the wrong one** — a scan
that cannot be satisfied gets disabled, and then nothing is checked at all.

- **framework facts — all five skills.** No procedure body makes a block kind, an action or
  capability concept, or a config key **the subject of a declarative clause**. `AGENTS.md` owns
  those, and a skill that asserts one is a second writer. **Naming one to route or to name a
  target puts it in a conditional or an object position, not a subject, and passes** (the mood
  rule above): `create-block` must say `generator` to send it to `fsdev run`, `add-flow` must say
  `action`, and every skill's verification step names a flow and an action. What fails is a skill
  asserting *what a generator is* instead of pointing at `AGENTS.md`. The install skill passes
  this too: it wires a project, it does not author blocks, and if it ever needs to assert a block
  kind that is the signal that its scope has drifted
- **project-shape facts — all five skills, and the verb is load-bearing.** No procedure body
  ...
  **What that does not forbid, because a naive grep would fire on it:** naming the *supported
  ...
  inputs, not stating that this project uses pnpm. **The mood rule separates these two without
  needing to know which is true:** `"this project uses pnpm"` makes the project the subject of a
  declarative clause and fails; `"handles npm, pnpm and Yarn"` and `"look for pnpm-lock.yaml"`
  have the *skill* as their subject, or are imperative, and pass. Same structural test as the
  framework-fact row, which is why both rows now read from one rule rather than two narrowings
...
>
> **Then mutate each positive into one negative per check — a single violation injected — and
> assert the matrix: the positives are green on every check, and each negative is red on exactly
> the check it violates and green on all the rest. Where a check's declared scope spans more than
> one class of file, it carries a negative in *each* class — one red per class, because a checker
> narrowed to any single class passes a set that lives only in another.**
...
|---|---|---|
| framework facts **(both classes)** | `"a generator streams model output"` **and** `"generators emit model-generated responses"` — the same shape in different words; once in `SKILL.md`, once in a companion file | `"if the block you wrote is a generator, verify with fsdev run"` — the term in a conditional, not a subject |
| project-shape facts **(both classes)** | `"this project uses pnpm"` — the project as subject of a declarative clause; once in each class | `"handles npm, pnpm and Yarn"`, and `pnpm-lock.yaml` as a thing to look for — the skill as subject, or imperative |
| file layout **(both classes)** | `"put your flows in src/flows/"` — a path it invented; once in each class | `"read the FSD section of AGENTS.md at the project root"` — the declared grounding source |
| first instruction | a fixture whose opening step writes a file before reading or running its declared source | *(single class — the opening step is `SKILL.md` by definition)* |
| grounding-source resolution | an install-shaped fixture naming a detection script the tree does not contain (§9's row) | *(single class — a filesystem assertion scoped to `install-fsd`)* |
| `{{fsd:…}}` packaging token **(both classes)** | a literal `{{fsd:agent-instructions}}` in a `SKILL.md` **and** one in a non-skill artifact — `plugin.json`, a detection script, a tree README | a fixture carrying `{{run}}` and `{{mountPath}}` — FIX-1159's invocation templating, which **must** survive packaging |
| outsider rule (issue/PR number) **(both classes)** | an issue ID in a `SKILL.md` **and** one in a non-skill artifact | *(the positive covers it)* |
| outsider rule (path, `pnpm --filter`) | a fixture carrying `pnpm --filter` and a repository-relative path | *(scoped to the canonical content; it does not run tree-wide)* |

**Scope is the second axis, and coverage there is a property of the fixture *set*, not a location
for each fixture.** State it that way or the letter can be satisfied by moving something: **for
every check, enumerate the classes of file its declared scope reaches, and put a negative in each
one.** The matrix then asserts a red per class, and a checker narrowed in *any* direction goes red.

**The reason the set-level form is the only correct one: a green result and an uninspected file
are indistinguishable.** A scan that never opens a file reports exactly what a scan that opened it
and found nothing reports. So a single negative proves only that the checker looked *where that
negative sits* — it says nothing about the rest of the declared scope, and relocating it merely
swaps which narrow implementation slips through. Two of these checks reach the **whole published
plugin tree** (the packaging-token scan, §9's most-likely failure, and the tracking-ID half of the
outsider rule) and three read **a skill's procedure wherever that skill's own files carry it** (the
two mood rows and file layout). Each of those spans two classes, so each gets two negatives:

| class | why it is a class, not a filename |
|---|---|
| `SKILL.md` | the primary procedure file, and the obvious thing a narrow checker reads |
| companion files in the plugin tree | `skills/install-fsd/` already ships detection scripts beside its `SKILL.md` (§7), a script's comment header is prose an assistant reads, §13 floats a shared `_spine.md` stitched in at build time, and `plugin.json` and tree READMEs carry text no skill owns |

A second writer that sets up shop in a companion file is the same defect as one in `SKILL.md`, and
an unresolved token in `plugin.json` is the same defect as one in a skill — but the two are caught
by different file walks, so both need a red. *(This was got wrong twice in opposite directions:
first every negative sat in `SKILL.md`, then the fix relocated them outside it. Neither covers the
other's blind spot, and "put it where the check reaches" reads as a location when the requirement
is coverage.)*
...
- **§10 had substituted a different rule for the one §6 states, and six findings were spent
  mechanising the substitute.** §6 decision 1 says a packaged skill **"states no framework fact of
  its own."** An earlier draft of §10 glossed that as **"explains"** a fact. *States* is a property
  of a clause — an assertion, in the indicative, with the term as its subject — and is decidable.
  *Explains* is a property of meaning and is not. The swap was invisible because it reads as a
  faithful paraphrase, and every subsequent round tried to make the gloss mechanical rather than
  checking it against the original: one exemplar per fact class, then a demand for paraphrases,
  then the observation that any finite exemplar set is a set a narrow scan can be tuned to.
  **The regress terminated on comparing the restatement to the thing it restates, not on another
  iteration of the restatement.** §10 now reads mood: a procedure body is imperative, and a
  framework or project-shape term may not be the subject of a declarative clause. A paraphrase
  fails by the same rule as its original, so paraphrases are instances of one pattern rather than
  an open-ended family. The convention costs authors nothing — procedures are imperative already —
  and it restates decision 1 as a shape: indicative claims about the framework are the canonical
  block's monopoly, which is why that payload was already exempt.
  **The move, for the next author who hits this, because the instance matters less than the
  move:** this is the **seventh** occurrence in this epic of one shape — *a restatement drifting
  from the thing it restates, with the restatement then treated as authoritative* (the epic spec
  versus its child specs; FIX-548's recorded requirements versus its enforcing steps; §10 versus
  §6). It is the dominant defect mode here. When a rule resists being made mechanical over several
  rounds, **stop improving the rule and go read the thing it is a restatement of** — the defect is
  more often in the copy than in the problem.
  **Declared limit, recorded rather than left to be discovered:** §6 `:98`'s rationale ("do not
  *carry* framework knowledge inside a skill") is broader than `:89`'s rule ("states no framework
  fact"). Mood enforces the rule, not the rationale — a skill can hold framework knowledge in
  imperative form and pass. No mechanical check closes that; it is a review concern, and §10 says
  so where the rule is.
  *No decision, deliverable, scope or cost changes, and no check is added, removed or re-scoped:
  §6 is untouched and this moves §10 **back toward** its wording rather than away. The approval
  pinned to `af2771111` stands, and it stands for a better reason than the previous entry gave —
  the residual risk recorded there fired on §10's restatement of decision 1, not on decision 1.*

- **The fixture set had a scope hole, and the first fix moved it rather than closed it.** Every
  negative originally sat in a `SKILL.md`, so a checker walking only `skills/**` went green on the
  whole matrix while an unresolved `{{fsd:…}}` token in `plugin.json` or an issue ID in a detection
  script shipped — the same vacuity as a no-op scan, reached through **scope** instead of through
  pattern. The correction relocated those negatives outside `SKILL.md`, which simply swapped which
  narrow implementation slipped through: with the semantic negatives living only in companion
  files, a checker that reads only companions turns them red while never inspecting the positive
  `SKILL.md` fixtures at all. **The governing fact is that a green result and an uninspected file
  are indistinguishable** — a scan that never opens a file reports what a clean file reports — so a
  single negative can only ever prove the checker looked where that negative sits. §10 now states
  coverage as a property of the fixture **set**: enumerate the classes of file each check's scope
  reaches and put a negative in each, so a checker narrowed in any direction goes red. Five checks
  span two classes and now carry two negatives each.
  **Why the phrasing mattered more than the content here:** the first fix was written as *"a
  negative goes where the check is claimed to reach"*, which reads as a location for each fixture
  when the requirement is coverage across all of them. A rule phrased as a location can be
  satisfied by moving something; a rule phrased as a property of the set cannot. Same family as the
  entry above — a requirement restated into a weaker neighbour of itself, and the restatement then
  acted on — arriving this time within a single round of its own fix.
  *No decision, deliverable, scope or cost changes, and no check is added, removed or re-scoped:
  §6 is untouched and the approval pinned to `af2771111` stands.*
