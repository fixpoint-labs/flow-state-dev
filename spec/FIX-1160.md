...
exercise what you just wrote                              ← §4 practice 5
    ...
    a GENERATOR:               fsdev run only — `fsdev block` cannot
                               verify one (§10, verified in the CLI)
    read result.success FIRST, then schemaValidation.*.passed
                               ← never the exit code, and never the
                                 schema fields alone
...
- **A skill that wrote a block or a flow reads `result.success` first, then
  ...
  **This rule was an instance of the defect it was written to catch**, which is the sharpest
  ...
  check a *failed* block passes. *Asserted on the packaged skills' text — every skill that ends
  in a `fsdev` verification names `success` and the `schemaValidation` fields.*
