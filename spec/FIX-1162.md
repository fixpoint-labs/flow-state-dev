...
`npx fsdev` in a directory where **nothing is installed** keeps failing at the registry until the
CLI is renamed and published. Decision 3 accepts that. Once `@flow-state-dev/cli` *is* installed,
`npx fsdev` runs our CLI from its `bin` key regardless (§10, verified).
...
6. **Record the outcome on the Linear issue** — which names are secured *as unscoped packages*,
   ...
   deliverable the issue asked for. *Depends on:* 1, 2, 5, 5b.
...
| **B** | **Rename now, publish as soon as the pipeline can** | Not today — the release plan does not assemble at this commit | The rename **plus** the whole first release brought forward: pipeline repair, the 0.1.0 version baseline and its review, both org confirmations, a both-scope token, then 25 packages public |
