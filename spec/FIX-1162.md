...
changeset and the Linear record). *What it commissions —* **every option: no new issues.** All
three release prerequisites already exist and already block **FIX-1187**: the ownership seam is
**FIX-1186** (filed from this spec), the changeset repair is **FIX-870** (pre-existing; step 5c
reuses it rather than filing a duplicate), and the scaffolder itself is **FIX-548** (edge added
this round — see §8). **Option A:** plus the rename — 34 files / 49
...
2c. **Only if the owner answers §12's retention question with *unpublish* — run it, here, and verify
   ...
   - *Carries:* `npm unpublish fsdev@0.0.0` — or `npm unpublish fsdev --force` to remove the package
     outright, which is what "give the name back" means, since a single remaining version is still a
     held name.
...
now a live release blocker rather than a future one.)*
   ...
   **Under C that leaves an edit nobody in this issue makes, so name its executor rather than
   trusting the comment.** §6's non-goals keep this branch from editing the epic spec, which is
   right — but "raised on the epic PR" is a hand-off, and this spec's own rule is that a hand-off
   names an owner and a definition of done. So: **under C, step 4's comment must carry the exact
   replacement text** (`npx fsdev init` → `npx @flow-state-dev/cli init`, transcripts and running
   index), and **step 6 records it as an open hand-off owned by the epic PR (#1301)** rather than as
   done. Under A there is nothing to hand off — the transcripts are already correct. *(Found by
   sweeping both branches of every fork after step 2c; this is the same "chosen outcome with no
   executor" shape, one altitude down and milder, since a comment on a live PR does have a reader.)*
6. **Record the outcome on the Linear issue** — which names are secured *as unscoped packages*,
   ...
   step 2c's verified registry result; plus the epic-transcript edit as an open hand-off.** That
   record *is* the deliverable the issue asked for — **but recording is not executing**, and for
...
npm org ls thought-fabric --json     role matches the roster's role for them,
                                     AND at least one is `admin` or `owner`
