...
2b. **Close the personal-account seam on the two published names — a release prerequisite under
   ...
   *Carries:* `npm owner add` a second owner on each name, and either transfer both packages to
   ...
   one owner, and the release token is confirmed against both. **Filed as its own blocking issue
   (§8's hand-off rule); it outlives this issue because it is verified at the release, not here.**
