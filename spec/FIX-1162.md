...
LAYER 2 — the real proof. One throwaway probe per namespace, published BY CI with the
release credential, never from a human npm login:
  @flow-state-dev/release-probe@0.0.0     → proves the primary scope
  @thought-fabric/release-probe@0.0.0     → proves the second scope
  <the selected unscoped name>@0.0.1      → bump the existing placeholder; proves the
                                            personal-account seam (step 2b's subject)
     → all three succeed → the credential covers the release surface
     → any 403           → the release failure, surfaced now instead of half-way through
  Then UNPUBLISH both scoped probes inside 72 hours (fresh, no dependents, no downloads —
  they qualify). The unscoped bump stays; we keep that package anyway.
```

**What layer 2 costs, stated plainly because it is not free:** two throwaway scoped package names
burned permanently (`name@version` is never reusable, even after unpublishing — we would never want
them again), one version number on a placeholder, and a small CI job that carries `NPM_TOKEN`.
**Under option C the third probe is `create-flow-state` only** — nothing publishes `fsdev`, so
nothing needs to prove it.
