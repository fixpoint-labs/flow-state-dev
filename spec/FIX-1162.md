...
as it is. The cure for the small residual exposure is to ship real software under both names, which
is what option A does for `fsdev`. **Whether to keep the `fsdev` placeholder if you pick C is a
...
2b. **Close the personal-account seam on the two published names.** *Test:* §10's layers 1–2
   ...
   **`jnhoffner`**; the scopes are different objects with different ownership. **It has two halves,
   ...
   - **Second owner on both names — unconditional across the A/C fork, but conditional on the
     ...
     `fsdev` permanently forecloses unpublishing it, because npm's post-72-hour removal requires a
     single owner/maintainer. **So do not run this half on `fsdev` until §12's retention question is
     answered** — running it first makes the answer for the owner. `create-flow-state` is
     unconditional; we ship under it either way. *Carries:* `npm owner add`
   ...
   - **Release-token write access — `create-flow-state` always, `fsdev` only under option A.**
     ...
     **FIX-1186 closes on the provisioning. Nothing the release does reports back to it.** Its
     ...
     recorded, the account behind `NPM_TOKEN` is an owner of every unscoped name the chosen option
     publishes, and the token is deliberately configured for both scopes plus those names.
...
satisfied. **FIX-1186's DoD is step 2b**: the `npm owner add` on both names, the roster recorded,
and the token configured. That is work this issue performs, so *wired* is not enough — wiring
FIX-1186 was already true the moment it was filed, which would have let FIX-1162 close with the
account mutation it owns still outstanding and nothing scheduled to do it.
...
LAYER 2 — the necessary condition. A query, run after step 2b's mutation.
           SCOPED TO WHAT THE CHOSEN OPTION PUBLISHES:

  npm owner ls create-flow-state  → must INCLUDE the account layer 1 named
                                    EVERY OPTION — FIX-548 publishes it regardless
                                    of what the CLI ends up called.

  npm owner ls fsdev              → must INCLUDE that account — OPTION A ONLY.
...
- **Step 2b closes that second path by design**, and sooner than the clock does. Its first half
  adds a second owner to both names under every option — which permanently fails the third
  condition. So the real deadline is *the 72 hours or step 2b, whichever comes first*, and step 2b
  runs as soon as you approve this spec.
...
- **Placeholder retention moved from a coordinator premise to the owner's fork, with a measured
  deadline.** Covered in the attribution entry below; the operative change is that step 2b's
  `npm owner add` on `fsdev` is now sequenced *behind* that answer, because a second owner
  permanently forecloses npm's post-72-hour removal path — the mutation would otherwise have
  decided the question by side effect, before the owner saw it.
...
- **Placeholder retention was taken back off the coordinator and put on the owner's fork.** It had
  ...
  now sequenced behind that answer**, since a second owner permanently forecloses the post-72-hour
  removal path — running it first would have decided the question by side effect. §9's squatter row
  went from retired back to conditional.
