...
flowchart TD
  ...
  SCOPED -.->|"a release spans BOTH scopes;<br/>one token, or it stops half-way"| TF
...
5. **File the rename issue, if the owner picked option A or B — via `issue-manager`, before this
   ...
   was chosen. **Blocks:** nothing that ships — no epic command depends on the unscoped name
   (§12). *Depends on:* the §12 fork being answered. *(Option C files nothing — record the
   decision instead.)*
...
| The `thought-fabric` organization turns out not to be ours, or the token is scoped to one org | **Publish nothing until it is settled.** `changeset publish` walks the packages it is given, so a token good for one scope and not the other publishes part of the set and fails on the rest. A partially published release cannot be cleanly retried — `name@version` is single-use — so this is the worst outcome reachable from here, and it surfaces *mid-publish* rather than before one. `RELEASING.md:82,107` already requires both orgs and a token authorised for both; §10 block 1 now checks both. |
...
npm org ls thought-fabric    → lists our members  → @thought-fabric/core can publish
                             → error / not found  → do NOT publish anything; a part-way
                                                    release cannot be cleanly retried
```
...
**Why this is a gate and not a footnote:** `changeset publish` walks the packages it is given. If
the token is authorised for one scope and not the other, it can publish some packages and then
fail on the rest, leaving a **partially published release** — and since a `name@version` pair can
never be reused, that state cannot be cleanly retried. Both scopes must be confirmed *before*
anything publishes.
...
npm org ls thought-fabric     lists members  → the 25th package (@thought-fabric/core)
                                               can publish too.
                              errors / 404   → do NOT publish. A release that starts and
                                               fails part-way leaves names half-taken at
                                               versions that can never be reused.
```

**Run both before anything publishes, not just before this issue closes.** A token authorised
for one scope and not the other produces the single worst outcome available here.
...
- **Review round 4 (Codex) — five findings, all folded, and every one made option B dearer.**
  ...
  **`@thought-fabric`**. *Verified by reading all 35 workspace manifests:* 25 publishable, 24 + 1
  ...
  exactly as unverified. Gating on one scope risks a **partially published release**, which
  `name@version` single-use makes unrecoverable. Decision 1, §7's diagram, §8 step 2, §9, §10
