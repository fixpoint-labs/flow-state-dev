...
2b. **Close the personal-account seam on the two published names.** *Test:* §10's block 2b.
   ...
   **Both this step and 5c need a release issue to gate, and none exists.** *(Checked: FIX-549,
   the first-publish ceremony, is `Done`; no open issue owns the first release.)* Until one exists,
   these two prerequisites are recorded on their own issues and nowhere else — which is precisely
   the "declared but unowned" shape this spec keeps re-learning, one level up. **Whoever opens the
   first-release issue must wire FIX-1186 and 5c into it as prerequisites**; that is the first
   thing it should carry.
...
5c. **File the release-plan repair — unconditionally, under every option.** *Verified by reading
   ...
   *Wired as:* a prerequisite of the first release, under every option — **the release, not an
   implementation issue.** Same shape as 2b, and the same missing target: see 2b's note, there is
   no open first-release issue to attach it to yet. **Do not wire it onto an implementation issue
   as a substitute** — that is what deadlocked FIX-548.
...
are filed and wired to the first-release issue**, and — under A — the rename issue exists with an
owner and the same wiring. Filed-but-not-blocking would let the release publish straight past it.

**The wiring target is being created, and this spec does not commission it.** *(A previous draft
required all three to be wired to a first-release issue while confirming none existed and no step
created one — so the plan could either never complete or close with its prerequisites unenforced.
Third time this spec has produced that shape: a step that requires a target nobody creates.)* The
coordinator has dispatched `issue-manager` to open a first-release tracking issue, taking FIX-1186
and 5c's repair as blocked-by — **a coordinator decision, reversible**, made because two
prerequisites had no owner. **Until its identifier lands here, treat this clause as pending rather
than satisfied**, and do not substitute an implementation issue as the target: that is what
deadlocked FIX-548.
...
- **`release.yml` cannot publish these names at all.** It runs `changeset publish` over
  **workspaces**, and *(verified against `pnpm-workspace.yaml` and every manifest)* neither `fsdev`
  ...
  and then the same command queues **all 25 unpublished workspaces**, which is a first release, not
  a probe.
- **It proves the wrong namespace.** Two unscoped names say nothing about `@flow-state-dev` or
  `@thought-fabric`, which carry **25 of the 25** packages the release actually publishes. A green
  probe with a token that cannot reach either scope is precisely the false pass this gate exists to
  prevent.
...
**One inversion worth recording, because it runs against the intuition that a real package is the
safer probe.** The throwaway is the *more reversible* of the two: it is unpublished inside 72 hours
while it still has one owner and no dependents, so it qualifies. `@flow-state-dev/contracts@0.0.0`
**cannot** be withdrawn on the same terms later — step 2b deliberately adds a **second owner**, and
npm's policy requires "a single owner/maintainer" after 72 hours. So the real-package probe leaves a
permanent stray version by construction. That is a cosmetic artifact in a version history rather
than a dead name, and it is the right trade — but it is a permanent one, not a free one.

**Two hazards this creates, stated so nobody discovers them later.** A stray `0.0.0` sits in
...
inside 72 hours; a permanent stray `0.0.0` in `@flow-state-dev/contracts`' version history; one or
two version numbers on placeholders we are keeping anyway; and a small CI job carrying `NPM_TOKEN`.
