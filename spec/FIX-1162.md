...
2b. **Close the personal-account seam on the two published names.** *Test:* §10's layers 1–2
   ...
   **`jnhoffner`**; the scopes are different objects with different ownership. **It has two halves,
   ...
   - **Release-token write access — `create-flow-state` always, `fsdev` only under option A.**
     ...
     **This half has two conditions, and separating them is what keeps FIX-1186 from deadlocking
     the release the way it once deadlocked FIX-548.**
     - ***Blocking* condition — satisfied BEFORE the release runs, and this is what "FIX-1186
       blocks FIX-1187" means:** the approved-owner roster is recorded; the account behind
       `NPM_TOKEN` is an owner of every unscoped name the chosen option publishes (§10 layer 2 —
       failing this is decisive, passing it is not proof); and the token is deliberately configured
       for both scopes plus those names.
     - ***Closing* condition — satisfied BY the release itself:** the attended first release
       published every namespace, recorded on FIX-1186, which then closes.

     **Do not collapse these into "done when the release succeeds."** That would make FIX-1186
     block a release whose completion is its own definition of done — the exact cycle already
     removed once tonight, one level up.
...
LAYER 2 — the necessary condition. A query, run after step 2b's mutation:
  npm owner ls fsdev              → must INCLUDE the account layer 1 named
  npm owner ls create-flow-state  → same
     → account absent  → the release WILL fail on that package. Decisive.
     → account present → necessary, NOT sufficient. A read-only token on that
                         same account still cannot publish.
...
source; §9 carries the detail.* **So the worst case is a partial release that completes on the second
attempt with a human watching — which is exactly what "attended" means.**

**Attended is a requirement, not an adjective.** Someone with the ability to fix a credential must be
watching the run and ready to re-run it. A release scheduled and walked away from is the one case
...
usefully — an invalid token, or a release account that is not an owner — and each failure is
decisive. **Neither success proves anything about publishing**, and neither may be recorded as
though it did.
