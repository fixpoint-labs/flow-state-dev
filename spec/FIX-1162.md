...
**Handed on** (recorded in §12, not built here): an unscoped name is claimed only by a package
whose own `package.json` carries it. `create-flow-state` is handled — FIX-548's workspace is
**named `create-flow-state` and published only as that unscoped package, with no scoped twin**
(decision 2), so shipping it claims the name as a side effect. `fsdev` is not handled by
anything, and needs the CLI rename that §12's fork decides. Whoever publishes either adds a
second owner at that point.
...
|---|---|---|---|
| **A** | **Rename now, publish at the normal first release** *(recommended)* | At the real release, not today | The rename work below. Exposure continues until that release |
| **B** | **Rename now, publish now** | Today | The rename work **plus** publishing 25 packages weeks early |
...
now. But publishing 25 packages weeks early (option B) buys a small risk reduction for a large,
permanent widening of what we have shipped, and option C throws away a name we can still have
for a day's work. A accepts a real exposure window and is honest about it.
...
- **The CLI rename surface, enumerated once so it is inherited rather than rediscovered
  ...
  A repo-wide exact-boundary search for `@flow-state-dev/cli` on 2026-08-18 finds **34 files,
  49 references** (excluding this spec), in seven groups:

  | Group | Files | Notes |
  |---|---|---|
  | changeset fragments | 14 | a fragment naming a package that no longer exists fails `changeset version` |
  | docs-site pages | 5 | `api/cli.md`, `cli/overview.md`, `devtool/setup.md`, `getting-started/installation.md`, `guides/nextjs-setup.md` — the last two carry `pnpm add -D` install lines in the primary onboarding path |
  | `.agents/skills/` | 4 | `debug-flow`, `diagnose`, and two in `prototype` |
  | **source files** | **3** | `packages/cli/src/index.ts`, `packages/cli/test/index.test.ts`, and **`packages/core/src/helpers/slash-command.ts` — a different package** |
  | READMEs | 2 | `packages/cli/README.md`, root `README.md` |
  | build/manifest | 2 | `packages/cli/package.json`, `tsconfig.base.json` |
  | orientation docs | 4 | `CLAUDE.md`, `CONTRIBUTING.md`, `docs/architecture/overview.md`, `packages/cli/CHANGELOG.md` |

  **No workspace declares a dependency on `@flow-state-dev/cli`** (39 manifests scanned), so no
  dependency edges change. *Corrected this round: the previous list named only the first three
  groups plus READMEs and the path map, and reported that as verified — it had checked those
  categories rather than the repo. §10 now carries an acceptance step so the next person does
  not inherit the same hand-maintained list.*
