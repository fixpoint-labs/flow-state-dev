...
**Step 2b is satisfied by FIX-1186's own definition of done, which is now entirely pre-release —
so this issue never waits on a publish.** Worth stating because step 6 depends on 2b, and while
2b carried a closing condition satisfied by the release, FIX-1162 could not close until the
release either. That was the same cycle one level down, inherited rather than authored.
