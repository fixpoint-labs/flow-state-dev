...
*What it commissions* (§8 steps 5 and 5b — counted here because the size that matters to the fork
is the work the decision creates, not the work this branch performs):

| Picked | Commissioned |
|---|---|
| **C** | **1 issue, 0 files** — step 5b's post-publish ownership issue, which every option files |
| **A** | that, plus the rename issue: **34 files / 49 references, 1 PR** — 3 source files, 14 changeset fragments, 5 `apps/docs` pages, 2 READMEs, 4 `.agents/skills/` files, 3 orientation docs, `package.json`, `tsconfig.base.json`, and the CHANGELOG heading |
| **B** | all of A, plus the first release itself: 6 mixed fragments repaired, the 0.1.0 baseline set and reviewed across **25 packages**, and 25 packages published |
...
5. **File the issue that delivers the *whole* selected outcome — via `issue-manager`, before this
   ...
   **Under A or B the issue also carries the snapshot guard** (§9's new row). Renaming the package
   gives `snapshot-release.yml` a side effect it does not have today: a dispatched snapshot would
   publish `fsdev` — and all 25 packages — before the release A promises to wait for. The guard is
   operational, not code: no snapshot dispatch between the rename landing and the first release.
...
6. **Record the outcome on the Linear issue** — which names are secured *as unscoped packages*,
   ...
   deliverable the issue asked for. *Depends on:* 1, 2, **3, 4,** 5b — **and on step 5 only under
   A or B.** Under C step 5 files nothing by design, and recording "option C chosen, no rename
   issue filed" *is* how step 5 is satisfied.
...
(step 4)**, the outcome is recorded, step 5b exists, and — under A or B — the option issue exists
with an owner and a blocking link to the release it applies to. Filed-but-not-blocking would let
the release publish straight past it. *(Steps 3 and 4 were missing from this sentence and from
step 6's dependencies, so the issue could have closed without either — the same "declared but
unowned" shape this section is about, one level down.)*
...
| **A snapshot is dispatched after the rename lands but before the first release** *(A or B only)* | It claims `fsdev` — and makes **all 25 packages public**. `snapshot-release.yml:33-40` runs an **unfiltered** `changeset version --snapshot` then `changeset publish --tag`, and after the rename the **14 fragments that declare the CLI** *(verified: all 14 declare it in frontmatter, none merely mention it)* put `fsdev` in every release plan — so the dispatch registers the name at a canary version. **This is latent, not scheduled:** the workflow is `workflow_dispatch` only, so nothing fires it on its own, and **two defects block it today — of which option A repairs only one.** A clears the six mixed fragments at the normal release; it does **not** commission the workflow's missing `registry-url`/`.npmrc` (a §12 follow-up), without which the publish cannot authenticate at all. **Guard, owned by step 5's issue:** do not dispatch a snapshot between the rename landing and the first release. If one is genuinely needed in that window, add the CLI to `.changeset/config.json`'s `ignore` (today `[]`) for the duration and remove it at the release. |
