...
LAYER 2 — the necessary conditions. Queries, run after step 2b's mutation.
  ...
  npm owner ls fsdev              → must INCLUDE ACCT — OPTION A ONLY.
     ...
     → ACCT absent from `fsdev` UNDER C → EXPECTED AND CORRECT. Not a finding,
                        not a blocking condition, and **not** something to fix
                        by granting the CI token owner rights on a package
                        nothing publishes. Record it as "n/a under C".
