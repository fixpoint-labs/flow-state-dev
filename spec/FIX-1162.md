...
npm names go to whoever registers them first. This epic wrote several into a launch plan believing
they were ours; for most of this spec's life they were not. **On 2026-08-18 the owner published
placeholders, and the two that onboarding depends on are now genuinely held.** *Verified from the
registry, not reported:*

| Name | Status | Maintainer | Published |
|---|---|---|---|
| `fsdev` | **200 — ours**, v0.0.0 | `jnhoffner` | 2026-08-18T21:56:04Z |
| `create-flow-state` | **200 — ours**, v0.0.0 | `jnhoffner` | 2026-08-18T21:55:09Z |
| `flow-state-app` | 404 — unclaimed | — | treated as deliberate; nothing plans around it |

**That closes the race this issue was opened to run, and three things follow.**

1. **The `fsdev` decision stopped being urgent.** It was "claim it before a stranger does." It is
   now "what do we want the headline command's package to be called," decidable at leisure — §12
   re-derives it from scratch on these facts rather than carrying the old lean forward.
2. **The names are on a *personal account*, not an organization.** `jnhoffner` owns both packages;
   the scopes are separate objects owned by whoever owns them. **If CI publishes with an
   org-scoped token, the first real release fails on exactly the two packages onboarding
   depends on** — §8 step 2b makes that a release prerequisite under every option.
3. **A placeholder is not a product.** Both are v0.0.0 with **no `bin`**, so `npx fsdev` and `npm
   create flow-state` now resolve, download our package, and fail with *no executable* instead of
   a clean 404 (§9). The claim also rests on a publish npm's
   [dispute policy](https://docs.npmjs.com/policies/disputes) discourages — it hardens the day
   real software ships under these names, and not before.

**Still open:** **whose scopes are these?** Five are registered, and the release publishes into
**two** of them — 24 of the 25 publishable workspaces are `@flow-state-dev/*`, and
`@thought-fabric/core` ships under `@thought-fabric`. Nothing is published under either scope
(*verified: `@flow-state-dev/cli`, `@flow-state-dev/core` and `@thought-fabric/core` all 404*),
npm hides member lists, and §12 carries the two commands that settle it.

**Why now** — no longer a squatter clock. Two real ones remain: the **72-hour unpublish window
closes ~2026-08-21T21:55Z**, after which the placeholders are permanent; and the rename in §12 is
cheap only until the first real publish, after which it becomes a breaking change for users.
...
**Then close the ownership seam the placeholders opened.** Both names sit on one personal account.
Getting a second owner onto each — and making sure whatever token CI publishes with can write to
them — is what turns a held name into a name the project can actually release under.

**On the stand-ins themselves.** This spec argued against them: npm's
[dispute policy](https://docs.npmjs.com/policies/disputes) says the Terms forbid publishing
"simply for the purposes of reserving [a name] for future use." That argument is now moot for
`fsdev` and `create-flow-state` — they are published, and this spec records the world as it is
rather than as it recommended. **The residual exposure is small but real:** a v0.0.0 package that
does nothing is what a dispute looks for, and the cure is the same either way — ship real software
under both names. It is an argument for finishing, not for unpublishing. *(Unpublishing is
available until ~2026-08-21T21:55Z and is only relevant if §12 picks option C for `fsdev`.)*

That leaves the rename, which §12 prices as a live fork. **Note it is the *name* at stake, not the
command:** our `fsdev` command comes from the CLI package's `bin` key and works under every option
...
3. **An unscoped name is claimed only by a package actually *named* that — which is why the
   placeholders worked, and why the live question is now what ships under `fsdev`, not who
   holds it.** Publishing `@flow-state-dev/cli` would **not** have claimed `fsdev`; publishing a
   package literally named `fsdev` did. *(This corrects an earlier draft that expected the CLI's
   first publish to claim it.)* So the name is held, and §12's fork is whether the CLI is renamed
   so the **real** software ships under it, or whether the placeholder is retired and we keep the
   scoped name.
   *Overtaken by events:* this spec rejected stand-in publishes on npm's dispute policy. The owner
   published them anyway on 2026-08-18, and the rule is now recorded rather than argued — see §2.
   *Still rejected:* running the unmodified snapshot workflow to claim anything, which publishes
   the scoped names and now also risks publishing 25 packages early (§9).
...
**Size — two numbers, because this issue's own diff is not what the fork costs you.**

*What this issue changes, under every option:* **0 production files · 0 CI test behaviours · 0 doc
surfaces · 1 PR** (an empty changeset and the Linear record).

*What it commissions* (§8 steps 2b, 5 and 5c — counted here because the size that matters to the
fork is the work the decision creates, not the work this branch performs):

| Picked | Commissioned |
|---|---|
| **every option** | **2 issues, 0 files** — the npm ownership/token seam (step 2b) and the release-plan repair (step 5c). Both are release blockers regardless of the fork |
| **C** | nothing beyond those two, plus one call: retire the `fsdev` placeholder before ~2026-08-21T21:55Z, or leave it dormant |
| **A** | those two, plus the rename issue: **34 files / 49 references, 1 PR** — 3 source files, 14 changeset fragments, 5 `apps/docs` pages, 2 READMEs, 4 `.agents/skills/` files, 3 orientation docs, `package.json`, `tsconfig.base.json`, and the CHANGELOG heading |
| ~~**B**~~ | **retired.** It bought a shorter exposure window, and the placeholders closed that window at no cost |
...
2b. **Close the personal-account seam on the two published names — a release prerequisite under
   *every* option, not an A/B one.** *Test:* §10's block 2b. *Depends on:* spec approval.
   `fsdev` and `create-flow-state` are owned by the personal account **`jnhoffner`**; the scopes
   are different objects with different ownership. **`release.yml` publishes through
   `changesets/action@v1` running `pnpm release:ci` → `changeset publish --provenance`, with a
   single `NPM_TOKEN` secret** *(verified by reading the workflow and `package.json`)* — so that
   one token must be able to write to **both scopes and both personal-account packages.** A token
   authorised only for the orgs publishes the scoped packages and fails on exactly the two that
   onboarding depends on, and per §9 the run does not abort — it exits 1 with a partial release.
   *Carries:* `npm owner add` a second owner on each name, and either transfer both packages to
   the publishing organization or confirm the release token covers them. *Definition of done:*
   `npm view fsdev maintainers` and `npm view create-flow-state maintainers` each show more than
   one owner, and the release token is confirmed against both. **Filed as
   [FIX-1186](https://linear.app/fixpoint-labs/issue/FIX-1186), which *blocks FIX-548* — the issue
   that actually publishes `create-flow-state`. It outlives this issue because it is verified at
   the release, not here.** *(Filed ahead of approval, unlike steps 5 and 5c, because it is a
   live release blocker discovered after the placeholders went up rather than a consequence of
   the fork.)*
...
5. **File the issue that delivers the *whole* selected outcome — via `issue-manager`, before this
   ...
   **Scope it to the option the owner picked, not just to the rename.** An issue that carries only
   ...
   | **C** | the placeholder decision only: retire `fsdev` (unpublish before ~2026-08-21T21:55Z) or leave it dormant and documented. No rename | — |
...
| `fsdev` is taken by someone else | **Largely retired — we hold it as of 2026-08-18.** It survives in one place only: **if option C unpublishes the placeholder**, the name returns to the pool and this becomes live again, unrecoverably — npm "does not resolve squatting claims on demand" and transfers names only on a trademark or IP claim. So under C, *leaving the placeholder dormant and retiring it are materially different choices*, and §12 says so. **Our `fsdev` command is unaffected either way** — it comes from the CLI's `bin` key (§12, verified). The `@fsdev` scope belongs to someone else, so `@fsdev/cli` was never a fallback. |
...
rename (step 1b).* **Assert the maintainer, never the status code.**

```
npm view fsdev maintainers              → ["jnhoffner"]        → ours; proceed
npm view create-flow-state maintainers  → ["jnhoffner"]        → ours; proceed
                                        → any other account    → STOP. Someone else holds it.
                                                                 Do not commission the rename;
                                                                 return to §12.
                                        → 404                  → the placeholder is gone
                                                                 (unpublished?). Stop and ask.
```

**Why the maintainer and not a 200 — this is the check-that-cannot-fail trap, and this spec has
now hit it three times.** A 200 means *some* package exists at that name; it is exactly what a
stranger's package returns. Asserting on 200 would pass most loudly in the one scenario the check
exists to catch. **Recorded result, 2026-08-18:** both return `jnhoffner`, `latest` = `0.0.0`,
published 21:55–21:56Z. *(Control: `flow-state-app` returns 404 and `create-fsd-app` returns
`keyready` — so the command distinguishes ours, unclaimed, and someone else's.)*
...
*Block 2b — after approval, on the two published names (step 2b).* Assert **redundancy and
publishability**, which are different things:
...
then, whichever token release.yml's NPM_TOKEN holds:
npm access list packages <that-account-or-org>
                                → must include BOTH unscoped names, not just the scopes
```

**The second command is the one people skip, and it is the one that fails the release.** A second
*owner* makes the name survive a person; it does **not** make CI able to publish it. `release.yml`
carries exactly one `NPM_TOKEN` into `changeset publish` (*verified*), so that token needs write
access to two scopes **and** two personal-account packages, or the first release goes out partial.
...
would otherwise be the last thing between us and a stranger's postinstall. **If option C
unpublishes `fsdev`, the name leaves our control and the rule bites again on that name.**
...
| **C** | **Keep `@flow-state-dev/cli`** | `pnpm add -D @flow-state-dev/cli` · `npx fsdev` cold never works | Nothing today | **A decision about the placeholder:** unpublish before ~2026-08-21T21:55Z, or leave a dead v0.0.0 package answering to our brand forever |
...
**The one thing C must not do is leave the placeholder unattended.** Under C we would hold a
v0.0.0 package, with no code, answering to our brand, indefinitely — which is both the shape npm's
dispute policy dislikes and a confusing 404-replacement for anyone who tries `npx fsdev`. **So C
carries a second decision: unpublish before ~2026-08-21T21:55Z, or keep it deliberately and
document why.** I would unpublish — but note that unpublishing genuinely returns the name to the
pool, and then the old squatter risk comes back for real (§9).
...
because nothing outside the repo depends on it. Under C we keep the longer name and either retire
`fsdev` (losing it for good, this time genuinely) or carry a dead package under our brand. **The
expensive error is neither of those; it is forgetting the placeholder decision under C**, which is
how a dormant v0.0.0 package quietly becomes permanent on 2026-08-21.
