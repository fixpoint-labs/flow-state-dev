...
- **Option A** — **one new issue**, filed through `issue-manager` by step 5 before this issue
  closes and wired as a blocker on the first release: the rename, 34 files / 49 references, 1 PR,
  plus the user-facing docs in the same change set.
...
**Second ownership and the release token are step 2b's, on every name the selected option retains** —
`create-flow-state` always — and that is a release blocker rather than a nicety.
...
- **Both names are held today; neither carries software.** `fsdev` and `create-flow-state` are
  ...
  `create-flow-state` becomes real when FIX-548 ships. **The second owner and the release token
  belong to §8 step 2b, on whichever names survive those two answers** — no longer deferred to
  "whoever publishes", which is CI, and which is how that step went ownerless twice.
