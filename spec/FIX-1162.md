...
2b. **Close the personal-account seam on the two published names.** *Test:* §10's block 2b.
   ...
   **`jnhoffner`**; the scopes are different objects with different ownership. **It has two halves,
   ...
   - **Second owner on both names — unconditional, under every option.** A held name with one
     ...
     that is ever revisited and `fsdev` retired, this half goes with it. *Carries:* `npm owner add`
     ...
     *Done when:* `npm owner ls` on each name returns **exactly the recorded roster**, not merely
     more than one account. **Cardinality is not enough:** `npm owner add` on a mistyped or
...
release credential, never from a human npm login.
  ...
  @flow-state-dev  →  publish a throwaway @flow-state-dev/release-probe@0.0.0,
                      then UNPUBLISH it inside 72 hours.
  ...
  create-flow-state@0.0.1   →  ALWAYS. FIX-548 publishes the scaffolder under
                               every option. A version bump on a package that
                               is ALREADY public — it makes nothing newly public.
  fsdev@0.0.1               →  UNDER OPTION A ONLY, and it is a fourth probe,
                               not a substitute for the third.

  NEVER a real framework package. See below — this was tried and reverted.
...
**Why the probe is a throwaway name and NEVER a real framework package — this was proposed, adopted
for one round, and reverted.** The appealing argument was: the thing being proved is *"the release
credential can write to this scope"*, which needs no new name, so publish `0.0.0` of a package we
ship anyway (`@flow-state-dev/contracts` — the only publishable workspace with no workspace
dependencies, so it would install cleanly). `0.0.0` is a version the real release never consumes, so
it looked free.

**It is not free, and the version number was never the cost. Publishing it makes the package itself
public and sets `latest`.** A real framework package would go live, outside the release ceremony,
before anyone reviewed what it ships — and anyone running `pnpm add @flow-state-dev/contracts` in
the window between the probe and the real release would install an unreviewed `0.0.0`. It also
bypasses everything `RELEASING.md` requires *(verified at `:62-94`)*: the pre-publish sanity checks
that it says to "run before **any** publish (automated or manual)" — `npm pack --dry-run`,
`publint`, `@arethetypeswrong/cli`, the stray-`console.log` scan — plus the first-publish ceremony's
0.1.0 baseline, the `version-packages` diff review, and the post-publish README/provenance check.
**A credential probe must not be a production release.** A throwaway name under the same scope
proves exactly the same thing about the credential and makes no framework package public.

**Both scope probes are therefore disposable and both get unpublished** inside 72 hours — fresh, no
dependents, no downloads, so they qualify. **The two unscoped probes are different in kind and need
no cleanup:** they are version bumps on packages that are *already* public and that we are keeping
regardless, so they make nothing newly public.

**What layer 2 costs, stated plainly because it is not free:** two throwaway scoped names burned
permanently (`name@version` is never reusable even after unpublishing — we would never want them
again), an unpublish step on each that has to actually happen, one or two version numbers on
placeholders, and a small CI job carrying `NPM_TOKEN`. **No framework package becomes public**, which
is the property that matters and the one the cheaper-looking alternative gave away.
...
- **A cheaper probe was proposed, defended, and reverted — and the real cost was somewhere neither
  party had looked.** The **coordinator** proposed replacing the throwaway scope probe with `0.0.0`
  of a package we ship anyway (`@flow-state-dev/contracts`), reasoning that `0.0.0` is a version the
  real release never consumes, so it is free. I adopted it for one round, having first argued
  *against* it on a claim that turned out to be wrong in the coordinator's favour — that
  `contracts@0.0.0` could never be withdrawn because step 2b adds a second owner. **Step 2b touches
  only `fsdev` and `create-flow-state`** (`npm owner` is per package-spec; scoped packages draw
  publish rights from org membership), so that objection was void and the substitution looked
  cheaper still.
  **Then the actual cost surfaced: the version number was never the point — publishing it makes the
  package itself public and sets `latest`.** A real framework package would go live outside the
  release ceremony, bypassing everything `RELEASING.md:62-94` requires "before **any** publish",
  and anyone installing it in the window would get an unreviewed `0.0.0`. **Reverted to throwaway
  probes in both scopes.** Recorded in full because the wrong answer was appealing twice over and
  will be re-proposed otherwise: *a credential probe must not be a production release.* Noted too
  that this error leaned toward the proposer's own position — the mirror of two I made in the same
  direction tonight.
