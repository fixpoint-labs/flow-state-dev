...
changeset and the Linear record). *What it commissions —* **every option: no new issues.** Both
release prerequisites already exist and already block **FIX-1187**: the ownership seam is
**FIX-1186** (filed from this spec) and the changeset repair is **FIX-870** (pre-existing; step 5c
reuses it rather than filing a duplicate). **Option A:** plus the rename — 34 files / 49
references, 1 PR. **Option C:** nothing further. **Option B is retired** (§12).
...
2b. **Close the personal-account seam on the two published names.** *Test:* §10's block 2b.
   ...
   **`jnhoffner`**; the scopes are different objects with different ownership. **It has two halves,
   ...
   - **Release-token write access — `create-flow-state` always, `fsdev` only under option A.**
     ...
     `create-flow-state` still ships a partial first release. *Done when:* §10 block 2b's proof has
     run for **every** namespace the chosen option publishes — **four probes under A, three under
     C** — which is **an action, not a query.**

   **Filed as [FIX-1186](https://linear.app/fixpoint-labs/issue/FIX-1186). It gates the *first
   publish*, not any implementation** — and an earlier draft got that wrong in a way that
   deadlocked the epic. It was wired to **block FIX-548**, but FIX-548 is the work that *builds*
   the real `create-flow-state`, and FIX-1186 stays open through the release that FIX-548's package
   is part of. So FIX-548 could not start until FIX-1186 closed, and FIX-1186 could not close until
   the release that needed FIX-548. **The blocking edge is removed** (Linear relation deleted; a
   non-blocking *related* edge kept so the pair stays discoverable). FIX-548 must be free to be
   built and merged; what it must not do is **publish** before the credential is proven.
   ...
   **The release it gates is [FIX-1187](https://linear.app/fixpoint-labs/issue/FIX-1187)** —
   ...
   tracking". *Verified:* open, and **blocked by both FIX-1186 and FIX-870**, which is the correct
   state. It was opened as a **coordinator decision, reversible**, because these two prerequisites
...
6. **Record the outcome on the Linear issue** — which names are secured *as unscoped packages*,
   ...
   step 5's rename issue. That record *is* the deliverable the issue asked for.
   *Depends on:* 1, 1b, 2, **2b, 3, 4,** 5c — **and on step 5
...
(step 4)**, the outcome is recorded, **FIX-1186 (2b) and FIX-870 (5c) are wired to FIX-1187** —
both already are — and, under A, the rename issue exists with an owner and the same wiring. Filed-but-not-blocking would let the release publish straight past it.

**The wiring target now exists: [FIX-1187](https://linear.app/fixpoint-labs/issue/FIX-1187).**
*Verified:* open, and blocked by **FIX-1186** (credential proof) and **FIX-870** (changeset
repair) — so it is genuinely blocked, which is the correct state. Opened as a **coordinator
decision, reversible**, because two prerequisites had no owner. **Do not substitute an
implementation issue as the target** — that is what deadlocked FIX-548. *(A previous draft required
wiring to a first-release issue while confirming none existed and no step created one, so the plan
could either never complete or close with its prerequisites unenforced. Third altitude at which
this spec produced that shape.)*
...
| **CI publishes with a token that cannot write to `fsdev` / `create-flow-state`** | **The new most-likely release failure, and it lands on exactly the two packages onboarding depends on.** Both are owned by the personal account `jnhoffner`; the scopes are separate objects. `release.yml` publishes through `changesets/action@v1` with **one** `NPM_TOKEN` *(verified)*, so an org-only token publishes the scoped packages (24 at the release) and 403s on the unscoped ones. Per the row above the run does **not** abort — it exits 1 with a partial release, and the half that fails is the half a new user touches first. **Prevented by step 2b, which is a release prerequisite under every option.** |
...
is publishing with it. But the obvious action does not work either**, and the previous draft got
that wrong too. It said *"publish a throwaway patch of each placeholder through `release.yml`'s own
credential."* Two defects, both verified:

- **`release.yml` cannot publish these names at all.** It runs `changeset publish` over
  **workspaces**, and *(verified against `pnpm-workspace.yaml` and every manifest)* neither `fsdev`
  nor `create-flow-state` is a workspace. Under option A the CLI *becomes* the `fsdev` workspace —
  and then the same command queues **every unpublished workspace** — 26 at the release — which is a first release, not
  a probe.
- **It proves the wrong namespace.** Two unscoped names say nothing about `@flow-state-dev` or
  `@thought-fabric`, which carry **24 of the release's 26** packages. And the converse is just as
  sharp: the scopes do **not** cover everything either — the remaining two (option A) or one
  (option C) are **unscoped**, reachable by no org lookup and by no scope-level grant. **That is
  precisely why the probe count is four under A and three under C**, one per namespace, rather than
  one probe standing in for the rest.

**The gate has to touch every namespace the release touches, with the release credential:**

```
LAYER 1 — identity. Cheap, no side effects, run in CI with NPM_TOKEN:
  npm whoami            → the account the token authenticates as
     → errors           → the token is invalid or expired. Stop here; nothing below matters.
  This proves identity ONLY. It does not prove write access (see above).

LAYER 2 — the real proof. One publish per namespace the release touches, BY CI with the
release credential, never from a human npm login.

  @flow-state-dev  →  publish the REAL package @flow-state-dev/contracts at 0.0.0
                      (no throwaway name; nothing to clean up — see below)
  @thought-fabric  →  publish a throwaway @thought-fabric/release-probe@0.0.0,
                      then UNPUBLISH it inside 72 hours. Its only real package
                      cannot stand in (below).
  create-flow-state@0.0.1   →  ALWAYS. FIX-548 publishes the scaffolder under
                               every option.
  fsdev@0.0.1               →  UNDER OPTION A ONLY, and it is a fourth probe,
                               not a substitute for the third.

     → every probe for the chosen option succeeds → the credential covers the
       release surface
     → any 403 → the release failure, surfaced now instead of half-way through

  Probe count: FOUR under option A, THREE under option C.
```

**Why four under A, and why this must not be "simplified" back to one unscoped probe.** Under A the
release publishes **both** unscoped names, and **npm's granular tokens are per-package** — a token
permitted for `fsdev` and not for `create-flow-state` passes a one-name probe and still leaves the
first release partial on the scaffolder, which is the package a brand-new user touches first.
Proving one unscoped name says nothing whatsoever about the other. Under C nothing publishes
`fsdev`, so its probe is not skipped for convenience — there is nothing of ours to prove there.

**The `@flow-state-dev` probe costs nothing, and that is worth explaining rather than asserting.**
The thing being proved is *"the release credential can write to this scope"*, which does not need a
new name — so publish `0.0.0` of a package we are shipping anyway. **Verified against the
manifests:** `@flow-state-dev/contracts` is the **only** publishable workspace with **no workspace
dependencies** (`"dependencies": {}`), so it publishes as a genuinely installable package rather
than one pinning siblings that do not exist yet. `0.0.0` is a version the real release never
consumes — `RELEASING.md` sets the `0.1.0` baseline first — and *(per §9's reading of the pinned
`@changesets/cli@2.30.0` source)* `getUnpublishedPackages` queues a workspace when
`!publishedVersions.includes(localVersion)`, so a later `0.1.0` still publishes normally. No
throwaway name, no 72-hour clock, no cleanup step to forget.

**Why the same trick does NOT work for `@thought-fabric`, which is the half that kills a clean
sweep.** *Verified:* `@thought-fabric/core` is the scope's **only** package, and it depends on
`@flow-state-dev/core` and `@flow-state-dev/testing` via `workspace:*`. Publishing it at `0.0.0`
rewrites those to exact versions that **are not on npm**, producing a permanently **uninstallable**
package under our real name — the precise failure §9 already documents. There is no zero-dependency
alternative in that scope to substitute. So that half keeps a throwaway, and keeps its unpublish.

**A correction to an earlier draft of this section, which argued the real-package probe was the
*less* reversible one. That was wrong, and wrong in the direction of defending the throwaway I had
proposed first.** The claim was that `@flow-state-dev/contracts@0.0.0` could never be withdrawn
because step 2b adds a second owner and npm requires "a single owner/maintainer" after 72 hours.
**Step 2b does not touch `contracts`** — it runs `npm owner add` on `fsdev` and `create-flow-state`
only, and `npm owner` operates per package-spec. Nor *should* it: `contracts` is **scoped**, so
publish rights come from organization membership, which is the very scope-versus-package
distinction this spec rests on. So the stated cost was never established.

**What is actually true:** inside 72 hours both probes are removable on the same terms — fresh, no
dependents, no downloads. After that both get hard, for the ordinary reasons rather than one we
created. **The `@flow-state-dev` probe is therefore at least as reversible as the throwaway it
replaces, and cheaper: no dead name, and no cleanup step to forget.** The intended posture is to
*leave* `contracts@0.0.0` in place — a stray version in a real package's history, cosmetic, with
`latest` resolving to `0.1.0` — rather than to schedule a removal. *(Whether a single version can
still be withdrawn after 72 hours is npm-policy detail this spec has not verified, and it is not
load-bearing: nothing here plans to remove it.)*

**Two consequences, stated so nobody discovers them later.** A stray `0.0.0` sits in
`contracts`' published history forever; `latest` will be `0.1.0`, so no user resolves to it. And
because `contracts`' *local* version is `0.0.0` today, a **direct `changeset publish` run before the
baseline is set** would now **skip** `contracts` as already-published, where today it would publish
it. The normal path (`release.yml` runs `changeset version` first) is unaffected; the snapshot path
suffixes versions and is unaffected; but anyone hand-running a bare publish pre-baseline should know.



**What layer 2 costs, stated plainly because it is not free — but it is now about half what an
earlier draft priced.** **One** throwaway scoped name (`@thought-fabric/release-probe`), unpublished
inside 72 hours; a stray `0.0.0` left deliberately in `@flow-state-dev/contracts`' version history
(cosmetic — `latest` resolves to `0.1.0`); one or two version numbers on placeholders we are keeping
anyway; and a small CI job carrying `NPM_TOKEN`.
*(An earlier draft burned **two** throwaway names. Replacing the `@flow-state-dev` one with a real
package we ship anyway removed it at no cost — the same substitution fails for `@thought-fabric`,
for the dependency reason above.)*

**One residual that layer 2 does not close, and it must not be papered over.** `release.yml`
delegates auth to `changesets/action@v1`. A probe job wiring its own `registry-url`/`NODE_AUTH_TOKEN`
proves *the token*, not *the action's wiring* — the same class of gap as `snapshot-release.yml`,
which carries `NPM_TOKEN` and still cannot authenticate because nothing writes an `.npmrc`. Have the
probe job reproduce `release.yml`'s setup rather than invent its own, and treat a green probe as
evidence about the credential, not a guarantee about the workflow.

**If layer 2 is judged not worth its cost, say so and take the fallback deliberately** rather than
pretending a cheaper check proved something: **the first release becomes the test.** That is
survivable — §9 verifies that `changeset publish` does not abort on a failed package and that a
re-run publishes only what is missing — but it must be run *attended*, by someone ready to fix a
credential and re-run, not scheduled and walked away from. **What is not acceptable is a green
...
| **C** | **Keep `@flow-state-dev/cli`** | `pnpm add -D @flow-state-dev/cli` · `npx fsdev` cold never works | Nothing today | **`fsdev` stays a reservation-only package indefinitely** — the one thing npm's dispute policy names, and only A cures it |
...
Two things carry it now. First, **the rename is cheap exactly once.** Today it is 34 files of text and a
...
**The cost I missed: under C, `fsdev` stays a reservation-only package forever.** This spec's own
...
background. *(It is a genuine consequence of the keep-both-placeholders call, which was not priced
when that call was made. The decision still stands on its asymmetry — unpublishing is irreversible
and hands the name back — but C now inherits a liability it did not appear to carry.)*

**Under C the `fsdev` placeholder simply stays published — so C has no *unpublish* decision to
make, though as priced above it does keep a reservation-only package.** **Decided by the
coordinator, not the owner** — I had leaned toward unpublishing under C and was
...
the first release, and FIX-1187 already carries two blockers (FIX-1186, FIX-870). **Toward B:** nothing short
...
- **The `contracts@0.0.0` probe was priced wrong, against the suggestion rather than for it.** An
  earlier draft claimed the real-package probe was *less* reversible than the throwaway because
  step 2b would add a second owner to it. **Step 2b does not touch `contracts`** — `npm owner`
  is per package-spec, and a scoped package draws its publish rights from organization membership
  instead. Corrected: inside 72 hours both probes are equally removable, so the substitution is
  cheaper than first written, not dearer. Recorded because the error ran in the direction of
  defending the design I had proposed first.
...
- **The scoped half of the credential proof stopped costing anything.** Publishing `0.0.0` of a
  package we ship anyway replaces a throwaway name — **but only for `@flow-state-dev`**, where
  `contracts` is the one publishable workspace with no workspace dependencies and so publishes
  installable. `@thought-fabric`'s only package pins two unpublished siblings, so the same move
  would leave a permanently uninstallable package under our real name; that scope keeps a
  throwaway. Recorded because it inverts the intuition: **the throwaway is the more reversible
  probe**, since it is unpublished inside 72 hours while it still has one owner, whereas
  `contracts@0.0.0` can never be withdrawn on those terms once step 2b adds the second owner.
- **A circular blocker, an unrunnable proof, and a cost the placeholder decision hid.** FIX-1186 was
  ...
  all 25. It also touched neither scope, so it proved nothing about 25 of the 25 packages the
  release ships. Replaced with a layered gate: identity in CI, then one throwaway probe **per
