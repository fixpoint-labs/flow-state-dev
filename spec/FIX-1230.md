...
- **The durable per-send status decision 2 requires is the delivery's own request record —
  ...
  **Note this lineage check is *not* the incarnation hazard §13 holds pending O2.** That one
  lives on the *queued* delivery path, where accept and load are separated by a queue, so O2
  decides whether the path ships at all. This one touches no queue: it is the authorization
  scope of a status lookup, live in every deployment under either O2 branch. They share a root
  cause and nothing else, and conflating them would park a fix that has no gate to wait for.
...
- **A relay binding may not carry a durable/suspending handler, and `relay` is never
  ...
  **SCOPE, AND IT IS A HOLE: this covers the DECLARED door only.** A public action reached by
  rule 3's fallthrough can also be `durable` and suspend, and the never-list makes that case
  permanently unresumable rather than merely refused — the refusal that closed the declared
  door removed the escape hatch from the door it does not cover. It cannot be closed at
  construction: the fallthrough's kind is caller-supplied, so there is no static pairing to
  validate. That makes it a door-design question rather than a validation one.
  **Escalated, unresolved — §13.**
...
**ESCALATED, UNRESOLVED — the durable-action hole in the *fallthrough* door, which §7's
re-entry refusal does not close.** §7 refuses a durable/suspending handler on a **declared**
`relay.on` binding and puts `RELAY_SOURCE` on the never-public-re-entry side. **That covers one
of this design's two doors.** Rule 3 of the resolution order sends a top-level/sibling message
to `flow.actions[kind]`, and a public action reached that way can equally be `durable: true`
(`flow.ts:233`) and call `ctx.suspend()`. Putting relay on the *permanent* never-list makes
that strictly worse than before: a suspended fallthrough delivery can never resume, and **no
configuration can change it** — the refusal that fixed the declared door removed the escape
hatch from the door it did not cover.

*Why this is not simply "extend the refusal", and why it is here rather than decided:* the
declared case is checkable at construction because a `relay.on` binding is a **static pairing**
of kind → handler, so an invalid combination exists in the flow definition and
`defineFlow`-time validation can name it. **The fallthrough has no static pairing.** Its `kind`
is a caller-supplied string, so the set of relay-reachable public actions is *every* public
action, and the only construction-time refusal available is *"no public action on a
relay-declaring flow may be `durable`"* — which breaks durable HTTP actions that have nothing
to do with relay. Enforcement therefore has to move to resolution time, and **that is a change
to what the second door reaches, not a validation detail**: it makes durable public actions
un-relayable as a product statement. Same content whether it is written as *refuse a durable
fallthrough target* or *disallow the fallthrough for durable actions* — the choice between
those two phrasings is real but secondary to whether the door narrows at all.

*So it is recorded and not specified.* Inventing a runtime check here would settle a door
question in the section that documents review notes. **This one needs the owner, and it is the
one item on this list whose absence is a live hole rather than a deferred improvement.**
