...
- **The preflight approves a session *incarnation*, and the delivery must land on that same
  ...
  **So: carry the expected incarnation with the delivery and revalidate it BEFORE ANY REQUEST
  OR ITEM PERSISTENCE; a mismatch drops the delivery, logged, and writes nothing to the
  replacement session.** Unconditional — the same `lineageId` remedy the sender-side status
  relation uses, applied at the other end.
  ...
  **"Before any persistence" is UNACHIEVABLE, and asking for it a third time was the wrong
  ...
  | # | write | where | before recipient context? |
  |---|---|---|---|
  | 1 | `activeRequests.register` | host, at enqueue (`createInboundTransportHost.ts:465`) | yes — before execution at all |
  | 2 | `stores.request.set` (the `in_progress` stub) | host, at enqueue (`:479`) | yes — same |
  | 3 | the same pair on the external-dispatcher branch | host (`:621`, `:636`) | yes |
  | 4 | `stores.session.set` — `latestRequestId` | `runAction.ts:819` | yes |
  | 5 | `emitItemAdded` + `emitItemDone` for `action.userMessage` | `runAction.ts:1005-1021` | yes |
  | — | `createExecutionContext` (first session load) | `runAction.ts:1215` | — |
  ...
  **So the requirement is RECONCILE ON MISMATCH, not "check earlier".** The check runs where the
  recipient record first exists, and because writes 1–5 may already have happened it must undo
  them: **delete the enqueue-time request record and its `activeRequests` entry, and leave
  `latestRequestId` untouched.** It must still precede writes 4 and 5, which are avoidable —
  being after them is a second defect, not a reason to skip the reconciliation for 1–3.
  ...
  *Test:* a delivery accepted, the recipient deleted and recreated under the same id before it
  runs, then assert the replacement session exposes **no request row** (the reconciliation —
  and the assertion the previous version of this test missed entirely), **no `activeRequests`
  entry**, an **unchanged `latestRequestId`**, and **empty reconstructed history** through
  `itemToLLMMessages`. Asserting the handler did not execute tests the mechanism; asserting the
  replacement session carries nothing of the old delivery tests the promise — and the request
  row is the part that survives session deletion, so it is the one a history-only assertion
  cannot see.
...
- **`timeoutMs` is validated at both boundaries and REFUSED out of range, never clamped.**
  ...
  **`NaN` is the one edge that does not reach the verb through the tool, and it needs no
  special handling — because a model cannot express it.** Measured against the pinned Zod
  (3.25.76): a plain `z.number()` **accepts** `Infinity`, `-1` and `2**31` and **rejects only**
...
6. **The `sendMessage` operation and its verb**, assembled the way
   ...
   NO REQUEST ROW, no `activeRequests` entry, an unchanged `latestRequestId`, and empty
   reconstructed history** (§7). Not "the handler did not run": the enqueue-time stub is
   ...
   see it. **The delivery reconciles on mismatch — deleting that stub — rather than relying on
   a check placed early enough.** The in-process `policy: "queue"` window needs no external
   dispatcher.
   *Depends on:* 5.
   ...
   **The persisted `(delivery requestId → sender session id + that session's `lineageId`)`
   ...
   PR b. **This is BP-030 at PR granularity:** PR a hands out delivery ids, so if the relation
   ...
   correlation, no wait. *Test:* a delivery created here has its relation persisted with the
   sender's lineage, readable by the PR b verb once it exists.
...
13. **The unknown-outcome contract** (decision 2) — **the *read* half; the relation itself is
    ...
    **PARTLY CONDITIONAL ON O3 — what a pruned delivery answers, and therefore the result
    ...
    timeout composition that moved to PR a. What is **not** settled is whether `deliveryStatus` can distinguish a
...
| Recipient deleted and recreated under the same id after the delivery was accepted | Dropped and logged, **and the acceptance-time writes reconciled** — the request record and `activeRequests` entry deleted, `latestRequestId` untouched (§7). Not "checked early enough": the stub is written at acceptance while the original session is still valid, so no earlier placement could detect a mismatch that does not yet exist. The replacement session exposes no request row and no history |
