...
**Size:** Large — 16 production files · 43 test behaviours · 6 doc surfaces · 2 PRs.
...
- **The reply registry entry is *taken*, not read-then-deleted.** "First reply wins" is not
  delivered by deregistering on the waiter's terminal paths: two `reply()` calls for one
...
- **One discriminated result shape for `send`, and the payload is part of it.** §5's example
...
5. **The `send` result shape** — one discriminated outcome covering *accepted* (with the
...
6. **The send operation and `ctx.requestHost.send`**, assembled the way
...
8. **The agent-facing tool**, exposing `fireAndForget`. *Test:* it is offered inside a
...
11. **`ctx.requestHost.reply`**, reading the correlation id off `metadata.relay` and closing
...
12. **The two refusals**, both at the send verb, satisfying §7's five constraints —
...
15. **The flow-integration scenario** — a synthetic fixture flow under
    ...
    **send → recipient runs → reply → sender resumes with the payload**, provider-free via
    ...
    puts exactly this case in that tier (`AGENTS.md:151`, `:154`). *Depends on:* 13.

| sub-PR | deliverables | depends_on |
|---|---|---|
| a | steps 1–8: `sessionKind` + backfill, the two-axis door, the input item, the send verb, fire-and-forget, the arbiter branch + key thread, the tool | — |
| b | steps 9–15: the wait helper, the reply registry, correlation, both refusals, the unknown-outcome contract | a |
...
round 4 — proving the correlation mechanism while proving nothing about the sender getting
...
- **After spec review, round 4** — four folds and one escalation. The `send` result gained a
