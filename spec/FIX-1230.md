...
- **The preflight approves a session *incarnation*, and the delivery must land on that same
  ...
  **What relay does, in two parts.**
  ...
  1. **A relay delivery does not stamp `latestRequestId` at all**, so the dangerous write never
     ...
     delivery's stream.
  2. **The residual window is named, not papered over.** A recreate can still land *after* the
     guard passes; a check at one instant cannot cover a change at the next without a
     conditional write or a lock, and part 1 removes relay's worst write but not time itself.
     **So relay does not claim a recipient can never be recreated mid-delivery.** It claims the
     delivery is dropped and reconciled when the mismatch is visible at the guard — which is
  ...
  *Test — the interleaving, not the outcome.* **The delete/recreate-before-run test does not
  exercise this window**: it recreates before the guard runs, so it proves only that the guard
  fires. Pin the ordering instead — a delete-and-recreate driven **between** the `get` and the
  write, asserting the replacement session's fields are not overwritten and its
  `latestRequestId` is untouched. Without that interleaving the test passes whether or not the
  window is handled, which would make it the sixth check on this spec that could not fail.
...
- **The framework's own tool wrapper defeats the blocking send, and its retry causes the
  ...
  **The seam already exists one layer down, and the tool executor is the outlier.**
  ...
  2. **Timeout** — the tool's own override, falling back to `flowTools?.defaults?.timeoutMs`.
     ...
     **It goes on a TOOL-SCOPED surface, not on generic `BlockConfig` — and an earlier draft
     put it in the wrong place for a reason worth keeping.** `BlockConfig`
     (`core/src/types/block.ts:767`) is one interface shared by all four block kinds. Only
     `buildToolExecutor` would read this field, so on a handler, router or sequencer it would
     do nothing — **and that is the same defect as the dead `config.retry` this very step
     exists to fix.** Shipping a new field that is inert on three of four kinds, inside the
     change that repairs an old field that was inert on the tool path, is not a placement nit.

     *The competing precedent, surfaced rather than averaged over (tenet 6).* `cacheable`
     (`:831`) **is** a tool-only knob living on generic `BlockConfig`, and its doc says so —
     *"No effect when the block is used outside a generator's tool slot."* So the house has
     two patterns and they disagree: a tool-only field on the shared interface with a
     no-effect note, versus a tool-scoped surface. **We take the tool-scoped surface here**,
     because a documented no-effect field is still a field three block kinds can set and be
     wrong about, and because the count matters — one such field is a wart, a second is a
     convention. *This does not retroactively move `cacheable`*: that is a live public field
     with its own consumers, and relocating it is not this issue's to do. Noted so whoever
     reconciles them later can see the choice was made rather than drifted into.

     *Explicitly NOT doing:* defining timeout semantics for general block execution so the
     field could sit on `BlockConfig` honestly. That is a much larger change than this issue
     and reaching for it would be the scope creep this placement avoids.
...
6. **The `sendMessage` operation and its verb**, assembled the way
   ...
   trustworthy (§7) — `metadata.relay = { correlationId, from, recipientLineageId }`
...
8. **The agent-facing tool** — an **exported tool factory an author adds to a generator's
   ...
   **This step also carries the executor change the composition needs** (§7):
   ...
   gains a **tool-scoped** `timeoutMs` — deliberately not on generic `BlockConfig`, where it
   would be inert on three of four block kinds (§7). **The merge helper moves from
...
**A mechanism named is not a seam decided — this spec got that wrong three times.** Each time a
...
reach), and the tombstone's **persistence** (a marker that must outlive the row retention
deletes, with no store named). The tell is the same in all three: a sentence that says *what*
happens without naming *where it lives*. **When you meet one in this document, stop and check
