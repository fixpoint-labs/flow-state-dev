...
- **The durable per-send status decision 2 requires is the delivery's own request record —
  ...
  **Note this is *not* the incarnation hazard §13 holds pending O2.** That one lives on the
  *queued* delivery path, where accept and load are separated by a queue, so O2 decides whether
  the path ships at all. This one touches no queue: it is the authorization scope of a status
  lookup, live in every deployment under either O2 branch. They share a root cause and nothing
  else, and conflating them would park a fix that has no gate to wait for.
- **A relay binding may not carry a durable/suspending handler, and `relay` is never
  ...
  **So: refuse a durable or suspending handler on a relay binding at construction, by name and
  ...
  the same construction. *Rejected:* **defining a relay-specific re-entry contract now.** It is
...
4. **The relay input item** — emitted by construction on every relay dispatch, from the
   ...
   declares no `userMessage` still produces a message the recipient's next generator turn
   *reconstructs* — asserted through `itemToLLMMessages`, not through the item being
   persisted. *Depends on:* 3.
...
6. **The `sendMessage` operation and its verb**, assembled the way
   ...
   caller-supplied (§7). **And it refuses `external-dispatcher` for BOTH modes whenever the
...
13. **The unknown-outcome contract** (decision 2) — the persisted **(delivery → sender
    ...
    meaning "did not happen" — plus `deliveryStatus(deliveryRequestId)` on the request host, returning
    `DeliveryStatusResult` (§7), so the caller can actually act on it. *Test:* a reply that lands
    after the listener is gone, and a delivery that runs after the sender is terminal,
    neither throwing and neither reported as a negative; **and a sender session deleted and
    recreated under the same id cannot read the previous incarnation's delivery** — it gets
    `unknown-delivery`. *Depends on:* 10.
...
|---|---|---|
| a | steps 1–8: **`sessionKind` + backfill — CONDITIONAL ON O1** (§12; step 1), **the two-axis door and the flow-boundary guard — partly conditional on O1** (step 3, where the guard becomes resolve-and-dispatch rather than a refusal), the input item, the `sendMessage` verb, fire-and-forget, the arbiter branch + key thread, the tool. **The other six steps carry no O1 gate and are what PR a begins on today** | — |
...
- **the door, three tests, and they are the security surface**: (a) a message naming a public
  ...
  **another flow** is **refused**; *under O1-yes* that same send must **succeed**, dispatched
  ...
  (f) a legacy record with no `sessionKind`, at **either** end, is refused **even when the
  flow declares a matching `relay.on` handler** — the ordering rule, and the case an earlier
  ...
  and validates. Six of these seven assert a refusal under O1-no (five under O1-yes, where
  (e) inverts); without (g) the suite proves the door closes and nothing about it opening
...
- **the message is visible to the recipient's next generator** when the binding declares no
  `userMessage` — run the recipient's persisted items through `itemToLLMMessages` and assert
  the message *appears*, then flip the input item to structural and assert it disappears.
...
fill in.** §12's **O2** is unanswered, and the refusal §8 step 6 ships is a *recommendation*
the owner has not accepted yet. A CI spec asserting it would settle O2 inside the section an
implementer reads as mandatory, which is the exact move §9's cross-flow row and the door
block's (e) were just corrected for: a fork left open in §12 and closed by implication
somewhere downstream. When O2 is answered, its test belongs here — and under *"support
queue-backed"* it is a blocking item, not an addition.
...
**A third finding is HELD pending §12's O2** — the queue-backed scope question, *not* the
flow-ownership one. It is recorded apart from the two above because the two gates resolve
independently, and because "held" means something different under each branch of O2.

- **HELD — the preflight approves a session *incarnation* that the delivery may not land on.**
  *The claim:* a recipient session can be deleted and recreated under the same session id
  between the preflight's read and the moment a **queued** `runAction` loads it — recreation is
  supported and produces a new `lineageId`. The preflight will have chosen the door, and under
  O1-no the sender flow's handler, from the *old* record; nothing downstream re-checks. So the
  handler can execute against a replacement session belonging to another flow, defeating both
  the flow boundary and the relay door.
  *The premise is verified, and this codebase already treats recreation-under-the-same-id as a
  live hazard.* It is anticipated and defended against elsewhere: `lineageId` is part of the
  derived child session id precisely "because otherwise a recreated session spawning the same
  seed derives the same child and *adopts* the previous lineage's workstream, silently
  inheriting a dead conversation's address" (`docs/architecture/state-and-scopes.md:569-571`).
  And the missing downstream recheck is real and deliberate — `createExecutionContext`
  validates the user, tenant and org bindings and **not** `flowKind` or `parentSessionId`
  (`docs/architecture/state-and-scopes.md:471`), which is the same gap §7 already leans on the
  preflight to close.
  *Why it is held rather than specified:* the window this finding needs is the **queued** one —
  accepted here, loaded later, on the fire-and-forget path. **O2 decides whether that path
  exists in release 1 at all.** Specifying an incarnation-binding protocol before that answer is
  work against a path we may refuse, and it would leave the spec reading as though it had
  committed to shipping queue-backed relay — the one thing O2 is asking the owner and has not
  been told.
  *What each O2 branch makes true — read this before taking "held" for "dismissed":*
  - **Under the standing recommendation (refuse both send modes on an external dispatcher):**
    the window shrinks to within-request, where the preflight's read and the dispatch are not
    separated by a queue, and the finding largely dissolves. *Largely,* not entirely — a
    same-process delete and recreate is still expressible; it just stops being a gap between two
    stages of one send.
  - **Under "support queue-backed deployments":** this is a **blocking correctness item that
    must be specified before implementation**, not a nice-to-have and not an implementer note.
    The shape is an expected incarnation carried into execution and revalidated there, or an
    otherwise atomic resolution, with the queued delete/recreate race tested. It would then be
    the **third** guarantee shown not to survive the queue boundary, alongside the two-axis door
    and recipient arbitration — which strengthens O2's own recommendation rather than sitting
    beside it.
