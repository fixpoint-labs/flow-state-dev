...
**Size:** Large — 15 production files · 24 test behaviours · 6 doc surfaces · 2 PRs.
...
- **The door, and it is the whole security property. It has TWO axes, and conflating them is
  ...
  **fallthrough to `flow.actions`** is what is gated, and it needs both:
  ...
  | | rule | protects |
  |---|---|---|
  | **recipient** is `"workstream"` | **refuse** — terminal, never `flow.actions` | the recipient's narrow declared surface |
  | **sender** is `"workstream"` | **refuse**, whatever the recipient is | the agent's confinement |
  | either kind is **absent** | **refuse** | the legacy record, per BP-030 above |
  | otherwise (`"top-level"` / `"sibling"` both ends) | falls through to `flow.actions[kind]` | — |
...
- **The sender's held key is *carried*, never recomputed (tenet 5).** The refusal in
  decision 1 compares two keys, and only one of them is easy: the delivery's key is resolved
  by the arbiter at dispatch. The **sender's** is `ResolvedDecision.key`, computed inside
  `createInboundTransportHost` (`:299-301`) and never handed onward — `createRequestHost`
  receives neither it nor the action/source/metadata a custom `ConcurrencyKey` closure reads
  (`arbiter.ts:104-118`). Recomputing it from the relay envelope would be a *second* writer of
  one value, and for a custom key function it can legitimately differ — the refusal then
  misses a real collision and `waitForResponse` hangs to its timeout instead of refusing in
  milliseconds. So the host threads the already-resolved key down the path that already
  carries identity and `effectiveRuntimeConfig`. **Decision 1 is at the gate right now; this
  is the difference between its refusal firing and quietly not.**
...
send(to, message, mode):
    ...
    if mode is waitForResponse:
        ...
        refuse "key-collision"        if the delivery's key == the key MY dispatch
                                      RESOLVED (carried in, never recomputed)
...
6. **The `RELAY_SOURCE` arbiter branch** — recipient's key, `waitTimeoutMs: Infinity` — **and
   threading the dispatch's resolved sender key** down to the request host (§7), which PR b's
   refusal reads. Landing the thread here rather than in b keeps b to behaviour. *Test:* a
   recipient busy longer than the 30s default still runs — the Q4 shape, as a CI spec; the
   threaded key equals what the arbiter resolved, including for a custom `ConcurrencyKey`.
   *Depends on:* 5.
...
11. **The two refusals**, both at the send verb: `key-collision` (decision 1) and
