...
**Size:** Large — 15 production files · 27 test behaviours · 6 doc surfaces · 2 PRs.
...
- **The sender's decision is *carried*, never recomputed (tenet 5) — and it is the whole
  decision, not just the key.** The refusal in decision 1 compares two keys, and only one is
  easy: the delivery's is resolved by the arbiter at dispatch. The **sender's** lives in
  `ResolvedDecision`, computed inside `createInboundTransportHost` (`:299-301`) and never
  handed onward — `createRequestHost` receives neither it nor the action/source/metadata a
  custom `ConcurrencyKey` closure reads (`arbiter.ts:104-118`). Recomputing would make a
  *second* writer of one value, and for a custom key function the two can legitimately
  differ, so the refusal would miss a real collision and `waitForResponse` would hang to its
  timeout instead of refusing in milliseconds. The host therefore threads the resolved
  decision down the path that already carries identity and `effectiveRuntimeConfig`.
- **A RESOLVED key is not a HELD key, and comparing the wrong one breaks the default
  configuration.** `normalizeConfig(undefined)` returns `{ policy: "allow", key: "session" }`
  (`arbiter.ts:88-93`), so a flow declaring no `concurrency` still resolves a **populated**
  session key — while `gate` short-circuits on `policy === "allow"` and acquires **nothing**
  (`:164-167`). A refusal that compared resolved keys would therefore fire on a
  self-addressed `waitForResponse` under the config **most apps have**, for exactly the case
  §9 promises is permitted. So the rule is:

  > **Refuse only when the sender actually acquired a key** — its policy is `queue` or
  > `reject` *and* its key is defined — **and the delivery would gate on that same key**
  > (the delivery's own policy is not `allow`, and its resolved key is equal). Both sides'
  > policies are read, because a caller-addressed action's override and the flow default can
  > differ between the two dispatches.

  **Measured, not reasoned:** Q6 cases 6-7 are the default config, added after the original
  five all declared `queue` and so never exercised it. Case 7 is a self-addressed blocking
  send there — the naive resolved-key comparison says *collide*, and the run says
  **permitted**. §6's decision 1 is unchanged and its *"apps that declare no concurrency
  policy are unaffected"* still holds; it is this mechanism that had to be stated correctly
  for that to be true.
- **How a relay dispatch resolves its *policy*, which has two answers because the door has
  two forms.** The precedent is in the same function: `actionConfig` is skipped for an event
  or detached dispatch because those pass a handler **block name** as `actionName` —
  provenance only — and a name collision would let an unrelated public action's policy decide
  whether the work runs (`arbiter.ts:145-159`). Relay's two forms fall on opposite sides of
  that line, so arbitration reads the resolved door form:
  - a **declared `relay.on[kind]`** binding — a relay coordinate, provenance-only against
    `flow.actions` → **the flow default**, like events and detached.
  - a **fallthrough to `flow.actions[kind]`** — a genuine public action, addressed as itself
    → **its own `concurrency` override applies**.

  Neither default alone is right: always consulting `flow.actions` leaks a same-named public
  action's policy into a declared binding, and never consulting it ignores a real override on
  the path that *is* that action.
- **The wait is abort-aware, because a bounded wait is exactly BP-035's cancel path.**
  `waitForItemMatch` takes the request's `AbortSignal` and tears down timer and subscription
  on it — including the **already-aborted** case, which needs a synchronous check because a
  listener added after `aborted` flipped never fires. `waitForCondition` already does both
  (`core/src/blocks/sequencer.ts:2329-2340`); a wait built beside it that does not is a leaked
  subscription and a late reply mutating a terminal response.
- **A relay message emits an input item, so §2's "visible in that session's history" is
  backed rather than asserted.** `ActionCore.userMessage` is optional (`flow.ts:218`) and
  `runAction` emits the user `message` item only when it is declared (`:1005-1006`) — later
  turns rebuild history from persisted items, so a binding that omits it (as this spec's own
  §5 example does) would deliver a question the recipient's next turn cannot see. The relay
  source therefore emits the input item **by construction**: the binding's `userMessage` when
  declared, a framework default carrying kind and payload otherwise. Automatic rather than a
  required mapper, so the promise cannot be lost by an author forgetting one.
...
send(to, message, mode):
    ...
    if mode is waitForResponse:
        ...
        refuse "key-collision"        ONLY IF I actually ACQUIRED a key
                                      (my policy is queue|reject, key defined)
                                      AND the delivery would gate on that same key
                                      (its policy is not allow, its key is equal)
                                      — my decision is carried in, never recomputed;
                                        a RESOLVED key is not a HELD one
...
3. **`RELAY_SOURCE` and the resolution rule** (§7): the declared handler first; with none,
   ...
   *Test:* the four in §10's door block. *Depends on:* 1, 2.
...
6. **The `RELAY_SOURCE` arbiter branch** — recipient's key, `waitTimeoutMs: Infinity`, and
   **policy resolved by door form**: flow default for a declared `relay.on` binding, the
   action's own override on the `flow.actions` fallthrough (§7). **Plus threading the
   dispatch's whole resolved decision** — policy *and* key — down to the request host, which
   PR b's refusal reads. Landing the thread here rather than in b keeps b to behaviour.
   *Test:* a recipient busy longer than the 30s default still runs — the Q4 shape, as a CI
   spec; the threaded decision equals what the arbiter resolved, including for a custom
   `ConcurrencyKey`; a declared binding does **not** inherit a same-named public action's
   `concurrency`, and a fallthrough **does**. *Depends on:* 5.
...
11. **The two refusals**, both at the send verb. `key-collision` fires **only when the
    sender actually acquired its key and the delivery would gate on that same key** (§7) —
    not on key equality alone, which would refuse on the default config. And
