...
- **EXTEND** `docs/architecture/detached-work.md` (`:18`) and
  ...
  type's own header. Relay adds two verbs to that seam. **This is not a find-and-replace from
  "four" to "six": the sentence is a claim of *closure*, not a count**, and what relay
  falsifies is the closure — the seam becomes something that has been deliberately reopened
  once. Whoever writes the prose decides what the claim becomes: re-state the seal at the new
  number carrying the same "adding another is a decision someone reviews" force, or replace the
  count with the property the count was standing in for. **Do not leave a bare number behind.**
