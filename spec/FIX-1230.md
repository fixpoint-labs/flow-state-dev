...
6. **The `sendMessage` operation and its verb**, assembled the way
   ...
   stamped at the seam, `metadata.relay = { correlationId, from, `**`recipientLineageId`**` }`
   ...
   this PR; `waitForResponse` refuses `mode-not-available`, and `no-durable-sender` when the
   sending session was not caller-supplied (§7). *Test:* the identity tuple
   ...
   deleted and recreated under the same id before it runs, does not execute the handler** —
   the in-process `policy: "queue"` window, which needs no external dispatcher (§7).
   *Depends on:* 5.
...
8. **The agent-facing tool** — an **exported tool factory an author adds to a generator's
   ...
   fire-and-forget half, which PR a is what ships. *Depends on:* 5.
...
13. **The unknown-outcome contract** (decision 2) — the persisted **(delivery → sender
    session id + that session's `lineageId`)** authorization relation so a later turn can
    resolve it **and a same-id replacement session cannot** (§7), the composition with
...
queue-backed dispatcher refusing fire-and-forget (the blocking refusal is unconditional and
testable now); under *support queue-backed*, the door and
