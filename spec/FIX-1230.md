...
- **The preflight approves a session *incarnation*, and the delivery must land on that same
  ...
  **"Before any persistence" is the load-bearing half, and "the handler must not run" is not
  enough.** `runAction` parses the input and emits `action.userMessage` — `emitItemAdded` +
  ...
  *Test:* a delivery accepted, the recipient deleted and recreated under the same id before it
  runs, and **the replacement session's history asserted untouched** — reconstructed through
  `itemToLLMMessages`, not merely "no handler output". Asserting the handler did not execute
  tests the mechanism; asserting nothing from the old delivery reached the new session tests
  the promise.
...
- **`timeoutMs` is validated at both boundaries and REFUSED out of range, never clamped.**
  ...
  *Test:* the numeric edges — `NaN`, `Infinity`, `-1`, `0`, and `2**31` — each returning
  `invalid-timeout` **as a result**, from the tool as well as the verb, plus the largest
  supported value accepted. **Assert the tool path returns rather than throws**, since that is
  the half the contradiction would have broken.
...
6. **The `sendMessage` operation and its verb**, assembled the way
   ...
   deleted and recreated under the same id before it runs, leaves the replacement session's
   history UNTOUCHED** — asserted through `itemToLLMMessages`, not by "the handler did not
   run", because `runAction` persists `action.userMessage` before `createExecutionContext`
   loads the session (`:1005-1021`), so a late revalidation passes the handler assertion while
   the old delivery's message is already in the new session (§7). The in-process
   `policy: "queue"` window needs no external dispatcher.
   *Depends on:* 5.
...
14. **The tool's blocking mode, and the status tool beside it** — **plus `timeoutMs`
    ...
    error. *Test:* the numeric edges returning `invalid-timeout` **through the tool**, and the
    largest supported value accepted. Both from the same
...
| Recipient deleted and recreated under the same id after the delivery was accepted | Dropped and logged, **before any request or item is persisted** (§7). The replacement session's history is untouched — not merely "the handler did not run", since `runAction` emits `action.userMessage` before the session is loaded (`runAction.ts:1005-1021`) |
...
- **`timeoutMs` numeric edges come back as a RETURNED `invalid-timeout`, not clamped and not
  thrown**: `NaN`, `Infinity`, `-1`, `0` and `2**31`, plus the largest supported value
  accepted. **Assert it through the tool, not only the verb** — a schema-level range check
...
- **REVISE** `docs/architecture/execution-and-errors.md:303-306` — **and a line naming the
  ...
  It states, as one of *"two rules [that] make the seam safe"*: *"**Identity is never a
