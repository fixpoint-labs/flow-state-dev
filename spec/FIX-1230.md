...
- **Item visibility has TWO axes, and this spec introduces TWO items — so it is four cells,
  ...
  **both defaults are `client: true`** — conversational items default
  ...
  | item | `client` | `history` |
  |---|---|---|
  | the **reply carrier** on the sender's stream | `false` — an internal correlation envelope, not something a UI should render, and a conversational carrier would surface it as a fake user message | `false` — `itemToLLMMessages` (`history.ts:57-70`) would otherwise replay it into the next generator turn as a user utterance, so the sender reads its own answer twice |
  | the **relay input item** on the recipient's stream | `true` — it is the message that arrived; a UI watching that session should show it | `true` — §2 promises the message is visible in that session's history, and a later turn rebuilds from items admitted by `itemToLLMMessages`, which takes only conversational items with `history: true` |
