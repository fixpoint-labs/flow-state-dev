...
**Size:** Large — 16 production files · 47 test behaviours · 6 doc surfaces · 2 PRs (each with a changeset).
...
2. **The `relay` declared group** on the flow definition and instance, mirroring
   ...
   *Depends on:* nothing.
3. **Partly conditional on O1** (the absent-kind refusal and the two-axis door; the
   flow-boundary check and `RELAY_SOURCE` itself are unaffected) — **the resolution rule**
   (§7): the declared handler first; with none,
...
13. **The unknown-outcome contract** (decision 2) — the `timedOut` result that never means
...
14. **The tool's blocking mode**, and the refusal surfaced to the model as a tool result it
    can act on rather than an exception. *Depends on:* 12, 13.
...
steps are exposed lets PR a start on the seven that are not.
...
**One finding is HELD, not dropped, pending §12's O1** — the flow-ownership question, which
...
and its backfill would dissolve. The finding below is real and verified; it is not worth
