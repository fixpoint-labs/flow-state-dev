...
- **The framework's own tool wrapper defeats the blocking send, and its retry causes the
  ...
  **The seam already exists one layer down, and the tool executor is the outlier.**
  ...
  1. **Retry** — `buildToolExecutor` resolves `mergeRetryPolicy(tool.config.retry,
     flowTools?.defaults?.retry)` instead of the flow default alone. No new field, no new
     helper, no new precedence: `maxAttempts: 1` is already "do not retry".
  2. **Timeout** — `tool.config.timeoutMs ?? flowTools?.defaults?.timeoutMs`. This is the one
     addition: **`BlockConfig` gains `timeoutMs?: number` beside the `retry` it already has**,
     because the asymmetry — retry overridable per block, timeout not — *is* the gap. **`0`
     needs no new semantics**: `withTimeout` already returns the promise untouched for
...
8. **The agent-facing tool** — an **exported tool factory an author adds to a generator's
   ...
   **This step also carries the executor change the composition needs** (§7): `buildToolExecutor`
   resolves `mergeRetryPolicy(tool.config.retry, flowTools?.defaults?.retry)` and
   `tool.config.timeoutMs ?? flowTools?.defaults?.timeoutMs`, and `BlockConfig` gains
   `timeoutMs?: number` beside its existing `retry`. **The test that proves it is per-tool
