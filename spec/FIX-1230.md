...
8. **The agent-facing tool** — a thin surface over **the same `sendMessage` verb**, exposing
   ...
   sits at the verb rather than at the tool. *Test:* it is offered inside a worker's declared
   surface and nowhere else, and its result is the same union step 5 defines.
   *Depends on:* 5.
...
**Are queue-backed deployments in scope for this issue's first release at all?** Two of this
spec's guarantees do not hold past the queue boundary, and they were found independently:
...
**One finding is HELD, not dropped, pending an open design question.** The owner is
checking whether workstreams get their own flow config rather than running their parent's.
