...
6. **The `sendMessage` operation and its verb**, assembled the way
   ...
   **The tombstone's WRITE side ships here too, for the same reason and by the same
   precedent.** O3 resolved to the tombstone branch, so a pruned delivery is a distinguishable
   answer rather than collapsing into `unknown-delivery`. But `deliveryStatus` only *reads* that
   marker in PR b — and if PR a wrote no marker, every delivery pruned by recipient retention
   during the PR-a-only window would be **permanently unreadable, with no later release able to
   repair it**: the request row and its relation are already gone by the time PR b deploys, so
   the marker cannot be created retroactively. **This is the same asymmetry as the relation
   above, and it takes the same answer** — the write belongs on the PR-a side of the line, the
   read on PR b's. It is not a new gate: gating PR a's retention behaviour on O3 would re-block
   work O3's closure released, and it is unnecessary once the write lands here.
...
8. **The agent-facing tool** — an **exported tool factory an author adds to a generator's
   ...
   **This step also carries the executor change the composition needs** (§7):
   ...
   gains `timeoutMs?: number` beside its existing `retry`. **The merge helper moves from
