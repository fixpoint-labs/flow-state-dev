...
- **`timeoutMs` is validated at both boundaries and REFUSED out of range, never clamped.**
  ...
  **This is caller-controllable input steering behaviour, so the answer is a refusal, not a
  ...
  refuses a `timeoutMs` that is not a finite integer within the supported range. **Both
  boundaries**: the public verb *and* the tool's input schema, because **a model supplies this
  value too** and a schema that admits `Infinity` hands the agent a way to ask for something the
  runtime cannot express. *Test:* the numeric edges — `NaN`, `Infinity`, `-1`, `0`, and
  `2**31` — each refused by name at both boundaries, and the largest supported value accepted.
...
14. **The tool's blocking mode, and the status tool beside it** — **plus `timeoutMs`
    validation at BOTH boundaries** (§7): the `sendMessage` verb and the tool's input schema
    each refuse `invalid-timeout` for `NaN`, `Infinity`, negatives and anything past
    `2**31 - 1`, rather than letting Node clamp them to a 1 ms timer and return an immediate
    `unknown`. The schema half is not redundant — **a model supplies this value**, so the
    boundary a generator crosses has to reject it too. *Test:* the numeric edges refused by
    name at each boundary, and the largest supported value accepted. Both from the same
...
| `timeoutMs` is `NaN`, `Infinity`, negative, or past `2**31 - 1` | Refused `invalid-timeout` at send time, at the verb **and** at the tool schema (§7). Not clamped: Node silently reduces all of these to a **1 ms** timer, so the caller asking for an unbounded wait would otherwise get an immediate `unknown` — the opposite of the request |
...
- **`timeoutMs` numeric edges are refused, not clamped**: `NaN`, `Infinity`, `-1`, `0` and
  `2**31` each refused `invalid-timeout` — **at the tool schema as well as the verb**, since a
  model supplies this too — and the largest supported value accepted. The clamp is what makes
  this worth testing: an unvalidated `Infinity` returns `unknown` in about a millisecond and
