...
**Size:** Large — 16 production files · 38 test behaviours · 6 doc surfaces · 2 PRs.
...
- **One discriminated result shape for `send`, and the payload is part of it.** §5's example
  ...
  and has to be stated once rather than implied: an outcome that distinguishes **replied**
  (carrying the recipient's payload), **unknown** (the wait ended, decision 2 — carrying the
  delivery's `requestId`), and **refused** (carrying which refusal). An implementation could
...
3. **`RELAY_SOURCE` and the resolution rule** (§7): the declared handler first; with none,
   ...
   **Plus the flow-boundary check** — `recipient.flowKind === flow.kind`, refused as
   ...
   runs first, ahead of resolving the declared handler** (§7 rule 0).
   *Test:* the six in §10's door block. *Depends on:* 1, 2.
...
5. **The `send` result shape** — one discriminated outcome covering *replied* (with the
   recipient's payload), *unknown* (with the delivery's `requestId`) and *refused* (with
   which refusal), per §7. Declared here because §5's public example reads it, so it is
   surface rather than an internal return. *Test:* each arm is reachable and carries its
...
10. **The reply registry** — **one map, `correlationId → ResponseEmitter`**, registered and
   ...
   *Test:* deregistration on every terminal path — reply, timeout, abort, and a synchronous
   dispatch throw; and two concurrent replies for one correlation id append exactly once.
...
*Recommendation:* **none from me — this is the coordinator's to put to the owner.** If
queue-backed *is* in scope, the door sub-fork (stamp at send time vs. defer to the worker,
§7) still has to be answered and I have no recommendation there either: it is the owner's
door and the two options weaken different things. What I will assert is that relay should not
build two guarantees on a layer whose own comment says it does not provide them.
*What would change it:* whether FIX-830 lands first. It owns arbitration for external
dispatch, and with it in place decision 1's half of this closes on its own — leaving only the
door sub-fork. If queue-backed is scoped out instead, AC 4's refusal extends to
fire-and-forget and the whole thing closes.
*If wrong:* every option is recoverable in a later release; **leaving it unanswered is
not** — the deployments the epic says need relay most would quietly get a weaker door *and*
an unenforced concurrency promise, both while the spec at this gate says otherwise.
