...
4. **One convergence point for the refusal** (tenet 5) — the two refusals (`key-collision`,
   `no-cross-worker-wake`) are decided at the `sendMessage` verb, which is the single place a relay
...
- **`@flow-state-dev/engine`** — a `RELAY_SOURCE` in `transport-sources.ts`; the resolution
  ...
  an optional `crossWorkerWake` flag on `FlowDispatcher`, which — per §12's recommendation —
  gates **both** send modes rather than only the blocking one.
- **Wake support alone must NOT re-enable external relay, and this needs saying because the
  plan invites the opposite reading.** Epic issue 2 supplies the cross-worker wake channel
  and nothing else. If flipping `crossWorkerWake` were sufficient, issue 2's author would
  re-open queue-backed relay in **both** modes — while the two-axis door still cannot be
  reproduced past the boundary and external dispatch is still unarbitrated. That would undo
  §12's recommendation through a one-line flag change in a different issue, by someone with
  no reason to know they were doing it. **So external relay is gated on all three
  independently: wake support, door authority across the boundary, and arbitration.** The
  flag is one of three preconditions, never a green light on its own — issue 2 satisfies one
  of them and leaves relay refused.
...
- **One discriminated result shape for `sendMessage`, and the payload is part of it.** §5's example
  branches on whether an answer came back and then reads it, so the shape is public surface
  and has to be stated once rather than implied: an outcome that distinguishes **accepted**
  (fire-and-forget's success — carrying the delivery's `requestId`), **replied** (carrying the
  recipient's payload), **unknown** (the wait ended, decision 2 — also carrying that
  `requestId`), and **refused** (carrying which refusal). **The `accepted` arm is not a later
  addition: PR a ships fire-and-forget, so without it the half that lands first has no way to
  report success** — it would borrow an arm meaning something else. Same failure as the reply
  payload, one step earlier in the plan. An implementation could
  satisfy every wait-and-correlation behaviour below and still hand the sender nothing, which
  is the whole ask-and-continue flow. That is the failure this spec has now hit three times:
  **the check asserts the mechanism and not the promise.**
...
sendMessage(to, kind, payload, mode):
    ...
    refuse "no-cross-worker-wake" unless the effective dispatcher can wake us
                                                  (BOTH modes — §12's recommendation)
...
5. **The `sendMessage` result shape** — one discriminated outcome covering *accepted* (with the
   delivery's `requestId`), *replied* (with the recipient's payload), *unknown* (with that
   `requestId`) and *refused* (with which refusal), per §7. Declared here because §5's public
   example reads it, so it is surface rather than an internal return. **`accepted` is what PR
   a's fire-and-forget returns, so it ships in this PR, not with the waiting half.**
   *Test:* each arm is reachable and carries its field. *Depends on:* nothing.
6. **The `sendMessage` operation and its verb**, assembled the way
   ...
   `mode-not-available`. **And it refuses `no-cross-worker-wake` for BOTH modes when the
   ...
   recommendation to be true of what we build. It reads an optional
   `crossWorkerWake?: boolean` on `FlowDispatcher` defaulting to
   `isInProcessDispatcher(dispatcher)`, so no existing dispatcher declares anything and epic
   issue 2 sets one flag rather than inverting a shipped guard. *Test:* the identity tuple
...
12. **The `key-collision` refusal**, at the `sendMessage` verb, satisfying §7's five
    ...
    on that same key, before anything queues or rejects. (`no-cross-worker-wake` is not here:
    it landed in step 6 with fire-and-forget, covering both modes — and per §7 it is gated on
    all three of wake support, door authority and arbitration, so epic issue 2 alone does not
    lift it.)
...
13. **The unknown-outcome contract** (decision 2) — the persisted **(delivery → sender
    ...
    the send (§7), and the `timedOut` result that never means
...
- **every result arm is reachable and carries its field** — `accepted` with the delivery's
  request id (the only success PR a can return), `replied` with the payload, `unknown` with
  the request id, `refused` with the reason
