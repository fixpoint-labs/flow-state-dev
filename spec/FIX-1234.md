...
decided**, in the exit check, onto state scoped to *this drain invocation* — the
per-request bag reached from the drain's own `ctx` — and the completion item reads that.
...
3. **Carry the verdict out of the exit decision.** Record, where the drain decides to
   ...
   (the per-request bag reached from the drain's own `ctx`), **not** the board's
   construction-scoped flow-state bag, which every concurrent drain of that board
   definition shares (§7). Nothing reads it yet.
   *Test:* the verdict survives a resume landing between the pool finishing and the
   completion item; **and** two concurrent drains of one board definition each report
   their own termination reason. *Depends on:* 2.
...
- **The carried verdict against cross-drain bleed:** two concurrent drains of the *same
  board definition*, one exiting for review and one not, each report their own reason. A
  verdict parked on the board's shared construction-scoped bag passes every single-drain
...
- **At the approval gate** — corrected *which* carrier holds that verdict. The spec named
  the board's flow-state bag as "run-scoped"; it is construction-scoped, allocated once per
  board definition and shared by every concurrent drain of it, so one drain's teardown
  would clear another's verdict mid-run. The carrier is now the drain's own
  invocation-scoped state, and step 3 gained a concurrent-drain test. The principle in §7
  — carry the verdict from where it is decided — is unchanged, as is §9's ladder and both
