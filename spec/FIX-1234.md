...
That scenario is unreachable while parking pins the request open.

**A second reason was claimed here, and it is false. It is withdrawn.** This spec asserted
that a board declaring `dispatch: { mode: "detached" }` has its launching request
*recaptured* when the background workstream parks its task — the row leaves `in_progress`,
so the hand-off exclusion stops applying. A POC built that path for real: a durable board
with a detached worker, started through the same inbound-transport seam production uses,
with the child's action genuinely claiming the row and parking it from inside its own
execution. With the park verified committed to the store *before* the parent's call
resolved, the parent still returned cleanly — no error, reason `handed-off`, four runs
identical. The recapture does not happen.

It is not a race that went our way. The exclusion requires the row to be `in_progress`, and
the launching request's collection mirror is hydrated eagerly and cannot observe a write
made by a separate request running concurrently — eager is the only mode the durable
collection exposes. The parent answers from its own last-known state for the life of its
request and never sees the park. Verdict, mechanism and citations: §12.

**What that costs the case, precisely.** The recapture defect is not a reason to build
this, and a detached board is not an audience for it. Nothing else here moves: the problem
above is untouched, rests on a different and undisputed mechanism, and is what justifies
the feature on its own.

**Epic membership rests on nothing else now.** The recapture defect was the stated reason
this issue "rides in the Relay epic even though it needs nothing from Relay's messaging
core." That reason is gone. What remains is the headline scenario in the first paragraph.
Whether that alone warrants membership, or whether the issue should move, is the epic
owner's call — flagged here, not decided either way.
...
1. **The mode is opt-in and off by default, and a detached board has to ask for it too.**
   *Rejected:* treating a parked row on a detached board as automatically released, on the
   argument that the board already said that work runs elsewhere. It silently changes
   *when* an existing caller's request resolves, for every detached board already running.
   > **The rule is unchanged; two facts under it were refuted.** This decision previously
   > argued the rejected alternative was "free, and the case we most want fixed", and that
   > declining it left "the recapture bug live for anyone who doesn't opt in". There is no
   > recapture bug — §1 and §12. So the alternative would now buy nothing on the path it was
   > argued for, and nothing is left live by rejecting it. The decision stands on its
   > remaining reason, which was always the load-bearing one: turning it on by default moves
   > an existing caller's resolution point without their asking.
   *Locks in:* we can't flip the default later without moving that resolution point for
   callers who never asked. Reversible only by accepting that change once, deliberately.
...
> **Relay integration — an earlier version of this callout was wrong, and this replaces
> it.** It told anyone wiring a detached board that park-exit was a required *second
> switch*, because a park would otherwise recapture the launching request. **That is not
> what happens.** A POC on the real detached path refuted it (§1, §12): the launching
> request returns cleanly across a park, reporting `handed-off`, and does so structurally
> rather than by luck — its collection mirror cannot see the child's write at all.
>
> **So if your board hands work to a background workstream, durable storage is the switch,
> and park-exit is not required for the request to end.**
>
> ```ts
> const board = taskBoard({
>   name: "asks",
>   collection: durableLedger,                          // durable storage — necessary
>   workers: { ask: { worker, dispatch: { mode: "detached" } } },
>   // park-exit is NOT required here — the detached hand-off already
>   // releases the launching request, park or no park (§12).
> });
> ```
>
> **Where park-exit is the answer** is the board that drains its work *inline*, in the
> launching request. There a parked row genuinely does hold that request open until the
> drain's safety cap trips — the problem in §1, which this refutation does not touch. If a
> Relay issue parks on an inline board, park-exit is what lets its request end, and the
> requirements in §6 decision 3 apply to it.
>
> A board that turns park-exit on must also be on the default termination rule and carry
> explicit ids on its `initialTasks` — but neither can bite you silently, because a board
> that breaks either is refused at construction (§6 decision 3).
...
1. **Add the board option and refuse the unsupported combinations** (decision 3).
   ...
   **Each message carries its own diagnosis, not just a rule.** The `onIdle` refusals are
   the ones a caller is most likely to meet with a coherent intent, so they say *why*:
   under `"complete"`, "a parked task's dependent still holds the drain open, so park-exit
   would work until that task gained a dependent" — plus the fix, return to the default
   `onIdle`; under `"wait"`, that the mode never consults the counts the option modifies, so
   it would be a total no-op. This is the whole reason decision 3 refuses rather than
   documents: a caller who wanted the combination learns what is actually in the way, at
   construction, instead of finding a board that half-works.
...
- **[user-facing prose] EXTEND** `apps/docs/docs/server/background-work.md` — **the
  content of this item changed when the detached premise was refuted; read §5 before
  writing it.** An earlier version had this page carry a "two-switch requirement" telling
  detached adopters that a park recaptures the launching request unless park-exit is on.
  That is false (§1, §12) and must not be published. What this page should say instead is
  the true and still-useful fact: a detached hand-off releases the launching request and
  **keeps** releasing it across a park, so a board handing work to a background workstream
  does not need park-exit for its request to end. Park-exit is for boards that drain
  inline. This page is where someone wiring detached dispatch lands, which is why the
  clarification belongs here rather than only on the task-board page. Cross-link → the new
  task-board subsection.
...
- **Settled:** the existing `awaitReview` / `resumeFromReview` pair already covers this
  gap — **REFUTED, for an inline board.** Two scenarios ran. Park-and-never-resume: the
  drain returns only when its iteration budget silently exhausts, and the task is abandoned
  at `awaiting_review`. Park-checkpoint-resume: the drain had *not* settled before the
  checkpoint, and moving the resume ahead of the checkpoint flips that assertion — so the
  check discriminates real timing rather than restating a fixed result. Mechanism:
  `awaiting_review` is a non-terminal status excluded from the board-exit path in both
  counting `onIdle` modes. Corroborated by the committed, passing
  `packages/orchestration/test/task-board/durable-board-freshness.test.ts`.
  **Scope, corrected:** this POC exercised an **inline** board driven by a sibling actor.
  It never used detached dispatch, and an earlier version of this entry wrongly extended
  its mechanism claim to "and on detached boards". The detached path is covered by the
  separate entry below, which reaches the opposite conclusion. This entry stands for the
  inline case, which is what the feature is justified by.
- **Settled:** a park on a **detached** board recaptures the launching request —
  **REFUTED.** The POC built the real path rather than a stand-in: a durable, user-scoped
  `defineTaskCollection` board with a `dispatch: { mode: "detached" }` worker, dispatched
  through `createDetachedStartOperation` wired to a genuine `createInboundTransportHost` —
  the same seam `FlowState#buildDetachedStartOperation` uses in production. The child's
  `runAction` really ran, claimed the row, and called `awaitReview()` on its own task from
  inside its execution. The interleaving was driven *more* favourably to the claim than
  either existing test: the park was verified store-committed — read straight off
  `stores.resourceState`, bypassing every mirror — strictly **before** the parent's
  `runAction` resolved. The parent returned cleanly anyway: `error === undefined`,
  `terminationReason: "handed-off"`, `counts.in_progress: 1`. Four runs, identical.
  **Mechanism, confirmed in code rather than inferred:** `shared.ts`'s
  `countWaitable` / `isHandedOff` require `task.status === "in_progress"` for the detached
  exclusion to apply, and `resource-backed.ts:57-60` documents that an eagerly-hydrated
  mirror cannot observe a write from a concurrently running separate request — and eager is
  the **only** mode `defineTaskCollection` exposes; there is no `prefetchMode` option. The
  parent's classifier therefore answers from its own last-known state for the entire life
  of its request, structurally blind to the child. Deterministic given the only reachable
  configuration, not a race that went one way.
  **What it does not touch:** the inline case above, and with it §6, §9 and §10. Only the
  detached claim falls.
...
- ~~**The recapture defect stays live for detached boards that don't opt in.**~~
  **Withdrawn — there is no recapture defect.** Refuted on the real detached path (see the
  settled claim above). Nothing is left live here, and there is no default change waiting
...
- **At the approval gate** — the spec's own secondary premise was **refuted by POC and
  swept out.** It claimed that a detached board's launching request is *recaptured* when the
  background workstream parks its task. Built on the real detached path — a durable board,
  a real inbound-transport host, the child's action genuinely parking its own task, with the
  park verified store-committed before the parent resolved — the parent returned cleanly
  every time, reporting `handed-off`. The mechanism is structural: the exclusion needs the
  row `in_progress`, and the parent's eagerly-hydrated mirror cannot see a concurrent
  request's write, eager being the only mode the durable collection offers. Swept in one
  pass across every site that asserted it: §1's "Why now" (rewritten to the true claim, with
  what was checked), §5's Relay callout (which told detached adopters to turn park-exit on
  for a reason that does not apply to them), §12's settled claims (the original entry now
  records that it exercised an *inline* board and never touched detached dispatch; the new
  verdict is its own entry), §11's `background-work.md` deliverable (which would have
  published the false claim), §12's follow-up, and the two now-false facts inside decision
  1's rationale. **The design did not move.** §6's rules, §9's ladder and §10 stand: they
  rest on the inline mechanism, which is undisputed and is what justifies the feature.
  Epic membership lost its stated reason and is flagged for the epic owner, not decided
  here.
