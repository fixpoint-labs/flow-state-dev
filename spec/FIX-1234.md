...
decided**, in the exit check, onto the run-scoped state the drain already installs and
tears down around the pool, and the completion item reads that. The `counts` stay a final
snapshot: they are descriptive, and a caller reading them wants the board's actual last
state.
...
3. **Carry the verdict out of the exit decision.** Record, where the drain decides to
   stop, that rows were excused as parked — on the run-scoped state the drain already
   installs before the pool and tears down after the completion item. Nothing reads it yet.
   *Test:* the verdict survives a resume landing between the pool finishing and the
   completion item. *Depends on:* 2.
