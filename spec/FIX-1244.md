...
**One bound worth knowing before you sign.** If the write commits and the change
announcement then fails, the call rejects and the drain does not run. The answer is
safely recorded, but re-delivering it is *refused* — the task is already queued — so the
work waits for the next thing that drains the board. Rare, not silent, and no answer is
lost or overwritten. Closing it needs the correlation token §12 defers to LAB-139.
...
> renames it later.
>
> **The verb renames here: `resumeFromReview` → `unpark`, with no deprecated alias.** The code
> symbol moves now; the persisted vocabulary does not — `awaitReview`, the enum member, and the
> `task-change` kind `"resumed"` still say review until FIX-1245, and two of those are stored
> values needing its dual-read. Renaming the verb this change already narrows costs each symbol
> one churn instead of two. §9's hand-written-collection row owns what the rename breaks, and
> why an alias could not have softened it.
...
**Cost of being wrong: low and visible, with two caveats.** A board stops re-queuing on
...
scopes it to them. Second, a reliability caveat, not a correctness one: on a durable
board a request whose basis predates the park can refuse a genuinely parked task (§7).
Re-delivery succeeds, so it costs a round trip; nothing is written wrongly.
...
**Non-goals:** the *persisted* rename — enum member, `awaitReview`, and the `task-change`
kind (FIX-1245); the *question* layer (FIX-1230); a second pause
model (FIX-274, FIX-765); structured payloads and answer revision (§12); who calls the
verb, and the Conductor flow.

**Size:** 4 production files, plus a rename across 8 more (§8) · 14 test behaviours (§10) ·
6 documentation surfaces (§11) · 1 PR.
...
flowchart TD
  ...
  B --> F["collection.unpark<br/>(shipped verb — renamed, now fenced)"]
...
- **`@flow-state-dev/orchestration` → `tasks/collection`.** `resumeFromReview` is renamed
  to `unpark` and narrowed to the one edge it owns, using the mechanism `unblock` already uses for
  ...
  name its shape is **unchanged** — same prose argument, same return type. Both backings;
  ...
  **And "both backings" is the honest bound on the guarantee, not a shorthand for
  ...
  boundary. A caller's own `unpark` therefore keeps whatever behaviour it has
...
**Why the board capability gains no `unpark` accessor, though an earlier draft had one.**
It would have been sugar over the substrate's own `unpark`, reached through
`ctx.cap.<board>.tasks()` — two lines a caller can already write — and it would have hung
that one name off two altitudes before anything needed it at both. The substrate verb still exists and is still
...
still throw; `unpark` then rejects, the composed step never drains, and an
...
its own.** The token rides `TaskTransitionOptions.write`, which `unpark`
...
overwritten — **on a store whose CAS is honoured where the contention is.** That is
every adapter that ships: in-memory is single-process by construction, and
`store-sqlite` and `store-postgres` version-check in the database. A store adapter
that accepted a write without checking the version would break this the way a
hand-written collection breaks the fence, and for the same reason: the guarantee is
the substrate's to make and the adapter's to keep.
...
1. **Fence the verb to its one edge**, per decision 1 and §6's rule, in the shared
   decline ladder so both backings inherit it from one place. **Rename it to `unpark` in the
   same step**; nothing else about its shape moves. The rename reaches further
   than the fence does: 8 production `.ts` files on `main`, most of the hits doc comments in
   files the fence never touches (`task-caps.ts`, `schema/task.ts`), and 46 mentions once tests
   are counted. None is a model-visible tool name. §11 owns the doc surfaces separately.
   Re-grep both; the counts here are a shape, not a checklist.
   *Test:* §9's first two rows — a task a worker is running, and a task already
...
| A hand-written collection implementation | **Breaks at compile time on the rename, and needs nothing else.** The verb is a required `TaskCollectionRef` method, so an implementer renames their method to `unpark` — that is the whole migration. **A deprecated alias would not have spared them:** an optional old name cannot satisfy a required new one, so the break lands either way, and the alias would put a second resume-shaped name back on the interface this change exists to clear. Pre-1.0 — a minor, a changeset, one migration line. Behaviour is untouched: `requireFrom` is internal to the built-in backings and never crosses the `TaskCollectionRef` boundary (§7), so a custom `unpark` inherits no fence. Do not write migration handling or tests for a surface this spec does not extend |
...
- **EXTEND** `apps/docs/docs/orchestration/task-board.md` — rewrite the *"Picking it
  ...
  short table, **the one case where the answer lands but the work does not start** (the
  post-commit announcement failure — say plainly that a re-delivery is refused and the
  work waits for the next drain), **both** throws a caller can hit — a task id that resolves to nothing
  (input-dependent, and the shape a wrong ledger takes), and this step on a non-resource
  board (new with this change, per §9) — and what an answer may carry. **One answer path on this page, and it is the
  ...
  accepted and nothing runs it. **Do not present `unpark` here as a second
...
- **EXTEND** `apps/docs/docs/orchestration/task-substrate.md` — its lifecycle section
  ...
  *"needs no guards to be advisory"*. Add `unpark` to that exception, next to
...
- **EXTEND** `docs/architecture/detached-work.md` — the park-exclusion table's
  ...
  published.
- **SWEEP the old verb name** out of every published surface that spells it —
  `task-board.md`, `task-substrate.md`, `packages/orchestration/README.md`,
  `packages/patterns/README.md`, and the internal `docs/atlas/conductor.html`
  (19 doc-surface mentions on `main` at time of writing — a different set from §8's
  production-file count; re-grep rather than trusting either). Mechanical, but it is the
  half of a rename that gets left standing.
...
- **Changeset:** `minor` (BP-022) — decision 1 is a behaviour change to a shipped verb and
  the same release renames it. The note carries both plainly, and the migration line
  (*rename your `resumeFromReview` to `unpark`*) is the only action a caller has to take.
...
- **FIX-1245** renames the *persisted* vocabulary to the product word: the enum member,
  `awaitReview`, and the `task-change` kind `"resumed"`. Two of those are stored values,
  so it owns the dual-read; this change owns only the code symbol it already narrows
  (§6's rule). The line where the two meet is §6's fence constant. Landing 1245 first and
  rebasing would work too — it is sequencing hygiene, not a correctness dependency.
...
- **After spec review** — renamed the verb (`resumeFromReview` → `unpark`) inside this
  change rather than leaving it to FIX-1245. The pivot is which symbol churns twice: a
  verb whose behaviour changes here and whose name changes later touches callers in two
  releases. FIX-1245 keeps the vocabulary that is *stored*, where the dual-read lives.
