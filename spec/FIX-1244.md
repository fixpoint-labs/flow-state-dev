...
> **Settled by the owner, 2026-08-28** —
> [#1461 (comment)](https://github.com/fixpoint-labs/flow-state-dev/pull/1461#issuecomment-5447345109):
> *"narrow the shipped re-queue verb. Don't add a second resume verb next to it."* Decision 2 is
> ratified in the same comment (*"the answering request is the new request. The substrate does
> not mint one."*), and it carries one leftover: the fence's from-status stays shipped
> `awaiting_review` until FIX-1245, and `needs_input` does **not** join the enum.
>
> Recorded here as the rule the implementer follows; the reasoning below is kept because it is
> what the cost lines and §9's rows derive from.
...
15 test behaviours (§10) · 6 documentation surfaces (§11) · 1 PR.
...
- **The invocation guard is not only about backing.** Resource backing is necessary and not
  sufficient. `assertParkExitSupported` returns at `onReview !== "exit"` before it reaches the
  id-less-seed check (`park-exit.ts`), so a **default `onReview: "hold"` board is never checked**
  — yet it can hold parked rows, and it can be resource-backed, so nothing in the backing test
  refuses it. Drain it a second time with an id-less `initialTask` and the seed step adds that
  task again: duplicate work, and duplicate side effects with it. **`unparkAndDrain` must apply
  the same stable-seed check the board already exposes (`hasIdlessInitialTasks`) when it runs**,
  under the same error class, and not merely require resource backing.
  **Why that rather than requiring `onReview: "exit"`:** the hazard is the seed, not the mode.
  Demanding `exit` would refuse a hold board with stable ids, which is safe, while still
  admitting nothing the seed check does not already cover. Reuse the flag; do not invent a
...
*derived*, not any single id prose can name. **The authority is the effective resource-scope
resolver, not `scope-keys.ts` alone** — that file answers only the last step. `scope-keys.ts`
gives a session key as bare `sessionId` when no tenant is present and `${tenantId}:${sessionId}`
when one is, and namespaces org keys on the flow kind under `isolateOrgState`. But a
session-scoped collection declared `sharedToWorkstream: true` never reaches that key at all:
`createExecutionContext` resolves it to the minted **lineage id** instead. A parent and every
descendant then share one ledger across *different* session ids, and a session deleted and
recreated under the *same* id gets a different one. Point the rule and the docs at the
resolver that actually runs. **State it as "resolves the same key, per the resolver", never as a
...
2. **`unparkAndDrain` on the handle**, per decision 2 — the internal re-queue step
   ...
   step returns on both branches. **Two invocation checks, not one** (§7): a non-resource backing, and a board carrying id-less
   `initialTasks` — the latter because `assertParkExitSupported` never reaches its own id-less
   check on a `hold` board. Both **throw when the
...
| Board is resource-backed but carries an id-less `initialTask` (any `onReview`, and unchecked on the default `hold`) | **Throws when the step runs**, under the same class as the backing refusal. Not a `declined`: like the backing, it is a property of the board, not of a row. Without it the second drain's seed re-adds the task and the answered work runs twice (§7) |
...
- **the id-less-seed refusal**: a resource-backed board on the default `onReview: "hold"` with
  an id-less `initialTask` must throw when the step runs, not drain. The board-level guard does
  not cover this one, so a test that only exercises an `exit` board proves nothing about it
...
- **EXTEND** `apps/docs/docs/orchestration/task-board.md` — rewrite the *"Picking it
  ...
  refused, that the answer is safe, and that what the caller wants is another drain), **the throws a caller can hit** — a task id that resolves to nothing (input-dependent), and
  two board-shape refusals new with this change (per §9): a non-resource board, and one whose
  `initialTasks` carry no stable id — and what an
  answer may carry. **The ledger is what the resolver returns, and `sharedToWorkstream` makes that a lineage id,
  not a session key** (§7) — the page must not teach "same session" as the rule.
  **The warning must also say what a wrong key actually costs**, not
...
- **After spec review** — the invocation guard grew a second check. Resource backing was
  treated as sufficient, but `assertParkExitSupported` returns at `onReview !== "exit"` before
  its id-less-seed check, so a default `hold` board is never tested for stable seed ids and a
  second drain re-adds an id-less initial task. The step applies that check itself.
- **After spec review** — the ledger rule now points at the effective resource-scope resolver
  rather than `scope-keys.ts`. A `sharedToWorkstream` session board resolves to a lineage id,
  so a parent and its descendants share one ledger across different session ids. Same defect
  class as the id lists this rule replaced: naming a part of the derivation as the whole.
