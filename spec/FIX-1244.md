...
**Size:** 4 production files (§8) · 14 test behaviours (§10) · 4 documentation surfaces
(§11) · 1 PR.
...
| A hand-written collection implementation | **Inherits neither change, and needs no migration because there is nothing to migrate to.** The signature is unchanged (the structured payload was withdrawn), and `requireFrom` is internal to the built-in backings — it never crosses the `TaskCollectionRef` boundary (§7), so a custom `resumeFromReview` gets no fence either. Do not write migration handling or tests for a surface this spec does not extend |
...
- **EXTEND** `apps/docs/docs/orchestration/task-substrate.md` — its lifecycle section
  ...
  *"needs no guards to be advisory"*. Add `resumeFromReview` to that exception, next to
...
- "Decision 3's structured bag doesn't specify where `data` lands on the task. … Prefer merging answer `data` into `metadata` inside the same atomic write rather than a new top-level field." — cursor[bot] ([thread](https://github.com/fixpoint-labs/flow-state-dev/pull/1461)) · *Declined; the field placement is unsafe and §12's trap bullet owns why.*
- "Have the internal requeue step retain the token, resolve the three-valued verdict, and continue to the drain when the write landed." — chatgpt-codex-connector[bot] ([thread](https://github.com/fixpoint-labs/flow-state-dev/pull/1461)) · *Declined; §7 owns the recovery contract and why the composed step needs none.*
- "For an object input, `{ taskId, answer }` and `{ id, feedback }` are incompatible contracts… Lock the input schema and exact keys in the spec." — chatgpt-codex-connector[bot] ([thread](https://github.com/fixpoint-labs/flow-state-dev/pull/1461)) · *Declined on BP-004; §7 fixes the shape, the names are yours — pick once and make §5, §10 and §11 match.*
...
discarding one (§6's decisions bind you; these don't).
