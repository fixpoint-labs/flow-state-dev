...
answer already has a carrier, and it is **not** the board meta: `task-board-meta` is
keyed by `collectionId` and carries no task id, so with sibling rows, a concurrent
drain, or detached dispatch it cannot establish that *this* task ran. The task's own
`task-change` items can — they carry `taskId` and the settled row. The table above
names both, and which question each answers. The name is deliberate on the same grounds —
...
2. **`unparkAndDrain` on the handle**, per decision 2 — the internal re-queue step
   ...
   from §7 (`recorded`, and nothing else). A non-resource backing **throws when the
