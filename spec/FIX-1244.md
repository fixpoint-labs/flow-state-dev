...
**Three bounds worth knowing before you sign.** This change closes none of them; it
names them.

1. **The answer lands, the work waits.** If the write commits and the change announcement
   then fails, the call rejects and the drain does not run. The answer is recorded, but
   re-delivering it is *refused* — the task is already queued — so the work waits for the
   next drain. Rare, not silent, nothing lost.
...
- **EXTEND** `apps/docs/docs/orchestration/task-board.md` — rewrite the *"Picking it
  ...
  work waits for the next drain), **both** throws a caller can hit — a task id that resolves to nothing (input-dependent),
