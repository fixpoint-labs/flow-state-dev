...
into a returned `declined`, so a `catch`-based caller stops catching. **Narrower than
it first looks:** a caller already passing `{ ifAllowed: true }` gets a `terminal`
decline today — that reason is checked before transition legality — so only callers
*omitting* the flag change at all. The changeset scopes it to them. **One reliability caveat too, not a correctness
...
- **EXTEND** `apps/docs/docs/orchestration/task-board.md` — rewrite the *"Picking it
  ...
  short table, **both** throws a caller can hit — a task id that resolves to nothing
  (input-dependent, and the shape a wrong ledger takes), and this step on a non-resource
  board (new with this change, per §9) — and what an answer may carry. **Lead with the composed step**, as §5
