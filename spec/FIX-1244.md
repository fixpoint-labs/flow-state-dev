...
> and renames the verb to `unpark` in one pass (§12). Do not write the guard against a member
...
- **FIX-1245 is now this spec's one blocking dependency, by decision, not by mechanism.**
  ...
  per BP-030) in its own PR; this one rebases onto that vocabulary and only then renames the
  verb `resumeFromReview` → `unpark` alongside its behaviour change. Both orders were tried in
