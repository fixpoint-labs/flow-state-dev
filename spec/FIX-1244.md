...
**Size:** 4 production files (§8) · 14 test behaviours (§10) · 3 documentation surfaces
(§11) · 1 PR.
...
- **After spec review** — withdrew the refreshed-read requirement and narrowed the
  ...
  risk.
- **After spec review** — pinned the invocation-only refusal with a test, because the
  choice to refuse an unsupported backing when the step runs rather than when the board
  is built had nothing asserting it: a guard moved back to construction would have
  passed every planned test while breaking every non-resource board. Also corrected the
  docs plan, which named the unsupported backing as the only throw a caller can hit when
  a missing task throws too and is the input-dependent one, and §7's stale
  cross-reference saying §11 removes the same-ledger warning when §11 keeps it.
