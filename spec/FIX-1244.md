...
**Cost of being wrong: low and visible, except in one case.** Such a board stops
re-queuing on upgrade and says why. Pre-1.0: a minor version and a changeset. The
exception runs the other way — answering a **terminal** task throws today, and the
forced advisory flag turns that throw into a returned `declined`, so a `catch`-based
caller stops catching and walks its success path. One case gets *quieter*, not louder.
The changeset has to say so.

#### 2. The verb cannot start a new request — so what ships instead?

**Plain terms.** Nothing lets running work launch a fresh action, a door closed on
purpose, so the answer *arrives* on a request — an operator's call, a scheduled wake, a
chat turn — and that is the new one.

**The trade-off.** Somebody has to be calling something when the answer lands; an answer
left in a mailbox starts no work.

**My recommendation: reframe the ticket.** Opening a runtime seam is a far larger
decision.

**What would change my mind:** answers that arrive with nobody there — a
scheduled wake calling this step, deployment rather than substrate.

**Cost of being wrong: low.** The mailbox case stays additive.

*(A third decision, a structured answer payload, was **withdrawn in review**: decision 1
queues the task on the first answer, so the merge it promised was unreachable.
§12 carries it.)*
...
- **`@flow-state-dev/orchestration` → `tasks/collection`.** `resumeFromReview` is
  ...
  **And "both backings" is the honest bound on the guarantee, not a shorthand for
  ...
  today — including re-queuing a running task and reporting `recorded` — and the
  composed step calling it inherits that. **This is scoped, not fixed:** the parked-only
...
the answering request needs the same **principal**, not the same session. The
session-scoped case is the strict one — and it is the shipped integration scenario's
(`scope: "session"`) — where the later request must run in the session that holds the
parked task. Telling a user-scoped caller to preserve a session id teaches a
superstition. Get the scope identity wrong and nothing complains:
...
| The answering request resolves a different ledger than the parked task's — a different session on a session-scoped board, a different principal at user or org scope (§7) | The task reads as missing, so the write **throws** and nothing drains. **Not** a silent no-op drain — that is the failure mode this replaces |
...
- **EXTEND** `apps/docs/docs/orchestration/task-board.md` — rewrite the *"Picking it
  ...
  **Keep one thing the old section got right:** the answering request has to resolve
  the *same ledger*. State it as scope identity per §7 — same principal at user or org
  scope, same **session** for a session-scoped board, which is what the page's own
  example declares. The old text explained this around a hand-rolled `runAction`; the
  requirement survives that example's removal, because it was never about the number
  of calls.
...
- **After spec review (round 9 — four accuracy defects, all verified against shipped
  code, all folded)** — the carrier table credited `task-board-meta` with showing
  whether the resumed work ran; it is keyed by `collectionId` and carries no task id,
  so it can prove nothing about one row. The task's own `task-change` items can, and
  the table now names both carriers and the question each answers. Round 8's own
  `drained` paragraph repeated the same mistake and is corrected with it. §10's
  concurrency test could not fail against the implementation it exists to rule out: a
  change landing after the *caller's* read is still visible to an internal `get()`
  pre-check, so the barrier now has to land inside the method's own read-to-write
  window. "Resource-backed boards only" was refusing at construction, which would break
  every request- and sequencer-backed board for a step they never call — park-exit's
  guard returns early without `onReview: "exit"`, so the refusal moves to invocation.
  And the composed step's input contract was never written down; it is now, shape
  fixed and names left to the implementer. No decision moved.
- **After spec review (round 10 — one folded, one declined)** — the parked-only fence
  was written as though it held everywhere. It holds on the two built-in backings:
  `requireFrom` is internal to them and is not a field on `TaskTransitionOptions`, so a
  hand-written `TaskCollectionRef` — a shape BP-030 already plans around — carries the
  fence nowhere and keeps today's behaviour. §7 now scopes the guarantee explicitly and
  §12 carries the public enforcement seam that would widen it. A safety claim that
  quietly fails at a supported extension point is precisely this epic's subject. The
  request to lock exact input key names was declined and recorded in §13: BP-004 settles
  the boundary before the naming, and names are on this PR's out-of-scope list.
- **After spec review (round 11 — three folded, two of them self-inflicted)** — round
  10 moved the non-resource refusal to invocation and left no representation for it:
  §9 still described a construction-time refusal, and `declined` requires a concrete
  `TaskStatus` while "this backing cannot park" is a property of the board. Resolved
  the way round 6 resolved the missing task — by taking a throw that already ships,
  here the taxonomy's own *malformed board declaration* class, so the "this spec adds
  no throws" claim stays true. Round 9 corrected the carrier table but left §8's goal
  check asserting on the board's terminal report, which §7 had just said proves nothing
  about one row; the check now asserts on the resumed row itself. And §10's "every
  non-parked state" omitted `blocked`, the one status an implementation can miss while
  everything else stays green, because `blocked → pending` is a legal transition and a
  legality-based fence would re-queue it.
- **After spec review (round 12 — two subtractions, no new content)** — the composed
  step no longer reports `drained`. On every successful return it equalled
  `outcome === "recorded"`, because the step drains on `recorded` and on nothing else
  and a drain that throws returns nothing at all, so it was a public wrapper schema, an
  assembly step, docs and tests all carrying no information a caller didn't already
  hold. The step now returns the shipped `TaskWriteOutcome` unwrapped, and the drain
  becomes a conditional **tap** rather than a `.stepIf`, which is what makes the write
  outcome survive both branches. Round 8's paragraph arguing the assembly was
  load-bearing goes with it — there is nothing left to assemble — and the honest half
  of round 9's correction stays: a drain having run is not proof this task ran, and the
  task-specific carrier is `task-change`, not the board-wide `task-board-meta`.
  Separately, Part I was cut from ~2,300 words to under 550. It had accreted across
  eleven review rounds past the ~400-word budget that exists to keep the decisions
  legible, and the fence, the retry rule and the correlation token were each explained
  three or four times over. Part II already carries all three properly, so Part I now
  states each once. §4's focus practices and two of the three examples went entirely;
  the two decisions kept their full six-part shape, which is the one thing the budget
  protects. LAB-139's evidence for Part I's case moved to §12, where the rest of that
  PR's coordination already lives.
- **After spec review (round 13 — four folded; two were claims about shipped code, two
  were the document contradicting itself)** — §1 said the shipped call re-queues *any*
  task and reports success. Not true of a terminal row: `ALLOWED_TRANSITIONS` leaves
  `completed`, `errored` and `cancelled` with no outgoing edge, so that call throws
  today. Forcing the advisory flag converts the throw into a returned `declined`, which
  makes exactly one path **quieter** on upgrade — a `catch`-based caller stops catching.
  That is a cost decision 1 was not pricing, and it now is. The same-session rule was
  categorical and is only true at session scope; `defineTaskCollection` takes
  `session | user | org` and says a user/org ledger already spans every session the
  principal touches, so the constraint is **scope identity**, of which the strict
  session form is one case. §10's Goal still told the implementer to prove completion
  from the board's report, contradicting §7 and the fix round 11 made to §8. And §11
  said to delete the session warning and, four lines later, to keep it.
