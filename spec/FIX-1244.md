...
1. **Tenet 5 — the guard lives inside the atomic write**, never as a pre-check. This is
   the change; everything else is plumbing, and §10's fence test is written to fail
   against a pre-check specifically.
2. **BP-035 (second path)** — the *refused* path is the second path, and here it is the
   whole feature. A suite that only proves the happy answer proves nothing.
3. **BP-030 (tolerate the old shape)** — the prose argument keeps working unchanged and
   hand-written collections need no migration, which is also why the fence stops at the
   two built-in backings (§7).
4. **BP-031 (never route on caller input)** — the answer is inert prose. Shipping no
   structured payload is what makes that true rather than aspirational; §12 carries the
   trap to close before any payload is added.
...
the answering request has to resolve the same **scope instance**, and which id that
is depends on the scope: `sessionId` at session scope, `userId` at user scope, `orgId`
at org scope (`core/src/types/resource-collection.ts`). The org case is the one worth
saying out loud, because "same principal" would get it wrong: **a different person in
the same org resolves the same ledger**, so one colleague can answer a task another's
worker parked. Session scope is the strict case, and it is the shipped integration
scenario's (`scope: "session"`). Get the scope instance wrong and nothing complains:
...
| The answering request resolves a different ledger than the parked task's — a different `sessionId`, `userId` or `orgId`, whichever the board's scope keys on (§7). **Not** a different person at org scope: that resolves the *same* ledger and is a supported answer | The task reads as missing, so the write **throws** and nothing drains. **Not** a silent no-op drain — that is the failure mode this replaces |
...
- **EXTEND** `apps/docs/docs/orchestration/task-board.md` — rewrite the *"Picking it
  ...
  **Keep one thing the old section got right:** the answering request has to resolve
  the *same ledger*. State it per §7 as the scope's own id — `sessionId`, `userId` or
  `orgId` — not as "same session" and not as "same person": an org-scoped board is
  answerable by anyone in the org. The page's own example is session-scoped, which is
  the strict case. The old text explained this around a hand-rolled `runAction`; the
...
- **After spec review** — restored §4 and the size count, which the budget cut had
  ...
  authority to drop them. Part I now runs over its ~400-word budget at ~830, and that
  is a real conflict rather than an oversight — two six-part asks plus the template's
  mandated sections do not fit under the ceiling, and the budget doc names the ask as
