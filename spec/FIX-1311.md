...
accumulates. Reach for one when the exchange is back-and-forth and wants a durable home:
a one-shot instruction is a dispatch into the target flow's own session, and work that
needs a claim and a settlement is a task board row. A team that wants different behaviour
...
- **CREATE** `apps/docs/docs/orchestration/channels.md`, in the `Orchestration` category
  ...
  **when a channel, and when something else** (the callout below) · declaring one and who
  its members are · opening it before anyone posts · posting and reading · **what the
  transcript proves and what it doesn't** · **where posting from another flow works and
  where it doesn't** · replacing the kind · what waking members costs today. One minimal
  example: the standup channel from §5, minted and posted into. Cross-links: ← from
  ...
  case a channel is not. The callout is the single place that teaches the fork: the link
  carries the pointer, the callout carries the reasoning, and no other page restates it.
  ...
  - **The callout — "When a channel, and when something else."** Short, and placed between
    ...
    5. **Do not put seat-owned work on a channel.** A post runs on the channel copy, and
       fan-out wakes members; neither one hands anybody a claim.
