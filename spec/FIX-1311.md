...
1. **A `CHANNEL.md` need not say which kind it runs. An omitted `flow:` selects the
   ...
   *Rejected:* requiring `flow: channel` on every file, mirroring `WORKER.md` exactly.
   ...
   answer. **It stays rejected while there is only one built-in:** with one kind in the box
   the key has nothing to disambiguate. *Also rejected, and also stamped:* teaching
   `hireWorkforce` to own this. A thin channel binder beside it is right while FIX-1361 and
   FIX-1367 are editing `hire.ts`.
   *Locks in:* **W3 ships exactly one opinionated built-in kind, not a menu.** Standup, a DM
   and an announce channel are not additional kinds; they are channels on this one,
   differentiated by members, charter and session state. A custom kind is the rare escape
   hatch (§5), not the common path. The default is a promise we keep. Apps written against
   it omit the key, so we can add built-in kinds but can never remove the default without
   breaking every one of them. Reversible in the safe direction only: requiring the key
   later is breaking, relaxing it later would have been free. **What would reopen it:**
   shipping a second built-in channel kind, where "which default?" becomes ambiguous (§12).
...
**Binding channel sessions (built-in is seeded)** *(what this spec proposes; today neither
function exists and the records have nowhere to go)*. An app does not register ChannelFlow
in order to use channels: the binder seeds the one built-in into the `kinds` map. What is
left is `channelInstances` and then `openChannels`, separate because they happen at
different times. Instances are registered when the server is built, sessions can only be
opened once it is running.
...
// Build time — the built-in is already in the kinds map. One instance per distinct
// kind, not per record.
...
**Registering a custom kind — the rare escape hatch, not the happy path.** Reach for it
only when the workflow graph genuinely diverges. It is one pass of `kinds` at boot, and the
factory carries the same contract the built-in does: `cardinality: "singleton"`, with
`flow.id === flow.kind`. There is no second registration API.

```ts
// Boot time, once. The kinds map is the registration door.
channelInstances(channels, { kinds: { "my-channel": createMyChannelFlow() } });
// A file carrying `flow: my-channel` runs on that kind's own instance, and every channel
// naming it is a session there. One instance per custom kind, still never one per file.
// A `flow:` naming a kind nobody passed refuses by name, never falling back to the built-in.

// The same map replaces the built-in wholesale, for an app that wants the standard
// behaviour with its own notify block:
...
**One kind in the box, and one registration door.** W3 ships this kind and no menu beside
it: standup, a DM and an announce channel are channels on it, differentiated by members,
charter and session state, not kinds of their own. An app registers nothing to use
channels. A team whose graph genuinely diverges passes their own factory through
`channelInstances`'s `kinds` map once at boot, under the same contract the built-in
carries: `cardinality: "singleton"`, `flow.id === flow.kind`. That map is the whole
registration surface. No new L1 type, and no second registration API beside the factory.
...
- **CREATE** `apps/docs/docs/orchestration/channels.md`, in the `Orchestration` category
  ...
  the built-in is seeded, so an app registers nothing to use channels, and a custom kind is
  one `kinds` pass at boot · what waking members costs today. One minimal example: the standup
  ...
  *Voice risks:* do not write "seamless" or "first-class"; introduce *kind* and *seat* in
  ...
  line and the `author` label is the only thing distinguishing participants. Separately,
  **do not teach kind registration as a routine step.** The built-in is seeded, so the
  page's happy path opens a channel and never registers one; the `kinds` map appears after
  it, marked as the escape hatch.
- **EXTEND** `packages/workforce/README.md` — a `Channels` section beside the seat-factory
  one, and rows in the API and error tables, at the density that file already sets. It leads
  with `openChannels` on a seeded built-in and documents the `kinds` map after it, as the
  escape hatch rather than a setup step.
...
- **A file convention for custom kinds is coming in FIX-1357. It changes nothing here
  (2026-09-12 lock).** Custom flow-kind factories get a home on disk under `workforce/`,
  and the folder is `flows/`, split by what the kind is for. A flat `flows/name.worker.ts`
  suffix scheme was rejected for teach: also scannable, worse to browse and discover.
  Worker and channel custom kinds share this one door, invented once in FIX-1357.
  ...
  **Build this issue exactly as specified.** The rules the lock keeps are the ones already
  here: one scan produces one `{ kinds }` map, documents never define kinds, `flow:` only
  names an already-registered kind, and a hand-passed `{ kinds }` stays valid until the
  scan ships. That last clause is what §5's escape hatch and §7's binder build, so no
  kinds-map work here is superseded. The convention adds a second way to fill the same map
  later; it does not replace this one.
...
- **Registration re-framed as the escape hatch (2026-09-12).** A second owner lock the same
  day, on prose and emphasis rather than shape: W3 ships exactly one opinionated built-in
  kind, the binder seeds it, and standup, a DM and an announce channel are channels on that
  one kind rather than kinds of their own. §5 no longer opens by teaching registration — its
  lead-in is now *"Binding channel sessions (built-in is seeded)"*, and the boot-time `kinds`
  pass moved into its own marked example as the rare escape hatch, keeping both the built-in
  override and the per-channel custom form. §7 and §11 carry the same teach: one seeded
  built-in, the `kinds` map as the whole registration surface, no new L1 type and no second
  registration API beside a `createChannelFlow`-style factory, which carries the built-in's
  `cardinality: "singleton"` / `flow.id === flow.kind` contract. Decision 1's *Rejected* and
  *Locks in* now say the key stays optional **while there is only one built-in**. **This
  sharpens decision 1 and is not a D-n.** No fork moved: an omitted `flow:` still selects the
  built-in, a named-but-unregistered kind still fails loudly, and the identity lock — one
  kind is one instance, one channel is one named session — is untouched. §12 already carried
  the reopen condition (a second built-in), so it was left alone.
- **Custom-kind file convention locked downstream (2026-09-12).** A third owner lock the
  same day gives custom flow-kind factories a home on disk: `workforce/flows/`, split
  `workers/` and `channels/`, with a flat suffix scheme rejected for teach. Home is
  FIX-1357, which invents the one scan both kinds share. **No design in this spec moves.**
  The lock keeps every rule already written here, including that a hand-passed `{ kinds }`
  stays valid until the scan ships, which is exactly what this issue builds. Recorded as a
  §12 pointer so an implementer does not have to wonder; §6 and the build plan are
  untouched and the spec stays approved.
