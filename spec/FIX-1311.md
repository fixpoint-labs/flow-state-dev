...
2. **A transcript line records who the framework can *prove* posted — the principal — and,
   separately, an optional author label the poster supplied, stored and displayed as a
   claim. The framework derives nothing from the claim, and refuses one naming a
   non-member.**
...
3. **Posting into a channel has two preconditions, and both are fenced rather than hidden:
   the channel's session must already exist, and a post from another flow only works where
   dispatch runs in-process. Only the transcript append is queued; waking members runs
   ...
   *Rejected:* auto-creating the session on first post (it would let any caller mint a
   ...
   back in one session per poster, which is the split decision 2's shape exists to avoid.
   ...
   *Locks in:* agent-to-channel posting is a single-process capability for now. On a
   queued deployment (`colocated`, `dispatch-only`, or any custom external dispatcher) a
   flow-to-flow post is refused by name, and only client posts through the ordinary action
   route still work. Apps that need both have to wait on the dispatch substrate, and the
   docs have to say so before someone designs a channel into a queued deployment.
