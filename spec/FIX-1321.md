...
This supplies the identity part of the [approved three-issue objective](https://github.com/fixpoint-labs/flow-state-dev/pull/1617). EM resolved delivery sequencing: FIX-1321 and FIX-1322 land atomically in one implementation PR, with FIX-1322 as integration owner. Addressed delivery/ownership and isolated storage remain FIX-1322/1323 responsibilities; complete floor proof still includes isolation before rollout is claimed safe. This sequencing decision neither approves an individual spec nor closes FIX-1315.
...
3. **Land the registry semantic switch with its live consumer/owner cutover.** FIX-1321 and FIX-1322 retain separate acceptance and goal accountability inside one shared implementation PR owned by FIX-1322. *Rejected:* an independently landed exact-ID switch, quick successive merges, aliases, first-wins fallback or a temporary collection ban. *Locks in:* a larger atomic review/merge unit, not a new child or runtime contract. This is EM's absorbed engineering sequencing decision, not a new owner ask or whole-spec approval.

**Non-goals for this issue's slice:** delivery/envelope and durable ownership changes (FIX-1322 supplies these in the shared PR), isolated storage/migration, Devtool switching, Workforce mint, chat redesign/removal, message boards, new transports.

**Size:** cross-repo core/engine authoring and registry cutover, Conductor and eleven ordinary factories, structural/CLI helpers and affected fixtures; eight behaviour groups; eight substantive documentation surfaces, seven example-repair surfaces and three direct-eval pages checked for compatibility. §8 owns the migration inventory. One issue-owned slice in the shared FIX-1321/FIX-1322 implementation PR.
...
One-argument consumers mechanically remain callable under the new signature, but kind-only persisted reads are **not** made safe by that fact. In particular, legacy `flowKind: "engineer"` must not resolve an unrelated instance whose exact ID is `"engineer"`. Existing action routes pass a selected kind into auth/dispatch, request continuation resolves a stored kind, and CLI discovery mixes registry and kind selection. FIX-1322 owns their full owner-ID, ingress and discovery cutover in the same implementation PR as this registry switch. No independently merged or released registry-only intermediate is permitted; no alias, first-wins fallback or temporary registration restriction hides the migration.

### 8. Implementation sequence

**PR plan:** after individual spec approvals and the later owner-approved cross-spec pass, FIX-1322 integrates both issue slices in one shared implementation PR. The steps below order work within that PR, not separate merges. Retain each issue's acceptance and goal evidence; do not start duplicate implementation workers independently landing FIX-1321 and FIX-1322.
...
4. **Integrate, prove and publish atomically:** FIX-1322 completes transport/owner/discovery migration in the shared PR before its merge or release. Exercise §10 and FIX-1322's real-path ownership/delivery proof; repair affected fixtures and docs examples and publish coordinated public-contract changesets for core/engine and affected public consumers (client metadata note in §13). No changeset in this never-merged spec PR. FIX-1323 stays separate with the required identity/owner prerequisites; its isolation proof remains necessary before claiming the complete floor or safe rollout.
...
Update current first-wins/pair assertions in `packages/engine/test/registry-routes.test.ts`; they defend the removed contract and must not survive beside the new one. Permanent tests earn their place by failing on misselection, invalid admission or lost compatibility, not by asserting map layout or copied fields. FIX-1322's integration owner carries separate FIX-1321 registry/admission and FIX-1322 delivery/owner acceptance evidence in the shared PR, including the live kind-addressed consumer migration. Registry smoke alone is not joint proof. The epic's complete dispatch/ownership/isolation goal still requires FIX-1323; neither this spec nor atomic landing claims safe rollout without it.
...
- **Landing blocker resolved by EM:** the former registry-first merge promise is replaced by §6's atomic FIX-1321/FIX-1322 landing, integrated by FIX-1322. The registry remains an internal build prerequisite, not an independently shippable semantic switch. Individual spec approval and the later cross-spec pass each still need owner approval; no implementation is authorized by this decision fan-down.
...
- **Remaining direction blocker outside this slice:** FIX-1322 still needs the owner's decision if historical custom-ID cross-flow children exist: offline re-key versus legacy adoption must reconcile new child-key derivation with preserving old record IDs. No affected deployment history is established, and atomic landing does not resolve that trade-off. No additional registry-local question remains.
- **Empirical settlements:** none requested. The resolved fork was delivery sequencing, not a repeatedly disputed runtime fact. Storage and legacy ownership remain FIX-1323/1322 responsibilities, not permission for fallback.
...
- **After EM sequencing decision** — replaced the unsafe registry-first landing fork with one shared FIX-1321/FIX-1322 implementation PR, integrated by FIX-1322; separate acceptance and FIX-1323 isolation proof remain, and spec approval is still pending. Direct decision fold: zero automatic review rounds added, aggregate one.
