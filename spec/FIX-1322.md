...
**Non-goals:** registry/cardinality (FIX-1321), storage namespaces (FIX-1323), new delivery modes, message boards, fan-out, Devtool, Workforce minting, chat redesign/removal.

**Planning size:** approximately 65 production files · 12 test behaviours · 30 documentation surfaces · **1 implementation PR**; partial owner enforcement is unsafe.
...
One coherent implementation PR after FIX-1321; these are vertical work slices, not independently shippable sub-PRs. Refresh #1600 before coding and converge its overlap rather than copying a stale diff.
...
- **FIX-1321 merges first:** exact-id registry and explicit collection declaration are technical prerequisites. No collection deployment is safe from registry changes alone. FIX-1323 can develop independently against the shared id/owner seam; full floor Proof requires all three.
...
- Linear currently has no hard blocked-by relations. This spec records the merge order without inventing a new product blocker. Project is Orchestration Primitives; no fitting existing milestone was established, so none is invented.
- **Open contract forks: none.** Approval requested is of Parts I+II. Engineering names and local placement remain implementation decisions; no approval, merge, spec closure or implementation is authorized by publication.
