...
3. **Historical cross-flow children are re-keyed, never adopted.** Children created under the old destination-kind derivation for a custom-id destination, plus the records and references that depend on their ids, are re-keyed during the quiesced offline migration, gated on the inventory preflight. Reject permanent legacy-key adoption and any second persisted lookup/precedence rule (tenet 3: one derivation). *Locks in:* record ids are preserved for every record that is not an affected child; verified absence of affected history is a documented preflight exclusion with no spec or runtime change. *What flips it:* a deployment that ran main after 2026-09-07T21:55Z, dispatched cross-flow into a custom-id flow, and references those child session ids outside the store.
...
Research baseline: fresh `origin/main` at `cc4bd1e41`; [PR #1600](https://github.com/fixpoint-labs/flow-state-dev/pull/1600) merged after it and is now part of main (see the child boundary below). The immutable epic reviewed here is [PR #1617 at e1d5b701](https://github.com/fixpoint-labs/flow-state-dev/blob/e1d5b701096105eb04954b4ec41545b3dc25e7cf/spec/_epics/flow-instances.md). Its stale pending-objective text is superseded by the owner's session approval, recorded in FIX-1320 comment `4a714d24-2f94-4066-af07-15a74e7355ab`; no individual approval is implied.
...
This deliberately uses one historical wire address field instead of adding an alias. Its old name is documented, not deprecated. All materializers must use resolved `flow.kind` for kind metadata and `flow.id` for owner fields; copying the address string into both is wrong. Singleton URLs, envelope values and storage ids stay unchanged. New owner fields are additive record data; the only re-keying is the migration's re-key of affected historical cross-flow children (§6 Decision 3).
...
[PR #1600](https://github.com/fixpoint-labs/flow-state-dev/pull/1600) merged to main on 2026-09-07T21:55Z (commit `7843f23f`), after the research baseline. It is landed code, not this issue's implementation: it introduced authored cross-flow selection and the `target` to `action` cutover, and main already derives cross-flow child keys from the target flow kind (`context/create-request-host.ts`) and stamps `metadata.dispatch.flowId` beside `flowKind` on cross-flow dispatches (`execution/dispatch-metadata.ts`). Preserve its named `flow-not-found` / `no-entry` outcomes and accepted-versus-not-started guarantees when building on it.

After the cutover, the new top-level owner field on requests and the existing `metadata.dispatch.flowId` destination stamp coincide on every cross-flow arrival. The implementation must say so where both are defined, or read one from the other; it must not write two independent owner facts that can drift.

Replace **kind inequality** with **resolved instance-id inequality** wherever it decides the same-instance/cross-instance boundary: static/carried dispatcher validation, recipient resolution, child-key derivation and lineage inheritance. Same instance retains existing child derivation and lineage. A different destination instance, including same-kind peers, gets #1600's cross-flow child treatment: destination id distinguishes derived child keys and a separate lineage is minted. Adoption and existing-session delivery require the stored session owner to equal the **resolved recipient**, not the sender. For two singletons, destination id preserves #1600's kind-based key bytes because id equals kind. Existing custom-id children with old derivation are re-keyed by the offline migration (§6 Decision 3), never silently adopted at new keys.
...
Persist the owner on every creation path and retain it on every subsequent update. Exclude `flowId` from conditional mutable request fields just like other indexed identity. Memory/filesystem stores and SQLite/Postgres adapters must round-trip it, including active-registry explicit serializers and schema migrations. FIX-1322 owns, in the shared FIX-1321+FIX-1322 implementation PR, the exact-owner list filter for session/request enumeration, spelled `flowId` to match the record field (`flowKind` remains the broad administrative kind filter), and the `flowId` projection on the session, request and `ChildSessionSummary` response types plus their docs (`background-work.md`, the engine README, `server-and-client.md`). FIX-1324 consumes these and adds none of them. Kind grouping is still useful; it is not ownership.
...
Commission a documented **adapter-native offline cutover**, not a generic migration service: quiesce producers and execution; drain old queues/active work; back up and inventory session/request records plus FIX-1323's cells; map explicit record ids to known registered ids; validate kind, tenant, session/request and parent/lineage relationships; backfill owner fields without changing record ids for every record that is not an affected cross-flow child, and re-key affected children with their dependent records/references in the same migration (§6 Decision 3); verify readback before resuming. An interrupted run is safe to repeat for already-identical mappings; a conflicting stamped owner stops it.

There is one procedure, not two. FIX-1322 writes the single quiesce/inventory/backfill/readback skeleton, the record-mapping steps, in `apps/docs/docs/persistence/overview.md`; FIX-1323 extends that same section with its cell rows and adapter recipes. The stop condition is named once: the offline preflight stops with the same `migration-required` outcome the runtime raises on leftover ownerless records. No store-wide transaction or all-scope enumeration is assumed. SQL schema changes may add nullable legacy owner columns but must not blanket-backfill `flowId = flowKind` for collection history.
...
One shared implementation PR integrates FIX-1321's registry/cardinality semantic switch with the complete FIX-1322 consumer/owner cutover; FIX-1322 is integration owner. The registry switch must not land ahead of its live kind-addressed consumers. The work slices below are FIX-1322's contribution, not independently shippable sub-PRs; integrate FIX-1321's separately specified acceptance and goal proof in the same landing. Do not dispatch duplicate implementation workers to land the two issues independently. Require approval of both individual specs and the later cross-spec gate before coding; this sequencing fold passes neither gate. #1600 is on main; build on its landed seams rather than a diff taken at the research head. No alias, first-wins fallback, temporary collection ban, new child or public pair bridges the cutover.
...
12. Explicit offline mapping preserves session/request agreement, re-keys affected cross-flow children with their references, is repeatable for identical mappings and stops on conflicting stamped owners.
...
| `apps/docs/docs/persistence/overview.md` | After persisted data: owner fields; ownerless history; the single quiesce/inventory/backfill/readback checklist (record-mapping steps), which FIX-1323 extends in place with cell rows and adapter recipes. Mapping table, not an invented API; `migration-required` named once as the stop condition. | Link setup/durability/FIX-1323-owned flow-isolation page; no automatic migration promise. |
...
| `apps/docs/docs/server/background-work.md` | Existing child and `{id}`/`{from}` explanation: instance equality, cross-instance lineage/refusal, legitimate child listing; `flowId` on `ChildSessionSummary`. | Link persistence and durable execution; preserve source restrictions. |
...
| `packages/engine/README.md` | Inbound transport and re-entry paragraphs: exact address, durable owner, direct execution guard; `flowId` list filter and response projection. | Link primary pages; keep terse. |
...
| `docs/architecture/server-and-client.md` | Canonical endpoints/client targeting and stored-owner authorization/re-entry; `flowId` filter and projection on session/request responses. | Link inbound contract; no client UI redesign. |
...
- **FIX-1321 + FIX-1322 land atomically:** exact-id registry and explicit collection declaration are technical prerequisites within one shared implementation PR, not a separately merged predecessor. FIX-1322 owns integration; each issue retains its own spec, acceptance and goal accountability. Both individual specs and the later cross-spec gate require owner approval; the engineering sequencing decision neither asks for that decision again nor authorizes implementation. FIX-1323 stays a separate implementation PR with the required identity/owner prerequisites. Core proof includes all three, including isolation; the owner's subsequent FIX-1324 scope addition also requires real Devtool UI proof before complete epic rollout.
- **#1600 landed** (main, 2026-09-07T21:55Z, `7843f23f`): its dispatch/action rename and child/reply/lineage seams are the baseline this issue builds on, so §7's boundary work replaces kind with resolved instance id in landed code rather than converging an open diff. It is not this issue's implementation or authority to start code.
...
- **Historical child keys — decided (§6 Decision 3, EM cross-spec pass 2026-09-08):** [review r3952348303](https://github.com/fixpoint-labs/flow-state-dev/pull/1633#discussion_r3952348303) identified the conflict between §7's destination-kind-to-id child derivation and its unchanged-record-id backfill. Resolution: keep one runtime derivation and re-key affected cross-flow children plus their dependent records/references during the quiesced offline migration, gated on the inventory preflight. Verified absence of affected history is a documented preflight exclusion and no spec or runtime change. No permanent legacy-key adoption and no second persisted lookup/precedence rule, ever. Because #1600 landed on 2026-09-07T21:55Z, affected history can only exist in a deployment that ran main after that time, dispatched cross-flow into a custom-id flow, and references those child session ids outside the store; that finding is what would flip the decision.
  Approval remains of Parts I+II; no approval, merge, spec closure or implementation is authorized by publication.
- Devtool viewing (FIX-1324) is now the fourth required epic outcome by owner direction (2026-09-07), with its own spec/approval gate, outside this issue's slice. It consumes the `flowId` list filter and response projections this issue owns (§7) and adds none of them. Workforce mint (FIX-1325) remains deferred. Chat removal FIX-1330 is neither a child nor blocker. Message-board routing remains outside the floor.
...
- **After cross-spec alignment (2026-09-08)** — the EM decided the historical-child-key fork (re-key affected children in the offline migration; no legacy-key adoption), because a second persisted lookup rule breaks tenet 3; recorded #1600 as landed on main; assigned the `flowId` list filter and response projections to this issue with FIX-1324 as consumer; and made the offline cutover one checklist that FIX-1323 extends, with `migration-required` as its single stop condition.
