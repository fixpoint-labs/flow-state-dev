...
**Size:** one implementation PR after the shared identity/owner cutover; 25 existing Devtool coverage targets, not a fixed edit count, plus any missing thin core-client projection plumbing; 7 observable regression groups; 5 primary documentation surfaces. Core isolation remains necessary for complete epic proof.
...
Consume FIX-1322's **exact-owner enumeration** through the existing `listSessions` surface; preserve its separate broad administrative kind filter. The exact-owner filter is record ownership, not a second action address or a competing `flowKind`/`flowId` precedence rule. FIX-1322 owns its semantics, authorized store filtering and owner admission. If an existing public client/HTTP filter or child/suspension owner projection is still absent after the shared cutover, agree that concrete consumer handoff with FIX-1322 and complete only its route/type/serialization exposure here; no second backend design or store/admission implementation. Use the core-approved spelling. A change to address semantics or authorization is a genuine core-contract fork and must be surfaced, not guessed.
...
The provider transition alone does not currently protect mounted consumers. `use-session-state`, `use-debug-resources`, `use-debug-resource-content`, `use-debug-collection-items` and `use-list-suspensions` commit awaited results without owner fences. Cover their snapshot/history, content/download, pagination cursor, error/disabled and loading state through the owned boundary or existing fence pattern; do not defer this work to FIX-1092. `use-replay` is synchronous request/cursor state, not an unfenced fetch; `use-live-mode` derives a target and owns a polling interval. Cover their state and subscriptions through the same retirement boundary rather than adding read fences indiscriminately. `selection-context.tsx` owns trace/block/item detail only: clear or retire it with the owned panel, never repurpose it as flow-instance authority.
...
| Tasks/ChildSessions links and descent return | Under FIX-1322's integration of #1600, explicit dispatch to another instance creates a child owned by that recipient. Parent-authorized descendant discovery preserves principal/tenant/org and parentage, not parent-owner equality; opening the child still requires its own admitted session read. Use returned child owner to switch instance+session atomically; each trail entry retains its owner. Return restores that owner. |
...
3. **Carry owner through inspection and navigation.** Modify `DevToolPanel.tsx` and owned read/detail boundaries as needed: `context/selection-context.tsx` (trace/block/item retirement only), `hooks/{use-session-requests,use-child-sessions,use-request-stream,use-session-state,use-debug-resources,use-debug-resource-content,use-debug-collection-items,use-replay,use-live-mode}.ts`, `components/detail/session-context.tsx`. Cover resource tree/content/download consumers through their owning boundary, retire old detail/replay/stream state and change child/task descent and return to carry owner. Keep session-addressed public reads unchanged.
4. **Correct existing control targets.** Modify `lib/client.ts`, `hooks/{use-action-dispatch,use-continue-request,use-list-suspensions,use-resume-suspension}.ts`, `components/workspace/{action-bar,suspensions-view}.tsx`, with panel call sites. Remove stored-kind recipient derivation and old completion callbacks that can reinstall another owner. Do not remove capabilities to avoid addressing them.
...
The coverage inventory is the original 18-file spine plus the five state/resource/suspension readers and replay/live hooks: 25 existing Devtool targets. This is not a promise to edit every target: an owned subtree can retire unchanged descendants, while still protecting resource display/download consumers in `components/detail/{resources-panel,resources-tree}.tsx`. The implementation must prove every named surface, not preserve an 18-file ceiling. Allow a small coordinated existing client/engine projection delta only if the core handoff has not already supplied it. No new permanent component, package or hook is commissioned.
...
3. Exercise live stream, replay/cursor/reconnect, interrupted Continue and suspension approve/reject against their real runtime states. Observe the correct owner and request continuing, not just successful HTTP status. After the shared cutover, use supported explicit cross-instance dispatch to create B's child from A; open it through Tasks/ChildSessions after B's own session admission, and return using the trail. Parent listing permission alone must not expose denied child details.
4. Delay real network responses during A→B→A and credential changes; cover session snapshots/history, resource trees/content/downloads, collection pages/cursors and suspension rows as well as list/create/action/stream results. Confirm no retired data, errors, disabled/loading flags or reconnects appear, including before passive resets run. Reload persisted selections; change backend/user/token and remove access or registration; verify no same-kind fallback or stale disclosure. Browser delay/interception may orchestrate timing, but response data must come from the real runtime.
...
- **PR #1600** overlaps the owner/child boundary and is integrated by FIX-1322. Its cross-flow dispatch changes current-main's same-flow child assumption; FIX-1322's §7 extends that boundary to exact recipient instances, including same-kind peers. Descendant discovery and child-detail authorization remain separate. **PR #1223** overlaps request-management tenant authorization; refresh that overlap through FIX-1322, preserve its intended tenant binding, and do not create another mandatory merge dependency for this UI spec.
...
Below-bar suggestions from [Cursor review](https://github.com/fixpoint-labs/flow-state-dev/pull/1634#pullrequestreview-5135199556), preserved verbatim for implementation/editorial judgment rather than made approval conditions:

- [r3952565260](https://github.com/fixpoint-labs/flow-state-dev/pull/1634#discussion_r3952565260): The size accounting (18 files, 7 regression groups, 5 doc surfaces) is useful tracking metadata but reads heavy in the Case/sign-off section (§6). Consider moving to §8 or a single "Scope footprint" line and dropping the repeated count from §8:144.
- [r3952565265](https://github.com/fixpoint-labs/flow-state-dev/pull/1634#discussion_r3952565265): This paragraph prescribes a lot of mechanism (provider owns atomic workspace, clear session before render, validate before install, no brief cached display). The outcome — "switching instance never shows prior owner's session, requests, streams, or detail" — plus a pointer to existing patterns (`DevToolPanel` session-reset comments, `use-session-requests` `heldIdentity`, `devtool-panel-session-reset.test.tsx`) would give implementers room without narrating every retirement boundary.
- [r3952565272](https://github.com/fixpoint-labs/flow-state-dev/pull/1634#discussion_r3952565272): The control-surface table (§7:115–124) plus this state-clearing checklist is accurate but reads like an implementation script. Consider one invariant ("no stale owner data survives the selection transition") and defer per-control wiring to the implementer, who already has the source files named in §8.
- [r3952565277](https://github.com/fixpoint-labs/flow-state-dev/pull/1634#discussion_r3952565277): The deprecated Kubernetes Dashboard citation doesn't support a decision here — React key guidance (line 130) and "don't add TanStack" (line 131) already cover the alternatives. Safe to cut for length.
- [r3952565281](https://github.com/fixpoint-labs/flow-state-dev/pull/1634#discussion_r3952565281): Most of this table restates §6–§7 (same-kind IDs, empty catalog, stale hints, rapid switch, credentials, cross-owner child). Consider keeping only cases **not** already covered: legacy ownerless data (row 156), long/opaque ID display (157), denied/missing + network (158). Point the rest at §6 rule 2.
- [r3952565285](https://github.com/fixpoint-labs/flow-state-dev/pull/1634#discussion_r3952565285): The goal in line 164 is right (real Devtool, two collection instances, no mocks). Steps 1–5 read like a QA runbook (screenshots, delay interception, keyboard/a11y). For a spec, a behavior bullet list matching the seven regression groups may be enough — the step-by-step choreography can live in the implementation PR or test plan.
- [r3952565297](https://github.com/fixpoint-labs/flow-state-dev/pull/1634#discussion_r3952565297): This `isCurrent()` vs `begin()` pitfall is important implementation input — A→B→A can revive stale callbacks if guards use tuple equality alone (`use-read-fence.ts:180–193`). Consider folding one sentence into §7 Navigation/async ("late-write guards use `begin()` predicates, not `isCurrent()` alone") and shortening or removing §13 so implementers don't miss it.

For the last note, §7 requires irreversible visit retirement; the helper choice remains implementation input. `begin()` also imposes latest-read-wins and is not a blanket substitute for guarding concurrent actions.
...
- **After first external spec review** — expanded the ownership-retirement coverage beyond the navigator/control spine to state, resource, suspension and replay/live consumers, because the existing unfenced readers would otherwise invalidate the promised no-stale-owner view. Kept the two Decisions, coordinated thin consumer plumbing and prerequisite-supported cross-instance child navigation.
