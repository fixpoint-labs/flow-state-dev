...
7. **A worker record carries its identity as one field. The team and the bare name are not
   materialized beside it.** The record is `id`, `declared`, `body` — plus the two fields only
   a directory read can produce (`dir`, `codePath`), which this factory never reads.
...
export interface WorkerManifest {
  ...
  body: string;
  /** Where the folder is. Set by the loader; a hand-built record omits it. Never read here. */
  dir?: string;
...
export function hireWorkforce(manifests: WorkerManifest[], options: HireOptions): Workforce;
```
...
*Decision 7 changes that type, and FIX-1335's follows it.* Drop `teamId` and `name`: they
appear in that spec only in the type's own declaration, and nothing in either issue reads
them. Keep `codePath` — its §9 does consume it. **`dir` needs a new reason or it goes:** its
only stated justification is that FIX-1325 resolves relative refs against it, and this factory
opens nothing — §7 says so in its first line. That is a correction to a claim about *this*
issue rather than a redesign of that one, and the field may well earn its place from the
loader's own error reporting. **Not edited from here** — FIX-1335's spec is approved; this is
routed to it.
...
helpers — were ratified in review round 1 and now read as decisions 1 and 5. §6 is the
sign-off surface.
