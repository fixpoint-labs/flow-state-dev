...
**Size:** Small–Medium — 3 production files · 13 test behaviours · 1 goal · 2 doc surfaces ·
...
```ts
/**
 * One worker, as declared on disk or hand-built. Declared once, in
 * `packages/workforce/src/manifest.ts`; FIX-1335's loader returns this type rather than a
 * second one of its own (decision 7).
 */
export interface WorkerManifest {
  /** The whole identity and the flow address — `"<teamId>.<name>"`, minted once by the loader. */
  id: string;
  /** Frontmatter exactly as written. */
  declared: Record<string, unknown>;
  /** The worker's instructions. Empty for a thin seat. Reaches the flow as `config.persona`. */
  body: string;
...
- **EXTEND** `apps/docs/docs/orchestration/workers-on-disk.md` — the page FIX-1335 creates —
  ...
  refuses it), and the address a hired worker answers on. **Create the page if FIX-1335 has not
  landed**, carrying only this issue's half. *Why extend rather than add a page:* a reader
...
- **A constraint on the Layer 1 factory, discoverable only from this side.** A collection flow
  ...
  here.**
- **FIX-1337's spec reasons about today's mechanism, and the fence supersedes that half after
  W2.** Its §6 decision 2 argues that a definition-time-only check is opt-in by construction
  *because* `createAgentRegistry` takes plain `Agent` objects and nothing routes through
  `defineAgent`. That argument is correct against the mechanism on `main` today and is the
  stated reason its check does not live in the definition builder. Once `defineAgent` is the
  only way to build an agent-shaped block, the bypass it names no longer exists and the
  reasoning needs revisiting — the *decision* may well survive on its other leg (the
  non-object-root hole, which the fence does not touch). **Flagged, not edited from here:**
  FIX-1337 is a sibling with its own spec PR and its own review, and this is routed to it.

**Something the fence left without a consumer.** `description` is reserved out of the settings
bag, and this factory now reads it for nothing — its only consumer was `Agent.description`. It
stays reserved because the alternative is worse: letting it flow into `config` would tax every
worker flow kind with declaring a `description` setting or refusing every hire. But a reserved
key nobody reads is the kind of thing tenet 3 kills, and the honest place to settle it is the
loader, which is what requires it. Not resolved here; recorded so it is not mistaken for
design.
