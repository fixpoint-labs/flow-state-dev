...
**Size:** 5 production files (3 in `core`, 1 in `engine`, 1 in `testing`) · 11 test behaviours · 7 documentation surfaces · 1 PR.
...
where the blueprint's mirrored properties are built:
    ...
    blueprint.config         <- freeze(probe ok ? probe.value : empty)
    blueprint.requiresConfig <- not probe ok
