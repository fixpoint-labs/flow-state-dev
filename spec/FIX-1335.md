...
**That is where this issue stops.** Turning a record into a running seat — one flow copy per
worker, configured by what its folder declared — is the seat factory's job (FIX-1325). The split is not bookkeeping. A workforce has thin seats in it: an
...
2. **A worker's identity is its team and its name joined by a dot — `engineering.lead`,
   ...
   *Cost:* this identity is the string anything referring to a worker has to use — a flow
   instance id, a board key. Changing its shape after adoption
...
3. **The loader reads what a worker folder declared and interprets none of it. Producing
   ...
   *Locks in:* epic [#1664](https://github.com/fixpoint-labs/flow-state-dev/pull/1664)
   theme 2 draws the line this decision sits on — **FIX-1335 stops at neutral manifests: no
   flow, no block, no registry construction**, and FIX-1325 turns a manifest into a flow per
   seat, full stop. An `Agent`-shaped return breaks that twice over. It cannot express a thin
   seat, so it would quietly teach that every scanned worker is an agent — the collapse
   theme 3 exists to prevent. And it would hand back a species the substrate no longer
   resolves: theme 2's fence settles that a name resolves to a `BlockDefinition` and to
   nothing else, which retires `createAgentRegistry` as a resolve path rather than leaving
   this loader one to feed. The cost is that this issue ships nothing runnable on its own:
   until FIX-1325 lands, the manifests have no consumer but a test.
   *(This decision used to justify itself on the Atlas's* "defineFlow EVERY SEAT ·
   AgentRegistry ONLY IF AGENT" *(`docs/atlas/workforce.html:876-877`, figcaption `:886`).
   The epic's Atlas-obligations table marks that line superseded by the same fence: its first
   clause survives, its second is struck. The substance below is unchanged; only its footing
   moved.)*
...
**Alternative: the loader builds and returns the running roster itself.** One call, nothing
for the author to wire. Rejected twice over. It makes the loader the only way in — a roster is
also written by hand today, and FIX-1325 treats a hand-built record as a first-class input, so
a loader that owns construction quietly turns the TypeScript path into a second-class one,
against theme 1 ("files and hand-written definitions produce the same entries"). And it cannot represent a thin seat at all, per
...
if (errors.length) {
  // A reported slot is a seat that did not load. Booting past it boots a short roster.
  throw new Error(
...
**`errors` is not advisory, and the example says so deliberately.** An earlier draft warned
and carried on. That models the wrong default: a reported slot is a seat the app was supposed
to have, and warn-and-continue boots a roster one worker short with nothing failing. The
loader returns `errors` rather than throwing because a library that hands back data does not
get to set an app's boot policy — which makes treating them as fatal the *app's* call to make
explicitly, not one to arrive at by leaving the array unread (§11).

**What happens next — deliberately not here.** FIX-1325 takes that array and gives every
worker one flow copy, thin seats included, configured from what its folder declared:

```ts
// FIX-1325, not this issue:
// const seats = hireWorkforce(workers, { kinds: { "worker-agent": workerAgentFlow } });
// flowRegistry.registerMany(seats);  // one copy per record — no second species, no registry entry
```

**Referring to a worker** *(before / after)*:

```
before  eng-manager        — the name a hand-written definition chose
after   engineering.lead   — the name its folder gives it
```
...
flowchart LR
  ...
  B --> Thin["thin seat<br/>flow copy, no persona"]
  B --> Ag["opinionated seat<br/>flow copy + persona from the body"]
```
...
**These comments are the union of both specs' wording, deliberately.** FIX-1325 declares the
same type in the same module, and the two drafts had drifted on `body` and `declared` — so
whoever landed second would have silently overwritten the other's comment. The union carries
both facts on `body` (what it is here, what it becomes there) and drops this draft's
*"kebab-case keys"*: FIX-1325 decision 3 rules out any translation between how the file
writes a setting's name and how the code writes it, so a comment that *prescribes* a case
convention invites exactly the reliance that decision refuses. Keys are described as written,
not constrained.
...
**Interpreting nothing is the point.** The loader does not know that `model` means a model,
or that `flow` names a kind FIX-1325 will look up. It checks that a `WORKER.md` has parseable
frontmatter with a `description` and hands the rest over untouched. This is what makes the
manifest *neutral*, and it is why FIX-1325 could reserve `flow` — and a later issue can claim
a key this loader has never heard of — without a format change or a second door.

**`description` stays required, on practice 3's legs rather than a consumer's.** FIX-1325
routed the question here when its substrate fence left the key unread by the factory (its
§12). Nothing was lost in that fence: `description` never had a code consumer to lose —
nothing in `packages/workforce/src` or `packages/orchestration/src` reads `agent.description`,
and core types it as a *"routing-facing summary surfaced in trace UI / DevTool. NOT the system
prompt"* (`packages/core/src/types/agent.ts:64`). The requirement stands on the dialect
instead: `SKILL.md` throws on a missing or empty `description`, and practice 3 commits these
two hand-written convention files to **one dialect** — a field required in one and optional in
the other is precisely the divergence that practice exists to prevent. It also keeps a roster
legible to the human reading it. **"Required" means required of a `WORKER.md`:** a
`worker.ts`-only seat declares no frontmatter at all, so it has no `description` and is
still valid (§9).
...
- **Untouched:** `createAgentRegistry`, `materializeAgent`, `defineAgent`, `definePersona`,
  ...
  and does not depend on their surviving, since theme 2's fence retires several as a resolve
  path in a Layer 1 change no child of this epic delivers.
...
raised this). The reason is the identity's blast radius: the minted `team.name` becomes a
flow instance id and a board key, while each *segment* is separately a path segment on disk. Lowercase-hyphen is the one shape safe in all of those, and it is the
...
| `tools` naming a tool no catalog has | Not this loader's call, and not resolvable here — the loader has no catalog. Resolution and its current dishonesty are FIX-1327's |

**The `worker.ts`-only row is a claim about this loader, not about booting.** Such a folder
is a valid *manifest* — theme 1's second door, and all this issue owes. It is not yet a
hireable seat: it declares no frontmatter, so it names no flow kind, and nothing in either
issue binds a `worker.ts` to a running worker. **What FIX-1325's factory does with a record
that names no kind is FIX-1325's row to write, and it is routed there** — the two rows are
deliberately written against each other so neither issue assumes the other handled it.

**On the `.` in an identity — checked, and no longer load-bearing.** Epic
[#1664](https://github.com/fixpoint-labs/flow-state-dev/pull/1664) theme 2 retires `agent-ref`
as a runtime resolve path, so it is not among the things this identity has to satisfy. The
check was made before that fence and is kept as a record rather than a dependency: a skill's
`agents:` map restricts the *assignee key* to `/^[a-zA-Z0-9][a-zA-Z0-9_-]*$/` and does **not**
restrict the ref *value* (`skill-md.ts` applies `isValidAgentKey` to the key only, in
`parseAgentsField`), so a dotted identity is legal there today with no change to that parser.
What the identity does have to satisfy is the flow route — decision 2, settled on the POC.
...
**Taxonomy:** every per-worker problem is non-fatal *to the loader* and collected, so one bad
folder never costs an app its other workers. The single fatal path is an unreadable root. What
an app owes a non-empty `errors` is a different question, and §11 answers it: collected is not
advisory.
...
- **the one easy to state wrongly:** a frontmatter key with no meaning to anything today must
  be *retrievable from the manifest* — `manifest.declared["team-visible"]` — not merely "does
  not throw". A parser that silently discards it also does not throw. **Assert on a key
  nothing reserves**: `flow` and `description` are spoken for (FIX-1325 reads the first and
  this loader requires the second), so neither can demonstrate that an *unknown* key survives
  verbatim. **The manifest is the
...
- **NEW PAGE** `apps/docs/docs/orchestration/workers-on-disk.md` (`sidebar_label: Workers on
  ...
  **Deliberately not cross-linked to the Agents page, and its *Current limits* bullet is not
  edited here.** An earlier draft did both. FIX-1325 §11 rules them out by name, and its
  reasoning post-dates the substrate fence and wins: static agent registration is a mechanism
  the Layer 1 change is removing, so pointing at it would teach a shape with a scheduled
  expiry and leave a dangling anchor when it goes. Whoever does that Layer 1 change owns
  re-pointing the Agents page.
  **One paragraph the page owes, because only the composed picture shows it.** `errors` is not
  advisory. This loader collects and *returns* them; FIX-1325's factory collects and *throws*.
  An app that warns on `errors` and carries on therefore fails asymmetrically — a mistyped
  *filename* costs it one seat quietly, while a mistyped *setting* costs it the whole boot.
  The missing seat is the worse failure and it is the quiet one. Say plainly that an app
  should treat a non-empty `errors` as fatal at startup unless it has a specific reason not
  to. Neither posture is wrong and neither changes: a function that hands back data does not
  get to set an app's boot policy, which is exactly why the page, not the library, has to say
  this.
...
- **Cross-spec alignment — no decision changed.** A `cross-spec-review` over the epic's three
  approved specs returned eight findings; six were this one's to absorb. That is expected
  rather than alarming: this spec was approved first and has been reshaped twice from outside
  since (the `/`→`.` joiner, then the owner-locked substrate fence), so it was the document
  furthest behind its siblings. **Every item below is alignment or re-justification. No
  decision was reopened, no label moved, and the spec PR stays open at `Spec Approved`.**
  1. **The kind key and the example file.** FIX-1325 reserves `flow`; this spec had written
     it `flow-kind`, and FIX-1325 owns that key as its only reader. Worse for a reader, the
     §5 example `WORKER.md` — the one §11 puts on a page FIX-1325 then extends — carried no
     kind key at all and declared `allowed-tools` where FIX-1325's schema declares `tools`, so
     run through that factory it would have failed twice. The example gains `flow:
     worker-agent` and `allowed-tools` becomes `tools`. §7's prose now names `flow` (the key
     FIX-1325 actually reserved), and §10's unknown-key assertion moved to `team-visible` —
     a key nothing reserves, since the behaviour being asserted is that an *unknown* key
     survives verbatim, which a reserved one cannot demonstrate.
  2. **The pre-fence registry shape, removed from every teaching surface.** Decision 3's
     substance survives the fence untouched — the loader stops at neutral manifests — but
     everything around it was stale. Its *Locks in* was re-justified on epic theme 2, because
     the Atlas lines it cited (`:876-877`, figcaption `:886`) are now marked superseded in the
     epic's own Atlas-obligations table. §5's "what happens next" snippet dropped
     `createAgentRegistry(seats.opinionated)` for FIX-1325's real shape (one flow copy per
     record, thin seats included, `hireWorkforce` synchronous). The §7 mermaid's leaves were
     redrawn — both seats are flow copies now; they differ by persona, not by a registry
     entry. §11's instruction to edit the Agents page's *Current limits* bullet and cross-link
     back was dropped, because FIX-1325 §11 forbids exactly that by name and its reasoning
     post-dates the fence. And `agent-ref` was demoted from the minted identity's live
     consumers (§6 cost, §5's before/after, §7, §9) to "a flow instance id, a board key" —
     the epic dropped it from that list deliberately.
  3. **The `worker.ts`-only seat, handed off explicitly.** §9 keeps its row: such a folder is
     a valid seat *to this loader*, which is what theme 1's second door means. What it now
     says out loud is that this is a claim about the loader and not about booting — the record
     names no flow kind, nothing in either issue binds a `worker.ts` to a running worker, and
     what FIX-1325's factory does with such a record is FIX-1325's row, routed there. The two
     rows are written against each other rather than independently.
  4. **The composed error posture, stated where it becomes visible.** Neither posture changed
     and neither could: this loader collects and *returns*, FIX-1325's factory collects and
     *throws*, and a library that hands back data does not get to set an app's boot policy.
     What was missing is that composed through §5's own call site the lenient half is the
     dangerous one — a mistyped filename costs one seat quietly, a mistyped setting costs the
     boot. §5's example stopped modelling warn-and-continue as the default, and §11 owes the
     shared page one paragraph saying `errors` is not advisory.
  5. **`description` — required, with its real reason.** FIX-1325 §12 routed the question here
     on the premise that its only consumer was `Agent.description`. That premise is false and
     was checked: nothing in `packages/workforce/src` or `packages/orchestration/src` reads
     `agent.description`, and core types it as a trace/DevTool label, so the fence took nothing
     from it. The requirement was re-footed on practice 3 — `SKILL.md` throws on a missing
     `description`, and the two hand-written convention files take one dialect — and §7 now
     says plainly that "required" means required *of a `WORKER.md`*, since a `worker.ts`-only
     seat has no frontmatter at all.
  6. **The shared type's doc comments, unioned.** Both specs declare `WorkerManifest` in
     `packages/workforce/src/manifest.ts` and agree on the fields, but the comments on `body`
     and `declared` had drifted, so whoever landed second would have overwritten the other.
     The union is taken, and `declared`'s normative *"kebab-case keys"* became descriptive:
     FIX-1325 decision 3 refuses any translation between how a file writes a setting's name and
     how the code writes it, so a comment prescribing a case convention invites the reliance
     that decision exists to remove.

  *Also touched, same class, not on the review's list:* §2's plain-terms sentence and §3's
  rejected alternative both still described the seat factory as building an agent registry,
  and §7's "Untouched" list read as though those symbols were expected to survive. Aligned,
  with no change to what either passage argues. Spec-evolution history above keeps its
  original language throughout — it is the record of what the spec used to say, not a surface
  that teaches.
