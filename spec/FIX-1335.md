...
2. **A worker's identity is its team and its name joined by a dot — `engineering.lead`,
   ...
   **Joined by `.`, because a `/` in an identity is not addressable.** This is substrate,
   ...
   | Leg | Result |
   |---|---|
   | `defineFlow` factory, id with `/` | **ACCEPTED** — `id=engineering/lead` |
   | `registry.register` / `registry.get("engineering/lead")` | **ADMITTED / RESOLVED** |
   | `POST …/engineering/lead/sessions` (two segments) | **404** `Route not found` |
   | `POST …/<one escaped segment>` | **404** `Route not found` |
