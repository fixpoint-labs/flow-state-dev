...
2. **One convergence point for a worker's identity (tenet 5)** — team and name are joined
   in exactly one helper. Two places that build the same id is how a registry key and a
...
flowchart LR
  ...
  W -->|"WorkerManifest[] + errors"| B{{"FIX-1325 · seat factory<br/>NOT THIS ISSUE"}}
  B --> Thin["thin seat<br/>defineFlow only"]
  B --> Ag["opinionated agent seat<br/>defineFlow + AgentRegistry entry"]
```

**The manifest.** One record per worker, carrying what the folder said and where it lives:

```ts
export interface WorkerManifest {
  /** Team-qualified identity, "<teamId>/<name>". Minted once, in one helper. */
  id: string;
  teamId: string;
  name: string;
  /** Absolute path of the worker folder — FIX-1325 resolves relative refs against it. */
  dir: string;
...
string; error: Error }> }`. Errors are keyed by the worker path (`engineering/intake`) rather
than a bare name, because a near-miss may have no valid identity to report under.
...
- **`@flow-state-dev/workforce`** — gains a **`./loader` subpath export** carrying the walk,
  the `WORKER.md` reader, and the identity helper. The root export stays free of `node:fs`,
...
**Identity and segment naming — the rules, stated.** `teams/<teamId>/workers/<workerName>/`
yields `id = "<teamId>/<workerName>"`, minted by one internal helper. Both segments must be:
...
raised this). The reason is the identity's blast radius: `team/name` becomes an `agent-ref`
value, a flow instance id, a board key, and a path segment. Lowercase-hyphen is the one shape
that is safe in all of those, and it is the shape an author already learned from `SKILL.md`.
The cost is real and worth naming: no underscores, no capitals, no non-ASCII team names —
`Platform_Eng` is rejected at startup with the rule in the message. Implement it as the
loader's own validator with worker-worded errors rather than calling `validateSkillName`,
whose messages say "Skill name".
...
restrict the `agent-ref` *value*, which is the identity this loader mints (`skill-md.ts`
applies `isValidAgentKey` to the key only, in `parseAgentsField`). So
