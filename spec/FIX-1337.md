...
2. **A declared result shape the provider can never accept is refused before it can reach a
   ...
   **This is not severable from decision 1; it is what stops decision 1 introducing a
   ...
   Two things carry this check, and neither depends on where it is put:
   - **Reachability.** Decision 1 is what makes an unusable shape reachable at all, and it
     makes it reachable *lazily* — the first time that worker is materialized, inside a
     running job. Refusing before a job can run is what stops decision 1 introducing a
     regression. Any call site early enough closes this; no call site can make it go away.
   - **The predicate hole.** Core's `assertStrictCompatible` treats primitives as
     strict-safe and returns a non-object root unchanged, so `z.number()` passes it
     ...
     root that is neither text nor an object has to be refused by name, not passed. This
     hole is in the predicate, so the check must be *widened* wherever it sits.
   **Where the check sits is no longer the question round 1 made it.** An owner-locked
   substrate fence has since made `defineAgent` the single door into an agent, so there is
   no second route to guard and no call site that is opt-in by construction. The requirement
   is unchanged and untouched by the fence — refused by name, before a job can run, on every
   route in — but what is left for the implementer is the widened predicate rather than a
   choice of location.
   ...
   *The predicate is the constraint, not the line that carries it* — a root that is neither
   text nor an object, and a shape carrying a transform, must both be refused before a job
   runs. This spec still builds against `materializeAgent` as it stands today (§12).
...
**The substrate fence, and what it does not change here.** An owner-locked fence makes
`defineAgent` the only door into an agent: it becomes a block factory, `materializeAgent` and
`agentBlock` collapse into it, and `createAgentRegistry` / `agent-ref` stop being a runtime
resolve path. **The code kill is not this issue** — it is sequenced as its own orchestration
issue, after FIX-1325's factory lands. This spec implements against `materializeAgent` as it
stands today, and must not be built against a Layer 1 API that does not exist yet. The fence's
only effect on this spec is which justification carries §6 decision 2 (see Spec evolution);
the decision, the design and the deliverables are unchanged.
...
- **cursor, on §6 decision 2:** *"Simpler: `assertStrictCompatible` in `materializeAgent`
  ...
  Defer define-time unless import-time failure is an explicit epic requirement."* — decision
  2 requires the outcome, not the call site. This is the leading candidate: it is the one
  point every route into a delegated agent passes through. Weigh it against whether a
  directly-mounted agent should be covered there too.
...
- **Review round 1** — three reviewers converged on §6 decision 2 from three directions and
  ...
  holes in it. *(The first of those two holes has since been closed by the substrate fence
  and that justification retired — see **Fence correction** below. The decision stands.)*
  Added decision 3 (no output transforms): strict-compatibility unwraps
...
- **Decision folded** — the product owner answered the one open fork, which covered both
  ...
  *"what may I declare?"* one answer. The design is untouched: decision 2's mechanism, the
  ...
  alone.
- **Fence correction** — an owner-locked substrate fence has since made `defineAgent` the
  single door into an agent, and that killed one of §6 decision 2's *written justifications*:
  round 1's *"`createAgentRegistry` takes plain `Agent` objects, so a check living only in
  `defineAgent` is opt-in by construction."* With one door there is no bypass, so that
  argument is retired — and with it the *definition-time-only* rejection that rested on it.
  **The decision itself is unchanged and was not re-derived.** It remains load-bearing for
  decision 1. What carries it now is what the fence does not touch: **reachability** (decision
  1 removes the `z.string()` branch that made an unusable shape inert, so the shape becomes
  reachable lazily at worker materialization, inside a running job) and **the predicate hole**
  (`assertStrictCompatible` passes a non-object root silently, while the generator forwards
  every non-string root to the provider as structured output, which must be an object — so the
  check has to be widened wherever it sits). The practical effect is that round 1's call-site
  question dissolves; the widened predicate is what remains for the implementer. Nothing else
  moved: decisions 1 and 3 and their declined alternatives, the six §10 behaviours, §11's
  pinned release-note wording, §9, and §13's notes all stand as they were.
