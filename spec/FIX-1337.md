...
3. **A declared result shape may not carry an output transform.** A shape whose fields
   ...
   *Why this is a decision and not an edge case:* strict-compatibility does not exclude it.
   ...
   down.
   *Rejected:* a JSON-safety gate at the delegated-result boundary only. Narrower, but it
   puts the rule in a second place and makes it conditional on how the agent is run — the
   shape discrimination decision 1 exists to remove (§4, practice 3).
   *Locks in:* an author transforming an output field on a *directly-mounted* agent is
   ...
   a path this issue didn't set out to touch, and it is the honest one: the transform runs
   client-side after parse, so the declared shape and the transported shape were already two
   different answers. Persistence is only where it becomes visible.
   *What would change my mind:* a real caller transforming a directly-mounted agent's
   output. Then the gate narrows to the delegated boundary and §4 practice 3 takes the hit.
...
**Alternative: an opt-in switch** — carried in full at §6 decision 1, together with the
migration call it turns on. Rejected there; not restated here.
