...
- **Corrected after approval (factual only, no direction change).** Two claims about what ships
  ...
  (`hireWorkforce`'s own param doc: *"the flows the app defined"*), so §2's fence row and §7's
  diagram no longer say "built-in". And the examples were written against `persona:`, which
  `main` now refuses; they say `instructions` (§5, §12).
