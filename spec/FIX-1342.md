...
| **Kinds** | `defineFlow` factories the app defined — there is no framework-provided set — passed as `hireWorkforce(manifests, { kinds })` |
...
> included. Then the boot scan is on W3's critical path rather than a follow-up. Even then it
> would not belong *inside* this issue: the atlas already teaches that scan as W3's own
> deliverable, covering kinds **and** blocks together, and a scan for kinds alone designed here
> is the half that the blocks half then has to match.
>
...
**News, not a decision:** the boot scan that would register kinds from files is **designed but
unowned.** The atlas (merged 2026-09-11) teaches W3 as "kinds + blocks file scan as the code
door", tagged *proposed · next* — so the mechanism has a home at design altitude. No Linear
issue carries it: W3's children are rooms, channels, resources, skills, the lab Proof, and this
one, and the nearest existing ticket is FIX-912, which is backlog and broader. Whichever way
the fork goes, that issue needs filing or the atlas promises work nobody is tracking (§12).
...
say no is sequencing rather than soundness: the atlas scopes that scan across kinds *and*
blocks, beside four sibling conventions, so a kinds-only version designed here is the half the
other halves then have to match. Priced in the fork above, filed as a follow-up in §12.
...
export const routerFlow = defineFlow({
  ...
  configSchema: z.object({ instructions: z.string(), desks: z.array(z.string()) }),
...
```ts
// app startup — every kind on the map is one the app defined
...
flowchart LR
  ...
  APP["the app's own flows —<br/>every kind on the map"] -->|"{ kinds }"| H
...
- **FIX-1344 part 1 (`persona` → `instructions`)** — **landed** (`c37eb1a54`, on `main`).
  A worker's body arrives as `instructions`; `persona:` is refused by name
  (`REFUSED_PERSONA_KEY`). The examples here are written against `instructions`, matching the
  docs. The sequencing note this bullet used to carry no longer applies.
...
- **The kinds + blocks boot scan** — how an app's own flow kinds and block definitions reach
  the maps without a startup line. Explicitly a **boot-scan / static-graph** problem, not
  hot-dynamic kinds after boot. The atlas already teaches it as W3's code door and pairs kinds
  with blocks, which is the reason not to design a kinds-only version here. No Linear issue
  carries it (W3's children are FIX-1352/1353/1354/1355/1356 and this one; FIX-912 is the
  nearest and is broader). Needs filing as a W3 sibling — §8 step 3.
...
- **Corrected after approval (factual only, no direction change).** Two claims about what ships
  were wrong. There is **no framework-provided kind set** — `kinds` is entirely app-supplied
  (`hireWorkforce`'s own param doc: *"the flows the app defined"*), so §2's fence row, §6's
  trade-off and §7's diagram no longer say "built-in". And the examples were written against
  `persona:`, which `main` now refuses; they say `instructions` (§5, §12).
