...
flowchart LR
  ...
  RES -->|"frontmatter is settings, body is the document"| DISK["one file, both jobs"]
```

This is the shape under approval, not a shape that ships. **A resource is a file, not a folder — and that breaks the shape its two siblings set.**
