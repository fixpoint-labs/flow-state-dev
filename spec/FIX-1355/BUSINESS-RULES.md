...
| BR-3 | A seat's skills are read | **Set equality against that seat's exact expected union** — not presence, and not "none of its sibling's". `recon` = {`report-format`, `port-scan`, `sweep`}, `triage` = {`report-format`, `port-scan`}, `scribe` = {`report-format`, `port-scan`}. A missing skill and a leaked one both go red, and nobody has to classify which values are shared. **`recon` and `triage` are one team and legitimately share the org and team folders** — only worker-level skills are seat-exclusive, so a sibling-exclusion here would fail a correct run. Isolation is **across teams**: the two `port-scan` folders carry different bodies, and each seat's record must hold its own team's token and not the other team's. Graded **off each seat**, through its own config inside a running block — not the transcript, which by BR-13 can never carry `scribe` | G |
...
| BR-12 | Each seat's line is read | It carries its own instructions token, its own document token, and its exact expected skill union. **Absence is asserted only of the values that are genuinely seat-exclusive** — the sibling's *instructions* token, the sibling's *document* token, and `recon`'s worker-level `sweep`, which `triage` must not hold. It is **not** asserted of `report-format` or `port-scan`: both seats are in `pentest` and both legitimately read them, so excluding them would fail a correct run | G · M |
