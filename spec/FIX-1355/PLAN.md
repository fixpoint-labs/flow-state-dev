...
|---|---|---|---|
| S1 | The tree · `goals/pentest-lab/lab/workforce/` | The ten files in [SPEC.md](SPEC.md). One held-out token per file, each in exactly one **convention** file and nowhere in the lab's code or the post. The fixture holds the *expected* values, as the sibling goals do | BR-1 BR-3 BR-4 BR-5 |
...
| S5 | The gate · `goals/pentest-lab/a-post-reaches-both-declared-seats/` | `goal.md`, `run.mts`, `fixtures/input.json` — every id, token and expected skill name lives in the fixture, graded against the transcript. `goal.md` pins one shared bounded-wait helper with explicit `timeoutMs` / `pollMs` (BR-19); no ad-hoc spin | BR-1 – BR-19 |
...
| VG | S5 | **The gate.** BR-1 to BR-19 in one run, graded after closing the store and rebuilding the host. *Controls:* the seats' documents swapped (each line carries the other's token); **index grading with one append held until the other lands**, so the asserted order is wrong every run, not sometimes; **the restart skipped with the store write suppressed**, so the live context serves what the file does not hold — skipping the restart alone changes nothing |
| V4 | VG | Second path (BP-035), as extra scenarios on VG's host: a member with no router address, and a post claiming a non-member author, behave as BR-9 and BR-17 say — neither takes the run down |
| VM | S6 | BR-20 on a real model: each member's answer names something only its own document says. Logged with its model and date. Required at completion (D2) |
...
| No token in the lab's code — only in the convention files | A marker the driver could have produced proves nothing was read |
...
[DECISIONS → Settled](DECISIONS.md#settled). Treat a false one as a spec defect rather than
working around it.
...
- **FIX-1357 may have landed.** Its generated maps could replace the hand-passed ones — only if
  strictly less code; the hand-passed map stays valid by epic D6.
...
- **`openChannels` has no org door**, so an app whose seats read file-declared documents must wrap
  its client. Filed, with the lab's wrap as the reproduction.
