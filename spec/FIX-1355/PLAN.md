...
| Assert a seat's sibling value is **absent** — but **only where that value is genuinely seat-exclusive.** Two questions, always both: *is the absent half asserted?* and *could this exclusion catch a value the seat is entitled to?* Where values are legitimately shared, grade **set equality against the exact expected union** instead, which catches a missing value and a leaked one without anyone classifying either | A presence-only check passes a seat carrying both; an over-wide exclusion **fails a correct run**, which is worse. Both have happened here. Three M-marked rules were graded on presence alone (BR-2, BR-20, BR-21) while this guardrail was already written — and the sweep that fixed them then over-applied to BR-12's skill names, where `recon` and `triage` share one team's folders by design. Exclusion is for worker-level skills and cross-**team** bodies; never for org or team folders two seats both read |
...
- **Add evidence and a red control for silent-seat dispatch** (Codex, round 5, on VG):

  > BR-13 now requires proving that the fan-out dispatched nothing to `audit.scribe`, but VG only
  > adds a control for the seat-side skill read and otherwise observes the transcript; a stray
  > dispatch whose seat produces no post or token therefore remains indistinguishable from no
  > dispatch. Record the fan-out's addressed seats and add a bounded mutation that deliberately
  > includes `audit.scribe` while suppressing its post-back, so the new no-dispatch assertion is
  > shown to turn red.

- **Exercise the short-transcript failure path** (Codex, round 5, on VM):

  > BR-19 requires a timeout or short transcript to fail loudly with the observed line count, but
  > none of VG or VM's controls suppresses or delays a member response. Consequently, an
  > implementation that treats a partial transcript as success or emits only a generic timeout can
  > satisfy every listed mutation. Add an env-gated control that withholds one seat's answer and
  > assert that the shared bounded-wait helper fails with the actual line count.
