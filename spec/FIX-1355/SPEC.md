...
| **asks whether a seat can call a tool its file gave it** | A `tools:` list is a setting. Nothing has watched a seat act on one | The seat calls the block its file names and answers with what only that call returns; its teammate, naming none, cannot |
...
```diff
  # teams/pentest/workers/recon/WORKER.md
+ ---
+ description: Sweeps the target list and reports what answered.
+ tools: [scope-check]      # the lab's own block, fenced to this seat alone
+ ---
```
...
flowchart LR
  ...
  S1 -->|"calls the block its tools: names"| TL["scope-check · the lab's block"]
  TL -.->|"returns what no file holds"| S1
...
- **Capabilities and `uses`.** The multi-tool package door, and FIX-1388's. A seat here
  reaches one block through `tools:`; the lab adds no second way in.
...
2. **[D2](DECISIONS.md#d2) · Two checks — a model-free contract gate, and a model-backed honesty check required at completion**, the latter now also grading a real tool call.
