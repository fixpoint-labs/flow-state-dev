...
2. **The default kind ships from the package that already does hiring, and anything heavier
...
3. **Each seat's skills are stored in that seat's own drawer, using the per-copy
   ...
   *Rejected:* keeping one shared drawer and filtering per seat, which is today's shape and is
   exactly the privacy-in-name-only problem in §1; also rejected: inventing a seat-scoped
   storage concept, when an existing flag already carries the seat's identity into the key.
   *Locks in:* a company-wide skill is **copied into** each seat when that seat first runs,
   not shared live. Editing the company copy afterwards does not reach into seats already
   running — refreshing them is a deliberate act, not something that happens quietly
   underneath a team.
   *Named after review, not re-decided:* separating the drawers and filling each one are two
   ...
   separates them (run, not read). Getting *that seat's* skills into *that seat's* drawer is
   a handoff that has to be built, and C3 now says so rather than leaving it implied.
