...
`worker-agent` — `packages/workforce/README.md`, `goals/`, and `workerAgentFlow` in
`packages/workforce/test/hire.test.ts`. The built-in kind id is `agent`. Migrating that
teaching surface is FIX-1366's; until it lands, an implementer reading an existing example
should not carry `worker-agent` out of it.
...
(FIX-1365) will make. Criteria 4, 5 and 6 were sharpened by the spec review: as first written,
4 was unsatisfiable and 5 could pass on a roster where every seat held the same catalog:
