...
The built-in is **already taught** — and taught well — two-thirds down a page about the file
format. The overview never mentions it. So the content is not missing; it is unfindable, sitting
beside a placeholder name that reads like a shipped default, with an options list that
undercounts the real API.
...
**No new page.** The front door is reshaped to answer the question it is asked, and links into
the section that already holds the answer. That section grows only where it was thin.
...
Three calls. The branch worth reading is the dashed one under Decision 1: this spec argued for a
new page, review produced the page that already existed, and the direction reversed. The
classifier is documented but **not promoted** — opt-in, off by default.
