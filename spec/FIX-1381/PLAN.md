...
| S3 | `workforce` · resolve the grant | Match each ref against the app's declared documents; refuse what does not match; build this seat's narrowed map, `ro` entries unwritable **and** closed to the model's write tool (D3). Collected, not thrown per seat | BR-1 BR-3 BR-9 BR-13 BR-14 |
| S4 | `workforce` · the hire step | Pass the narrowed map as that seat's own resource map at the mint. A seat with no declaration passes nothing and mints as today (D1) | BR-3 BR-4 BR-7 BR-8 BR-12 |
...
flowchart TD
  ...
  S2 --> S3["S3 · resolve against declared documents"]
...
| V6 | S4 | The kind's own block resources stay reachable and writable on the narrowest seat (BR-7); a name collision resolves as today (BR-8) |
...
**POC:** `spec-poc/FIX-1381-seat-resource-allowlist/`, seven cases, run with
`pnpm exec vitest run --config spec-poc/FIX-1381-seat-resource-allowlist/vitest.config.ts`.
Four premises pinned, **all four held** ([DECISIONS.md](DECISIONS.md) → *Settled*). Nothing
changed because of it — worth reporting rather than passing silently.
