...
decided without asking, and what was left alone — which on a refactor is most of the value.
...
<a name="d2"></a>
## D2 · The symlinked-root refusal lands in the shared open, closing FIX-1375's workforce half
...
| **Because** | A flag whose only purpose is keeping two of four readers wrong is the drift this issue exists to end, and the next ticket would remove it. Both readers' headers already say symlinks are never followed at any level; only the code disagrees. This issue's locked guidance says symlink honesty lands **once**, in the shared walk |
| **Locks in** | One behaviour changes. An operator whose root is a symlink gets a thrown error where a roster used to load — loud, at boot, reversible in one line. But it is a behaviour change inside a change otherwise sold as pure movement, so it is signed as one, and it is why the changeset is `minor` |

**Half of FIX-1375, not all of it.** The two readers it names inside this package are fixed
here. Its third, `orchestration`'s skills-folder reader, is across the boundary and keeps the
unguarded root — so the issue is not closed by this PR.
...
- **The worker reader's failures gain a `kind` tag.** The other three carry one, and a shared
  enumerator must report into one shape. Callers reading `errors` are unaffected; callers
  *constructing* the type are not — half of why the changeset is `minor`.
- **`openRoot` throws rather than collecting.** All four readers throw on a bad root already;
  the channels and resources wording is kept.
- **The org folder open stays in the resources reader.** One caller, two lines. Moving it would
  hand the channels reader an `org/` scope it is not allowed to use.
...
- **A `minor` changeset, not a `patch`.** The pre-1.0 rule is "can this break somebody", and
  twice it can: a symlinked root that boots today throws after this, and the `kind` every worker
  failure now carries stops a mock or constructor of the exported result type compiling.
...
**None.** One coordination question is live, but it is sequencing rather than a fork, and is
recorded so no reviewer reads it as undecided.

**Who exports the primitives — this issue or FIX-1357?** That spec, now Spec Approved, publishes
the same primitives from the loader subpath and names the three ignore-list copies as a net
subtraction. Not the same work: FIX-1357 **publishes** what exists, FIX-1389 **creates** what
does not, and the overlap is exactly the ignore list. Whichever lands first does it; the other
finds it done. Neither blocks, neither re-gates. Flagged up to the epic
([ER-14](https://github.com/fixpoint-labs/flow-state-dev/pull/1718)), whose seam table has no row
for it.

## How it got here

- **Draft (Sep 17)** — measured the duplication before designing it. Two of the four walk
  primitives were already extracted, which shrank the issue; the team walk and root open were
  not. The enumerator was chosen over a slot reader on FIX-1368's depth, and the root refusal
  folded in on the issue's locked guidance.
- **Review round 1 (Sep 17)** — direction ratified, no decision changed. The red state moved to
  the mutation (V3): the new team-folder cases characterize behaviour already present. Changeset
  raised to `minor`, BR-3 corrected to spare org-level output, FIX-1375 narrowed to its
  workforce half.
