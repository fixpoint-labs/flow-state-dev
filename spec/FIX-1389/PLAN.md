...
|---|---|---|---|
| S1 | `workforce` · the channels and resources test suites | **Write first, red.** A symlinked team folder and an unreadable one, a case each, mirroring the worker suite's existing pair. They fail on today's code — that is the point | BR-5 BR-6 |
...
| S5 | `workforce` · the worker reader's result type | Its failure entries gain `kind`, so the enumerator reports into one shape. Reuse the siblings' `unreadable-slot` for structural conditions | BR-9 |
...
| S7 | Docs · changeset | `packages/workforce/README.md`: the loader entry gains the primitives. One `patch` changeset. **No `apps/docs` change** — none of this is author-facing | — |
...
```mermaid
flowchart TD
  S1["S1 · the two missing tests, red"] --> S2["S2 · the shared primitives"]
...
|---|---|---|
| V1 | S1 | Both new cases fail against unmodified code, naming the missing report — then pass after S3. Green before S2 means the test is wrong, not the code |
...
| V3 | S3 | **The mutation, re-run.** Fold the team-folder reporting out of the enumerator: at least three suites must fail. One failure means the extract inherited the coverage gap |
...
The enumerator's name and signature, and whether it is a generator or takes a callback, are
yours. `forEachTeam` in the spec's diff is illustrative.
...
- **EXTEND** `packages/workforce/README.md` — one line on the loader entry, naming the
  primitives a convention consumes. *Voice risk:* it is a list of exports, not a tutorial.
- **One `patch` changeset** — name the root refusal as well as the widened type. An operator
  reading only the changeset should learn about the refusal.
...
times, twice without the symlink guard.

## At implement time

All three were in flight when this was written. Re-check them before building:
