...
|---|---|---|---|
| S0 | `spec/FIX-1394/evidence/` · the factual base | **Already built and green.** Re-derives BR-1 to BR-5 from the repo; `--negative-control` plants a sixth convention reader and confirms C1 goes red | BR-1 – BR-5 |
| S1 | `spec-poc/FIX-1394-probes/` · the probe harness | The six probes as one variant-agnostic harness: give it a built variant, get a cell per probe. Written **before** any variant (D2) | BR-8 – BR-14 |
| S2 | variant A · reuse `SKILL.md` | The existing skill format as the one package — instructions, `allowed-tools`, bundled `files[]` — attached always-on, held opt-in | BR-9 – BR-14 |
| S3 | variant B · reuse the seat folder | A file plus colocated `skills/`, `resources/`, `blocks/` as the package, with a library copy attached opt-in | BR-9 – BR-14 |
| S4 | variant C · a new package file | Present because reuse-vs-create is the owner's to close (ER-13), not because it is recommended | BR-9 – BR-14 |
| S5 | variant D · don't collapse | Write the placement rule and close only the real gap: a package's tool failing silently instead of naming the line to add (D1's *Locks in*) | BR-9 – BR-15 |
| S6 | `spec/FIX-1394/RATIFY.md` | The filled matrix, the chosen variant, what it means for ER-2. Mirrored to the epic-spec | BR-15 |
...
|---|---|---|
| V0 | S0 | All 20 claims green; `--negative-control` prints PASS. Re-run before the ratify, not only at draft |
| V1 | S1 | The harness reports a cell for all six probes against a deliberately empty variant, and every cell reads *fail*. A harness that cannot produce a red column cannot report one (tenet 7) |
| V2 | S2 – S5 | Each variant has a recorded cell for **all six** probes, including probes it was not built for. A blank cell is a result nobody has |
| V3 | S2 – S5 | BR-13 re-runs S0's C4 assertions **with the variant's package attached**. The gate holds or the variant fails, whatever else it passed |
| V4 | S2 – S5 | BR-14: the existing workforce suite passes unmodified against a tree with no package authored |
...
byte as today (BR-14, V4). Every variant is checked there, not only where it looks good.
...
| Record every cell, including expected failures and probes a variant was not built for (BR-8) | A matrix with blanks is a matrix that can be read whichever way its author prefers |
...
**No docs change from this issue.** No format ships and no file's meaning moves. The docs work
belongs to whichever ship ticket the ratify cuts, and it is substantial —
`apps/docs/docs/workforce/` and `apps/docs/docs/skills/` both teach the current split, and a
collapse rewrites both. Flagged so the ratify prices it rather than discovering it.
...
risk:* the honest version of this rule sounds like an apology. It is a design decision.
...
- **Check whether the [Open](DECISIONS.md#open) question was answered** before fixing the probe
  set. If the five session walls stay, the set is bigger than six and S1 needs re-shaping.

## Follow-ups

- **An opt-in document has no mechanism** (BR-5). If P3 fails in all four variants, that is a
  separate issue and needs filing: a file-declared resource installs at flow level and is refused
  `prefetchMode: "lazy"`, so nothing can load one on activation.
