...
| BR-10 | The roster the binder holds names different members than the open channel does | The row holds what the **open channel** holds, not the roster. The binder never writes membership. A file edit does **not** reach an open channel, so it does not reach the row either — what the row buys is that the disagreement is visible rather than silent | CI · V10 — `openInventory` alone over a roster naming different members; red state is the row taking the roster's |
