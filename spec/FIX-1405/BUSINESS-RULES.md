...
| BR-10 | The roster the binder holds names different members than the open channel does | The row holds what the **open channel** holds, not the roster. The binder never writes membership; a re-bind refreshing the channel's own projection from the file is how a file edit reaches the row, one boot later | CI · V10 — `openInventory` alone over a roster naming different members; red state is the row taking the roster's |
