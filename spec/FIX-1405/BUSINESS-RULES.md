...
| BR-17 | A seat asks which channels it is in | One prefix read of `inventory/members/<seatId>/`, never a read of every channel row followed by a scan of each one's member array (BP-033). The narrowing is applied by the runtime after it loads the collection, not pushed into the store — see [DECISIONS → Why a third collection](DECISIONS.md) | CI · the red state is the channel collection being read and filtered on the `members` field |
