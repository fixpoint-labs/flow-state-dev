...
| BR-10 | The roster the binder holds names different members than the open channel does | The row holds what the **open channel** holds, not the roster. The binder never writes membership. An edit to a file's `members:` does not reach an open channel today, so it does not reach the row either — what the row buys is that the disagreement is visible rather than silent. If a later change re-derives an open channel's projection, the row follows it by construction, because the channel is still the writer | CI · V10 — `openInventory` alone over a roster naming different members; red state is the row taking the roster's |
...
| BR-17 | A seat asks which channels it is in | One prefix read of `inventory/members/<seatId>/`, never a read of every channel row followed by a scan of each one's member array (BP-033). The narrowing is applied by the runtime after it loads the collection, not pushed into the store — see [DECISIONS → Why a third collection](DECISIONS.md#the-membership-index) | CI · the red state is the channel collection being read and filtered on the `members` field |
