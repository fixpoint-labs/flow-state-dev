...
index is **not** the cheaper one.

How the two compare on volume is deliberately not the argument here. It depends on how many
members a channel has, so it is a claim that needs a condition attached, and the decision never
rested on it — an earlier draft asserted the index moved less, a later one asserted it moved more,
and both were reasons invented after the fact. The load-bearing part needs no condition:
...
| Filter membership in memory over the channel collection | What round 1 caught, though the reason is narrower than it first looked. Both shapes list-then-discard today, so this is not ruled out on cost; what rules it out is that `members` is a field, so no prefix could ever narrow it at the store — see [Why a third collection](#the-membership-index) |
