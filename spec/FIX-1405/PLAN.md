...
| Every inventory read narrows by key (BP-033) | `list()` narrows by key prefix only, and the runtime applies that prefix after loading the collection — so the index is not a smaller read today, and on any real roster is the worse of the two. It is the only shape a source-side narrowing could ever reach: a question the keys cannot express needs a key that can, not a loop over the collection ([DECISIONS → Why a third collection](DECISIONS.md#the-membership-index)) |
