...
| **edits a `CHANNEL.md` after the channel opened** | The edit does not reach the channel at all — a re-bind leaves an open channel exactly as it is — and nothing at run time reports either state | The inventory mirrors what the open channel holds, never what the file says, so the gap between the two is **visible** instead of silent. **The edit still does not reach the open channel**, and nothing in this wave changes that |
