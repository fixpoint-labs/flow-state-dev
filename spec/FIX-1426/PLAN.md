...
| S3 | `goals/devforce-lab/lab/workforce/flows/workers/coder.ts` | The working kind: a `task.actions` entry mounting `harnessManager`, **plus its own board declaration** (same `boardId`, same ledger id) and a same-flow gating dispatcher, per D1's settled constraint. The harness is a **slot**. **Comment the board declaration as the interim L1 tax it is** (soft→FIX-1408), so the next lab does not copy it as the way boards are declared — authoring is channel-attached, FIX-1385 | BR-6 BR-10 BR-11 BR-12 |
