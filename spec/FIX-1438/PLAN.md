...
|---|---|---|---|
| S1 | `harness-manager` · the phase contract | A completion-check context that carries the run's outcome, sitting beside the prompt context that carries the ask. The shared base keeps what both need; the outcome goes on the completion side only, mirroring how the carry-forward reason sits on the prompt side only (D1) | BR-1 BR-2 BR-3 BR-4 BR-6 |
| S2 | `harness-manager` · the verdict handler's completion arm | Hand the reported outcome word to the check. Compare nothing on it. The value crosses as reported — not narrowed to a boolean, not defaulted when unrecognised (D1) | BR-1 BR-2 BR-3 BR-4 BR-5 BR-7 BR-11 |
...
```mermaid
flowchart TD
  S1["S1 · the completion-check context"] --> S2["S2 · hand the outcome down"]
...
|---|---|---|
| V1 | S1 S2 | A stubbed run reporting each of `finished`, `stopped-at-limit`, a word the framework does not define, and nothing at all reaches the check as four distinguishable values (BR-1 BR-2 BR-3 BR-4). The unrecognised word is not mapped to anything |
| V2 | S2 | A check that never reads the outcome settles its rows exactly as before, on every one of those four (BR-7). This is [D1](DECISIONS.md#d1)'s cost asserted, not assumed — it is the check that must go red if S2 ever grows a rule of its own |
...
| V4 | S1 | No outcome is reachable from a prompt builder's context (BR-6) |
...
|---|---|---|
| The outcome word | `stopped-at-limit` | The framework's own vocabulary, already public on the harness contract. A phase author types it |
...
Everything else is yours to name, including what the completion-check context type is called.

## Guardrails

| Rule | Because |
|---|---|
| The outcome reaches the check as reported — never narrowed, never defaulted when unrecognised (BP-030) | A vendor word this version does not know, silently becoming `finished`, is the same silent partial success in a new place |
...
| The outcome is absent from the prompt builder's context, always | Its mirror is already a rule: the carry-forward reason is kept off the completion check because it describes the *previous* attempt. A value that means "this attempt" in one place and "the last one" in another lies by position |
| The manager compares nothing on the outcome except to phrase a failure reason | A comparison there makes the run's narration an authority over the done-condition — the inversion this issue locks out |
...
the phase contract:
    ...
    the completion side    → + how this run stopped            ← the whole change

the manager, after a clean verdict:
    ask the phase: done?, handing it the run's outcome word as reported
...
- Check whether the verdict handler still widens the outcome to a plain string on its input schema. It does today, deliberately; whether the value is narrowed on the way to the check is yours, subject to the guardrail that an unrecognised word must survive.
