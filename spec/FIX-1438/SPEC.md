...
A supervised run reports two things when it ends: whether it ended cleanly, and *how* it stopped. Only the first is read. The second — the framework's own three-way word, one of `finished`, `stopped-at-limit`, `failed` — is written to the run record and never consulted by anything that decides.

## What changes

![Two columns. Today, a finished run reports both a status and an outcome word, only the status reaches the completion check, the outcome word goes nowhere, and the row settles completed. After, both reach the check, the check answers not done, and the row re-opens with the reason attached.](figures/what-changes.svg)

Follow the second line out of each box. Today it dead-ends: `stopped-at-limit` is recorded and no decision reads it, so the phase's check answers from the branch alone and a half-finished commit closes the row. After, the same word reaches the check, and the phase — still the only thing that decides — has what it needs to say *not done*.
...
+   if (run.outcome === "stopped-at-limit") return false
...
The manager still reads `status` alone to decide whether the attempt failed, and still asks the phase whether the job is done. The only new edge is the second one into the check. The manager compares nothing on the outcome word.
...
- **The prompt builder's context.** The outcome is not on it; a prompt still learns about the last attempt through `feedback`.

## Sign off

1. **[D1](DECISIONS.md#d1) · The phase's completion check receives the run's outcome word; the manager still decides nothing on it.** If wrong: every phase carries one more call the framework cannot enforce, so a phase that forgets closes rows on partial work exactly as today, silently.
