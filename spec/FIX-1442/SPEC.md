...
3. **[D3](DECISIONS.md#d3) · Reserve `__fsd_default_org__` for unauthenticated single-organization operation.** Recommend one exported `DEFAULT_ORG_ID`; configured authentication cannot claim it. Check existing data before upgrading; a collision requires an offline rename before reopening. Evidence of a collision before release changes the proposed spelling. **If wrong:** changing the value after data is written requires a migration.
