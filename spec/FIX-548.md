...
flowchart TB
  ...
  E --> F[6 · install<br/>resolved package manager]
...
**Step 6 — install.** Run the resolved package manager's install in the target directory. One
manager, one lockfile. The manifest's `"type": "module"` and its dependency set must already be in
place, because the package manager reads that file and rewrites it.
...
| step 6, the install | everything above, plus the key | `npm install` in that directory. **Exactly the promise §9 makes** |
...
6. **The install** — run the resolved package manager, surface its failure, and print the recovery
   that genuinely completes the project. *Test:* the failure path leaves every file and every edit
   on disk and prints the install command against that directory. *Depends on:* 3, 5.
...
|---|---|---|
| a | the package: invocation surface, bare-app step, the template, the `AGENTS.md` append, the credential stop, the install, and the printed block (steps 1–7) | — |
...
| The install fails (step 6) | The scaffold stays and is **genuinely finishable**: the six template files, all four changes, the `AGENTS.md` section and the key are already on disk, so the developer's own package-manager install is the *only* thing outstanding and it completes the project. Report the manager's own output and print that command against that directory. **Two earlier revisions made this claim falsely**, and both are why the pipeline reads as it does: the first appended `AGENTS.md` *after* the install, so the section had not been written; the second appended it before but let that append fail and continue, so the section might not exist. The claim holds now because step 4 precedes this **and** a failed step 4 ends the run (§7). |
...
**A failed install can**: the six template files, all four changes, the `AGENTS.md` section and the
key are already on disk, so the developer's own install command against that directory completes
it. **Two failures cannot, and both end in inspect-and-restart.** A failed `create-next-app`: the
...
**"Finishable" is a claim about what is on disk, and it broke twice in two different ways.** First
...
The test is one question asked in both directions: **name the scenario where this check comes out
the other way, and confirm that scenario is reachable.** If nothing makes it fail, it proves
nothing; if nothing makes it pass, it is not a check but an obstacle. **The fourth item above is
the one that shows why the question has to be asked of the *predicate*, not the intent** — "assert
the ordering" sounds like it covers the failure path, and it does not; only "assert the run stops"
does. Recorded here rather than in a review thread because a reader who meets these separately
files them as unrelated mistakes and re-derives the lesson each time.
