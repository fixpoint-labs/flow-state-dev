...
1. **The package is `create-flow-state`, published unscoped.** Invoked as `npm create
   flow-state my-app`, or `npx create-flow-state my-app`.
   *Rejected:* `create-fsd-app` and `create-fsdev-app` (both considered by FIX-1162), and
   publishing under the `@flow-state-dev/` scope as this spec's previous draft proposed.
   *Locks in:* the headline command matches the published brand and reads as a sentence. The
   owner holds `create-flow-state`, `create-fsd-app` and `flow-state-app` on npm as of today, so
   **FIX-1162 no longer gates this issue by any route** — not just the one the previous draft
   worked around. It also makes FIX-1162's recorded finding stale: `create-fsd-app` was reported
   unobtainable and it is now ours, so the epic's `create-fsdev-app` prose and FIX-1162's own
   proposal both need correcting (§12).
...
7. **No path writes the API key before it has proven git will not publish it.** The template's
   host scaffold happens to supply the ignore entry (`create-next-app`'s `.gitignore` already
   carries `.env*` — verified), so this is an assertion, not a code branch: check, add ours if
   absent, and only then let init write the key.
...
**The trade-off.** Two templates cost a second recorded goal run, on machines we do not control,
on every release, forever — the epic named that as an accepted cost when the Node template still
...
**The version is pinned in the spawned specifier, not left to `npx` to resolve.** The command is
`create-next-app@<tested major>`, held in one constant in this package and bumped as a deliberate
change with a goal-check run behind it — never a bare `create-next-app`, which resolves whatever
shipped that morning. The pin is what makes §9's flag-drift row a *loud* failure at our next
release rather than a silent one in a stranger's terminal, and the CI argv assertion (§10) reads
the pinned specifier so removing it fails a test. The pin cannot detect upstream drift on its own
— nothing stubbed can — so the goal check is the detector and the pin is what stops the detection
gap reaching users in between.
...
| `app/api/flows/[...path]/route.ts` | `createNextHandler(flowstate)` from `@flow-state-dev/next` — the platform-agnostic adapter, one line, no Vercel assumption |
| `app/api/flows/route.ts` | the sibling bare route. `list_flows` is registered as `GET /` on the mount base and the client's `listFlows()` requests it, but a required catch-all cannot match zero segments. `@flow-state-dev/next` exports no bare handler (only `@flow-state-dev/vercel` does), so this one forwards by hand with a comment saying why |
...
scaffolds a real `create-next-app@16` project and runs both against every candidate shape — run
it with `bash spec-poc/FIX-548-next-template-shape/probe.sh`. It settled decision 4 and turned up
four facts nothing in this epic knew: `create-next-app` writes its **own** `AGENTS.md` (with its
own delimiters, re-added by `next dev`) and a `CLAUDE.md`; its `.gitignore` already carries
`.env*`; and `--agents-md` defaults on from saved preferences. The first of those matters most —
**the greenfield Next path appends to an existing `AGENTS.md`, it never creates one**, which is
the case everyone had filed under brownfield.
...
| `.env.local` would be written and git would not ignore it | Add the ignore entry first; if it cannot be added, refuse before the key is written (decision 7). |
...
| `create-next-app` changes its flag surface or generated layout | The pinned specifier (§7 step 2) means an upstream change does not reach users until we bump it, so this surfaces as a failing goal check on *our* release rather than as a hang in a stranger's terminal. Treated as a patch-level fix. Nothing stubbed can detect this — the goal check is the only detector. |
...
**Taxonomy:** refusals leave the disk untouched; failures leave a directory the developer can
inspect and finish by hand. Nothing is cleaned up behind the user, because deleting a directory
a stranger may have put something in is worse than leaving one they can `rm`.

### 10. Testing strategy

**Goal & goal check** (the epic's proof for this issue). One recorded run under
`goals/create-flow-state/` — or two, if decision 2 is overturned:
...
- **`create-next-app` is invoked with install skipped, every flag explicit, and the pinned major
  in the specifier** — asserted on the argv, since the symptoms otherwise are "the first hour is
...
- **FIX-1162 no longer gates this issue at all** (decision 1). The owner holds the name.
...
- **The epic's prose and FIX-1162's proposal are stale on the name.** Both say
  `create-fsdev-app` on the strength of `create-fsd-app` being unobtainable. The owner now holds
  `create-flow-state`, `create-fsd-app` and `flow-state-app`; decision 1 picks the first.
- **`create-next-app` writes `AGENTS.md` and `CLAUDE.md` itself**, and `next dev` re-adds its
  own block. Epic Q1's append-never-overwrite guarantee therefore binds the *greenfield* path
  too, which the epic files under brownfield. The POC confirmed an appended FSD section survives
  a `next dev` run.
...
on a question the spec can answer either way. The answer changes whether §10 records one goal run
or three-per-template, and whether decision 2b's flag exists. Nothing else.

**Verified premises** (measured or read from the code, so a reviewer need not re-derive them):

- **Module loading in a real `create-next-app@16` project, probed** (the POC in §7): Turbopack
  does **not** resolve an extensionless import to a `.mts` file (`Module not found`); an explicit
  `.mts` specifier fails TypeScript with `TS5097` unless `allowImportingTsExtensions` is set;
  `.mts` with that flag builds; `fsdev.config.ts` with `"type": "module"` builds **and** loads
  under a native `import()` with no warning; without the `type` field the same file loads but
  emits `MODULE_TYPELESS_PACKAGE_JSON`. Confirmed end to end: `next dev` served `GET /api/flows`
...
- **A POC settled the shape the re-draft depends on.** Owning the template end to end means
  ...
  turned up that `create-next-app` writes its own `AGENTS.md` and `CLAUDE.md`, that `next dev`
  re-adds its block, and that its `.gitignore` already ignores `.env*` — so the greenfield path
  *appends* to an agent-instructions file, a case the epic had filed under brownfield.
