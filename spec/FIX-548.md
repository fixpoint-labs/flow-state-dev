...
2. **One template ships: the Next.js chat app — conditional on a supported minimal-project path
   ...
   **This document is written against one template throughout, and shipping two is not a
   ...
   the four changes it makes are Next-specific (`allowImportingTsExtensions`, and the `next dev` /
...
`create-next-app` unconditionally, three of its four changes to scaffolder-written files are Next
...
flowchart TB
  ...
  B --> C[3 · write the template<br/>6 files · + 4 changes to files create-next-app wrote]
...
`"type": "module"` and the two loopback-narrowed scripts from step 3, and **no FSD dependencies
...
| **step 4, the `AGENTS.md` append** | the six template files and all four changes; **no key, no dependencies** | nothing, and it does not try — **the run stops**, so there is no recovery message making a claim about a section that is not there |
...
**A failed dependency add can**: the six template files, all four changes, the `AGENTS.md` section
