...
8. **The generated project is never reachable off the developer's own machine without
   ...
   - **The demo flow refuses to serve when `process.env.NODE_ENV === "production"`**, on the
     default resolver. Declared on the flow itself, so the request is refused **before any model
     invocation**, with an error naming the missing authentication.

   **The predicate is `=== "production"`, not "outside development", and the difference is the
   whole decision.** `NODE_ENV` is **unset** in an ordinary shell, and the CLI never sets it —
   `packages/cli/src/commands/run.ts` contains no `NODE_ENV` assignment, and neither does anything
   else under `packages/cli/src`. So a guard reading *refuse unless development* would refuse
   **`npm exec -- fsdev run chat send …`, a command this scaffold prints**, on a developer's own
   laptop, before any model call. The guard would fire on the scenario it exists to permit.

   *Named because it keeps happening:* state the scenario a guard exists for, then state the
   scenario it will actually meet most often, and check they are different. Here they were the
   same, and the earlier "outside development" wording is what made it invisible.

   *The codebase has both conventions and this picks one deliberately rather than averaging.*
   `packages/engine`'s block-trace default keys on `NODE_ENV !== "production"`; its store-factory
   default keys on `=== "development"`. For a guard on the printed path the first is the only one
   that works, and it is the one taken.

   *What `=== "production"` costs, stated because it narrows the promise.* It is **fail-open on an
   unset variable**. It fires where it matters — `next start` and every mainstream platform deploy
   set `NODE_ENV=production`, and §10's negative assertion runs `npm run build && npm start` and is
   what proves the mechanism rather than assuming it. It does **not** fire in a hand-rolled host
   that runs neither of the template's scripts and leaves `NODE_ENV` unset. That path ships no file
   of ours, but the gap is real and is not papered over.
   *Rejected for closing it:* having the CLI set `NODE_ENV=development` — that is a change to
   `packages/cli`, which §7 declares untouched, and it would alter behaviour for every existing
   `fsdev run` caller to fix a generated project. Also rejected: a template-written marker variable
   in `.env.local` as the development signal — fail-closed, but it invents a mechanism, adds a
   variable to explain in the first five minutes, and breaks on a developer who copies `.env.local`
   ...
   *This is a product statement, not a footnote — and it is the wording to use, because a looser
   one was wrong twice:*

   > **The generated app refuses to serve its demo flow in production until real authentication is
   > configured. Once it is, the app serves normally wherever it is deployed — container, platform,
   > or bare host. In development it also binds to the local machine only, which is a development
   > default the developer can change in one word.**

   *An earlier revision of this spec, and the PR description built from it, said instead that the
   app "does not serve off-host until authentication is configured — on every path the template
   ships and every mainstream deploy."* **That is false in two directions at once**, and both
   matter for what the owner is being asked to approve:
   - **It promises more than the mechanism can deliver.** A hardcoded `--hostname 127.0.0.1` in a
     `package.json` script is not in the path of a platform deploy at all, so no bind of ours
     governs "off-host" there.
   - **It describes the wrong control, and thereby understates the product.** The thing that
     protects a deployed app is the flow's **refusal to serve in production without
     authentication**, not a bind — and once the developer configures authentication the app is
     meant to serve off-host normally. The old wording reads as though the scaffold permanently
     restricts where the app can run, which would be a worse product than the one being specified.

   For a demo scaffold this is the right default, and the owner should see it as a promise about
   what the thing does rather than as an implementation note.
...
? OPENAI_API_KEY ›  sk-••••••••
  ...
  Added     7 dependencies via npm  (@flow-state-dev/*, @ai-sdk/openai, zod)
...
- **Two negative assertions, because every control here is otherwise proved only by its happy
  ...
  - **With the app built and started in production mode on the default resolver, a request to the
    ...
    Run it as `npm run build && npm start` — **this is also what proves the guard's predicate is
    right**, since it establishes that the real path actually sets `NODE_ENV=production` rather
    than assuming it (decision 8).
  - **And the complement, on the same run: `npm exec -- fsdev run chat send …` succeeds.** The
    guard must *not* fire on the command the scaffold prints, where `NODE_ENV` is unset. Without
    this assertion the pair above passes on a guard that refuses everything, which is precisely
    the shape the "outside development" wording would have shipped.
