...
three lines above them. `Updated` is not cosmetic: those two keys are edits to files
`create-next-app` wrote, so a summary listing only new paths would silently omit the two changes
a reader is most likely to want flagged. Everything from `Wrote` down is **init's output, printed by init** (epic theme 5) — that is
...
- **the run reports every change it makes to a file it did not create** — the two decision-4 keys,
  in `package.json` and `tsconfig.json`. They are edits to `create-next-app`'s files rather than
  new paths, so a filename-only summary misses them entirely, and they are precisely the changes
  a reader would be surprised by. The epic's proof compares the run's report against the actual
  diff; a report that omits its own edits fails that comparison for a real reason
