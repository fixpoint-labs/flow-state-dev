...
3. **`create-flow-state` writes the whole FSD half of the template as real checked-in files —
   ...
   *Locks in:* the template is a directory a person can read and diff, which is what checking it
   ...
   held to the contract at all. It also means init's
...
4. **The template's config is `fsdev.config.ts`, and the template adds `"type": "module"` to the
   ...
   *This changes the file set and weakens one argument above.* The tsconfig key is a **seventh
   materialised change** (§7), not a free one. And `allowImportingTsExtensions` was cited above as
   ...
   **22.18** — `packages/cli/src/load-config.ts` says so in its runtime-floor note and in the
   ...
   and the message init uses — not late, as a config-load error, after a project exists.
...
5. **The demo flow reaches whichever provider the developer has a key for, through an intent —
   ...
   google] }, providerPreference: [<the chosen provider>] }`, and only the chosen provider's
   `@ai-sdk/*` package is installed.

   **`providerPreference` is required, not decoration.** Without it the developer's choice can be
   silently discarded: if `AI_GATEWAY_API_KEY` or `OPENROUTER_API_KEY` is already in the
   environment, `providerDetection`'s phase 3 marks **every** gateway-supported provider
   available, the intent's fixed candidate order stands, and the first entry wins through the
   gateway while the direct key just collected goes unused. A developer with a gateway key in
   their shell is common, not an edge case, and no clean-environment run can see it — which is
   why it is stated here rather than left to the goal check. `providerPreference` stable-reorders
   each intent's candidates before availability filtering, so the chosen provider is tried first
   and the fallback order behind it is unchanged. Existing framework surface; nothing new.
...
**Who prints what.** The `Created` and `Updated` blocks are **this package's**, along with the
three lines above them. The split is by **what the run did to the developer's directory**, not by
which of our steps did it: `Created` is a path that did not exist, `Updated` is any change to a
file `create-next-app` wrote — two added keys and one wholesale replacement. `app/page.tsx` sits
in `Updated` for that reason. It is a template file, but reporting it as *created* would
misdescribe the one change most likely to surprise someone who had glanced at the stock page,
and a summary listing only new paths would omit all three of the run's edits. Everything from `Wrote` down is **init's output, printed by init** (epic theme 5) — that is
where the byte-identical-to-a-standalone-run guarantee applies, and the next-steps block is the
part theme 5 actually binds.

*This corrects an earlier draft that put `Created` on init's side of the line.* It could not
hold: §7's step-4 table hands init broad declarations that config, flows and wiring exist and
**never a file list**, so init cannot name six files it did not author and was not told about.
The two ways out were widening the seam — giving back the narrowness decision 3 bought — or
printing a summary nobody can know is correct. Neither is needed, because the wrapper wrote those
files and knows exactly what they are. It also matters more than a formatting nit: the epic's
goal check for this issue **compares the run's report against the actual diff**, so a summary
that cannot name what was written is a check that cannot pass.

Two details in the block are load-bearing rather than cosmetic: `npm exec` needs the `--` separator or npm consumes `--input` as its own configuration
...
**two keys in files `create-next-app` wrote** — `"type": "module"` in the manifest and
`allowImportingTsExtensions` in `tsconfig.json`. Both are decision 4's, and the second is what
makes the config loadable by the CLI at all. This is the entire product surface:
...
| *tsconfig* | `allowImportingTsExtensions: true` added — decision 4. Required, not stylistic: the config imports the demo flow, native Node will not infer the extension, and the `.ts` specifier that satisfies Node is a `TS5097` error without this key. Nothing else in the file is touched |
...
**Not** crossing the seam: the list of template files. This package prints its own `Created`
summary (§5) because it is the only side that knows what it wrote — init receives declarations,
not filenames. Keeping it out is what holds the seam at the width decision 3 chose, and it is
what lets the run's report be compared against the actual diff (§10).
...
3. **The template** — the six files **and both edits to files `create-next-app` wrote**:
   `"type": "module"` in `package.json` and `allowImportingTsExtensions` in `tsconfig.json`.
   Neither is optional and the second is not cosmetic — without it the config's `.ts` import of
   the demo flow is a `TS5097` build error, and with an extensionless import instead the CLI
   cannot load the config at all (decision 4).
   *Test:* the generated project builds and its mounted route serves; **the config loads under a
   ...
   with no warning; `.env.local` is ignored by git before the key is written.
   *Depends on:* nothing.
...
| Node runtime below the floor the generated project needs | Refuse in step 1, before anything is created, with the same floor and the same message init uses. The floor is **22.18** — native type stripping, without which every printed `fsdev` command fails to load `fsdev.config.ts` (decision 4). Do **not** narrow the package's `engines` below the repo-wide `>=22` — that is the scaffolder's own runtime, a different question; FIX-1159 §9 records why. |
...
init does not author the six template files, so it cannot complete the shape; and applying the
template to a directory somebody else's tool partly filled is the brownfield case, which is an
explicit non-goal. A resume mode would need all three of those to change, and it would be a
...
**An init failure can**: the six template files and all three edits are already on disk, so
init's own retry command against that directory completes it. **A failed `create-next-app`
cannot**: the target is partly populated by a tool we do not control, step 1 refuses a non-empty
target, init does not author the template, and brownfield is a non-goal — so there the
...
- **the `Created` summary names exactly the template files this package materialised** — compare
  ...
  3 but before init is called. That window contains exactly this package's writes. Everything
  outside it is somebody else's and is not in this list — `create-next-app`'s own scaffold before
  it, and init's `.env.local`, `AGENTS.md` append, dependencies and lockfile after it.
- **the run reports every change it makes to a file it did not create** — all **three**: the two
  decision-4 keys in `package.json` and `tsconfig.json`, and the wholesale replacement of
  `app/page.tsx`. None is a new path, so a filename-only summary misses every one of them, and
  they are precisely the changes a reader would be surprised by. Assert the category too, not
  just the presence of the name: a run that reports `app/page.tsx` as *created* has misdescribed
  what it did to a file the developer may have already opened. The epic's proof compares the
  run's report against the actual diff; a report that omits or miscategorises its own edits fails
  that comparison for a real reason
...
- **FIX-1159 blocks implementation, and now constrains part of this specification.** This package
  cannot be built until init's library seam exists, and cannot merge before init merges. §7's
  step-4 table is the required interface; it is **smaller than the previous draft's**, because
  decision 3 moved the config and the mount here.

  **The edge is firmer than this spec previously recorded it.** It used to be the library seam
  plus the shared next-steps block — both about what crosses the seam at runtime and what gets
  printed, neither about this template's own files. Epic theme 9 adds the **wiring contract**,
  which does constrain them. Specification can still proceed in parallel, but not as far as
  before: the shapes below are conformance targets, not local calls.
- **This template conforms to FIX-1159's wiring contract (epic theme 9).** The contract is the
  part both entry paths share, and it covers exactly three things: **the Next.js mount pair**
  (`app/api/flows/route.ts` + `app/api/flows/[...path]/route.ts`), **`fsdev.config.ts`'s shape**
  including its required store profile, and **the dependency set**. This package writes them;
  FIX-1159 specifies them. Everything else in §7 — the demo flow, the chat page, the
  agent-instructions section, the manifest edit — is this issue's own and is outside the
  contract. Recorded so an implementer conforms rather than inventing the mount shape a second
  time; the check is theme 8's parity requirement, that this template and the brownfield Next
  path deliver the same mounted-route shape. **No new artifact, package or dependency comes with
  this**: the contract travels as skill content on the path skill content already takes
  (FIX-1159 authors → FIX-1160 packages → the plugin ships).
