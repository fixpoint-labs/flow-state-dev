...
2. **One template ships: the Next.js chat app — conditional on a supported minimal-project path
   ...
   **This document is written against one template throughout, and shipping two is not a
   ...
   step 2 down assumes Next.js: the pipeline spawns `create-next-app` unconditionally, two of the
   three changes it makes to scaffolder-written files are Next-specific (`allowImportingTsExtensions`,
   which exists because Turbopack and native Node disagree on extensions, and the `next dev` script
   value decision 8 narrows), the mount pair and the chat page are Next files,
...
8. **In development the generated app binds to the developer's machine only. What happens on a
   ...
   ***And the product statement is withdrawn.*** Four successive versions of it were false, each
   ...
   replaces it is a **description, written out in §12** — what the developer gets in development,
   what they get in production, and that the demo flow accepts any caller because it is on the
   framework default resolver. §12 also records why this template carries **no credential** while
   FIX-1159's demo flow carries one: the two are different files, and epic theme 8's matrix makes
   the asymmetry a rule rather than an inconsistency.

   *A note on how this paragraph read one revision ago, because it is the pattern this spec keeps
   producing:* it said the description was "in §12" while §12 contained no such thing — the
   description existed only here, in the sentence pointing elsewhere. **A cross-reference to
   content that does not exist is the same defect as a requirement nothing enforces**, and it
   appeared one round after §13's note was written, in the text of the person who wrote it. It is
   logged rather than quietly corrected, because a note that its own author violates immediately is
   evidence about how much the note is worth.
...
? OPENAI_API_KEY ›  sk-••••••••
  ...
  drive one at a time.
  `npm run dev` serves on 127.0.0.1 only. The demo flow accepts any caller,
  so configure authentication before you deploy this anywhere.
...
nothing is handed back to be printed by someone else.

**The two security lines above changed this round, and *where they live* is the part that matters.**
They previously read *"This app serves on 127.0.0.1 only, and will not serve in production until you
configure authentication."* **Both halves became false** when decision 8 withdrew the `start`
narrowing and the production refusal: `scripts.start` keeps `create-next-app`'s value, so the app
does not serve on loopback only, and nothing refuses a production request. The replacement says the
two things that are true — `npm run dev` binds loopback, and the demo flow accepts any caller.

**But this text is inside FIX-1159's canonical block, so this spec cannot fix it alone**, and that
is a bigger finding than the wording. The block is embedded verbatim and §10 asserts normalized
equality against canonical, which means **a withdrawn promise left in canonical would be re-shipped
by this template to every user, and the equality check would force it to stay there.** §12 carries
it to FIX-1159 as a required change rather than a suggestion. What is *authored* elsewhere is the text
...
**Every package in that argument set carries an exact version, and the versions come from a
per-package map rather than one string.** The scaffolder holds a **version map** — one entry per
package, beside the `create-next-app` pin — and step 6 renders it into the `add` arguments:

```
npm install --save-exact \
  @flow-state-dev/core@<core>  @flow-state-dev/engine@<engine> \
  @flow-state-dev/next@<next>  @flow-state-dev/react@<react> \
  @flow-state-dev/cli@<cli>    @flow-state-dev/devtool@<devtool> \
  @ai-sdk/openai@<sdk>  zod@<zod>
```

**One placeholder cannot version six independently released packages**, and an earlier revision of
this paragraph used one. `.changeset/config.json` declares **`"fixed": []` and `"linked": []`**, so
`core`, `engine`, `next`, `react`, `cli` and `devtool` each move on their own — after any release
where two of them differ, a single `<v>` requests a version that is wrong for one and very likely
does not exist for another, and the scaffolder fails on its own install. *The map is not a
workaround for the missing release group; it is what independent versioning requires.* **A `fixed`
group would collapse it to one entry, and that is FIX-1162's release work — §12 carries it as a
cross-issue note and this spec does not depend on it**, because the map is correct either way.

**`zod` is pinned to a Zod 3 range compatible with `@flow-state-dev/core`, and this is the entry
that breaks a build if it floats.** `packages/core/package.json` depends on `zod: ^3.25.76`, and
core's public block surface types schemas as v3 `ZodTypeAny` — `packages/core/src/types/block.ts`
imports it for `stateSchema` and the connector targets, and `types/agent.ts` for `outputSchema`.
The generated flow imports `z` directly and hands its `z.object(...)` to those APIs. **A bare `zod`
resolves to Zod 4, whose schema types are not assignable to v3 `ZodTypeAny`, so the generated Next
project fails `next build` on a type error before it ever reaches a model.** An earlier revision
left `zod` deliberately floating while pinning the FSD packages, which did not leave the risk
general — it aimed it precisely at the one type boundary the generated flow crosses on every run.

*Whether core's schema boundary should accept Zod 4 is a real question and it is not this
spec's.* Raised as a separate issue rather than carried here: this template must work against the
framework as it ships, and pinning v3 costs nothing if the boundary later widens.

**The exactness is asserted on the written manifest, not on the flag.** `npm install pkg@1.2.3`
does **not** persist `1.2.3` — npm defaults to `save-exact=false` with `save-prefix=^`, so it
writes `^1.2.3`; pnpm behaves the same without `-E`. So the `add` command carries the
manager-appropriate exact flag (`--save-exact` / `-E`), **and §10 asserts the outcome**: every FSD
entry and `zod` in the generated `package.json` is a literal version with no range operator.
Checking the manifest rather than the argv is deliberate — it catches a wrong flag, a manager whose
default changes, and a later lockfile refresh that re-resolves a caret, none of which an argv
assertion sees. *Without this the spec's pinning guarantee would be prose: the argument would say
`1.2.3` and the manifest would say `^1.2.3`, and the drift the pin exists to prevent would arrive
on the developer's next install.*
...
*What it costs, and it is the same cost already accepted once:* bumping the map is routine
...
| The developer deploys the generated app as-is | **Nothing of ours stops them, and nothing of ours refuses the request.** `scripts.start` is unmodified so the app binds wherever Next binds, and the demo flow is on the framework default resolver, so the deployed endpoint runs model calls for any caller, billed to the developer's provider key (§12). This is the state epic **§5 Q7** is deciding; decision 8 withdrew the production refusal an earlier revision of this row mandated. **Do not implement one before Q7 answers** — the epic's rule is explicit and this row is where an implementer would otherwise pick it up. |
...
- **every pinned entry in the generated `package.json` is a literal version, with no range
  operator** — asserted on the **written manifest**, not on the `add` argv. `npm install pkg@1.2.3`
  persists `^1.2.3` unless `--save-exact` is passed, and pnpm behaves the same without `-E`, so an
  argv assertion passes while the manifest drifts on the developer's next install. Checking the
  outcome also catches a manager whose default changes and a lockfile refresh that re-resolves a
  caret. **Assert `zod` specifically**: it is the entry whose drift is a build break rather than a
  version skew (§7)
- **the version map has an entry per package, and the emitted `add` uses it** — assert that the six
  `@flow-state-dev/*` arguments do not all carry the same version string when the map says they
  differ. The failure this guards is a single placeholder rendered into six independently released
  packages, which requests a nonexistent version and fails the scaffolder's own install
...
- **The next-steps block's security lines are FIX-1159's to author, and one of them must change —
  this is a required correction, not a request.** §5's transcript renders two lines that describe
  the bind and the demo flow's openness. An earlier version said *"This app serves on 127.0.0.1
  only, and will not serve in production until you configure authentication."* **Both halves are
  now false** (decision 8 withdrew the `start` narrowing and the production refusal), and the
  replacement is *"`npm run dev` serves on 127.0.0.1 only. The demo flow accepts any caller, so
  configure authentication before you deploy this anywhere."*

  **The mechanism is why this cannot wait.** This package embeds canonical **verbatim** and §10
  asserts normalized equality over the whole block. So if the withdrawn promise survives in
  canonical, **this template re-ships it to every user of the scaffolder, and the equality check
  actively prevents us from correcting it** — the check designed to stop drift would pin a false
  security claim in place. A shipper cannot fix this from its own side by construction.
  *If FIX-1159 declines the wording, the alternative is moving the lines outside the delimiters as
  this package's own prose, which is worse (theme 5 exists to stop unowned per-host prose) — so it
  is raised as a change FIX-1159 owes rather than something assumed or worked around.*
...
- **What a developer actually gets, stated as description rather than as alarm.** This is the
  behaviour decision 8 stopped promising about, written out here because it is what epic §5 Q7 is
  deciding and it should be legible in the spec that ships it:
  - **In development**, `npm run dev` binds `127.0.0.1`. The app is reachable from that machine and
    nowhere else, because we set that default in a script we authored.
  - **In production**, `npm run build && npm start` binds wherever Next binds — `0.0.0.0` by
    default — because we changed nothing about `scripts.start`. Nothing of ours narrows it and
    nothing of ours refuses a request.
  - **The demo flow accepts any caller.** It is on the framework's default resolver, which trusts a
    caller-supplied `body.userId`. So a deployed generated app is **an endpoint that runs model
    calls for anyone who finds it, billed to the developer's provider key, with no tenant
    boundary** — and `git check-ignore` keeps the key out of version control, which is a different
    problem from keeping it out of a stranger's requests.
  - **What closes it** is configuring real authentication, which is what the `AGENTS.md` section
    exists to teach and what the docs surfaces in §11 point at. Until Q7 answers, nothing in the
    generated project does it for them, and this spec does not claim otherwise.
- **The greenfield demo flow carries no credential, and FIX-1159's demo flow does. Both are
  correct, and they are different files.** Stated explicitly because two specs in one epic
  describing "the demo flow" with opposite authentication postures is how a reader ends up
  believing the wrong one. This template writes `src/flows/chat/flow.ts`; the brownfield skill
  writes `flows/hello/flow.mts`, and FIX-1159's Non-goals disclaim "the greenfield command and its
  template in every part". **The asymmetry is a rule, not an oversight** — epic theme 8's matrix is
  the statement of record, and it turns on two questions that cut *across* greenfield/brownfield
  rather than along it:

  | | loopback bind | credential | why |
  |---|---|---|---|
  | **greenfield, mounted-route** (this issue) | `dev` only; **production unresolved, §5 Q7** | **no** | the demo is a browser page, so any token it holds an attacker reads from the same page |
  | brownfield, mounted-route | no — their dev script, theme 6 forbids editing it | yes | the only control available |
  | brownfield, second-process | yes — our printed `fsdev serve --host 127.0.0.1` | yes | both: we author the start command *and* ship no browser client |

  A path gets the **bind** when *we author the command that starts the server*, and the
  **credential** when *the demo ships no browser client to leak it*. This template authors
  `scripts.dev` and ships a chat page, which is exactly why it has the first and not the second.
...
**It runs in the other direction too, and that half was missing until it had cost us twice.** A
*withdrawal* has the same blast radius as an addition: when a promise stops being true, it has to
stop being made **everywhere it appears**, and the surfaces that restate it are exactly the ones
nobody re-reads.

| round | what was withdrawn | where it survived | kind of surface |
|---|---|---|---|
| `scripts.start` narrowing | epic theme 6 exception (c) dropped `start` | decision 8, the **change table**, §8, §10 | table row |
| the production refusal | decision 8, after the rail was measured | the **file table** row, step 3, a §12 premise, a follow-up | table row |
| the same refusal again | decision 8 | **the generated success transcript** | printed output |
| the §12 description | — | decision 8 pointed at a §12 that did not contain it | cross-reference |
| the same refusal, a third time | decision 8 | **§9's deployment row**, still mandating it | table row |

**Look at the last column: four of the five were a row, not a sentence.** That is not a
coincidence and it is the most actionable thing in this note. Prose restates a claim in language a
reader recognises as a restatement; **a table row states it as a fact, in a compressed form, in a
section organised by a different question** — §9 is organised by *failure case*, §7's file table by
*file*, neither by *which control ships*. So a `Ctrl-F` for the withdrawn wording misses them,
because the row never used that wording. **When you withdraw a claim, walk every table and matrix
whose rows could assert it, by reading the rows — not by searching for the phrase.**

**So the rule is one rule with two directions.** *Additions:* name the artifact that will execute
the requirement — a step, a test, a manifest entry, a tracker edge — not merely the paragraph
describing it. *Withdrawals:* enumerate every surface that **restates** the claim before declaring
it withdrawn — the decision, the tables, the transcripts, the printed output, the tests, and
anything downstream embedding it verbatim. **A promise withdrawn in one section and still printed in
another is not withdrawn**; it is worse than before, because the document now disagrees with itself
and the surviving copy is usually the one a user reads.

**The lesson in neither direction is "be more careful."** It is that editing the *statement* feels
like closing the loop and usually is not: a spec has several places that each look authoritative,
and the one a machine or a person actually executes — or reads — is often not the one edited. If
the answer to "is it closed?" is "the spec now says so", it is still open.

**The transcript case is the sharpest, and it generalises past this document.** The withdrawn
promise survived in text this package embeds **verbatim from another issue**, under an equality
check. Left there, the scaffolder would re-ship a false security claim to every user *and the drift
check would prevent the shipper from correcting it*. **When you withdraw a claim, check whether any
downstream artifact embeds it under an equality constraint** — that is where a withdrawal cannot be
completed from your own side.
...
**Four traps to resolve at the keyboard.** The spec is approved and closed; these are the things a
review found that you settle while building, not by re-opening it. Each is real, each is a
one-sitting decision, and none changes the approach.

- **The version map, or a fixed release group — decide it with FIX-1162's release work in front of
  you.** §7 specifies a per-package version map because `.changeset/config.json` has `"fixed": []`
  and `"linked": []`, so the six `@flow-state-dev/*` packages version independently and one shared
  placeholder would request a version that does not exist for at least one of them. **If FIX-1162
  has adopted a `fixed` group by the time you build, the map collapses to a single entry** and you
  should take that — it is strictly less to maintain. If it has not, ship the map. Do not block on
  the question; both are correct.

- **Pinning is asserted on the written manifest, not on the command you typed.** `npm install
  pkg@1.2.3` persists `^1.2.3` — npm defaults to `save-exact=false` with `save-prefix=^`, and pnpm
  needs `-E`. Pass the manager's exact flag **and** assert that the generated `package.json`
  contains literal versions with no range operator. Check the file, not the argv: that also catches
  a manager whose default changes and a lockfile refresh that re-resolves a caret.

- **Do not build a production refusal, whatever you find in older text.** Epic **§5 Q7** is open and
  no issue implements one until it is answered. Earlier revisions of this spec mandated a
  `NODE_ENV === "production"` refusal on the demo flow; it is withdrawn, the rail it assumed does
  not exist on this template's path, and §9's deployment row now says so explicitly. **If you find a
  surviving instruction to add one, it is stale — delete it rather than implement it.**

- **`probe.sh` reads its evidence from process output, and process output is environment-dependent.**
  If you touch it: any check that decides a claim by looking at stdout or stderr must clear whatever
  suppresses the signal it reads — `NODE_NO_WARNINGS` and `NODE_OPTIONS` for Node warnings — or
  classify the environment as cannot-verify (exit `2`). Twice now a hidden signal made the probe
  report a **true** claim as **disproved**, which is the one failure direction that teaches people
  to stop believing a red result.
...
- **The product statement was false in two directions, and it was mine.** This spec, and the PR
  ...
  restriction on where the app may run. Decision 8 took a corrected statement — *the app refuses to
  serve its demo flow in production until real authentication is configured, serves normally
  anywhere once it is, and binds locally in development.* **That correction was itself false and is
  superseded**: the production refusal it named did not exist, as the rail measurement two entries
  below established, and `scripts.start` contradicted the "serves normally anywhere" half. **Read
  decision 8, not this entry** — it now makes no product statement at all. *Left in place rather
  than rewritten, because four successive corrections each looking final is the evidence for why
  the statement was withdrawn instead of fixed a fifth time.*
...
- **The `start` narrowing withdrawn, the production refusal withdrawn, and the product statement
  withdrawn with them — all three because the rail was measured instead of derived.** Epic theme 6
  exception (c) now covers **`dev` only**: narrowing `start` is *harmful*, since *"a platform deploy
  never runs `next start`"* is false of Docker, Render, Railway, Fly, Heroku and App Runner, and
  Next's `-H` carries no environment binding, so `HOST=0.0.0.0` cannot override a hard-coded flag —
  the container binds loopback, health checks never connect, and the app reads as dead. That was
  withdrawn at epic level and **this spec had not followed**, which is the eighth restatement drift
  in this epic and the reason §13's note exists. Separately, a POC ran `executeServeCommand` and
  found the guard keys on the **bind address** rather than the environment, enforces **whole-app**
  rather than per-flow, refuses a **host-authenticated app as a false positive**, and is not on the
  Next mounted route's path at all — confirmed independently here: the symbol appears only under
  `packages/node/` and `packages/cli/src/commands/serve.ts`. So the old decision named a control
  that does not exist and could not be described. **Four successive product statements were false,
  every one of them derived from our own rules rather than measured, and the derivation never once
  failed loudly** — so the statement is withdrawn rather than rewritten a fifth time, and what a
  rail should do is epic **§5 Q7**. §10 keeps the dev-bind assertion, adds its mirror (**the built
  app is reachable from a non-loopback address and `scripts.start` is byte-identical**, which
  asserts the *absence* of a narrowing and so needs no Q7 answer), and records the missing
  production assertion as a decision rather than an oversight. The `--allow-unauthenticated` escape
  is logged in §12 as a hazard for whoever writes deployment docs: it is the sanctioned workaround
  for the false positive and it **disables the guard wholesale**, so anyone recommending it owes
  the reader that sentence.
- **FSD dependencies are pinned to an exact version in the `add` arguments.** `.changeset/config.json`
  declares `"fixed": []` and `"linked": []`, so every FSD package versions independently and, pre-1.0,
  a minor is breaking by convention — meaning bare names resolve whatever `latest` is when a stranger
  runs the command rather than what the checked-in template was built against. **The asymmetry is what
  made it worse than an ordinary risk:** this spec pins `create-next-app` exactly and argues at length
  that a range lets upstream drift reach users, which made the generated project look reproducible
  while its FSD half was not pinned at all. Raised with **FIX-1162** rather than decided alone: a
  `fixed` release group would collapse the pin from N version strings to one, and the exact pin works
  under either answer.

- **The pinning fix had three defects of its own, one of them a build break.** *(1)* It pinned the
  FSD packages and **deliberately left `zod` floating** — which did not leave the risk general, it
  aimed it at the one type boundary the generated flow crosses on every run. `packages/core` depends
  on `zod ^3.25.76` and types its public block surface as v3 `ZodTypeAny` (`types/block.ts` for
  `stateSchema` and connector targets, `types/agent.ts` for `outputSchema`), while a bare `zod`
  resolves to Zod 4, whose schemas are not assignable to it — **so the generated project fails
  `next build` on a type error before reaching a model.** `zod` is now pinned to a compatible v3
  range; whether core's boundary should accept v4 is a framework question raised separately rather
  than carried here. *(2)* It used **one version placeholder for six independently released
  packages**, which `.changeset/config.json` (`"fixed": []`, `"linked": []`) makes impossible —
  after any release where two differ, the single string requests a version that is wrong for one and
  nonexistent for another. Replaced with a **per-package version map**; a `fixed` release group would
  collapse it to one entry, which is FIX-1162's work and a note rather than a dependency, since the
  map is correct either way. *(3)* It assumed `add pkg@1.2.3` persists `1.2.3`. **It does not** — npm
  defaults to `save-prefix=^` and pnpm needs `-E` — so the guarantee was prose while the manifest
  said `^1.2.3`. The exact flag is now specified **and §10 asserts the written manifest**, which is
  the stronger check: it catches a wrong flag, a changed default, and a later lockfile refresh, none
  of which an argv assertion sees.
- **§13's note is sharpened to name tables, because the data says the surface matters.** Four of
  the five withdrawal residuals were a **table row**, not a sentence — including §9's deployment row,
  which still instructed an implementer to build the production refusal decision 8 had withdrawn,
  three surfaces after the withdrawal. Prose restates a claim in language a reader recognises as a
  restatement; **a row states it as a fact, compressed, in a section organised by a different
  question** — §9 by failure case, §7's file table by file, neither by which control ships — so a
  search for the withdrawn wording never finds it. The rule now says to walk every table and matrix
  by **reading the rows**, not by searching for the phrase.
- **The POC's unfreeze clause fired a second time, and was applied without re-arguing it.** The
  native `import()` check reads stderr to decide whether Node's documented
  `MODULE_TYPELESS_PACKAGE_JSON` warning appeared, and warning suppression is inherited: under
  `NODE_NO_WARNINGS=1` or `--no-warnings`, the no-`type`-field variant emits nothing, classifies as
  `clean`, and the probe reports a **true claim as disproved**. Same direction as the `git init`
  case, different cause, so the standing clause decided it. Two instances are enough to state the
  shape the README now carries: **this probe reads its evidence from process output, and process
  output is environment-dependent** — any check deciding a claim from stdout/stderr clears what
  suppresses the signal it reads, or classifies the environment as cannot-verify.
