...
**Who prints what.** The `Created` block is **this package's**, along with the three lines above
it. Everything from `Wrote` down is **init's output, printed by init** (epic theme 5) — that is
...
- **the `Created` summary names exactly the files the run wrote** — compare the reported list
  against the actual diff of the generated directory, in both directions, so a file written but
  not reported fails as loudly as one reported but not written. The epic's proof for this issue
  requires that comparison; it also pins the seam, since a summary that drifts from the diff is
  the first sign the file list has crossed into init
