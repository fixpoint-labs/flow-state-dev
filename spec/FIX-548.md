...
| Case | Expected |
|---|---|
| `.env.local` is about to be written and git would not ignore it | Add the ignore entry, then write the key. This is the ordinary path, not an error — `create-next-app`'s own `.gitignore` already carries `.env*` (verified), so the entry is usually already there and this is an assertion that it is. |
