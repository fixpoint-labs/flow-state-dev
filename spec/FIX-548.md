...
*Every deterministic local write is steps 3–4, before the two steps that can fail (5 and 6), so a
failure leaves a project the printed recovery genuinely completes. The table below step 7 says what
each recovery restores.*
...
**Every deterministic local write happens before the two steps that can fail, and that ordering
is a promise §9 makes rather than a preference.** The rule: *for each step, ask what the printed
recovery actually restores.* Applied here, it moved the `AGENTS.md` append from last to fourth.

| fallible step | what is already on disk when it fails | what the printed recovery restores |
|---|---|---|
...
| step 5, the credential stop | the six template files, all four changes, **and the `AGENTS.md` section** | the developer adds the ignore line, writes the key, and installs — three named steps, and nothing is missing that they would have to author |
...
4. **The `AGENTS.md` append** — FIX-1160's canonical block embedded verbatim and appended, never
   ...
   two fallible steps, so a later failure leaves nothing the developer would have to author**
   (§7). *Test:* normalized equality against canonical via FIX-1160's exported comparison, every
   branch included; Next's own block and `CLAUDE.md` intact byte-for-byte afterwards.
...
| The install fails (step 6) | The scaffold stays and is **genuinely finishable**: the six template files, all four changes, the `AGENTS.md` section and the key are already on disk, so the developer's own package-manager install is the *only* thing outstanding and it completes the project. Report the manager's own output and print that command against that directory. **This row is why the append is step 4** — it was step 5's last item in an earlier revision, so it had not run when the install failed, and an install cannot restore canonical text (§7). |
| The `AGENTS.md` append fails (step 4 — unwritable, or the file vanished) | The scaffold stays and is usable: this is the one job whose absence stops nothing from running. Continue to the remaining steps rather than aborting, and name the section as not added in the summary. **Do not** print instructions to paste the block by hand — it is canonical text this package embeds, and a hand copy is exactly the drift theme 9 exists to prevent; point at the docs page instead. |
...
contradiction this taxonomy used to carry.

**"Finishable" is a claim about ordering, and it has to be re-checked whenever a step moves.** It
held only because every deterministic write precedes the fallible install; an earlier revision put
the `AGENTS.md` append after it and the word silently became false. The general form is in §7's
recovery table: *for each step, ask what the printed recovery actually restores* — and recovery
instructions that ask the developer to reproduce canonical text do not count, because the whole
point of a single authored source is that nobody hand-copies it. Nothing is ever cleaned up behind the
...
both through.** Nearly every finding against this spec and its POC has been *a check that cannot
fail the claim it is cited for*: a marker count that survives a truncated section, a green result
from an npm install hiding a manifest missing `@flow-state-dev/engine`, a control exercised only on
the path it protects. The goal check's `-y` was the **mirror image** — *a check that cannot pass*,
because the run it specified suppressed the key prompt the same run needed answered. **They are the
same defect.** In both, the check's outcome is decoupled from the thing it claims to measure, so it
reports a constant. The test is one question asked in both directions: **name the scenario where
this check comes out the other way, and confirm that scenario is reachable.** If nothing makes it
fail, it proves nothing; if nothing makes it pass, it is not a check but an obstacle. Recorded here
rather than in a review thread because a reader who meets these separately files them as unrelated
mistakes and re-derives the lesson each time.
