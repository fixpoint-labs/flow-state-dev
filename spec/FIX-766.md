...
4a. **Rename the runtime path/name literals** in `sequencer.ts` (`:1576`, `:1612`, `:1799`, `:1804`, `:1830`, `:1842`) and the fixtures that assert on them. *Test:* the cross-version continuation test in §10 — **not** an assertion on the emitted id's shape, which cannot detect duplicate execution. **Call out in the changeset that this changes replay-log keys**, so a completed side chain from a pre-rename run is not recognised: it is the one part of this rename with a runtime consequence beyond naming. *Depends on:* 2.
...
- **A cross-version continuation test**, since E4 is the one encoding with a runtime consequence. Interrupt a run whose side-chain child has **completed**, then continue it with the renamed code and assert the child does **not** execute a second time (count invocations, don't inspect ids). An assertion on the emitted `blockInstanceId`'s shape cannot detect duplicate execution — only running the continuation across the rename boundary can. `goals/continue-interrupted-run/` is the existing shape to build this from.
