...
3. **Scope is set by a guard that covers all three ways the concept is encoded, not by a list and not by one parse.** Enumerating this change has now failed six times, and — the part worth noticing — **two of those failures happened after I replaced listing with parsing.** Parsing was right and still under-counted, because it only saw one of three encodings. *Alternative rejected:* the hand-written list, and then the identifier-only parse that replaced it. *Locks in:* **five** publishable packages change together (`core`, `contracts`, `engine`, `testing`, `devtool`), and completeness is enforced by one CI guard that recognises the concept in **all three** of its encodings — identifiers (`WorkConfig`), string-literal union members (`FlowErrorScope`'s `"work"`, invisible to an identifier walk), and the word `background` used for tier 2 (`composeBackgroundSignal`, which a `work`-token rule cannot match by construction). It also locks in the deliberate **non**-renames of §7.
...
1. **BP-003 (verification evidence), applied to the scope numbers themselves.** Every count in this spec was produced by a script, and each sweep was run against a synthetic violation and against correct post-rename code to prove it can return both answers. This is the practice this issue has failed six times — twice *after* parsing replaced listing, which is why the guard now covers all three encodings rather than one.
...
**The concept is encoded three ways, and a guard keyed to one of them reports done while a whole category is out of its view.** This is the spec's own lesson, learned twice during its review:
...
**Scope, machine-derived.** One script, `sweep.mjs`, covers all three encodings; every row below comes from it (`--files` mode drives the controls, plain mode the repo):

| Rule | Surface | Sites | Files |
|---|---|---:|---:|
| R1 | The four DSL verbs — code (104 calls + 17 declarations) | 121 | 25 |
...
- **The completeness guard** — the graduated script from step 1, run as a test, with **one fixture per encoding**: an identifier (`WorkConfig`), a `"work"` union member not bound to `phase` (`FlowErrorScope`), and a tier-2 `background` name (`composeBackgroundSignal`). Each must be shown to fire; a guard that cannot fail is not a guard (tenet 7). Two of this spec's six scope misses were a guard that was green because a whole encoding was outside its view, so per-encoding coverage is the specific thing being tested.
