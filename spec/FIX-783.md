...
4. **Reverse the bullet prohibition.** **Remove** the sentence "Emit `p` as a string for
   every section; leave `items` as null." Replace with which sections carry which, using
   **the same split criterion 2 checks** — write the prompt against that list, not a looser
   paraphrase, or the criterion and the instruction can drift: `Executive summary` is prose
   only; `The case`, `What argues against`, `Critical near-term inflection`, `Pre-committed
   exit triggers` and `Why not the adjacent tier` put load-bearing figures and claims in
   `items` and keep `p` for the judgment connecting them; `Deferred follow-on` may use either;
   `Citations` is a list. **Widen the shape declaration in the same step** — the line above the section list
...
| `AAPL` | 26 | Degraded — missing `analyst-estimates`, `earnings-transcript`, `sec-filings`, `prediction-markets`, and two discover-contexts |
| `JPM` | 23 | More degraded — the AAPL gaps plus `discover-market-context`, `sector-context`, `sector-peers` |
...
(`flows/analysis/tools/runtime/fixtures.ts:95`), and `get_prediction_markets` is wired
unconditionally into the sentiment analyst (`agents/analysts/analysts.ts:207`) with no
`costPreset` gate. Failures are rescued per memo — `memoStatusSchema` carries an `error` state
(`flows/analysis/resources.ts:19`) — so the run degrades rather than aborts.

**The call: run `NVDA` + `AAPL`, and treat AAPL's memo as degraded on purpose.** AAPL is the
better second ticker (26 files vs JPM's 23), but its fundamentals and sentiment analysts will
produce errored memos, and those feed the PM memo criterion 4 asks a human to judge. That is
acceptable — criterion 4 compares *openers*, which come from the prompt rather than from the
upstream evidence, and §9 already requires that thin evidence read as thin. **Say in the PR
which memo was degraded**, so nobody reads AAPL's thinner bullets as a failure of this change.
...
behaviour changes is wrong).

**Stance precondition — check this before judging any prose.** Read
...
without this check two same-stance runs satisfy everything while leaving the actual requirement
untested. **If both come back the same stance** the pair is not valid evidence: re-run the
second ticker as `JPM` (accepting its heavier fixture gaps) before judging criterion 4. If no
fixture ticker yields a differing stance, say so explicitly in the PR and mark criterion 4 as
judged on a weaker pair — do not quietly accept two same-stance memos.

1. Eight body sections, with the `h` values and order §8 step 2 tabulates.

2. **These five sections each carry at least one `items` bullet, and those bullets carry a
   figure or a named upstream claim** — not a sentence lifted from the paragraph beside it:
   `The case`, `What argues against`, `Critical near-term inflection`, `Pre-committed exit
   triggers`, `Why not the adjacent tier`. Naming them is the point: "evidence-bearing" left
   undefined is satisfied by a memo that simply declines to classify any section as evidence,
   which is a criterion that cannot fail. The other three are settled explicitly —
   `Executive summary` is **prose only** (decision 2: it is the verdict), `Citations` carries
   bullets but is **exempt** from the figures-or-claims test (it lists upstream memos by name
   by design), and `Deferred follow-on` is **optional** either way, since deferral is often a
   judgment with no figure attached.

3. No two sections restate each other. Concretely checkable half: `Executive summary` is one
   paragraph, carries no `items`, and none of the figures appearing in the evidence bullets
   below it are repeated there. The rest — that `The case` and `What argues against` argue
...
4. Read side by side, the two memos differ in **how each section opens**, not merely in their
   first line. Compare the opening clause of all eight sections pairwise: if six or more open
   with the same construction, the template survived and this fails, whatever the topic
   sentences say. This is the criterion the whole issue turns on and it is a human judgment;
   state the verdict in the PR, with the pairwise comparison, rather than implying it.
...
5. The legacy report minted in step 0 — Past Reports → the `NVDA · 2026-05-06 · fast · fixture`
   ...
   no bullet lists, and no empty-list artifacts. Same dev server as step 0
   (`pnpm --filter @flow-state-dev/trading-desk dev`). If that row is missing, either the
...
wrong three times — first depending on an uncommitted report corpus, then on a CLI invocation
that writes to a database the UI never reads, then on a dialog date that resolves to no fixture
and a shell redirect into a directory that does not exist. Each time the error was the same
shape: verifying the layer that had just changed and not the layer that runs first. So rather
than repeat the claim, here is the ledger:

| Criterion | Verified by reading the code/docs | **Not** verified — no run was performed |
|---|---|---|
| Step 0 / criterion 5 (legacy report) | CLI hardcodes `cli-user` with no override flag (`cli/src/commands/run.ts:292,355`, option list); app queries `devuser` (`app/page.tsx:40`) via `listSessions` (`useFlow.ts:132,146`) filtered server-side (`session-routes.ts:61`); tuple fallback is same-scope (`app/page.tsx:62-75`). Dialog defaults read off `app/page.tsx:38,142,143,144`; fixture resolution probes the exact date (`flows/analysis/lib/ticker-resolver.ts`) and `2026-05-06` is the only NVDA snapshot on disk. | **Step 0 has never been executed.** An attempt to settle it empirically was made in parallel and produced no usable result, so the date finding rests on the citations above and Codex's — not on a run. Unverified: that the dev server starts, that the dialog exposes these four fields as assumed, and that the row opens. |
| 1–4 (two memos) | `runArtifacts` has `inputSchema: z.object({})` and its header documents the exact CLI form; the artifacts bundle carries `decisionSnapshot.finalRating` for the stance check (`run-artifacts.ts:86,115`, `decision-snapshot-resource.ts:34`); `--capture` creates parent dirs (`run.ts:162`) but the `>` redirect does not, hence `mkdir -p`; action-level `date` default is `2026-05-06` (`state.ts:43`); cwd rules per `AGENTS.md` + `portfolio-db.ts:55`; fixture inventory counted on disk. | That a `full` fixture run completes, what AAPL's degraded memos look like, whether the two runs actually land on differing stances, and whether the pairwise opener comparison is judgeable in practice. All require the run. |
