...
Then in the browser: **New Analysis** → ticker `NVDA`, source `fixture`, preset `fast` → run it
to completion. Note the row's title (`NVDA · <date> · fast · fixture`) — that is the row
criterion 5 opens.
...
not apply. Run them **from `labs/trading-desk`**; `--capture` creates its parent directory, so
no `mkdir` is needed:
...
for T in NVDA AAPL; do
  ...
  pnpm fsdev run analysis analyze \
    -i "{\"ticker\":\"$T\",\"dataSource\":\"fixture\",\"costPreset\":\"full\"}" \
...
4. Read side by side, the two memos' paragraph openers differ. This is the criterion the
   ...
   than implying it.
5. The legacy report minted in step 0 — Past Reports → the `NVDA · <date> · fast · fixture`
   ...
   (`pnpm --filter @flow-state-dev/trading-desk dev`). If that row is missing, the step-0 run
   went through the CLI rather than the app and is owned by `cli-user`; redo it through the
   New Analysis dialog rather than working around it.
...
wrong twice — first depending on an uncommitted report corpus, then on a CLI invocation that
writes to a database the UI never reads. Both times the claim "executable from a clean checkout"
was made from reading the layer that had just changed and not the layer beneath it. So rather
than repeat the claim, here is the ledger:

| Criterion | Verified by reading the code/docs | **Not** verified — no run was performed |
|---|---|---|
| 0 / 5 (legacy report) | CLI hardcodes `cli-user` with no override flag (`cli/src/commands/run.ts:292,355`, option list); app queries `devuser` (`app/page.tsx:40`) through `listSessions` (`useFlow.ts:132,146`) filtered server-side (`session-routes.ts:61`); tuple fallback is same-scope (`app/page.tsx:62-75`). Hence: mint via the app. | That the dev server starts cleanly, that the New Analysis dialog exposes preset/source as assumed, and that the resulting row is openable. Nobody has run it. |
| 1–4 (two memos) | `runArtifacts` exists with `inputSchema: z.object({})` and its header documents the exact CLI form; `--capture` creates parent dirs (`cli/src/commands/run.ts:162`); cwd rules per `AGENTS.md` + `portfolio-db.ts:55`; fixture inventory counted on disk. | That a `full` fixture run completes, what AAPL's degraded memos actually look like, and whether two memos suffice to judge opener variation. All require the run. |
