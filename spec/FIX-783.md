...
2. **Bullets carry the evidence — in these five sections:** `The case`, `What argues against`,
   `Critical near-term inflection`, `Pre-committed exit triggers`, `Why not the adjacent tier`.
   Naming them is the point: "evidence-bearing" left undefined is satisfied by a memo that
   declines to classify any section as evidence, which is a criterion that cannot fail. The
   other three are settled explicitly — `Executive summary` is **prose only** (decision 2: it
   is the verdict), `Citations` carries bullets but is **exempt** from the figures-or-claims
   test (it lists upstream memos by name by design), and `Deferred follow-on` is **optional**
   either way, since deferral is often a judgment with no figure attached.

   The requirement has two halves, because a flat "all five must carry a figure" would
   contradict §9 — where an errored or unavailable stage is non-evidence, fewer bullets is
   correct, and thin evidence must read as thin — and on the deliberately degraded AAPL run it
   would reward inventing support to fill a checklist, in an epic about not fabricating:

   - **On the NVDA run (complete corpus, no errored analyst memos): all five carry at least one
     bullet.** There is no evidence gap to excuse an empty one, so an empty section here is a
     failure. This is the half that can fail, and it is why the strict form is pinned to the
     run that has every input.
   - **On a run with errored upstream memos (AAPL as planned): a section may carry fewer
     bullets, or none, where its evidence never arrived** — that is §9 working, not a defect.
     What must still hold on *every* run: each figure or named upstream claim the memo actually
     rests on appears in `items`, not buried mid-paragraph in `p`. A memo that still hides its
     numbers in prose fails regardless of how thin its inputs were. **Never pad.**
