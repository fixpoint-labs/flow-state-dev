...
2. **No new signal is wired into the rating, the setup score or the capital
   ...
   **Where that absence was searched.** A claim asserting the *absence* of an
   effect needs a named search behind it, not an assurance — a reader can check
   what a change does by reading the code it names, but "this affects nothing
   else" is unbounded unless the search space is stated. Checked, no trend-depth
   read: `lib/setup-score.ts` (`{ trend, sma50, sma200 }` only),
   `lib/evidence-gate.ts` and `evidenceBasis` (`setup-score.ts:109` counts
   component *presence*, not values; the gate never touches `finalRating`, `:26`),
   `lib/reward-to-risk.ts` (scenario probability buckets only),
   `lib/mandate-gates.ts` and `lib/risk-mandate.ts` — the capital gates — (zero
   indicator references), `orchestration/run-artifacts-action.ts` (six resources,
   `technicalData` absent), and `test/output-schemas-strict.spec.ts`
   (`indicatorsSchema` absent from `cases`, so BP-016 does not bind). **Two do
   carry trend depth and need no change:** `analysts.ts` via `asDataBlock`, which
   stringifies the payload wholesale, and `lib/valuation-spine.ts:76-85`, which is
   route 3 below.
...
> have it.
>
> **What being wrong costs.** Low, and cheap to reverse. Picking A and regretting
> it means deleting one field and one prompt clause — the underlying number is
> unchanged and no stored report is invalidated. Picking B and regretting it means
> shipping a release whose headline value a reader cannot see, and then doing this
> review again.
>
> **What the fork does *not* decide: whether this release can move a rating.** The
> evidence gate and the deterministic setup-score inputs are untouched on either
> option (decision 2). But the trend prose reaches the portfolio manager either
> way, and the PM may leave the rating band citing what the memos surfaced — so
> `finalRating`, and the sizing that follows it, can move under A *and* under B.
> Publishing a verdict word does not open that route and choosing the bare number
> does not close it. **Cheap to reverse is not the same as cannot move the
> rating**, and the fork is about *whose threshold decides the word* — the desk's,
> stated and tested, or the model's, improvised per run. This deserves a ruling,
> not a long look.
...
**Segment 2 must exercise the parallel-result seam, not a spine payload.** The
analyst generator consumes the direct output of
`.parallel(attributedTools(...)).step(generator)`
(`agents/_recipe/define-analyst.ts:57-58`) — **not** the `technicalData`
resource. So a segment-2 test that hands a spine-shaped payload to `asDataBlock`
and asserts the new keys appear stays green even if the real parallel result
drops them, and the generator schema's null defaults absorb the loss: the memo
carries no trend depth and everything passes. Drive the assertion from the
parallel result the generator actually receives. §10 states segment 2's intent
correctly; this is the seam that makes it real rather than nominal.

**In the before/after regeneration capture, assert three values and merely record
two.** **Assert** the momentum sub-score, the setup score and the model-implied
rating: all three are computed from the indicator inputs by `computeSetupScore`
and `modelImpliedRating`, pass through no generator, and are reproducible run to
run. **Record** `finalRating` and whether `ratingOverrideReason` came back
non-empty — capture both before and after, report them in the PR body, and do
**not** put them in an `expect`. The reason is that fixture mode stubs the *data*
tools only: every analyst, research, risk and PM generator calls a real model
(`goals/trading-desk-headless/fixture-run-clean/goal.md`;
`.agents/skills/verify-trading-desk/SKILL.md`), so those two values legitimately
differ between valid runs and a fixed assertion on either would flake. This is
also what makes §10 self-consistent: its own lines directly below the capture
block already say an override appearing where none did before is **a finding to
report, not a failure**, which is a description of evidence to read rather than a
value to pin.

**`prices.json` stays a ONE-MONTH file, and the guard asserts that span — not
"is a tail of the series".** The tail-identity assertion as first stated is
satisfied by any suffix, and a 500-bar series is trivially its own tail, so the
"re-record both from one `2y` pull" route can write the whole response to
`prices.json` and pass all four guards. That is not untidy, it is wrong at the
reader: the technical analyst's prompt announces its data as
`priceHistory (1-month OHLC bars)` (`technical.prompt.md`, the `<data>` inventory
line), so two years of bars arriving under that label make the analyst
characterize two years of movement as a month's. Concretely, whichever route step
8 takes:

- `prices.json` keeps `"range": "1mo"` and holds **only the final month of
  bars** — preserve each ticker's existing count when authoring the series to
  terminate on the existing bars; take a one-month tail when re-recording.
- The guard asserts **the span**: `range` is `"1mo"`, the bar count is within a
  one-month band rather than merely non-empty, **and** those bars equal the final
  *N* of `_source-daily-2y.json`. Span first, tail second — the tail check alone
  cannot fail on this defect.
- The reason nothing downstream saves you: `loadFixture` keys on the **tool**, not
  the requested range, so there is one `prices.json` per snapshot and a `2y`
  request replays whatever it contains. An over-long file is never rejected, only
  read.
