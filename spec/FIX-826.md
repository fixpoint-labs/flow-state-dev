...
entitlement (the epic's standing fence); and re-recording the fixture corpus
...
5. **Read the executing path at every boundary this spec asserts.** Three of
   ...
   wrong package; it lives in `blocks/internal/build-block.ts`), the crossing scan
...
- **`flows/analysis/tools/indicators-math.ts`** holds all the new math, beside the
  ...
  - **The crossing scan compares nearest NON-EQUAL observations, not adjacent
    bars.** A strict adjacent sign-change test on the difference series misses a
    cross that passes *through* equality: for `−1, 0, +1` neither adjacent pair
    has opposite non-zero signs, so a real crossing goes unreported — a false
    negative arriving through the algorithm rather than through the
    representation decision 3 exists to protect. Track the last **non-zero** sign
    and compare each new non-zero sign against it; a difference is a crossing,
    dated at the bar where the new sign appears. This also gives the touch case
    for free: `−1, 0, −1` has the same sign on both sides, so nothing is
    recorded. Both must be tested (§10).
...
- **`agents/analysts/prompts/technical.prompt.md`** — the `<data>` inventory, the
  "Setup" and "Momentum" section guidance, and **the clause carrying the owner's
  guardrail**: when the strength read is null the analyst states plainly that the
  desk could not characterize the trend, and never narrates strength from the
  stack label alone. Also: a `found: false` crossing means *searched, none found
  within N sessions*, and must never be written up as "no crossover" without the
  window.
- **The strength read widens the EXISTING `trend` metric key — it does not add a
  ...
  stack label and the strength read (`"up · ADX 31 (strong)"`), or the label plus
  an explicit not-measured (`"up · strength not measured"`). This also keeps the
...
4. **Build the strength read**, gated on the indicator's own required-input
   ...
   components, and **the zero-range guard** — a `NaN` from a flat tape becomes
   null, never `0` (§7). *Test:* the §10 strength behaviours, including magnitude,
   the below-warm-up case, and the constant-series boundary. *Depends on:* 3.
5. **Build the slopes and the two extensions**, each gated on its own bar
   requirement independently (§9's table), and each returning null on a zero
   denominator rather than dividing by it. *Test:* the partial-history behaviour
   — a 50-day slope present while the 200-day slope and 200-day extension are
   null — plus the constant-series case. *Depends on:* 3.
6. **Build the crossing scan** for the two moving-average crossings, comparing
   **nearest non-equal observations** (§7 — an adjacent-pair test misses a cross
   through equality), with the three-state result and a **computed** lookback
   window. The window is the number of bars on which both legs exist minus one,
   per crossing — it is published, so it must be derived, not assumed, and the two
   crossings do not share a value. *Test:* the §10 crossing behaviours, all three
   states, plus the through-equality / touch pair. *Depends on:* 3.
...
8. **Re-author the three checked-in `indicators.json` fixtures** (NVDA, AAPL,
   ...
   existing recorded figures, **and add the guard test that reads the raw fixture
   files** (§10 — a guard on the parsed payload passes against the very defect it
   exists to catch, because `.default(null)` fills the gap silently).
   *Depends on:* 1.
9. **Update the technical analyst prompt** — inventory, section guidance, the
   cannot-characterize clause, and **the widened `trend` metric key** carrying
...
| **A crossing that passes exactly through equality** (`−1, 0, +1`) | **Reported as a crossing**, dated at the bar where the new non-zero sign appears. An adjacent-pair sign test misses this entirely; the scan compares nearest non-equal observations (§7) |
| A touch that does not cross (`−1, 0, −1`) | Nothing recorded. Same sign either side of the equality, so it is not a crossing — and this falls out of the same rule rather than needing its own special case |
...
were used. The three `indicators.json` files must be re-authored (step 8).

**The guard test must read the raw fixture files, not the handler's output.** A
stale fixture does not throw and does not drop keys: handler outputs are validated
(`build-block.ts`'s `validateSchema`), and that parse applies `.default(null)`, so
the payload comes back with explicit nulls that are indistinguishable from a
genuine short-history read. A guard asserting on the parsed payload would
therefore pass against exactly the defect it exists to catch. It walks the
checked-in JSON on disk and asserts every non-`source` field in `indicatorsSchema`
has a counterpart there. That is the durable fix for the class — it fails the next
time anyone adds an indicator field and forgets the corpus — rather than for this
instance.
...
- **The constant-series boundary:** 30+ identical OHLC bars yield a null strength
  ...
  Assert on all five fields; this is the case the library's own zero guard misses.
...
- **Crossing through equality, both directions, as a pair:** `−1, 0, +1` **is** a
  crossing and is dated at the sign flip; `−1, 0, −1` is **not** one. Assert both
  in one test — they are the same rule, and an adjacent-pair implementation fails
  the first while passing the second, which is what makes it look correct.
...
- **The fixture guard** (raw files, per above), and segments 2 and 3.
