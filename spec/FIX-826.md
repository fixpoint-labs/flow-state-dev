...
- **`flows/analysis/tools/indicators-math.ts`** holds all the new math, beside the
  ...
  - **The crossing rule is stated as a PROPERTY of the series, not as a scan.**
    ...
    *procedure* rather than in the intent: first an adjacent-pair sign test that
    ...
    the date lands one observation early and the direction inverts. §10 asserts
    date *and* direction for exactly that reason.
  ...
  - **The slope and extension formulas are stated here, in units, because they
    ...
    scaled ×100**; leaving it a fraction publishes `0.159` for `15.9%`, the same
...
- **`agents/analysts/prompts/technical.prompt.md`** — the `<data>` inventory, the
  ...
  absence of one:
  - **strength measured, trend present** — "up · ADX 31 (strong)".
  - **strength measured, no trend** — "up · ADX 18.9 — no established trend".
    This is decision 1's honest conflict, and it is a *finding*: the desk looked,
    and the directional pressure is not there.
  - **strength not measured** — "up · trend strength not measured". Never spelled
    as "no established trend", which would assert the finding above from data
    that produced none.
...
6. **Build the crossing detection** for the two moving-average crossings, **to
   ...
   the opposite sign, with date, direction and age all read off *that* bar — with
...
|---|---|---|
| `adx.value` / `plusDI` / `minusDI` | 27 | `interval * 2 - 1` at interval 14. All three gate together — no inner partial-null inside the block. The components are themselves finite from bar 14 (§7, measured), so gating on 27 is what guarantees the block is never published half-populated |
...
| **A crossing that passes exactly through equality** (`−1, 0, +1`) | **Reported as a crossing at the `+1` bar, direction bullish.** The zero is not a non-zero observation, so equality is stepped over rather than hiding the cross — and both the date and the direction come from the **newer** of the two non-zero observations, never the older (§7's property) |
...
- **Below the gate, the whole block is null** — a 26-bar series yields a null
  strength block, asserted as the block being null rather than as null components.
...
- **Slopes assert VALUES, not presence.** A slope that reverses its endpoints,
  ...
  minimum), asserted against §7's formula to a tolerance, with the values
  computed rather than eyeballed:

  | Series (70 bars) | `sma50[t]` | `sma50[t−20]` | **Expected `sma50Pct20`** | Reversed endpoints | Raw delta | Left as fraction |
  |---|---|---|---|---|---|---|
  | rising, `close = 100 + i` | 145.5 | 125.5 | **+15.936** | −13.746 | +20.0 | +0.159 |
  | falling, `close = 200 − i` | 154.5 | 174.5 | **−11.461** | +12.945 | −20.0 | −0.115 |
  | flat, `close = 150` | 150.0 | 150.0 | **0.0** | 0.0 | 0.0 | 0.0 |

  The rising and falling rows are what carry the test: each of the three break
  modes lands on a **different value** there, so a single equality assertion
  distinguishes all four implementations. The flat row does **not** discriminate
  between them — every column reads 0.0 — and it is included for a different
  ...
  same bars.
- **Extensions, signed and gated:** price above its 50-day yields a **positive**
  extension and price below yields a **negative** one, asserted as a sign, not as
  presence — the sign is the whole read. Plus the independent-gating pair: an
...
- **Crossing through equality, and the touch, as a pair:** `−1, 0, +1` **is** a
  crossing, dated at the `+1` bar and reported **bullish**; `−1, 0, −1` is **not**
  ...
  look correct. **Assert the direction, not only the date:** an implementation
  that reads its answer off the older of the two non-zero observations dates the
  crossing one observation early *and* inverts its direction, and a
  presence-only assertion passes against both.
...
- **After spec review (round 3)** — two findings, both about the honesty of a
  ...
  **The crossing rule stopped being a procedure.** Round 2's "compare nearest
  ...
  has the opposite sign; date, direction and age all read off that bar) and says
  ...
  against that one statement; §10 now asserts direction as well as date, since
