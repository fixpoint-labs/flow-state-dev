...
**No new signal enters the rating.** None of `adx`, the slopes or the crossings is
read by the setup score, the valuation envelope or the evidence gate — this is
evidence the technical analyst reads, not a new input to the desk's arithmetic.
**Two caveats the earlier "nothing here touches the rating" wording hid**, both
verified in source and neither a wiring change: widening the window to two years
moves `obv` (a cumulative sum, so more history changes it outright), and
regenerating the fixture corpus replaces the *invented* `trend` / `sma50` /
`sma200` values that feed the momentum sub-score, so the **fixture-mode** setup
score and rating can move. Decision 2 carries the measurements and §10 asserts the
before/after. The **evidence gate does hold**: it counts how many components are
present, not what they read.
...
2. **The new signals do not enter the rating, the setup score, or the capital
   gates — but this release does move two published numbers by other routes, and
   the earlier wording denied it.**
   The *signals* claim holds and was re-verified in source: `computeSetupScore`
   takes `technicals: { trend?, sma50?, sma200? }` (`lib/setup-score.ts:29-35`)
   and reads no `adx`, `slopes` or `crossovers` field anywhere. Nothing in the
   rating path learns about trend depth.

   **What was false is the second sentence — "No number the desk already
   publishes changes value because of this issue."** It is wrong twice, both
   measured rather than argued:

   | Route | What moves | Evidence |
   |---|---|---|
   | **Widening the window to 2y** (decision 6) | **`obv`** — materially, including a sign flip. It is a cumulative running sum from the *first* bar (`indicators-math.ts:185-188`), so doubling the history changes it outright. `rsi14` / `macd` / `atr14` shift by ~1e-9 relative (converged Wilder/EMA warm-up); `sma50`, `sma200`, Bollinger and Stochastic are **identical**, being trailing-window reads | `node spec-poc/FIX-826-trend-math/window-widening.mjs` — measured `obv` −4066 → 5518 on the same final bar |
   | **Regenerating the fixture corpus** (§10, added round 7) | **`trend`, `sma50`, `sma200`** in fixture mode — which are exactly `momentumSub`'s three inputs (`setup-score.ts:94-105`: ±20 on the trend label, ±10 on the `sma50 > sma200` ordering). NVDA's fixture currently reads `124.8 > 112.3`, a golden cross worth `+10`. Momentum carries weight `0.15`, so a full swing moves the composite by up to 4.5 points, and the composite feeds `modelImpliedRating` via `valuation-spine.ts:76-85` — so **the fixture-mode rating can move** | §10's before/after regeneration assertions |

   **This is amended rather than defended.** The honest statement is: *no number
   changes because the new signals were wired into anything* — that part was the
   point and it holds — *but the window widening moves `obv` in live mode, and the
   corpus regeneration can move the fixture-mode setup score and rating.* The
   second is a fixture-data correction, not a model change: those fixture values
   were invented, and replacing an invented input with a computed one is expected
   to move a score computed from it. **The right response is to publish the
   before/after, not to constrain the generated series to preserve the old
   outputs** — authoring a history backwards from the outputs you want is how the
   corpus reached this state.
...
- **`flows/analysis/tools/indicators-math.ts`** holds all the new math, beside the
  ...
  - **`.pdi` / `.mdi` are typed `number | void`, but they are NEVER `undefined`
    ...
    **The prohibition on `finite()` is WITHDRAWN — FIX-1063 fixed the helper, and
    this instruction is now backwards.** Every earlier round of this spec said to
    route around `finite()` because it coerced a non-finite input to `0`, turning
    "measured nothing" into a published zero. Verified on `origin/main` after
    `a1928f57a`, the helper now reads:
    ...
    That **is** the absence-aware check this spec was asking the implementer to
    hand-roll: `NaN` in, `null` out. **Use it.** Writing a parallel
    `NaN`-to-null helper beside it would be a second implementation of one rule —
    the same defect §7 rules out for the band and §13 rules out for goldens, and a
    direct violation of tenet 5. The *hazard* is unchanged and still real (a
    zero-range series makes `pdi`/`mdi` arrive `NaN`, per the previous bullet);
    only the remedy moved, because the shared helper stopped being wrong.

    **This is the clearest example of why a landed dependency gets re-read rather
    than assumed:** the instruction was correct when written, was still correct
    one commit before the merge, and became actively harmful the moment #1236
    landed — while looking exactly as authoritative as before.
...
- **`flows/analysis/tools/schemas.ts`** — `indicatorsSchema` grows the three
  ...
  **Note this DIVERGES from the base, deliberately.** FIX-1063 (`a1928f57a`)
  ...
  default explicitly — an earlier draft said this issue "inherits FIX-1063's
  nullability convention", which is now wrong. Inheriting it literally would mean
  a session persisted before this lands, or any fixture not yet regenerated,
...
10. **Extend** `indicators-math.ts`'s module header to cover the new fields.
    **Mostly done already — do not redo it.** Earlier rounds scoped this step as
    "remove the stale claims that routines return finite numbers and fall back to
    `0`". FIX-1063 (`a1928f57a`) already rewrote that header; it now reads "return
    a finite number or `null` … the routine returns `null` — the indicator was not
    measured". What remains is additive: note that the strength block, the slopes
...
| The library's `+DI` / `−DI` read as `NaN` (zero-range series) | Handled as absence — the block goes null. **Route them through the file's `finite()` helper, which returns `null` for `NaN` since FIX-1063 (`a1928f57a`)** — do *not* hand-roll a second `NaN`-to-null check beside it (§7; earlier rounds said the opposite, when the helper still coerced to `0`). This, not `undefined`, is the reachable non-finite case |
...
> **Capture the momentum sub-score, the setup score and the rating for all three
> tickers before and after regeneration, and record the table in the PR.** Assert
> the *after* values, so a later corpus change that moves a rating has to say so
> in a diff rather than sliding through.
...
HAS LANDED.** Its implementation PR **#1236** merged as **`a1928f57a`**, and the
data-honesty contract this issue extends is now on `main`. The code dependency is
**cleared**; this issue remains gated only on the owner's decision-7 ruling below.

**Re-verified against `origin/main` after the merge, field by field** — because a
dependency landing is the moment a spec's description of its own base is most
likely to have quietly gone stale, and the temptation is to assume things got
*better* rather than *different*. Most of what this spec assumed holds. **Three
things do not**, and all three change instructions this spec gives:

| What this spec assumed | What actually landed | Consequence |
|---|---|---|
| `indicatorsSchema` fields nullable, `trend` nullable | ✅ Every field `.nullable()`; `trend` is `z.enum([...]).nullable()`; `trendLabel` returns `"up" \| "down" \| "flat" \| null`; the empty-payload builder nulls each field | **Holds** |
| This issue "inherits FIX-1063's nullability convention" — i.e. `.nullable().default(null)` | FIX-1063 used **bare `.nullable()`**. There are **zero** `.default(null)` calls in the merged `indicatorsSchema` | **Differs.** The new fields still take `.nullable().default(null)` per BP-023/BP-030 — but that is now a deliberate *divergence* from the base, not inheritance, and §7 says so |
| `finite()` coerces a non-finite input to `0`, so the new code must avoid it | **`finite()` was rewritten**: `(value) => typeof value === "number" && Number.isFinite(value) ? value : null`. It now returns **`null`** for `NaN` | **The prohibition is inverted** — see §7. Following the old instruction would hand-roll a duplicate of a helper that is now correct |
| §8 step 10 must strip the header's "returns finite numbers / falls back to `0`" claims | **FIX-1063 already did it.** The header now reads "return a finite number or `null` … the routine returns `null` — the indicator was not measured (FIX-1063)" | **Step 10 is largely obsolete** — narrowed, not deleted |
| The existing test suite still encodes the old zero/flat contract | **Already rewritten** — `rsi([1,2,3],14)` asserts `toBeNull()`, `out.rsi14` asserts `toBeNull()` | §10's warning is now past tense |

The seven stale one-year comments (§8 step 11) were re-checked on `origin/main`
and **all seven are still there** — FIX-1063 did not touch them.
...
| "Widening 1y → 2y changes no published number" (decision 2, round 7 wording) | **REFUTED in round 8** — `obv` measured −4066 → 5518 on the same final bar; `rsi14` / `macd` move ~1e-9; `sma50` / `sma200` / Bollinger / Stochastic identical (`window-widening.mjs`) |
...
**Blast-radius sweep (round 8).** Three claims this spec made about what it does
*not* affect have now been false on inspection — the gate read ("changes no
published number", false of the memo metric), the widening cost ("no second
request", false because `lib/cache.ts` keys on the args including `range`), and
decision 2 against the setup score. Each was written from what the change *meant*
rather than from what the code *does*. A blast-radius claim is the one kind of
sentence a reviewer trusts without checking, which is why this document kept
getting them wrong. So every remaining one in §6 and §7 was read against source:

| Claim | Verified against | Verdict |
|---|---|---|
| New signals don't feed the setup score / rating / capital gates | `setup-score.ts:29-35` takes `{trend, sma50, sma200}`; zero references to `adx` / `slopes` / `crossovers` | **HOLDS** |
| "No number the desk already publishes changes value" | `obv` is cumulative (`indicators-math.ts:185-188`); fixture regeneration replaces `momentumSub`'s inputs | **FALSE — amended in decision 2** |
| "The setup score, valuation envelope and evidence gate are untouched" (§2) | Score moves via `momentumSub`; envelope reads the score (`valuation-spine.ts:76-85`) | **FALSE for the first two — amended.** The **evidence gate HOLDS**: `evidenceBasis` counts component *presence* (`setup-score.ts:109`), not values |
| No change to the reward-to-risk figure | `lib/reward-to-risk.ts` reads scenario probability buckets only; no indicator input | **HOLDS** |
| Decision 6: no new provider or entitlement | `case "2y": return 750` present in both `yahoo.ts` and `finnhub.ts` | **HOLDS** |
| "No wiring change in `analysts.ts`" | `asDataBlock` is `JSON.stringify(data, null, 2)` — the whole payload, wholesale (`lib/helpers.ts:28-30`) | **HOLDS** |
| "No `run-artifacts.ts` change" | `run-artifacts-action.ts:52-59` declares six resources; `technicalData` appears zero times in the file | **HOLDS** |
| BP-016 does not bind (schema absent from the strict walker) | `output-schemas-strict.spec.ts` `cases`: zero occurrences of `indicatorsSchema` | **HOLDS** |
| §9: "No formatter change" — prompts read raw JSON | Same `asDataBlock` path; `null` renders as `null` | **HOLDS** |

**The sweep then widened to dependency statements**, on the same reasoning — a
claim about the state of another PR ages the same way a claim about consequence
does, and #1236 merged mid-round:

| Dependency claim | Verdict |
|---|---|
| "FIX-1063 must land first / #1236 has not merged / implementation cannot start" | **STALE — merged `a1928f57a`.** Corrected here, in §1, §7, §8 and the PR body |
| "This issue inherits FIX-1063's nullability convention" | **FALSE** — the base landed bare `.nullable()`, no `.default(null)`. The new fields diverge deliberately (§7) |
| "`finite()` coerces `NaN` to `0`, so avoid it" | **FALSE, and inverted** — the helper now returns `null`. The prohibition is withdrawn (§7) |
| "§8 step 10 must strip the header's finite/zero claims" | **ALREADY DONE** by FIX-1063; step 10 narrowed to additive work |
| "The existing suite still asserts the old zero/flat contract" | **ALREADY REWRITTEN**; §10's warning is past tense |
| "The seven stale one-year comments" (round 7) | **STILL PRESENT** on `origin/main` — re-checked; FIX-1063 did not touch them |

**Fourteen claims checked in total across both halves: nine hold, five were
false or stale — and every one of the five was amended rather than defended.** Both failures
were in the same sentence family — the sweeping "nothing else changes" summary
rather than the specific mechanism claims, which all held. That is the pattern
worth carrying forward: this spec's *narrow* claims about particular files have
been reliable; its *broad* claims about consequence have not.
...
- **After spec review (round 8)** — two findings, both real, plus a directed
  blast-radius sweep. No decision reversed; decision 2 **amended**, and decision 7
  and the owner fork untouched.
  **Decision 2's promise was false, and it is amended rather than defended.**
  Round 7's corpus work explicitly expects the existing indicator values to
  change — and `trend`, `sma50` and `sma200` are exactly `momentumSub`'s inputs
  (`setup-score.ts:94-105`), which reach `modelImpliedRating` through
  `valuation-spine.ts:76-85`. So regenerating the corpus can move the
  fixture-mode setup score and rating while decision 2 said no published number
  moves. The baseline goal passes straight through it, because a *changed* rating
  still satisfies `status === "completed"` + non-null `finalRating` + a published
  memo. Folded as §10's before/after capture of momentum sub-score, setup score
  and rating across regeneration, asserted on the after-values. **Explicitly
  rejected: constraining the generated series to preserve those outputs** —
  authoring a history backwards from the numbers you want is how the corpus
  reached this state, and doing it under a guard would be worse than the original.
  The *signals* half of decision 2 was re-verified and holds: nothing in the
  rating path reads `adx`, the slopes or the crossings.
  **A second, larger version of the same claim was found by measuring rather than
  reading.** A third POC (`window-widening.mjs`) feeds the same final bars 1y vs
  2y of lead-in: **`obv` moves outright — measured −4066 → 5518, including a sign
  flip** — because it is a cumulative sum from the first bar. `rsi14` and `macd`
  shift ~1e-9 (converged EMA/Wilder warm-up); `sma50`, `sma200`, Bollinger and
  Stochastic are bit-identical, being trailing-window reads. So the widening moves
  a published number in **live** mode too, independent of any fixture work.
  **The ADX `100` boundary was self-contradictory.** Decision 7's prose
  generalised to "every upper bound is exclusive" while its table said
  `50 ≤ v ≤ 100` and §10 asserts `100 → strong`. Under the literal prose, `100`
  falls out of *every* band — the strongest possible trend classified as
  unmeasurable, the band function's version of the `NaN`-into-`else` defect §10
  already guards. And it is reachable: `adx-internals.mjs` prints `100.0000` at
  bar 27 on the monotone series. Now stated as lower bounds inclusive, the three
  **interior** upper bounds exclusive, the scale maximum **inclusive**.
  **The pattern was named and swept, not just fixed.** Three blast-radius claims
  have now been false on inspection (the gate read, the widening cost, decision 2)
  — each written from what the change *meant* rather than what the code *does*.
  Every remaining "does not affect" statement in §6 and §7 was read against
  source: **nine checked, seven hold, two false and amended.** The two failures
  were both sweeping consequence summaries; every narrow mechanism claim held.
  The full table is in §12.
  **FIX-1063 landed mid-round (#1236, merge `a1928f57a`), and its base was
  re-read rather than assumed.** The code dependency is cleared — this issue is
  now gated only on the owner's decision-7 ruling. Re-verifying the merged shape
  field by field found the spec's description of its own base had gone stale in
  three ways that each change an instruction, none of which a "the dependency
  landed, good" reading would have caught:
  **(a) `finite()` was rewritten and the spec's prohibition on it is now
  backwards.** Every earlier round said to route *around* the helper because it
  coerced a non-finite input to `0`; the merged version returns `null` for `NaN`,
  which is exactly the absence-aware check the spec was telling the implementer to
  hand-roll. Following the old instruction would have produced a second
  implementation of one rule, against tenet 5. Withdrawn, and the §9 row inverted
  with it. This is the sharpest illustration of why a landed dependency gets
  re-read: the instruction was correct when written, correct one commit before the
  merge, and actively harmful after — while reading exactly as authoritative.
  **(b) "Inherits FIX-1063's nullability convention" is false.** The base landed
  bare `.nullable()` with zero `.default(null)`. Measured on the repo's zod
  3.25.76: an absent bare-`.nullable()` key **fails** `safeParse`, while
  `.nullable().default(null)` yields `null`. The new fields still take the default
  per BP-023/BP-030 — but that is a deliberate divergence, not inheritance, and
  §9's stale-fixture reasoning depends on it.
  **(c) Two planned work items were already done** — FIX-1063 rewrote the
  `indicators-math.ts` header (step 10 narrowed to additive work) and rewrote the
  suite's old zero/flat assertions (§10's warning is past tense).
  What held: every field nullable including `trend`, `trendLabel` returning null,
  the empty-payload builder nulling each field, and all seven stale one-year
  comments still present. The blast-radius sweep was widened to cover dependency
  statements on the same reasoning — **fourteen claims checked across both halves,
  nine hold, five false or stale, all five amended.**
