...
- **Slopes assert VALUES, not presence.** A slope that reverses its endpoints,
  ...
  fixtures and cross-checked against the shipped `simpleMovingAverage()` at every
  bar (0 mismatches), not derived by hand — per §13, which is the rule this table
  broke once already.
...
- **Extensions, valued and gated:** on the same two 70-bar series wrapped in the
  suite's `bar(close, range = 1)` helper (`high = close + 1`, `low = close − 1`,
  so ATR-14 is exactly 2.0 on a monotone series), the extension is an exact
  figure, derived by the same run as the slope goldens:
