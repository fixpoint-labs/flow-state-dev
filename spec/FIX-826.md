...
**Size:** Small–Medium — 5 production files · 3 fixture files re-authored · ~19
...
- **Slopes assert VALUES, not presence.** A slope that reverses its endpoints,
  ...
  already. **Re-verified in round 6 by a second, independent method**: every one
  of the 15 values above was recomputed by both the library and a naive trailing
  mean, agreeing bar-for-bar across all three 70-bar fixtures (0 mismatches), and
  every figure matched what is written here. Reproduce with
  `node spec-poc/FIX-826-trend-math/goldens.mjs`.
...
| §10's 15 slope / extension goldens (round 5's re-derivation) | **CONFIRMED** by a second independent method, 0 mismatches |
