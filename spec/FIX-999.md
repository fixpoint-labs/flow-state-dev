...
3. **The seam exposes exactly two things: one *spawn* verb, and one *liveness* read. Nothing else.**
   ...
   **(b) Liveness is a read, and it answers three questions** — *which of these requests are alive?*, *which of these sessions have a live request?*, and *is liveness authoritative in this deployment at all?* The first two are needed by different consumers and neither subsumes the other: a session with a second live request would wrongly answer "alive" for a specific dead one, and a never-claimed task row has no request id to ask about. Both take a set and return a set, because both consumers are sweeps. **The third is not a convenience** — the memory registry is per-process and is the default store, so on a default deployment every peer's healthy request reads as absent and a liveness-joined decision inverts rather than degrades (§7.3). **This is not a re-split of (a)** — it is a different shape entirely, a read rather than a request-start, and it changes nothing about why (a) is merged.
...
**Semantics, because a liveness read is easy to specify wrongly:** absence means *"no live registration"*, which is **not** proof of death — a registration can lag, and the registry is a liveness hint, not a tombstone. Consumers must treat absence as permission to *act on their own fence* (FIX-1005's claim ticket, FIX-982's claim race), never as a settled verdict. A consumer that reads it as "definitely dead" will double-run work, and that failure looks like a seam bug.

**And on the default store, absence is routinely wrong.** The memory registry is a `Map` on the instance (`memory/active-request-registry.ts:4-5`), so **every peer process's perfectly healthy request reads as absent.** Postgres is genuinely shared — a real `active_requests` table with `INSERT … ON CONFLICT(request_id) DO UPDATE` (`store-postgres/src/active-request-registry.ts:75-79`); SQLite is shared only if the DB file is; the filesystem adapter is shared across processes on one host. **Memory is the default.** So a liveness-joined decision on a default deployment isn't degraded, it is inverted — which is why the next paragraph is a requirement rather than a caveat.

**The seam surfaces whether liveness is authoritative here, and that is a requirement of this issue** *(owner decision, alongside Option A)*. Liveness-joined reclamation is only correct when the registry is genuinely shared **and** heartbeats are on and frequent enough relative to the stale threshold — and both are operator-settable and silently wrong when violated, including on Postgres with `request.heartbeatIntervalMs: 0`. FIX-1005's Decision 5 declines to infer sharedness at runtime because *nothing on the adapter interface reports it*, which is true today. So the seam exposes the signal and a consumer's gate can **check** its precondition instead of trusting a README. What this spec owns is the **boundary half** — the signal crossing the seam. The adapter half is a store-contract change and should not ride here; see §12.
...
in core — declared, names nothing from engine:
    ...
    interface RuntimeAccess {
        ...
        liveness: {                          ← set in, set out: both consumers sweep
            ...
            isAuthoritative() -> bool             ← shared registry AND heartbeats on.
                                                    False on the DEFAULT store, where
                                                    a peer's live request reads absent.
                                                    Gates refuse; they don't proceed.
        }
    }
...
| **Liveness read on the default (memory) store** | Every **other process's** healthy request reads as absent — the registry is a per-instance `Map` (`memory/active-request-registry.ts:4-5`). Not a degrade: a liveness-joined decision **inverts**. The seam reports that liveness is not authoritative here so a consumer's gate can refuse rather than proceed. |
| Shared registry, but heartbeats disabled (`heartbeatIntervalMs: 0`) | Same class, and it bites on **Postgres too** — entries go stale and read as dead while the work is running. Sharedness alone is not the precondition; sharedness **and** a heartbeat interval meaningfully below the stale threshold is. |
| A tenant asks about a request id it doesn't own | Reads as absent, not as an error — the same answer a nonexistent id gives, so the read is not an existence oracle across tenants. Behaviour 19. |
...
20. **The seam reports liveness as non-authoritative on the memory store and authoritative on a shared one.** *Falsification:* run the same assertion across two in-process "peers" on the memory registry and confirm each reads the other's live request as absent — if that passes as "alive", the test is sharing one instance and proves nothing.
...
**The count has now moved twice, so here is the shape rather than another number.** It started at four (the issue's, mostly aspirational), fell to one when three of those turned out dissolved or unfiled, and rose to two when a cross-spec pass found a capability this spec wasn't shipping. What did *not* change through any of it is the rule: **nothing goes on the seam without a named consumer that is blocked without it.** The rule is what set the scope; the count is just where the rule currently lands, and quoting it as a fixed fact is what made it wrong twice.

**The adapter sharedness flag should be its own issue, and I am saying so rather than absorbing or dropping it.** The seam surfaces *whether liveness is authoritative* (§7.3); something underneath has to answer that, and today nothing does. But making it answerable means adding a declaration to `ActiveRequestRegistry` and implementing it in four adapters — memory, filesystem, SQLite, Postgres — which is a **store-contract change**, the same shape and the same reason FIX-982 declined to absorb `listBySessionId`. Three arguments for splitting it out:

- **It is reviewed differently.** A boundary change and a four-adapter contract change have different second-path checklists, and this issue is already Large.
- **Its consumers are not only me.** FIX-1005's gate needs it; an operator preflight or a diagnostic surface would want it next. A signal with plural consumers shouldn't be a private detail of whichever spec noticed first — that is the mistake FIX-982's Decision 9 and this spec's own first draft both made.
- **Neither spec is blocked by splitting it.** My liveness reads work without it; only FIX-1005's *gate* needs it, so the dependency edge is theirs, not mine. **Named plainly: if the flag slips, FIX-1005 cannot safely enable liveness-joined reclamation** — that is the consequence, and it belongs to FIX-1005's sequencing rather than being hidden inside mine.

If it lands as its own issue, this spec consumes it and the seam's third read becomes a pass-through. If the owner would rather it ride somewhere, it rides FIX-1005 — not here, since FIX-1005 owns the gate that gives it meaning.
