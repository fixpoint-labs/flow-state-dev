...
**One verb ships:** create-or-adopt a child session bound to a target flow, and start a request in it. Review established that these can't be two independent services — the child's flow kind is the *target's*, and a session is later resolved by that field (§7.2). Flow-resolution-by-kind happens inside, so no flow instance crosses into `core`.
...
3. **One verb, not two services: create-or-adopt the child session *bound to the target flow kind* and dispatch into it, as a single operation.**
   *Rejected:* two independent services (`sessions.getOrCreateChild` + `dispatch.start`) — the shape this spec carried into review; and a `flows.get(kind)` accessor beside them.
   *Locks in:* **the two cannot be independent, because the child session's `flowKind` is the dispatch target's, not the caller's.** `SessionRecord.flowKind` is required, and eight sites in `resource-routes.ts` resolve a session's flow with `ctx.registry.get(session.flowKind)` — so a child bound to the *current* flow, then dispatched into another, is later resolved and exposed under the wrong flow. A separate `getOrCreateChild` would have to take the target kind anyway, at which point it is dispatch's argument sitting on the wrong verb. The flow instance still never becomes a value `core` has to name, and an unregistered kind is a named error before any record is written. **Conflict handling survives the merge:** the verb returns a discriminated outcome, and a lost create race returns `conflict(existing)` **without dispatching**, so the consumer keeps the discriminator FIX-982 needs (its Decision 12 tells *our* child from a squatter) rather than having it swallowed.
...
#### 7.2 The verb, and what binds it
...
2. **Create-or-adopt the child session**, bound to the **target** flow's kind, using the conditional-write sentinel the scope stores already ship. `parentSessionId` is **this** request's session; tenant, user and org are **this** request's. The caller supplies the child key and its own bookkeeping fields, nothing else.
3. **Start the request** into the child, through the host-level start operation (§7.3).
...
#### 7.3 What the verb dispatches *through*, and what a flow must declare to be a target
...
#### 7.4 Where the construction inputs come from
...
#### 7.5 The process boundary — and the premise review corrected
...
#### 7.6 Keeping the premise honest
...
in engine — built ONCE per request, at context construction:
    ...
    buildRuntimeAccess({ stores, flowRegistry, hostStart, identity }) =>
        spawn(args):
            ...
            hostStart({ sessionId: child.id, entry, input: args.input,
...
1. **Declare the surface in `core`** — the verb's interface, the context member, and the flow-level internal-entry declaration, with the identity fields conspicuously absent from every argument type. **Test:** a type-level assertion that the argument types cannot express a tenant, plus the existing capability type tests still pass. *Depends on:* nothing. **This is BP-004's step and the one worth getting right before anything below it exists.**
2. **Admission first, before anything can dispatch** — the `resolveActionCore` branch for the seam's source, resolving `internalEntries` and returning `undefined` rather than falling through to `flow.actions`. **Test:** behaviours 4 and 5. *Depends on:* 1. **Ordered ahead of the verb deliberately: build the gate before the thing that walks through it, so no intermediate commit can dispatch into a named action.**
3. **Implement the verb in `engine`**, bound to one request's identity, composing the host-level start operation. **Test:** behaviours 1–3, 6. *Depends on:* 2.
4. **Thread the construction inputs** — registry, stores and the host start operation onto the internal config; `createExecutionContext` builds the bundle once; then each of §7.4's call sites, the test harness last. **Test:** behaviours 7 and 9. *Depends on:* 3.
5. **The worker-only obligation** (Decision 6b) — construct a start producer in `worker-only` mode, and fail by name at construction where one cannot exist. **Test:** behaviour 8. *Depends on:* 4. **This one touches a shipped mode, so it is the step most likely to surface something the spec missed.**
6. **The no-host path** — a named error from the capability-side helper rather than an `undefined` call. **Test:** behaviour 10. *Depends on:* 1.
7. **Move the premise probe into a type-checked project** (§7.6), and add the changeset (`minor` for `core` and `engine`; the context type and `FlowInstance` each gain a member). **Test:** behaviour 11. *Depends on:* 1.
...
| Child created, then the start fails | The child record survives. The verb is **not** atomic across the two writes and does not pretend to be: a retry finds the existing child and returns `conflict(existing)`, which the consumer already has to handle. Named here because the alternative — deleting the child on a failed start — races a concurrent adopter. |
...
2. **FIX-982 is Spec Approved and its §7.2 names the seam as exposing "`stores`" and "`flow`". This spec exposes neither — and after review round 1 the divergence is materially larger than when the question was first asked.** Three things moved, and the owner should judge item 2 against the new size rather than the old one:
   ...
   FIX-982's own §12 item 3 anticipated *"if FIX-999 exposes something narrower, the spawn capability may need a second seam entry"*, and all three changes are still inside that sentence. **My reading is unchanged — a note on [#1063](https://github.com/fixpoint-labs/flow-state-dev/pull/1063) rather than re-approval — but the case for re-approval is stronger than it was, and whether an approved direction gets re-opened is not a call I should make quietly.**
