...
11. **A configurable ceiling on concurrent detached fan-out ships here, with a safe default. Cross-process cancellation does not.**
   ...
   **This is an explicit stopgap and the spec says so rather than overselling it.** The real spend brake is a cost/token budget — [FIX-933](https://linear.app/fixpoint-labs/issue/FIX-933), *Cost/budget ceiling for delegated work* — which bounds the thing that actually costs money. A count-based ceiling is a proxy, shipped now because this seam ships now.
   **Named limitation — where liveness is unavailable this is not a lineage-wide bound at all.** Decision 10's gate refuses liveness on any registry not declared shared, which includes the **default in-memory one**. Concurrency is then uncountable and the ceiling degrades to a **per-request branching bound**: how many spawns *this one request* may issue. Each recursive child gets a fresh quota of 32, so breadth can grow as **`32^depth`** with **no lineage-wide concurrency or spend limit at all.** Round 3 is right about that, and it is named here as weaker rather than reported as the same guarantee.
   **So the degraded mode is visible at construction, not discovered from a bill.** Wherever the liveness gate refuses, the ceiling's construction emits a **named** signal — the same posture as the gate's own refusal (Decision 3), one step down in severity because the seam still works: the ceiling is *degraded*, not *absent*. The operator learns which bound is in force when the host is built. A silent downgrade is the failure to avoid.
...
- **How it is counted:** the liveness verb (§7.2(d)) over the lineage's recorded request ids — one call into the seam, reads bounded by that id set. This is the second reason the batch *shape* is a constraint rather than a preference; it is not a claim about a single registry query.
...
- **When liveness is not enabled** (Decision 10's gate refused it), concurrency is not countable and the ceiling degrades to a **per-request branching bound** — how many spawns *this* request may issue. Each recursive child gets a fresh quota, so breadth grows as **`32^depth`** and **there is no lineage-wide bound at all.** This is the shipped default's behaviour, since the default in-memory registry is not shared.
- **The degraded mode is announced at construction.** Where the liveness gate refuses, the ceiling's construction emits a **named** signal saying which bound is in force — a warning rather than a refusal, because the seam still functions with the weaker bound (the gate itself refuses; this degrades). The refusal at call time also names which bound refused it. Between them there is no path where an operator gets the weaker guarantee without being told, which is the whole point of folding this as honesty instead of machinery.
...
4. **`engine`, construction:** construction inputs on the runtime config / run options; the bundle built once in `createExecutionContext`; the seven sites forwarding; the `worker-only` construction-time failure; **the liveness enablement gate** — wire the verb only when the registry is shared and the heartbeat/stale ratio holds, and refuse by name otherwise; **and the ceiling's degraded-mode signal**, emitted by name in the same place when the gate refused, so the weaker bound is announced rather than discovered (§7.5).
...
| **Host constructed with liveness unavailable** | The ceiling's degraded mode is announced **by name at construction** — breadth is bounded per request only, so lineage-wide it can reach `32^depth`. The operator is told when the host is built, not by a bill. |
...
18. The fan-out ceiling refuses by name **before** any child session is derived. *Falsification:* assert no session record exists for the derived key afterwards. **Not asserted:** that the ceiling is never exceeded under concurrent spawns — it is an advisory bound (Decision 11), and a test claiming a hard maximum would encode a guarantee the design does not make.
19. With liveness unavailable, the ceiling still refuses, the refusal **names the per-request bound** rather than the concurrency bound, **and construction has already emitted the named degraded-mode signal**. *Falsification:* build the host on the default in-memory registry and assert the signal is present — a silent downgrade passes every other test in this list.
...
| File | Change | Notes |
|---|---|---|
| `apps/docs` — deployment/config page | **Extend** | The fan-out ceiling: what it bounds, the default, when to raise it, that reaching it refuses rather than queues, and that it is a **best-effort** bound rather than a hard maximum. Written to the outsider rule — no issue numbers, no "this used to be unbounded". |
| `apps/docs` — same page | **Extend** | Why liveness-dependent behaviour is unavailable on a per-process registry, framed as a deployment requirement (a shared registry plus heartbeats), not as a limitation to apologize for. **And the two modes of the ceiling side by side:** with a shared registry it bounds concurrent work across a whole lineage; without one it bounds only how many spawns a single request may issue, which does not bound a recursive tree. Say that plainly — an operator choosing a store is choosing between them. |
...
| **FIX-1026** | Cross-process abort **delivery** and cross-process cancel. | Owns what the interrupt verb deliberately does not promise (Decision 3), and is the "cancel later" half of the owner's ceiling decision. **Now has its own approved spec — [PR #1095](https://github.com/fixpoint-labs/flow-state-dev/pull/1095)**, which polls the flag on `runAction`'s existing heartbeat tick and converges on the same in-process controller. **Still not a blocker, and this verb's result shape does not change when it lands** — `recorded` starts being delivered. The two specs overlap in `stores/types.ts`: this one changes the registry interface, that one the request-store interface. |
