...
different places — one finished and reported with dependent work held back, one picked up again.
Today they are indistinguishable, and the obvious fixes make it worse rather than better: the
intuitive one records a failure as a *success*, which lets dependent work proceed on a failed
dependency. Getting it right takes one change in the worker and two settings on the board, all of
which we established by **running** the alternatives rather than reasoning about them (§7c).
...
decision 2 was revised five times by reading and refuted each time, then settled on the sixth by
**running** every candidate against a real board. Tenet 2 (composition over features) — what
remains composes four primitives that already exist.
...
2. **A coding run has three endings, not two, and the board must tell them apart.**
   ...
   | What happened | Where the row ends up | How it gets there |
   |---|---|---|
   | The run **failed on its own terms** | **Terminally settled**, reporting why, **and dependents stay blocked** | the worker **cancels** its own row |
   | We **lost** the run — shutdown, lease loss, cancellation | **Recoverable.** Another attempt may claim it | the worker **throws** |
   | The run **succeeded** | Settled complete | the worker **returns** |
   ...
   **And it locks in two board settings**, both configuration rather than code, both
   load-bearing and easy to omit (§7c): coding tasks must carry a **retry allowance**, or a lost
   run is written off instead of picked up; and the board must set **`onError: "fail"`**, or a
   lost run's error is swallowed and the workstream request — the record this epic's proof reads
   — says the run succeeded.

   **This decision has been revised six times, and the ground under it is soft rather than
   settled.** Each revision was a *different* surface: the task row's status, the retry budget,
   dependency satisfaction, the workstream request's status, drain timing, and foreground
   scoping. The current version is the first settled by **execution** rather than by reading
   (§7c, POC), which is why it is likelier to hold — but the pattern says this decision touches
   more of the system than the rest of this issue does. The specific trap it escapes: "the worker
   returns an errored result", the intuitive version and the one this spec carried, silently
   lands the row in `completed`, which **satisfies dependents** and lets downstream work proceed
   on a failed dependency.

   *Locks in, additionally:* we express "failed terminally" by **cancelling**, because the board
   has no verb for it. That is a deliberate semantic wart, recorded rather than hidden, and it is
   the concrete evidence behind [FIX-1181](https://linear.app/fixpoint-labs/issue/FIX-1181).
   Anyone reading the board later will see coding failures as `cancelled` rows and should not
   conclude a human cancelled them.
...
**Size:** Small — 3 production files in one package · 12 test behaviours · 2 doc surfaces ·
...
const codingBoard = taskBoard({
  ...
  // Without this the board swallows a lost run's error and the workstream
  // request is recorded completed — see §7c.
  onError: "fail",
});
```
...
**(c) Decision 2's three endings, settled by running them.** This row of the design has been
revised six times and every revision was refuted by someone reading one more surface. The
version below was arrived at by **executing all five candidate settlement routes** against a real
board with a real dependent task, inline and through the real detached hand-off — see the POC
block at the end of this section. Treat the table as evidence-backed and the surrounding prose as
the reasoning that survived.

**Two objects, not one.** Every earlier analysis discussed the **task row**. The **workstream
request** is a separate record, it is what LAB-135 reads, and it can disagree with the row. Both
have to be right, and one setting decides the second:

| Ending | Mechanism | Task row | Workstream request |
|---|---|---|---|
| Agent failed on its own terms | worker **cancels** its own row, clears the claim, returns | `cancelled` — terminal, dependents **blocked**, not retried | completed (the run *ran*; the ending is on the row) |
| Run was **lost** (abort, lease loss) | worker **rethrows** | re-pends with allowance; declined at the fence if displaced | **failed** — requires `onError: "fail"` |
| Succeeded | worker returns | `completed` | completed |

**Why the worker cancels rather than returning an errored result.** The intuitive version — and
the one this spec carried until the POC ran — was for the worker to return an errored handle. It
does not work: `recordSuccess` calls `collection.complete()` **unconditionally, blind to the
output's shape**, so the row lands `completed` and is indistinguishable from real success. A
dependent with `deps: [target]` then **runs** (`depsSatisfied` tests `status === "completed"`),
and the board reports `terminationReason: "all-completed"`. A `maxAttempts: 5` budget is never
consulted — not correctly bypassed, simply not looked at. That is failure laundered into success,
and downstream work proceeding on a failed dependency.

The POC mapped every route for one with the shape decision 2's first row needs — terminal,
non-`completed`, deps-blocking, not retried. **Two have it:** `cancel()`, and `fail()` on a task
with no retry budget. The second is unavailable here, because the *lost* row requires an
allowance to be present and `fail()` re-pends whenever one is. **`cancel()` is the only verb that
serves both rows at once.**

A worker settles its own row through the pattern `spawn-detached` already uses: settle, then
clear `currentClaim` from the worker-body state, and `recordSuccess` sees an empty slot and
returns without settling. It is also belt-and-braces — `recordSuccess` passes `ifAllowed: true`,
so an already-terminal row is *declined* rather than throwing, even if the clear is missed.

**`cancel()` is a semantic wart, and it is recorded rather than hidden.** We are borrowing a
cancellation verb to mean "failed terminally with a reason" because the board has no verb for
that. Consequences: a coding failure appears on the board as a `cancelled` row, and anyone
reading later must not infer a human cancelled it. This is the concrete evidence behind
[FIX-1181](https://linear.app/fixpoint-labs/issue/FIX-1181) — no longer hypothetical.

**Why not discriminate in the rescue recorder** (an earlier draft's answer): `createRecordError`'s
only error write is `TaskCollectionRef.fail()`, which re-pends when retry budget remains, and the
transition options expose no force-terminal flag. Forcing it terminal would disable configured
retries for every unrelated task-board worker. The recorder is shared; leave it alone.

**The setting the *request* status depends on.** The board defaults to `onError: "skip"`, which
captures the error on the task and lets the worker sequencer continue — so a lost run's rethrow
is **swallowed and the workstream request is recorded completed.** The task row would be right
and the record LAB-135 reads would say the run succeeded, which is this layer's own proof
reporting green on the exact failure it exists to expose. **This board must set
`onError: "fail"`**, which rethrows after the `fail()` write so the board fails and the request
is recorded failed.

That composes with the `cancel()` fold rather than fighting it: after the fold the *only* throw
a worker produces is a lost run, so failing the board on a throw is exactly right and an
agent-side failure never reaches it. **Verify the request status, not only the row** — and if
propagation turns out not to reach the request record, narrow the contract in §12 and say so
rather than promising behaviour the substrate does not have.
...
- **Abort, claim still valid, retry budget remaining:** re-pends. Recoverable — and re-pending
  *is* the recovery, so no special handling is needed. Confirmed by execution: two real detached
  dispatches, `attempts=2`, a fresh child finished the work.
- **Abort, claim still valid, `maxAttempts` unset:** terminal `errored`. **Not recoverable** —
  the killed run is written off. Also reproduced.
...
This is only safe *because* an agent-side failure now cancels instead of throwing: with failures
also throwing, an allowance re-runs the agent over its own half-finished work. That is not a
worry — the POC ran it, and `maxAttempts: 3` with a throwing worker spawned **three separate real
detached child sessions for one task.** **The two halves are load-bearing together and neither
works alone.**

Do not confuse the two budgets: `deniedByBudget` in `routeFailure` never flips true on this path.
It belongs to the collection-wide `maxTotalRetries` (FIX-948), which is a different budget from a
task's own `maxAttempts`.
...
**POC — throwaway, on this branch, never merged.** `spec-poc/LAB-133-decision-2-state-space/`
(setup and four run commands in its README). `route-map.mts` runs every settlement route against
a real 2-task board; `detached-three-way.mts` runs the three endings through the real
`dispatch: { mode: "detached" }` hand-off plus the coupling-harm demonstration;
`workstream-readback{,-sqlite}.mts` run §5's readback against the real HTTP route on two store
adapters. **What it showed:** refuted the return-an-errored-handle mechanism (the headline
above); confirmed `cancel()` as the only route serving both rows; confirmed the retry-allowance
condition, the coupling harm, and the displaced-claim fence (with a real 1.15 s lapse past a 1 s
lease, not a simulation); and corrected §5's readback shape and its adapter dependence (§7f).

**(c-bis) The block's own change is one line of scope, and it must be gated.** Under the fold
above the block barely changes: a terminal error subtype **already** returns an errored handle
today, so an agent-side failure needs nothing new. The one change is a **mid-stream SDK throw
that is not an abort** — under decision 2 that is an agent-side ending and must reach the worker
as a value it can act on, not as an exception.

**Gate that on the opt-out.** §4 practice 3 promises a caller without the opt-out keeps today's
behaviour, and a mid-stream throw *is* today's behaviour on the foreground path. Ungated, an
ordinary `claudeCodeAgent` call whose query throws would start returning an errored handle, its
caller would read a normal return, and the foreground request would look successful — the same
defect as the workstream-request one above, in the same shape, on the path this issue is not
even touching. So:

- opt-out **set**: non-abort mid-stream throw → return an errored handle (the worker then cancels).
- opt-out **absent**: rethrow, exactly as today.
- **aborts rethrow on both paths** — that needs no gate, since it is also today's behaviour.
...
3. **The block's gated throw-conversion** (§7c-bis): with the opt-out set, a non-abort mid-stream
   throw becomes an errored handle; without it, today's rethrow. Aborts rethrow either way.
   *Test:* both paths, and that the foreground one is unchanged. *Depends on:* 2.
4. **The worker cancels its own row on an agent-side failure** (§7c) — the step decision 2 rests
   on. Cancel, clear `currentClaim`, return. *Test:* all three rows of decision 2's table,
   asserted on the resulting **task row and workstream request**, including that a dependent
   stays blocked. *Depends on:* 3.
5. **The board's two settings** (§7c) — `onError: "fail"` so a lost run's error reaches the
   request record, and a retry allowance so the lost row is genuinely re-claimable.
   Configuration, not code, and both load-bearing. *Test:* a lost run fails the workstream
   request and leaves the row re-claimable; with the allowance unset it settles terminally (pin
   both, so neither requirement can be silently dropped). *Depends on:* 4.
6. **The adapter and the composition** in the goal check (§7e): task goal/context → prompt, then
   the agent. *Depends on:* 2.
7. **The goal check** end to end — a real coding task, in-process, drain budget raised past the
   run (§9), on a store adapter that honours `withItems` (§7e-bis). *Depends on:* 5, 6.
8. **Docs** (§11).
...
| Case | Expected |
|---|---|
| Run outlives the drain budget, **and unwinds inside the shutdown window** | The abort propagates normally, so this is the ordinary lost path: rethrow → `recordError` → `fail()` re-pends with an allowance (terminal without one). No lease-lapse recovery is involved — the row is settled or re-pended before the process goes |
| Run outlives the drain budget, **and cannot unwind before exit** | Nothing runs. The row stays `in_progress`, the lease lapses, and recovery happens on the next claim with `abandonments` advancing — the abandon path proper. Past the allowance it settles `errored`. **Only this sub-case is the cancel→abandon→retry storm**; conflating it with the row above produces wrong tests and wrong operational guidance. The goal check **must** raise `detachedDrainTimeoutMs` past the expected run, with a comment saying this is the finding, not a workaround |
| Context aborted mid-run | Rethrow (§7c). With a retry allowance the row re-pends and is re-claimed; if the claim was already displaced the write is declined at the fence and the successor owns it. **With no allowance it settles terminally `errored`** — which is why the allowance is a stated requirement, not a nicety |
| A lost run, read back from state | The workstream **request** must read failed, not completed. Requires `onError: "fail"` (§7c); on the default `"skip"` the error is swallowed and the run reads as a success — the failure mode that would make LAB-135's proof report green on a lost run |
| A dependent task on a failed coding task | Stays blocked. `depsSatisfied` tests `status === "completed"`, and an agent-side failure lands `cancelled`. This is the behaviour the `cancel()` fold exists to get right — assert it, since the refuted version passed every row-level check while letting dependents run |
...
**CI specs — twelve behaviours.** The list is short on purpose: four further candidates were cut
...
- **All three rows of decision 2's table** (practice 2, §7c), asserted **on the resulting task
  ...
  the signal, not the outcome. Specifically: a lost run leaves the task **re-claimable**; an
  agent-side failure settles **terminally** and starts **no new attempt even with retry allowance
  remaining**; a success settles complete. Break each on purpose.
- **A dependent stays blocked on a failed coding task** (§7c). This is the assertion that
  separates the current mechanism from the refuted one — the version that returned an errored
  handle passed every row-status check while letting dependents run, so a suite without this row
  cannot tell the two apart.
- **A lost run reads as failed from state** (§7c) — the **workstream request**, not the task row.
  On the default `onError: "skip"` this reads completed, which would make the epic's own proof
  report green on a lost run. Assert the request record.
- **The board's two settings are load-bearing** (§7c): with the retry allowance unset a lost run
  settles terminally rather than becoming re-claimable, and with `onError` left at `"skip"` the
  request reads completed. Pin both, so neither can be dropped later without a test going red.
- **The throw-conversion is gated** (§7c-bis): with the opt-out absent, a non-abort mid-stream
  throw still **throws** — the foreground path is untouched. Same shape of defect as the request
  one, on a path this issue does not otherwise touch.
...
- **Framework gap, filed with evidence:** [FIX-1181](https://linear.app/fixpoint-labs/issue/FIX-1181)
  — the board cannot express "settle terminally with a reason"; retry-vs-terminal is decided by
  config rather than by what happened. This design routes around it by borrowing `cancel()`
  (§7c), which is why coding failures will read as `cancelled` rows until that lands.

**Settled by POC** — recorded so a later reviewer with no memory of the thread cannot reopen them
blind. Harness: `spec-poc/LAB-133-decision-2-state-space/` on this branch.

- **Returning an errored handle settles the task terminally** — **REFUTED**. `recordSuccess`
  calls `complete()` blind to output shape: the row lands `completed`, a dependent with
  `deps: [target]` runs, and the board reports `terminationReason: "all-completed"`. Reproduced
  inline and through the real detached hand-off.
- **Some existing route gives terminal, non-completed, deps-blocking, non-retried** — **CONFIRMED**:
  `cancel()`, and `fail()` with no retry budget. Only `cancel()` also serves the lost row, which
  requires an allowance to be present.
- **A lost run re-pends when retry budget remains** — **CONFIRMED**: two real detached dispatches,
  `attempts=2`, a fresh child finished the work. With no allowance: `errored`, written off.
- **Mis-signalling an agent failure as a throw re-runs the agent** — **CONFIRMED**: `maxAttempts: 3`
  with a throwing worker spawned three separate real detached child sessions for one task.
- **The displaced-claim fence declines a stale settle** — **CONFIRMED** with a real lease lapse
  (1.15 s past a 1 s lease), not a simulation.
- **`include_items` behaviour is adapter-dependent** — **CONFIRMED**: the in-memory store ignores
  the flag by documented contract; `store-sqlite` branches on it (§7e-bis).
...
- **After spec review, decision 2 was found false in both directions** — a run we *lost* was
  ...
  difference instead**, so orchestration is untouched and the change is back to one package. One
  condition rides with it: coding tasks must carry a retry allowance, or a lost run settles
  terminally rather than becoming re-claimable.
- **After a POC ran decision 2 rather than reading it** — the "agent-side failure *returns* an
  errored handle" half was **refuted by execution**: `recordSuccess` calls `complete()` blind to
  output shape, so the row landed `completed`, a dependent task ran, and the board reported
  `all-completed`. Failure laundered into success. The POC mapped every settlement route and found
  `cancel()` is the only one that is terminal, non-completed, deps-blocking, non-retried **and**
  compatible with the lost row needing a retry allowance — so **the worker now cancels its own
  row**, a semantic wart recorded rather than hidden and now the evidence behind FIX-1181. Two
  further defects came with it, both in surfaces nobody had looked at: on the board's default
  `onError: "skip"` a lost run's error is swallowed and the **workstream request** — the record
  the epic's proof reads — says the run succeeded, so the board must set `onError: "fail"`; and
  the return-instead-of-throw change had to be **gated on the opt-out**, since ungated it would
  make a foreground request look successful after a mid-stream throw. The drain case was also
  split: a run that unwinds inside the shutdown window takes the ordinary lost path, and only one
  that cannot unwind before exit reaches lease-lapse recovery.
