...
deployment choice; `runtime-detached-start.test.ts` covers both topologies). The board
*rejecting* today's block is `task-board-detached-config.test.ts:155`. The abandonment path is
`lease-recovery.test.ts` and `collection/recovery.test.ts`. Cross-request resume is
`agent.spec.ts`.
