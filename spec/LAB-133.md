...
The correction: when a run is **killed** — a shutdown, a lost claim — the board currently writes
it down as a job that *failed*. It didn't fail; we lost it, and it should be picked up again.
Today nothing tells those two apart, which makes the retry rule below false exactly where it
matters most. One branch in the board's failure recorder fixes it.
...
2. **A coding run has three endings, not two, and the board must tell them apart.**
   ...
   | What happened | Where the row ends up | Today |
   |---|---|---|
   | We **lost** the run — shutdown, lease loss, cancellation | **Recoverable.** Another attempt may claim it | Settled as failed. Never picked up again |
   | The run **failed on its own terms** | **Terminally settled**, reporting why | Returned to `pending` and **re-run**, when the task carries `maxAttempts` |
   | The run **succeeded** | Settled complete | Correct |

   *Rejected:* letting either failure fall through to the board's retry. Re-running a coding
   ...
   **Two reviewers found this hole from opposite ends** — one that a killed run is written down
   as failed, one that a failed run is re-run — which is the tell that the *wording* was doing
   too little work, not just the mechanism. Both are the same defect: the runner has a two-way
   split (throw means fail, return means complete) where the decision needs three. So it is
   **one discrimination in one rescue path**, not two patches (§7c). The block half already
   ships and is tested: a terminal error subtype already returns an errored handle rather than
   throwing.
...
**Size:** Small — 4 production files across two packages · 8 test behaviours · 2 doc surfaces ·
1 PR. (The second package is one branch in the board's failure recorder, §7c.)
...
**Packages touched: `@flow-state-dev/claude-code`, plus one branch in
`@flow-state-dev/orchestration` (§7c).** `@flow-state-dev/engine` is untouched — the start gate,
...
**(c) The runner has two outcomes and decision 2 needs three. This is the framework work.**

The worker body is `.tap(recordSuccess).rescue([{ block: recordError }])`. A **return** routes to
`recordSuccess`; a **throw** routes to `recordError`, which calls `collection.fail(claim.taskId, …)`
unconditionally. Two exits, and neither produces the third outcome decision 2 requires. Both
failures the reviewers found follow from that:

- **A killed run is written down as failed.** The block forwards `ctx.signal` to the SDK's abort
  controller, so a shutdown drain or a lease loss rejects through the same mid-stream path a
  genuine agent failure takes. `recordError` fails the row: terminal, never re-claimed.
- **A failed run is re-run.** `fail()` is not always terminal — `shouldRetryOnFail` re-pends the
  row when the task carries a `maxAttempts` it has not spent. So a transient SDK error *after the
  agent has already modified files* launches a second side-effecting attempt over its own
  half-finished work. (Conditional on `maxAttempts` being set — it is optional and defaults to
  no retry — but a board driving real work will set it, and decision 2 should hold either way.)

**One discrimination fixes both:** the rescue path must tell **abort/cancellation** from an
**agent-side ending**.

- Aborted → **skip the settlement write entirely and return.** The row stays `in_progress` with a
  lapsing lease, which is the state lease recovery already handles and the state a killed run
  should be in. Stopping lease renewal there is right and already happens.
- Agent-side ending → settle **terminally**, without the retry re-pend.

Three notes for the implementer, none of which changes the direction. `recordError` is **shared
with the inline worker path**, so the branch touches both — almost certainly correct, since an
aborted inline worker marked failed is the same bug, but it is a behaviour change on a path this
issue is not otherwise touching, so cover both. Writing nothing on an abort is the safe direction
anyway: the commonest abort is the shutdown drain, where the stores are closing under the write.
And **if this turns out not to be discriminable in one rescue path without a larger change, stop
and escalate rather than shipping decision 2 with a hole under it** — the fallback is to narrow
decision 2 to what the design delivers and file the gap.
...
3. **The block's abort rethrow** (§7c): surface an aborted context and abort-like rejections as
   a throw, distinguishable from an agent-side ending. *Depends on:* 2.
4. **The three-way outcome in the runner** (§7c) — the step decision 2 rests on. One
   discrimination in the rescue path: abort → skip settlement, leave the row recoverable;
   agent-side ending → settle terminally, no retry re-pend. *Test:* all three rows of decision
   2's table, on the detached path, plus no regression on the inline path. Escalate rather than
   improvise if it needs more than a branch. *Depends on:* 3.
...
| Context aborted mid-run | Rethrow in the block **and** skip settlement in the rescue recorder (§7c). The row stays `in_progress` with a lapsing lease and is re-claimed. Today it is settled `failed` and never picked up — this is the case the fold exists for |
...
- **What must not change** (practice 3): with the opt-out absent, the schema is still declared.
  ...
  regression today.
- **All three rows of decision 2's table** (practice 2, §7c), asserted **past the rescue
  recorder** — a test that only checks the block throws passes while the board still fails the
  row, which is the bug. Specifically: a killed run leaves the task recoverable and **not
  settled**; an agent-side failure settles **terminally** and starts **no new attempt even when
  `maxAttempts` remains**; a success settles complete. Break each on purpose.
- **The three-way split does not regress the inline worker** (§7c): the recorder is shared, so an
  ordinary inline failure must still settle the row failed.
...
and the board's rescue recorder — all reachable without a server, so every behaviour above is a
tracer bullet. `packages/integration-tests/src/scenarios/task-board-detached-on-error.test.ts` is
the closest existing shape for the abort behaviour.
...
- **After spec review, the one thing that grew** — decision 2 turned out to be false in both
  directions, and the wording was as much at fault as the mechanism. The runner has a two-way
  split (throw fails the row, return completes it) where the decision needs three outcomes: a run
  we *lost* was being settled as failed and never re-claimed, and a run that *failed on its own
  terms* was being re-pended and re-run over its own half-finished work whenever the task carried
  `maxAttempts`. Two reviewers found one end each. Decision 2 is now stated as an explicit
  three-row table and the fix is a single discrimination in one rescue path, in a second package.
  Three smaller corrections landed with it: the opt-out must also stop the SDK **resume**, or a
  ...
  the SDK is invoked; and the opt-out was confirmed to require a construction-time flag, since the
  board inspects the block's static definition before any context exists — the runtime branch an
  earlier draft proposed could not have worked.
