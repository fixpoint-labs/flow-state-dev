...
2. **The proof rests on what the run *did*; what it *thought its job was* is graded separately
   ...
   which either pins the kill line to a surface half of real runs never populate, or quietly
   counts an unexercised half as proven. *Locks in:* we sign the epic off on evidence that
   exists, and accept that a run which kept no plan **proves nothing about the plan half** —
   the verdict log records which arm each run took, so if it never fires we find out rather
   than assume.
...
counts    items 34 · tool_outputs 9 · file-op rows 3 · plan rows 0 · pages followed 1
```
...
PASS — 3 of 3 held-out paths present, each with a recorded kind and a settled outcome;
       the item stream and the file-op collection name the same 3 mutations, both ways;
       order non-decreasing across 9 tool_outputs at 6 distinct positions.
...
FAIL — src/summary.ts appears in the item stream as a Write and has no row in the
       file-op collection: the graph lost a tool-driven mutation
...
- **Whether a plan was kept** comes from the **plan collection**; **why it is empty** comes from
  ...
  *harness transcript*.
- **Nothing parses a tool result's prose.** The path is a field.
...
file-mutation tool names, used only for the stream-versus-collection agreement check. It must
match the recorder's shipped table; nothing else in the reader knows a tool name.
...
>     stream and collection name the same mutations -> a difference either way FAILS
...
2. **The grader**, over the account and the held-out expectation, with a can't-tell branch on
   ...
   between surfaces, and carrying each of the three plan arms; each must resolve for its own
   named reason. *Depends on:* 1.
...
| A path in the stream with no collection row, or the reverse | **FAIL** — a tool-driven mutation the graph lost |
...
| Either collection read returns 403 | Abort before grading — the reader cannot work, and this is the predecessor's assertion, not ours to re-grade |
...
**Second path (BP-035):** the empty-plan run (measured at roughly half of real runs); the
multi-page collection read; a second run in the same workstream, whose rows are namespaced per
run and must not be mixed in; and the calibration path, which must be runnable with no model.
...
exactly, or the goal stops before spending a run. Then the run must have finished and both
collection reads must have returned 200.

**Eight graded assertions**, in observable terms:

1. Every held-out path appears in the account with a recorded kind and a settled outcome
2. The item stream and the file-op collection name the same set of mutations, both ways
...
7. No page was left unfollowed — every `nextCursor` exhausted
...
about what the run **did** and are always evaluable. Assertion 5's UNMEASURED arm does not fail
the goal and does not silently pass either: it is named in the evidence, and the verdict log
records the arm per run so a drift toward never exercising the plan half is visible. This is
the predecessor's three-verdict rule applied to one half of a check whose other half still
resolves — same table, same "must name why", same logging.
...
supplies the two collections the account reads. Nothing in this epic merges until sign-off.
