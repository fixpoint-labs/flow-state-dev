...
**The stub must expose `generateStep` / `streamStep`, not just `generate` — and this is
load-bearing on decision 1.** A `generate()`-only mock is the obvious shape to reach for and it
silently measures the wrong thing: it drives `generator.ts`'s **legacy SDK-owned path**, a
compatibility shim for adapters that lack step methods. Production never takes it — the comment
at `packages/core/src/blocks/generator.ts:2929` says so ("the real AI-SDK adapter always takes
the owned path"), and `packages/core/src/models/createAiSdkModelResolver.ts:1153-1154` wires
`generateStep` / `streamStep` on every real model. Measuring the brief on a shim production
never executes is exactly the "stand-in written for the test" decision 1 rejects, and it fails
**green**, which is the dangerous direction. Follow the shape in
`packages/core/test/generator-owned-loop.test.ts`.
...
**Which context shape the precondition depends on** (read from source, not run): object-form
context — `context: { tag: … }` — aggregates into the single combined system message, which is
what makes "exactly one" true. A raw **string** context entry takes the `passThrough` branch in
`packages/core/src/blocks/context-aggregator.ts` and becomes an *additional* leading system
message. That is outside the recipe assumed here, so it is not a defect — but it is precisely
what the precondition assertion exists to catch, and a reader should know the assertion is
shape-dependent rather than universal.
...
the goal:
    ...
    eight times:
        ...
        AND capture what each slot function returned on THIS pass
    ...
    require the standing half IDENTICAL between them
    require the changing half DIFFERENT between them               ← or the test is half a test
    require each changing half to carry ITS OWN issue's fixture facts
    ...
    on a red signal, before reporting anything (decision 3):
        compare the slot outputs CAPTURED ALONGSIDE those same eight runs
...
3. **The recipe and the capture** in `run.mts` — the generator, the engine context with an
   injected `generateStep`/`streamStep`-capable recording resolver, and one named comparator
   used by every check below. Each run captures **two** things: the messages the stub was
   handed, and what each slot function returned on that same pass (step 7 needs the second).
   *Depends on:* 2.
4. **Signal: eight passes, byte-identical.** *Depends on:* 3.
5. **Signal: the precondition, then two issues** — standing half identical, **changing half
   different**, and each changing half carrying its own issue's fixture facts. The last two are
   what stop the check passing when issue B is never really assembled. *Depends on:* 3.
...
7. **The localization step** decision 3 requires: on a red signal, compare the slot outputs
   captured alongside those same eight runs — never a fresh re-run, which samples different
   values and can misattribute recipe drift to the seam. *Depends on:* 3, 4.
...
| Drift is in the recipe's own formatters, not in assembly | Localize before reporting (decision 3). The check cannot tell the two apart on its own — that is what step 7 in §8 is for. Localization must read the slot outputs captured **on the same runs**; a fresh re-run is a different sample and can report "slots were stable" for drift the slots caused |
| Issue B is never really assembled — omitted, or the tail formatter renders issue A both times | **This must fail, and without the extra assertions it would pass.** All three signals stay green: the eight same-issue captures agree, the standing halves "match" because the two briefs are the same brief, and the anti-game check still fires. Requiring the changing halves to **differ**, each carrying its own fixture's facts, is what closes it |
| Two issues produce different-length changing halves | Fine. Only the standing half is required identical; the changing half is required *different* |
...
- **Anti-game** — the drift check, plus the two vacuous-green holes it does not cover. A hollow
  pass looks like: eight identical passes over a comparator that could never have returned
  false; a "byte-identical" claim over a brief so thin nothing in it could drift; **or a
  standing-half comparison between two briefs that are secretly the same brief** — which is why
  the changing halves must be required to differ and to carry their own fixture's facts. The
  check must not assert on `assembleMessages`'s return value directly — that is the unit suite's
  job; this grades what the model was handed, through the step-method path production uses.
...
- **Settled: the standing/changing boundary is not readable from an emitted item for a
  tool-free generator — CONFIRMED.** `systemPrefixCount` reaches an item only through
  `emitGeneratorStep`, whose two call sites both sit behind a guard that breaks first when a
  step is terminal with no framework tool calls. A throwaway POC ran it rather than reading it:
  tool-free non-streaming emitted **0** `generator_step` items (all emitted types `[]`, result
  `done`); the same harness with a tool emitted **1**; tool-free streaming called `streamStep`
  once and emitted real `message` / `content.*` items but still **0** `generator_step`. Full
  core suite green at 2011/2011 before the throwaway was deleted. So the §7 derivation stands,
  now on evidence rather than on a source read.
- **Settled: a `generate()`-only stub measures the wrong path — CONFIRMED**, from the same run.
  It drives the legacy SDK-owned branch, which contains no `emitGeneratorStep` call sites at all
  and which production never takes. The stub must expose `generateStep` / `streamStep` (§7).
  This one is why the POC was worth running: the wrong shape produces a **passing** check.
...
- **After POC settlement** — required the model stub to expose `generateStep` / `streamStep`,
  because the run showed a `generate()`-only stub drives a legacy compatibility path production
  never takes, and measuring the brief there would have passed green while testing the wrong
  mechanism. Both settled claims moved from source-read to evidence-backed in §12.
- **After spec review** — required the two-issue check to assert the changing halves **differ**
  and carry their own fixture's facts, because standing-half equality alone passes when issue B
  is never really assembled. Moved localization onto slot outputs captured during the same eight
  runs rather than a fresh re-run, because a re-run is a different sample and can blame the seam
  for the recipe's own drift. Both were vacuous-green holes of the same kind the spec already
