...
2. **A second attempt re-runs the agent from the beginning, in the checkout the first one
   left behind.**
   *Rejected:* carrying the run's conversation across attempts — continuing a coding session
...
**Size:** Large — 12 production files · 14 test behaviours · 4 doc surfaces · 2 PRs.
...
```
GET /sessions/<conductor session>/resources/runs?topicPrefix=runs/FIX-1166/implement

{ "sdkSessionId": "…", "workspacePath": "/…/worktrees/FIX-1166-implement",
  "branch": "fix/FIX-1166", "attempt": 2, "lastOutcome": "failed:max-turns",
  "costUsd": 1.94 }
```
...
- **The manager** is a sequencer composed as that board's one detached worker: provision →
  ...
  detached worker in a flow becomes a route on one shared workstream flow.
...
6. **The run record** — the `runs/*` collection and its schema. *Test:* a row round-trips and
   is readable from the conductor session while written from a child. *Depends on:* 5.
...
- **Continuing a coding session** across a wait is **FIX-1179 / FIX-1246**'s, consumed by the
  ...
  what that costs a retry. The epic's own limit on this remains open and is not touched by this
  spec.
