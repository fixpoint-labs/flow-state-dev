...
| Two answers race the same row | **The first always takes effect and the row is never overwritten** — the conditional `open → answered` refuses the second write (§7). The second is reported as refused **except inside the recovery window** (task still `pending`, row `answered`), where it is indistinguishable from a replay of the first and reports a drain instead. Its body is dropped either way; the row it names is already answered. **Telling those two apart would put a body comparison inside the one admitted exception** — the window is FIX-1244's to remove, not the guard's to grow |
...
- **EXTEND** `labs/conductor/README.md` — a section on asking and answering: what an operator
  ...
  FIX-1246 lands; a second answer landing in the narrow recovery window is dropped rather than
  refused, §9). Labs carry their own README and document their known gaps.
...
- **A named limit, not a task: `answer`'s refusal is right on the common path, not on the
  ...
  operator about:** a second answer that lands inside it reports a drain rather than a refusal,
  and its body is dropped — the row it names is already answered (§9). Say so in the README's
