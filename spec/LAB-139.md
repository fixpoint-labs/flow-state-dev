...
**The `answer` action reads the board row first, and refuses locally when it is not held.**
...
On a held row the action then does two writes: patch the inbox row to `answered`, then
...
the answer action, in the coordinator's own session, zero-model:
      ...
      read the board row → NOT held for review? refuse, write nothing, stop
...
- **A second answer over an already re-queued row reports `recorded`** and overwrites the row's
  ...
  option refuses this, **§7's `answer` reads the board row and refuses before it calls** — the
  duplicate, the terminal row and the never-parked row are all declined in conductor's own
  action, on the common path. What that cannot cover is the window between the read and the
  call (§13). **So FIX-1244 buys the race, not the behaviour**: §9's rows ship this cycle and
  the parked-only primitive hardens them, rather than being owed to a Backlog issue.
...
- **After review — a completion criterion that nothing shipped could satisfy, and the guard that
  makes it buildable.** §10's behaviour 12 (*answer a row that was never parked; the action
  surfaces the refusal*) sat under CI specs, while §12's own measurement showed no shipped verb
  produces that refusal: `resumeFromReview` passes no `requireFrom` and every remaining guard is
  behind `ifAllowed`, which this design does not adopt, so a never-parked `pending` row reports
  `recorded`. §9 was honest that the mechanism was with the product owner; §10 was not, and it
  had turned one of those rows into green CI. The fix is conductor's, not the owner's: the
  `answer` action now **reads the board row and refuses locally when it is not held**, before it
  calls — a product-level guard in the action, not a second primitive in the substrate. That
  shrinks what is carried to the gate rather than growing it. The duplicate answer, the terminal
  row and the racing answers are all declined on the common path, and **FIX-1244 now buys the
  race rather than the behaviour** — §13 names that window as a limit, and §12's measurement
  stands with its conclusion rewritten rather than retracted.
