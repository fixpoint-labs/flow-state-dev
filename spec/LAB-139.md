...
over.** The named row must be **the question the task is sitting on**. That is its key's attempt
segment against the task's `attempts` — equal while the task is parked or `pending`, and
**exactly one behind** once this answer's own wake has claimed the row, because a claim advances
the counter (`applyClaimToTask`). The correlation is on the **question**, not on the counter;
reading it as counter-equality is what made a failed wake unrecoverable below. The correlation is already in the key, so this costs no
field and no body comparison. It is a precondition of the whole action, proceed and recovery
alike: **the row named and the question the task is actually sitting on have to be the same
thing.** Proceed happened to exclude the mismatch already, because it requires the row `open`
and the withdrawal arms clear an earlier attempt's open row — but that made it a consequence of
another arm being correct rather than a property of this one, and answered rows are retained
forever, so recovery had nothing excluding it at all.

**Then recovery is one rule, not a list of admitted pairs: a named row from this attempt that is
already `answered` means the patch is durable, and the action restarts the sequence from that
prefix.** It never re-patches. It reads the task to see how far the sequence got:

- task **parked** → `resumeFromReview`, then drain.
- task **`pending`** → drain only.
- task **`in_progress` that nobody owns** — the lease has lapsed — → drain only. The drain's
  own lease-lapse reclaim is the recovery; nothing here reaches inside it.
- **anything else** → decline: a live run holds the row, or the task is terminal.
...
the answer action, in the coordinator's own session, zero-model:
      ...
      FIRST: the row's attempt segment must equal the task's `attempts`
             → not this attempt's question? decline, whatever the statuses say
...
5. **The `answer` action**, zero-model: **the attempt correlation first** (the named row's
   attempt segment equals the task's `attempts`, for every path), the **paired guard** (task
...
| The wake **claims and then fails to hand off** | `attempts` has advanced and the row is `in_progress` owned by nobody, so the answer is durable and nothing is running it. Recovery's third arm drains; the substrate's lease-lapse reclaim does the rest (§7). The correlation reads the *question*, not the counter, which is why the answer is still recognised as this attempt's |
...
| An answer names a row from an **earlier attempt** | Declined before any status is read — the row's attempt segment must equal the task's current `attempts` (§7). Covers the retained `answered` row of an answered earlier question, which otherwise matches recovery against a task parked on a later one |
...
- **Round four — two self-contradictions and the window the last fold opened.** The correlation
  ...
  `pending`, one behind once this answer's own claim has advanced it), and recovery gained a
