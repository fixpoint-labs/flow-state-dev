...
over.** The named row must belong to **the attempt the task is on** — its key's attempt segment
equals the task's current `attempts`. The correlation is already in the key, so this costs no
...
- **anything else** → decline — terminal, `in_progress`, any status saying the answer already
  ran or the job has moved on.
...
16. **The half-finished answer recovers, from either prefix.** Two arms, and a suite with only
    ...
    `answered`, task `pending` — and re-run: it drains only. Then run each once more: still one
    dispatch, nothing rewritten.
