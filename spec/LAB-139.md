...
On a parked row the action then does three things, and the third is the one easy to leave out:
...
the answer action, in the coordinator's own session, zero-model:
      ...
      read the board row → not parked? refuse, write nothing, stop
...
5. **The `answer` action**, zero-model: the **parked-only guard** (`awaiting_review`; every
   other status refuses locally), the row patch, `resumeFromReview` with no feedback, and **the
   `wake` drain the action runs itself**, naming the coordinator session. Plus the `status`
   action's inbox half. *Test:* §10's behaviour 12 for the guard, and behaviour 7 **driven
   through `answer` alone** — a test that drains by hand passes with the missing drain in place.
   *Depends on:* 1, 4.
