...
**So recovery is one rule, not a list of admitted pairs: a named row that is already `answered`
means the patch is durable, and the action restarts the sequence from that prefix.** It never
re-patches. It reads the task to see how far the sequence got:
...
5. **The `answer` action**, zero-model: the **paired guard** (task `awaiting_review` **and**
   row `open`) and **the recovery rule** — an already-`answered` row means the patch is durable,
