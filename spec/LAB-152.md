...
The only written-down definition of what a coding harness is given and what it hands back
lives inside the Claude Code package. The run manager reads six things off that handle —
status, session id, how the run ended, its final message, its token usage and its cost — and
four of them exist only on Claude Code's own extension of the shape, not on anything shared.
The field that says where a run came from lists exactly Claude's two doors.

So a second harness cannot build against a contract without installing Claude Code to get the
types, and a manager that wants to drive "any harness" is today programming against an
accident of one package's internals.

**Why now** — the Codex harness (LAB-153) and the manager's harness slot (LAB-154) both build
against this shape. They are specced in parallel and cannot land before it does.

### 2. Solution in plain terms

Move the contract — what a harness is handed, and what it returns — into the framework's core
package, which every harness package already depends on. Open the "where did this come from"
field so it can name any harness. Add to the shared handle the four fields the manager already
reads, plus one honest bit: whether the cost figure was reported by the harness or estimated
from usage. Add to the shared input an optional *session to resume*, and make the Claude Code
harness honour it on the background-work path, which is where the manager runs it.

Claude Code's own types become the neutral ones plus Claude's extras. Every existing caller
and every handle already saved in session state keeps working, unchanged.

**Philosophy** — tenet 4 (*infrastructure, not knowledge*): the contract names capabilities —
dispatch, hand back, resume by id, abort — and never a vendor's notion; a clause naming a
Claude idea is a defect. Tenet 5 (*fix at the owning layer*): one shape where every harness
and the manager converge, instead of each consumer re-deriving it from Claude Code. Tenet 3
(*earn every addition*): no new package; the public surface grows by one type module.
