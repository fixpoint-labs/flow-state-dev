...
5. **`source` is open, but its values are a rule: `<package>/<door>`, and the two are
   `claude-code/sdk` and `codex/sdk`.**
   ...
   *Locks in:* every conforming harness names itself `<package>/<door>`; the SDK door here emits
   `claude-code/sdk` and the Codex harness (LAB-153) `codex/sdk`. The type stays a plain string
   so a harness we don't ship can name itself — the rule, not the type, is what makes the values
   comparable. Handles persisted under the old enum (`"sdk"`, `"cli-remote"`) still parse
   unchanged (BP-030, §9), and nothing branches on the value today.
...
| Field | Meaning | Today |
|---|---|---|
| source | which harness, and which door, produced it — a string, required to read `<package>/<door>` (decision 5) | enum of Claude's two doors → widened |
...
| Case | Expected |
|---|---|
| Handle persisted before this change, `source: "sdk"` or `"cli-remote"` | Parses; the field is a string now. New SDK-door handles carry `claude-code/sdk` (decision 5); nothing branches on either value |
...
- the SDK door's handle names itself `claude-code/sdk` — the `<package>/<door>` rule
  (decision 5)
