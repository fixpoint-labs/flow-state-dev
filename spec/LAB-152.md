...
4. **The harness slot's three feeds are typed here, in core beside the contract.**
   The resolver signature the working directory and the session to resume arrive through, and
   the session hook — named **`onSession`** — are exported from `@flow-state-dev/core` with the
   input and the handle. LAB-153 and LAB-154 import them.
   *Rejected:* the epic's own "whichever of LAB-153/LAB-154 lands first adds them" (theme 7, as
   amended 2026-09-02). The two land in parallel, so that races two owners to define one shape —
   and the epic's end-state POC caught them already disagreeing, the hook being `onThread` in
   one spec and `onSession` in the other.
   *Locks in:* one declaration, in the contract's own package, before either consumer needs it.
   These are *feed* signatures — how a host hands a harness its per-run configuration — not
   fields on the input or the handle (decision 2, §7).

5. **`source` is open, but its values are a rule: `<package>/<door>` — and the rule binds every
   writer of the field, not only the manager-drivable harnesses. The three values are
   `claude-code/sdk`, `claude-code/cli-remote`, and `codex/sdk`.**
   *Rejected:* leaving the convention as an unenforced note on the field. LAB-154 records
   `source` on the run row expressly so LAB-141 can later check that a run came from the harness
   it dispatched — a check that compares strings, so unpinned values make it a check of nothing.
   *Also rejected:* pinning only the harness values and leaving the CLI door's `cli-remote`
   off-convention. Both doors write `source` today, so a partial rule leaves one unpinned
   literal in the same field — the "free-form string nobody pinned" problem this decision
   exists to remove, re-created in miniature. Stating the rule once for every value the field
   can hold is cheaper than applying it to a subset and documenting the exception. The CLI door
   not being a manager-drivable harness is why it is never *selected* as one; it is not a reason
   its `source` should be shaped differently.
   *Locks in:* every writer of `source` names itself `<package>/<door>` — the SDK door here
   emits `claude-code/sdk`, the CLI door `claude-code/cli-remote`, and the Codex harness
   (LAB-153) `codex/sdk`. The type stays a plain string so a harness we don't ship can name
   itself — the rule, not the type, is what makes the values comparable. Handles persisted under
   the old enum (`"sdk"`, `"cli-remote"`) still parse unchanged (BP-030, §9), and nothing
   branches on the value today.
...
**Size:** Small — 5 production files · 9 test behaviours · 2 doc surfaces · 1 PR.
...
```mermaid
flowchart BT
  CORE["@flow-state-dev/core<br/>types/harness — input · handle · schemas · HarnessBlock alias · feed types"]
...
**The three feeds** — the contract also exports the signatures a host feeds that configuration
through, because both consumers declare them today and land in parallel (decision 4): one
resolver signature, used for the working directory and for the session to resume
(`(ctx) => T | Promise<T>`), and the session hook `onSession`
(`(sessionId, ctx) => void | Promise<void>`), which a harness calls the moment its vendor names
a session. Types only — nothing in this issue reads them; LAB-153's harness and LAB-154's slot
import them from core rather than each declaring their own.

> **Correction (2026-09-05, folded back from implementation).** The resolver originally took
> the run's input alongside the context (`(input, ctx) => …`); that parameter is gone. The prompt
> is the one value a caller — or a model holding the block as a tool — controls, so a resolver
> that can see it can be written, in ordinary-looking code, to derive the working directory, the
> sandbox fence, or the session to resume from caller-controllable input (BP-031) — and the
> sandbox case is the sharp one, because a fence drawn from the caller's own text is not a fence.
> Documentation could not close it, so a resolver is handed the context and nothing else.
...
| Field | Meaning | Today |
|---|---|---|
| source | which harness, and which door, produced it — a string, required to read `<package>/<door>` by every writer (decision 5): `claude-code/sdk`, `claude-code/cli-remote`, `codex/sdk` | enum of Claude's two doors → widened, and both doors restate their value under the rule |
...
**Conformance** is stated once, as a type alias in core — a block over the neutral input and
handle *types*: `BlockDefinition<any, any, HarnessRunInput, HarnessRunHandle>`, the
`TaskWorker`-shaped spelling, not the schema-typed one, which rejects every real harness (§13)
— and proved twice for Claude Code: at the type level (`claudeCodeAgent()` is assignable to the
alias) and at runtime (its returned handle parses against the neutral schema), which is the
half that carries the weight (§13). The alias is a type, not an installation point (epic theme
9). The neutral schema's *declared* shape carries no vendor field; an extended handle still parses against it — vendor
...
1. **Define the contract in core** — the input and handle types, their schemas, the block
   alias, and the two feed signatures (decision 4); exported from the `types` subpath (types)
   and the root (schemas). *Test:* the schemas accept a minimal neutral handle; a
   persisted-style handle with `source: "sdk"` still parses (decision 1, BP-030).
   *Depends on:* nothing.
2. **Claude Code conforms** — the shared envelope in `claude-code` becomes a re-export of
   ...
   envelope. Both doors take their `<package>/<door>` value (decision 5) — the CLI door's
   `cli-remote` is narrowed twice in `src/cli/types.ts` (a TS literal and a `z.literal`) as well
   as written in `src/cli/dispatch.ts`, so all three move together. Delete the package-local
   envelope module rather than keeping two. *Test:*
...
| Case | Expected |
|---|---|
| Handle persisted before this change, `source: "sdk"` or `"cli-remote"` | Parses; the field is a string now. New handles carry `claude-code/sdk` (SDK door) and `claude-code/cli-remote` (CLI door) — decision 5. Both legacy spellings dual-read to their new value, and nothing branches on any of the four |
...
- the SDK door's handle names itself `claude-code/sdk`, and the CLI door's
  `claude-code/cli-remote` — the `<package>/<door>` rule, which binds every writer (decision 5)
- the CLI door's handle still parses against the envelope; its `source` value is the only thing
...
- **EXTEND** `packages/core/README.md` → "Types (`@flow-state-dev/core/types`)": one paragraph
  naming the harness contract — what a harness is handed, what it returns, the resolver and
  `onSession` signatures a host feeds it through, and that any harness package declares these
  shapes. Cross-link → the Claude Code SDK page.
...
- **`packages/claude-code/README.md`** — the "Session state" block publishes the CLI handle as
  `source: "cli-remote"`; it becomes `claude-code/cli-remote` (decision 5). Nothing else beyond
  what the handle rename touches, and nothing a Claude Code user does differently.
...
- The working directory stays harness configuration, not input (§7). A manager with a block
  ...
  to pick; this contract precludes neither. It picks the channel, not the types: the resolver
  and `onSession` signatures are declared here (decision 4) for it to import.
...
- **Cursor, the `HarnessBlock` alias** ([thread](https://github.com/fixpoint-labs/flow-state-dev/pull/1534#discussion_r3909412081)):
  ...
  **Answered — keep it.** LAB-154 §7 types its `harness` slot against core's alias
  ("LAB-152's §13 asked whether the alias earns its place — this slot is the answer: yes"), so
  the alias has a real consumer and is not ceremony.

  **The spelling is not free** *(epic implementer note, from the LAB-140 end-state POC —
  `spec-poc/LAB-140-end-state/` on epic PR [#1362](https://github.com/fixpoint-labs/flow-state-dev/pull/1362);
  `pnpm tsc -p spec-poc/LAB-140-end-state/tsconfig.json`)*. Type-checking both candidate
  spellings showed the schema-typed one — a block whose input and output *schemas* are the
  neutral ones — **rejects every real harness**, because both Claude Code and Codex hand back
  the neutral handle *plus* a vendor extension. The spelling that accepts them is the
  `TaskWorker`-shaped one, `BlockDefinition<any, any, HarnessRunInput, HarnessRunHandle>`. Use
  it. And take the consequence with it: an alias typed over `any, any` proves less than it
  looks like it does, so **the runtime parse test — the returned handle parsing against the
  neutral schema — is the real conformance proof, and the type assertion is the cheap half.**
...
- **Cross-spec alignment (epic LAB-140, 2026-09-05)** — three records the epic already decided,
  folded in without reopening the design: the conformance alias is spelled
  `BlockDefinition<any, any, …>` and the runtime parse is its real proof (the end-state POC
  type-checked both spellings and found the schema-typed one rejects every real harness), which
  also answers §13's open alias question — LAB-154's slot types against it; this spec declares
  the harness slot's three feed types (the resolver signature and the `onSession` hook) rather
  than leaving them to whichever consumer lands first; and `source`'s `<package>/<door>`
  convention became a rule with its two values named, because LAB-154 records `source` for a
  check LAB-141 makes against it.
- **The `source` rule covers the CLI door too (2026-09-05)** — the alignment pass named
  `claude-code/sdk` and `codex/sdk` and left the CLI door's value open, since `claude --remote`
  is a non-goal for the manager. Pinned as `claude-code/cli-remote`: both doors write the field,
  so a rule that reaches only the manager-drivable ones leaves an unpinned literal in the field
  the rule exists to make comparable. Not being manager-drivable is why the door is never
  selected as a harness; it says nothing about how its `source` should read. Decision 5, §7,
  §8, §9, §10.
