...
seq.step(
  codexAgent({
    ...
    cwd: (ctx) => managerState(ctx).workspacePath,
    resume: (ctx) => managerState(ctx).previousSessionId, // null → fresh thread
...
resolver and the `resume` resolver (both the resolver signature `(ctx) → value`,
...
> `(input, ctx) => …` when this spec was approved; the input parameter is gone. The prompt is
> the one value a caller — or a model holding the block as a tool — controls, so a resolver
> that can see it can be written, in ordinary-looking code, to derive the working directory,
> the sandbox fence or the thread to continue from caller input (BP-031), and the sandbox case
> is the sharp one because a fence drawn from the caller's own text is not a fence. LAB-152
> owns the type; a resolver is handed the context and nothing else.
...
codexAgent(options):
    ...
    execute(input, ctx):
        ...
        directory     ← cwd resolver(ctx)                 ← decision 2
        continue-id   ← resume resolver(ctx)              ← decision 2, null = fresh
