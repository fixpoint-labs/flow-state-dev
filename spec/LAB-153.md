...
seq.step(
  codexAgent({
    ...
    onThread: (id, ctx) => managerState(ctx).rememberSession(id),
...
invocation before anything is spawned — decision 2), and the write side of `resume`: a
hook the host configures that the block calls once, with the thread id, the moment the
stream names it and before any turn work is consumed — the manager's slot feeds all three
from state the caller never touches (LAB-154). Forwarded verbatim, typed locally so
...
implementer's call with the code open.
