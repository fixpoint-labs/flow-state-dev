...
children and its capabilities' own top-level schemas for a `sessionStateSchema`.
`assertDetachedBoardSupported` returns early here — this board declares no detached
workers. Neither guard is relaxed, and `detached: true`'s schema-off behaviour is not
deleted.
...
| The previous attempt ran under a different harness | Not reachable here: one harness per manager, and the row's assignee is frozen — a board with a hand-off seat freezes its ledger's assignee exactly as a detached one does, because the assignee is the seat the row is dispatched by. The row records `source` beside the id (§7); the check — hand the id down only when it matches — is LAB-141's |
...
child-side claim gate; the detached runner is no longer on the conductor's path), a
scripted SDK `query` — plus a scripted *harness block* for the slot behaviours. PR a's
behaviours live in `packages/claude-code/test/sdk/agent.spec.ts` and the lab's
`manager.spec.ts`; PR b's in the package's own suite.
...
**Stale comments on `main`, to correct while implementing.** *Not review notes — found
refreshing this spec against the merged dispatch recut (PR #1555). Outside this spec's
design, inside the files it moves.*

- `labs/conductor/src/manager.ts:284` and `:1385` — both justify the sequencer-state choice
  and `detached: true` as things "a detached board refuses". The seat hands off; the refusal
  is the claim gate's, applied to the task entry at `defineFlow`. Correct them as the code
  they annotate moves into the package.
- `labs/conductor/src/flow.ts:2` and `:11` — the module header still says "a board, one
  detached worker" and "A detached board refuses three things at construction". The board
  holds a dispatcher seat; the construction refusals it satisfies are
  `assertHandOffBoardSupported`'s, which adds a fourth — a seat that hands off must be named.
- `labs/conductor/test/harness.ts:4` — the suite header names "the real task board and its
  detached runner" as the seam it drives, the same claim §10's Discipline paragraph carried.
