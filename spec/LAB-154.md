...
`cwd` and `resume`, each `(ctx) → value` reading the manager's own run state — and
one hook — `onSession`, `(id, ctx) → void`, writing to it — and the
harness package closes over all three. **LAB-152 declares those three types in core and
this spec imports them**; what is LAB-154's to pick is the *slot* that bundles them — the
factory option — not the callbacks' names or their signatures. Today's code is the other
shape (the harness's `cwd` callback reaches into the manager's sequencer state by cast),
...
> **Correction (2026-09-05, folded back from implementation).** The resolver signature was
> `(input, ctx) → value` when this spec was approved; the input parameter is gone. The prompt is
> the one value a caller — or a model holding the block as a tool — controls, so a resolver that
> can see it can be written, in ordinary-looking code, to derive the working directory, the
> sandbox fence or the session to resume from caller input (BP-031), and the sandbox case is the
> sharp one because a fence drawn from the caller's own text is not a fence. LAB-152 owns the
> type; a resolver is handed the context and nothing else. The slot and its three feeds are
> unchanged.
...
**A host builds a manager and hands it a harness** *(the manager's own option names are
illustrative — the implementer settles them; `cwd`, `resume` and `onSession` are LAB-152's
and fixed)*:
...
claudeCodeAgent({
  detached: true,
  resume: (ctx) => previousIdFor(ctx),           // → null or ""  ⇒ a fresh session, as today
...
called once at construction with `{ cwd, resume, onSession }` — the two resolver signatures
and the session-hook type are **LAB-152's, declared in core and imported here**, so the slot
composes the contract rather than restating it. The manager builds the two
...
per-task switch start fresh instead of resuming a foreign session. The values are
**LAB-152's to pin** — `claude-code/sdk` and `codex/sdk`, under its `<package>/<door>`
convention — and this spec records whichever the handle reports rather than deriving one of
its own; the field stays a widened string, so a handle persisted before that convention
still parses (BP-030). This spec records the
...
composes it into the step's abort signal and fires that signal when the deadline passes.
That is the manager's half, and all this spec promises. How promptly the step then returns
is the *harness's* half, and the two harnesses differ today: Codex races the signal and
throws the moment it fires (LAB-153), while `claudeCodeAgent` forwards the signal into the
SDK's abort controller and then awaits the SDK's stream, so it returns when the vendor
does. FIX-1301 (#1547) makes Claude Code race its own signal too — epic theme 7's rule that
a harness stops waiting on its *own* signal, not on the vendor's stream — and until it
merges this spec does not state that rule as fact for both harnesses (§12).
Neither harness bounds what it *ran*: a command the vendor's process spawned can outlive
the kill (LAB-153's POC, and the same in kind for Claude Code). The sandbox the host
configures on the harness is the fence for a runaway command. The option's doc and the
package README say exactly this much and promise no more.
...
in claude-code, where query options are assembled:
    if detached and resume resolver given: resume ← resolver(ctx) or nothing
...
inode check?* Whether the option is called `harness` or `runner` is not the question; the
hook is `onSession` because LAB-152 names it, not because this spec picked it.
...
8. **The deadline's doc** (§7) on the option, the README and the docs page — **check
   FIX-1301 (#1547) first** (§12): merged, the doc may say a harness stops waiting when the
   manager's signal fires; not merged, it says the manager fires the signal and how promptly
   a harness returns is the harness's, and a follow-up tightens it once FIX-1301 lands.
   **docs, READMEs, package map, changesets** per §11. *Depends on:* 5–7.
...
schemas, the block alias the slot types against and `decide` reads, and the two resolver
signatures and the session-hook type the slot hands down (LAB-152 declares them in core;
this spec imports them). **FIX-1291 (#1523) — satisfied**, done 2026-09-04: the manager
that moves is already the one using the framework's capability and state declarations, so
this spec relocates no casts and nothing here waits on it. LAB-153 is *not* a dependency:
Codex plugs into the slot in LAB-141. LAB-150 and LAB-141 depend on this.

**Soft dependency — FIX-1301 (#1547), on PR b's documentation step only.** Epic theme 7
binds both harnesses to the same rule: a harness block stops waiting when its *own* signal
fires, not when the vendor's stream closes. Codex delivers it (LAB-153, race-and-throw);
Claude Code does not today, and FIX-1301 is the fix — green, clean, awaiting merge. **The
code needs nothing from it: it does not block the package move, and PR b ships either
way.** Only the wording of step 8 depends on it. If FIX-1301 has not merged when PR b is
ready, ship the honest wording — *the manager fires the signal; how promptly a harness
returns is the harness's* — and correct it to the unconditional promise when FIX-1301
lands.
...
- **Refreshed against `main` (2026-09-05)** — PR #1513 renamed the task status
  ...
  mechanisms are unchanged.
- **Cross-spec alignment (epic LAB-140, 2026-09-05)** — three edits from the coherence
  pass over the epic's approved specs, none touching direction. FIX-1291 is done, so §12
  records it satisfied rather than blocking. The deadline is no longer stated as a promise
  both harnesses keep: the manager fires the signal, Codex races it, Claude Code returns
  when the vendor's stream closes until FIX-1301 (#1547) lands — which §12 names a soft
  dependency of PR b's *documentation* step only, never of the move. And the three feeds
  the slot hands down are LAB-152's declarations imported here, not this spec's to name or
  co-own, with `source`'s values (`claude-code/sdk`, `codex/sdk`) pinned there too. The
  `onSession` wording, decisions 1–3, the PR split, and the resume and lease mechanisms are
  unchanged.
