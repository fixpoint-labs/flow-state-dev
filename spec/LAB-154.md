...
| The previous attempt ran under a different harness | Not reachable here: one harness per manager and the row's assignee is frozen on a detached board. The row records `source` beside the id (§7); the check — hand the id down only when it matches — is LAB-141's |
