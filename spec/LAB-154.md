...
hands any block a capability, and the manager hands the harness two things per run — where
to work, and which conversation to continue — through one typed channel the harness's own
resolvers read. The manager reads a run's result off the neutral contract's fields, so a
...
the checkout path, Claude Code honours it on the background path (and refuses one
in-session, where session state already owns continuity), and a real run proves that an
answered question continues the conversation that asked it.
...
2. **An answered run continues its session, and that is proved on the real path, not
   ...
   session id down the same channel as the checkout path; Claude Code honours the id only
   on the background path and throws at construction if given one in-session.
   ...
   *Locks in:* "answered" means continued. A harness that cannot resume by id cannot be
   this manager's harness — the contract's resume clause becomes load-bearing rather than
   advisory. The typed resume association *on the task* stays FIX-1179's; this spec reads
...
retention for run records or inbox rows; any change to the task board's detached rules
(`assertDetachedBoardSupported` is not relaxed and `detached: true`'s schema-off behaviour
is not deleted).

**Size:** Large — 21 production files · 13 test behaviours + 1 goal check · 7 doc surfaces · 2 PRs.
...
the harness package closes over them. Today's code is the other shape (the harness's `cwd`
...
The factory keeps the contract explicit and typed, and both values arrive in one call — the
...
1. **BP-031 (never decide from caller input)** — the checkout path and the session id
   reach the harness only through the manager's resolvers, which read server-derived state.
...
const manager = harnessManager({
  ...
  harness: ({ cwd, resume }) =>
    claudeCodeAgent({ cwd, resume, detached: true, recordWork: true, model: "…" }),
});
```
...
```ts
  harness: ({ cwd, resume }) =>
    codexAgent({ cwd, resume, thread: { sandboxMode: "workspace-write" } }),
```
...
```ts
claudeCodeAgent({ detached: true, resume: (_input, ctx) => previousIdFor(ctx) });
// resolver → null or ""  ⇒ a fresh session, as today
...
flowchart BT
  ...
  ORCH["@flow-state-dev/orchestration<br/>task board · detached runner · task substrate"]
  ...
  CC["@flow-state-dev/claude-code<br/>claudeCodeAgent gains a resume resolver<br/>(background path only); costUsd retired"]
  ...
  LAB --> HM
  LAB --> CC
  LAB -. "harness: ({cwd, resume}) => claudeCodeAgent(…)" .-> HM
```
...
from the package; `implementPhase()` is the lab's implementation of it.
...
called once at construction with `{ cwd, resume }`; the manager builds both resolvers over
its own sequencer state, so the harness never learns the state's shape. The step that
...
write inside the factory, where it belongs.

**Resume, both sides of the slot.** *Manager side:* `previousSessionId` is already
captured before the opening write clears it and carried on the manager's state; the
`resume` resolver returns it. *Harness side:* `claudeCodeAgent` gains a `resume` resolver
option. On the background path (`detached: true`) the resolved id — and only that; the
session provider stays unconsulted — becomes the SDK's `resume`. In-session (`detached:
false`) the option is refused at construction: session state owns continuity there, and
an explicit id would be a second owner. `null` or `""` means a fresh session. The Codex
block already takes the same pair (LAB-153, decision 2).

**The verdict reads the contract.** `decide`'s input becomes the neutral handle's fields:
`status`, `sessionId`, `outcome`, `finalMessage`, `usage`, `cost`. Success is `status:
"completed"` as today; the failure reason names `outcome` (and `finalMessage` when there is
one) instead of the SDK subtype. `cost.usd` lands on the run row where `costUsd` did; the
row may also record `cost.basis` so a report can say *estimated*. `costUsd` comes off the
...
serialisation, which the inode did not. The settle hook stays synchronous — the reads and
the unlink are sync — and both exits (`onSettled` on the harness step, the rescue) call
the one release. No module-level state remains, so two managers in one process cannot
touch each other's lock by construction rather than by keying.
...
harnessManager(options):
    ...
    sequencer over the manager's state
      .tap(open the row; capture previousSessionId before the clear)
      .step(prepare: build the prompt; acquire the checkout → { lockPath, token } onto state)
      .step(harness, { abortSignal: deadline, onSettled: ctx → releaseFromState(ctx) })
      .step(decide: read { status, outcome, cost, … }; ask / done / fail, as today)
      .rescue(recordFailure: releaseFromState(ctx); write the row; rethrow)
...
in claude-code, where query options are assembled:
    ...
    if not detached and resume resolver given: throw at construction
```

What this asks a reviewer: *is a factory that hands the harness two resolvers the right
shape for "continue this run in this checkout" to arrive as one instruction* — and *is a
token in the lock file an honest replacement for the inode check?* Whether the option is
called `harness` or `runner` is not the question.
...
the SDK resumes by id from a `resume` option (the in-session path); the lock file already
...
1. **`claudeCodeAgent` honours a `resume` resolver on the background path** and refuses
   one in-session (decision 2). Forward it into the capability's options like every other
   agent option. *Test:* the scripted-SDK behaviours in §10 for resume. *Depends on:*
   nothing.
2. **The manager's verdict reads the neutral fields** — `decide`'s input schema, the
   failure reason, the run-row write — and the lab's manager passes `resume` beside `cwd`
   so an answered run continues (decision 2, manager half). Retire `costUsd` from the
   Claude handle and its schema. *Test:* the verdict behaviours in §10; the existing
   conductor suite green; a repo-wide typecheck shows no reader of `costUsd`. *Depends on:*
   1, and LAB-152's implementation on `main` (the neutral schemas).
...
5. **The slot** (decision 1): the `harness` factory option typed against core's alias;
   the step reads its result; the lab's `conductorFlow` writes the factory. *Test:* §10's
   slot behaviours, including a fake harness with no Claude Code import. *Depends on:* 4.
...
|---|---|---|
| a | Claude Code's background-path `resume`; the manager's verdict on the neutral fields; `costUsd` retired; the goal check | — |
| b | `@flow-state-dev/harness-manager` with the slot, the lease as a value, `readable` removed; `labs/conductor` as consumer; docs | a |
...
| The previous attempt threw before any session existed | The run row's `sessionId` is `null` after the opening clear, so the next attempt starts fresh — a failed resume does not chase a dead id (see next row) |
| The id exists but the vendor cannot resume it (session gone, another machine) | The harness throws; the attempt fails with the vendor's reason; the opening write of the next attempt clears the row's `sessionId`, so that attempt starts fresh. Self-healing on retry; documented |
| The previous attempt ran under a different harness | Not reachable here: one harness per manager and the row's assignee is frozen on a detached board. LAB-141 owns the per-task case; §12 flags recording `source` beside the id for it |
...
| The harness step is cancelled by the deadline | Throws into the rescue as today; the row re-pends with the reason; a vendor subprocess may outlive it — the sandbox is the fence, and the doc says so |
| The harness block's resolvers run outside a manager | The `cwd` resolver throws (no checkout on state), as `managerState` does today |
...
*Anti-game:* the second prompt never contains the fact; nothing reads the vendor's session
store or transcript; a fresh-session pass would have no way to know the fact.
...
- `claudeCodeAgent({ resume })` without `detached` throws at construction
- the capability forwards `resume` like every other option
- the manager's `resume` resolver returns `null` on attempt 1 and the previous attempt's
  session id on attempt 2, and both resolvers are handed to the factory in one call
...
`createFlowState`, the real board and detached runner, a scripted SDK `query` — plus a
scripted *harness block* for the slot behaviours. PR a's behaviours live in
`packages/claude-code/test/sdk/agent.spec.ts` and the lab's `manager.spec.ts`; PR b's in
...
- **EXTEND** `apps/docs/docs/tools/claude-code-sdk.md` → "Turning it off for background
  ...
  resolver, that `null` means fresh, that in-session it is refused. Drop `costUsd` from
  the handle description LAB-152 extended.
...
- **EXTEND** `labs/conductor/README.md` — it is now the consumer: what lives here, what
  is imported, the two lines that pick the harness.
...
- When a harness is chosen per task, the previous attempt's session id can belong to a
  different harness. Record the handle's `source` beside `sessionId` on the run row and
  hand the id down only when it matches the harness now dispatched; otherwise start fresh.
  Cheap to add here as a recorded field; the *check* is LAB-141's, because only it knows
  which harness a row is bound to.
