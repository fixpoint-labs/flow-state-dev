...
|---|---|---|---|
| ER-1 | Path level is scope: `org/<slot>/` is shared across teams, `teams/<teamId>/<slot>/` belongs to one, and both carry the same slots | FIX-1356 defines · 1352, 1354 consume | Each reader's tests · every child's spec review |
...
| ER-16 | Every child's route reads *spec* by default; only a `Bug` label re-routes it, and an unlabelled issue's route stays unset rather than guessed | Fail-closed routing. FIX-1358 carries no Kind label; its blank spec-PR cell records an unanswered question, not a `direct` route |
...
| ER-22 | The docs teach the tree — `org/` and `teams/<teamId>/` with the same slots — as something an author writes, not something the framework has | FIX-1358's atlas teach and the epic's docs pass |
