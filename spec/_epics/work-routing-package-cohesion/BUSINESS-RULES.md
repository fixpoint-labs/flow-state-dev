...
| ER-23 | **A shared contract's spec is approved before the next child specs against it.** FIX-1405's reader contract closes before FIX-817 starts; the `tools:` fence settles across FIX-1416 → FIX-1394 → FIX-1415 in that order | Two issues designing one discovery API at the same time is how you ship two of them. Orders the spec work without re-parenting — and it **outlives FIX-817 leaving the set** ([D6](DECISIONS.md#d6)): the dependency is on the contract, not on the parent |
