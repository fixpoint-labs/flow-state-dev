...
| ER-2 | **A seat's package format is whatever [FIX-1394](https://github.com/fixpoint-labs/flow-state-dev/pull/1915)'s ratify records**, and no child may pre-name it. *If the matrix collapses:* one format, two attachment modes — seat **always-on**, library **opt-in** — and a tool, a skill and an instruction have one home between them. *If it does not:* **don't collapse** is the result with the same force, not a failed matrix (FIX-1394's BR-16), and today's two surfaces stand. Either way no child invents a **third** shape. *After POC ratify* is the rule, not a hedge | FIX-1394, at the ratify | FIX-1394's POC matrix · the proof, which may not invent a third shape |
...
| ER-16 | A child's Linear state is mirrored the moment it changes. **Not holding, as of 2026-09-19:** `api.linear.app` is unreachable from every session that has touched this epic (403 at the egress proxy), so roughly **eight status writes are queued and unwritten** and no child's Linear state has moved. Until it is reachable the **epic branch is the durable copy** — including for the four approved specs, whose closed spec PRs would otherwise have handed that role to Linear — and the [set table](SPEC.md#the-set--as-of-2026-09-19) is the current picture. Treat the tracker as stale, not as a second opinion | The epic wake derives blocked-by from Linear, so stale state silently mis-orders the set. That is the live exposure. The wrap term used to be a second one — it required every child to be *Linear*-terminal, which nothing could satisfy while the writes are queued — and has been [re-derived against GitHub](PLAN.md#terminal) so the epic cannot be held open by an unreachable tracker. The writes are still owed |
