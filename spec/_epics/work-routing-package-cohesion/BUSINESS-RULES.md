...
| ER-3 | Inventory is **two layers**: a **declared roster** composed at read time from the existing readers, and the **live org resource** ChannelFlow updates as seats and channels open. Membership, author check, fan-out and DM find-or-create are answered by the live layer; the declared layer answers what the tree says exists | FIX-1405 | FIX-1405's two prove-pressure callers · FIX-817's and FIX-1415's spec review |
...
before it. ER-3's declared layer does not substitute for the live one, its public export is on the
owner's objective gate, and FIX-1405 implementation waits for that gate and its own spec. ER-4 is
...
| ER-12 | No second `WorkerRegistry`, mega-loader, mega-bus or parallel index of truth; no Graft rebuild for membership or DM lookup. **Compose is not a registry** — ER-3's declared layer derives per read, registers nothing, and may not stand in for the live resource | D5. One derivation over the readers that exist; one live resource for what is open |
...
| # | Rule | Because |
|---|---|---|
| ER-15 | A cross-cutting question is commented **up** on the epic PR, never decided locally — including FIX-1408's unclosed walls, where **FIX-1394's POC owns the evidence and this epic owns the decision** | [DECISIONS.md](DECISIONS.md) is the single place. A local answer is a second authority |
...
| ER-17 | Every child routes to *spec* by default; only a `Bug` label re-routes | Fail-closed. All seven carry `Design` + `Feature`, so all seven spec |
| ER-18 | A child depending on FIX-1377 or FIX-1416 writes against their **approved specs** and names them; neither has landed | FIX-1377 puts the team layer on `WorkerConfig` as its own key, composed framework-default → team → seat. FIX-1416 makes the blocks scan the tool catalog behind the `tools:` fence, a worker-colocated folder joining the seat's declared list |
...
| ER-23 | **A shared contract's spec is approved before the next child specs against it.** FIX-1405's reader contract closes before FIX-817 starts; the `tools:` fence settles across FIX-1416 → FIX-1394 → FIX-1415 in that order | Two children designing one discovery API at the same time is how a set ships two of them. Orders the spec work without re-parenting |

## The proof

| # | The epic's first cut is done when | Proved by |
|---|---|---|
| ER-20 | Work filed on a **channel or org board** wakes a seat that runs it, and a coordinator seat assigns work to **team** seats — on the real path, seats and channels declared in files | **Unowned today.** FIX-1430 is the only candidate; adopting it is part of the set question ([Open](DECISIONS.md#open)) |
...
**ER-20 is the rule to read twice.** Until the gate ratifies FIX-1430 into the role, nothing in the
set runs the epic's own exit gate. Wrapping on ER-1, ER-2 and ER-3 shipping would be claiming
