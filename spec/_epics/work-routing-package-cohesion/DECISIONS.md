...
| <a name="authorship-correction"></a>**A package format, if one is built, may not be designed disk-only** — and that is *all* this row ever carried. **Corrected 2026-09-19:** it previously read as the owner also ratifying *authored in Markdown, as the new file* and retiring *don't collapse*. **He did not; that fork is still his** | The owner's words were a **constraint**, not a choice of format: packages an LLM writes are **stored as a resource, never saved to disk**, so a format readable only off the filesystem is wrong on arrival. The epic read that as answering the authorship fork too. Two independent records say otherwise — [#1921](https://github.com/fixpoint-labs/flow-state-dev/pull/1921)'s live body (*"one decision left"*, and it asks this one), and the **FSD Architect** on that PR: *"Authorship fork (C vs don't-collapse) — correctly left open. Architect does not close it"*, with a soft fence of **no ship ticket and no sibling inventing `PACKAGE.md`** until it lands. Same defect class as [the seventh child](#d6-seven): a remembered decision contradicted by the record | [ER-2](BUSINESS-RULES.md) · [Open](#open) |
...
| <a name="documents-answer"></a>**Documents are org-scoped for now** — a seat's document is simply a resource under that seat on the org, and there is no per-seat document slot in v1 | The owner, 2026-09-19, answering FIX-1394's second ratify fork **in his own words rather than by taking the recommendation**. Per-resource configurability is not refused, it is *deferred to a mechanism that does not exist yet*: **resource templates** — "used to define the resource, but isn't the resource itself". Until those exist there is nothing to configure against, which is why v1 gets the org scope and no slot. **The exclusion is structural, not provisional:** a package attaches to **one seat** and a document installs at **org** scope, so a format declaring one would be a seat-level unit minting an org-level resource — true whichever format wins, so it is not a tiebreak. **It closes this fork only; the ratify still waits on [authorship](#authorship-correction)** | [ER-2](BUSINESS-RULES.md) · FIX-1394's ratify |
...
**One of FIX-1394's two ratify forks is closed** — [documents](#documents-answer), on 2026-09-19.
**The other is open and it is the owner's:** *who writes a package — a team in Markdown, or an
engineer in TypeScript?* **Plain terms:** the working part already exists and an engineer writes it
in TypeScript today; the format would let a non-engineer write the same capability as a Markdown
file. Identical capability either way — what changes is who can produce one without an engineer.
**Trade-off:** building it costs a new file convention and a reader; not building it leaves
hand-over a conversation with an engineer. **The recommendation on
[#1921](https://github.com/fixpoint-labs/flow-state-dev/pull/1921) is build it, as the new file,
scoped to instructions and tools** — the machinery already ships, so this is the cheapest version
of it we will get. **What would change it:** whether anyone outside the app's engineers will
actually write one; if not, *don't collapse* is the honest answer. **Cost of being wrong:**
asymmetric — not building keeps the option, building ships an eighth convention into a tree that
has seven, and a published format is expensive to withdraw. **Until it lands, ER-8 holds the ship
tickets and no sibling may invent a package shape.** The set
