...
bar, and every child of the set now has a PR. The lane the tracker holds and this figure does not is
[FIX-1451](SPEC.md#the-sixth-child) — deliberately, until it is confirmed a child or re-homed.

**The critical path ran boards → the proof, and both halves are behind us.** FIX-1385 merged on
2026-09-19 and FIX-1430's build followed it onto
[#1929](https://github.com/fixpoint-labs/flow-state-dev/pull/1929). What is in front of the epic now
is not a build: it is [ER-20's own run](SPEC.md#er-20-has-not-run), which needs a model credential
this epic has never had. The inventory and package cohesion are *beside* the critical path, not
behind it.
...
| **FIX-1405** runtime inventory | spec → POC → impl → **in review** | The three existing readers · hire · `channelInstances` · open sessions (D5) | Two layers: a declared roster composed at read time (replacing both labs' private `LabRoster`), and the live org resource | FIX-817, on its approved reader contract (ER-23) · FIX-1415 · `team.*` wildcards, later. **Not FIX-1385** | Medium · PR-A and S4 merged, S5–S7 open on [#1928](https://github.com/fixpoint-labs/flow-state-dev/pull/1928) |
...
| **FIX-1430** manager-queue lab · *the proof* | spec → impl → **in review**, goal check **NOT RUN** | ER-1 · **ER-2 as a fence, not an input** · ER-4 · ER-5 — consumes four, owns none | A coordinator seat assigning over a channel board to linked seats, queue columns as views. Plus the drain-width comparison ER-15 called for ([width 1](DECISIONS.md#drain-width)) | The epic's wrap, **once ER-20 runs** | Medium · [#1929](https://github.com/fixpoint-labs/flow-state-dev/pull/1929) |
...
4. **FIX-1385's implementation landed** ([#1922](https://github.com/fixpoint-labs/flow-state-dev/pull/1922))
   → the proof's build ran. That was the set's only true blocking edge, and it is discharged.
5. **ER-20 runs on the real path** → the epic can wrap. This is now the only thing in front of it,
   and it is blocked on an environment, not on work: [the gate has not run](SPEC.md#er-20-has-not-run).
6. **W3's last implementing child merges** → the ship fence lifts (ER-14). That is **FIX-1435**,
   still `Todo`. Three W4 ship PRs merged under the fence and **#1928 is parked by it** — whether
   the fence narrows is [a re-gate with the owner](DECISIONS.md#er-14-re-gate), not an epic call.
...
| One channel flow factory, two PRs | FIX-1385 (**merged**) and FIX-1405's **S5–S7 ([#1928](https://github.com/fixpoint-labs/flow-state-dev/pull/1928))** | Both edit `defineChannelFlow` and the binder. **Discharged in the order it named:** FIX-1385 shipped whole first, so #1928 is built on top of it rather than racing it, and its own deviations note where the landed code won. Board *declaration* stays built-in-kind-only; channel *inventory* is every kind's |
...
merge is what makes code shipped. **Two children are terminal: FIX-1408 by decision, FIX-1385 by
merge.** FIX-1405 and FIX-1430 have open PRs; FIX-1394's ratify is half answered.

**ER-20 has not run, so the wrap has not opened.** Merging #1929 makes FIX-1430 terminal and still
leaves the gate unrun — [the row reads `NOT RUN`](SPEC.md#er-20-has-not-run) and the term is
enforced against the rule as written. The last step before the wrap is somebody running that check
with a model credential and the verdict log saying PASS.
