...
| Has Devtool's row 6, a sealed `references/` document marked read-only, been read live? | **No, and no surface may say it has.** It is proved by automated checks (FIX-1481's V2, V3 and V5, and FIX-1547's snapshot test), because no hired tree in the repo declares a sealed document and inventing one is the special wrapper ER-Devtool refuses. **The live read binds FIX-1455**: the first real hire declaring a `references/` document or an `ro` grant reads row 6 live | W5 D11, the owner's call Sep 24, noted on FIX-1455 the same day |
