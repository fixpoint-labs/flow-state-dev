...
| What fences a door into a hired seat? | **The owner pin `{ orgId, userId? }` copied from the hire row, never the seat's address.** Org is compared first, and a miss looks like a seat that does not exist. Every door (list, open, run, resume, drain, debug listing, stored data) consults that one pin at a framework seam, proved on the real path by who is asking. **The fence is only as good as the principal the host resolves**, which FIX-1503 makes verified by default | FIX-1528 ER-1 and D3; built by [#2091](https://github.com/fixpoint-labs/flow-state-dev/pull/2091), [#2106](https://github.com/fixpoint-labs/flow-state-dev/pull/2106), [#2108](https://github.com/fixpoint-labs/flow-state-dev/pull/2108) and [#2127](https://github.com/fixpoint-labs/flow-state-dev/pull/2127) |
| Where does a hired seat keep a person's data, and how does data already stored get there? | **One cell per (org, person)**, shared by that person's seats in that org. A seat reads **none** of the person's app-wide data, and the app's flows do not read the seat's. Existing data moves **only by the operator's documented copy step**, and only resources that provably belonged to a seat. No read falls back to the old cell, and the framework never moves or deletes it. The person's own cross-org data outside hired seats is **not decided** and is not re-keyed | FIX-1538 D1 and D2 ([#2121](https://github.com/fixpoint-labs/flow-state-dev/pull/2121), built by [#2127](https://github.com/fixpoint-labs/flow-state-dev/pull/2127)); FIX-1528 ER-4, and ER-5 as narrowed by [#2126](https://github.com/fixpoint-labs/flow-state-dev/pull/2126) |
| Does any epic here teach or expose user planes? | **No, not before FIX-1486** gives flow and session listings org identity. A user's own boards, seat inventory and listings stay untaught. This binds FIX-1455's navigator, which ships tenant-scoped (its D8). **FIX-1486 is org *selection*, and a one-user, one-org deployment does not need it**: the org's own inventory is not a user plane, and W5 shipped a view of it without FIX-1486 (next row) | The Architect on [#2070](https://github.com/fixpoint-labs/flow-state-dev/pull/2070), decision 2; FIX-1528 D4. The FIX-1486 scope: W5 D10, which moved FIX-1502 from *blocked by* to *related* |
| Can a session see its whole organization's seats and channels? | **Yes, through the ordinary collection route.** The three shared inventory collections declare a browser read, once, so any session on a flow that installs them lists the org's registered seats, channels and memberships. The org comes from the session, never the request; no org is selected or named; the debug endpoints stay off. **Costs:** when a second person joins an org, *every member sees the org's layout* is the default, and narrowing it then breaks every reader built on it. An epic that shows an org's seats or channels reads it here, not through a second declaration or the debug panel | W5 D10 and FIX-1502 D1, an [ER-17](https://github.com/fixpoint-labs/flow-state-dev/blob/main/specs/epics/FIX-1457/BUSINESS-RULES.md#er-17) contract change the owner signed; built by [#2147](https://github.com/fixpoint-labs/flow-state-dev/pull/2147). Production-route evidence: W5's settlement of Sep 22, two orgs and a refused control |
| Does an inventory row mean a seat is working now? | **No — it means *registered in this org*.** Rows are never deleted, so a fired seat stays listed until FIX-1540 lands, and a membership shows who was in a channel when it registered. **No reader filters rows into its own idea of *live***: that would make a view a second registry. What *live* means is FIX-1485's to decide, and a reader picks up the change with no edit of its own | FIX-1502 D2 |
| Has Devtool's row 6, a sealed `references/` document marked read-only, been read live? | **No, and no surface may say it has.** It is proved by automated checks (FIX-1481's V2, V3 and V5, and FIX-1547's snapshot test), because no hired tree in the repo declares a sealed document and inventing one is the special wrapper ER-Devtool refuses. **The first real hire declaring a `references/` document or an `ro` grant reads row 6 live.** The kitchen-sink rebuild wrapped without one (no seat under `apps/kitchen-sink/workforce/` on `main` declares `references/`), so the debt now rides **FIX-1469**, in Backlog under the wrapped epic | W5 D11, the owner's call Sep 24, noted on FIX-1455 the same day; carried past FIX-1455's wrap, Sep 24 |
| Which organization does kitchen-sink run under, and what happens to a store written before? | **One named organization, `kitchen-sink`**, set by host code and never read from a request. Admin tokens are `kitchen-sink:<token>`; a token naming any other org is dropped with a logged refusal. A store written before the change is **refused at boot**, naming the channels it found under another org. **There is no upgrade path**: a developer wipes `.fsdev` (or points Postgres at an empty database). An epic whose demo, goal or proof runs in kitchen-sink runs in this org and writes no migration for older stores | The owner's call, Sep 24: FIX-1455 D9, FIX-1500 D6, and FIX-1500 H1 (*"just wipe everything and start from fresh"*). Built by [#2159](https://github.com/fixpoint-labs/flow-state-dev/pull/2159); on `main` in `lib/kitchen-sink-principal.ts`, `lib/workforce-admin-auth.ts` and the boot guard in `fsdev.config.ts` |
| Who is a terminal run? | **Whoever the app's own resolver says**, asked through `resolveInProcessPrincipal` with `source: "cli"` and no request, exactly as an HTTP action is asked. A resolver that refuses stops the run before anything is written. `--org` and `--user` are **local to `fsdev run` and `fsdev chat`**: `--org` skips the resolver, `--user` alone keeps the app's org; `fsdev serve` and `fsdev dev` take neither. A goal or Lab that runs a flow from the terminal gets the app's identity, not a CLI-invented one | FIX-1551 ([#2170](https://github.com/fixpoint-labs/flow-state-dev/pull/2170), spec [#2158](https://github.com/fixpoint-labs/flow-state-dev/pull/2158)); `packages/cli/src/principal.ts` and the `fsdev` README on `main` |
| When does a hired seat get kitchen-sink's admin authentication? | **Only when its owner pin names the admin user** (`workforce-admin`), which is how the admin action pins what it hires. A seat hired in-app by a seat's `hire` tool is pinned to the org alone and keeps the ordinary resolver, so any member can open it. The decision is read from the server-set pin, never a request. **The pin stays the fence** (PR-7): this only decides who the caller is | FIX-1563 ([#2169](https://github.com/fixpoint-labs/flow-state-dev/pull/2169); tests re-keyed to the named org on [#2191](https://github.com/fixpoint-labs/flow-state-dev/pull/2191)); `withAdminAuthentication` on `main` |
| Where does content for an open leaf go in `FlowNavigator`? | **Two slots, one meaning each.** `leafToolbar` is actions on the leaf's own row, revealed on hover or focus, so it holds icon buttons. `leafDetail` is **the one content slot**, on its own lines under an open leaf's row. **No new slot without the owner**: FIX-1561 shipped *no new slot* and `leafDetail` is its one owner-approved exception. Kitchen-sink's seat details and "Hire another" stay **in the rail**, in `leafDetail`, not in a popover or side panel. The navigator is published on `@flow-state-dev/react` and Devtool wraps the same component, so this binds any epic that draws in it | FIX-1561 D1 and its exception, owner-approved Sep 24; FIX-1500 E4, the owner's choice of B. Built by [#2203](https://github.com/fixpoint-labs/flow-state-dev/pull/2203), with PR-D [#2193](https://github.com/fixpoint-labs/flow-state-dev/pull/2193) and spec amendment [#2215](https://github.com/fixpoint-labs/flow-state-dev/pull/2215) |
