...
**Sep 8** onward, and 90 issues still have no parent. **W4 and W3 wrapped
three minutes apart on Sep 20.** Plane isolation is the shortest closed bar: its first children
shipped on Sep 22, it passed its gate on Sep 23 and wrapped on Sep 24, 8 of 8. **Two more closed
the same day**: W5 at 21:05 UTC, when Linear marked it Done with its late seventh child, and **the
kitchen-sink rebuild at 23:40 UTC**, when its last child merged, four days after it became its own
epic. No bar crosses the now line. Eight lanes: seven closed, and one that never opened.
...
|---|---|---|---|
| [FIX-1562](https://linear.app/fixpoint-labs/issue/FIX-1562) | W5 | The multi-seat-collab goal's V7 leg is FIX-1497's merge-time scope check, still live. It fails on **every** branch that touches `packages/` whenever that goal runs | In Development |
...
| [FIX-1546](https://linear.app/fixpoint-labs/issue/FIX-1546) | plane isolation | Schedule index identity has no storage cell, so two orgs' schedules can collapse | Done |
| [FIX-1543](https://linear.app/fixpoint-labs/issue/FIX-1543) | plane isolation | Two copies of the owner-pin helper can drift apart | Done |
...
**One child outlived its epic.** [FIX-1469](https://linear.app/fixpoint-labs/issue/FIX-1469), the
deferred S7 of FIX-1467, is still in Backlog under the wrapped kitchen-sink rebuild, and it carries
Devtool's row 6 live read. Nothing is driving it until an epic picks it up.
...
- **Re-parenting the 90 unparented issues.** They are the pre-epic era. Sweeping them under epics
