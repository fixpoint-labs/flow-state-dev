...
[FIX-1497](https://linear.app/fixpoint-labs/issue/FIX-1497). **As of 2026-09-24 two have passed** —
ER-DevForce and ER-Collab — and **ER-Devtool has not**: row 4 passed live, row 5 is in spec, and
row 6 is proved by automated checks without a live read. The state of each is flagged in the rule
...
| <a name="er-26"></a>ER-26 | **MET, 2026-09-22.** [FIX-1496](https://linear.app/fixpoint-labs/issue/FIX-1496)'s [#2051](https://github.com/fixpoint-labs/flow-state-dev/pull/2051) stood the live hire up, and the runs this rule fenced ran after it: ER-Collab's graded run ([#2065](https://github.com/fixpoint-labs/flow-state-dev/pull/2065)) and checklist row 4 ([#2066](https://github.com/fixpoint-labs/flow-state-dev/pull/2066)). The clause, as written: **no exit proof's *run* is graded before a live hired Workforce exists, and the path that stands one up is FIX-1496.** [ER-3](#what-every-exit-proof-must-satisfy) and [ER-Devtool](#er-devtool) both forbid a fixture, a wrapper or code written only to make an inspection possible, so the checklist has nothing legitimate to read until one is standing — and **row 6 is the sharp case: no tree in the repository has a sealed document**, so 1481's row-6 code can be entirely correct and the row still cannot go green live. **That stays true after this rule was met**, and [D11](DECISIONS.md#d11) carries it. **This fences the run, not the code.** FIX-1481's and FIX-1502's implementation PRs are **not** blocked by it and must not be sequenced behind it | The ER-Devtool producers consume it; [FIX-1496](https://linear.app/fixpoint-labs/issue/FIX-1496) discharged it | The checklist run itself · [ER-20](#er-20)'s wrap sweep |
...
|---|---|---|
| <a name="er-devtool"></a>**ER-Devtool** | **The documented Devtool checklist below is green on a live hired Workforce** ([ER-3](#what-every-exit-proof-must-satisfy)), **with no special wrapper** — no bespoke debug app, no fixture panel, no code written only to make the inspection possible. **Row 6 counts on its automated checks** until a real hire declares a sealed document ([D11](DECISIONS.md#d11), 2026-09-24) | **Two producers, not one, since 2026-09-22.** [FIX-1481](https://linear.app/fixpoint-labs/issue/FIX-1481) (**Done**) owns **rows 4 and 6**; [FIX-1502](https://linear.app/fixpoint-labs/issue/FIX-1502) (*in spec*) owns **row 5**, the org-level inventory view, **built in W5** ([D10](DECISIONS.md#d10)) and no longer blocked by [FIX-1486](https://linear.app/fixpoint-labs/issue/FIX-1486); rows 1–3 ride [FIX-1320](https://linear.app/fixpoint-labs/issue/FIX-1320), which this epic does not run. **1481 still verifies the whole checklist**; 1502 builds the consumer 1481 deferred. **Status, 2026-09-24:** **row 4 PASS, live** — graded on a live hire through the shipped `fsdev dev` and DevTool ([#2066](https://github.com/fixpoint-labs/flow-state-dev/pull/2066), verdict in `goals/devtool-workforce-visibility/the-checklist-rows/goal.md`). **Row 5 in spec** (FIX-1502). **Row 6 proved by FIX-1481's automated checks V2, V3 and V5** ([#2039](https://github.com/fixpoint-labs/flow-state-dev/pull/2039)) **and not read live** ([D11](DECISIONS.md#d11)): no hired tree in the repository declares a sealed document, and giving one a subject needs kind code whose only job is the inspection, which this rule forbids. The live read moves to the first real hire with a `references/` document or an `ro` grant (noted on [FIX-1455](https://linear.app/fixpoint-labs/issue/FIX-1455)); the mitigation is [FIX-1547](https://linear.app/fixpoint-labs/issue/FIX-1547), a test that a hired team's reference reaches the debug snapshot as `writable: false`. **Row 6 has not gone green live.** Rows 1–3 wait on FIX-1320's verification. **A filed ticket is not a green row** |
| <a name="er-devforce"></a>**ER-DevForce** | **One DevForce path completes and produces a real artifact** — code, a PR, or a work product that exists outside the run — **not a mock and not a transcript**, with seats and channels used honestly rather than stubbed past | **PASS, 2026-09-22** — [FIX-1496](https://linear.app/fixpoint-labs/issue/FIX-1496) **Done**, [#2051](https://github.com/fixpoint-labs/flow-state-dev/pull/2051) merged, verdict row in `goals/devforce-lab/it-ships-an-artifact-a-person-can-open/goal.md`, naming the local bare-repository leg. **1496's BR-10 and BR-17 were re-proved by [FIX-1515](https://linear.app/fixpoint-labs/issue/FIX-1515)** ([#2073](https://github.com/fixpoint-labs/flow-state-dev/pull/2073)): the model-free sibling gate `it-wakes-the-seat-a-file-declared`, red on the org-less read after FIX-1442, is PASS again, and was re-run on `origin/main` at `95049473f` on 2026-09-24 and passed. **[FIX-1496](https://linear.app/fixpoint-labs/issue/FIX-1496)**, spec [#2023](https://github.com/fixpoint-labs/flow-state-dev/pull/2023) at head `25e7507`. A four-gap delta on the existing `goals/devforce-lab/`, not a new Lab tree. **Which artifact counts — the EM's cut, taken.** *Real* is three falsifiable properties, and the proof runs **one automated leg**: the temp-directory repository gains a **bare clone at a declared path the run pushes to**, so the artifact resolves at its address after the goal's process has exited, needing `git` and a temporary directory but **no `gh`, no token and no network** (its `PLAN.md` S3, `BR-1`, `BR-15`). The **credentialed pull-request leg is documented as the human release run** and is never inferred from whether `gh` happens to be installed (`BR-14`) — one leg in CI, and the verdict names which. **Channels are in, not optional:** 1496 kept channel driving rather than deferring it, because this rule's ratified wording says *with seats and channels used honestly rather than stubbed past*, which puts channels inside the criterion (its *Decided, not asked* → [the channel](https://github.com/fixpoint-labs/flow-state-dev/blob/spec/FIX-1496/specs/issues/FIX-1496/DECISIONS.md#channel)). **The cut was approved at 1496's own direction gate** ([#2023](https://github.com/fixpoint-labs/flow-state-dev/pull/2023) merged) and is the leg the PASS ran; this row records it, it is not a separate epic decision |
| <a name="er-collab"></a>**ER-Collab** | **≥2 seats across ≥1 channel**: work **filed**, **assigned**, **drained**, and a **cross-seat handoff or reply observed in Devtool**. One graded scenario, run on the real path | **PASS, 2026-09-22** — [FIX-1497](https://linear.app/fixpoint-labs/issue/FIX-1497) **Done**, [#2065](https://github.com/fixpoint-labs/flow-state-dev/pull/2065) merged, verdict row in `goals/multi-seat-collab/it-hands-a-row-between-two-seats-in-view/goal.md`, the handoff read in Chromium on the shipped DevTool. Composes boards ([FIX-1385](https://linear.app/fixpoint-labs/issue/FIX-1385)), inventory ([FIX-1405](https://linear.app/fixpoint-labs/issue/FIX-1405)), manager-queue ([FIX-1430](https://linear.app/fixpoint-labs/issue/FIX-1430)), dispatch honesty ([FIX-1440](https://linear.app/fixpoint-labs/issue/FIX-1440)). *Observed in Devtool* makes it depend on ER-Devtool, which is why it was **held behind [FIX-1481](https://linear.app/fixpoint-labs/issue/FIX-1481)** — recorded in Linear as a soft dependency, not a hard block. **[FIX-1474](https://linear.app/fixpoint-labs/issue/FIX-1474) is not this producer** — its own body keeps this gate on today's separate paths and forbids widening it until that child lands |
...
*"looks good"* is not a proof. Each row says what passes and what the surface exposes. **The *Today*
column was drafted on 2026-09-20 as a source read** of `packages/devtool/src` and
`packages/workforce` on `main`, with no row observed live. **Where it stands on 2026-09-24:** the
rendering for rows 4 and 6 shipped ([#2032](https://github.com/fixpoint-labs/flow-state-dev/pull/2032),
[#2039](https://github.com/fixpoint-labs/flow-state-dev/pull/2039)); row 4 was graded live
([#2066](https://github.com/fixpoint-labs/flow-state-dev/pull/2066)); row 6 counts per
[D11](DECISIONS.md#d11); row 5 is built in W5 per [D10](DECISIONS.md#d10); rows 1–3 have not been
graded and ride FIX-1320. Each row's cell carries its own dated update.
...
| 4 · **Parked rows** | A parked row is visible **with its reason** without expanding raw JSON, and Waiting-on-you reads as a view over parked + reason ([ER-2](#what-every-exit-proof-must-satisfy)) — **never as a new status value** ([ER-8](#what-no-child-may-do)) | **As drafted: fails.** `parked` is a status pill and a counts-ribbon entry; the row's columns are id, goal, status, assignee, child session, kind. **The reason is only inside the per-row `view` JSON expander**. **Passed live 2026-09-22**: a Reason column ([#2032](https://github.com/fixpoint-labs/flow-state-dev/pull/2032)), graded on a live hire ([#2066](https://github.com/fixpoint-labs/flow-state-dev/pull/2066)) |
| 5 · **Inventory** · **[FIX-1502](https://linear.app/fixpoint-labs/issue/FIX-1502)** | The three org-scoped inventory collections — `inventory/seats/*`, `inventory/channels/*`, `inventory/members/*` — are readable for a live org, so *who exists* and *who is in which channel* are answerable without walking folders | The Resources panel is **session-scoped** (`GET /sessions/:id/debug/resources`) and **off unless `FSDEV_DEBUG_ENDPOINTS=1`**. Inventory is reachable only through a session whose flow declares those collections; **there is no org-level inventory view**. **Built in W5, decided 2026-09-24** ([D10](DECISIONS.md#d10)): read through a channel session, which already declares the three collections — see below |
| 6 · **Resources + references** | A seat's documents are visible, and a **read-only `references/` document is distinguishable from a mutable `resources/` one** — the distinction FIX-1467 just settled is legible in the inspector | **As drafted: fails.** The resources tree carries a scope badge (`session` / `user` / `org`) and **nothing marking read-only**. References install as org-scoped resources, so they are visible but **indistinguishable from mutable org resources**. **The mark shipped 2026-09-22** ([#2039](https://github.com/fixpoint-labs/flow-state-dev/pull/2039)), proved by FIX-1481's V2, V3 and V5. **Not read live** — no hired tree declares a sealed document; the live read waits for the first real hire that does, and [FIX-1547](https://linear.app/fixpoint-labs/issue/FIX-1547) is the mitigation ([D11](DECISIONS.md#d11)) |

**Two rows failed when this was drafted and one was half-built, and that was the finding** — ER-Devtool is not a polish
pass over a working inspector. As drafted, rows 4 and 6 needed rendering that did not exist; it has
since shipped (#2032, #2039). Rows 3 and 5 are session-scoped where the proof needs an org-level
reading; row 5 closes that gap through a channel session ([D10](DECISIONS.md#d10)). **Rows 1–3 read on the instance and
...
**What FIX-1502 builds, and the one gap it does not.** Four gaps, all verified on `origin/main`. A
2026-09-22 POC settlement priced the first three as ordinary work, and they are FIX-1502's. The
fourth binds only if row 5 means a surface standing apart from any session — and on 2026-09-24 the
owner ruled it does not ([D10](DECISIONS.md#d10)), which is why FIX-1502 is no longer blocked by
FIX-1486.
...
**Selection stays out of FIX-1502.** It opens the read on the three collections (gap 2) and reads
through a channel session (gap 3), always for the org that session already carries. It adds no org
picker; that address is FIX-1486's, and a one-user, one-org deployment does not need it
...
> is **selection**. A child that inherits the access framing will try to widen a read to reach
> another org, which is not what [D10](DECISIONS.md#d10) opens.
