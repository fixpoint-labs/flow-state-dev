...
| 4 · **Parked rows** | A parked row is visible **with its reason** without expanding raw JSON, and Waiting-on-you reads as a view over parked + reason ([ER-2](#what-every-exit-proof-must-satisfy)) — **never as a new status value** ([ER-8](#what-no-child-may-do)) | **Fails as written.** `parked` is a status pill and a counts-ribbon entry; the row's columns are id, goal, status, assignee, child session, kind. **The reason is only inside the per-row `view` JSON expander**. **Passed live 2026-09-22**: a Reason column ([#2032](https://github.com/fixpoint-labs/flow-state-dev/pull/2032)), graded on a live hire ([#2066](https://github.com/fixpoint-labs/flow-state-dev/pull/2066)) |
...
**Where it stands, 2026-09-24:** row 4 passed live; row 6's mark shipped and is proved by automated
checks, with its live read carried ([D11](DECISIONS.md#d11)); row 5 is FIX-1502's to build in W5
([D10](DECISIONS.md#d10)); rows 1–3 still ride FIX-1320.
