...
that is not W5's**: rows 1–3 ride FIX-1320 outright, and row 5 waits on FIX-1486 under one reading
of the row and not under the other — which reading is [Open 1](DECISIONS.md#open), and the figure's
`BLOCKED BY` marker reads against the first of them. The ship fence
...
| **[FIX-1502](https://linear.app/fixpoint-labs/issue/FIX-1502)** org-level inventory view | spec → impl · **Backlog, no spec** · **blocked by [FIX-1486](https://linear.app/fixpoint-labs/issue/FIX-1486) in Linear**, under one reading of row 5 ([Open 1](DECISIONS.md#open)) | The three `inventory/*` collections · and, **only if row 5 means a surface apart from any session**, **a way to *select* an org, which does not exist yet** — an org-scoped read always answers with the org its session already carries, and nothing can choose another | **Checklist row 5** — a reader and view answering *who exists* and *who is in which channel* for a live org | Row 5 of ER-Devtool, and nothing else | Unknown · **startable inside W5 or not, on [Open 1](DECISIONS.md#open)'s answer** — the 2026-09-22 settlement priced the read itself at app wiring, three lines on the collection factories and a reader. **Org *selection* is the [ER-25](BUSINESS-RULES.md) growth; row 5's reader is not** |
...
6. **[FIX-1486](https://linear.app/fixpoint-labs/issue/FIX-1486) lands** → FIX-1502 unblocks under
   every reading of row 5. **Whether row 5 needed this milestone at all is
   [Open 1](DECISIONS.md#open)**: the 2026-09-22 settlement showed the org-scoped read already
   served on the production route, so this step binds only if row 5 means a surface standing apart
   from any session. W5 does not own **org selection** and must not build it
   ([ER-25](BUSINESS-RULES.md)); nothing else in the set waits on it.
