...
| ER-5 | A person in one org keeps the seat-only resource data their hired seats saved, once the operator runs FIX-1538's documented copy step ([FIX-1538 D2](../../issues/FIX-1538/DECISIONS.md#d2)); until then each seat opens with an empty cell. A seat's user state and any resource an app flow also declares are not copied unless the operator can attribute them. The step names what it cannot attribute to one org instead of guessing. The original data is never moved or deleted. The rule does not cover the person's app-wide data: a seat no longer reads it, even in one org ([FIX-1538 D1](../../issues/FIX-1538/DECISIONS.md#d1)) | FIX-1538 | FIX-1538's BR-13 in CI; its BR-14 – BR-16 and BR-18 by the documented step, walked once on SQLite |
