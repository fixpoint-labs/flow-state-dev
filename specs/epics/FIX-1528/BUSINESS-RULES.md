...
| ER-2 | A board drains only onto seats on its own plane. A drain onto another org's seat, or onto another user's private seat in the same org, is refused at admission and the task is not claimed | FIX-1534 | Its drain leg, on a real claimed task and worker pool |
...
| ER-4 | Data a hired seat stores for its owner lives in a cell keyed by (org, user). Signing into another org shows none of it, and the same kind hired there starts empty | FIX-1538 | The assembled goal ([ER-15](#the-proof)) |
| ER-5 | A person in one org sees no change. Data already written under the bare user id is found, and anything that cannot be attributed to one org is named, not silently moved or dropped | FIX-1538 | FIX-1538's spec review and migration test |

## What no child may do

| # | Rule | Because |
|---|---|---|
| ER-6 | Add a second hire store, or `UserWorkforce`, Hire, Role, Agent, Team or Channel as Layer 1 | One store, and the owner picks the cell. Invent-kill |
| ER-7 | Mix planes on assign, drain, list, catalog, open, restart or debug, including through a "helpful" global inventory. Write, impersonate or push into another person's private cell | The fence is only true if no door is soft |
| ER-8 | Collapse the roster owner into the principal's org, or the reverse | They answer different questions. Org is security. Owner is whose seat it is |
| ER-9 | Re-key the person's cross-org data, or user scope for flows that are not hired seats | [D2](DECISIONS.md#d2). That plane's default is a separate product call |
| ER-10 | Teach or document user planes, ship org-owned hire cells, or make portability, bridge seats, notify, always-on seats or board assignment to user ids part of this epic's exit | [D4](DECISIONS.md#d4) and the product decisions |
...
|---|---|---|
| ER-15 | Alice's Acme private seat, which has written a marker to its own data, is invisible and inert from Globex and from Bob in Acme: not listed, not opened, not run, not drained onto, not in the debug listing, and its marker is absent from Alice's Globex seat of the same kind. Alice's own Acme run still reads it | FIX-1538's goal under `goals/hire-plane/`, on the real router and worker pool |
