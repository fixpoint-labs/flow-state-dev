...
| ER-3 | An answer carries confidence and per-option probabilities only when the model returned them. The block never supplies a number the model did not. Absent confidence fails closed only at a gate; a consumer branching on the bare answer never consults it ([D2](DECISIONS.md#d2)) | FIX-1554 decides · FIX-1555, FIX-1557, FIX-1558, FIX-1559 consume | Each consumer's spec review |
...
| ER-11 | Make orchestration, memory or a resource host import Jev or the lab, build an evaluator unasked, or name or resolve a model. Edit the stock agent kind's skill loop. A `<x>Evaluator(model)` helper the app calls with its own model is not building one unasked; it ships only where the consumer owns the question | [D3](DECISIONS.md#d3). The stock fence from FIX-1362 and FIX-1363 holds |
