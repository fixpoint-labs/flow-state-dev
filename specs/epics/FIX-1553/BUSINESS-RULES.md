...
| ER-4 | In `cascadingRouter` a gated edge opens only when the choice matches and confidence reaches the floor. Absent or low lands on the author's `ambiguous` leaf, visibly in the trace, never on a sibling | FIX-1558 | Its tests · leg (c) |
...
| ER-7 | Memory runs with no evaluator installed. With one passed, a documented seam calls it | FIX-1555 | Leg (f) |
| ER-8 | Every doc and contract that says "four block kinds" says five, in the same change that ships the kind | FIX-1554 | Its PR · [DOCS.md](DOCS.md) |
...
| # | Rule | Because |
|---|---|---|
| ER-13 | A proof leg counts toward the lead measure only when it runs the real path (`fsdev run` or a `goals/` check) on a real evaluation model. A mocked spec proves the unit, not the leg | Goal 1's bar, applied here. A mock can hand the gate any confidence it wants |
