...
| ER-2 | A post with no seat author runs each member **agent** seat of that channel once, through an internal receiver the agent kind declares. `desk-clerk` and `followup-runner` members are not run | FIX-1590 introduces it in kitchen-sink host glue · FIX-1602 lands it in Workforce | FIX-1590's browser check · FIX-1602's · at epic level, FIX-1601's leg b |
| ER-3 | A post whose author is a seat wakes no seat ([D2](DECISIONS.md#d2)) | FIX-1590 decides and introduces it in host glue · FIX-1602 lands it in Workforce · FIX-1594 consumes | FIX-1590's package test · FIX-1594's browser check · FIX-1602's · at epic level, FIX-1601's leg b |
...
|---|---|---|
| ER-17 | [The goal](SPEC.md#the-goal-and-how-well-know-its-met) is met, on a production build, keyless, all four checks run on one `main` commit: a message to `support.otto` and its reply survive a reload; a note to `support.ada` gets a model's reply, not the note; one post to `support.desk` runs `support.iris` and `support.otto` once each; a member agent's reply shows in `support.desk` under its name, survives a reload, and runs no seat. The post runs through Workforce's stock seat-wake routing, with kitchen-sink moved onto it (FIX-1602). Each check's named control has been seen to FAIL | FIX-1601's QA plan ([#2288](https://github.com/fixpoint-labs/flow-state-dev/pull/2288)), per [the closure issue](../../../docs/contributing/orchestration.md#the-closure-issue-every-epic-ends-in-qa) |
| ER-18 | The workforce-shell checks and the `a-channel-holds-the-work-a-seat-drains` goal stay green. Leg V14 keeps its intent, nobody told about their own post and the fan-out addressing the whole roster; its expected delivery set follows FIX-1602's rule, so a member whose seat can hear a post is not in it (FIX-1603) | Each child's PR · CI |
| ER-19 | Kitchen-sink's README tells a reader the three ways of talking | [DOCS.md](DOCS.md) · complete before FIX-1601; the last touch, to the fan-out wording, is FIX-1602's |
