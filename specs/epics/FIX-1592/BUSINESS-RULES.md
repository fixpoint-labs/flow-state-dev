...
|---|---|---|
| ER-17 | [The goal](SPEC.md#the-goal-and-how-well-know-its-met) is met, on a production build, keyless, all four checks run on one `main` commit: a message to `support.otto` and its reply survive a reload; a note to `support.ada` gets a model's reply, not the note; one post to `support.desk` runs `support.iris` and `support.otto` once each; a member agent's reply shows in `support.desk` under its name, survives a reload, and runs no seat. Each check's named control has been seen to FAIL | FIX-1585's, FIX-1589's, FIX-1590's and FIX-1594's browser goal checks |
