...
| ER-7 | One scripted model. FIX-1585 maps the agent kind's generator in kitchen-sink's test resolver; each child adds its own scenario to `apps/kitchen-sink/lib/e2e-mock-script.ts` ([D3](DECISIONS.md#d3)) | FIX-1585 decides · FIX-1590, FIX-1594 consume | Each child's browser check, run keyless |
...
|---|---|---|
| ER-17 | On a production build, keyless: a message to `support.otto` and its reply survive a reload; one post to `support.desk` runs `support.iris` and `support.otto` once each; a member agent's reply shows in `support.desk` under its name, survives a reload, and runs no seat | FIX-1585's, FIX-1590's and FIX-1594's browser checks |
