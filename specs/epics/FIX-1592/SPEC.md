...
| **posts to `support.desk`** | Every member is "notified" by a stub that names them and stops. No seat runs | Each member seat that can answer runs its answering action, and its reply shows in that seat's conversation |
...
| **Not doing** | Assistant tools that post or ask · channel admin: create, delete, invite ([FIX-1415](https://linear.app/fixpoint-labs/issue/FIX-1415)) · verified per-participant identity ([FIX-1493](https://linear.app/fixpoint-labs/issue/FIX-1493)) · any Layer 1 Channel, Notify or Dispatcher substrate |
| **Kill line** | Model-backed answering needs a Workforce or Layer 1 change beyond [FIX-1585's D1](https://github.com/fixpoint-labs/flow-state-dev/blob/spec/FIX-1585/specs/issues/FIX-1585/DECISIONS.md) expose line, or the desk can't be proven without a live model key |
...
worse than unreachable, because it looks like it works. The other two gaps, a fan-out that
wakes nobody and boards nobody fills, show up the same day for the same reader.

## What's in the box

![What's in the box. In the box, shipped in kitchen-sink: talk from the page (FIX-1585), a clerk that answers from a model or files to a board (FIX-1589), a post that wakes member seats (FIX-1590), and a board panel where a person picks up an escalations row and runs the followups drain (FIX-1591). The fence: every browser verb is an action a flow already declares, and the only package change is FIX-1585's transcript expose line. Composed in by the app: the model, the one kind-to-action map, the notify router. Replaced in one line: the model, a kind's answering action. Not built: Assistant tools that post or ask, channel admin, verified identity, Layer 1 substrate, a kitchen-sink-only messaging API, live updates of other people's posts.](figures/end-state.svg)
...
| [FIX-1590](https://linear.app/fixpoint-labs/issue/FIX-1590) · posts wake seats | The notify stub becomes a dispatcher per member, keyed through FIX-1585's D3 map | A post that wakes nobody contradicts the page it sits on. The weakest of the four ([D1](DECISIONS.md#d1)) | Backlog · after FIX-1589 |
...
**0 done · 1 in spec review · 3 not started.** Whether four is really three: FIX-1590 is the one
I'd cut. No Proof line reads it, and one post to `support.desk` wakes up to four model-backed
seats. It stays because a post that wakes nobody is the honesty gap FIX-1589 closes for the
clerk. Collapse trigger: if waking needs more than the dispatcher-per-member shape the channel
docs teach, it leaves the epic.
...
flowchart LR
  ...
  A -->|"the D3 map"| C["FIX-1590 · posts wake seats"]
  B -->|"a seat worth waking"| C
  B -->|"rows on both boards"| D["FIX-1591 · the boards work"]
  L["FIX-1459 · PACKAGE.md"] -.->|"frees agent-worker-flow.ts"| C
```

A solid edge is a hard dependency, wired in Linear as blocked-by. The dashed edge is another
thread's work that fences a file, not a child. After FIX-1589, FIX-1590 and FIX-1591 can run
side by side.
...
- **The post contract and the notify rule**: a post naming no author reaches every member, and
  nobody is told about their own post (FIX-1476 BR-16a, goal leg V14).
...
- **Related, not children:** [FIX-1459](https://linear.app/fixpoint-labs/issue/FIX-1459),
...
1. **[D1](DECISIONS.md#d1) · A usable desk is worth four issues, run 1585 → 1589 → 1590 and 1591
   side by side.** If wrong: a cycle spent on the reference app's desk while another subsystem's
   goal stays red. The Kill line is the stop.
2. **[D4](DECISIONS.md#d4) · The browser checks run keyless on kitchen-sink's deterministic
...
3. **[D3](DECISIONS.md#d3) · Serving `escalations` retires the reference app's
   unattended-board demo.** Decided by the owner on 2026-09-25; listed so it is signed with the
   rest. If wrong: a reader of kitchen-sink no longer sees the warning a declared, undrained board
   produces, and learns it from the docs alone.
