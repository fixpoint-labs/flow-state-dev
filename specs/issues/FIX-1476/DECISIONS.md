...
| **Now settled** | **`channels/ada-wren/`, members `[support.ada, support.wren]`** — chosen by the implementer and folded here, which closes the last thing this card left open. The pair is not arbitrary: **Ada works the desk** (`flow: desk-clerk`) and **Wren runs the board the desk's work lands on** (`flow: followup-runner`, draining `followups`), so the two already hand work to each other. The DM is the channel that pair would actually have, which is what makes it a reference rather than a demo. It renames cleanly from `ada-dm` because it holds no `boards:` — verified by renaming the folder mid-run and watching the goal still pass; a channel *with* boards would have re-keyed its ledgers and orphaned their rows ([Pins](PLAN.md#pins)) |
