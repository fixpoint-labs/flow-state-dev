...
| **[D2](DECISIONS.md#d2)'s third channel was a one-member DM** | Approved at spec review | **Reversed by the product owner** after seeing it built, on [#2007](https://github.com/fixpoint-labs/flow-state-dev/pull/2007), then refined on [#2010](https://github.com/fixpoint-labs/flow-state-dev/pull/2010) — two seat members, and no post notifies its own poster **in any channel** ([D6](DECISIONS.md#d6)). A first reading of the reversal produced a *silent* DM; that was wrong and is recorded as rejected rather than quietly replaced | — (a decision, not a fact; the words are in D6) |
