...
| The notify block is handed the channel's id, so per-channel delivery is user-space | `git grep -n "The channel's session id" origin/main -- packages/workforce/src/channel/channel-flow.ts` | `:524`, inside `channelNotifyInputSchema`. [D6](DECISIONS.md#d6)'s silent DM rests on this — no `packages/workforce` change |
...
| S2 | `workforce/teams/support/channels/{desk,<dm>,noticeboard}/CHANNEL.md` | Three instances. `desk`: five members, `boards: [followups, escalations]`. **`<dm>`: exactly two members, both seats, no `flow:`, and no notification delivered — [D6](DECISIONS.md#d6). Its folder name is the implementer's and is not pinned.** `noticeboard`: `flow: digest`. Each charter says in one line which lesson it carries | BR-3 BR-4 BR-6 BR-17 |
...
| V13 | S5 S6 | A post in `support.<dm>` notifies nobody: no delivery to either member ([BR-16](BUSINESS-RULES.md)) | Let the notify block treat the DM like any other channel — a delivery per member appears, which is the noise [D6](DECISIONS.md#d6) reversed |
