...
| V13 | S5 S6 | **Both halves, on two channels, graded as a property and not as a field.** A post by one member of `support.desk` notifies the other **four** and not its poster; a post in `support.ada-wren` notifies the one other member and not its poster ([BR-16](BUSINESS-RULES.md)). Addressees are **named**, not counted as non-zero. Written against the observable outcome rather than the field the skip compares, so it survives the framework change [D6](DECISIONS.md#author-identity) names as the real fix | Two, and both are needed. **(i)** Drop the skip — the poster is notified too (`desk` 5, DM 2). **(ii)** Make the block deliver nothing — the poster still gets nothing, so the first half alone goes green on a dead channel. A leg asserting only *"the poster got none"* is the check-that-cannot-fail this table forbids. **Note for whoever writes it:** the skip compares `author`. A skip on `principal` fails (i) on every channel — that comparison is never true here, and [D6](DECISIONS.md#author-identity) has the reason |
| V14 | S2 | `support.ada-wren` declares exactly two members, both of them seats ([BR-17](BUSINESS-RULES.md)) | Leave the roster at one member — the pre-reversal shape passes every other leg |
