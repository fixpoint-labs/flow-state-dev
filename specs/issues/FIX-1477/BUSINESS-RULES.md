...
| # | When | Then | Proved by |
|---|---|---|---|
| BR-7 | A **kind** row is opened | **Zero** session-list requests are made, for any number of instances under it | CI · a counting fake session client |
...
| BR-11 | A response arrives after the leaf that asked for it was collapsed, or another leaf was opened | It is discarded. No leaf ever shows another leaf's sessions | CI · resolve two requests out of order |
| BR-12 | A session row carries no recorded owning instance | It is listed under its kind and never attributed to an instance (BP-030) | CI · a fixture session with no instance id |
...
| BR-14 | A file in the **reference app** was installed from the component registry | It is byte-identical to its source in that registry. Where the app's version is the better one, it is pushed back into the registry; where it is not, it is reverted | CI · compare every target the registry manifest declares |
...
| BR-19 | A roster is rendered | It shows the organization's seats, read from the standing roster collection. The collection is organization-scoped at the store, so nothing in the component filters | CI, against a fixture collection |
...
**Nothing here is fatal.** A flow list that will not load leaves the rail with a retry and the
stream untouched — the app is still usable, because the stream does not depend on the rail. A
session list that fails marks its own leaf and leaves every sibling alone. A roster or board read
that fails marks its own region. There is no path in which a failure in a resource-backed region
stops a turn from being sent or rendered, and that is the point of keeping them on separate
reads.
...
— the developer tool, with its own skin and its own affordances, browsing kinds, instances and
sessions through the same component. That is [ER-24](../../epics/FIX-1455/BUSINESS-RULES.md),
and it is the claim the epic's completion gate rests on. Proved by the developer tool running
with its navigator folder deleted, not by an export list.
