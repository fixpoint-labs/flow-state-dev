...
| BR-29 | A host renders these components against a deployment that authenticates | They read through the transport the host already supplies — the `fetcher` its clients take — so every request carries its credential. A component that builds its own client lists flows successfully (that route is exempt from authorization) and then fails everything under it — the **navigator** on every session read, and a **panel** on every collection read, which is the same failure one surface over | CI · **V7** for the navigator, through the developer tool's own bearer path, and **V16** for the panels, through the host-passed `PanelRowSource` |
...
| BR-19 | A roster is rendered | It shows the organization's seats, read from the standing roster collection. The collection is organization-scoped at the store, so nothing in the component filters. The governing session is one whose flow declares the collection, and the collection has to permit a browser read before any session can serve one ([PLAN.md → Blocked on](PLAN.md#blocked-on), [D4](DECISIONS.md#d4)). **All of them**, not the first page — the list route pages, so a panel that reads page one and stops truncates silently | CI · **V11** (projection and paging) and **V16** (the host's transport), against a fixture collection. The **isolation** half is proved where this rule locates it — at the collection, not in the component — by `packages/workforce/test/cross-org-collection-read.test.ts` (introduced by [PR-A2](https://github.com/fixpoint-labs/flow-state-dev/pull/2036)), which plants seats in two organizations and asserts a session reads only its own, while the caller claims the other in the body, the query and two headers. **Not V15** — that asserts which *problem strings* each panel shows, and nothing about which seat rows arrive |
| BR-20 | The roster reports seats that were skipped at boot | The count and the list are shown, not only logged. "The roster" and "what answers" are two numbers ([FIX-1475](https://linear.app/fixpoint-labs/issue/FIX-1475)'s BR-24). The report reaches the browser through its own org-scoped resource, because a module export cannot ([PLAN.md → BR-20's transport](PLAN.md#br20-transport)) | CI · **V14** (the skipped seat is on screen) and **V15** (the right organization's, and cleared on the next boot) |
| BR-21 | A board column is rendered | It shows the rows that board holds, grouped by the existing task statuses — **all** of them, paged rather than truncated at the first page. No new status vocabulary is minted | CI · **V11** (projection and paging) and **V16** (the host's transport) |
