...
| S8 | kitchen-sink · the shell | `app/page.tsx`: the rail becomes one `FlowNavigator` with a Channels and a Seats section; the right panel becomes standing `BoardColumns` + `Roster` and **loses its build-mode conditional**; the narrow-width order is wired. **Remove** `components/session-sidebar.tsx`. One wiring the panels cannot do for themselves: the boot's skipped-seat report is **written to an org-scoped resource** and read back, because a module export cannot reach the browser ([BR-20's transport](#br20-transport)). **Binding the session to a viewer's organization is not `S8`'s** — there is no viewer credential in this repository, so `S8` ships that as a documented limit and the binding itself belongs to [FIX-1503](https://linear.app/fixpoint-labs/issue/FIX-1503) ([Blocked on](#blocked-on)) | BR-13 BR-18 BR-20 BR-25 – BR-28 |
...
issue does not expect to build identity. Neither side is waiting on the other by accident.

**Decided: this issue ships `S8` with the limit.** Not a product call left open — it is
derivable from decisions already made. The identity work was deferred, and FIX-1503's *Out*
names this issue and rules out hard-blocking kitchen-sink, so waiting would mean holding a
finished shell behind an epic that has explicitly declined to gate it. **`PR-C` is complete when
the panels work in an unconfigured clone and the limit is documented.** The seat-list fork in
[DECISIONS.md](DECISIONS.md#open) stays the only open question in this issue.
