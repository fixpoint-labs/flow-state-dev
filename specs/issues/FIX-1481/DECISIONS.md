...
flowchart TD
  ...
  D2 -.->|"rejected"| X4["declare a client surface on the inventory<br/>an org read with no org to address"]
```
...
| **Because** | Three things are missing and only one is DevTool's. **Nothing opens an inventory** — no application calls `openInventory`, so there are no rows for any door to serve. **The non-debug door cannot see them** — the manifest route skips every resource without a `client` config and the inventory collections declare none; and declaring one would not yield an *org-level* read, because there is no org to address. With no principal resolver configured every session binds to `DEFAULT_ORG_ID` (`packages/engine/src/routes/session-routes.ts`, `handleCreateSession`: `orgId: ctx.principal?.orgId ?? DEFAULT_ORG_ID`), and the listing beside it withholds every row not on that org. *Which org* is a question the surface cannot be asked — an **absent addressing axis**, not a security hole, and exactly what [FIX-1486](https://linear.app/fixpoint-labs/issue/FIX-1486) supplies, which is exactly why [FIX-1502](https://linear.app/fixpoint-labs/issue/FIX-1502) is blocked on it. **And no reader exists to inherit it** — building one here is the substrate growth [ER-25](../../epics/FIX-1457/BUSINESS-RULES.md) names, a read surface plus an authorization model arriving under the label *polish*, so it is raised up rather than answered locally ([ER-17](../../epics/FIX-1457/BUSINESS-RULES.md#er-17)) |
...
| Declare `client: { state: { read: true } }` on the inventory collections | Opens the non-debug door in one line, onto one org — `DEFAULT_ORG_ID`, the only org a session can bind to until [FIX-1486](https://linear.app/fixpoint-labs/issue/FIX-1486) lands. A one-line change that ships an org-level view nobody can point at an org is not cheap |
