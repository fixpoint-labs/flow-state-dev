...
| Do not add a `client` config to the inventory or reference collections to "just make row 5 work" | [D2](DECISIONS.md#d2) has the reasoning. One line, and an org-level view that can only ever be pointed at `DEFAULT_ORG_ID`. **Not BP-031** — an earlier wording said so, and the correction on [D2](DECISIONS.md#d2) explains why that was wrong |
