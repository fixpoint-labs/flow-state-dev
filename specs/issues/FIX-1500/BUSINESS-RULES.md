...
| BR-34 | The first boot after this change runs over a store written before it | The boot completes, and every file-declared channel is open and readable under the kitchen-sink organization. **Whether earlier conversation history carries over is open** ([H1](DECISIONS.md#h1)). The recommendation is to leave it behind: unread, not deleted | V19 · red states are the unhandled channel open (`Request failed (403)`) **and** a boot that swallows it, leaving the channel bound to the old organization |
