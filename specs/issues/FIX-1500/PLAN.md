...
| V19 | S10 | **On a persistent store written by the app before PR-B** (a real pre-change boot, then the PR-B boot over the same location): after the first named-organization boot, **every file-declared channel's session is bound to `KITCHEN_SINK_ORG_ID`, and reading it through the app's router as the kitchen-sink principal returns 200**, for the session and for one of its boards. Asserted per channel, from the store and the route, not from the boot's log | Leave the upgrade unhandled: the boot fails with `could not be opened — Request failed (403)`, the POC's N8. **Swallow that 403 and carry on:** the boot completes, the channel session is still bound to the development organization, and the read returns 403. **Both must turn V19 red** (BR-34) |
