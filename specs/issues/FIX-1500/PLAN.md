...
| S8 | kitchen-sink · the rail | **Merged in [#2193](https://github.com/fixpoint-labs/flow-state-dev/pull/2193), and amended since ([E4](DECISIONS.md#e4)).** A seat row carries **one** row action in `leafToolbar`: an icon button with an `aria-label`. It opens the seat's `SeatDetail`, with its kind, and the hire affordance **outside the rail row**, in a popover or the right-hand panel. `leafToolbar` draws on the row itself ([FIX-1561 D1](../FIX-1561/DECISIONS.md#d1)), so nothing wider than an icon goes in it, and this issue adds no navigator slot. The hire affordance calls S7, then refreshes the roster by a **specified, checked remount** ([D4](DECISIONS.md#d4)). `RosterProps` publishes no refresh, ref or version, and this issue adds none. `Roster`, `SeatDetail` and the hire use **one** roster ref, `HIRED_ROSTER_RESOURCE`, passed as `collectionRef`, because both components default to `"roster"`. No create affordance for a channel or a board | BR-5 BR-16 BR-24 BR-29 – BR-31 |
