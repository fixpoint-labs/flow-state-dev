...
| S10 | kitchen-sink · the one organization ([D6](DECISIONS.md#d6)) | Pass one host-level `resolvePrincipal` to `createFlowState` in `apps/kitchen-sink/fsdev.config.ts`. It returns `{ userId: "devuser", orgId: KITCHEN_SINK_ORG_ID }`, both constants, and reads **nothing** from the request (BP-031). Every flow with no resolver of its own resolves through it: `chat-agent`, every seat, every channel. `workforce-admin` and `weekly-digest` keep theirs ([E3](DECISIONS.md#e3)). **The user is a constant as well**, because read routes carry no body (POC N7). Keep the Playwright suite green: its per-test `?e2eUserId=` no longer separates users, and how isolation survives is yours to decide, provided the organization still comes only from the constant. **A store written before this PR** has its channel sessions in the development organization, and the first boot after it cannot open them (POC N8). Handle that so the boot does not fail (BP-030), and say what happens to earlier conversations | BR-1 BR-3 BR-4 BR-33 BR-34 |
...
| PR-B | S10 | — | **Kitchen-sink only, no package change, no changeset.** It changes who every request in the app runs as, so it goes alone, and its e2e run is the fence. Without it, S7's hire is refused in the app as shipped ([D6](DECISIONS.md#d6)) |
...
| V19 | S10 | The first boot over a store written before PR-B completes, and does not fail at the channel open | Leave the upgrade unhandled: the boot fails with `could not be opened — Request failed (403)`, the POC's N8 (BR-34) |
...
| kitchen-sink | `KITCHEN_SINK_ORG_ID = "kitchen-sink"` | The organization an operator names in `WORKFORCE_ADMIN_TOKENS` to administer the rail's seats, and the one the docs name |
...
- [`poc/named-org/`](poc/named-org/README.md): D6's mechanism, run through the app's real router
  ...
  consequences recorded in S10, S7 and V19.
...
gives every request one named organization, set in host code. Real identity is
...
| Operator tokens configured | **The whole spine still works**, in the kitchen-sink organization. A token bound to `kitchen-sink` administers the same seats: it can fire a rail or mara hire. Its own hires are user-owned and are not listed in the rail. A token bound to any other organization administers seats this rail never shows |
