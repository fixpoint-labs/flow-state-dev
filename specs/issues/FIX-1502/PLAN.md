...
| S2 | `devtool` · a new Inventory tab in the session workspace | Shown when the manifest lists a readable collection on one of the three patterns. Reads every page through the production collection read | BR-7 – BR-16 |
...
| A refused or failed read is never drawn as an empty organization | An empty table after a 403 is a green row on a broken read |
