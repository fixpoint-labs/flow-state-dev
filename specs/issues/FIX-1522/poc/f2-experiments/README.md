...
| **C2** (closed at `ef5228f8`) | Fully parameterised deep patterns get past the structural guard. `workforce/[r]/[owner]/[seat]` and `[a]/[b]/[c]/[d]` both register, and a run in bob's session lists `ALICE-PRIVATE`. The guard only inspects patterns that start with the literal `workforce/roster/`, but a `[param]` segment matches any segment. (`*`-led patterns such as `*/*/*/*` register too, but match nothing, because `*` is literal inside a parameterised pattern and a bare `*/…` has no prefix to anchor on) |
| **D** (closed at `ef5228f8`) | The single-segment **browser** roster (`defineHiredRosterCollection`) does not stop an exact-key read. A block in bob's session calls `getOptional("~alice/research")` on it and gets alice's full row, instructions and settings included. `list` filters by pattern, but store-backed `get`/`getOptional` do not. The HTTP item route refuses the same key (`400 Topic does not match collection pattern`), so the hole is server-side only. This confirms Bugbot's first-head finding, still live at `fdca49fd` |
...
### Third run: #2091 at `ef5228f8`

**16 passed.** Both holes from the second run are closed:
- **C2:** `workforce/[r]/[owner]/[seat]` and `[a]/[b]/[c]/[d]` are refused at registration.
- **D:** bob's exact-key read through the browser roster returns `undefined` on the server. The HTTP route still answers `400`.

A, B, B2, C, E1–E3, E6–E9 and R1–R3 still hold. The user id is now escaped in the private storage key too (`~${encodeUserSegment(userId)}/…`), which closes the `/`-in-user-id concern.
