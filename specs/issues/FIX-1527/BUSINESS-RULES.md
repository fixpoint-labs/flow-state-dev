...
| BR-2 | Mara calls `hire` under the development organization, which is every default kitchen-sink run today | Refused before any write. No roster row, no inventory row, nothing registered. The error names the organization | V2 · VG |
...
and it's expected in the default run until the app has a real organization
([F1](DECISIONS.md#f1)). A failing `register` deletes the row it just wrote (consumed). Nothing
...
- On the real path, mara under a real model is asked to hire and calls `hire`. With F1 resolved
  as *ship now*, the run ends in BR-2's refusal and writes nothing.
