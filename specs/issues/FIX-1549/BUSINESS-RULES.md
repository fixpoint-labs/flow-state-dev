...
- **An owner segment** is a key segment beginning `~`. `ownerSegment(userId)` builds one:
  `~` plus `encodeUserSegment(userId)`.
- **An owner key** is a stored key with at least one owner segment.
- **Armed** means the registry holds, or has held, a flow declaring an owner-private collection.
- **Reaches** means a pattern can resolve onto a key the owner-private collection's pattern
  matches (segment by segment: a literal meets an equal literal, a parameter or `*` or `**`).

Rule numbers are kept from the approved set where the rule survives. BR-20 to BR-23 are new.
...
| BR-8 | An owner-private collection enables a browser read, names a parameter its pattern lacks, or uses `**` | Refused at definition, with the pinned message. Moved from registration | CI |
...
| BR-20 | Two owner-private collections that reach each other with a different pattern, parameter or scope | Refused with the overlap message | CI |
...
| BR-21 | An owner-private collection reads or writes a key | Served only when the segment at its owner parameter is `ownerSegment(caller)` and no other segment begins `~`. With no caller, nothing is served. Otherwise it answers as BR-15 to BR-17 | CI |
...
| BR-14 | Any pattern a Workforce app was refused on `main` | Still refused. The message is the pinned generic one | CI · corpus equivalence, armed |
| BR-22 | A row written before this change, at `workforce/roster/~<user>/<seat>` | Read back through Workforce's private collection with no migration | CI · a row planted store-direct |
| BR-23 | Anyone greps `packages/engine/src` for `roster`, `hire`, `seat` or `workforce` | No hit, except the trace store's persisted `_roster.json`, which is allowlisted by name | CI · a guard test |
...
**Reserved key shape (D5).** In every app, a key segment beginning `~` belongs to an owner-private
collection. No other collection reads or writes one. No deployment rule.
