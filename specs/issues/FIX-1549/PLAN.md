...
|---|---|---|---|
| S1 | `core` · `ResourceCollectionConfig`, `defineResourceCollection` | **Add** `ownerPrivate?: { param: string }` and its define-time shape checks: the parameter occurs exactly once in the pattern, no `**`, no browser read (`client.state.read`, `client.content.read`, `client.content.prefetch`). **Add** `ownerSegment(userId)`. **Remove** the roster call from `defineResourceCollection` (D4) | BR-1 BR-8 |
| S2 | `core` · collection patterns | **Remove** `assertRosterCollectionIsNotDeep`, its private helpers and both re-exports | BR-1 |
| S3 | `engine` · a new owner-private module (not `hire-plane.ts`) | The one key predicate `(collection, storageKey, userId) → admitted`, which takes a key's first `~` segment as its owner; the generic reach test between two collections of one scope; and the startup fence. No Workforce name (D1 D3 D5) | BR-4…BR-7 BR-14 BR-20 BR-21 BR-24 |
...
| S4 | `engine` · the flow registry | Replace the per-flow roster scan with the registry-level fence, armed by any flow declaring an owner-private collection. Validate before mutating. Never disarm (D1) | BR-2…BR-7 BR-9…BR-11 BR-24 |
...
| S7 | `engine` · the instance pin | Move out of `context/hire-plane.ts` into a module named for the pin. `"roster-owner"` → `"owning-user"`. Reword every comment and doc string in `packages/engine/src` that speaks of Workforce's roster, hires or seats (the pin's callers, `stores/scope-keys.ts`'s owner-pinned cell, `resources/internal.ts`, `flowstate/types.ts`, `flow-registry.ts`, `createFlowState.ts`). Task-board and ordinary uses of "seat" stay. Engine README's "Hired seats" becomes "Owner-pinned instances" (D6) | BR-12 BR-23 |
| S8 | `engine` · a guard test | Fails on concrete Workforce coupling in `packages/engine/src`: an import from `@flow-state-dev/workforce`; any roster identifier Core exports today (`HIRED_ROSTER_*`, `*HiredRosterPrivateCollection`, `assertRosterCollectionIsNotDeep`) or S6 deletes; the `workforce/roster` key; the `"roster-owner"` reason; a module path naming `hire-plane`. Not a word ban | BR-23 |
...
| PR-B | S7 S8 · Engine README · changeset | PR-A | Renames and comment rewrites across the Engine files S7 lists. No behaviour but one error `reason` literal. The guard can only go green once PR-A has removed the roster code |
...
| V3 | PR-A | BR-6, BR-7, BR-9, BR-10, BR-20, BR-24 | Disarm on unregister (BR-9); compare declarations by identity (BR-10); compare patterns across scopes (BR-24) |
...
| V8 | PR-A | BR-21: an owner-private collection serves only the caller's segment; refuses a write under another user's segment, under a non-`~` owner value, and where a `~` segment comes before the owner parameter; a `~` segment after it is served as data | Compare the raw user id instead of `ownerSegment`; refuse every second `~` segment |
| V9 | PR-A | BR-8 at definition, each shape: `client.state.read`, `client.content.read`, `client.content.prefetch`; a parameter the pattern lacks; a parameter it repeats (`drafts/[owner]/[owner]`); `**` | Check only that the parameter is present |
| V10 | PR-A | BR-22: rows planted at `workforce/roster/~alice/research` and `workforce/roster/~alice/~research` read back through Workforce's collection for Alice | Change the marker; refuse every second `~` segment |
...
| V12 | PR-B | BR-23: the guard passes | Import `HIRED_ROSTER_PRIVATE_PATTERN` into any Engine file |
...
| Define refusals | `Owner-private collection "<P>" must not enable a browser read.` · `… must declare parameter "<p>" exactly once.` · `… must not use "**": its owner sits at one segment.` | BR-8 |
...
ownerKeyAdmits(collection, key, user):            ← the one predicate
    owner ← index of the key's first segment beginning "~"   ← later ones are data
    if collection.ownerPrivate is unset: return owner is none
    if owner is none or user is missing: return false
    return owner == index of collection's owner parameter
       and key[owner] == ownerSegment(user)
...
registry.register(flow):
    ...
    if declares or armed:
        ...
        in its scope and is not the same declaration        ← throws, nothing mutated
...
- `validatePattern` and `resolveCollectionKey` accept a `~`-leading parameter value, as the
  private roster relies on; checked on `3448303`. Workforce's row schema accepts a seat id such as
  `~research`. Both shipped hire paths refuse one through `seatAddress` before writing, and a
  boot skips such a row, but an app writing the private collection directly can store one. That
  is why BR-21 reads only the first `~` segment.
...
- **Task-board words in Layer 1.** Engine's `runAction` gate reads `gatedBy.boardId`; Core's
  ...
  of leak Jake named. A separate `audit-coherence` pass, not a PR in this DAG.
- **[FIX-1568](https://linear.app/fixpoint-labs/issue/FIX-1568) retargets after PR-A.** It fences
  single-resource writes and `/state` raw-map reads against another user's private roster row,
  and was written against Engine's hire-plane module and the `workforce/roster/~` shape. Once
  PR-A lands, it uses S3's generic owner-private predicate and the `~` owner segment. It does not
  re-hardcode Workforce's paths or move policy into Core.
