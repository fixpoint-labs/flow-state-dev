...
| BR-14 | The activator runs with no evaluator passed | Slash and keyword resolve as today, and no evaluator code loads: nothing is imported, constructed or model-resolved on the evaluator path, not merely no evaluator row in the trace (ER-5, ER-15 (d)) | V3 · leg (d) |
