...
| S5 | `goals/cascading-router/fails-closed-on-a-real-evaluation-model/` | The goal check VG. It proves this issue, on the shape of epic leg (c); the epic's lead measure reads only FIX-1556's assembled goal, which runs leg (c) itself (ER-15). Both goals stay | ER-15 (c) |
...
| VG | S5 | **Goal, real models** (ER-13), this issue's own proof; the epic's lead measure reads FIX-1556's assembled goal, not this one: `pnpm tsx goals/cascading-router/fails-closed-on-a-real-evaluation-model/run.mts`. The SPEC's two-level tree on held-out tickets: with Jev via the gateway, a clear billing-urgent ticket lands on the gated leaf and each walked level's answer carries `confidence`; with `openai.evaluationModel("gpt-5.4-mini")`, every ticket lands on `ambiguous` with `no-confidence`. **Anti-game:** assert the route and the verdict reasons, never Jev's exact numbers. **Control:** `GOAL_CONTROL=open-on-missing` swaps in a gate that opens on missing confidence and must fail the OpenAI leg |
