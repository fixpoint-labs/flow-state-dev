...
| S12 | `react` · the open-leaf detail slot ([exception](DECISIONS.md#d1-leafdetail)) | **Added after merge.** One new optional slot, `leafDetail` (working name), for content inside an open leaf. It draws on its own line directly under the open leaf's row, inside the rail, indented at the leaf's column, and only while the leaf is open. The row stays one line. Kitchen-sink's seat pane moves into it from `leafToolbar` ([FIX-1500 E4](../FIX-1500/DECISIONS.md#e4)) | — |
...
| Slot | `leafDetail` (working name) | [The exception](DECISIONS.md#d1-leafdetail). Published, so the name is settled in the PR that ships it, and both specs follow |
...
| No new prop, slot, class name or stylesheet; one new custom property, `--fsd-nav-guide`. **One exception, owner-approved after merge: the `leafDetail` slot** ([S12](#surfaces), [the exception](DECISIONS.md#d1-leafdetail)) | Theming already runs through `--fsd-nav-*`, and a host has to be able to colour the lines or set them `transparent` |
