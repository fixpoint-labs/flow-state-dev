/**
 * FIX-1561 POC SKETCH — a copy of `FlowNavigator.ts` with the proposed layout
 * applied, so the goal check can be seen going green and the two reveal
 * options can be screenshotted. Not production code, not the implementation,
 * not an API promise. Changes against the real file, all marked `SKETCH`:
 *
 * - one label column per level: every row reserves the twisty's width, and a
 *   level steps right by exactly that width (16px);
 * - notes start on the label column of the level they describe;
 * - `leafToolbar` renders on the leaf's own row, portalled into the row's
...
import { useFlowContext } from "../../../../../../packages/react/src/context/FlowContext";
import {
  groupFlowsIntoSections,
  type FlowNavigatorKindGroup,
  type FlowNavigatorLeaf,
  type FlowNavigatorSection
} from "../../../../../../packages/react/src/components/flow-navigator/grouping";
import {
  useFlowInventory,
  useLeafSessions,
  type FlowNavigatorFlowSource,
  type FlowNavigatorSessionSource
} from "../../../../../../packages/react/src/components/flow-navigator/reads";
import { arrangeSessionRows, type SessionRow } from "../../../../../../packages/react/src/components/flow-navigator/dispatch-runs";
...
// SKETCH: the step equals the twisty column, so a child's twisty sits under its
// parent's label and every level's labels share one x.
const TWISTY = 10;
const GAP = 6;
const INDENT_STEP = TWISTY + GAP;
const GUTTER = TWISTY + GAP;

let reveal: "always" | "hover" = "always";
/** SKETCH: pick the reveal variant for the screenshots. */
export function setSketchReveal(next: "always" | "hover"): void {
  reveal = next;
}
...
function Row(props: {
  ...
  /** SKETCH: where an open leaf's actions are portalled to. */
  readonly actionsRef?: (el: HTMLElement | null) => void;
}): ReactNode {
  const { depth, isSelected = false, button, content, trailing, actionsRef } = props;
  ...
  return createElement(
    ...
    hasTrailing || actionsRef !== undefined
      ? createElement(
          "span",
          // Opacity, never display/visibility: a hidden action stays in the tab
          // order, and focusing it is what reveals it.
          { style: { ...trailingStyle, opacity: shown ? 1 : 0 } },
          hasTrailing ? trailing : null,
          actionsRef === undefined
            ? null
            : createElement("span", { ref: actionsRef, style: { display: "contents" } })
        )
      : null
  );
}
...
function noteStyle(depth: number): Record<string, unknown> {
  return {
    margin: 0,
    // SKETCH: on the label column of the level the note describes.
    padding: `4px 8px 4px ${8 + depth * INDENT_STEP + GUTTER}px`,
...
function twisty(isOpen: boolean | null): ReactNode {
  // SKETCH: `null` is a leaf row's empty twisty column, so its label lines up.
  return createElement(
    "span",
...
function LeafSessionList(props: {
  ...
  readonly onSelectSession: (sessionId: string, leaf: FlowNavigatorLeaf) => void;
  readonly slots: FlowNavigatorSlots;
  /** SKETCH: the leaf row's trailing area, where the toolbar is portalled. */
  ...
  return createElement(
    ...
    toolbar === undefined || toolbar === null || actionsEl === null
      ? null
      : createPortal(toolbar, actionsEl),
    rows
  );
}

/** SKETCH: one instance row and its leaf, holding the row's portal target. */
function InstanceNode(props: {
  readonly kind: string;
  readonly instance: FlowListEntry;
  readonly open: ReadonlySet<string>;
  readonly toggle: (key: string) => void;
  readonly leafProps: Omit<Parameters<typeof LeafSessionList>[0], "leaf" | "depth" | "actionsEl">;
}): ReactNode {
  const { kind, instance, open, toggle, leafProps } = props;
  const [actionsEl, setActionsEl] = useState<HTMLElement | null>(null);
  const key = `instance:${instance.id}`;
  const instanceOpen = open.has(key);
  const leaf: FlowNavigatorLeaf = { kind, address: instance.id, cardinality: "collection" };
  ...
  return createElement(
    ...
    createElement(Row, {
      ...
      content: [twisty(instanceOpen), createElement("span", { style: label }, instance.id)],
      trailing: leafProps.slots.rowTrailing?.({
        type: "instance",
        kind,
        instance,
        isOpen: instanceOpen
    ...
    instanceOpen
      ? createElement(LeafSessionList, { leaf, depth: 2, actionsEl, ...leafProps })
      : null
  );
}
...
function KindRow(props: {
  ...
  // SKETCH: a singleton's kind row IS its leaf row, so its actions land here.
  const [kindActionsEl, setKindActionsEl] = useState<HTMLElement | null>(null);
  ...
  return createElement(
    ...
    createElement(Row, {
      depth: 0,
      ...(group.leaf !== null ? { actionsRef: setKindActionsEl } : {}),
      button: {
...
export type { FlowNavigatorLeaf, FlowNavigatorSection } from "../../../../../../packages/react/src/components/flow-navigator/grouping";
