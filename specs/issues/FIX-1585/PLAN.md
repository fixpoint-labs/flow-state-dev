...
| S7 | Docs and release note | [DOCS.md](DOCS.md)'s operations; one `minor` changeset for `@flow-state-dev/workforce` (`read` now returns the recent lines, and new posts no longer land in `state.transcript`). kitchen-sink is private: no changeset | — |
