...
|---|---|
| **The real need** | The epic's goal, [leg a](../../epics/FIX-1592/SPEC.md#the-goal-and-how-well-know-its-met): *"talking to a seat is a conversation with that agent"*. This issue proves it on `support.otto`; [FIX-1589](https://linear.app/fixpoint-labs/issue/FIX-1589) proves it on the clerk. The owner, 2026-09-25: *"A seat is a direct conversation. I should be able to talk to a seat using the normal agent flow that comes with workforce."* The desk half is not an epic leg by itself: it is the post from the page, which the epic's legs b and c are read from ([ER-5](../../epics/FIX-1592/BUSINESS-RULES.md#what-a-person-gets)) |
...
| How we verify | |
|---|---|
| **Goal check** | `goals/kitchen-sink-talk/keeps-both-sides-across-a-reload/`. Its otto leg is the epic's leg a for `support.otto`, and it runs again with the other three checks on one `main` commit at the epic's wrap ([ER-17](../../epics/FIX-1592/BUSINESS-RULES.md#the-proof)). It drives a real browser against kitchen-sink's production build. Its legs are V6 and V7 in [PLAN.md](PLAN.md). Run by the implementer at completion, locally first; handed to `fsd-qa` over the mailbox only if no browser can run here. Verdict in the implementation PR |
