...
Three places change: the kitchen-sink README (the epic's shared text, the parts this issue
owns), the channels guide (one paragraph), and two file headers. The epic's
...
>   payload: (input, ctx) => ({ ...input, author: ctx.flow.id }),
> });
> ```
>
> The model picks the board and writes the goal; the `author` comes from the flow, not from
> the model. The tool's result is the dispatch, not the row: the row is written when the
