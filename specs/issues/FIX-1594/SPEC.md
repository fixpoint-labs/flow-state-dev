...
Feature · `workforce` + kitchen-sink · medium · 1 PR · epic [FIX-1592](https://linear.app/fixpoint-labs/issue/FIX-1592), leg c · after [FIX-1590](https://linear.app/fixpoint-labs/issue/FIX-1590)
...
| **Control that must fail** | `GOAL_CONTROL=no-author-filter` drops the epic's D2 filter: otto's line wakes iris, so the "woken once" half fails. `GOAL_CONTROL=post-without-author` makes the tool send no author: the line reads `devuser`, so the "under its name" half fails. Today's `main` fails both: no seat can post |
...
flowchart LR
  ...
  H["hire · the seat's own id"] -->|"author"| T
...
The author enters at the hire, not at the model. The channel's own `post` is the only place a
line is written and the only place membership is checked.
...
- The channel panel. A seat's line is labelled by its author, like every line, and shows on the
  next read, not live.
- `desk-clerk` and `followup-runner` seats don't post back.

## Sign off

**[The goal](#the-goal-and-how-well-know-its-met), at that size:** otto's reply is a line in the
channel under its own name, survives a reload, and wakes nobody. If wrong: the page still can't
show Jake's last sentence, or we'd call a seat-only answer done.
...
1. **[D1](DECISIONS.md#d1) · An agent replies in a channel by choosing to, with a
   ...
   prompt.
2. **[D2](DECISIONS.md#d2) · The name on the line is the seat's own id, written into the seat's
   settings by the hire.** The model never supplies it. If wrong: a hand-written worker kind not
   built from the contract refuses at boot until it adds one key.
3. **[D3](DECISIONS.md#d3) · The channel's own `post` is the only gate.** The tool reports that
   it handed the post over, not that it landed. If wrong: an agent that names a channel it isn't
   in believes it posted, and nothing appears.
