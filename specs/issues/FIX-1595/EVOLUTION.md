...
| `items.history()` always appends the in-flight request's items, whatever the limit (the retry-after-mid-turn-failure guarantee in `loadLLMHistory`, `packages/engine/src/context/history.ts`) | **Retained** as the default; **extended** with an opt-in to leave them out | The evaluator needs prior turns only (BR-7), and public views couldn't express it ([D3](DECISIONS.md#d3)) | S0, V0 | Optional field; omitted is today's behaviour |
