...
| # | When | Then | Proved by |
|---|---|---|---|
| QR-1 | Any of the five children ([FIX-1602](https://linear.app/fixpoint-labs/issue/FIX-1602) included), [FIX-1598](https://linear.app/fixpoint-labs/issue/FIX-1598) or the epic amendments [#2289](https://github.com/fixpoint-labs/flow-state-dev/pull/2289) and [#2294](https://github.com/fixpoint-labs/flow-state-dev/pull/2294) are not merged, or CI is red on `main` | No run. This issue is blocked by each child and by FIX-1598 | Linear · the report lists each merge commit |
...
| QR-11 | Part 3 runs | All five children's goal checks, FIX-1602's included, pass with their held-outs, each control fails its own leg, and the epic's ER-18 set stays green | P3.1 to P3.3 |
...
| Leg b passes on kitchen-sink's own wake routing, not Workforce's stock seat-wake routing | P4, stock routing |
