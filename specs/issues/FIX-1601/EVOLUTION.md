...
| Part 4's stock-routing check failed any `notifyFor` in `apps/kitchen-sink`. Source: this spec's [PLAN → part 4](PLAN.md), stock-routing row | **Amended** by FIX-1604 | The closure run on `55de0734d` found kitchen-sink's `notifyFor(seats)` only returns Workforce's `wakeMemberSeats(seats, { fallback: notifyMember })` in the test-mode controls, a name FIX-1602's PLAN S3 kept on purpose. No loop over seats, no dispatcher or router in the app: the routing is right, the wording was stale | The row now fails an app-built dispatcher, router or seat loop, and allows a thin wrapper of `wakeMemberSeats` | The check still reads the commit's source; what it proves is unchanged |
