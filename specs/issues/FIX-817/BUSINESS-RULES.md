...
| BR-8a | A collection is projected and **is** `llmReadable` | Present in the resources manifest. `collectReadableResources` reaches only static resources and `collectCollections`, and `collectCollections` drops projected refs (`isProjectedRef`) — so a manifest built on that helper alone would report an in-scope resource as absent. A manifest that lies by omission is worse than none; the projected collections are reached too (`collectProjectedCollections`) | CI · assert a projected `llmReadable` collection appears |
